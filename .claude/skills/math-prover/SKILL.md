...
**記述スタイルの正本**: `docs/context/証明の記述スタイルと四層の検証.md` を必ず読み、従うこと。
このファイルは Typst 時代の書式例を残しているが、**証明の正本形式は構造化テキスト
（`structured-latex/`）である**（新規に Typst で書かない）。ルールの正本は `docs/context/証明の記述スタイルと四層の検証.md` とし、
食い違った場合はそちらが勝つ。併せて `docs/context/` は全ファイル読むこと。
