...
- 検証範囲: 一段前像集合 $\operatorname{Pre}_F(z)$ と一段前像数 $d_F(z)$、$z\ne w$ での非交差、完全逆像 $F^{-1}(B_F(\sigma_F(q)))=\bigcup_{z\in B_F(\sigma_F(q))}\operatorname{Pre}_F(z)$ の所属同値の各段、保存式 $|B_F(q)|=\sum_{z\in B_F(\sigma_F(q))}d_F(z)$ の 4 等号、全組走査と有限加算による有限決定
...
| `check_count_conservation.sage` | $\lvert B_F(q)\rvert=\lvert F^{-1}(B_F(\sigma_F(q)))\rvert=\lvert\bigcup_z\operatorname{Pre}_F(z)\rvert=\sum_z\lvert\operatorname{Pre}_F(z)\rvert=\sum_z d_F(z)$ の 4 等号を分けて検査（非交差も再確認） | PASS | 769 写像・安定ファイバー 2,201 個で成立 |
| `check_finite_decidability.sage` | $A^V\times A^V$ の全組 $(y,z)$ で $F(y)=z$ を検査して得た $\operatorname{Pre}_F(z)$・$d_F(z)$ が定義どおりの値に一致し、各 $B_F(\sigma_F(q))$ 上の有限加算が保存式の値に一致すること | PASS | 769 写像・配位番号の比較 21,505 回・加算 3,585 回で成立 |
