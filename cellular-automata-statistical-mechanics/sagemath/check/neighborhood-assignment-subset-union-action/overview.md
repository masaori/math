...
- 併せて検証するラベル:
  `def_finite_carrier_subset_space`、
...
- 基礎集合の元数は `|V| = 0, 1, 2, 3` である。近傍割り当ての総数は順に 1、2、16、512 で、
...
- したがってこれは有限範囲の全数検査であって、任意の有限基礎集合に対する一般証明ではない。
...
- 合成の向きについて補足を得た。本文は `U_{N star M} = U_M ∘ U_N` を主張しており、
  向きを逆にした `U_N ∘ U_M` では一致しない具体例が二元基礎集合に存在する
