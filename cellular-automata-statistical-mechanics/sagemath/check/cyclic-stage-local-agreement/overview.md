# SageMath 検算: 有限巡回群の族・局所収束・有限段階の量の列
...
- 有限剰余上の群演算、整数との有限窓上一致による局所収束、有限局所観測の各段階の元数と
  可算非交和の符号化機構、固定した真理値表が定める有限自己写像の族、反復不動点数列と
  正値域上の素因数指数ベクトル値の列を、本文の段ごとに分けて検算する。
...
| `check_finite_window_agreement.sage` | 閾値以後の有限窓上一致による局所収束と大域単射でない境界 | PASS | 窓元対 14,625 件と局所収束の証人段階 65 件で一致し、周期 1..49 で非単射の証人を確認 |
| `check_finite_observation_stage_count.sage` | 半径ごとの有限局所観測の元数 | PASS | 半径 0..8 の全 174,762 状態表で `2^(2s+1)` と一致 |
| `check_finite_observation_catalogue_prefix.sage` | 可算非交和を自然数へ符号化する機構の有限接頭辞 | PASS | 半径 0..8 の全 174,762 状態表を自然数の初期区間へ重複なく符号化し逆対応を確認 |
...
- 有限局所観測は半径 0..8 の各段階で全状態表を列挙し、元数 `2^(2s+1)` を検査する。
  同じ範囲の各状態表を互いに素な自然数区間へ写し、半径と状態表の組へ戻せることを検査する。
  これは可算非交和の一般証明そのものではなく、その明示的な符号化機構の有限接頭辞検算である。
