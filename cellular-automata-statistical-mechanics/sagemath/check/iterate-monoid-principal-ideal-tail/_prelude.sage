...
def scan_bound(stage_size):
    ...
    人手証明の上界 K = M^M より小さいが、衝突指数が K 以下であることは呼び出し側で別に検査する。"""
    m = 2 ** stage_size
...
def monoid_and_collision(table):
    """P_F の有限代表 (F^0, ..., F^{j-1}) と最初の衝突 (i, j) を返す。F^0..F^{j-1} は相異なる。"""
    scan = scan_bound(len(table).bit_length() - 1)
    powers = power_tables(table, scan)
...
def tail_by_definition(powers, n, last_exponent):
    """I_n(F) = {F^{n+k} | k in N} を、指数 n..last_exponent の範囲で列挙した frozenset。
    衝突 j 以上まで列挙すれば有限代表の全てを尽くす（呼び出し側が last_exponent >= n + j を保証する）。"""
    return frozenset(powers[e] for e in range(n, last_exponent + 1))
