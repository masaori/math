...
有限差分逆温度、有限状態数比、有限離散対称性、有限置換時間発展、有限 Yang--Baxter 条件、
有限組合せ散乱データについて、可算側と実数・複素数側の境界は四層と具体版への導出まで完了した。
結晶化比較は、$q=0$ で正則な係数の代数的評価から有限基底全単射と整数指数表を抽出する部分写像であり、
解析的極限とは扱わない。同じ抽出結果を持つ異なるスカラー規格化の有限反例により、有限表から
結晶化前の作用素を一意に復元できないことも確定した。

次の対象はまず状態台帳の対象表を上から照合し、そこに未完了対象が無い場合にだけ
「連続物理の可算な担い手」を上から照合して決める。現在の先頭未完了対象は
「有限半順序の被覆関係と被覆近傍割り当てによる生成」の Lean 具体版である。完了済み対象の
記録に層間不一致があれば先に修正し、台帳の旧記述を次の未完了対象と誤認しない。有限 Yang--Baxter
条件だけから非定数な連続族の存在を主張せず、有限組合せ散乱表だけから任意の量子 $R$、複素 Bethe 根、
固有ベクトル、量子転送行列スペクトルを復元できるとも主張しない。既存成果の再検査や対象行の追加だけを
新しい研究成果としない。

「有限半順序の被覆関係と被覆近傍割り当てによる生成」が完了した後、可算な担い手の台帳を
上から照合した次の対象は「可解性」である。Yang--Baxter 条件、
非退化性（古典的双ユニタリ性）、線形性の各有限検査を「可解性一般の判定」と同一視しない。
一次文献は双ユニタリ性が可積分性と独立で、Yang--Baxter 型 CA についても観測量の一般解法は
未確立とする。その対象へ進む tick では、有限集合上の個別証明書の入力・出力・判定手続きと、
それから従わない観測量の厳密解を分けた構造化記述へ進む。
