...
隣接持ち上げ、全 $n^3$ 入力による有限決定、成立例と二元集合上の不成立例の構造化記述まで完了した。
次回 tick は全層レビュー後、複素数値スペクトルパラメータ族の定義を一次文献へ戻って確定し、有限写像との
比較対象・比較写像・保存される構造・失われる情報を記述する。有限 Yang–Baxter 条件だけから連続族の
存在を主張しない。既存成果の再検査や対象行の追加だけを新しい研究成果としない。
