...
同じ有限時間発展が実数値生成子を一意に定めない有限反例を分離する。次回 tick はこの対象の
構造化記述を既存の巡回型・有限対称性へ戻ってレビューした後、整数置換行列の冪、有限位数、
有限位相符号の複素固有対、全固有値の一の冪根性、整数持ち上げによる実数値位相生成子の非一意性を
段別の SageMath 検算へ移す。既存成果の再検査や対象行の追加だけを新しい研究成果としない。
