...
現在の先頭対象は、state が定める「有限語個数の実数実現による規格化境界」である。対数順序群から
実数体への加法的な比較写像、正の有限語個数と正の語長に限った規格化値、全二元語族での有限段階の
一定性は構造化本文、段別の SageMath 検算、有限二元語・有限台素数指数ベクトル・
加法的実数実現・正の語長による除算に固定した Lean 具体版まで済んでいる。
次は全層レビュー後、加法的実数実現の零・加法性と正の自然数による除算に必要な構造だけを残す Lean
必要十分版と具体版への導出を作る。一般語族の極限と位相的エントロピーの存在を先取りしない。これは README の
既存の問い「境界」と現行 roadmap の完了条件を具体化するもので、新しい上位目的の選定ではない。
