...
4. 整理完了後は、README が正本とする `structured-latex/research-roadmap.ts` の現在の段階へ進む。
   既に定まった目的の実行に再開許可を求めない。README・マニフェストの目的を変更する判断は分離する。

## 現在の研究対象

対象は README が正本とする `structured-latex/research-roadmap.ts` の現在地と、
状態台帳の「現在の研究対象」で定める。成果整理は全層完了しており、既存成果の再検査だけを繰り返さない。
有限舞台の数え上げから有限台有理素数ベクトルの完備性境界までを終え、現在地は
「可算側と実数側の境界の確定」である。既存の目的の実行に再開許可を求めない。

「熱力学極限の存在」について有限段階の不等式列だけで閉じる範囲から、有限有理確率分布の
エントロピーと実数境界までの対象は四層と具体版への導出まで完了した。2026-09-16 の完了条件監査では、
「連続物理の可算な担い手」の逆温度以後に未記録境界が残ると分かった。先頭の「有限差分逆温度と
実数微分の境界」は、構造化本文と四つの段別 SageMath 検算まで完了している。有限差分の実評価だけでは、
同じ二端点を通る異なる実数補間の一点微分を決められない。実対数、実数補間、微分、極限は
実数への脱出として分離し、状態数零への対数や任意の連続補間の分類は扱わない。

有限差分逆温度と実数微分の境界、および有限状態数比と実数指数カノニカル分布の境界は、
四層と具体版への導出まで完了した。ただしその後の完了監査は、可算な担い手の台帳が
「まだ可算な担い手が見つかっていない」と明記する対象を見落としていた。

`research-roadmap.ts` の現在地は引き続き「可算側と実数側の境界の確定」である。
「有限離散対称性と連続対称性の境界」は、構造化記述、四つの段別 SageMath 検算、Lean 具体版、
有限型上の可換置換群、加法群値保存写像、整除可能な加法群から有限群への作用だけを残した Lean
必要十分版と具体版への導出まで完了した。整数値保存写像の既存定義だけから加法的保存量全体の
格子や連続対称性との対応を導いたとは扱わない。全層レビューと台帳照合の結果、次の先頭未完了対象は
「有限置換時間発展と複素位相実現の境界」である。有限可逆大域写像から内在的に得る置換行列、巡回型、
有限位数と可算な位相符号を先に確定し、その後で複素固有値による実現と実数値生成子を比較する。
ヒルベルト空間を目標仕様として先取りせず、複素指数写像、時間尺度、対数の持ち上げを選ぶ脱出点と、
同じ有限時間発展が実数値生成子を一意に定めない有限反例を分離する。構造化記述と、整数置換行列の冪、
有限位数、有限位相符号の複素固有対、全固有値の一の冪根性、整数持ち上げによる実数値位相生成子の
非一意性を分けた五つの SageMath 検算まで完了した。Lean 具体版では、有限舞台・二元状態上の
整数置換行列の冪、有限位数、複素係数作用、全固有値の一の冪根性、有限位相値の一の冪根性、
整数持ち上げの非一意性までを形式化した。各周期軌道の有限位相符号から軌道に支えられた
非零固有ベクトルを構成し、固有対等式を人手証明と同じ順序で Lean 具体版へ追加した。
有限添字型上の相異なる点列、後退写像、係数の共変関係、有限和だけを残した Lean 必要十分版と
具体版への導出も完了し、この対象は四層が揃った。次回 tick は全層レビュー後、可算な担い手の
台帳を上から照合して先頭の未完了境界を確定する。既存成果の再検査や対象行の追加だけを新しい
研究成果としない。

完了した量の構成の制約も維持する。状態数が零なら対数を定義せず、隣接差は両端が正の場合だけ、
非単位刻みの除算は全素数係数が刻みで整除される場合だけとする。有限校正は指定範囲の全数結果であり、
任意の保存写像・セル数・回数や連続理論との対応を得たとは扱わない。
...
4. 成果整理の未解消の欠陥があれば先に直す。無ければ「現在の研究対象」の先頭の未完了対象を
   一層進める。対象追加は現在の段取りの完了条件に必要なものに限り、既存対象への依存を記録する。
