...
「熱力学極限の存在」について有限段階の不等式列だけで閉じる範囲と、その次の「有限べき比較と
スケーリング指数の境界」は四層と具体版への導出まで完了した。現在の先頭対象は、state が定める
「有限語個数と位相的エントロピーの境界」である。全二元有限語族と長さ K+1 の連続した一を禁じる
有限語族について、長さ K までの個数表の一致と次の長さでの分岐は、構造化本文と三つの SageMath
検算まで済んでいる。次は全層レビュー後、有限二元語、禁制語族、個数表の一致と分岐を人手証明と
同じ順序で Lean 具体版へ移す。無限舞台の全配位、実対数、極限、位相的エントロピー、実数体、
複素数体を先取りしない。これは README の既存の問い「境界」と現行 roadmap の完了条件を
具体化するもので、新しい上位目的の選定ではない。
