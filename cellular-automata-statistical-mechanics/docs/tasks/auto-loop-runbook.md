...
README の問い「CA だけから実数を経由せず熱力学を構成できるか」と、段取り正本の
「有限舞台の対数順序群値の熱力学の構成」が今回の対象を定める。新しい上位ゴールは設けない。
成果整理は全層完了しているので、未完の研究へ接続する。既存成果の再検査だけを繰り返さない。

最初に、既存の反復の不動点集合とその自然数の個数を読み、数え上げが零となる入力と、
正の個数に対数順序群の値を対応させられる入力を区別する。保存量による分割を用いるなら、
物理的意味を先に入れず、有限集合上の写像と繊維として定義する。未定義の対数や差分の除算を
式の見た目で済ませず、量の所属・定義域と必要な演算が正本で定義されていることを確かめる。
周期点数だけで熱力学全体の完成を主張しない。

完了条件は段取り正本の三条件を使う。状態数・エントロピー・逆温度・自由エントロピーの
量が有限・可算側で定義され、実数・複素数への脱出の有無を追え、初等規則の明示した有限範囲で
厳密計算と本文が対応すること。命題の昇格には下記の四層を要する。
次の対象は状態台帳の「現在の研究対象」へ記録し、各行に定義域・成立条件・否定結果を残す。
