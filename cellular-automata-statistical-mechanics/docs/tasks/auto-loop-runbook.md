...
先頭の未完了対象は「有限離散対称性と連続対称性の境界」である。整数値保存写像の既存定義だけから
加法的保存量全体の格子や連続対称性との対応を導いたとは扱わない。構造化記述と四つの段別
SageMath 検算、有限舞台・二元状態・整数値保存写像・実数加法群に固定した Lean 具体版まで完了している。
次回 tick は全層レビュー後、有限群、整数値関数の加法群、整除可能な加法群だけを残した Lean
必要十分版と具体版への導出を作る。
