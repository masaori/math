...
有限舞台の数え上げ・正値域・対数順序群値の量・初等規則による有限校正を終え、
現在地は「一次元・任意半径の一様規則」である。既存の目的の実行に再開許可を求めない。

次は、整数オフセット集合から有限巡回舞台への写像を明示し、複数のオフセットが
同じセルへ戻る場合と相異なる場合を分離する。入力の真理値表の個数、実現可能な近傍入力、
大域写像の個数を混同しない。半径一の既存校正へ戻る比較写像と適用範囲を示す。
