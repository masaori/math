...
「熱力学極限の存在」について有限段階の不等式列だけで閉じる範囲は四層を完了した。現在の先頭対象は、
その次に state が定める「有限べき比較とスケーリング指数の境界」である。正整数値有限表の
交差冪上界証明書・有限決定・同じ有限表を持つ二列が次段階で分かれる反例は、構造化本文・二つの
SageMath 検算・Lean 具体版まで済んでいる。次は全層レビュー後、有限な添字型、正整数値、自然数の
冪と順序だけを残した Lean 必要十分版と具体版への導出を作る。指数候補は正整数の順序対のまま扱い、
商、根、対数、漸近的な指数、スケーリング極限、実数値の臨界指数を先取りしない。これは README の
