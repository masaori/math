...
有限舞台の数え上げ・正値域・対数順序群値の量・初等規則による有限校正と、
一次元・任意半径の周期境界における規則表制限を終え、現在地は「格子と可算群の舞台」である。
既存の目的の実行に再開許可を求めない。

次は、有限巡回群の族、整数への比較写像、有限段階で定義される量の列を本文で定義する。
局所一致だけで運べる主張と、無限舞台の全配位または量の極限を要する主張を分離する。
数学的な近似を述べる前に比較写像と収束概念を明示し、誤差または完全な局所一致を与える。
先頭の対象を一層ずつ進め、段取りの完了条件と台帳へ記録する。
