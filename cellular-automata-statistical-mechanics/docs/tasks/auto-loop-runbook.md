...
具体版への導出も完了し、この対象は四層が揃った。可算な担い手の台帳を上から照合した次の対象は
「有限 Yang–Baxter 条件と連続スペクトルパラメータの境界」である。有限集合上の二体写像、三体直積上の
隣接持ち上げ、全 $n^3$ 入力による有限決定、成立例と二元集合上の不成立例に加え、一次文献で異なる二つの
Yang–Baxter 規約を確定した。有限 braid 解は複素線形化と成分交換により定数な複素パラメータ族へ
情報を失わずに埋め込める。一方、一元集合上でも同じ一点の有限解へ制限される異なる複素族があるため、
スペクトル依存性は一つの有限写像から復元できない。この比較の構造化記述まで完了した。
次回 tick は全層レビュー後、有限条件・比較写像・非一意性反例を段別の SageMath 検算へ接続する。
有限 Yang–Baxter 条件だけから非定数な連続族の存在を主張しない。既存成果の再検査や対象行の追加だけを
新しい研究成果としない。
