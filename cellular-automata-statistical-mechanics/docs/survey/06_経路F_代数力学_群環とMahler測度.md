...
### 1.4 Garden of Eden と従順性（舞台グラフが効く場所）
...
で扱われている。**GOE 定理の成否は「舞台グラフ ↔ 統計力学の振る舞い」の対応の既存例**である（→ 種 S8）。

## 2. Martin–Odlyzko–Wolfram（1984）— 有限環上の代数的解析
...
同じ形の主張である。両者を並べる作業が空白（→ 種 S2）。

## 3. 代数力学：エントロピー ＝ Mahler 測度
...
**帰属**: $m(P)$ は一般に超越的（型 E1+E3）。

### 3.3 $p$ 進エントロピー（$\Lambda$ 側）
...
CA はこの構造の最も具体的な供給源だが、**CA の文献と $p$ 進エントロピーの文献は交わっていない**（→ 種 S2）。

## 4. 線形 CA のエントロピーは $\Lambda$ に住む（$\mathbb{R}$ 脱出なし）
