# 02. 経路 B — 可積分 CA（結晶基底と Yang–Baxter 写像）
...
**これは「ultradiscretization ＝ 可算側への射影」という見方を強く示唆する**（→ 種 S3）。
...
## 2. Yang–Baxter 写像（set-theoretical YBE）
...
- 非退化な YB 写像は「双ユニタリゲート」の古典版になる（→ 経路 C）。
...
**可積分性という概念が、CA の側では決定可能な有限条件に化ける**（→ 種 S5）。

## 3. 3D consistency と quad-graph
...
1. **2 値（$|X|=2$）に限ったときの YB 写像の完全分類が、CA の言葉で書かれていない。**
   ...
   「初等 CA 256 個のうちどれが YB 写像に由来するか」という形の表はない（→ 種 S5）。
2. **ultradiscretization を「可算化」として定式化した文献がない。**
   トロピカル幾何としての整理（Inoue–Kuniba–Takagi）はあるが、
   「$\mathbb{R}$ 脱出を除去する操作」としての位置づけは無い（→ 種 S3）。
