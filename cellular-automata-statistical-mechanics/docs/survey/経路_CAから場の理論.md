...
（→ 種「可逆CAの量子論は可算」）。これは `docs/context/物理数学を可算側で特徴づける課題表.md` の課題「Hamiltonian $=\frac{i}{\Delta t}\log U$ の可算化」への具体的な回答になる。
...
つまり **「CAから場の理論」の経路の核心部分（有限格子）は完全に代数的・可算**（「見かけの脱出」型）。

$\mathbb{R}$ に出るのは (i) 連続極限、(ii) 重みを $\exp$ で書く段（型「指数評価」/「極限・積分」）。
...
Dirac 方程式が出るのは分散関係の**小波数展開**＝型「極限・積分」の脱出。
