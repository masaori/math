...
Wetterich の三文献を本文まで確認すると、有限側で直接与えられているのは次の範囲である。

- 一時刻の有限個の二値変数には有限個の配位があり、Grassmann 単項式の有限基底を配位で添字づける。
- 離散時刻間の局所 Grassmann 因子を有限基底で展開し、その係数を step evolution 行列として抽出する。
  明示模型の局所ブロックでは、この行列は有限次の行列として書かれる。
- Grassmann 生成子の交換で生じる符号は整数の単元 $\{+1,-1\}$ に属する。明示模型の一部では、
  off-diagonal 成分の符号を Grassmann 変数の符号変更で吸収できるため、符号規約と更新写像を
  同一視してはならない。
- 連続 Lorentz 対称性、連続時間発展、Dirac/Thirring 型の記述は naive continuum limit の側にあり、
  有限基底の係数抽出だけからは従わない。

従って次に構造化する対象は「フェルミオン」という物理的意味ではなく、有限全順序添字集合、
整数係数、反交換生成子の有限単項式、最高次係数抽出、基底置換の符号である。これらを定義した後にだけ、
有限 step evolution 行列との比較写像を置く。Pfaffian 表現、Rule 150、連続場の方程式は別対象であり、
この有限代数から先取りしない。
