# 05. 経路 E — CA から場の理論（Grassmann・Dirac・量子形式）
...
（→ 種 S7）。これは `docs/context/03` の課題 2.3 の具体的な回答になる。

**注意**: 本プロジェクトは 't Hooft 解釈の物理的・哲学的な当否には立ち入らない
（README §1.4）。使うのは「決定論的 CA → 置換行列 → 可算スペクトル」という数学的な部分だけ。

## 2. Wetterich — 確率的 CA ＝ フェルミオン QFT
...
つまり **経路 E の核心部分（有限格子）は完全に代数的・可算**（型 E0 の見かけの脱出）。

$\mathbb{R}$ に出るのは (i) 連続極限、(ii) 重みを $\exp$ で書く段（型 E2/E3）。
Wetterich 自身は連続極限を目標にしているので、可算側で閉じる範囲は明示されていない。
**この切り分けは本プロジェクトが埋められる空白**。

## 3. 量子 CA から Dirac 方程式
...
Dirac 方程式が出るのは分散関係の**小波数展開**＝型 E3 の脱出。

## 4. Reggeon 場の理論（古典的な橋）

有向浸透（DP）普遍類 ↔ Reggeon 場の理論（非ユニタリ QFT）の対応（Cardy–Sugar 1980）。
Domany–Kinzel CA は DP 普遍類の標準的な CA 実現（→ 経路 A §3）。
...
2. **可逆 CA の置換行列としてのスペクトル（1 の冪根）が、
   ...
   $\operatorname{Tr}U^N$ は周期点の数え上げであり、`R-Lambda-duality` の枠組みが直接使える（→ 種 S7）。
3. **場の理論側の「作用素代数」と CA の規則の対応が、経路 D（QCA 指数）と分断されている。**
