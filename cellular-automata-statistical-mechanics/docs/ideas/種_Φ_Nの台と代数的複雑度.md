...
検証すべきこと:
  - 加法的規則（0, 60, 90, 102, 150, 170, 204, 240 …）で台 ⊆ {2} が成り立つか（命題 1）。
  - 可逆規則（second-order CA / ERCA）で M = 0 が成り立つか（命題 2）。
...
| 既知 | Martin–Odlyzko–Wolfram が線形の場合を尽くしている | **命題 1・2 は既知に近い。新規性は仮説 種「Φ_N の台と代数的複雑度」の側にしかない** |
