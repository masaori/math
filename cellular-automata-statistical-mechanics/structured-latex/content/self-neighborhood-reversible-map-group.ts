...

  {
    ...
    proof: [
      ...
      paragraph([
        ...
        ref("claim_self_neighborhood_injective_iff_pointwise_bijective"), " により各セルの値写像 ",
        math(String.raw`g_v:A\to A`), " は全単射である。2 元集合 ", math(String.raw`A`),
        " 上の全単射は ", math(String.raw`\mathrm{id}_A`), " または ", math(String.raw`\nu`),
        " のどちらかである（", ref("claim_self_neighborhood_involution"), " の証明）。そこで",
      ]),
      displayMath(String.raw`S_F:=\{v\in V:g_v=\nu\}`),
      paragraph([
        "と置く。各 ", math(String.raw`x\in A^V`), " と ", math(String.raw`v\in V`), " について、",
