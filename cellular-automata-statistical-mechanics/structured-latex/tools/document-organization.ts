...
export const documentOrganization = [
  {
    ...
    sections: [
      ...
      },
      {
        ...
        output: "有限段階の規格化値を定義できる正確な定義域と実数脱出点を得て、全二元語族では極限を使わず一定値になる一方、加法性だけでは異なる有限個数を識別しない境界を得る。",
        main: "主張は、全二元語族の対数順序群値を加法的な実数実現へ送り、正の語長で割った値が全有限段階で同じになる一方、零実数実現が相異なる有限個数を同じ値へ送るため、一般語族の比較には比較写像の具体化または追加の識別条件が要ることである。",
        mainLabels: [
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        output: "有限巡回群の族、有限窓での完全一致による局所収束、可算な有限局所観測と有限台配位、非可算な全配位の境界、各有限段階の大域写像・不動点数・対数順序群値の列と、量の極限を未定義のまま持ち込まない境界を得る。さらに、群構造を持たない一般の舞台では近傍の一様な輸送と平行移動不変性が失われる有限反例を得る。",
        main: "主張は、各有限剰余舞台が明示した演算で巡回群をなし、半径sの有限窓では周期が2s+1以上なら整数の等号関係と完全一致して局所収束する一方、有限局所観測と有限台配位は高々可算だが全配位は対角線反証により非可算であり、大域的な埋め込み・全配位の一致・量の収束を局所一致から含意しないことである。また、一般の舞台では近傍の元数がセルごとに異なりうるため、共通の局所真理値表を運ぶ近傍の全単射も、セルの入れ替えに対する不変性も舞台からは導かれない。",
        mainLabels: [
          ...
          "claim_integer_stage_finite_support_configurations_countable",
