  ...
  },
  {
    ...
    evidence: [
      ...
      },
      {
        ...
        why: "有限舞台の量と初等規則の校正を終え、次の対象を任意半径の局所表と周期境界の重複へ定めた。実現可能入力と制限の等号判定、異なる大域写像と実現繊維の元数、半径一への比較を記述し、SageMathの有限範囲の各段検算を完了した。Lean具体版は両立入力・引き戻し・入力実現・大域接着、整数オフセットの巡回射影、衝突と差の周期整除の同値、像の元数 min(L,2r+1)、オフセット数 2r+1 が周期 L 以下となる単射境界、異なる大域写像と各実現繊維の個数式まで進んだ。初等規則番号と半径一比較が残る。",
      },
      {
        kind: "path",
