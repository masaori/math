  ...
  },
  {
    ...
    evidence: [
      ...
      },
      {
        ...
        why: "有限舞台の量と初等規則の校正を終え、次の対象を任意半径の局所表と周期境界の重複へ定めた。実現可能入力と制限の等号判定、異なる大域写像と実現繊維の元数、半径一への比較を記述し、SageMathの有限範囲の各段検算とLean具体版を完了した。Lean必要十分版では両立入力の実現と局所・全セルの等号判定から有限性・二値性を除き、有限性を一般の入出力集合の両立入力数と表数へ分離した。大域写像数・実現繊維数の一般式と全具体版への導出が残る。",
      },
      {
        kind: "path",
