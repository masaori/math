// 自動生成ファイル — 直接編集しない。
// 生成元: content/ の全ブロックの labels（と、翻訳ロケールにしか無いブロックの labels）
// 再生成: node <system>/codegen/structured-text-index/cli.ts --project <このディレクトリ>
//
// このユニオン型が「実在するラベル」の全体であり、ref() / ノートの targets は
// これ以外を受け付けない。存在しないラベルへの参照はコンパイル時に落ちる。

export const ALL_LABELS = [
  "claim_additive_realization_need_not_distinguish_counts",
  "claim_all_finite_self_maps_finite",
  "claim_all_self_maps_binary_ca_full_neighborhood_specialization",
  "claim_all_self_transpose_assignments_composition_closed_iff_subsingleton",
  "claim_antichain_order_convex",
  "claim_bare_stage_loses_uniform_transport",
  "claim_bijective_self_map_all_elements_periodic",
  "claim_bijective_self_map_orbits_partition_carrier",
  "claim_binary_bijection_is_identity_or_negation",
  "claim_binary_ca_commuting_configuration_symmetries_finite_group",
  "claim_binary_ca_fiber_count_partition",
  "claim_binary_ca_finite_difference_does_not_determine_derivative",
  "claim_binary_ca_fixed_point_count_bound",
  "claim_binary_ca_integer_conserved_observables_additive_group",
  "claim_binary_ca_logarithmic_free_count_bound",
  "claim_binary_ca_logarithmic_free_count_fibers",
  "claim_binary_ca_logarithmic_gap_division_obstruction",
  "claim_binary_ca_permutation_complex_eigenvalue_root_of_unity",
  "claim_binary_ca_permutation_matrix_powers_encode_iterates",
  "claim_binary_ca_phase_code_realizes_complex_eigenpair",
  "claim_binary_ca_positive_count_domain_nonempty",
  "claim_binary_ca_real_parameter_symmetry_action_trivial",
  "claim_binary_ca_real_phase_generator_not_unique",
  "claim_binary_ca_reversible_cycle_type_realizes_every_partition",
  "claim_binary_ca_reversible_global_finite_order_identity",
  "claim_binary_ca_symmetry_pullback_preserves_conserved_observables",
  "claim_binary_ca_unit_difference_ratio",
  "claim_binary_ca_unit_difference_real_evaluation",
  "claim_binary_field_linear_global_map_additive",
  "claim_binary_field_linear_global_map_preserves_scalar_multiplication",
  "claim_binary_field_linear_global_map_preserves_zero",
  "claim_binary_field_linear_membership_finite_decidable",
  "claim_binary_finite_bath_count_distribution_is_marginal",
  "claim_binary_finite_bath_count_not_always_exponential_canonical",
  "claim_binary_finite_positive_bath_count_exponential_comparison",
  "claim_binary_finite_total_shell_cardinality",
  "claim_binary_state_addition_cancellation",
  "claim_block_partition_blocks_are_membership_fibers",
  "claim_block_phase_characterization",
  "claim_block_phase_membership_finite_decidable",
  "claim_carrier_cardinality_partitions_finite",
  "claim_classical_nondegeneracy_does_not_imply_yang_baxter",
  "claim_collision_finite_decidability",
  "claim_collision_shift",
  "claim_composed_neighborhood_associative",
  "claim_composed_neighborhood_distributes_over_pointwise_union",
  "claim_composed_neighborhood_monotone",
  "claim_composite_map_support_bound_can_be_strict",
  "claim_composite_map_support_bounded_by_composed_support",
  "claim_composition_idempotent_neighborhood_assignment_characterization",
  "claim_composition_idempotent_neighborhood_assignment_finite_decidable",
  "claim_composition_not_left_distributive_over_pointwise_intersection",
  "claim_composition_not_right_distributive_over_pointwise_intersection",
  "claim_composition_power_additive",
  "claim_composition_power_included_in_closure",
  "claim_conjugacy_class_code_image_bijection",
  "claim_conjugacy_class_count_finite_decidability",
  "claim_conjugacy_class_relation_is_equivalence",
  "claim_covering_neighborhood_assignment_closure_eq",
  "claim_covering_neighborhood_assignment_generates",
  "claim_covering_neighborhood_assignment_included",
  "claim_covering_neighborhood_assignment_reachability_eq",
  "claim_covering_subset_one_step",
  "claim_crystallization_extraction_not_injective",
  "claim_cyclic_admissible_input_count",
  "claim_cyclic_elementary_encoding_bijection",
  "claim_cyclic_input_pullback_bijection",
  "claim_cyclic_input_realization",
  "claim_cyclic_offset_collision",
  "claim_cyclic_offset_image_count",
  "claim_cyclic_offset_injective_boundary",
  "claim_cyclic_radius_one_collapse",
  "claim_cyclic_radius_one_comparison",
  "claim_cyclic_rule_global_equality",
  "claim_cyclic_rule_realization_fiber_count",
  "claim_cyclic_rule_table_count",
  "claim_cyclic_stage_family_locally_converges",
  "claim_cyclic_stage_is_finite_cyclic_group",
  "claim_cyclic_stage_projection_not_globally_injective",
  "claim_cyclic_stage_projection_preserves_addition",
  "claim_cyclic_stage_shift_logarithmic_density_obstruction",
  "claim_cyclic_stage_uniform_conditional_kernel_normalized",
  "claim_cyclic_stage_uniform_distribution_normalized",
  "claim_cyclic_stage_uniform_marginal_formula",
  "claim_cyclic_stage_window_embeddings_compatible",
  "claim_cyclic_stage_window_image_cardinality",
  "claim_cyclic_stages_eventually_match_integer_window",
  "claim_cyclic_uniform_global_count",
  "claim_dependency_source_set_finite",
  "claim_dependency_transfer",
  "claim_deterministic_rules_are_zero_one_probabilistic_rules",
  "claim_deterministic_transfer_matrix_entry",
  "claim_deterministic_transfer_matrix_power_entry",
  "claim_down_set_boundary_outgoing",
  "claim_down_set_no_incoming_edge",
  "claim_down_set_order_convex",
  "claim_empty_neighborhood_assignment_is_composition_absorbing",
  "claim_event_order_locally_finite",
  "claim_event_set_cardinality",
  "claim_exact_solution_candidate_certificates_individually_decidable",
  "claim_finite_binary_ca_specializes_to_finite_self_map",
  "claim_finite_braid_solution_gives_constant_complex_yang_baxter_family",
  "claim_finite_exterior_basis_permutation_top_sign",
  "claim_finite_exterior_generators_square_zero_anticommute",
  "claim_finite_exterior_word_adjacent_swap_sign",
  "claim_finite_gibbs_transition_weight_normalized",
  "claim_finite_gibbs_transition_weight_strictly_positive",
  "claim_finite_internal_scattering_compatibilities_decidable",
  "claim_finite_local_factor_step_comparison_decidable",
  "claim_finite_neighborhood_assignment_monoid_cardinality_decidable",
  "claim_finite_neighborhood_assignments_form_idempotent_semiring",
  "claim_finite_neighborhood_assignments_form_monoid",
  "claim_finite_neighborhood_assignments_form_ordered_monoid",
  "claim_finite_pair_map_classical_nondegeneracy_decidable",
  "claim_finite_partial_order_locally_finite",
  "claim_finite_poset_covering_relation_finite_decidable",
  "claim_finite_poset_interval_endpoints",
  "claim_finite_poset_interval_finite",
  "claim_finite_poset_interval_strictly_smaller",
  "claim_finite_power_bound_certificate_decidable",
  "claim_finite_power_bound_cutoff_not_global",
  "claim_finite_propagation_boundary",
  "claim_finite_rational_entropy_real_comparison",
  "claim_finite_real_distribution_not_always_rational",
  "claim_finite_self_map_image_finite",
  "claim_finite_self_map_injective_iff_all_periodic",
  "claim_finite_self_map_injective_iff_surjective",
  "claim_finite_self_map_injectivity_finite_decidability",
  "claim_finite_self_map_orbit_finite",
  "claim_finite_self_map_repeating_tail",
  "claim_finite_signed_coefficient_extraction_decidable",
  "claim_finite_submultiplicative_count_certificate_decidable",
  "claim_finite_submultiplicative_count_cutoff_not_global",
  "claim_finite_submultiplicative_count_multiple_index_density_bound",
  "claim_finite_swap_is_yang_baxter",
  "claim_finite_top_coefficient_matrix_not_automatically_update",
  "claim_finite_two_symbol_word_set_cardinality",
  "claim_finite_word_counts_do_not_determine_next_length",
  "claim_finite_yang_baxter_condition_decidable",
  "claim_fixed_iff_min_period_divides",
  "claim_fixed_neighborhood_reversible_maps_not_composition_closed",
  "claim_fixed_point_count_bounded_by_cardinality",
  "claim_fixed_point_count_decomposition",
  "claim_fixed_point_count_finite_decidability",
  "claim_flip_test_equivalence",
  "claim_forbidden_one_run_words_full_below_cutoff",
  "claim_full_two_symbol_word_realized_density_constant",
  "claim_general_binary_rule_need_not_preserve_zero",
  "claim_general_binary_rule_not_forced_reversible",
  "claim_general_self_neighborhood_pointwise_form",
  "claim_general_self_neighborhood_reversible_pointwise_bijective",
  "claim_global_flip_characterization",
  "claim_global_map_composition_representable_on_composed_neighborhood",
  "claim_identity_neighborhood_assignment_is_composition_identity",
  "claim_identity_neighborhood_subset_union_map",
  "claim_integer_stage_finite_observation_catalogue_countable",
  "claim_integer_stage_finite_support_configurations_countable",
  "claim_integer_stage_full_configurations_uncountable",
  "claim_inverse_map_is_ca_iff_support_subset",
  "claim_inverse_support_finite_decidability",
  "claim_inverse_support_not_in_neighborhood_counterexample",
  "claim_invertible_neighborhood_assignment_cardinality_decidable",
  "claim_invertible_neighborhood_assignment_inverse_unique",
  "claim_invertible_neighborhood_assignments_are_permutations",
  "claim_iterate_collision_gives_repeating_tail",
  "claim_iterate_collision_stabilizes_tails",
  "claim_iterate_composition_addition",
  "claim_iterate_map_collision_finite_representatives",
  "claim_iterate_monoid_before_rounded_preperiod_not_root",
  "claim_iterate_monoid_conjugacy_finite_decidability",
  "claim_iterate_monoid_conjugacy_invariant_family_finite_decidability",
  "claim_iterate_monoid_conjugacy_iterate_equality_equivalence",
  "claim_iterate_monoid_conjugacy_numerical_profile_counterexample_ca_specialization",
  "claim_iterate_monoid_conjugacy_numerical_profile_invariant",
  "claim_iterate_monoid_conjugacy_numerical_profile_not_complete",
  "claim_iterate_monoid_conjugacy_one_period_iterate_power",
  "claim_iterate_monoid_conjugacy_preserves_collision_start",
  "claim_iterate_monoid_conjugacy_preserves_minimal_period",
  "claim_iterate_monoid_conjugacy_preserves_tree_branching",
  "claim_iterate_monoid_conjugacy_preserves_tree_depth",
  "claim_iterate_monoid_conjugacy_transports_cycle_idempotent",
  "claim_iterate_monoid_conjugacy_transports_iterates",
  "claim_iterate_monoid_conjugacy_transports_one_period_iterates",
  "claim_iterate_monoid_conjugacy_transports_one_period_map",
  "claim_iterate_monoid_conjugacy_transports_stable_fibers",
  "claim_iterate_monoid_conjugacy_transports_stable_image",
  "claim_iterate_monoid_conjugacy_transports_tree_edges",
  "claim_iterate_monoid_cycle_group_finite_decidability",
  "claim_iterate_monoid_cycle_idempotent_candidate_is_idempotent",
  "claim_iterate_monoid_cycle_idempotent_commutes_with_generator",
  "claim_iterate_monoid_cycle_idempotent_finite_decidability",
  "claim_iterate_monoid_cycle_idempotent_retracts_stable_image",
  "claim_iterate_monoid_cycle_idempotent_unique",
  "claim_iterate_monoid_cycle_part_group_laws",
  "claim_iterate_monoid_cycle_part_is_cyclic_of_order_min_period",
  "claim_iterate_monoid_cycle_part_pairwise_distinct",
  "claim_iterate_monoid_depth_layer_branching_finite_decidability",
  "claim_iterate_monoid_depth_layer_preimage_finite_decidability",
  "claim_iterate_monoid_distinct_stable_fibers_disjoint",
  "claim_iterate_monoid_fiber_tree_depth_decrement",
  "claim_iterate_monoid_fiber_tree_depth_equals_rounded_preperiod",
  "claim_iterate_monoid_fiber_tree_depth_zero_iff_root",
  "claim_iterate_monoid_fiber_tree_edge_count",
  "claim_iterate_monoid_fiber_tree_finite_decidability",
  "claim_iterate_monoid_fiber_tree_no_cycle",
  "claim_iterate_monoid_finite_decidability",
  "claim_iterate_monoid_finite_subset_preimage_decomposition",
  "claim_iterate_monoid_first_stable_equals_min_collision_start",
  "claim_iterate_monoid_generated_ideal_finite_chain_decidable",
  "claim_iterate_monoid_generated_ideals_comparable",
  "claim_iterate_monoid_generator_bijective_on_stable_image",
  "claim_iterate_monoid_global_period_at_point_min_preperiod",
  "claim_iterate_monoid_idempotent_uniqueness_fails",
  "claim_iterate_monoid_idempotents_finite_decidability",
  "claim_iterate_monoid_min_preperiod_decrements",
  "claim_iterate_monoid_minimal_period_divides_every_period",
  "claim_iterate_monoid_minimal_period_finite_decidability",
  "claim_iterate_monoid_no_collision_before_min_start",
  "claim_iterate_monoid_one_period_map_fixes_root",
  "claim_iterate_monoid_one_period_map_preserves_fiber",
  "claim_iterate_monoid_one_period_map_reaches_root",
  "claim_iterate_monoid_one_period_map_unique_fixed_point",
  "claim_iterate_monoid_one_period_step_composition",
  "claim_iterate_monoid_period_multiple_propagates",
  "claim_iterate_monoid_period_propagates_after_collision_start",
  "claim_iterate_monoid_positive_depth_layer_exact_preimage",
  "claim_iterate_monoid_positive_depth_layer_predecessor_count",
  "claim_iterate_monoid_principal_ideal_equivalence_relation",
  "claim_iterate_monoid_root_depth_preperiod_correspondence_finite_decidability",
  "claim_iterate_monoid_rounded_preperiod_reaches_root",
  "claim_iterate_monoid_stable_fiber_branching_finite_decidability",
  "claim_iterate_monoid_stable_fiber_cardinality_decomposition",
  "claim_iterate_monoid_stable_fiber_depth_finite_decidability",
  "claim_iterate_monoid_stable_fiber_depth_partition",
  "claim_iterate_monoid_stable_fiber_depth_transition",
  "claim_iterate_monoid_stable_fiber_dynamics_finite_decidability",
  "claim_iterate_monoid_stable_fiber_exact_preimage",
  "claim_iterate_monoid_stable_fiber_image_can_be_strict",
  "claim_iterate_monoid_stable_fiber_predecessor_count_conservation",
  "claim_iterate_monoid_stable_fiber_predecessor_set_finite",
  "claim_iterate_monoid_stable_fiber_predecessors_disjoint",
  "claim_iterate_monoid_stable_fiber_preimage_decomposition",
  "claim_iterate_monoid_stable_fiber_unique_representative",
  "claim_iterate_monoid_stable_fibers_finite_decidability",
  "claim_iterate_monoid_stable_image_finite_decidability",
  "claim_iterate_monoid_stable_period_multiple_exists",
  "claim_iterate_monoid_stable_power_image_equals_stable_image",
  "claim_iterate_monoid_stable_representative_belongs_to_fiber",
  "claim_iterate_monoid_stable_tail_equals_cycle_part",
  "claim_iterate_monoid_tail_absorbs_composition",
  "claim_iterate_monoid_tail_equality_iff_collision_start",
  "claim_iterate_monoid_tail_finite_decidability",
  "claim_iterate_monoid_tail_is_principal_ideal",
  "claim_iterate_monoid_tails_descend",
  "claim_iterate_monoid_tails_strict_then_stable",
  "claim_iterate_monoid_transient_cycle_finite_decidability",
  "claim_iterate_monoid_transient_cycle_partition_cardinality",
  "claim_iterate_monoid_zero_depth_layer_exact_preimage",
  "claim_iterate_monoid_zero_depth_layer_predecessor_count",
  "claim_iterate_monoid_zero_depth_maps_to_zero_depth",
  "claim_iterate_powers_form_finite_commutative_monoid",
  "claim_locality_restricts_cycle_type",
  "claim_min_preperiod_period_bound",
  "claim_min_preperiod_period_finite_decidability",
  "claim_minimal_inverse_neighborhood_finite_decidability",
  "claim_neighborhood_assignment_composition_not_commutative",
  "claim_neighborhood_assignment_idempotent_iff_subset_union_map_idempotent",
  "claim_neighborhood_assignment_inclusion_iff_union_eq",
  "claim_neighborhood_assignment_intersection_and_distributivity_finite_decidable",
  "claim_neighborhood_assignment_monoid_center_characterization",
  "claim_neighborhood_assignment_monoid_center_finite_decidability",
  "claim_neighborhood_assignment_pointwise_inclusion_finite_decidable",
  "claim_neighborhood_assignment_pointwise_inclusion_partial_order",
  "claim_neighborhood_assignment_pointwise_intersection_laws",
  "claim_neighborhood_assignment_pointwise_union_intersection_lattice",
  "claim_neighborhood_assignment_pointwise_union_laws",
  "claim_neighborhood_assignment_recovered_from_singletons",
  "claim_neighborhood_assignment_subset_union_map_composition",
  "claim_neighborhood_assignment_subset_union_map_finite_decidable",
  "claim_neighborhood_assignment_subset_union_map_injective",
  "claim_neighborhood_assignment_transpose_finitely_decidable",
  "claim_neighborhood_assignment_transpose_involutive",
  "claim_neighborhood_assignment_transpose_membership",
  "claim_neighborhood_assignment_transpose_preserves_lattice_operations",
  "claim_neighborhood_assignment_transpose_reverses_composition",
  "claim_neighborhood_mutual_reachability_component_membership",
  "claim_neighborhood_mutual_reachability_component_nonempty",
  "claim_neighborhood_mutual_reachability_component_order_antisymmetric",
  "claim_neighborhood_mutual_reachability_component_order_finite_decidable",
  "claim_neighborhood_mutual_reachability_component_order_is_partial_order",
  "claim_neighborhood_mutual_reachability_component_order_reflexive",
  "claim_neighborhood_mutual_reachability_component_order_representative_independent",
  "claim_neighborhood_mutual_reachability_component_order_transitive",
  "claim_neighborhood_mutual_reachability_component_representative",
  "claim_neighborhood_mutual_reachability_component_self_transpose",
  "claim_neighborhood_mutual_reachability_components_partition",
  "claim_neighborhood_mutual_reachability_reflexive",
  "claim_neighborhood_mutual_reachability_symmetric",
  "claim_neighborhood_mutual_reachability_transitive",
  "claim_neighborhood_reachability_preorder_finite_decidable",
  "claim_neighborhood_reachability_preorder_not_antisymmetric",
  "claim_neighborhood_reachability_preorder_reflexive",
  "claim_neighborhood_reachability_preorder_transitive",
  "claim_no_dependency_on_redundant_element",
  "claim_no_mutual_reachability",
  "claim_nonidentity_idempotent_not_forced",
  "claim_one_step_boundary_finite",
  "claim_one_step_dependency_finite_decidability",
  "claim_one_step_equals_covering",
  "claim_one_step_subset_covering",
  "claim_one_step_subset_reachability",
  "claim_orbit_collision",
  "claim_order_convex_intersection",
  "claim_order_convex_subset_finite",
  "claim_order_interval_finite",
  "claim_order_iso_not_time_preserving",
  "claim_partial_order_neighborhood_assignment_closure_eq",
  "claim_partial_order_neighborhood_assignment_component_singleton",
  "claim_partial_order_neighborhood_assignment_preorder_eq",
  "claim_partial_order_neighborhood_assignment_quotient_singletons",
  "claim_partial_order_neighborhood_assignment_reflexive",
  "claim_partial_order_neighborhood_assignment_transitive",
  "claim_partial_order_quotient_realization_finite_decidable",
  "claim_partial_order_quotient_realization_map_bijective",
  "claim_partial_order_quotient_realization_order",
  "claim_path_time_increment_exact",
  "claim_path_time_strictly_increases",
  "claim_period_descends_to_min_preperiod",
  "claim_period_multiples",
  "claim_periodic_iff_min_preperiod_zero",
  "claim_periodic_orbit_card_eq_min_period",
  "claim_periodicity_pair_iff_collision",
  "claim_positive_count_domain_finitely_decidable",
  "claim_positive_count_domain_iff_period_divides",
  "claim_positive_count_domain_small_witness",
  "claim_positive_idempotent_iterate_exists",
  "claim_positive_integer_reciprocal_converges_rationally",
  "claim_prime_logarithm_inverse",
  "claim_prime_logarithm_ordered_group",
  "claim_prime_logarithm_product",
  "claim_prime_logarithm_ratio",
  "claim_prime_vector_additive_realization_natural_multiple",
  "claim_prime_vector_integer_division",
  "claim_prime_vector_real_evaluation_of_prime_logarithm",
  "claim_prime_vector_zero_realization_additive",
  "claim_prime_vectors_abelian_group",
  "claim_probabilistic_finite_step_rational_closure",
  "claim_probabilistic_membership_finite_decidable",
  "claim_propagation_ball_finite",
  "claim_rational_prime_vector_asymptotically_distinct_cauchy_sequences_uncountable",
  "claim_rational_prime_vector_finite_sum_distance_nonnegative",
  "claim_rational_prime_vector_geometric_truncations_cauchy",
  "claim_rational_prime_vector_geometric_truncations_no_limit",
  "claim_rational_transition_weight_not_always_finite_gibbs",
  "claim_reachability_approximation_monotone",
  "claim_reachability_approximation_recursion",
  "claim_reachability_approximation_stabilizes",
  "claim_reachability_approximation_stable_forever",
  "claim_reachability_finite",
  "claim_reachability_irreflexive",
  "claim_reachability_minimal",
  "claim_reachability_partial_order",
  "claim_reachability_transitive",
  "claim_recursive_preimage_tree_code_child_preperiod_increment",
  "claim_recursive_preimage_tree_code_complete_invariant",
  "claim_recursive_preimage_tree_code_completeness",
  "claim_recursive_preimage_tree_code_conjugacy_invariance",
  "claim_recursive_preimage_tree_code_finite_decidability",
  "claim_recursive_preimage_tree_code_preperiod_upper_bound",
  "claim_reflexive_neighborhood_assignment_idempotent_iff_transitive",
  "claim_reflexive_transitive_closure_contains_original",
  "claim_reflexive_transitive_closure_finite_decidable",
  "claim_reflexive_transitive_closure_idempotent",
  "claim_reflexive_transitive_closure_minimal",
  "claim_reflexive_transitive_closure_reflexive",
  "claim_reflexive_transitive_closure_transitive",
  "claim_relation_path_finite",
  "claim_representable_implies_support_subset",
  "claim_reversible_conjugacy_classes_bijection_partitions",
  "claim_reversible_cycle_type_completeness",
  "claim_reversible_cycle_type_conjugacy_invariance",
  "claim_reversible_cycle_type_realizes_every_partition",
  "claim_reversible_cycle_type_sum",
  "claim_second_order_base_family_unique",
  "claim_second_order_evolution_after_inverse",
  "claim_second_order_inverse_after_evolution",
  "claim_second_order_membership_finite_decidable",
  "claim_self_neighborhood_flip_composition_symmetric_difference",
  "claim_self_neighborhood_injective_iff_pointwise_bijective",
  "claim_self_neighborhood_involution",
  "claim_self_neighborhood_realized_cycle_types",
  "claim_self_neighborhood_reversible_map_cycle_types_general",
  "claim_self_neighborhood_reversible_maps_classified_by_flip_sets",
  "claim_self_neighborhood_reversible_maps_finite_commutative_group",
  "claim_self_transpose_composition_closure_finitely_decidable",
  "claim_self_transpose_composition_edge_witness_is_self_transpose",
  "claim_self_transpose_composition_iff_commute",
  "claim_self_transpose_composition_loop_witness_is_self_transpose",
  "claim_self_transpose_iff_symmetric_membership",
  "claim_self_transpose_neighborhood_assignment_count",
  "claim_self_transpose_neighborhood_assignments_finitely_decidable",
  "claim_self_transpose_neighborhood_assignments_not_composition_closed",
  "claim_self_transpose_pair_encoding_bijection",
  "claim_shift_rationalized_logarithmic_density_converges_rationally",
  "claim_shift_rationalized_logarithmic_density_not_eventually_constant",
  "claim_shift_rationalized_logarithmic_density_vector_converges",
  "claim_single_cell_flip_positive_count_domain",
  "claim_single_finite_yang_baxter_map_does_not_determine_spectral_dependence",
  "claim_stage_global_maps_count",
  "claim_stage_realized_cycle_types_decidable",
  "claim_start_cell_in_propagation_ball",
  "claim_subset_union_map_image_is_union_preserving_maps",
  "claim_subset_union_map_preserves_empty",
  "claim_subset_union_map_preserves_union",
  "claim_subsingleton_neighborhood_composition_distributes_over_intersection",
  "claim_subsingleton_neighborhood_composition_equals_intersection",
  "claim_support_finite_decidability",
  "claim_support_invariance",
  "claim_support_is_minimum_representing_set",
  "claim_support_subset_implies_representable",
  "claim_synchronous_rule_not_forced_block_local",
  "claim_three_cell_cyclic_shift_reversible",
  "claim_time_slice_antichain",
  "claim_time_strictly_increases",
  "claim_totalistic_membership_finite_decidable",
  "claim_totalistic_pairwise_characterization",
  "claim_transfer_matrix_power_equals_finite_step_weight",
  "claim_transitive_and_factorable_neighborhood_assignment_independent",
  "claim_two_element_composition_intersection_nondistributivity",
  "claim_two_element_pair_map_not_always_yang_baxter",
  "claim_union_preserving_map_count",
  "claim_union_preserving_map_determined_by_singletons",
  "claim_union_preserving_map_finite_decidable",
  "claim_union_preserving_map_representation_unique",
  "claim_unordered_carrier_pair_count",
  "claim_up_set_order_convex",
  "def_all_self_transpose_assignments_composition_closed",
  "def_antichain",
  "def_bare_stage_transport_counterexample",
  "def_base_value_extension",
  "def_bijective_self_map_conjugacy_classes",
  "def_bijective_self_maps",
  "def_binary_ca_commuting_configuration_symmetries",
  "def_binary_ca_fiber_logarithmic_entropy",
  "def_binary_ca_fiber_multiplicity",
  "def_binary_ca_fixed_point_fibers",
  "def_binary_ca_integer_conserved_observable",
  "def_binary_ca_integer_conserved_observable_group",
  "def_binary_ca_interpolated_derivative_temperature",
  "def_binary_ca_logarithmic_free_count",
  "def_binary_ca_orbit_complex_phase_vector",
  "def_binary_ca_permutation_matrix_complex_action",
  "def_binary_ca_positive_fiber_levels",
  "def_binary_ca_real_entropy_interpolants",
  "def_binary_ca_real_fiber_entropy_sample",
  "def_binary_ca_real_parameter_symmetry_action",
  "def_binary_ca_real_phase_generator",
  "def_binary_ca_reversible_global_finite_order",
  "def_binary_ca_reversible_global_permutation_matrix",
  "def_binary_ca_reversible_phase_codes",
  "def_binary_ca_reversible_phase_complex_realization",
  "def_binary_ca_unit_logarithmic_difference",
  "def_binary_configuration_pointwise_addition",
  "def_binary_configuration_scalar_multiplication",
  "def_binary_configuration_zero",
  "def_binary_field_linear_local_rule_family",
  "def_binary_finite_bath_count_canonical_distribution",
  "def_binary_finite_real_exponential_canonical_distribution",
  "def_binary_finite_second_observation_multiplicity",
  "def_binary_finite_total_observation_shell",
  "def_binary_finite_two_configuration_integer_observations",
  "def_binary_state_addition",
  "def_binary_state_multiplication",
  "def_block_local_rule_family",
  "def_block_partition",
  "def_block_phase_update",
  "def_block_sweep_update",
  "def_ca_dependency_path",
  "def_ca_reachability",
  "def_ca_reflexive_reachability",
  "def_cardinality_notation",
  "def_carrier_cardinality_partitions",
  "def_complex_spectral_parameter_yang_baxter_family",
  "def_composed_local_rule_family",
  "def_composed_neighborhood",
  "def_composite_support_strict_inclusion_witness",
  "def_composition_idempotent_neighborhood_assignment",
  "def_composition_left_intersection_nondistributivity_witness",
  "def_composition_right_intersection_nondistributivity_witness",
  "def_conjugacy_class_all_self_maps",
  "def_conjugacy_class_code_image",
  "def_conjugacy_class_quotient",
  "def_conjugacy_class_relation",
  "def_covering_neighborhood_assignment",
  "def_covering_relation",
  "def_cyclic_admissible_inputs",
  "def_cyclic_elementary_encoding",
  "def_cyclic_input_pullback",
  "def_cyclic_integer_remainder",
  "def_cyclic_offset_projection",
  "def_cyclic_stage_family",
  "def_cyclic_stage_fixed_point_count_sequence",
  "def_cyclic_stage_global_map_family",
  "def_cyclic_stage_group_operations",
  "def_cyclic_stage_local_convergence",
  "def_cyclic_stage_logarithmic_count_sequence",
  "def_cyclic_stage_logarithmic_density_domain",
  "def_cyclic_stage_outside_agreement",
  "def_cyclic_stage_positive_count_domain",
  "def_cyclic_stage_shift_rule_family",
  "def_cyclic_stage_uniform_conditional_kernel",
  "def_cyclic_stage_uniform_distribution_family",
  "def_cyclic_stage_window_equality_relations",
  "def_cyclic_stage_window_image",
  "def_cyclic_uniform_rule_map",
  "def_dependency_path",
  "def_dependency_source_set",
  "def_deterministic_rule_zero_one_embedding",
  "def_down_set",
  "def_empty_neighborhood_assignment",
  "def_essential_dependency",
  "def_essential_dependency_support",
  "def_event_set",
  "def_finite_block_schedule",
  "def_finite_ca",
  "def_finite_carrier_subset_space",
  "def_finite_closed_symmetric_stage",
  "def_finite_configuration_map_cell_map",
  "def_finite_count_density_comparison",
  "def_finite_exterior_anticommuting_product",
  "def_finite_exterior_basis_table",
  "def_finite_exterior_coefficient_table",
  "def_finite_exterior_top_coefficient",
  "def_finite_exterior_word_monomial",
  "def_finite_gibbs_transition_weight",
  "def_finite_index_interval",
  "def_finite_internal_scattering_tables",
  "def_finite_local_factor_step_evolution_matrix",
  "def_finite_local_update_coefficient_factor",
  "def_finite_neighborhood_assignment_space",
  "def_finite_neighborhood_system",
  "def_finite_ordered_local_factor_product",
  "def_finite_pair_map",
  "def_finite_pair_map_adjacent_lifts",
  "def_finite_pair_map_classical_nondegeneracy",
  "def_finite_pair_map_complex_linearization_comparison",
  "def_finite_poset_covering_relation",
  "def_finite_poset_interval",
  "def_finite_power_bound_certificate",
  "def_finite_power_bound_cutoff_extensions",
  "def_finite_rational_prime_vector_entropy",
  "def_finite_rational_probability_distribution_support",
  "def_finite_real_row_partition_sum",
  "def_finite_relation_covering",
  "def_finite_self_map",
  "def_finite_self_map_indicator_matrix",
  "def_finite_self_map_injective_surjective",
  "def_finite_self_map_iterate",
  "def_finite_self_neighborhood_flip_map",
  "def_finite_submultiplicative_count_certificate",
  "def_finite_submultiplicative_count_cutoff_extensions",
  "def_finite_support_rational_prime_vectors",
  "def_finite_top_coefficient_matrix",
  "def_finite_two_symbol_word_set",
  "def_finite_word_realized_logarithmic_density",
  "def_finite_yang_baxter_map",
  "def_fixed_points_of_iterate",
  "def_flip_map",
  "def_forbidden_one_run_word_family",
  "def_full_neighborhood_assignment",
  "def_global_map",
  "def_global_map_essential_dependency_assignment",
  "def_identity_neighborhood_assignment",
  "def_incomparability",
  "def_increasing_prime_sequence",
  "def_integer_offset_interval",
  "def_integer_stage_finite_observation_catalogue",
  "def_integer_stage_finite_support_configurations",
  "def_internal_scattering_affine_lift",
  "def_internal_scattering_affine_yang_baxter_compatibility",
  "def_internal_scattering_integer_difference_compatibility",
  "def_internal_scattering_integer_normalization",
  "def_internal_scattering_operator_compatibility",
  "def_inverse_cell_map_support",
  "def_inverse_global_map",
  "def_invertible_neighborhood_assignment",
  "def_iterate_monoid",
  "def_iterate_monoid_collision_start",
  "def_iterate_monoid_conjugacy_bijection",
  "def_iterate_monoid_conjugacy_numerical_profile",
  "def_iterate_monoid_conjugacy_numerical_profile_counterexample",
  "def_iterate_monoid_cycle_idempotent_candidate",
  "def_iterate_monoid_cycle_operation_and_successor",
  "def_iterate_monoid_fiber_tree_depth",
  "def_iterate_monoid_fiber_tree_edges",
  "def_iterate_monoid_generated_ideal",
  "def_iterate_monoid_generated_ideal_chain",
  "def_iterate_monoid_idempotent",
  "def_iterate_monoid_minimal_positive_period",
  "def_iterate_monoid_minimal_stable_period_multiple",
  "def_iterate_monoid_one_period_map",
  "def_iterate_monoid_principal_ideal_equivalence",
  "def_iterate_monoid_root_reach_exponent",
  "def_iterate_monoid_rounded_preperiod_depth",
  "def_iterate_monoid_stable_fiber",
  "def_iterate_monoid_stable_fiber_depth_layer",
  "def_iterate_monoid_stable_fiber_index_map",
  "def_iterate_monoid_stable_fiber_predecessor_count",
  "def_iterate_monoid_stable_fiber_predecessor_set",
  "def_iterate_monoid_stable_fiber_rooted_tree",
  "def_iterate_monoid_stable_image",
  "def_iterate_monoid_stable_image_inverse_candidate",
  "def_iterate_monoid_stable_period_multiple_exponents",
  "def_iterate_monoid_tail",
  "def_iterate_monoid_transient_and_cycle_parts",
  "def_local_rule_representable",
  "def_local_truth_table",
  "def_locally_finite_partial_order",
  "def_min_period",
  "def_min_preperiod",
  "def_negation_map",
  "def_neighborhood_assignment_composition_power",
  "def_neighborhood_assignment_monoid_center",
  "def_neighborhood_assignment_pointwise_inclusion",
  "def_neighborhood_assignment_pointwise_intersection",
  "def_neighborhood_assignment_pointwise_union",
  "def_neighborhood_assignment_reachability_approximation",
  "def_neighborhood_assignment_reflexive_transitive_closure",
  "def_neighborhood_assignment_subset_union_map",
  "def_neighborhood_assignment_transpose",
  "def_neighborhood_mutual_reachability",
  "def_neighborhood_mutual_reachability_component",
  "def_neighborhood_mutual_reachability_component_order",
  "def_neighborhood_mutual_reachability_component_set",
  "def_neighborhood_reachability_preorder",
  "def_noncommutative_neighborhood_assignment_witness",
  "def_normalized_q_operator_crystallization_extraction",
  "def_one_step_boundary",
  "def_one_step_dependency",
  "def_orbit",
  "def_order_convex_subset",
  "def_order_interval",
  "def_pair_set_neighborhood_reconstruction",
  "def_partial_order",
  "def_partial_order_neighborhood_assignment",
  "def_partial_order_quotient_realization_map",
  "def_periodic_points",
  "def_periodicity_pairs",
  "def_permutation_neighborhood_assignment",
  "def_positive_fixed_point_count_domain",
  "def_positive_fixed_point_count_rational_input",
  "def_positive_rational_epsilon_convergence",
  "def_positive_rational_prime_valuation",
  "def_prime_integer_vectors",
  "def_prime_logarithm",
  "def_prime_vector_additive_operations",
  "def_prime_vector_additive_real_realization",
  "def_prime_vector_order",
  "def_prime_vector_real_logarithmic_evaluation",
  "def_prime_vector_reconstruction",
  "def_prime_vector_zero_real_realization",
  "def_probabilistic_global_transition_weight",
  "def_probabilistic_local_output_weight",
  "def_propagation_ball",
  "def_rational_prime_vector_cauchy_sequence_asymptotic_agreement",
  "def_rational_prime_vector_finite_sum_cauchy",
  "def_rational_prime_vector_finite_sum_convergence",
  "def_rational_prime_vector_finite_sum_distance",
  "def_rational_prime_vector_geometric_truncation_sequence",
  "def_rational_prime_vector_logarithmic_real_evaluation",
  "def_rational_prime_vector_zero",
  "def_rational_probabilistic_local_rule_family",
  "def_rational_transfer_matrix_power_trace",
  "def_rational_transition_real_comparison",
  "def_rational_transition_transfer_matrix",
  "def_reachability",
  "def_reachability_preorder_antisymmetry_counterexample",
  "def_realized_period_set",
  "def_recursive_preimage_tree_code",
  "def_recursive_preimage_tree_code_base_word",
  "def_recursive_preimage_tree_code_children",
  "def_recursive_preimage_tree_code_component_code",
  "def_recursive_preimage_tree_code_map_code",
  "def_recursive_preimage_tree_code_multiset_hierarchy",
  "def_recursive_preimage_tree_code_periodic_orbits",
  "def_redundant_extension",
  "def_reflexive_reachability",
  "def_restriction_map",
  "def_reversible_cycle_type",
  "def_second_order_configuration_space",
  "def_second_order_global_evolution",
  "def_second_order_inverse_candidate",
  "def_second_order_local_rule_family",
  "def_second_order_recovered_global_map",
  "def_self_neighborhood_stage",
  "def_self_transpose_composition_nonclosure_witness",
  "def_self_transpose_neighborhood_assignment",
  "def_self_transpose_pair_encoding",
  "def_shift_rationalized_logarithmic_density_sequence",
  "def_single_cell_flip_for_positive_count",
  "def_single_cell_identity_rational_transition_weight",
  "def_single_edge_neighborhood_assignment",
  "def_stage_global_maps",
  "def_stage_realized_cycle_types",
  "def_stage_reversible_global_maps",
  "def_state_set",
  "def_three_cell_cyclic_dependency_stage",
  "def_time_slice",
  "def_totalistic_local_rule_family",
  "def_totalistic_local_signature",
  "def_transitive_neighborhood_assignment",
  "def_two_element_intersection_nondistributivity_witnesses",
  "def_two_step_factorable_neighborhood_assignment",
  "def_union_preserving_subset_map",
  "def_unordered_carrier_pairs",
  "def_up_set",
  "remark_causal_comparison_scope",
  "remark_causal_set_not_claimed",
  "remark_causal_set_source",
  "remark_cyclic_stage_local_global_limit_boundaries",
  "remark_dependency_order_tools_specialization",
  "remark_finite_certificates_not_general_solvability",
  "remark_finite_local_factor_comparison_scope",
  "remark_finite_power_bound_scaling_boundary",
  "remark_finite_signed_coefficient_extraction_continuum_boundary",
  "remark_finite_signed_coefficient_extraction_scope",
  "remark_finite_submultiplicative_count_limit_boundary",
  "remark_finite_word_count_entropy_boundary",
  "remark_finite_word_real_normalization_before_entropy_limit",
  "remark_inverse_map_locality_not_claimed",
  "remark_iterate_monoid_principal_ideal_external_name_deferred",
  "remark_iteration_not_claimed",
  "remark_local_rule_representation_scope",
  "remark_locality_counting_criterion_insufficient",
  "remark_reversibility_ca_inverse_locality_not_claimed",
  "remark_reversibility_not_claimed",
  "remark_yang_baxter_conventions_and_sources",
  "theorem_cyclic_stage_uniform_conditional_kernel_invariance",
  "theorem_cyclic_stage_uniform_marginals_consistent",
  "theorem_deterministic_transfer_trace_equals_fixed_point_count",
  "theorem_finite_local_factor_step_matrix_equals_update_indicator",
  "theorem_minimal_carrier_size_for_composition_intersection_nondistributivity",
  "theorem_probabilistic_global_transition_normalized",
  "theorem_second_order_global_evolution_bijective",
] as const

/** content/ に実在するラベル。相互参照はこの型の値しか指せない。 */
export type Label = (typeof ALL_LABELS)[number]

/**
 * 翻訳ロケールにしか無いブロックのラベル（locales.config.ts が理由つきで認めたもの）。
 * **原文はこれを指せない**。原文から指せば、原文の解決で未解決参照になる。
 */
export const TRANSLATION_ONLY_LABELS = [

] as const

export type TranslationOnlyLabel = (typeof TRANSLATION_ONLY_LABELS)[number]

/** 翻訳ロケールの content が使うラベル。原文のラベルに翻訳限定のものを足しただけ。 */
export type AnyLocaleLabel = Label | TranslationOnlyLabel
