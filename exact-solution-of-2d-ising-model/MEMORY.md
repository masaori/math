...
## 進行中（2026-07-26）: 章 C′（偶セクターの半整数運動量モード）— **土台のみ完了、固有値はまだ**
...
**やっていないこと**: `V^{(+)}` の固有値そのもの。**`remark_remaining_input_even_sector` は
まだ解消されていない。** 008 章後半＋009 章に相当する部分（下記 C′-8〜C′-15）が残っている。
...
- C′-11・C′-12: `B_1B_2B_1 = A(θ~)` と `A(θ~)` の対角化。**これらは θ についての 2×2 の主張なので
  既存ブロック（`T_V_hatZ_hatY`, `eigenvector_of_A_theta` 等）を再利用できる見込み**
...
このゴールは着手前から「008 章＋009 章の合計に相当」と共有されていた。1 セッションでは
土台までしか到達できなかった。**残りを分割するなら C′-8〜C′-12（`A(θ~)` の対角化まで）と
C′-13〜C′-15（フェルミオンと固有値）の 2 つに切るのが自然**である。前者が済めば
`γ(θ~_μ)` が確定し、後者はほぼ 009 章の写しになる。
