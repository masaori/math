...
機械可読棚卸しは全263件について、二章分類、イジング固有語彙、説明粒度、直接依存、強連結成分を縮約した章内トポロジカル順を保存する。数学的道具立てからイジング章への前提依存は生成時に拒否する。状態と次の一歩は `docs/tasks/paper-organization-state.md`、再生成規則は `docs/tasks/paper-organization-runbook.md` を正本とする。
