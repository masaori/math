...
両方の生成子を同時に負号へ移す可逆行列による共役へ進む。直後の順39は偶セクターの転送行列と
共役写像を定義するが順37・38には依存せず、可逆元・共役写像・行列指数関数による共役・可換行列の
指数積公式を追加する一方で、Pauli 行列の積、クロネッカー積の成分と転置、エルミート性を入力から
外すため、順38の後を節境界とした。
...
一意な閉包、順39との相互非依存、順39の本文・直接依存、入力集合の追加・除外、および粒度判定を
