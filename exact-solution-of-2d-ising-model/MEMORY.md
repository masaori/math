...
1. **`gamma_2_theta_is_0`（`TV1_hatZ_hatY_022`）は `s_2^* ≠ 0` を暗黙に仮定している。**
   `γ_2` は `s_2^*` を因子に持つので、`s_2^* = 0` なら `sin θ`, `c_1cos θ - s_1c_2` と無関係に
   `γ_2 ≡ 0`。正しい同値は
   `γ_2(θ) = 0 ⟺ s_2^* = 0 ∨ (sin θ = 0 ∧ c_1cos θ - s_1c_2 = 0)`。
2. **同ブロックの「`sin θ_μ = 0 ⟺ θ_μ = 0, ±π, ±2π, … ⟺ μ = ±M`」は誤り。**
   正しくは `sin θ_μ = 0 ⟺ M ∣ 2μ` で、`ℳ = {-M,…,-1,1,…,M}` の範囲では
   `M` が偶数のとき `μ = ±M/2`（`θ_μ = ±π`）も該当する。原文はこの場合を落としている。
   `μ = ±M/2` では条件が `c_1 = -s_1c_2` になるので `c_1, s_1, c_2 > 0` から排除できるが、
   当該ブロックはその正値性を述べていない。
3. **`det_A_theta`（`TV1_hatZ_hatY_035`）の `det A(θ_μ) = 1` は関係 `c_2 s_2^* = c_2^*` を要する。**
   `A(θ)` の定義だけから無条件に言えるのは `det A(θ) = γ_1(θ)^2 + γ_2(θ)γ_2(-θ)` まで。
   これが `1` になるのは (i) `c_1^2 - s_1^2 = 1`、(ii) `(c_2^*)^2 - (s_2^*)^2 = 1`、
   (iii) `c_2 s_2^* = c_2^*` の 3 つが揃うとき。原文は `det B_1 = det B_2 = 1` から (i)(ii) は
   使っているが、**`A(θ)` の定義に現れる `c_2` と `B_1, B_2` に現れる `c_2^*` を結ぶ (iii)**
   （双対関係 `sinh(2K_2)sinh(2K_2^*) = 1` の帰結 `cosh(2K_2)sinh(2K_2^*) = cosh(2K_2^*)`）を
   明示していない。(iii) を落とすと `det A` は `θ` に依存し `1` にならない。
4. **`eigenvector_of_A_theta`（`TV1_hatZ_hatY_027`）の「`γ_2 = 0` のとき `A(θ_μ) = I`」も追加仮定が要る。**
   `γ_2 = 0` から出るのは `A = γ_1 I` まで。上の (i)(ii)(iii) を足しても `γ_1^2 = 1`、すなわち
   `A = ±I` まで。`+I` の確定には `γ_1 ≥ 1`（`TV1_hatZ_hatY_036`）が別途必要。
5. 固有値・固有ベクトルの符号対応（`λ_± = γ_1 ± √(-γ_2γ_2)` ↔ `v_± = c(±i√(γ_2γ_2), γ_2(-θ))`）は
