...
006/007 章に続き、**残っていた 489 箇所すべて**を置換した。これで `structured-latex/content/`
全 15 ファイルに抽象テンソル積の記法は 1 箇所も残っていない（README 2 節のゴール設定を本文が満たした）。
...
- 証明の内容・段階構造・ラベルは変更していない。変更した 34 ブロックには何をどう置換したかを
  `conversion.notes` に記録した。
- 検証: `npm run check`（ラベル生成物の同期・型検査・内容検証 174 ブロック 791 参照・移行漏れ・
  負テスト・実行時テスト）と `sagemath/tools/verify-check-linkage.ts`（99 check）をすべて通した。
