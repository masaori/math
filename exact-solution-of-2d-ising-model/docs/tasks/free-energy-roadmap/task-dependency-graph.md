...
- 章 A（009 章）・章 B・章 B2（010 章）・章 C（011 章）・章 D（012 章）は**完了**。
  章 C′ は**土台（013 章、C′-1〜C′-7）と `A(θ~)` の対角化（015 章、C′-12）まで完了**。
- **次は章 C′ の C′-8〜C′-11（`T_V` の作用と `B_1B_2B_1 = A(θ~)`）と C′-13 以降。**
  C′-12 は C′-8〜C′-11 とは独立に（`A(θ~)` の定義だけを入力として）閉じているので、
  どちらから着手してもよい。(A)〜(D) と反交換関係は 013 章、`A(θ~)` の対角化は 015 章で済んでいる。
