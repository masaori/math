...
| `open_chain_spin_sums` | `Σ_s e^{K E(s)} = 2(2cosh K)^{M−1}`、`Σ_s s(M)s(1)e^{K E(s)} = 2(2 sinh K)^{M−1}`（`E(s) = Σ_{m<M}s(m)s(m+1)`）。`t_m := s(m)s(m+1)` への変数変換（全単射）だけ。**`K > 0` で両方とも正** |
...
- **次に着手できるもの**（依存関係の順ではなく、独立に選べる）:
  ...
  2. **`c(M) = c_+(M)` の本文化（任意）** — 018 章は係数 2 の粗い評価で済ませている。
