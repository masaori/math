...
| `bridge`（`EvenSectorBridge`） | `W P^{(+)} = V^{(+)} P^{(+)}`、および `V^{(+)}` が実行列であること | **未形式化**。`lean/docs/ch011-formalization.md` 4 章: 「`sector_decomposition_of_rayleigh_sup` の (2) `W P^{(±)} = V^{(±)}P^{(±)}` は未形式化。`V^{(±)}`, `V₁^{(±)}`, `sector_replacement_of_V1`, `V1_restriction_to_eigenspaces` に依存する。これらは章 004 / 010 の対象」。実際 `Part010/Claim011_SectorReplacement.lean:43` は 004 章の `V1_restriction_to_eigenspaces` を「Lean 未形式化のため仮定として置く」と明記して `hres` を受け取っている。**実行列性**のほうは「実行列の `matExp` が実行列になる」ことが要るが、その補題は本リポジトリの Lean 側に無い（`grep -rn "map (fun r : ℝ => (r : ℂ))" lean/Ising2D/` は章 018 の橋渡しにしかヒットしない） |
| `htr` | `tr(εV^{(+)}) > 0` | **未形式化**。章 018 自身の `closing_004_claim_H1_plus_in_sigma_z_form` / `closing_005_claim_open_chain_spin_sums` / `closing_006_theorem_trace_of_epsilon_V_plus` の枝で、配置基底での 1 次元開鎖のスピン和を要する（`015_ch018-formalization-findings.md` 3 章）。章 018 の主鎖とは独立で、循環参照は無い |
