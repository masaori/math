...
同 claim の (2) `W P^{(±)} = V^{(±)} P^{(±)}` は、章 004 / 010 の
`V_1^{(±)}`, `V^{(±)}`, `sector_replacement_of_V1`, `V1_restriction_to_eigenspaces` に依存する。
これらは本タスク（章 011）の担当範囲外で Lean 側にも未形式化のため、
(2) は形式化していない。(1) と (3) は形式化済み。
