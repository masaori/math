...
| `008_TV1_hatZ_hatY_part2.mjs` の `TV1_hatZ_hatY_022`（`gamma_2_theta_is_0`） | `γ_2(θ_μ) = 0` の同値条件で **`s_2^* = 0` の場合が落ちている**（`γ_2` は `s_2^*` を因子に持つ）。原文の形は `s_2^* ≠ 0` の下でのみ正しい | `Ising2D/Part008/Definition019_ThetaGamma.lean` に修正版 `gamma2_eq_zero_iff` と、原文の形 `gamma2_eq_zero_iff_of_s2star_ne_zero` を用意 |
| 同上 | 最後の同値 **「`sin θ_μ = 0 ⟺ μ = ±M`」は誤り**。正しくは `M ∣ 2μ` で、`M` が偶数なら `μ = ±M/2`（`θ_μ = ±π`）も該当する。排除には `c_1, s_1, c_2 > 0` が要るが原文は述べていない | 同ファイルに `sin_thetaMu_eq_zero_iff`（`sin θ_μ = 0 ⟺ M ∣ 2μ`）を用意し、冒頭コメントに明記 |
| `008_TV1_hatZ_hatY_part2.mjs` の `TV1_hatZ_hatY_035`（`det_A_theta`） | `det A(θ_μ) = 1` に必要な関係のうち、**`c_2 s_2^* = c_2^*`（双対関係 `sinh(2K_2)sinh(2K_2^*) = 1` の帰結）が明示されていない**。これを落とすと `det A` は `θ` 依存になり `1` にならない | `Ising2D/Part008/Claim027_EigenATheta.lean` の `det_AMat`（無条件）と `det_AMat_eq_one`（3 関係を仮定）に分離 |
| `008_TV1_hatZ_hatY_part2.mjs` の `TV1_hatZ_hatY_027`（`eigenvector_of_A_theta`） | 場合分け 1) の「この場合 `A(θ_μ) = I`」は `γ_2 = 0` だけからは出ない（出るのは `A = γ_1 I`。上の 3 関係を足しても `A = ±I` まで）。`+I` の確定には `γ_1 ≥ 1`（`TV1_hatZ_hatY_036`）が要る | 同ファイルの `AMat_of_gamma2_eq_zero` / `gamma1_sq_eq_one_of_gamma2_eq_zero` として分離し、冒頭コメントに明記 |
| 同上 | 固有値と固有ベクトルの符号対応（`λ_± = γ_1 ± √(-γ_2γ_2)` と `v_± = c(±i√(γ_2γ_2), γ_2(-θ))`）は、**`arg^{[0,2π)}` 分枝での `√(-1·z) = -√(-1)√z` を使ってはじめて整合する**（原文はこの分枝規約を proof 中で導いてはいるが、statement 側には分枝の指定が無い） | 検算の結果**原文は正しい**。Lean 側は平方根関数を使わず `t^2 = γ_2(θ)γ_2(-θ)` の仮定形にし、`i t` 側の固有値が `γ_1 - i t` であることを明示（`AMat_mulVec_col_pos`） |
