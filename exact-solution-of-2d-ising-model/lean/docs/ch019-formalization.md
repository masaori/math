...
| `Ising2D.quad_le_quad_absVec_epsilonR_of_nonneg` | 同上の非負版（`0 ≤ W_{kl}` だけ） | (3)（抽象版で判明した最小の仮定） |
...
### 1.2 抽象版（`lean/Ising2D/Abstract/PermSector.lean`、名前空間 `Ising2D.Abstract`）

| Lean の名前 | 内容 | 人手証明のラベル |
| --- | --- | --- |
| `Abstract.permMat` | 写像 `π : n → n` の置換行列 `(permMat π)_{ij} = δ_{j,π(i)}` | `epsilon_is_sign_flip_permutation` (1) |
| `Abstract.permMat_entry_zero_or_one` / `permMat_row_sum` | 成分は `0/1`、各行の和は `1` | 同 (1) |
| `Abstract.permMat_mulVec` | `(εx)_i = x_{π(i)}` | 同 (3) |
| `Abstract.permMat_mulVec_single` | `ε e_k = e_{π(k)}` | 同 (1) |
| `Abstract.permMat_isSymm` / `permMat_mul_self` | 対合の置換行列は対称・対合 | 同 (1)(2) の帰結 |
| `Abstract.absVec` | `u_k := |x_k|` | `abs_vector_moves_to_even_sector` |
| `Abstract.permMat_mulVec_absVec` | `εx = -x ⇒ εu = u` | 同 (1) |
| `Abstract.vecNormSq_absVec` | `‖u‖ = ‖x‖` | 同 (2) |
| `Abstract.quad_eq_sum` | `xᵀWx = Σ_k Σ_l x_k W_{kl} x_l` | 同 (3) の成分表示 |
| `Abstract.abs_quad_le_quad_absVec` / `quad_le_quad_absVec` | `uᵀWu ≥ |xᵀWx| ≥ xᵀWx`（仮定は `0 ≤ W_{kl}` だけ） | 同 (3) |
| `Abstract.sectorSet_nonempty_of_mem` | セクターに非零ベクトルがあれば正規化できる | `c_minus_le_c_plus` Step 1 |
| `Abstract.sectorSet_pos_nonempty` | `𝓡_+ ≠ ∅`（`π` の不動点の有無によらない） | 同 Step 1 |
| `Abstract.sectorSet_neg_nonempty` | `𝓡_- ≠ ∅`（`π` が不動点をもたないことだけを使う） | `epsilon_is_sign_flip_permutation` (4) |
| `Abstract.sectorRayleighSup_nonneg` | `c_s ≥ 0` | 同 Step 1 |
| **`Abstract.sectorRayleighSup_neg_le_pos`** | **`c_-(M) ≤ c_+(M)`** | **`c_minus_le_c_plus`** |
| **`Abstract.rayleighSup_eq_sectorRayleighSup_pos`** | **`c(M) = c_+(M)`** | **`c_equals_c_plus` Step 1** |

## 2. 2 本立ての対応表と「抽象版で判明した本質」

| 人手証明のラベル | 具体版 | 抽象版 | 具体版は抽象版の系か |
| --- | --- | --- | --- |
| `epsilon_is_sign_flip_permutation` | `Ising2D.epsilon_apply` / `epsilonR_*` / `oddUnit` 系（`Conf M` 上の `ε = σ^x_1⋯σ^x_M`） | `Abstract.permMat_*`（任意の有限型 `n` と任意の対合 `π`） | (1) の成分計算（`σ^x` のクロネッカー積）以外は**すべて系** |
| `abs_vector_moves_to_even_sector` | `Ising2D.epsilonR_mulVec_absVec` / `vecNormSq_absVec_eq` / `quad_le_quad_absVec_epsilonR` | `Abstract.permMat_mulVec_absVec` / `vecNormSq_absVec` / `quad_le_quad_absVec` | **すべて系** |
| `c_minus_le_c_plus` | `Ising2D.c_minus_le_c_plus`（`ε = epsilonR M`） | `Abstract.sectorRayleighSup_neg_le_pos` | **系** |
| `c_equals_c_plus` | `Ising2D.c_equals_c_plus` | `Abstract.rayleighSup_eq_sectorRayleighSup_pos` | **系** |
...
- **人手証明 (2) の「`π` は不動点をもたない」は `c_-(M) ≤ c_+(M)` には効いていない。**
  抽象版 `Abstract.sectorRayleighSup_neg_le_pos` は不動点について何も仮定していない。
...
- **`𝓡_+ ≠ ∅` にも不動点の有無は効いていない。** `e_i + e_{π(i)}` は `π(i) = i` でも
  （`2e_i` として）非零で `ε` 不変だからである（`Abstract.sectorSet_pos_nonempty`）。
