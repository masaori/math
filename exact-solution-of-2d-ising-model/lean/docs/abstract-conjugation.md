...
| 抽象版 | `Ising2D/Abstract/Conjugation.lean` | `Ising2D.Abstract.Conj.sandwich_mul` / `sandwich_one` / `sandwich_comp` / `sandwich_add` / `conjMulAutHom` |
| 抽象版からの導出 | `Ising2D/Part000/Claim045_ConjugationIsRingHomAbstract.lean` | `Ising2D.Conjugation.matrix_conj_mul_of_abstract` ほか |
...
の証明に、**加法も分配法則も一度も現れない**。抽象版ではこれらを
...
- **合成則 (3) には可逆性が一切効いていない。** `a (b x b') a' = (ab) x (b'a')` は結合律だけで
  ...
  合成則そのものの証明ではない。行列側の導出 `matrix_conj_comp_of_abstract` でも、
...
原文は (3) を `T_A ∘ T_B = T_{AB}` という等式として述べているが、抽象版ではこれを

```
Ising2D.Abstract.Conj.conjMulAutHom : Mˣ →* MulAut M
```
...
具体版（`matrix_conj_mul` / `matrix_conj_one` / `matrix_conj_comp`）は、抽象版の系として
そのまま得られた（`matrix_conj_mul_of_abstract` / `matrix_conj_one_of_abstract` /
`matrix_conj_comp_of_abstract`）。行列側で追加した事実は
...
なお既存の具体版ファイルにある一般環版 `Ising2D.Conjugation.T` は、抽象版の
...
抽象版はそこを開いて、仮定ごとに分けた形で置き直したものである。

## 5. 本文（人手証明）へは持ち込まない

`exact-solution-of-2d-ising-model/README.md` 4 節の規約どおり、抽象版は Lean の中だけに置く。
