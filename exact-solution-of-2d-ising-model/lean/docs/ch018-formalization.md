...
| `bridge` | `W P^{(+)} = V^{(+)} P^{(+)}` と `V^{(+)}` が実行列であること | 章 011 の `symmetrized_transfer_matrix_on_sectors` が **Lean 未形式化**。004 章の `V1_restriction_to_eigenspaces` に依存し、そちらも未形式化（`Part010/Claim011_SectorReplacement.lean` が同じものを仮定 `hres` として置いている）。実行列性のほうは「実行列の `exp` が実行列である」ことを要し、本リポジトリの Lean 側にその補題が無い |
