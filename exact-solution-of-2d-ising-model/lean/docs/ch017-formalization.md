...
- 具体版: `Ising2D/Part017/`
- 抽象版: `Ising2D/Abstract/PairedFermion.lean`, `Ising2D/Abstract/ConstantC.lean`,
  `Ising2D/Abstract/SimpleEigenvalue.lean`
  （加えて、章 009 の `Ising2D/Abstract/NumberOperator.lean` と
  `Ising2D/Abstract/JointEigenspace.lean` を**そのまま再利用している**）

---

## 0. 本章の結論（先に要点）

**章 017 の主張のうち 9 本は、章 009 の抽象版をそのまま特殊化して得られる。**
章 009（整数運動量）と章 017（半整数運動量）の違いは、抽象版の言葉では

> **対をなす添字を与える単射 `σ` の取り方だけ**（章 009 は `σ(μ) = -μ`、
> 章 017 は `σ(μ) = M+1-μ`）

であり、`Ising2D/Abstract/PairedFermion.lean` の `acomm_cre_ann_comp` が
その 1 行の橋渡しである。消滅演算子を `a'_i := a_{σ i}` と付け替えれば
`δ_{ν,σ μ}` は Kronecker の `δ_{μν}` になり、章 009 の
`Abstract.num_mul_num` / `commute_num_num` / `projOn_*` / `two_pow_smul_tau_projOn` /
...
| --- | --- | --- |
| `Abstract.annPaired` / `numPaired` | `a'_i := a_{σ i}`、`n_i := c_i a_{σ i}` | `def_check_number_operator` |
| `Abstract.acomm_cre_ann_comp` | `δ_{ν,σ μ}` 型の CAR は付け替えで Kronecker の `δ_{μν}` になる（`σ` の単射性だけ） | `check_number_operator_idempotent` (2)、`check_number_operators_commute` Step 1 |
| `Abstract.two_pow_smul_tau_noncommProd` | `2^{|s|} τ(∏_{i∈s} n_i) = τ(1)` | `trace_of_check_number_operator_product` |
| `Abstract.const_eq_of_trace_ratio` | `T = cp = aτ`, `T' = c⁻¹p = a⁻¹τ`, `a,p>0`, `T>0` ⇒ `c = a` | `constant_c_value_even_sector` |
| `Abstract.sum_weight_lt_of_ne_univ` | 重みが狭義正なら `∑ w_i(χ_T(i)-1/2)` は `T = univ` でのみ最大 | `max_eigenvalue_of_V_plus_simple` (2) |
...
| `CheckFermiSetup.nOp_eq_numPaired` | `ň_μ` が抽象版 `numPaired` の特殊化であることの確認 | 同上 |
...
| `VPlus` / `VPlusInv` | `V^{(+)}`（`= Vmat M K1 (-1) s2 K2star`）とその逆行列 | `def_V_plus_and_T_V_plus` |
...
| 人手証明のラベル | 具体版 | 抽象版 |
| --- | --- | --- |
| `def_check_number_operator` | `CheckFermiSetup.nOp` | `Abstract.numPaired`（= `Abstract.num` に付け替えを噛ませたもの） |
| `check_number_operator_idempotent` | `CheckFermiSetup.cre_sq` / `ann_sq` / `ann_mul_cre` / `nOp_mul_self` | `Abstract.sq_eq_zero_of_acomm_self` / `ann_mul_cre` / `num_mul_num`（**章 009 と同一**） |
| `check_number_operators_commute` | `CheckFermiSetup.commute_*` | `Abstract.commute_cre_num` / `commute_ann_num` / `commute_num_num`（**章 009 と同一**） |
| `trace_of_check_number_operator_product` | `CheckFermiSetup.trace_nOpProd` | `Abstract.two_pow_smul_tau_noncommProd` |
| `check_joint_eigenspace_decomposition` (1)(2)(3)(4) | `CheckFermiSetup.Qproj_*` / `sum_Qproj` / `nOp_mul_Qproj` / `trace_Qproj` | `Abstract.projOn_*` / `sum_projOn` / `num_mul_projOn` / `two_pow_smul_tau_projOn`（**章 009 と同一**） |
| `eigenvalues_of_check_Vprime` | `CheckFermiSetup.Vprime_mul_Qproj` / `Qproj_mul_Vprime` | `Abstract.exp_mul_eq_of_mul_eq_smul`（章 009 と同一）／その左右反転 `Abstract.mul_exp_eq_of_mul_eq_smul` |
...
3. `V_plus_is_positive_definite` の statement は `(V^{(+)})^{-1}` も正定値だと述べるが、
