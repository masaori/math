...
- 抽象版: `lean/Ising2D/Abstract/TVActionSandwich.lean`（新規は 1 本だけ。理由は 2 節）
...
| `Ising2D.TV_hatZ_hatY_via_sandwich` | **整数運動量版（008 章）を同じ抽象版から導いた別証明** | `<T_V_hatZ_hatY>`（008 章） |

### 抽象版

| Lean の名前 | 内容 | 人手証明のラベル |
| --- | --- | --- |
| `Ising2D.Abstract.actsBy_sandwich` | `T_1` が `B_1`、`T_2` が `B_2` で作用するなら `T_1∘T_2∘T_1` は `B_1B_2B_1` で作用する（ℂ 上の任意の加群・任意の ℂ 線型写像） | `T_V_plus_check_Z_Y` / `calc_of_TxT_check_Z_Y` / `linearity_of_T_on_check_Z_Y` |
| `Ising2D.Abstract.actsBy_TV_sandwich` | 上を `T_{(V)} = T_{g_1}∘T_{g_2}∘T_{g_1}` の形で述べた版（任意の ℂ-代数の単元） | 同上 |
...
**本章の主要主張はすべて既存の抽象版の特殊化で閉じた。** 新しく足したのは、
原文が `T_actions → linearity → calc_of_TxT → linearity → calc_of_TxT` と
4 段で往復している部分を 1 本にまとめた `Abstract.actsBy_sandwich` だけである。

| 人手証明のラベル（章 014） | 具体版 | 抽象版 |
| --- | --- | --- |
...
| `nesting_of_commutator_of_H_and_check_Z` | `Ising2D.nesting_H1Plus_checkZ_even` ほか 8 本 | 既存の `Abstract.adCLM_pow_even` / `adCLM_pow_odd_z` / `adCLM_pow_odd_y` |
| `cosh_sinh_coefficient_conversion_for_check` | `Ising2D.conversion_H1Plus_checkZ_even` ほか 8 本 | 同上（`(α, β, s)` を付け替えただけ） |
| `extract_taylor_coefficient_of_check_Z_Y` | `Ising2D.extract_taylor_H1Plus_checkZ` ほか 4 本 | 既存の `Abstract.exp_conj_two_dim_z` / `exp_conj_two_dim_y` |
| `T_actions_on_check_Z_Y` / `calc_of_TxT_check_Z_Y` | `Ising2D.actsBy_TConj_V1halfPlus` / `actsBy_TConj_V2_check` ほか | 既存の `Abstract.twoDimConjMat` / `exp_conj_two_dim_actsBy` / `conj_smul_eq` |
| `linearity_of_T_on_check_Z_Y` | `Ising2D.linearity_of_T_on_check` ほか | 既存の `Ising2D.TConj_linear`（任意の環 + ℂ-代数） |
| `def_B1_theta_B2` | 既存の `Ising2D.B1mat` / `B2mat`（最初から `θ : ℂ` 一般） | 既存の `Abstract.twoDimConjMat` |
| `factorization_of_A_theta_general` | 既存の `Ising2D.B1_mul_B2_mul_B1_eq_AMat`（最初から `θ : ℝ` 一般） | （2×2 行列の計算そのもので、取り払える構造が無い） |
| **`T_V_plus_check_Z_Y`** | **`Ising2D.TVPlus_checkZ_checkY`** | **`Abstract.actsBy_sandwich` / `actsBy_TV_sandwich`（新規）** |

### 抽象版で判明した本質（README 4 節が求めている答え）

**整数運動量版（008 章）と半整数運動量版（本章）は、同じ抽象版の別の特殊化である。**
...
**008 章の結論を本章の抽象版から導き直した**。本章の結論
`Ising2D.TVPlus_checkZ_checkY` とこの定理は、`Abstract.actsBy_TV_sandwich` に渡す
`ActsBy` が `(Ẑ^{(-)}, Ŷ, B_1(θ_μ))` か `(Ž, Y̌, B_1(θ~_μ))` かだけが違う。

さらに、抽象版から次の 3 点が分かった。
...
1. **原文が「入れ子交換子の偶奇の場合分け」（`nesting_of_commutator_of_H_and_check_Z`）と
   ...
   抽象版では `(α, β, s)` の付け替えに完全に吸収される。原文で
...
2. **原文が 4 段の往復（`T_actions` → 線型性 → `calc_of_TxT` → 線型性 → `calc_of_TxT`）で
   ...
   （`Abstract.actsBy_sandwich` は ℂ 上の任意の加群と任意の ℂ 線型写像で成り立つ）。
3. `V_2` の前因子 `(2s_2)^{M/2}` が共役で打ち消えることに効いているのは、
   任意の ℂ-代数で成り立つ `(c g) a (c⁻¹ g⁻¹) = g a g⁻¹`（既存の `Abstract.conj_smul_eq`）
