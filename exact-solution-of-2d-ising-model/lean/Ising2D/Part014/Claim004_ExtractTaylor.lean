...
`Ising2D.NecSuf.exp_conj_two_dim_z` / `exp_conj_two_dim_y`
...
`cosh`, `sinh` のテイラー展開と突き合わせている。必要十分版ではこの突き合わせが
`Complex.hasSum_cosh` と `Ising2D.NecSuf.hasSum_sinhc` に 1 対 1 で対応する
（`NecSuf/ExpConjugation.lean` の `exp_conj_two_dim_z` の証明）。

`sinh(s)/s` は `s = 0` で 0 割りになるので、Lean では `Ising2D.NecSuf.sinhc`
（`s = 0` では `1`）を経由し、最後に `s · sinhc(s) = sinh(s)`（`NecSuf.mul_sinhc`）で
