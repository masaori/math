...
**抽象版**は `Ising2D/Abstract/TVActionSandwich.lean` の
`Ising2D.Abstract.actsBy_TV_sandwich`（`exp(x₁) · c exp(x₂) · exp(x₁)` の共役の
...
theorem TVPlus_checkZ_checkY (hM : M ≠ 0) (K : IsingConst) (μ : ℤ) (K1 K2star : ℂ)
  ...
  have h := Abstract.actsBy_TV_sandwich (actsBy_TConj_V1halfPlus hM K1 μ)
...
/-! ## 整数運動量版と半整数運動量版は同じ抽象版の別の特殊化であること

008 章の結論 `<T_V_hatZ_hatY>`（既存の `Ising2D.TV_hatZ_hatY`）も、
本章の結論と**まったく同じ抽象版 `Abstract.actsBy_TV_sandwich` の特殊化**として得られる。
渡すものが `(hat(Z)^{(-)}_μ, hat(Y)_μ, B_1(θ_μ))` か
`(check(Z)_μ, check(Y)_μ, B_1(θ~_μ))` かの違いしかない。 -/

/-- **整数運動量版（008 章 `<T_V_hatZ_hatY>`）を同じ抽象版から導いたもの**。
既存の `Ising2D.TV_hatZ_hatY` と同じ主張である（`Abstract.actsBy_TV_sandwich` 経由の別証明）。 -/
theorem TV_hatZ_hatY_via_sandwich (hM : M ≠ 0) (K : IsingConst) (μ : ℤ) (K1 K2star : ℂ)
  ...
  have h := Abstract.actsBy_TV_sandwich (actsBy_TConj_V1half hM K1 μ)
