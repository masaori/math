...
**抽象版は置かない。** この主張は `S_1^{(+)}, S_2` というこの模型の具体的な対象についての
...
/-- **原文 `def_V_plus_and_T_V_plus` の `V^{(+)}`**:
`V^{(+)} = (V_1^{(+)})^{1/2} V_2 (V_1^{(+)})^{1/2}`。
`H_1^{(+)} = Ising2D.H1 M (-1)` なので、章 009 の `Vmat` に `η = -1` を代入したものである。 -/
noncomputable def VPlus (M : ℕ) (K1 : ℂ) (s2 : ℝ) (K2star : ℂ) : TensorPow M :=
  Vmat M K1 (-1) s2 K2star
...
theorem VPlus_mul_VPlusInv {K1 K2star : ℂ} {s2 : ℝ} (hs2 : 0 < s2) :
    VPlus M K1 s2 K2star * VPlusInv M K1 s2 K2star = 1 :=
  Vmat_mul_VmatInv K1 (-1) hs2 K2star

theorem VPlusInv_mul_VPlus {K1 K2star : ℂ} {s2 : ℝ} (hs2 : 0 < s2) :
    VPlusInv M K1 s2 K2star * VPlus M K1 s2 K2star = 1 :=
