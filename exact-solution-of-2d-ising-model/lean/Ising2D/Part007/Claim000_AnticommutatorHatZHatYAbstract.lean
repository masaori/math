/-
# `hat(Z)`, `hat(Y)` の反交換関係 — 具体版を抽象版の特殊化として導出する
...
| **抽象版** | `Abstract.acomm_fourier_clifford` / `acomm_fourier_clifford_flip`（`Abstract/FourierClifford.lean`） | 体 `K` 上の任意の環 `A`、Clifford 関係、`ζ` が 1 の原始 `M` 乗根、両側の重みの積 |
...
/-! ## `hat(Z)`, `hat(Y)` を抽象版の形（重み × 原始根のべき）に書き直す -/
...
/-- `deltaMod` を抽象版の右辺（`if M ∣ (-μ + -ν) then 1 else 0`）と同一視する。 -/
...
/-- **原文 1（複号同順）を抽象版の特殊化として導いた形**:
`[hat(Z)_μ^{(±)}, hat(Z)_ν^{(±)}]₊ = 2M δ^M_{μ+ν,0} I`。

抽象版 `Abstract.acomm_fourier_clifford` に `K := ℂ`, `A := TensorPow M`, `x = y := Z`,
`ζ := zetaM M`, `u = v := firstSign η` を代入したもの。
重みについて使うのは `firstSign η j * firstSign η j = 1`（原文の `η^2 = 1`）だけである。 -/
theorem acomm_hatZ_hatZ_same_of_abstract (hM : M ≠ 0) {η : ℂ} (hη : η * η = 1) (μ ν : ℤ) :
  ...
  rw [hatZ_eq_weighted_zetaM, hatZ_eq_weighted_zetaM,
    Abstract.acomm_fourier_clifford hM (isPrimitiveRoot_zetaM hM) Z Z acomm_Z_Z_clifford
      (firstSign η) (firstSign η) huv (-μ) (-ν),
    deltaMod_eq_ite_neg]

/-- **原文 4 を抽象版の特殊化として導いた形**: `[hat(Y)_μ, hat(Y)_ν]₊ = 2M δ^M_{μ+ν,0} I`。 -/
theorem acomm_hatY_hatY_of_abstract (hM : M ≠ 0) (μ ν : ℤ) :
    acomm (hatY M μ) (hatY M ν)
      = (2 * (M : ℂ) * deltaMod M (μ + ν) 0) • (1 : TensorPow M) := by
  rw [hatY_eq_weighted_zetaM, hatY_eq_weighted_zetaM,
    Abstract.acomm_fourier_clifford hM (isPrimitiveRoot_zetaM hM) Y Y acomm_Y_Y_clifford
      (fun _ => (1 : ℂ)) (fun _ => (1 : ℂ)) (fun _ => one_mul 1) (-μ) (-ν),
    deltaMod_eq_ite_neg]

/-- **原文 2（複号逆）を抽象版の特殊化として導いた形**:
`[hat(Z)_μ^{(+)}, hat(Z)_ν^{(-)}]₊ = (2M δ^M_{μ+ν,0} - 4 exp(-2π√-1(μ+ν)/M)) I`。

抽象版 `Abstract.acomm_fourier_clifford_flip` に同じ代入をしたもの。重みについて使うのは
「積が `j = 1` でだけ `-1`、他では `1`」（原文の「`j = 1` の項の符号だけが反転する」）だけ。 -/
theorem acomm_hatZPlus_hatZMinus_of_abstract (hM : M ≠ 0) (μ ν : ℤ) :
  ...
  rw [hatZPlus_def, hatZMinus_def, hatZ_eq_weighted_zetaM, hatZ_eq_weighted_zetaM,
    Abstract.acomm_fourier_clifford_flip hM (isPrimitiveRoot_zetaM hM) Z Z acomm_Z_Z_clifford
