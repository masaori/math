...
theorem exp_pi_apply (d : ι → ℂ) (I : ι) :
  ...
  exact NecSuf.map_exp_of_continuous (Pi.evalRingHom (fun _ : ι => ℂ) I)
...
このうち 2 は**必要十分版 `Ising2D.NecSuf.map_exp_of_continuous` の特殊化そのもの**
（連続な環準同型 `Pi.evalRingHom` に適用したもの）であり、上の証明で実際にそう導いてある。

1 も原理的には同じ必要十分版（`diagonal` が連続な環準同型であること）から出るが、
