...
本ファイルの (1)(2)(3) はすべて抽象版の**系として**導いてある。

## 人手証明との対応

| 人手証明 | 本ファイル |
| --- | --- |
| `u_k := |x_k|` | `Ising2D.Abstract.absVec` |
...
theorem epsilonR_mulVec_absVec {x : Conf M → ℝ}
    ...
    epsilonR M *ᵥ Abstract.absVec x = (1 : ℝ) • Abstract.absVec x :=
  Abstract.permMat_mulVec_absVec hx

/-- **人手証明 (2) `‖u‖ = ‖x‖`**（`‖·‖²` の形）。 -/
theorem vecNormSq_absVec_eq (x : Conf M → ℝ) :
    vecNormSq (Abstract.absVec x) = vecNormSq x :=
  Abstract.vecNormSq_absVec x

/-- **人手証明 (3) の第 1 の不等号** `uᵀWu ≥ |xᵀWx|`。 -/
theorem abs_quad_le_quad_absVec_epsilonR {W : Matrix (Conf M) (Conf M) ℝ}
    (hW : ∀ k l, 0 < W k l) (x : Conf M → ℝ) :
    |x ⬝ᵥ W *ᵥ x| ≤ Abstract.absVec x ⬝ᵥ W *ᵥ Abstract.absVec x :=
  Abstract.abs_quad_le_quad_absVec (fun k l => (hW k l).le) x

/-- **人手証明 (3)** `uᵀWu ≥ xᵀWx`（人手証明どおり `W_{kl} > 0` を仮定した版）。 -/
theorem quad_le_quad_absVec_epsilonR {W : Matrix (Conf M) (Conf M) ℝ}
    (hW : ∀ k l, 0 < W k l) (x : Conf M → ℝ) :
    x ⬝ᵥ W *ᵥ x ≤ Abstract.absVec x ⬝ᵥ W *ᵥ Abstract.absVec x :=
  Abstract.quad_le_quad_absVec (fun k l => (hW k l).le) x

/-- 上の**非負版**（抽象版で分かった、本当に効いている仮定）。 -/
theorem quad_le_quad_absVec_epsilonR_of_nonneg {W : Matrix (Conf M) (Conf M) ℝ}
    (hW : ∀ k l, 0 ≤ W k l) (x : Conf M → ℝ) :
    x ⬝ᵥ W *ᵥ x ≤ Abstract.absVec x ⬝ᵥ W *ᵥ Abstract.absVec x :=
  Abstract.quad_le_quad_absVec hW x
...
theorem exists_even_sector_unit_ge {W : Matrix (Conf M) (Conf M) ℝ}
    ...
    ∃ u : Conf M → ℝ, epsilonR M *ᵥ u = (1 : ℝ) • u ∧ vecNormSq u = 1 ∧
      x ⬝ᵥ W *ᵥ x ≤ u ⬝ᵥ W *ᵥ u :=
  ⟨Abstract.absVec x, epsilonR_mulVec_absVec hx,
