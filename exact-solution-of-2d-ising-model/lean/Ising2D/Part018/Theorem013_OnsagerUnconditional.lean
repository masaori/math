...
| `bridge` | 章 011 の実行列 `W` と `V^{(+)}` の橋渡し（`W P^{(+)} = V^{(+)} P^{(+)}`、`V^{(+)}` が実行列であること） | 章 011 の `symmetrized_transfer_matrix_on_sectors` は Lean 未形式化。`V₁` の固有空間制限と下流のセクター置換は形式化済みで、実行列 `W` と複素 `TensorPow` 上の物理的転送行列の同一視、および既存の `Vsym`・`epsProj` への最終接続が残る |
