...
## 未証明の穴（`commutation_V_psi` を無条件には証明できない理由）

`T_{(V)}` が `(hat(Z)_μ^{(-)}, hat(Y)_μ)` に行列 `A(θ_μ)` で作用すること
（原文 `T_V_hatZ_hatY`）は、`parts 008` の 001〜005（`exp(X)Y exp(-X)` の級数展開と
**ネストした交換子のテイラー係数抽出**）に依存し、本リポジトリでは未形式化である。
`Part008/Definition016_TV.lean` と同じ方針で、本ファイルでも
`hT : ActsBy T (hat(Z)_μ^{(-)}) (hat(Y)_μ) (AMat K (thetaMu M μ))` を**明示的な仮定**として持つ。
未証明のまま穴埋めすることはしない（証明の穴を残すのではなく、明示的な仮定として
関数の引数に持ち上げてある）。仮定から先（`P_μ` の列が固有ベクトルであること、
したがって `ψ_μ^†, ψ_μ` が `T` の固有ベクトルであること）は完全に証明されている。
-/
