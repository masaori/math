...
## 仮定 `hbr` について（**当初「原文の穴」と書いたのは誤り。訂正済み**）

本ファイルの反交換関係は仮定 `hbr : (M : ℤ) ∣ (μ + ν) → tν = tμ` を持つ。
**これは原文の穴ではなく、本ファイルの形式化方針に由来する仮定である。**

当初この冒頭コメントは「原文 `anticommutator_of_psi` は平方根の分枝の一致を暗黙に仮定しており、
それが原文の穴である」と書いていたが、**これは誤り**であった。原文の `√` は
`calculation_formulae_038_definition_sqrt_of_complex_number`（ラベル `def_sqrt_cc`）で
偏角を `arg^{[0,2π)}` に取った**単一値の写像 `ℂ → ℂ`** として定義されており、
「同じ複素数に 2 つの `√` の値がありうる」という事態は原文の枠組みでは起こらない。
したがって根号の中身が複素数として等しいことさえ言えれば `t_ν = t_μ` が従う。

本ファイルは `t` を「`t^2 = γ_2(θ_μ)γ_2(-θ_μ)` を満たす任意の複素数」として受け取る
（分枝関数を導入しない）方針を採っているため、`t_ν = ±t_μ` の自由度が**形式化の側で**生じる。
`hbr` はその自由度を潰すためのものであって、原文に欠けている仮定ではない。

なお、この検討の過程で**原文に実際の飛躍が 1 段あった**ことは事実である。原文は
「`μ + ν ≡ 0 (mod M)` すなわち `θ_ν = -θ_μ`」と書いていたが、正しくは `θ_ν = -θ_μ + 2kπ` であり、
根号の中身が一致する根拠は `γ_2` の `2π` 周期性（`gamma2_thetaMu_of_dvd`）である。
この点は**本文側で既に修正済み**（詳細は
`docs/tasks/2026-07_original-text-gaps/050_psi_anticommutator_sqrt_branch.md`）。

分枝が違えば結論が成り立たないこと自体は正しく、検算は次のとおりである。
`c_μ := 1/(2√M γ_2(-θ_μ))` として `[ψ_μ^†, ψ_ν^†]₊` の係数は
`c_μ c_ν(-t_μ t_ν + γ_2(-θ_μ)γ_2(-θ_ν)) · 2Mδ` であり、`M ∣ μ+ν` のとき
`γ_2(-θ_μ)γ_2(-θ_ν) = γ_2(θ_μ)γ_2(-θ_μ) = t_μ^2` だから括弧は `t_μ(t_μ - t_ν)`。
`γ_2(θ_μ) ≠ 0` より `t_μ^2 = -|γ_2(θ_μ)|^2 ≠ 0` すなわち `t_μ ≠ 0` なので、
`t_ν = t_μ` なら `0`（原文どおり）、`t_ν = -t_μ` なら `2t_μ^2 ≠ 0` となって成り立たない。
