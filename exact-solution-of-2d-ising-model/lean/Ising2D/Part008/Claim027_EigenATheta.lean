...
1. **`det_A_theta`（`TV1_hatZ_hatY_035`）の `det A(θ_μ) = 1` は追加の代数関係を要する。**
   `A(θ)` の定義だけから無条件に言えるのは
   ...
   （`det_AMat_eq_one`）:
     ...
     (iii) `c_2 s_2^* = c_2^*`（双対関係 `sinh(2K_2) sinh(2K_2^*) = 1` から従う
           `cosh(2K_2) sinh(2K_2^*) = cosh(2K_2^*)`）
   原文は `det B_1 = det B_2 = 1` から (i)(ii) は使っているが、(iii)
   （`A(θ)` の定義に現れる `c_2` と、`B_1, B_2` に現れる `c_2^*` を結びつける関係）を
   明示していない。実際 (iii) を落とすと `det A` は `θ` に依存し、`1` にならない。
2. **`eigenvector_of_A_theta` の場合分け 1) の「この場合 `A(θ_μ) = I`」も追加仮定を要する。**
   `γ_2(θ) = 0` から言えるのは `A(θ) = γ_1(θ) I`（`AMat_of_gamma2_eq_zero`）までで、
   上の (i)(ii)(iii) を足すと `γ_1(θ)^2 = 1`、すなわち `A(θ) = ±I` までしか出ない
   （`gamma1_sq_eq_one_of_gamma2_eq_zero`）。`+I` を選ぶには `γ_1 ≥ 1`
   （原文 `TV1_hatZ_hatY_036_claim_gamma1_geq_1`）が別途必要である。
-/
