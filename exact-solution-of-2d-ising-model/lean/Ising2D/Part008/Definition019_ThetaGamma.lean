...
1. **`gamma_2_theta_is_0`（`TV1_hatZ_hatY_022`）は `s_2^* ≠ 0` を暗黙に仮定している。**
   原文は `γ_2(θ_μ) = 0 ⟺ {sin θ_μ = 0, c_2 s_1 - c_1 cos θ_μ = 0}` と書くが、
   `γ_2` は `s_2^*` を因子に持つので `s_2^* = 0` のときは右辺と無関係に `γ_2 ≡ 0` である。
   正しい同値は `γ_2(θ) = 0 ⟺ s_2^* = 0 ∨ (sin θ = 0 ∧ c_1 cos θ - s_1 c_2 = 0)`
   （`gamma2_eq_zero_iff`）。原文の形は `s_2^* ≠ 0` の下でのみ成り立つ
   （`gamma2_eq_zero_iff_of_s2star_ne_zero`）。
2. **`gamma_2_theta_is_0` の最後の同値 `θ_μ = 0, ±π, ±2π, … ⟺ μ = ±M` は誤り。**
   `θ_μ = 2πμ/M` に対し `sin θ_μ = 0 ⟺ M ∣ 2μ` であり（`sin_thetaMu_eq_zero_iff`）、
   `ℳ = {-M,…,-1,1,…,M}` の範囲でこれを満たすのは `μ = ±M` だけでなく、
   `M` が偶数のときは `μ = ±M/2`（`θ_μ = ±π`）もある。原文はこの場合を落としている。
   なお `μ = ±M/2` では `cos θ_μ = -1` なので残る条件は `c_1 = -s_1 c_2` となり、
   `c_1, s_1, c_2 > 0`（`cosh`, `sinh` の正値性）を使えば排除できるが、
   原文の当該ブロックはその正値性を述べていない。
-/
