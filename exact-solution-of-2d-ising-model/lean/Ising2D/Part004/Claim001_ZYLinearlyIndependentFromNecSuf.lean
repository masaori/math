/-
# `Z_m, Y_m` の線型独立性 — 具体版を必要十分版の特殊化として導出する
...
| **必要十分版** | `NecSuf.linearIndependent_of_clifford_necSuf`（`NecSuf/CliffordIndependence.lean`） | 任意の可換環 `S` 上の任意の環 `A` の族が Clifford 関係を満たすこと＋スカラーの忠実性＋`2` の非零因子性 |

本ファイルは、**具体版が必要十分版の特殊化にすぎないこと**を実際の導出として書く
（`ZY_linearIndependent_of_necSuf`）。特殊化で埋めるべき仮定は 2 つだけで、
どちらも複素行列に固有の事実である。
...
（必要十分版の仮定 `hfaithful` を複素行列について埋める部分。） -/
...
/-- **具体版 `ZY_linearIndependent` を必要十分版の特殊化として導出した形**。

必要十分版 `NecSuf.linearIndependent_of_clifford_necSuf` に
`S := ℂ`, `A := TensorPow M`, `e := ZY`（`<anticommutator_of_Z_and_Y>` の 3 式をまとめた
Clifford 関係 `acomm_ZY`）を代入したものが、そのまま原文の主張になる。 -/
theorem ZY_linearIndependent_of_necSuf (M : ℕ) :
    LinearIndependent ℂ (ZY : Fin M ⊕ Fin M → TensorPow M) :=
  NecSuf.linearIndependent_of_clifford_necSuf ZY acomm_ZY
