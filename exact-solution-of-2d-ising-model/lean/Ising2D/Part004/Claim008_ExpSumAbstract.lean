...
| **抽象版** | `Abstract.sum_zpow_primitiveRoot`（`Abstract/RootOfUnitySum.lean`） | 任意の体 `K` と、その中の 1 の原始 `M` 乗根 `ζ` |

本ファイルは、**具体版が抽象版の特殊化にすぎないこと**を導出として書く
（`expPhase_sum_of_abstract`）。特殊化で埋めるべきなのは
...
/-- 原文の位相因子を、添字の符号を反転した形の整数べきで書く（抽象版へ渡す形）。 -/
...
/-- **具体版 `expPhase_sum` を抽象版の特殊化として導出した形**。

抽象版 `Abstract.sum_zpow_primitiveRoot` に `K := ℂ`, `ζ := zetaM M`, `k := -k` を代入し、
`M ∣ -k ⟺ M ∣ k` で整理すると、原文 `exp_sum` の主張がそのまま出る。 -/
theorem expPhase_sum_of_abstract {M : ℕ} (hM : M ≠ 0) (k : ℤ) :
  ...
  rw [Finset.sum_congr rfl fun j _ => hrw j,
    Abstract.sum_zpow_primitiveRoot hM (isPrimitiveRoot_zetaM hM) (-k), deltaMod, sub_zero]
  congr 1
  simp only [dvd_neg]
