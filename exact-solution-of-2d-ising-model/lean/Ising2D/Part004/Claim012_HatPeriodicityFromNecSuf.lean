/-
# `<hatZ_hatY_M_periodicity>` — 具体版を必要十分版の特殊化として導出する
...
| **必要十分版** | `NecSuf.transform_periodic`（`NecSuf/DiscreteFourier.lean`） | 任意の体 `K`、`ζ^M = 1`、任意の `K`-加群 |
...
/-- 原文の位相因子を、必要十分版が要求する形 `ζ^{a μ}` に書き換える
...
/-- **`hat(Z)` の `M` 周期性を必要十分版の特殊化として導出した形**:
`hat(Z)_{μ+M}^{(η)} = hat(Z)_μ^{(η)}`。 -/
theorem hatZ_periodic_of_necSuf (hM : M ≠ 0) (η : ℂ) (μ : ℤ) :
    hatZ M η (μ + M) = hatZ M η μ := by
  simp_rw [hatZ, expPhase_eq_zetaM_zpow_freq]
  exact NecSuf.transform_periodic hM (isPrimitiveRoot_zetaM hM)
    (fun j : Fin M => -(((j : ℕ) : ℤ) + 1)) (firstSign η) Z μ

/-- **`hat(Y)` の `M` 周期性を必要十分版の特殊化として導出した形**:
`hat(Y)_{μ+M} = hat(Y)_μ`。 -/
theorem hatY_periodic_of_necSuf (hM : M ≠ 0) (μ : ℤ) :
    hatY M (μ + M) = hatY M μ := by
  have h := NecSuf.transform_periodic hM (isPrimitiveRoot_zetaM hM)
    (fun j : Fin M => -(((j : ℕ) : ℤ) + 1)) (fun _ : Fin M => (1 : ℂ)) Y μ
  simp only [one_mul] at h
  simpa only [hatY, expPhase_eq_zetaM_zpow_freq] using h

/-- **`<hatZ_hatY_M_periodicity>` の主張（`hat(Z)^{(-)}` の側）を必要十分版から導出した形**:
`hat(Z)_M^{(-)} = hat(Z)_{-M}^{(-)}`。 -/
theorem hatZMinus_M_eq_neg_M_of_necSuf (hM : M ≠ 0) :
    hatZMinus M (M : ℤ) = hatZMinus M (-(M : ℤ)) := by
  have h1 : hatZ M 1 ((0 : ℤ) + M) = hatZ M 1 0 := hatZ_periodic_of_necSuf hM 1 0
  have h2 : hatZ M 1 ((-(M : ℤ)) + M) = hatZ M 1 (-(M : ℤ)) :=
    hatZ_periodic_of_necSuf hM 1 _
  rw [neg_add_cancel] at h2
  rw [hatZMinus_def, hatZMinus_def, ← h2, ← h1, zero_add]

/-- **`<hatZ_hatY_M_periodicity>` の主張（`hat(Y)` の側）を必要十分版から導出した形**:
`hat(Y)_M = hat(Y)_{-M}`。 -/
theorem hatY_M_eq_neg_M_of_necSuf (hM : M ≠ 0) :
    hatY M (M : ℤ) = hatY M (-(M : ℤ)) := by
  have h1 : hatY M ((0 : ℤ) + M) = hatY M 0 := hatY_periodic_of_necSuf hM 0
  have h2 : hatY M ((-(M : ℤ)) + M) = hatY M (-(M : ℤ)) := hatY_periodic_of_necSuf hM _
  rw [neg_add_cancel] at h2
  rw [← h2, ← h1, zero_add]
