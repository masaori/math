...
**(2) は形式化していない。** 理由は `Ising2D/docs/ch011-formalization.md` と
`docs/tasks/2026-07_lean-ch009-013/` の記録を参照（`V^{(±)}`, `V₁^{(±)}` は章 004/010 の
対象で、本タスクの担当範囲外の定義に依存する）。

## この主張に効いている構造（抽象版を別に置かない理由）
