...
（具体版は `Ising2D/Part004/ClaimExpOfDiagonalMatrix.lean`）

## この主張に本質的に効いている構造は何か

人手証明は
「対角行列の積と冪（冪は成分ごとの冪）→ 指数級数の部分和（部分和も対角）→ 極限（成分ごとに収束）」
という 3 段で `exp(D)_{kk} = e^{d_k}`, `exp(D)_{kl} = 0 (k ≠ l)` を示している。
この 3 段に効いているのは次の 2 点だけである。

1. **`diagonal : (ι → ℂ) → Mat(ι, ℂ)` が連続な環準同型であること。**
   「対角行列の積と冪」（冪を保つ）と「指数級数の部分和」（有限和を保つ・スカラー倍を保つ）は
   環準同型であること、「極限」（極限と交換する）は連続性そのものである。
...
人手証明 `exp_of_diagonal_matrix` の 3 つの中間目標（冪・有限部分和・極限）を
