/-
# 必要十分版: CAR から出る「生成消滅対の和」との交換子（`X̌` の梯子作用）
...
* 人手証明が `μ ∈ 𝓜̌` に絞ることで「合同式を解く段」を消しているのは、
  必要十分版では**「反交換子の値がクロネッカーのデルタになる」という仮定 `hed` / `hde` の形**に
...
/-- **人手証明 Step 2' の必要十分版**: `[X̌, e μ] = -(g μ · e μ)`。
