/-
# `log` の単調性による挟み撃ちと、`log` の分解（必要十分版）
...
* さらに `M` で割る操作は**線型な後処理**にすぎず、人手証明が得ている評価
  `|log Z/(MN) - log c/M| ≤ log 2/N` は、必要十分版の `|log Z/N - log c| ≤ log B/N` の両辺を
...
/-- **必要十分版の挟み撃ち評価**: `c^N ≤ Z ≤ B c^N`（`c > 0`, `B ≥ 1`, `N ≥ 1`）なら
