...
`Ising2D/Part013/Claim002_AntiperiodicExpSumFromNecSuf.lean` にある。
...
* **添字の周期性も既存の必要十分版のまま特殊化できる。** `NecSuf.transform_periodic`
  （重み `w_j`・周波数 `a_j` が任意）に `w_j = ξ^{-j}`, `a_j = j`, `ζ = ξ^2` を入れれば
  `check(Z)_{μ+M} = check(Z)_μ` になる。効いているのは `ζ^M = 1` の 1 点だけ。

* 逆変換（`recover_Z_Y_from_check_Z_Y`）だけは既存の `inverse_dft_necSuf`
...
/-! ## 反周期的な逆変換（`recover_Z_Y_from_check_Z_Y` の必要十分版） -/

/-- `Fin M` の 2 元 `j, m` について `M ∣ (j - m) ⟺ j = m`（`|j - m| < M` だから）。
`NecSuf.fin_dvd_sub_iff_eq`（`DiscreteFourier.lean`）と同じ主張だが、
...
/-- **必要十分版の反周期的離散フーリエ逆変換（`recover_Z_Y_from_check_Z_Y` の骨格）**。
...
`inverse_dft_necSuf`（整数運動量版、`NecSuf/DiscreteFourier.lean`）と証明の骨格は同じで、
...
/-! ## 反周期的フーリエ和どうしの反交換子（`anticommutator_of_check_Z_Y` の必要十分版） -/

section Clifford

variable {A : Type*} [Ring A] [Algebra K A]

/-- **必要十分版（`anticommutator_of_check_Z_Y` の骨格）**:
