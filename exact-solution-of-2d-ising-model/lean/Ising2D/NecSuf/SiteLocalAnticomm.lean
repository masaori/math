/-
# 必要十分版: サイト局所な反可換性は「積」全体の反交換関係へ持ち上がる
...
`Ising2D/Part006/Claim000_AnticommutatorZYFromNecSuf.lean` にある。
...
/-- **必要十分版の核心（原文の「サイト `μ` の因子だけが符号だけ違う」の中身）**。
...
/-- **必要十分版の Jordan–Wigner 文字列の族**: サイト `i` に
...
/-- **必要十分版: Jordan–Wigner 文字列どうしの反交換**（原文の計算の骨格）。
...
/-- **必要十分版: Jordan–Wigner 文字列の自乗**（原文の `μ = ν` の場合）。
