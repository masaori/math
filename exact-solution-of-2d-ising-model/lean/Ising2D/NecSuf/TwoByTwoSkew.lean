/-
# `!![g, a; -b, g]` 型の 2×2 行列の固有値・固有ベクトル・対角化（**必要十分版**）
...
/-- **必要十分版の固有ベクトル**: `s^2 = -(a b)` なら `(-s, b)` は固有値 `g + s` の固有ベクトル。 -/
...
/-- **必要十分版の対角化**: `A P = P D` かつ `det P` が単元なら `A = P D P⁻¹`。
