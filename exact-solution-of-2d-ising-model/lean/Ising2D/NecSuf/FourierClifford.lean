/-
# 必要十分版: Clifford 族の「フーリエ和」どうしの反交換子
...
`Ising2D/Part007/Claim000_AnticommutatorHatZHatYFromNecSuf.lean` にある。
...
theorem acomm_sum_smul_clifford_necSuf {ι : Type*} [Fintype ι] [DecidableEq ι]
    (c d : ι → S) (x y : ι → A)
...
theorem acomm_fourier_clifford_weights {M : ℕ} {ζ : K} (hζ0 : ζ ≠ 0)
  ...
  rw [acomm_sum_smul_clifford_necSuf _ _ _ _ h]
  congr 1
  rw [Finset.mul_sum]
...
/-- **原文 1・4 の必要十分版**（重みの積が全サイトで `1` の場合）:
...
/-- **原文 2 の必要十分版**（`j = 1` の項だけ重みの積が `-1` の場合）:
