...
`Ising2D/Part004/Claim012_HatPeriodicityFromNecSuf.lean` と
`Ising2D/Part004/Claim013_RecoverZYFromNecSuf.lean` にある。
...
（`|j - m| < M` だから）。必要十分版を複素数側から独立させるために本ファイルにも置く。 -/
...
/-- **必要十分版の周期性**: 重み `w : Fin M → K` と周波数 `a : Fin M → ℤ` をもつ変換
...
/-- **必要十分版の離散フーリエ逆変換（`recover_Z_Y_from_hatZ_hatY` の骨格）**。
...
theorem inverse_dft_necSuf {M : ℕ} (hM : M ≠ 0) {ζ : K} (hζ : IsPrimitiveRoot ζ M)
    (x : Fin M → V) (m : Fin M) :
