...
人手証明の `B = π²/(12 c₀(1+c₀))` は、この抽象版の `C b²/(c₀(1+c₀)a²)` に
`a = 1/2`, `b = π`, `C = 1/48` を代入したものと**厳密に一致する**
（`(1/48)·π²/(c₀(1+c₀)·(1/4)) = π²/(12c₀(1+c₀))`）。すなわち人手証明の定数は最良化されていないが、
抽象版と同じ値である。

`arsinh(y) - log(2y) ∈ [0, 1/(4y²)]` は `Ising2D/Abstract/HyperbolicBounds.lean` にある。
-/
...
/-- **抽象版（`closed_form_log_integral` (1)）**:
...
/-- `∫_0^b dθ/(δ² + a²θ²)^{3/2} = b/(δ²√(δ²+a²b²))`（**抽象版**、`closed_form_log_integral` (3)）。
