/-
# 共役添字の対合で対をなすフェルミオン（**抽象版**）

対応する人手証明のラベル（具体版は `Ising2D/Part017/`）:

- `def_check_number_operator`（`Ising2D.Abstract.numPaired`）
- `check_number_operator_idempotent` / `check_number_operators_commute`
  （**新しい抽象版は不要**。`Ising2D/Abstract/NumberOperator.lean` の
  `num_mul_num` / `commute_num_num` がそのまま使える）
- `trace_of_check_number_operator_product`（`Ising2D.Abstract.two_pow_smul_tau_noncommProd`）
...
`Ising2D/Abstract/NumberOperator.lean` と `Ising2D/Abstract/JointEigenspace.lean` の
定理群は、`(c, a')` に対してそのまま適用できる。

**つまり、整数運動量版と半整数運動量版は同じ抽象版の別の特殊化であり、
...
新しい内容を持たない。既存の `Abstract.two_pow_smul_tau_projOn` を
...
/-- **人手証明 `trace_of_check_number_operator_product` の抽象版**:
`2^{|s|} τ(∏_{i∈s} n_i) = τ(1)`。

`Abstract.two_pow_smul_tau_projOn` を `T = s` と特殊化したものである
