/-
# 抽象版: 「基底を全部含む部分多元環は全体」と「サイトごとの元の積は部分多元環に入る」
...
`Ising2D/Part004/Claim014_ZYGenerateAlgebraAbstract.lean`）。
...
/-- **原文 Step 2 の帰納法（抽象版）**: 文字列 `p n` は部分多元環 `S` に属する。 -/
...
/-- **原文 Step 2 の結論（抽象版）**: 局所元 `σ^z_n, σ^y_n, σ^x_n` はすべて `S` に属する。 -/
