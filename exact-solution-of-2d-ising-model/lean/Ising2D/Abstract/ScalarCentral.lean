...
/-! ## `<scalar_identity_commutes>` の抽象版 -/

section ScalarCommutes

variable {S : Type*} [CommSemiring S]

/-- **`<scalar_identity_commutes>` の抽象版（半環）**。
...
/-- **`<scalar_identity_commutes>` の抽象版（環・差の形）**。
...
/-- **`<centralizer_is_scalar>` の抽象版（係数は任意の半環）**。
...
/-- **`<centralizer_is_scalar>` の抽象版（係数が可換な場合）**。
