...
**章 009 と章 017 は同じ構造なので、本ファイルの抽象版 1 本を両章の特殊化として使う。**
...
抽象版を書いてはじめて分かること: 原文が (1) で 2 つに分けて主張している性質のうち、
冪等性は独立な仮定ではない。 -/
...
/-- **人手証明 `joint_eigenspace_decomposition` (5) の `DirectSum.IsInternal` 形（抽象版）**:
