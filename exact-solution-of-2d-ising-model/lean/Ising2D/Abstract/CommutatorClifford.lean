/-
# Clifford 型の反交換関係から交換関係を出す計算（**抽象版**）
...
## このファイルの目的（`lean/README.md`「具体版と抽象版の 2 本立て」）
...
**抽象版は Lean の中だけに置く**（`exact-solution-of-2d-ising-model/README.md` 4 節）。
...
/-- **抽象版の核**: 積の交換子は、2 つの反交換子がスカラー倍の `1` になるなら
線型結合に潰れる。
