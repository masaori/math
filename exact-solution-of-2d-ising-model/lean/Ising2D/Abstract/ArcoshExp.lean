/-
# `g^2 = 1 + r^2` から `e^{±arcosh g} = g ± r` を出す（**抽象版**）
...
詳細は `Ising2D/Abstract/Arcosh.lean` 冒頭を参照（`lean/README.md` の
...
/-- **抽象版 Step 1**: `1 ≤ g`, `0 ≤ r`, `g^2 = 1 + r^2` なら `sinh (arcosh g) = r`。 -/
...
/-- **抽象版**: `1 < g` なら `arcosh g > 0`（人手証明 `def_gamma_theta_tilde_mu` の末尾）。
