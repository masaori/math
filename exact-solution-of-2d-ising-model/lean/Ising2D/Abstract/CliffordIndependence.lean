...
`Ising2D/Part004/Claim001_ZYLinearlyIndependentAbstract.lean` にある。
...
/-- **抽象版の本体**: Clifford 関係 `[e_a, e_b]₊ = 2 δ_{ab} · 1` を満たす族は線型独立。

仮定は Clifford 関係のほかに、スカラーの忠実性 `hfaithful` と `2` の非零因子性 `hregular`
だけである（台が行列であることも、係数が複素数であることも使わない）。 -/
theorem linearIndependent_of_clifford_abstract {ι : Type*} [Fintype ι] [DecidableEq ι]
    (e : ι → A)
    (hcl : ∀ a b, acomm (e a) (e b) = (if a = b then (2 : S) else 0) • (1 : A))
