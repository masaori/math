...
- `def_number_operator`（`Ising2D.Abstract.num`）
- `number_operator_idempotent`（`Ising2D.Abstract.num_mul_num`）
- `number_operators_commute`（`Ising2D.Abstract.commute_num_num`）
- `trace_of_number_operator_product`（`Ising2D.Abstract.two_mul_tau_num_mul`）
