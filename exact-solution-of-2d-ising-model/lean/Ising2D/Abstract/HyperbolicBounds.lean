...
効いているのはこのうち `arsinh y - log(2y) = O(1/y²)` だけである**（`Abstract/LogDivergentIntegral.lean`）。
-/
...
/-- **抽象版**: `1 - y ≤ exp (-y)` から出る `exp y ≤ (1-y)⁻¹`（`y < 1`）。 -/
