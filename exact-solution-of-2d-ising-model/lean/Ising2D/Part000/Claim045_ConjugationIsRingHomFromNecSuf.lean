...
必要十分版は `Ising2D/NecSuf/Conjugation.lean`（`Ising2D.NecSuf.Conj.*`）にある。

本ファイルは、**具体版（`Mat(n,ℂ)` と `Matrix.inv` による共役）を必要十分版の系として導出する**
（`exact-solution-of-2d-ising-model/README.md` 4 節の規約）。

導出の要点は、必要十分版が仮定を最小まで削ってあるので、行列側で用意すべき事実が
次の 3 つに限られることである。
...
/-- 既存の具体版 `T`（一般の環 `R` と単元 `B : Rˣ`）は、必要十分版の挟み込み写像そのものである。 -/
theorem T_eq_sandwich {R : Type*} [Ring R] (B : Rˣ) (A : R) :
    T B A = Conj.sandwich (B : R) ((B⁻¹ : Rˣ) : R) A := rfl

/-- 既存の具体版 `T` は、必要十分版のモノイド準同型としての共役と一致する
...
/-- **(1) 乗法性**（原文の記法）を必要十分版 `Conj.sandwich_mul` の系として得る。
使うのは `B⁻¹ * B = 1` の 1 本だけである。 -/
theorem matrix_conj_mul_of_necSuf (B : Matrix n n ℂ) (hB : IsUnit B.det) (A C : Matrix n n ℂ) :
    B * (A * C) * B⁻¹ = (B * A * B⁻¹) * (B * C * B⁻¹) :=
  Conj.sandwich_mul (b := B) (u := B⁻¹) (nonsing_inv_mul B hB) A C

/-- **(2) 単位性**（原文の記法）を必要十分版 `Conj.sandwich_one` の系として得る。
使うのは `B * B⁻¹ = 1` の 1 本だけである。 -/
theorem matrix_conj_one_of_necSuf (B : Matrix n n ℂ) (hB : IsUnit B.det) :
    B * 1 * B⁻¹ = 1 :=
  Conj.sandwich_one (b := B) (u := B⁻¹) (mul_nonsing_inv B hB)

/-- **(3) 合成則**（原文の記法）を必要十分版 `Conj.sandwich_comp` の系として得る。
必要十分版の側には仮定が無く、行列側で足すのは `(AB)⁻¹ = B⁻¹A⁻¹` の書き直しだけである
（これも正則性を要さない）。 -/
theorem matrix_conj_comp_of_necSuf (A B X : Matrix n n ℂ) :
    A * (B * X * B⁻¹) * A⁻¹ = (A * B) * X * (A * B)⁻¹ := by
  rw [Matrix.mul_inv_rev]
  exact Conj.sandwich_comp A A⁻¹ B B⁻¹ X

/-- 正則行列 `B` を単元 `(Matrix n n ℂ)ˣ` として見たときの逆元は `Matrix.inv` と一致する。
必要十分版（単元で述べてある）と原文の記法（`Matrix.inv`）をつなぐ橋。 -/
@[simp]
...
（必要十分版 `Conj.conjRingAutHom` の特殊化）。 -/
...
必要十分版 `Conj.sandwich_add` の系であり、**正則性を一切使わない**。 -/
theorem matrix_conj_add_of_necSuf (B : Matrix n n ℂ) (A C : Matrix n n ℂ) :
    B * (A + C) * B⁻¹ = B * A * B⁻¹ + B * C * B⁻¹ :=
