...
**抽象版**: `Ising2D/Abstract/ArcoshExp.lean`
（`Ising2D.Abstract.sinh_arcosh_of_sq` / `exp_arcosh_of_sq` / `exp_neg_arcosh_of_sq` /
...
* `arccosh` は mathlib の `Real.arcosh` を使う（`lean/README.md` の
  ...
  `Ising2D/Abstract/Arcosh.lean` 冒頭。綴りは `arcosh`）。
...
theorem gammaTilde_pos (hdual : P.const.c2 * P.const.s2star = P.const.c2star)
  ...
  Abstract.arcosh_pos_of_one_lt (one_lt_gamma1R_thetaTilde P hdual hM μ)
...
theorem sinh_gammaTilde (hdual : P.const.c2 * P.const.s2star = P.const.c2star)
  ...
  Abstract.sinh_arcosh_of_sq (one_le_gamma1R P (thetaTilde M μ))
...
theorem lambdaPlusR_eq_exp (hdual : P.const.c2 * P.const.s2star = P.const.c2star)
  ...
  rw [lambdaPlusR]
  exact (Abstract.exp_arcosh_of_sq (one_le_gamma1R P (thetaTilde M μ))
...
theorem lambdaMinusR_eq_exp (hdual : P.const.c2 * P.const.s2star = P.const.c2star)
  ...
  rw [lambdaMinusR]
  exact (Abstract.exp_neg_arcosh_of_sq (one_le_gamma1R P (thetaTilde M μ))
    (absGamma2_nonneg _ _) (gamma1R_sq_thetaTilde P hdual M μ)).symm
