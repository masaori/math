/-
# `check(Z), check(Y)` からの復元を**必要十分版から導出**する
...
/-- **`inverse_dft_check` を `NecSuf.inverse_dft_antiperiodic` の特殊化として導いたもの**。 -/
theorem inverse_dft_check_of_necSuf (hM : M ≠ 0) (x : Fin M → TensorPow M) (m : Fin M) :
  ...
  have h := NecSuf.inverse_dft_antiperiodic (K := ℂ) (V := TensorPow M) hM hξ x m
  refine Eq.trans ?_ h
...
theorem recover_checkZ_of_necSuf (hM : M ≠ 0) (m : Fin M) :
    ∑ μ : Fin M, checkPhase M (-(((m : ℕ) : ℤ) + 1)) (((μ : ℕ) : ℤ) + 1) •
        checkZ M (((μ : ℕ) : ℤ) + 1)
      = (M : ℂ) • Z m := by
  simp_rw [checkZ]
  exact inverse_dft_check_of_necSuf hM Z m

/-- **原文第 2 式を必要十分版の系として導いたもの**:
`∑_{μ=1}^{M} check(Y)_μ e^{ij θ~_μ} = M Y_j`。 -/
theorem recover_checkY_of_necSuf (hM : M ≠ 0) (m : Fin M) :
    ∑ μ : Fin M, checkPhase M (-(((m : ℕ) : ℤ) + 1)) (((μ : ℕ) : ℤ) + 1) •
        checkY M (((μ : ℕ) : ℤ) + 1)
      = (M : ℂ) • Y m := by
  simp_rw [checkY]
  exact inverse_dft_check_of_necSuf hM Y m
