...
抽象版は `Ising2D/Abstract/AntiperiodicFourier.lean` と
`Ising2D/Abstract/DiscreteFourier.lean`。本ファイルは両者をつなぐ
（`exact-solution-of-2d-ising-model/README.md` 4 節「抽象版から具体版が特殊化で
得られる場合は、具体版を抽象版の系として導出する」）。

## 何が特殊化で得られるか

| 具体版 | 抽象版 | 特殊化の中身 |
| --- | --- | --- |
| `antiperiodic_exp_sum` | `Abstract.sum_zpow_antiperiodic` | `ξ := e^{-iπ/M}`（1 の原始 `2M` 乗根） |
| `def_half_integer_modes` (1) | `Abstract.pow_half_eq_neg_one` | 同上 |
| `def_half_integer_modes` (2) | `Abstract.transform_periodic`（**整数運動量と共通**） | 重み `w_j = ξ^{-j}`、周波数 `a_j = j`、`ζ = ξ^2` |

とくに (2) は**整数運動量版 `hatZ_hatY_M_periodicity` とまったく同じ抽象版**の
...
/-- **`e^{-2π√-1/N}` は 1 の原始 `N` 乗根**（抽象版へ渡すための橋）。 -/
...
/-! ## `antiperiodic_exp_sum` を抽象版から導く -/

/-- **`antiperiodic_exp_sum` を `Abstract.sum_zpow_antiperiodic` の特殊化として導いたもの**
（`ξ := expPhase (2M) 1 = e^{-iπ/M}`）。 -/
theorem antiperiodic_exp_sum_of_abstract (hM : M ≠ 0) (k : ℤ) :
  ...
  rw [Finset.sum_congr rfl fun μ _ => hL μ,
    Abstract.sum_zpow_antiperiodic (K := ℂ) hM hξ (-k), ← expPhase_eq_zpow_one, hdelta]

/-- **`def_half_integer_modes` (1)（反周期性）を `Abstract.pow_half_eq_neg_one` から導いたもの**。 -/
theorem checkPhase_M_of_abstract (hM : M ≠ 0) : (expPhase (2 * M) 1) ^ M = -1 :=
  Abstract.pow_half_eq_neg_one hM (isPrimitiveRoot_expPhase_one (by omega))

/-! ## 添字の周期性を（整数運動量と共通の）抽象版から導く -/

/-- 半整数運動量の位相因子を「重み × 整数運動量の位相」に分解する:

  `e^{-i b θ~_μ} = e^{iπ b/M} · (e^{-2π√-1/M})^{b μ}`

抽象版 `Abstract.transform_periodic`（重み `w_j` と周波数 `a_j` が任意）へ
渡すための形である。 -/
...
/-- **`def_half_integer_modes` (2) を `Abstract.transform_periodic` の特殊化として導いたもの**。

整数運動量の `hatZ_hatY_M_periodicity`（`Part004/Claim012_HatPeriodicityAbstract.lean`）と
**同じ抽象版**を、重み `w_j = e^{iπ j/M}` に取り替えて使っているだけである。 -/
theorem checkZ_period_of_abstract (hM : M ≠ 0) (μ : ℤ) :
    checkZ M (μ + (M : ℤ)) = checkZ M μ := by
  have hζ : IsPrimitiveRoot (expPhase M 1) M := isPrimitiveRoot_expPhase_one hM
  have h := Abstract.transform_periodic (K := ℂ) (V := TensorPow M) hM hζ
      (fun j : Fin M => ((j : ℕ) : ℤ) + 1)
...
/-- **`def_half_integer_modes` (2)（`check(Y)` 側）**を同じ抽象版から導いたもの。 -/
theorem checkY_period_of_abstract (hM : M ≠ 0) (μ : ℤ) :
