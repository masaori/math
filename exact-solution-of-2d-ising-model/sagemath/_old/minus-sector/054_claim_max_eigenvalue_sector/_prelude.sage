...
def rayleigh_argmax(A_real):
    ...
    # SageMath 10.9 では RDF 行列の eigenmatrix_right が numpy 配列の連続性の検査で落ちるため、
    # 実対称行列の固有分解を numpy.linalg.eigh で直接行う。
    import numpy as np
    d = A_real.nrows()
    w, V = np.linalg.eigh(np.array([[float(A_real[i, j]) for j in range(d)] for i in range(d)]))
    i0 = int(np.argmax(w))
    v = vector(RDF, [RDF(t) for t in V[:, i0]])
    return RDF(w[i0]), v / v.norm()
