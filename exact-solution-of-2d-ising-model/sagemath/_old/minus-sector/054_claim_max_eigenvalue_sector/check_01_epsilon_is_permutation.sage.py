...
# This file was *autogenerated* from the file check_01_epsilon_is_permutation.sage
from sage.all_cmdline import *   # import sage library

_sage_const_20260727 = Integer(20260727); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_5 = Integer(5); _sage_const_2 = Integer(2)# =========================================================================
...
w_01 = w_perm = w_pi = w_act = w_sym = _sage_const_0 
n_empty = _sage_const_0 
for M in SEC_M:
    O = SpinOps(M)
    d = O.d
    E = eps_real(O)
    ...
    for i in range(d):
        for j in range(d):
            w_01 = max(w_01, min(abs(E[i, j]), abs(E[i, j] - _sage_const_1 )))

    # (2) 各行・各列の和がちょうど 1
    for i in range(d):
        w_perm = max(w_perm, abs(sum(E[i, j] for j in range(d)) - _sage_const_1 ))
        w_perm = max(w_perm, abs(sum(E[j, i] for j in range(d)) - _sage_const_1 ))
    ...
    for k in range(d):
        ...
        if pi[k] == k:
            w_pi = max(w_pi, RDF(_sage_const_1 ))

    # (4) (εx)_k = x_{π(k)}（π は対合なので添字は π(k) でよい）
    for _ in range(_sage_const_5 ):
        x = vector(RDF, [RDF(random() * _sage_const_2  - _sage_const_1 ) for _ in range(d)])
        y = E * x
        for k in range(d):
    ...
    if Bm.ncols() != _sage_const_2  ** (M - _sage_const_1 ):
        n_empty += _sage_const_1 
...
print(f"  (6) dim(F^{{(-)}} ∩ R^{{2^M}}) = 2^{{M-1}} の違反件数: {n_empty}  ->  "
      f"{'PASS' if n_empty == _sage_const_0  else 'FAIL'}")
ok_all &= (n_empty == _sage_const_0 )
