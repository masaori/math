...
# This file was *autogenerated* from the file check_03_Vprime_on_psi.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0p0 = RealNumber('0.0'); _sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_1en6 = RealNumber('1e-6')# ---------------------------------------------------------
...
for M in EVEN_M:
    ...
    for params in EVEN_PARAMS:
        ...
        w = {'dag': _sage_const_0p0 , 'ann': _sage_const_0p0 }
        for nu in range(_sage_const_1 , M + _sage_const_1 ):
            pdn, _ = fermions(O, K1v, K2v, nu)
            _, pcn = fermions(O, K1v, K2v, _sage_const_1  - nu)
            N = pdn * pcn
            for mu in range(_sage_const_1  - M, M + _sage_const_1 ):
                pdm, pm = fermions(O, K1v, K2v, mu)
                w['dag'] = max(w['dag'],
                               opnorm(comm(N, pdm) - delta_M(mu - nu, _sage_const_0 , M) * pdn))
                w['ann'] = max(w['ann'],
                               opnorm(comm(N, pm) + delta_M(nu + mu, _sage_const_1 , M) * pcn))
        worst = max(w.values())
        ok = worst <= TOL
...
for M in EVEN_M:
    for params in EVEN_PARAMS:
        K1v, K2v = RDF(params['K1']), RDF(params['K2'])
        gmin = min([gamma_tilde(K1v, K2v, M, mu) for mu in range(_sage_const_1 , M + _sage_const_1 )])
        ok = gmin > _sage_const_1en6 
        print(f"  M={M}, K1={params['K1']}, K2={params['K2']}: "
