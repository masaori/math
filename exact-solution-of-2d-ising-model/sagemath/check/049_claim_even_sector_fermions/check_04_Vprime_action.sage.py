...
# This file was *autogenerated* from the file sagemath/check/049_claim_even_sector_fermions/check_04_Vprime_action.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_2 = Integer(2); _sage_const_1en2 = RealNumber('1e-2')# =========================================================================
...
def X_over(O, P, mus):
    ...
    X = matrix(CDF, O.d, O.d, _sage_const_0 )
    for mu in mus:
        pdag, _ = psi_pair(O, mu, P)
        _, psi1m = psi_pair(O, M + _sage_const_1  - mu, P)
        X = X + gamma_tilde(O.M, mu, P) * (pdag * psi1m - Id / _sage_const_2 )
    return matrix(CDF, X)


ok_all = True
w_inv = w_a = w_b = w_dag = w_psi = _sage_const_0 
worst_short = _sage_const_0    # 和の範囲を狭めた場合の残差
worst_long = _sage_const_0     # 和の範囲を広げた場合の残差
...
for M in FERMI_M:
    ...
    for p in FERMI_PARAMS:
        ...
        Xs = X_over(O, P, list(range(_sage_const_1 , M)))        # {1..M-1}
        Xl = X_over(O, P, list(range(_sage_const_1 , M + _sage_const_2 )))    # {1..M+1}
...
print(f"  和の範囲を {{1..M-1}} に狭めると (a) の残差 = {float(worst_short):.3e} "
      f"-> {'期待どおり不成立（数え落とし）' if worst_short > _sage_const_1en2  else '判定不能'}")
print(f"  和の範囲を {{1..M+1}} に広げると (a) の残差 = {float(worst_long):.3e} "
      f"-> {'期待どおり不成立（重複）' if worst_long > _sage_const_1en2  else '判定不能'}")
ok_all &= (worst_short > _sage_const_1en2  and worst_long > _sage_const_1en2 )
