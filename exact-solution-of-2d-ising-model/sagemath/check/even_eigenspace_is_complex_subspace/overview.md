# SageMath Check: even_eigenspace_is_complex_subspace

## 対象

**対象ラベル**: `even_eigenspace_is_complex_subspace` （structured-latex 側の安定識別子）
...
| `check_smul_eigenvector_hypothesis.sage` | `a[εf]_r=af_r` | **PASS** |
| `check_smul_vector_definition.sage` | `af_r=[af]_r` | **PASS** |

全17等式を、`M=2` の全スピン反転行列と `εf=f`、`εg=g` を満たす二つのベクトルに対して
...
```bash
bash sagemath/tools/run-all-checks.sh even_eigenspace_is_complex_subspace
```

実行ログは `sagemath/check/even_eigenspace_is_complex_subspace/logs/` に保存する。
