...
def log_Lambda_delta_over_M(M, P, delta):
    """(1/M) log Λ^{(δ)}_M。M が大きいと Λ 自体は overflow するので対数で計算する。"""
    s = RDF(0)
    for mu in range(1, M + 1):
        t = RDF(2 * pi * (RDF(mu) - RDF(delta)) / M)
        y = max(RDF(g1(t, P)), RDF(1))
        s = s + RDF(arccosh(y))
    return RDF(log(2 * P['s2']) / 2 + s / (2 * M))
...
#   P^{(±)} := (I ± ε)/2                     (def_epsilon_projectors)
# ---------------------------------------------------------
def eps_op(O):
    """ε = σ^x_1 ⋯ σ^x_M"""
...
def D0_op(O):
    """D_0 = Σ_{m=1}^{M-1} σ^z_m σ^z_{m+1}"""
    out = matrix(CDF, O.d, O.d, 0)
    for m in range(1, O.M):
        out = out + O.SZ[m] * O.SZ[m + 1]
    return matrix(CDF, out)


def G_op(O):
    """G = σ^z_M σ^z_1"""
    return matrix(CDF, O.SZ[O.M] * O.SZ[1])


def W_op(O, K1, K2):
    """W = V_1^{1/2} V_2 V_1^{1/2}（V_1 = exp(K_1 Σ_{m=1}^{M} σ^z_m σ^z_{m+1})、周期境界）"""
    D = D0_op(O) + G_op(O)
    B = matrix(CDF, (RDF(K1) / 2 * D).exp())
    V2, _ = V2_mat(O, K2)
    return matrix(CDF, B * V2 * B)


def P_pm(O, sgn):
    """P^{(±)} = (I ± ε)/2"""
    return (identity_matrix(CDF, O.d) + sgn * eps_op(O)) / 2
...
def Lambda_delta_M(O, P, delta):
    ...
    for mu in range(1, M + 1):
        t = RDF(2 * pi * (RDF(mu) - RDF(delta)) / M)
        # 厳密な臨界点かつ δ = 0 では γ_1(θ_M) = 1 が理論値だが、倍精度では
        # 1 をわずかに下回りうる（arccosh の定義域外）。理論値どおり 1 で丸める。
        y = max(RDF(g1(t, P)), RDF(1))
        s = s + RDF(arccosh(y))
    return RDF(pref * exp(s / 2))
