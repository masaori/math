...
for M in STEP_M:
    ...
    for p in STEP_PARAMS:
        ...
        idx = list(range(1, M + 1)) + [0, -1]
        pairs = {mu: psi_pair(O, mu, P) for mu in set(idx + [1 - m for m in idx])}
        ...
        for mu in idx:
            ...
            # -------------------------------------------------------------
            for k in (-1, 1, 2):
                S.add("periodicity (1) gamma_1(theta~_{mu+kM}) = gamma_1(theta~_mu)",
                      g1(th_tilde(M, mu + k * M), P), g1(t, P))
                S.add("periodicity (1) gamma_2(+theta~_{mu+kM}) = gamma_2(+theta~_mu)",
                      g2(th_tilde(M, mu + k * M), P), a)
                S.add("periodicity (1) gamma_2(-theta~_{mu+kM}) = gamma_2(-theta~_mu)",
                      g2(-th_tilde(M, mu + k * M), P), b)
                S.add("periodicity (1) gamma(theta~_{mu+kM}) = gamma(theta~_mu)",
                      gamma_tilde(M, mu + k * M, P), gamma_tilde(M, mu, P))
                pd2, ps2 = psi_pair(O, mu + k * M, P)
                S.add("periodicity (2) psi^dagger_{mu+kM} = psi^dagger_mu", pd2, pdag)
                S.add("periodicity (2) psi_{mu+kM} = psi_mu", ps2, psi)
            S.add("periodicity (3) gamma_2(theta~_{1-mu}) = gamma_2(-theta~_mu)",
                  g2(th_tilde(M, 1 - mu), P), b)
            S.add("periodicity (3) gamma_2(-theta~_{1-mu}) = gamma_2(theta~_mu)",
                  g2(-th_tilde(M, 1 - mu), P), a)
            S.add("periodicity (3) gamma_1(theta~_{1-mu}) = gamma_1(theta~_mu)",
                  g1(th_tilde(M, 1 - mu), P), g1(t, P))
            S.add("periodicity (3) gamma(theta~_{1-mu}) = gamma(theta~_mu)",
                  gamma_tilde(M, 1 - mu, P), gamma_tilde(M, mu, P))
            ...
            for nu in idx:
                ...
                if d == 1:
                    # Step 1 の各段（nu = (1-mu)+kM なる k がある場合）
                    S.add("anticommutator Step1 a_nu = gamma_2(theta~_{1-mu}) = b_mu", an, b)
                    S.add("anticommutator Step1 b_nu = gamma_2(-theta~_{1-mu}) = a_mu", bn, a)
                ...
                S.add("anticommutator Step4 (2) 第 2 式 = delta^M_{(mu+nu,1)} I",
            ...
            for nu in range(1, M + 1):
                pdn, _ = pairs[nu]
                _, ps1n = pairs[1 - nu]
                d0 = delta_M(M, mu - nu, 0)
                d1 = delta_M(M, mu + nu, 1)
                S.add("action_of_T_check_Vprime Step1 [psi^d_nu psi_{1-nu}, psi^d_mu] = delta psi^d_nu",
                      comm(pdn * ps1n, pdag), d0 * pdn)
                S.add("action_of_T_check_Vprime Step1' [psi^d_nu psi_{1-nu}, psi_mu] = -delta psi_{1-nu}",
