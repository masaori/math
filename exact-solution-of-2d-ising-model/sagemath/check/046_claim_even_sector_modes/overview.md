...
確定させるのは、**半整数運動量モードなら閉じること**である：`checkZ, checkY` は
`H_1^{(+)}, H_2` に対して (A)〜(D) の交換関係を満たし、反交換関係の対は `μ+ν ≡ 1 (mod M)` になる。

以前ここにあった check_01（`why_008_applies_only_to_minus_sector`、整数運動量の `hatZ^{(+)}` では
`H_2` との交換関係が閉じないこと）は、そのブロックが本文から参照用ノート
`structured-latex/notes/integer_momentum_route_not_adopted.ts` へ退避されたのに合わせて、
`sagemath/_old/integer-momentum-route/why_008_applies_only_to_minus_sector/` へログごと移した。
...
本文の主張の量化範囲を `μ ∈ ℤ` から `𝓜̌ = {1,…,M}`（`def_check_index_set`）へ絞り、
共役添字を `1−μ` から `M+1−μ` へ書き換えたのに合わせて、check_02 / check_03 を更新して
**再実行した**（`run-log.txt` はその出力）。

- check_02 の「共役添字」は `θ~_{M+1−μ} = 2π − θ~_μ` と `Ž_{M+1−μ} = Ž_{1−μ}` を見る形に変えた。
- check_03 の反交換関係のデルタは、合同式 `(μ+ν−1) % M == 0` ではなく
  `ν == M+1−μ` で作り、両者が一致することを `assert` で確かめている
  （`def_check_index_set` (5) の数値的裏付け）。
- `H_1^{(+)}, H_2` の表示は `Ž_{M+1−μ}` を使う形に書き換えた（値は変わらない）。

`𝓜̌` そのものの性質（最小性・共役についての閉性・自己共役点・合同式の消滅）は
`sagemath/check/052_def_check_index_set/` で独立に検証している。
