...
def chain(prefix, exprs, reasons):
    ...
    """
    assert len(reasons) == len(exprs) - 1, prefix
    for k in range(len(reasons)):
        S.add(f"{prefix} ({k + 1}) {reasons[k]}", exprs[k], exprs[k + 1])
...
for M in STEP_M:
    ...
    # -----------------------------------------------------------------
    Zero = matrix(CDF, O.d, O.d, 0)

    def msum(terms):
        return sum(terms, matrix(CDF, O.d, O.d, 0))
    ...
    H2sum = msum([O.Z[a] * O.Y[a] for a in range(1, M + 1)])
    S.add("commutator Step1 def_H2: H_2 = sum_a Z_aY_a", H2, H2sum)
    for j in range(1, M + 1):
        Zj, Yj = O.Z[j], O.Y[j]
        others = [a for a in range(1, M + 1) if a != j]
        chain("commutator Step1 [H_2,Z_j]", [
            comm(H2, Zj),
            comm(H2sum, Zj),
            msum([comm(O.Z[a] * O.Y[a], Zj) for a in range(1, M + 1)]),
        ...
        chain("commutator Step1 [H_2,Y_j]", [
            comm(H2, Yj),
            comm(H2sum, Yj),
            msum([comm(O.Z[a] * O.Y[a], Yj) for a in range(1, M + 1)]),
    ...
    for j in range(2, M + 1):
        Zj = O.Z[j]
        rest = [m for m in bulk if m != j - 1]
        chain("commutator Step1 [H_1^{(+)},Z_j] (2<=j<=M)", [
            comm(H1p, Zj),
            comm(H1sum, Zj),
            msum([comm(O.Y[m] * O.Z[m + 1], Zj) for m in bulk]) - comm(YM * Z1, Zj),
            msum([comm(O.Y[m] * O.Z[m + 1], Zj) for m in bulk]) - Zero,
            msum([comm(O.Y[m] * O.Z[m + 1], Zj) for m in bulk]),
            comm(O.Y[j - 1] * Zj, Zj) + msum([comm(O.Y[m] * O.Z[m + 1], Zj) for m in rest]),
            comm(O.Y[j - 1] * Zj, Zj) + msum([Zero for m in rest]),
            comm(O.Y[j - 1] * Zj, Zj),
            2 * O.Y[j - 1],
            2 * Yflat(O, j - 1),
    ...
    chain("commutator Step1 [H_1^{(+)},Z_1]", [
        comm(H1p, Z1),
        comm(H1sum, Z1),
        msum([comm(O.Y[m] * O.Z[m + 1], Z1) for m in bulk]) + comm(-YM * Z1, Z1),
    ...
    for j in range(1, M):
        ...
        chain("commutator Step1 [H_1^{(+)},Y_j] (1<=j<=M-1)", [
            comm(H1p, Yj),
            comm(H1sum, Yj),
            msum([comm(O.Y[m] * O.Z[m + 1], Yj) for m in bulk]) - comm(YM * Z1, Yj),
            msum([comm(O.Y[m] * O.Z[m + 1], Yj) for m in bulk]) - Zero,
            msum([comm(O.Y[m] * O.Z[m + 1], Yj) for m in bulk]),
            comm(Yj * O.Z[j + 1], Yj) + msum([comm(O.Y[m] * O.Z[m + 1], Yj) for m in rest]),
            comm(Yj * O.Z[j + 1], Yj) + msum([Zero for m in rest]),
            comm(Yj * O.Z[j + 1], Yj),
            -2 * O.Z[j + 1],
            -2 * Zflat(O, j + 1),
        ], ["def_H1_plus の境界符号", "第 1 引数の C 線型性",
    ...
    chain("commutator Step1 [H_1^{(+)},Y_M]", [
        comm(H1p, YM),
        comm(H1sum, YM),
        msum([comm(O.Y[m] * O.Z[m + 1], YM) for m in bulk]) + comm(-YM * Z1, YM),
    ...
    # Step 1 の主張（4 式）
    for j in range(1, M + 1):
        S.add("commutator Step1 主張 [H_2, Z_j] = -2Y_j", comm(H2, O.Z[j]), -2 * O.Y[j])
        S.add("commutator Step1 主張 [H_2, Y_j] = 2Z_j", comm(H2, O.Y[j]), 2 * O.Z[j])
