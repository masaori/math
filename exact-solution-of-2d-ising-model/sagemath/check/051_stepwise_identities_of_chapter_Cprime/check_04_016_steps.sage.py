...
# This file was *autogenerated* from the file check_04_016_steps.sage
from sage.all_cmdline import *   # import sage library

_sage_const_1 = Integer(1); _sage_const_2 = Integer(2); _sage_const_0 = Integer(0); _sage_const_4 = Integer(4); _sage_const_3 = Integer(3); _sage_const_1en8 = RealNumber('1e-8'); _sage_const_1en7 = RealNumber('1e-7')# =========================================================================
...
for M in STEP_M:
    ...
    for p in STEP_PARAMS:
        ...
        idx = list(range(_sage_const_1 , M + _sage_const_1 ))
        pairs = {mu: psi_pair(O, mu, P) for mu in set(idx + [M + _sage_const_1  - m for m in idx] + [_sage_const_1  - m for m in idx])}
        ...
        for mu in idx:
            ...
            Pm = Pcheck(M, mu, P)
            cmu = CDF(_sage_const_1 ) / (_sage_const_2  * RDF(sqrt(RDF(M))) * b)

            # -------------------------------------------------------------
            # def_check_fermi
            # -------------------------------------------------------------
            S.add("def_check_fermi (psi^dagger) = P^ の第 1 列",
                  pdag, Pm[_sage_const_0 , _sage_const_0 ] * Zc + Pm[_sage_const_1 , _sage_const_0 ] * Yc)
            S.add("def_check_fermi (psi) = P^ の第 2 列",
                  psi, Pm[_sage_const_0 , _sage_const_1 ] * Zc + Pm[_sage_const_1 , _sage_const_1 ] * Yc)
            ...
            S.add("periodicity (3) gamma_2(theta~_{M+1-mu}) = gamma_2(-theta~_mu)",
                  g2(th_tilde(M, M + _sage_const_1  - mu), P), b)
            S.add("periodicity (3) gamma_2(-theta~_{M+1-mu}) = gamma_2(theta~_mu)",
                  g2(-th_tilde(M, M + _sage_const_1  - mu), P), a)
            S.add("periodicity (3) gamma_1(theta~_{M+1-mu}) = gamma_1(theta~_mu)",
                  g1(th_tilde(M, M + _sage_const_1  - mu), P), g1(t, P))
            S.add("periodicity (3) gamma(theta~_{M+1-mu}) = gamma(theta~_mu)",
                  gamma_tilde(M, M + _sage_const_1  - mu, P), gamma_tilde(M, mu, P))
            # 書き換えの正当性: psi_{M+1-mu} = psi_{1-mu}（周期性による）
            _, ps_new = pairs[M + _sage_const_1  - mu]
            _, ps_old = pairs[_sage_const_1  - mu]
            ...
            for nu in idx:
                ...
                cnu = CDF(_sage_const_1 ) / (_sage_const_2  * RDF(sqrt(RDF(M))) * bn)
                pdn, psn = pairs[nu]
                Zn, Yn = checkZ(O, nu), checkY(O, nu)
                # def_check_index_set (5): mu, nu in 𝓜̌ では合同式が消える
                d = _sage_const_1  if nu == M + _sage_const_1  - mu else _sage_const_0 
                S.add("def_check_index_set (5) delta^M_{(mu+nu,1)} = delta_{nu,M+1-mu}",
                      CDF(delta_M(M, mu + nu, _sage_const_1 )), CDF(d))
                ...
                if d == _sage_const_1 :
                    ...
                    S.add("anticommutator Step1 b_mu b_nu = a_mu b_mu = -r^2",
                          b * bn, CDF(-r ** _sage_const_2 ))
                    S.add("anticommutator Step1 c_mu c_nu = 1/(4M b_mu b_nu)",
                          cmu * cnu, CDF(_sage_const_1 ) / (_sage_const_4  * M * (b * bn)))
                    S.add("anticommutator Step1 c_mu c_nu = -1/(4Mr^2)",
                          cmu * cnu, CDF(-_sage_const_1 ) / (_sage_const_4  * M * r ** _sage_const_2 ))
                ...
                S.add("anticommutator Step2 (2) 中間 2 項は [checkZ,checkY]_+ = 0 で消える",
                      Zc * Yn + Yn * Zc, matrix(CDF, O.d, O.d, _sage_const_0 ))
                S.add("anticommutator Step2 (3) = c_mu c_nu (r_mu r_nu + b_mu b_nu) 2M delta I",
                      four, cmu * cnu * (r * rn + b * bn) * _sage_const_2  * M * d * Id)
                S.add("anticommutator Step2 (4) 第 1 式 = 0",
                      pdag * pdn + pdn * pdag, matrix(CDF, O.d, O.d, _sage_const_0 ))
                S.add("anticommutator Step3 第 3 式 = 0",
                      psi * psn + psn * psi, matrix(CDF, O.d, O.d, _sage_const_0 ))
                S.add("anticommutator Step4 (1) 第 2 式の 4 項展開",
                      pdag * psn + psn * pdag,
                      cmu * cnu * (-r * rn + b * bn) * _sage_const_2  * M * d * Id)
            ...
            S.add("commutation_V_plus (行ベクトル×行列の結合律) ((A,B)G)H = (A,B)(GH)",
                  (Pm[_sage_const_0 , _sage_const_0 ] * Zc + Pm[_sage_const_1 , _sage_const_0 ] * Yc), pdag)
            ...
            for nu in range(_sage_const_1 , M + _sage_const_1 ):
                pdn, _ = pairs[nu]
                _, ps1n = pairs[M + _sage_const_1  - nu]
                # 合同式なしのデルタ（def_check_index_set (5)）
                d0 = _sage_const_1  if mu == nu else _sage_const_0 
                d1 = _sage_const_1  if mu == M + _sage_const_1  - nu else _sage_const_0 
                S.add("action_of_T Step1 delta_{M+1-nu,M+1-mu} = delta_{mu,nu}",
                      CDF(delta_M(M, mu - nu, _sage_const_0 )), CDF(d0))
                S.add("action_of_T Step1' delta_{mu,M+1-nu} = delta^M_{(mu+nu,1)}",
                      CDF(delta_M(M, mu + nu, _sage_const_1 )), CDF(d1))
            ...
            for n in (_sage_const_0 , _sage_const_1 , _sage_const_2 , _sage_const_3 ):
            ...
            S.add("T_eq_on_check_Z_Y Step1 (checkZ,checkY) = (psi^d,psi) P^{-1} の第 1 列",
                  Zc, Q[_sage_const_0 , _sage_const_0 ] * pdag + Q[_sage_const_1 , _sage_const_0 ] * psi)
            S.add("T_eq_on_check_Z_Y Step1 第 2 列",
                  Yc, Q[_sage_const_0 , _sage_const_1 ] * pdag + Q[_sage_const_1 , _sage_const_1 ] * psi)
        ...
        for m in range(_sage_const_1 , M + _sage_const_1 ):
            zm = sum([checkZ(O, mu) * eiph(m * th_tilde(M, mu)) for mu in range(_sage_const_1 , M + _sage_const_1 )],
        ...
        S.add("T_V_plus_eq Step3 乗法性 T(xy) = T(x)T(y)",
              Vp * (O.Z[_sage_const_1 ] * O.Y[_sage_const_1 ]) * Vpi, (Vp * O.Z[_sage_const_1 ] * Vpi) * (Vp * O.Y[_sage_const_1 ] * Vpi))
        for i0 in range(O.d):
            for j0 in range(O.d):
                E = matrix(CDF, O.d, O.d, _sage_const_0 ); E[i0, j0] = CDF(_sage_const_1 )
        ...
        S.add("V_plus_eq_c Step2 W はすべての元と可換（行列単位で検査）",
              W * O.Z[_sage_const_1 ], O.Z[_sage_const_1 ] * W)
        S.add("V_plus_eq_c Step3 W = cI （centralizer_is_scalar）", W, c * Id)
        S.add("V_plus_eq_c Step5 V^{(+)} = c V^'", Vp, c * Vpr)
        assert abs(c) > _sage_const_1en8 , "Step4 の c != 0 が破れた"

ok_all = S.report_all(tol=_sage_const_1en7 )
