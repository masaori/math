...
for M in STEP_M:
    ...
    for mu in range(_sage_const_1 , M + _sage_const_1 ):
        t = th_tilde(M, mu)
        # checkZ_{1-mu} の 3 段
        c0 = checkZ(O, _sage_const_1  - mu)
        c1 = sum([O.Z[k] * eiph(-k * th_tilde(M, _sage_const_1  - mu)) for k in range(_sage_const_1 , M + _sage_const_1 )],
                 matrix(CDF, O.d, O.d, _sage_const_0 ))
        c2 = sum([O.Z[k] * eiph(-k * (-t)) for k in range(_sage_const_1 , M + _sage_const_1 )],
                 matrix(CDF, O.d, O.d, _sage_const_0 ))
    ...
    # H_2 側の 4 段
    h0 = sum([checkZ(O, _sage_const_1  - mu) * checkY(O, mu) for mu in range(_sage_const_1 , M + _sage_const_1 )],
             matrix(CDF, O.d, O.d, _sage_const_0 )) / M
    ...
    # H_1^{(+)} 側の 4 段
    k0 = sum([checkY(O, mu) * checkZ(O, _sage_const_1  - mu) * eiph(-th_tilde(M, mu))
              for mu in range(_sage_const_1 , M + _sage_const_1 )], matrix(CDF, O.d, O.d, _sage_const_0 )) / M
