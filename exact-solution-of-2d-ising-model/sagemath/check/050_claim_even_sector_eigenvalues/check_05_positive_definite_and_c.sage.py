...
# This file was *autogenerated* from the file sagemath/check/050_claim_even_sector_eigenvalues/check_05_positive_definite_and_c.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0 = Integer(0); _sage_const_2 = Integer(2); _sage_const_8 = Integer(8)# =========================================================================
...
ok_all = True
w_sym1 = _sage_const_0 
w_sym2 = _sage_const_0 
w_form = _sage_const_0 
w_herm = _sage_const_0 
w_U1 = _sage_const_0 
w_U2 = _sage_const_0 
w_tau = _sage_const_0 
w_ratio = _sage_const_0 
w_c = _sage_const_0 
min_ev = None
min_trV = None
min_trVinv = None
...
for M in EIG_M:
    ...
    for p in EIG_PARAMS:
        ...
        pref = CDF((_sage_const_2  * P['s2']) ** (RDF(M) / _sage_const_2 ))
        B = matrix(CDF, (S1 / _sage_const_2 ).exp())
        A = matrix(CDF, S2.exp())
...
print(f"  V^{{(+)}} の固有値の全体最小 = {float(min_ev):.3e}  ->  "
      f"{'PASS（正定値）' if min_ev > _sage_const_0  else 'FAIL'}")
print(f"  tr(V^{{(+)}}) の全体最小 = {float(min_trV):.3e}、"
      f"tr((V^{{(+)}})^{{-1}}) の全体最小 = {float(min_trVinv):.3e}  ->  "
      f"{'PASS（ともに正）' if min(min_trV, min_trVinv) > _sage_const_0  else 'FAIL'}")
ok_all &= (min_ev > _sage_const_0  and min_trV > _sage_const_0  and min_trVinv > _sage_const_0 )
