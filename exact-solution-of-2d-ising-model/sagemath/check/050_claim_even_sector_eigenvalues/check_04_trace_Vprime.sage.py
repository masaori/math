...
# This file was *autogenerated* from the file check_04_trace_Vprime.sage
...
for M in EIG_M:
    ...
    for p in EIG_PARAMS:
        ...
        pred = RDF(_sage_const_1 )
        for mu in range(_sage_const_1 , M + _sage_const_1 ):
            pred = pred * RDF(_sage_const_2  * cosh(gamma_tilde(M, mu, P) / _sage_const_2 ))
        tr = Vpr.trace()
        tri = Vpri.trace()
...
print(f"  tr(V^') の全体最小 = {float(min_tr):.3e}  ->  "
      f"{'PASS（正）' if min_tr > _sage_const_0  else 'FAIL'}")
