...
# This file was *autogenerated* from the file sagemath/check/047_claim_even_sector_T_action/check_04_T_V_plus_and_factorization.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0p0 = RealNumber('0.0'); _sage_const_0 = Integer(0); _sage_const_60 = Integer(60); _sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_1en6 = RealNumber('1e-6')# ---------------------------------------------------------
...
for params in EVEN_PARAMS:
    ...
    for k in range(_sage_const_0 , _sage_const_60 ):
        t = CDF(_sage_const_2  * pi * k / _sage_const_60 )
        P = B1(K1v, t) * B2(K2s) * B1(K1v, t)
        worst = max(worst, (P - A_theta(K1v, K2v, t)).norm(_sage_const_1 ))
    ok = worst <= TOL
...
for M in EVEN_M:
    ...
    for params in EVEN_PARAMS:
        ...
        w = {'compose': _sage_const_0p0 , 'A': _sage_const_0p0 }
        for mu in range(_sage_const_1 , M + _sage_const_1 ):
            ...
            TY = Vp * Yc * Vpinv
            w['A'] = max(w['A'],
                         opnorm(TZ - combine(Zc, Yc, Amat.column(_sage_const_0 ))),
                         opnorm(TY - combine(Zc, Yc, Amat.column(_sage_const_1 ))))
        worst = max(w.values())
        ok = worst <= TOL
...
for M in EVEN_M:
    for params in EVEN_PARAMS:
        ...
        for mu in range(_sage_const_1 , M + _sage_const_1 ):
            t = th_tilde(M, mu)
            g2 = A_theta(K1v, K2v, t)[_sage_const_0 , _sage_const_1 ]
            a = abs(g2)
            smallest = a if smallest is None else min(smallest, a)
        ok = smallest > _sage_const_1en6 
        print(f"  M={M}, K1={params['K1']}, K2={params['K2']}: "
