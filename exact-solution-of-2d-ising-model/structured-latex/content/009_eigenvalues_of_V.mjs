...
\mathrm{tr}(Q) = \dim_{\mathbb{C}} \mathrm{im}\,Q`),
...

  {
    ...
    statement: [
      ...
      displayMath(
        String.raw`\mathcal{I} := \{\mu \in \{1,\dots,M\} \mid \gamma_2(\theta_\mu) \neq 0\},
\qquad m := |\mathcal{I}|`,
      ),
      paragraph([
        "とおく。",
...
= n_\nu n_\mu`,
...

  {
    ...
    sourcePath: SRC,
    sourceOrdinal: 9,
    title: { tex: String.raw`\mathrm{tr}(n_{\mu_1}\cdots n_{\mu_k}) = 2^{M-k}` },
    ...
    statement: [
      ...
      displayMath(
        String.raw`\mathrm{tr}\!\left(n_{\mu_1} n_{\mu_2}\cdots n_{\mu_k}\right) = 2^{M-k}`,
      ),
      paragraph([
        "（",
        math(String.raw`k = 0`),
        ...
        " である。）とくに ",
        math(String.raw`\mathrm{tr}(n_\mu) = 2^{M-1}`),
        "。",
      ]),
    ],
    proof: [
...
U\sigma_k^z U^{-1} = \begin{cases}-\sigma_k^z & (k\ \text{奇数}) \\ +\sigma_k^z & (k\ \text{偶数})\end{cases}`,
      ...
      paragraph([
        "Step 2（",
        math(String.raw`S_2`),
        " への作用）。Step 1 の第 1 式より、各 ",
        math(String.raw`m`),
        " について ",
        math(String.raw`U\sigma_m^x U^{-1} = -\sigma_m^x`),
        " だから",
      ]),
      displayMath(
        String.raw`U S_2 U^{-1} = K_2^*\sum_{m=1}^{M} U\sigma_m^x U^{-1}
= -K_2^*\sum_{m=1}^{M}\sigma_m^x = -S_2`,
      ),
      paragraph([
        "（共役 ",
      ...
      paragraph([
        ...
        " と ",
        math(String.raw`m+1`),
        " は連続する整数なので、ちょうど一方が奇数で他方が偶数である。よって Step 1 の第 3 式から符号は ",
        math(String.raw`(-1)\cdot(+1) = -1`),
        " となり",
      ]),
      displayMath(
        String.raw`U\left(\sigma_m^z\sigma_{m+1}^z\right)U^{-1}
= \left(U\sigma_m^z U^{-1}\right)\left(U\sigma_{m+1}^z U^{-1}\right)
= -\,\sigma_m^z\sigma_{m+1}^z`,
      ...
      paragraph([
        ...
        math(String.raw`G`),
        " への作用）。",
        math(String.raw`G = \sigma_1^y\,\sigma_2^x\cdots\sigma_{M-1}^x\,\sigma_M^y`),
        " の各因子の符号を Step 1 から数える。",
      ]),
      list([
        [
          math(String.raw`\sigma_1^y`),
          "：",
          math(String.raw`1`),
          " は奇数なので符号 ",
          math(String.raw`+1`),
        ...
        [
          math(String.raw`\sigma_k^x\ (2 \leq k \leq M-1)`),
          "：各々符号 ",
          math(String.raw`-1`),
          "。個数は ",
          math(String.raw`M-2`),
          " なので寄与は ",
          math(String.raw`(-1)^{M-2}`),
          "。",
        ],
        [
      ...
      paragraph(["したがって全体の符号は"]),
      displayMath(
        String.raw`\begin{cases}
(+1)\cdot(-1)^{M-2}\cdot(-1) = (+1)(+1)(-1) = -1 & (M\ \text{偶数}) \\
(+1)\cdot(-1)^{M-2}\cdot(+1) = (+1)(-1)(+1) = -1 & (M\ \text{奇数})
\end{cases}`,
      ),
      paragraph([
        "（",
      ...
      paragraph([
        "Step 5（結論）。",
        ref("iH_is_real_symmetric"),
        " の表示に Step 3・Step 4 を代入して",
      ]),
      displayMath(
        String.raw`U S_1^{(\pm)} U^{-1}
= K_1\sum_{m=1}^{M-1} U\left(\sigma_m^z\sigma_{m+1}^z\right)U^{-1} \mp K_1\,UGU^{-1}
= -K_1\sum_{m=1}^{M-1}\sigma_m^z\sigma_{m+1}^z \pm K_1 G
= -\,S_1^{(\pm)}`,
      ),
      paragraph([
        ...
        " なので、",
        math(String.raw`UH_1^{(\pm)}U^{-1} = -H_1^{(\pm)}`),
        "、",
        math(String.raw`UH_2U^{-1} = -H_2`),
        " も従う。",
      ]),
    ],
    conversion: {
