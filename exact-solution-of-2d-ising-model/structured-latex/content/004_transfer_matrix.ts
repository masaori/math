...
\end{aligned}`,
  ...
  {
    id: "transfer_matrix_010a_definition_hatZ_pm",
    kind: "definition",
    ...
    statement: [
      paragraph([
        math(String.raw`M\in\mathbb Z_{\geq1}`),
        " とする。自然数から整数への標準包含を ",
        ...
        " と書く。これらは零、単位元、和、積、順序を保つものとする。",
        math(String.raw`M\geq1`),
        " なので、",
        math(String.raw`(M_{\mathbb N})_{\mathbb Z}=M`),
        " となる一意な ",
        math(String.raw`M_{\mathbb N}\in\mathbb N_{\geq1}`),
        " がある。ここで ",
        math(String.raw`(M_{\mathbb N})_{\mathbb Z}:=\iota_{\mathbb N\to\mathbb Z}(M_{\mathbb N})`),
        " である。また、",
        math(String.raw`M_{\mathbb R}:=\iota_{\mathbb Z\to\mathbb R}(M)\in\mathbb R_{>0}`),
        " と書く。",
        ref("inclusion_rr_to_cc"),
        " による複素数像を ",
        math(String.raw`M_{\mathbb C}:=(M_{\mathbb R})_{\mathbb C}\in\mathbb C`),
        " と書く。実数から複素数への包含の定義により ",
        math(String.raw`M_{\mathbb C}=(M_{\mathbb R},0_{\mathbb R})`),
        " であるから、",
        math(String.raw`M_{\mathbb R}>0_{\mathbb R}`),
        " だから ",
        math(String.raw`M_{\mathbb R}\ne0_{\mathbb R}`),
        " かつ ",
        math(String.raw`M_{\mathbb C}\ne0_{\mathbb C}`),
        " である。したがって以下の実位相の分母は零でない。複素スカラーの和と積には ",
        ref("complex_numbers_form_a_field"),
        " の体構造を用いる。",
      ]),
      paragraph([
        math(String.raw`\mathcal M:=\{-M,\dots,-1,1,\dots,M\}\subset\mathbb Z`),
        " とする。",
      ...
      displayMath(
        String.raw`j_{\mathbb Z}:=\iota_{\mathbb N\to\mathbb Z}(j),\qquad
j_{\mathbb R}:=\iota_{\mathbb Z\to\mathbb R}(j_{\mathbb Z}),\qquad
\mu_{\mathbb R}:=\iota_{\mathbb Z\to\mathbb R}(\mu)`,
      ),
      paragraph([
        "と書く。包含写像が順序を保ち、",
        math(String.raw`1\leq j\leq M_{\mathbb N}`),
        " だから",
      ]),
      displayMath(
        String.raw`1_{\mathbb Z}=\iota_{\mathbb N\to\mathbb Z}(1)
\leq j_{\mathbb Z}
\leq\iota_{\mathbb N\to\mathbb Z}(M_{\mathbb N})=M`,
      ...
      paragraph([
        ...
        math(String.raw`2^M`),
        " は、その自然数像による ",
        math(String.raw`2^{M_{\mathbb N}}`),
        " の略記とする。したがって ",
        ref("def_jordan_wigner_Z_matrices"),
        " から ",
        math(String.raw`Z_{j_{\mathbb Z}}\in\mathrm{Mat}(2^M,\mathbb C)=\mathrm{Mat}(2^{M_{\mathbb N}},\mathbb C)`),
        " である。次に、",
      ]),
      displayMath(
        String.raw`\phi_{j,\mu}:=-\frac{2\pi j_{\mathbb R}\mu_{\mathbb R}}{M_{\mathbb R}}\in\mathbb R,
\qquad i(\phi_{j,\mu})_{\mathbb C}\in\mathbb C`,
      ...
      paragraph([
        ...
        " とする。ここで書いた複素指数の定義と所属は現行本文では未整備であり、",
        "現行のオイラー表示もその欠落を明記している。したがって複素指数の先行定義が整うまでは、この位相因子の所属を未解決として扱う。以下ではその所属を仮定し、二つの符号の各々について",
      ]),
      displayMath(
        String.raw`\begin{aligned}
\hat{Z}_\mu^{(\pm)}
...
\end{aligned}`,
      ...
      paragraph([
        "と定める。各スカラーと各 ",
        math(String.raw`Z_{j_{\mathbb Z}}`),
        " の積、およびその有限和は ",
        math(String.raw`\mathrm{Mat}(2^{M_{\mathbb N}},\mathbb C)`),
        " に属する。",
        math(String.raw`M_{\mathbb N}=1`),
        " では ",
        math(String.raw`\sum_{j=2}^{M_{\mathbb N}}`),
        " を空和、すなわち同じ行列空間の零行列とする。",
      ]),
    ],
    conversion: {
      status: "converted",
      notes: [
        "旧来の複合定義から Fourier 変換した Z 行列族だけを分離した。旧複合ラベルは二定義の一方だけを指せないため廃止し、新しい参照は def_hatZ_pm を用いる。二表示の間は、有限和の分割と先頭係数の評価を別の等号へ分けた。",
        "複素指数の先行定義と所属証明は現行本文に無いため、その所属を仮定した条件付きの定義として明記した。先行定義が整うまでは本文完成・節境界確定とは扱わない。",
      ],
    },
  },
  {
    id: "transfer_matrix_010b_definition_hatY",
    kind: "definition",
    origin: { path: "_old/typst/parts/004_転送行列/009_definition_Zhat_Yhatの定義.typ", ordinal: 10 },
...
\leq\iota_{\mathbb N\to\mathbb Z}(M_{\mathbb N})=M`,
      ...
      paragraph([
        ...
        ref("def_jordan_wigner_Y_matrices"),
        " から ",
        math(String.raw`Y_{j_{\mathbb Z}}\in\mathrm{Mat}(2^M,\mathbb C)=\mathrm{Mat}(2^{M_{\mathbb N}},\mathbb C)`),
        " である。次に、",
      ]),
      displayMath(
        String.raw`\psi_{j,\mu}:=-\frac{2\pi j_{\mathbb R}\mu_{\mathbb R}}{M_{\mathbb R}}\in\mathbb R,
\qquad i(\psi_{j,\mu})_{\mathbb C}\in\mathbb C`,
      ...
      paragraph([
        ...
        math(String.raw`i=(0_{\mathbb R},1_{\mathbb R})\in\mathbb C`),
        " とする。ここで書いた複素指数の定義と所属は現行本文では未整備であり、",
        "現行のオイラー表示もその欠落を明記している。したがって複素指数の先行定義が整うまでは、この位相因子の所属を未解決として扱う。以下ではその所属を仮定して",
      ]),
      displayMath(
        String.raw`\hat{Y}_\mu
:= \sum_{j=1}^{M_{\mathbb N}} \exp(i(\psi_{j,\mu})_{\mathbb C})Y_{j_{\mathbb Z}}
\in\mathrm{Mat}(2^{M_{\mathbb N}},\mathbb C)`,
      ),
      paragraph([
        "と定める。各スカラーと各 ",
        math(String.raw`Y_{j_{\mathbb Z}}`),
        " の積、およびその有限和は ",
        math(String.raw`\mathrm{Mat}(2^{M_{\mathbb N}},\mathbb C)`),
        " に属する。",
      ]),
    ...
    conversion: {
      status: "converted",
      notes: [
        "旧来の複合定義から Fourier 変換した Y 行列族だけを分離した。格子幅と添字の包含像、分母の非零性、実位相・複素位相・行列和の所属を本定義内に明記し、hat Z の定義とは独立にした。",
        "複素指数の先行定義と所属証明は現行本文に無いため、その所属を仮定した条件付きの定義として明記した。先行定義が整うまでは本文完成・節境界確定とは扱わない。",
      ],
    },
  },
...
\exp\!\left(-i\frac{2\pi j}{M}\right) &&(\because\ \hat{Y}_j,\ \hat{Z}_{-j}^{(\pm)}\ \text{の定義 }\blkref{def_hatY},\ \blkref{def_hatZ_pm}\text{ の展開}) \\
...
\overbrace{\left(\sum_{k_2=1}^M \exp\!\left(-i k_2\frac{2\pi j}{M}\right)Y_{k_2}\right)}^{\hat{Y}_j} &&(\because\ \hat{Z}_{-j}^{(-)},\ \hat{Y}_j\ \text{の定義 }\blkref{def_hatZ_pm},\ \blkref{def_hatY}\text{ の展開}) \\
&= \frac{1}{M}\sum_{j\in\{1,\dots,M\}}\sum_{k_1,k_2=1}^M
    ...
    conversion: {
      ...
      notes: [
        ...
        "2026-09-04 の式変形統一で、証明冒頭の参照の前置き（以下 … を用いる）を削り、" +
          "定義の展開の行末根拠へ \\blkref{def_hatZ_pm} と \\blkref{def_hatY} を、文字列 exp_sum で書かれていた" +
...
&&(\because\ \text{定義}\ \hat{Z}_\mu^{(\pm)}\ \blkref{def_hatZ_pm}\ \text{で}\ \mu = M,\ \text{複号下では}\ j=1\ \text{の係数も}\ 1)\\
&= \sum_{j=1}^{M} \exp\!\left(-i \frac{2\pi j (-M)}{M}\right)Z_j
...
&&(\because\ \text{定義}\ \hat{Y}_\mu\ \blkref{def_hatY}\ \text{で}\ \mu = M)\\
&= \sum_{j=1}^{M} \exp\!\left(-i \frac{2\pi j (-M)}{M}\right)Y_j
&&(\because\ \text{上で示した各}\ j\ \text{についての係数の一致})\\
&= \hat{Y}_{-M}
&&(\because\ \text{定義}\ \hat{Y}_\mu\ \blkref{def_hatY}\ \text{で}\ \mu = -M)
\end{aligned}`,
    ...
    conversion: {
      status: "converted",
      notes: [
        "2026-09-04 の式変形統一で、複素数の四則だけを述べる三行を削り、三角関数の整数点での値から係数の一致へ直接進めた。末尾の参照一覧も削り、定義を使う四行の行末へ対応する Fourier 変換の定義参照を移した。主張・式変形の実質・根拠は変えていない。",
      ],
    },
  },
  {
    ...
    statement: [
      paragraph([
        ...
        math(String.raw`+1`),
        "（uniform）であり、すなわち ",
        math(String.raw`\hat{Z}_\mu^{(-)} = \sum_{j=1}^M \exp\!\left(-i j\frac{2\pi\mu}{M}\right)Z_j`),
        " である。各 ",
        math(String.raw`m \in \{1,\dots,M\}`),
        " について、次が成り立つ。",
      ]),
    ...
    proof: [
      ...
      displayMath(
        String.raw`\sum_{\mu=1}^M \exp\!\left((m-j)\cdot\frac{2\pi i\mu}{M}\right) = M\,\delta^M_{(m-j,\,0)} \quad (\because \text{指数和の直交性}\ \blkref{exp_sum})`,
      ),
      paragraph([
        "が成り立つ。さらに ",
...
&= \sum_{\mu=1}^M \left(\sum_{j=1}^M \exp\!\left(-i j\frac{2\pi\mu}{M}\right)Y_j\right)\exp\!\left(i m\frac{2\pi\mu}{M}\right) \quad (\because \hat{Y}_\mu \text{ の定義}\ \blkref{def_hatY}) \\
...
&= \sum_{\mu=1}^M \left(\sum_{j=1}^M \exp\!\left(-i j\frac{2\pi\mu}{M}\right)Z_j\right)\exp\!\left(i m\frac{2\pi\mu}{M}\right) \quad (\because \hat{Z}_\mu^{(-)} \text{ の定義}\ \blkref{def_hatZ_pm}) \\
...
\end{aligned}`,
    ...
    conversion: {
      status: "converted",
      notes: [
        "2026-09-04 の式変形統一で、指数和の直交性と Fourier 変換の定義への後置き参照を削り、それぞれを使う等号の行末へ対応するラベル参照を移した。内容・式変形・根拠は変えていない。",
      ],
    },
  },
  {
