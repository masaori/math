...
= m_{2a+1}\,m_{2b+1}`,
      ...
      paragraph([
        "が成り立つ（",
        math(String.raw`2k = (k-1)+(k+1)`),
        " なので、上の 2 式のうち偶奇の合う方を ",
        math(String.raw`a = \lfloor (k-1)/2\rfloor`),
        " 等に対して適用する）。",
      ]),
      paragraph([
...
= \|x\|^2\,\mathrm{tr}(A)`,
...

  {
    ...
    statement: [
      ...
      displayMath(
        String.raw`c_\pm(M) := \sup\left\{\, x^\top W x \ \middle|\ x \in \mathcal{F}^{(\pm)}\cap\mathbb{R}^{2^M},\ \|x\| = 1 \,\right\}`,
      ),
      paragraph(["とおくと（複号同順）、"]),
      list([
