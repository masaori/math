...
= Y + [X,Y] + \tfrac{1}{2}[X,[X,Y]] + \tfrac{1}{6}[X,[X,[X,Y]]] + \cdots`,
      ...
      paragraph([
        ...
        ref("def_ad_X_matrix"),
        " の ",
        math(String.raw`\mathrm{ad}_X(W)=[X,W]=XW-WX`),
        "、",
      ]),
      displayMath(
        String.raw`\mathrm{ad}_X^{m}
:= \underbrace{\mathrm{ad}_X\circ\cdots\circ\mathrm{ad}_X}_{m},
\qquad \mathrm{ad}_X^{0}:=\mathrm{id}_{\mathrm{Mat}(n,\mathbb{C})}
\qquad (m\in\mathbb{Z}_{\ge 0})`,
      ),
      paragraph([
        "であり、",
        math(String.raw`\underbrace{[X,[X,\dots,[X,Y]\dots]]}_{n\text{ times}}=\mathrm{ad}_X^{n}(Y)`),
        " とする。",
        ...
        " を ",
        math(String.raw`V:=\mathrm{Mat}(n,\mathbb{C})`),
        "（有限次元 ",
        math(String.raw`\mathbb{C}`),
        "-ノルム線型空間、",
        math(String.raw`\dim_{\mathbb{C}}V=n^2`),
        "）と ",
        math(String.raw`\mathrm{ad}_X\in\mathrm{End}(V)`),
        " に適用したものである。主張には右辺の級数が ",
    ...
    proof: [
      paragraph([
        ...
        math(String.raw`\mathrm{ad}_X\in\mathrm{End}(V)`),
        " であること。",
        math(String.raw`W,W'\in\mathrm{Mat}(n,\mathbb{C})`),
        "、",
        math(String.raw`c\in\mathbb{C}`),
        " について、行列の積の分配律とスカラー倍の結合性より",
      ]),
      displayMath(
        String.raw`\begin{aligned}
\mathrm{ad}_X(W+W') &= X(W+W')-(W+W')X \\
&= (XW-WX)+(XW'-W'X) = \mathrm{ad}_X(W)+\mathrm{ad}_X(W') \\
\mathrm{ad}_X(cW) &= X(cW)-(cW)X = c(XW-WX) = c\,\mathrm{ad}_X(W)
\end{aligned}`,
      ),
      paragraph([
        "よって ",
      ...
      paragraph([
        ...
        math(String.raw`V=\mathrm{Mat}(n,\mathbb{C})`),
        "、線型写像 ",
        math(String.raw`\mathrm{ad}_X:V\to V`),
        " に適用すると、級数 ",
        math(String.raw`\sum_{m=0}^{\infty}\frac{1}{m!}\mathrm{ad}_X^{m}`),
        " は線型写像 ",
        math(String.raw`V\to V`),
        " に各点収束する。すなわち各 ",
        math(String.raw`Y\in V`),
        " について ",
        math(String.raw`\mathrm{Mat}(n,\mathbb{C})`),
        " の中で",
      ]),
      displayMath(
        String.raw`\lim_{N\to\infty}\sum_{m=0}^{N}\frac{1}{m!}\,\mathrm{ad}_X^{m}(Y)
\quad \text{が存在し、その値が } \exp(\mathrm{ad}_X)(Y) \text{ である}`,
      ),
      paragraph([
        "（",
        ref("def_exp"),
        " の ",
        math(String.raw`\exp(\mathrm{ad}_X)=\sum_{m=0}^{\infty}\frac{1}{m!}\mathrm{ad}_X^{m}`),
        " は、まさにこの各点収束先として定義されている）。これは主張の級数の収束と等式",
      ]),
      displayMath(
        String.raw`e^{\mathrm{ad}_X}(Y)
...
= Y+[X,Y]+\tfrac{1}{2}[X,[X,Y]]+\tfrac{1}{6}[X,[X,[X,Y]]]+\cdots`,
      ...
      paragraph([
        ...
        "、",
        math(String.raw`\frac{1}{1!}\mathrm{ad}_X^{1}(Y)=[X,Y]`),
        "、",
        math(String.raw`\frac{1}{2!}=\frac12`),
        "、",
        math(String.raw`\frac{1}{3!}=\frac16`),
        " を代入しただけの書き換えである）。",
      ]),
      paragraph([
        "Step 3: 解析的操作の所在。本証明で非可算集合 ",
        math(String.raw`\mathbb{C}`),
        "（および ",
        math(String.raw`\mathbb{R}`),
        "）の解析を使うのは Step 2 の各点収束だけであり、その根拠は ",
        ref("exp_converges"),
        "、さらにその土台である ",
        ref("matrix_completeness"),
        "（",
        ...
        " の完備性、ひいては ",
        math(String.raw`\mathbb{R}`),
        " の完備性）である。Step 1 は有限個の行列の代数計算のみである。",
      ]),
      paragraph([
        "以上より主張は証明された。したがって本ブロックは Brian Hall の参考記述ではなく、このリポジトリ内で証明済みの命題であり、証明の根拠として使用してよい。なお同じ内容は、後掲の ",
        ref("matrix_exp_conjugation"),
        " (1) とその証明の Step 1 でも独立に示されているが、本証明はそれを用いていない（循環しない）。",
      ]),
    ],
    conversion: {
      status: "converted",
      notes: [
        "原文（Typst）の proof は「Brian Hall Prop 3.35 の参考記述であり、このリポジトリでは未証明。" +
          "証明の根拠として使用することは禁止する」という注記だけだった。しかし本主張は行列環 " +
          "Mat(n,C) 上の主張にすぎず、ad_X の C-線型性（有限個の行列の代数計算）と " +
          "labels: exp_converges（有限次元ノルム線型空間の自己準同型の exp 級数の各点収束）だけから" +
          "従うため、証明を書いて使用禁止の注記を撤回した。Lie 群論は使っていない。",
      ],
    },
  },
  {
    ...
    statement: [
      ...
      paragraph([
        ...
        " の写像として well-defined であること（すなわち ",
    ...
    proof: [
      paragraph([
        ...
        math(String.raw`\mathrm{M}(n,\mathbb{C})`),
        " の可逆元全体であるから ",
        math(String.raw`G\subset\mathrm{M}(n,\mathbb{C})`),
        "。仮定より ",
        math(String.raw`X\in\mathrm{M}(n,\mathbb{C})`),
        " かつ ",
...
\exp(X)^{-1}=\exp(-X)`,
      ...
      paragraph([
        "が成り立つ（級数の収束は同主張の (1)）。とくに ",
        ref("def_ad_X_matrix"),
        " の定義により",
      ]),
      displayMath(
        String.raw`\mathrm{Ad}_{\exp(X)}(Y)
= \exp(X)\,Y\,\exp(X)^{-1}
= \exp(X)\,Y\,\exp(-X)
= \exp(\mathrm{ad}_X)(Y)`,
      ),
      ...
      paragraph([
        ...
        " が ",
        math(String.raw`G\to G`),
        " として well-defined であること。ここではじめて仮定「",
        math(String.raw`\forall t\in\mathbb{R},\ \exp(tX)\in G`),
        "」を使う。",
        math(String.raw`t=1`),
        " として ",
        math(String.raw`\exp(X)=\exp(1\cdot X)\in G`),
        "。",
        ref("def_matrix_lie_group"),
        " より ",
        math(String.raw`G`),
        " は ",
        math(String.raw`\mathbf{GL}(n,\mathbb{C})`),
        " の部分群であるから、逆元について閉じており ",
        math(String.raw`\exp(X)^{-1}\in G`),
        "、積について閉じているから ",
        math(String.raw`Y\in G`),
        " と合わせて",
      ]),
      displayMath(
        String.raw`\mathrm{Ad}_{\exp(X)}(Y)=\exp(X)\,Y\,\exp(X)^{-1}\in G`,
      ),
      paragraph([
        "すなわち ",
        math(String.raw`\mathrm{Ad}_{\exp(X)}:G\to G`),
        " は well-defined である。",
      ]),
      paragraph([
        "Step 4: 使った仮定の整理。Step 2 の 2 つの等号は ",
        math(String.raw`X,Y\in\mathrm{Mat}(n,\mathbb{C})`),
        " だけから従い、",
        math(String.raw`G`),
        " が Matrix Lie 群であることも ",
        math(String.raw`\exp(tX)\in G`),
        " も一切使っていない。これらの仮定が効くのは Step 3（",
        math(String.raw`\mathrm{Ad}_{\exp(X)}`),
        " の終域が ",
        math(String.raw`G`),
        " に収まること）だけである。したがって本ブロックは Brian Hall の参考記述ではなく証明済みの命題であり、証明の根拠として使用してよい。",
      ]),
      paragraph([
        "なお、Brian Hall の原命題のうち一般の Lie 群 ",
        math(String.raw`G`),
        "・Lie 環 ",
        math(String.raw`\mathfrak{g}`),
        " に対する版（本節冒頭の ",
        math(String.raw`\mathrm{Ad}(\exp(X))=\exp(\mathrm{ad}(X))`),
        "、",
        ref("def_Ad_ad_lie"),
        " の記号による）は依然として未証明である。その差分は「",
        math(String.raw`G`),
        " が行列群とは限らない抽象 Lie 群のとき、",
        math(String.raw`\mathrm{Ad}_g`),
        " を ",
        math(String.raw`g h g^{-1}`),
        " の単位元における微分として定義し、",
        math(String.raw`\mathrm{Aut}(G)`),
        " が Lie 群で ",
        math(String.raw`\mathrm{End}(\mathfrak{g})`),
        " がその Lie 環であることを使う部分」であり、そこには行列の積・行列ノルムでは代替できない多様体構造が必要である。本ブロックの主張はその特別な場合（",
        math(String.raw`G\subset\mathbf{GL}(n,\mathbb{C})`),
        "、",
        math(String.raw`\mathrm{Ad}_g(h)=ghg^{-1}`),
        " が行列の積で直接定義される場合）に限定されており、上の証明はその限定のもとで完結している。",
      ]),
    ],
    conversion: {
      status: "converted",
      notes: [
        "原文（Typst）の proof は「Brian Hall Prop 3.35 の参考記述であり、このリポジトリでは未証明。" +
          "証明の根拠として使用することは禁止する」という注記だけだった。しかし本ブロックの statement は " +
          "G ⊂ GL(n,C) を仮定した行列版であり、Ad_g(h)=ghg^{-1} も行列の積で直接定義されているため、" +
          "labels: matrix_exp_conjugation（行列版 e^X Y e^{-X} = e^{ad_X}(Y)）を適用するだけで証明できる。" +
          "証明を書いて使用禁止の注記を撤回した。exp(tX) ∈ G の仮定は Ad_{exp X} の終域が G に収まること" +
          "（well-defined 性）にのみ使う。抽象 Lie 群版との差分は proof 末尾に明記した。",
      ],
    },
  },
  {
