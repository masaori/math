...
Y_m = \sigma_1^x \cdots \sigma_{m-1}^x\, \sigma_m^y`,
  ...
  },
  {
    ...
    targets: ["T_V_plus_check_Z_Y", "def_T_V_plus", "V1_in_Z_Y_epsilon", "V2_in_Z_Y"],
...
\qquad (\dim_{\mathbb{C}} \mathcal{V} = 2M)`,
      ...
      paragraph([
        ...
        math(String.raw`X`),
        " とし、その指数関数による共役 ",
        math(String.raw`x \mapsto \exp(X) x\, \exp(-X)`),
        " を考えると、次が起きる。",
      ]),
      list([
        [
        ...
        [
          "したがって共役の級数展開 ",
          math(String.raw`\exp(X) x \exp(-X) = \sum_{n\geq 0} \frac{1}{n!}[X,[X,\dots,[X,x]\dots]]`),
          " の各項がすべて ",
          math(String.raw`\mathcal{V}`),
          " の元であり、",
          math(String.raw`\exp(X)\mathcal{V}\exp(-X) = \mathcal{V}`),
          "。すなわち共役は ",
          math(String.raw`\mathcal{V}`),
          " 上の線型変換に制限される。",
        ],
...
= \left(\hat{Z}_\mu^{(-)},\; \hat{Y}_\mu\right) A\!\left(\frac{2\pi\mu}{M}\right)`,
  ...
  },
  {
    ...
    targets: ["V_plus_eq_c_check_Vprime", "T_V_plus_eq_T_check_Vprime", "centralizer_is_scalar"],
