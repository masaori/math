 ...
 *
 * 使い方: node structured-latex/tools/verify-no-lost-proofs.ts
 *
...
const projectRoot = resolve(structuredLatexDir, "..");
 ...
 * 「証明が入っていない」と判定する。
 */
function proofIsEmptyExceptTodo(block: ConvertedBlock): boolean {
  ...
  for (const node of proof) walk(node);
 ...
 */
function extractProofBody(source: string): string | null {
 ...
 */
function typstHasRealProof(source: string): boolean {
...
async function main(): Promise<void> {
  ...
  if (lost.length > 0) {
    console.error("移行漏れを検出（Typst 原本に証明があるのに構造化側が TODO のまま）:");
    for (const l of lost) {
      console.error(`  - ${l.id}`);
  ...
  console.log(`no lost proofs: TODO ブロック ${checked} 件はいずれも原本側も未証明`);
}
