# 06. 文献調査：$Z$ → Massieu → 2 次元 Ising 厳密解（臨界指数）

**問い**：分配関数 $Z$ から Massieu 自由エントロピー（[04](04_分配関数と自由エネルギー.md)）を介して 2 次元 Ising の厳密解（臨界指数）を導く先行研究はあるか。

**要点（結論先出し）**：可算（$\Lambda$・$\overline{\mathbb{Q}}$）枠そのものを使った論文は無い（枠付けが本プロジェクト固有）。だが [05](05_相転移はΦで論じられるか.md) (a) の「Massieu $=\log Z$ ＋代数的零点 → 臨界指数」と**数学的に同一の研究プログラム**が確立している ── **Lee–Yang / Fisher 零点による臨界指数決定**。整数係数分配関数 $=Z$、零点 $=$ 代数的 $\in\overline{\mathbb{Q}}$、指数抽出 $=N\to\infty$ スケーリング $=\mathbb{R}$、という対応が綺麗につく。可算で再定式化する点だけが未踏。

## 1. なぜ Lee–Yang / Fisher が一致なのか

[05](05_相転移はΦで論じられるか.md) (a) の主張：有限格子で $Z_N(x)=\sum_n\Omega_N(n)\,x^n\in\mathbb{Z}[x]$（整数係数）→ 零点は代数的 $\in\overline{\mathbb{Q}}$ → 転移は零点の正実軸への詰め寄り。文献での対応：

- 有限 Ising 分配関数は低温変数 $x=e^{-2\beta J}$（または $\tanh\beta J$）で**整数係数多項式**（Kaufman 多項式・再帰関係）。係数は各エネルギーの配置数 $\Omega_N\in\mathbb{N}$＝まさに $Z$。
- その零点（Fisher 零点＝複素温度、Lee–Yang 零点＝複素磁場）から**有限サイズスケーリングで臨界指数 $\nu,\sigma$ 等を抽出**。
- 2 次元 Ising の自己双対点では、Boltzmann 変数の臨界値 $x_c=\sqrt2-1\in\overline{\mathbb{Q}}$（$u^2+2u-1=0$ の正根）＝**代数的**。$\mathbb{R}$ 移行は $x_c\to\beta_c=\frac{1}{2J}\ln(1+\sqrt2)$ の $\ln$、Brillouin 連続積分、指数抽出のみ。

つまり (a) は Lee–Yang/Fisher プログラムを「零点の代数性」と「可算な量化言明」として読み直したものに相当。
...
2. **「零点が代数的 $\in\overline{\mathbb{Q}}$」「感受率の非有界性 $\sup_N\chi_N=\infty$ を $\forall M\in\mathbb{Q}\,\exists N$ で書く」という可算な再定式化を明示的にやった文献は見当たらない**。Lee–Yang/Fisher は数値的・解析的に $\mathbb{C}$ で扱う。→ 本プロジェクト固有の貢献ポイントになりうる。
3. 構成的／計算可能／可述的（reverse math 的）な統計力学で Ising 指数を扱う系統は今回ヒットせず。空白。

## 4. 一言

**「$Z$→Massieu→Ising 臨界指数」の実体は Lee–Yang/Fisher 零点プログラム（特に arXiv:1905.02379）。整数係数分配関数＝$Z$、零点＝代数的＝$\overline{\mathbb{Q}}$、指数抽出＝$N\to\infty$ スケーリング＝$\mathbb{R}$、という対応がつく。可算（$\Lambda$ と $\overline{\mathbb{Q}}$）で再定式化する点だけが未踏で、そこが本プロジェクトの差分。**
