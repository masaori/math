...
⇒ **6-1 の成熟形**：自由エネルギー（$\infty$ の特殊値）と $p$ 進エントロピー（$p$ の特殊値）は、同一 $P$ の**一つの $L$ 函数の異なる素点での特殊値**。双対＝可解性の特徴づけ＝同じ数論的特殊性の二つの顔。**この数論の詳細は `docs/research/Boyd-Deninger-Beilinson/`**（Mahler 測度＝$L$ 値、Beilinson 正則子、Boyd 予想、Besser–Deninger $p$ 進版）にまとめた。
...
### 9.4 ゴールの定式化：グラフ数え上げを素材とする

**素材の再設定**: 可積分格子模型に限らず、**グラフ数え上げ問題**（完全マッチング＝dimer、全域木、固有彩色、独立集合、Euler 向き…）を素材とする。いずれも整数列 $Z_N\in\mathbb{Z}_{>0}$ で、周期的（転送行列 $Z_N=\mathrm{Tr}(T^N)$ 型）なら $\Phi_N=\log Z_N\in\Lambda$ と成長率 $\frac1N\log Z_N\to\log\lambda_{\max}=\log m(P)$（スペクトル曲線 $P$ の Mahler 測度）を持つ。

**問いの精密化**: 成長率（極限）は Lind–Schmidt–Ward で**ほぼ常に存在**する（$\mathbb{Z}^d$ 作用の位相的エントロピー＝Mahler 測度）。ゆえ問いは「極限が存在するか」でなく「**極限が特殊値／閉形式か（$\{P=0\}$ が数論的に特殊か）**」。ゴール ＝ **グラフ数え上げ問題の中から $\log Z_N$ の極限が特殊 $L$ 値になるものを探す・分類する**。

**具体例（既存）**:
| 数え上げ | $\log Z_N$ の極限 | 型 |
|---|---|---|
| $\mathbb{Z}^2$ 全域木 | $\frac4\pi\beta(2)=\frac4\pi G$（Catalan, $\beta(2)=L(\chi_{-4},2)$）| Dirichlet $L$（種数0）|
| dimer/菱形タイリング | Kasteleyn 曲線の Mahler 測度（KOS）| 曲線次第で $L$ 値 |
| $m(1{+}x{+}y)$（Smyth）| $\frac{3\sqrt3}{4\pi}L(\chi_{-3},2)$ | Dirichlet $L$（種数0）|
| Ising | Onsager（臨界 Dirichlet $L$、特殊温度 楕円 $L$）| 種数0/1 |
| hard hexagon | Baxter（代数的, 黄金比系）| 代数的 |
...
**この方向の既存研究の広域リサーチ**: `docs/research/数え上げエントロピーと特殊値/` に集約（4 領域：Mahler 測度＝$L$ 値の既知結果／数え上げ成長率が特殊値になる例／スペクトル曲線の数論とトロピカル／周期と特殊値の一般論）。確立（LSW/Lyons/KOS = エントロピー＝Mahler 測度、多数の $m(P)=L$ 値証明例）と予想（一般 Boyd/Beilinson、motivic 分類）を分別。
