...
**注意 6.7（$e_{F_2}$ の帰属）.** 上の四素片 $W(a),W(a^{-1}),W(b),W(b^{-1})$ は単位元 $e_{F_2}$ を含まない（（分割）で $e_{F_2}$ は独立成分）。$(\ast a)$ 右辺 $a\cdot_{F_2}W(a^{-1})=F_2\setminus W(a)$ は $e_{F_2}$ を含むので、第一分解は $e_{F_2}$ を $g_2\cdot_{F_2}A_2$ 側で回収する（$e_{F_2}=a\cdot_{F_2}a^{-1}$, $a^{-1}\in W(a^{-1})=A_2$）。同様に第二分解も $e_{F_2}$ を $h_2\cdot_{F_2}B_2$ 側で回収。両分解が独立に $e_{F_2}$ を復元するので、定義 6.2（二つの復元は素片集合 $\{A_i\}$ と $\{B_k\}$ が互いに素であればよく、復元後の $E$ は同一でよい）と整合。厳密な逐語版は Wagon *The Banach–Tarski Paradox* 定理 5.2 を参照。

**注意 6.7（$e_{F_2}$ と有限例外の再配分）.** 上の素朴な分割では単位元 $e_{F_2}$ と一部の語の帰属が二重になる。標準的には、$W(a^{-1})$ を「$a^{-n}$（$n\ge1$）の形の語」とそれ以外に細分し $e_{F_2}$ を吸収する（Wagon 1985 §5.3 の整理）ことで、完全に互いに素な $A_1,A_2,B_1,B_2$（各4片）と移動元を取れる。結論（$F_2$ パラドキシカル）は変わらない。厳密な逐語版は Wagon *The Banach–Tarski Paradox* 定理 5.2 を参照。
