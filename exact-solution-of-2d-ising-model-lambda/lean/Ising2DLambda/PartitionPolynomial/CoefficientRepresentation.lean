...
人手証明は 5 つの等号からなる一続きの式である（Step という段を持たない）。
各等号とこのファイルの対応:

  第 1 の等号（分配多項式の定義）        partitionPolynomial の展開
  第 2 の等号（配位全体の類別）          CoefficientSum.biUnion_brokenFiber（被覆）
                                        brokenFiber_pairwiseDisjoint（互いに素）
                                        Finset.sum_biUnion（和を類ごとに束ねる）
  第 3 の等号（類の中では b(σ) = m）     sum_brokenFiber_monomial の中の `hterm`
  第 4 の等号（同じ元を |A_{L,m}| 個）   sum_brokenFiber_monomial の中の `Finset.sum_const`
  第 5 の等号（多重度の定義）            CoefficientSum.multiplicity_eq_card_brokenFiber

  結論そのもの                          partitionPolynomial_eq_sum_multiplicity

第 2 の等号が引く類別は、主張「多重度の総和は配位の総数に等しい」の具体版で既に自分で
示してある（被覆と互いに素性）。同じ類別を使うので引き写さずにそれを引く。
そこで使う「互いに素な有限個の有限集合の合併の上の和は、各集合の上の和の和」は
...
/-- 第 2 の等号のうち互いに素であることを、`Finset.sum_biUnion` が要求する形へ言い換えたもの。
...
/-- 第 3・第 4・第 5 の等号。1 つの類 `A_{L,m}` の中では `b(σ) = m` なので、足し合わせる単項式は
どれも `x^m` に等しく（第 3）、類の上の和は `x^m` を `|A_{L,m}|` 個足したものになり（第 4）、
その個数が多重度 `Ω_L(m)` である（第 5）。 -/
lemma sum_brokenFiber_monomial (m : ℕ) :
  ...
  -- 第 4 の等号: 同じ元を `|A_{L,m}|` 個足したものは `|A_{L,m}| • x^m`。
  -- 第 5 の等号: この個数が多重度である（多重度の定義）。
  rw [multiplicity_eq_card_brokenFiber, nsmul_eq_mul]
  simp

/-- 結論。分配多項式の `x^m` の係数は多重度 `Ω_L(m)` である。 -/
theorem partitionPolynomial_eq_sum_multiplicity :
    partitionPolynomial L
      = ∑ m ∈ range (2 * L ^ 2 + 1),
          Polynomial.C (multiplicity L m : ℤ) * Polynomial.X ^ m := by
  -- 第 1 の等号（分配多項式の定義）と、第 2 の等号のうち被覆:
  -- 配位全体を破れボンド数の値ごとの類の合併として書き直す。
  rw [partitionPolynomial, ← biUnion_brokenFiber L]
  -- 第 2 の等号の残り: 互いに素なので、合併の上の和は各類の上の和の和になる。
  rw [sum_biUnion (brokenFiber_pairwiseDisjoint L)]
  -- 第 3・第 4・第 5 の等号: 各類の上の和を計算する。
