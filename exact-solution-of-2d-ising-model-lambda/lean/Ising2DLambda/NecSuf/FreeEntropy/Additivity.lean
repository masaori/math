...
加法性（`sub_add_sub_of_mul`）:
  ...
  使っている性質            なぜ削れないか
  `AddCommGroup G`          「引き算どうしの和」へ並べ替える第 5 の等号に、逆元と可換性の両方が要る。
                            可換モノイドでは `(x₁+x₂)-(y₁+y₂) = (x₁-y₁)+(x₂-y₂)` を書けない。
  `e (m * n) = e m + e n`   第 4 の等号で分子側と分母側の両方へ適用する。
  （`m ≠ 0`, `n ≠ 0` のとき）
...
冪の法則（`map_pow_eq_nsmul`）:
  ...
  使っている性質            なぜ削れないか
  `Monoid M`                `x^k` を書き、`x^(k+1) = x^k * x` を使う帰納段の第 1 の等号に要る。
  `AddCommMonoid G`         帰納段の第 4 の等号 `k • g + g = (k+1) • g` に要る（逆元は使わない。
                            冪の法則は引き算を一度も使っていない）。
  述語 `P` と閉性           加法性が使えるのは述語を満たす元どうしの積に限るので、
                            `P 1`・`P x` から `P (x^k)` を作れないと帰納段の第 2 の等号で適用できない
                            （具体版ではこれが「正の有理数の積は正の有理数」にあたる）。
  `f 1 = 0`                 帰納法の出発点（`k = 0` の段）そのもの。

証明手順は具体版と同じである（加法性は第 4・第 5 の等号、冪の法則は `k` についての帰納法の 2 段）。
加法性の前置きと第 2 の等号（表示を取り、積の表示を作ること）は具体版に固有の書き換えなので、
ここでは仮定として受け取る。
...
/-- 加法性の第 4・第 5 の等号。`e` が積を和へ移すなら、分子どうし・分母どうしを掛けた表示の差は
それぞれの差の和である。 -/
theorem sub_add_sub_of_mul {G : Type*} [AddCommGroup G] (e : ℕ → G)
  ...
  -- 第 4 の等号。加法性を分子側と分母側の両方へ適用する。
  rw [he ha₁ ha₂, he hb₁ hb₂]
  -- 第 5 の等号。ℤ の中の並べ替えにあたる部分（可換群の中で引き算どうしの和へ直す）。
  abel

/-- 冪の法則（帰納法の 2 段）。`f` が述語 `P` の上で積を和へ移し `f 1 = 0` を満たすなら、
...
theorem map_pow_eq_nsmul {M : Type*} [Monoid M] {G : Type*} [AddCommMonoid G]
  ...
  | 0 => by
    -- 帰納法の出発点（k = 0 の段）。x^0 = 1 なので、両辺とも単位元である。
    rw [pow_zero, h1, zero_nsmul]
  | k + 1 => by
    -- 帰納段の第 1・第 2 の等号。x^(k+1) = x^k · x へ加法性を適用する（P が積で閉じているので適用できる）。
    have hxk : P (x ^ k) := by
      induction k with
