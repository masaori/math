...
1. `hφO` / `hφO'` / `hφinvO` / `hφinvO'`。Υ が O × O' の上の全単射であることに要る。
   証明が `φ` について使うのは `φ` と `φ⁻¹` の往復（`Equiv` の 2 本の規則）だけであり、
   単射性を述べた補題は一度も引いていない。ただし**往復は単射性より強い**ので、
   これを「単射性が要らない」とは書かない。`Equiv.Perm` を単射な写像へ弱められるか、が正しい問いで、
   答えは「弱められない」である（`Finset.card_nbij'` に逆向きの写像が要る）。
...
**型の有限性についての注意（誤読しやすいので明示する）。**
`Fintype ι` というインスタンスは仮定に書いていないが、`card_crossInv_eq_two_mul` については
**有限性が仮定から導ける**。`htotal` により相異なる 2 点 a, b には `lt a b` か `lt b a` の
一方が成り立ち、`hP` によりその対は `P` に属するので、ι の 2 元部分集合から有限集合 `P` への
単射ができ、ι は有限になる。したがってこの主張について
「型の有限性を使っていない」とは書けない——書けるのは
「有限性をインスタンスとして別に置く必要がない（`hP` と `htotal` に含まれている）」までである。
有限性がほんとうに要らないのは `card_pairs_image_eq` の側だけで、
そちらは `P` も `htotal` も使わないので ι は無限でよい。

以上を除けば、具体版との差は次のとおりである。行配位であること・巡回シフト `S` があること・
O が軌道であることはいずれも使っていない。すなわちこの 2 主張は「交わらない 2 つの有限集合」と
