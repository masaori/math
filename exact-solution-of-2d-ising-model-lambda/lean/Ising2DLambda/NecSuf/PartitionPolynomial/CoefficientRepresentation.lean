...
不定元の冪であること・格子の形・スピンの値が `{+1,-1}` であることは、どこにも使っていない。
  ...
  `Fintype α`               類 `fiber f m` を有限集合として扱い、その上の和を取るため。
                            無限集合では第 2 の等号の「和の和」が意味をなさない。
  `DecidableEq α`           第 2 の等号で使う `Finset.sum_biUnion` が合併を取るのに要る。
                            決定可能でないと `biUnion` が定義できない。
  `f a ≤ N`（有界性）       第 2 の等号の被覆に要る。これが無いと `f a` の類が
                            添字の範囲 `{0,…,N}` の外に出て、合併が全体にならない。
  `AddCommMonoid M`         第 2 の等号で和の順序を組み替えるのに可換性と結合性が要る。
                            第 4 の等号で「同じ元を `k` 個足したもの」を `k • g m` と書くのに
...
証明手順は具体版と同じで、人手証明の 5 つの等号をこの順にたどる（別の論法へ差し替えていない）。
第 2 の等号が使う類別は、主張「多重度の総和は配位の総数に等しい」の必要十分版で示した
...
omit [DecidableEq α] in
/-- 第 3・第 4 の等号。1 つの類 `fiber f m` の中では `f a = m` なので、足し合わせる値は
どれも `g m` に等しく（第 3）、類の上の和はそれを `|fiber f m|` 個足したものになる（第 4）。 -/
...
/-- 結論。値 `f a` だけで決まる量の全体和は、値ごとの個数で束ねられる
（具体版では、この個数が多重度であることが第 5 の等号にあたる）。 -/
theorem sum_comp_eq_sum_nsmul {M : Type*} [AddCommMonoid M] (g : ℕ → M) (hf : ∀ a, f a ≤ N) :
  ...
  -- 第 2 の等号の残り（互いに素なので合併の上の和は各類の上の和の和）
