/-
「開境界正方形の密度の下組は周期境界の密度の下組に含まれる（Archimedes 性。q は 1 以下）」の具体版を、
必要十分版 `lowerSetOfSequence_subset_of_eventually_le_add_error_necSuf` の特殊化として導く。
渡すのは、推移律・右加法単調性（`rationalLogOrderLE_add_right`）、証人の半分（`(1/2)·ε` と準備の第一の三性質）、
誤差の列 `L ↦ (2/L)·ι(log q)` が正の元の逆元をやがて下回らないこと（準備の第二〜第四: Archimedes 性の倍率 `n`
と `rationalLogOrderLE_neg_le_scaled_toRational_logRat`）、`L ≥ 1` での項ごとの比較
（`rationalLogOrderLE_periodicOpenDensity_bounds_of_le_one` の左）である。
...
theorem openSquareDensityLowerSet_subset_periodicDensityLowerSet_of_le_one_from_necSuf
    {q : ℚ} (hq0 : 0 < q) (hq1 : q ≤ 1) :
    openSquareDensityLowerSet q ⊆ periodicDensityLowerSet q := by
  show NecSuf.ThermodynamicLimit.lowerSetOfSequence rationalLogOrderLE (openSquareDensitySequence q) ⊆
    NecSuf.ThermodynamicLimit.lowerSetOfSequence rationalLogOrderLE (periodicDensitySequence q)
  refine NecSuf.ThermodynamicLimit.lowerSetOfSequence_subset_of_eventually_le_add_error_necSuf
    rationalLogOrderLE
    (fun hxy hyz => rationalLogOrderLE_trans hxy hyz)
    (fun z hxy => rationalLogOrderLE_add_right hxy z)
    ?_ _ _ (fun L => ((2 : ℚ) / (L : ℚ)) • toRational (logRat q)) ?_ ?_
  · -- 証人の半分（準備の第一）
    intro ε hε0 hεne
