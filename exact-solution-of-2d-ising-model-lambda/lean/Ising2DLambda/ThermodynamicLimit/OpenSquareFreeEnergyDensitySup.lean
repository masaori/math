...
定義ブロックなので必要十分版は無い。
名前の末尾の `Sup` は、削除済みの旧実数値経路のファイル `OpenSquareFreeEnergyDensity.lean`（本文に対応するブロックが
無く、後片付け待ち）と名前が衝突しないために付けた。旧経路を消したら `Sup` を外してよい。
  ...
  完備性で上限を取る: f^op(q) := sup ρ_ℝ(A^op(q))                           `openSquareFreeEnergyDensitySup`（`sSup`）
  上限の特徴づけ: 上界である／最小である                                    `realizeRational_le_openSquareFreeEnergyDensitySup`（`le_csSup`）
                                                                              `openSquareFreeEnergyDensitySup_le_of_forall_le`（`csSup_le`）
...
実数体の完備性はここで使う。 -/
noncomputable def openSquareFreeEnergyDensitySup (q : ℚ) : ℝ :=
  sSup (openSquareRealizedLowerSet q)

/-- 上限は上界である: `μ ∈ A^op(q)` なら `ρ_ℝ(μ) ≤ f^op(q)`。 -/
theorem realizeRational_le_openSquareFreeEnergyDensitySup {q : ℚ} (hq : 0 < q)
    {μ : RationalLogOrderGroup} (hμ : μ ∈ openSquareDensityLowerSet q) :
    realizeRational μ ≤ openSquareFreeEnergyDensitySup q :=
  le_csSup (openSquareRealizedLowerSet_bddAbove hq) ⟨μ, hμ, rfl⟩

/-- 上限は最小の上界である: すべての `μ ∈ A^op(q)` で `ρ_ℝ(μ) ≤ b` なら `f^op(q) ≤ b`。 -/
theorem openSquareFreeEnergyDensitySup_le_of_forall_le {q : ℚ} (hq : 0 < q) {b : ℝ}
    (hb : ∀ μ ∈ openSquareDensityLowerSet q, realizeRational μ ≤ b) :
    openSquareFreeEnergyDensitySup q ≤ b := by
