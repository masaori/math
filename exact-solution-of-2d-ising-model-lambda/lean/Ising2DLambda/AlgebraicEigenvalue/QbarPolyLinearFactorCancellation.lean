...
theorem qbarPolyLinearFactorCancellation (w : Qbar) (A B : QbarPoly) (n : ℕ)
  ...
  have hzero : (Polynomial.X - qbarConst w).coeff 0 = -w := by
    calc
      (Polynomial.X - qbarConst w).coeff 0 = (Polynomial.X : QbarPoly).coeff 0 - (qbarConst w).coeff 0 := coeff_sub _ _ _
      _ = 0 - w := by simp only [qbarConst, coeff_X_zero, coeff_C_zero]
      _ = -w := zero_sub w
  have hone : (Polynomial.X - qbarConst w).coeff 1 = 1 := by
    calc
      (Polynomial.X - qbarConst w).coeff 1 = (Polynomial.X : QbarPoly).coeff 1 - (qbarConst w).coeff 1 := coeff_sub _ _ _
      _ = 1 - 0 := by simp only [qbarConst, coeff_X_one, coeff_C_succ]
      _ = 1 := sub_zero 1
  have hhigher (i : ℕ) (hi : 2 ≤ i) : (Polynomial.X - qbarConst w).coeff i = 0 := by
    calc
      (Polynomial.X - qbarConst w).coeff i = (Polynomial.X : QbarPoly).coeff i - (qbarConst w).coeff i := coeff_sub _ _ _
      _ = 0 - 0 := by
        rw [coeff_X_of_ne_one (by omega)]
        change 0 - (Polynomial.C w).coeff i = 0 - 0
        rw [coeff_C_of_ne_zero (by omega)]
      _ = 0 := sub_self 0
  have hproduct (P : QbarPoly) (m : ℕ) :
      ((Polynomial.X - qbarConst w) * P).coeff (m + 1) = P.coeff m + (-w) * P.coeff (m + 1) := by
    let f : ℕ → Qbar := fun i => (Polynomial.X - qbarConst w).coeff i * P.coeff (m + 1 - i)
    ...
    calc
      ((Polynomial.X - qbarConst w) * P).coeff (m + 1)
          = ∑ i ∈ Finset.range (m + 1 + 1), (Polynomial.X - qbarConst w).coeff i * P.coeff (m + 1 - i) := by
            rw [coeff_mul, Finset.Nat.sum_antidiagonal_eq_sum_range_succ_mk]
      _ = (Polynomial.X - qbarConst w).coeff 0 * P.coeff (m + 1) + (Polynomial.X - qbarConst w).coeff 1 * P.coeff m +
          ∑ i ∈ Finset.Ico 2 (m + 1 + 1), (Polynomial.X - qbarConst w).coeff i * P.coeff (m + 1 - i) := by
