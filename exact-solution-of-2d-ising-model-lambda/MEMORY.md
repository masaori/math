...
次は、$\mu_L$ の元を掛ける操作が $\mu_L$ から $\mu_L$ への全単射であることを示す。
逆写像は $w^{L-1}$ を掛ける操作として書く（逆元 $z^{-1}$ を使わない）。
その行き先が $\mu_L$ に収まることは、上の「1 の冪根の冪は 1 の冪根である」が与える。
