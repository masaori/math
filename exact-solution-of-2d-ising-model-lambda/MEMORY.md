...
1. **レビュー**: Fisher 零点の評価・集合・代数性について、本文、SageMath、Lean の具体版・
   必要十分版・導出、入口 import、sorry 登録を突き合わせる。特に $L=1$ では根が無いことを
   存在主張へすり替えていないかを見る。
2. **次に進めるセクションは「Kramers--Wannier 双対」**。指数関数・実結合を入口に置かず、
   高温展開と低温展開の有限な数え上げから可算な多項式恒等式として導く。
3. **並列の作業ストリーム（式変形の書き方の統一）を毎 tick 1 件進める**
   （次は姉妹側の「$\det A(\theta_\mu)=1$」の Step 4）。
