...
修正は無かった。次は、新しく取った根が既出の根と相異なることの背理法本体である。
...
**2026-08-11 の tick 120 は、前回成果をレビューし、零でない代数的数の冪が零でないことを
四層すべてで完了させた。**

1. **レビュー**: 零でない代数的数の冪について、本文の帰納法、SageMath、Lean の具体版・
   必要十分版・導出を突き合わせる。
2. **次に進めるセクションは「零でない代数的数の同じ元の有限和は零でないこと」**。
   状態台帳のセクション表が正本であり、着手時に標数 0 の側と埋め込みの論法を数えて割り直す。
