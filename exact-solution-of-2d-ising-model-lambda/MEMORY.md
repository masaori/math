...
2026-08-15 の tick 289 のレビューでは、前 tick の「倍数の辺での上限への任意近接（$1\le t$）」の
本文・SageMath・Lean 三本を突き合わせた。論法は一致していたが、有限モデルの上界検査が ball 差の
下端しか見ていなかったため、分配関数の整数冪による厳密比較へ修正した。次の本文は
「開境界密度の下からの評価と下限の存在」。
