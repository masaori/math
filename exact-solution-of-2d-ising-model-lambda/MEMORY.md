...
反対向きの往復を、横向き辺・縦向き辺それぞれについて定義の代入を含む三段の鎖へ直した。
...
1. **レビュー**: 四セクターの定義について、本文の二つの切断辺集合と Lean・SageMath の辺番号条件が
   一致し、$L=1$ の自己ループも二本の番号つき辺として扱われているかを突き合わせる。
2. **次に進めるセクションは「双対辺写像と破れた辺集合の自明セクター」**。低温側の破れた辺集合を
   高温側の偶部分グラフと暗黙に同一視せず、交差する双対辺へ送る写像を定義してから結ぶ。
3. **並列の作業ストリーム（式変形の書き方の統一）を毎 tick 1 件進める**
   （次は姉妹側の「$T_{(V')}$ の $\psi$ への作用」の Step 3）。
