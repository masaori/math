...
残るのは、有限積の分配則で組にわたる和を軌道ごとの和の積へ組み替える段と、
...
> **2026-08-09 の tick 33 は前進していない。** 機械の負荷平均が 1004（別リポジトリの
> エージェントが多数の型検査を同時に実行）で、構造化テキストの検査だけで 20 分以上かかり、
> 四層を 45 分の上限に収められる見込みが無かった。レビューとその修正だけを push した。
> 下の項目は tick 33 の時点でも未着手のままである。

1. **有限積の分配則で、組にわたる和を軌道ごとの和の積へ組み替えること**
   （章「固有値の代数性」の続き。台帳のセクション 10f'''b3）。
   $\sum_{\alpha\in\mathfrak{A}_L}\prod_{O}W_{O}(\mathrm{ch}(U),\alpha(O))$ を
   $\prod_{O}\bigl(\sum_{\psi}W_{O}(\mathrm{ch}(U),\psi)\bigr)$ の形にする。
   **着手時にまず確かめること**: Lean の $\mathfrak{A}_L$（`OrbitPermFamily`）は
   依存関数型なので、mathlib の既製の分配則が直接は当たらない可能性がある。
   当たらないなら人手証明の分配則をそのまま帰納法で書く。
