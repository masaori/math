...
残るのは、軌道の上の互換とその符号が $-1$ であること、巡回シフトの制限が
$\lvert O\rvert-1$ 個の互換の積であること、そこから符号が $(-1)^{\lvert O\rvert-1}$ で
あること、そして各軌道の因子の和が $t^{\lvert O\rvert}-1$ になる段である。
...
**2026-08-10 の tick 46 は、レビューで本文を 1 件直したうえで、10f'''c2b2（互換の軌道への
制限の符号が $-1$ であること）を四層すべてで完了させた。**
論法は、転倒対の集合 $\mathrm{Inv}_{O}$ を 3 つの部分（$(\tau_a,\tau_b)$ そのもの、
$\tau_a\prec\tau\prec\tau_b$ を満たす $\tau$ が作る 2 種類の対）へ両包含で分け、
$\mathrm{inv}_{O}=2|M|+1$（奇数）を出す、というものである。
必要十分版が示したのは、この議論が順序の非対称性と推移律しか使っていないことである。

1. **レビュー**: 今 tick で書いた本文 2 ブロック（`def_orbit_inversion_set` /
   `claim_orbit_transposition_sign`）と Lean 3 本（`OrbitTranspositionSign` 系）。
   とくに、「互いに素な有限集合の合併の個数は個数の和である」を地の文の根拠として
   使っている点（同じ事実を他所でも使っているので主張として置くか）と、
   $|A|=|M|$ の全単射の全射性を「定義から」で済ませている点を見る。
2. そのあと 10f'''c2c（巡回シフトの制限が $\lvert O\rvert-1$ 個の互換の積）、
