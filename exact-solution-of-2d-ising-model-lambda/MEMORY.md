...
**二候補の選択に平行移動の対称性の障害が無いことを一辺二の全数検査で確定した。** 単一の偶部分グラフでは覆えない 16 ファイバーはどれも、非自明な平行移動（3 通り）で $(D,E)$ が固定されない。従って $(D,E,\varphi)$ を固定する非自明な平行移動は存在せず、候補を二本持つ 1,984 置換のどれについても二候補を入れ替える平行移動はない。さらに 16 ファイバーの集合は平行移動で閉じ、大きさ 4 の軌道がちょうど 4 本に分かれるので、平行移動と可換な選択規則は軌道の代表 4 ファイバー上の選択だけで決まる（`even-subgraph-orbit-translation-stabilizer` とノート `note_kac_ward_claim_selection_even_subgraph_action_character_translation_stabilizer` に固定）。次は代表ファイバー上で、切断・接合位置のデータから二候補の一方を定める規則を調べる。
