...
## 現在の到達点（2026-09-03 の 148 回目の tick 時点）

**固定部分群が非自明な鍵では、位相反転候補辺に固定部分群不変な完全マッチングが存在しないことを確定した。** 一辺二で、置換を持つ全 $609$ ファイバー鍵のうち非零平行移動の固定部分群が非自明なものは $89$ 個。うち $81$ 個は有効な偶部分グラフ $H$ を持たず一様被覆検査（巻き付き文字が非自明な $64$ 鍵）の対象外で、対角方向の固定鍵 $37$ 個は全て対象外だった。対象に入る軸方向固定の $8$ 鍵（固定部分群の位数 $2$、ファイバーの大きさ $32$ が $4$ 個・$288$ が $4$ 個）では、完全マッチング自体は存在するのに固定部分群不変なものが一つも無い（`translation-fixed-fiber-invariant-matching`。不変マッチングと、点別固定部分群が一致する候補辺だけが落ちる頂点軌道の商二部グラフの完全マッチングとの一対一対応で判定し、全数値を等号 assert で固定して再実行した）。従って一辺二の時点で平行移動群全体と可換な符号反転マッチング族は候補辺からは作れない。共変性は固定部分群が自明な軌道に限るか、この $8$ 鍵で候補辺以外の構造を使う必要がある。次はこの $8$ 鍵の商グラフでどの頂点軌道が孤立するかを調べ、共変性を課す部分群を絞る。
