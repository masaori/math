...
**三分類のどの部分和も固定ファイバー内では選択和に一致しないことを一辺二の全数計算で確定し、$D$ の縮約帰納はファイバーを跨ぐ形でなければならないという設計判断を固定した。** ファイバー位相和を順序に依らない三分類（接触なし・切り替え可能対あり・全対切り替え不能）の部分和 $S_Z,S_X,S_Y$ へ分けると、全 $609$ ファイバー×四スピン構造の $2{,}436$ 組で $S_Z+S_X+S_Y=U$（選択和）は成立するが、$S_X\ne0$ が $736$ 組、$S_Y\ne0$ が $648$ 組、$S_Z\ne U$ が $1{,}384$ 組ある。$S_Y\ne0$ の最初の例は $D=\{v(0,1)\}$、$E=\{h(0,1),h(1,1),v(0,0),v(1,1)\}$、$(a,b)=(0,0)$ で $S_Y=-2$。切り替え平滑化は $D$ を変えてファイバーを移すので、固定ファイバー内の符号反転対合だけでは残余を消せない。観察を `note_kac_ward_claim_fully_unswitchable_contacts_witness_doubled_edges_fiber_phase_sums` と SageMath `fully-unswitchable-fiber-phase-sums` に固定した。

次 tick は、縮約帰納の一段として、完全切り替え不能な残余の接触が指す反転対の辺の平滑化がファイバー $(D,E)$ をどう移すか（$D$ の減少と位相寄与の変化）を確定する。

前進前レビューでは、前 tick の符号保存対応の不存在の有限反例をノート・overview・台帳・MEMORY で照合した。件数と最初の例は検算と一致し、「何も言っていない主張」や不一致は無かったため、レビュー修正は無かった。
