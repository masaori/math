...
1. **レビュー**: 全スピン反転の定義と各辺の破れの不変性について、本文、SageMath、Lean の具体版・
   必要十分版・導出、入口 import、sorry 登録を突き合わせる。
2. **次に進めるセクションは「破れた辺の集合の二つの原像」**。同じ破れた辺の集合を与える二つの
   配位は互いに等しいか全スピン反転で結ばれることを、周期格子の連結性を明示して示す。
3. **並列の作業ストリーム（式変形の書き方の統一）を毎 tick 1 件進める**
   （次は姉妹側の「$\det A(\theta_\mu)=1$」の Step 5）。
