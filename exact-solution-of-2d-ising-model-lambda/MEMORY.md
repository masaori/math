...
残るのは、巡回シフトの制限が $\lvert O\rvert-1$ 個の互換の積であること
（$\Psi^{O,\tau_0}_{\lvert O\rvert-1}=S\!\restriction_{O}$）、そこから符号が
$(-1)^{\lvert O\rvert-1}$ であること、そして各軌道の因子の和が $t^{\lvert O\rvert}-1$ になる段である。
