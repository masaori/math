...
2026-08-14 の tick 245 のレビューでは、前 tick の「実代数的数の順序」が一つの定義ブロックに
狭義順序・広義順序・三分法を同居させていたため、「1 ブロック 1 主張」に従って二つの定義と
一つの主張へ分割し、SageMath の対象ラベルも同じ粒度へ揃えた。前進前に main へ反映した。
