...
2026-08-13 の tick 193 のレビューで、「一と $s$ の一次独立性」の SageMath 検証が
仮定から $b\cdot s=-a$ を得る移項の五段のうち、仮定に依存しない恒等変形を直接検査せず、
対偶の標本検査だけで代用していると分かった。`QQbar` 上の各中間式の等号を
4324 組で検査するよう補った。本文の背理法、対偶の厳密標本検査、姉妹側の
「エルミート行列の $\exp$ は正定値」 (1) の書式変更には修正を要する不一致は無い。
