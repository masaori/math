...
`008_TV1_hatZ_hatY_part2` に残る根拠なしの鎖から続ける。
...
1. **レビュー**: 高温展開の四セクター分解の Lean 三層を本文と突き合わせ、有限和の分割手順と
   必要十分版の仮定が一致しているか、四つの組の書き並べ方が本文の順序と一致するかを見る。
2. **次に進めるセクションは「四境界条件の混合の双対恒等式の組み立て」**。
   状態台帳の先頭行だけを進める。
