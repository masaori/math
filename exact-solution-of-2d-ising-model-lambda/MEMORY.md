...
`def_real_algebraic_order`（$a<_R b:\Leftrightarrow\exists w\ne0,\ b-a=w\cdot w$、
$a\le_R b$。順序の三分法は $z:=b-a$ への平方の三分法から従う）。順序と加法・乗法の両立、
...
三ブロックを有限標本で厳密検査した（次数 2 以下に限る理由は overview に注記）。
**Lean は未着手**で、次 tick はこの三ブロックの Lean（具体版・必要十分版・導出）を進める。
次の式変形統一は姉妹側「トレース冪の挟み撃ち」
Step 3 冒頭の比の単調性と積表示から続ける。次の本文は「零点と有理点の距離の二乗」
...
- **レビュー**: 「非負係数条件と負の第一係数条件の積」の Lean 三本を本文・SageMath と突き合わせる。
- **次に進めるセクションは「負の第二係数条件どうしの積」**。状態台帳の先頭行だけを進める。
- **並列の作業ストリーム（式変形の書き方の統一）を毎 tick 1 件進める**。
  次は姉妹側 `010_transfer_matrix_bridge` の次の根拠なしの計算を機械走査で特定する。
