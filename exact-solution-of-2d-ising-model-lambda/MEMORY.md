...
**弧署名の三水準圧縮はいずれも頂点項の分解を保たないことを有限検査で確定した。** counts は型 $8{,}890$ 種・階数 $5{,}898$・直接衝突 $243$ 件、turnword は型 $9{,}225$ 種・階数 $6{,}198$・直接衝突 $160$ 件、turnwrapword は型 $9{,}998$ 種・階数 $6{,}708$・直接衝突 $65$ 件で、全て合同線型系に解が無かった（`parity-identity-simple-cycle-arc-signature-compression`）。とくに内部頂点の $D/C$ 所属だけを落とす turnwrapword も失敗するため、閉じた弧公式には所属情報の一部を残す必要がある。観測直後に結果を assert へ固定し、overview を PASS へ更新した。本文主張と Lean の追加対象はない。次は直接衝突を比較し、内部所属情報のどの位置が必要かを切り分ける。
