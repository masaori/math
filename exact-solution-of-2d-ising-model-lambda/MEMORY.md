...
## 現在の到達点（2026-08-30 の 32 回目の tick 時点）

**台の辺を両向きに使う閉歩道の Kac--Ward 展開での扱いを、厳密計算の観察で決着させた。** 一辺 $L=2,3$ の周期正方格子で、四つの Kac--Ward 行列式 $\det(I-x\,M^{a,b})$ を $\mathbb Q(\zeta_8)$ 係数多項式として厳密計算し、偶部分グラフ $A$ の切断線偶奇 $(h,v)$ に符号 $(-1)^{c h+d v+e hv}$（全 8 候補）を付けた多項式の平方と比較した（新検算 `torus-kac-ward-even-subgraph-square`）。結果、四つのスピン構造すべてで $\det(I-x\,M^{a,b})=\bigl(\sum_A(-1)^{(1+a)h(A)+(1+b)v(A)+h(A)v(A)}x^{|A|}\bigr)^2$ が成り立った（$(0,1)$ と $(1,0)$ での候補の二重一致は正方トーラスの軸対称でホモロジー類 $(1,0)$・$(0,1)$ の類別和が一致するため）。帰結: 行列式の置換展開のうち台の辺を両向きに使う項は**相殺で消えるのではなく平方の交差項を担う**ので、除外の分解は不要であり、目標は「行列式＝符号付き偶部分グラフ多項式の平方」という行列式レベルの恒等式である。反復分解（接触対数の整礎帰納）はこの恒等式の証明で各閉路の回転位相符号を確定するために使う。観察は notes（`note_kac_ward_definition_determinants_even_subgraph_square_observation`）に記録した（一般の $L$ は未証明）。次は行列式の置換展開の非零項が向き付き辺が相異なる非後退閉路の族で尽くされることを本文の主張として固定する。
