...
**1. Onsager の閉形式へは、この論文はつながっていない。** 自由エネルギー密度は $\Lambda_{\mathbb{Q}}$ の
Cauchy 列と下組として存在を示すところまでで、$-\beta f=\log2+\frac1{2\pi}\int\log(\dots)$ の形の
**閉形式は導いていない。本文にも台帳にもその予定は無い。** 閉形式は姉妹プロジェクト
`exact-solution-of-2d-ising-model` の `012_free_energy.ts`「Onsager の自由エネルギーの表式」にあり、
そちらは転送行列の対角化とフェルミオン化を経由する（実数体・複素数体を使う）。
**この論文で閉形式まで行くかどうかは未決である。** 行くなら、可算側でどこまで運び、どこで
実数体へ脱出するかを先に決める必要がある（現状の脱出点は「下組の上限」の一点だけである）。

**2. 台帳の todo 表が空である。** 自動ループは tick 458 以降、前進する項目が無く、姉妹側の
式変形統一だけを回している。$L=2,\dots,8$ の七点の観察からは、先頭距離の列についての
可算な命題候補が立っていない。**命題候補が立つか、上の 1 を決めるまで、本文は伸びない。**

**3. 行き止まりの主張 18 件の切り分け。** 引用漏れか不要かを 1 件ずつ読んで判断する。
うち 4 件は転送行列章にあり（配位と行配位の族の対応、配位の重み、行列の冪の成分、閉じた道との対応）、
トレース定理の証明が `\blkref` で引いていない可能性が高い。

**4. 道具の章に入れていない道具が 63 件ある。** 節の中に本論のブロックが混ざっていて節ごと
動かせないため。節を割れば道具の章へ移せる（二次体と正錐・実閉部分体・代数的数係数の多項式・
有理係数の対数順序群）。

**5. 「臨界指数を零点列で書く」の続き**（本文末尾の TODO）。先頭距離の列
