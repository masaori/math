...
実験スクリプト `sagemath/check/finite-size-scaling-observation/l3-enlarged-stack-experiment.sage` へ段階・根ごとの経過秒つきログ（`l3-stage-log-2026-08-18.txt` へ永続化）を入れ、8 GiB スタックで再実行した。分配多項式（次数 12）の構成・全 12 根の `QQbar` での分離・距離の二乗の 256 bit 包含区間・先頭候補の分離検証はすべて 0.1 秒以内に完了し、最後の `candidate_distance.minpoly()`（先頭距離 $d_1(3)$ の `AA` 上の最小多項式計算）だけが 480 秒内に完了しなかった（スタック超過は再発せず）。律速は合成的な代数的数の最小多項式化（PARI の数体統合）である。次の検算として「終結式による $d_1(3)$ の定義多項式の直接構成」を台帳の todo へ積んだ（零点の最小多項式と臨界点の定義多項式から $\mathbb Z[x]$ 上の消去計算で構成し、既存の包含区間で既約因子を選ぶ）。本文・Lean への変更なし。観察ノートと overview は確定した内容へ更新した。
