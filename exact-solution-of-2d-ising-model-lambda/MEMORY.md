...
**標準形の四寄与は、鍵から計算できる幾何統計の $\mathbb F_2$ 線型結合でも個別には書けないことを全数で固定した。** 鍵 $(D,E)$ から機械的に計算できる幾何統計 $15$ 本（辺数・二重辺数・巻き付き偶奇・交差対・成分数・頂点数・次数 $4$ 頂点数の各偶奇とその積）を候補基底に、一辺二の全 $369$ 鍵と一辺三の $D=\varnothing$ の自明文字 $677$ 鍵で線型方程式を厳密に解くと、書けるのは動辺数（$=|E|$ の偶奇。自明）と総和（標的の位相式）だけで、残る三寄与と三通りの二寄与和はいずれも書けない（`parity-identity-standard-form-statistics`）。従って一般証明は、寄与を組み替えて大域統計へ帰着する経路では組めず、四寄与を一括で扱い辺対ごとの寄与を逐次相殺する形で組む。次はその局所対応（切断線辺対と内部辺対を対にする規則）を構成する。
