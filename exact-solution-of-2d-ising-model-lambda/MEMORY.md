...
2026-08-18 の tick 432 は、律速の最小多項式計算を含まない区間経路で $L=4$ の観察を完了し、標本を三点へ増やした。`sagemath/check/finite-size-scaling-observation/l4-interval-observation.sage` で、分配多項式の構成・全 32 根の `QQbar` での分離・距離の二乗の 256 bit 包含区間・先頭候補の分離検証が 2.2 秒で完了した（段階別ログ `l4-stage-log-2026-08-18.txt`）。$Z_4=2\,q_4(x)$（$q_4$ はモニックで定数項 1 の次数 32 の既約多項式）。実対数へ脱出した読みは $a_\rho(4)\approx1.1990376682431301$。読みの列は約 $1.130,1.211,1.199$（$L=2,3,4$）で単調でなく、三点でも一般則・収束先を一意に定めないため本文へ昇格する命題候補は無い。$d_1(4)$ の定義多項式（終結式）は未実施（$L=3$ の次数 96 に対し次数 2048 の消去多項式の既約分解の可否が別問題）。本文・Lean への変更なし。
