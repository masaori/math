...
Lambda 側は check（948 ブロック・参照すべて解決）・linkage（430 検算）・対象 SageMath（覆えない 16 ファイバー・候補辺 4,608、分裂 4,096・合併 512 を assert で固定、5 分 4 秒）・PDF を通した。新しい成果は観察ノートと有限検算であり、本文主張と Lean の追加対象はない。
