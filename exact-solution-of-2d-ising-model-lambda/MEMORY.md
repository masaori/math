...
次は、シフト行列 $U$ と転送行列 $T$ を $\mathrm{Ev}_{\xi}$ で運んだ 2 つの行列が可換であること
（$UT=TU$ を評価で運ぶ）を示し、そのうえで転送行列がシフト行列の各固有空間を
それ自身へ写すことを組み立てる（対角化）。
