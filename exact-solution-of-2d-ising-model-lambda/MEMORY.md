...
1. **レビュー**: 自己双対条件と二次方程式の同値の Lean 三本を本文と突き合わせ、具体版の二方向が
   各段に一対一で対応することと、必要十分版が同じ鎖を既製定理へ丸投げせず組み立てていることを見る。
2. **次に進めるセクションは「自己双対方程式の二根と $x_c=\sqrt2-1$ の特定」**。
   状態台帳の先頭行だけを進める。正の根の特定では実代数的数の可算な順序を明示する。
3. **並列の作業ストリーム（式変形の書き方の統一）を毎 tick 1 件進める**
   （次の対象は状態台帳と保管庫の最新記録を見る）。
