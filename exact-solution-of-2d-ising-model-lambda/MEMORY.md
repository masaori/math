...
| Lean 具体版 | 上記の定義と主張と定理に対応する形式化（2026-08-11 の tick 90 で `claim_qbar_smul_sum`（スカラー倍が列ベクトルの有限和を保つこと）の Lean を置いた。未着手の主張は無い）。`lake build` と `check-no-sorry.sh`（定理 534 件を登録）が通る |
