...
## 現在の到達点（2026-09-03 の 121 回目の tick 時点）

**台を固定した集約でも三分類の部分和は消えないこと、一辺二では台の全分解がファイバーとして実現されることを全数計算で確定した。** 選択和側の偶部分グラフ対 $(P,Q)$ は $P\cap Q=D$、$P\triangle Q=E$ で分類され、和集合 $P\cup Q=D\cup E$ はファイバーを跨いで共有される。そこで全ファイバーを台 $F=D\cup E$ で束ね（台 $104$ 個×四スピン構造 $416$ 組）、三分類の部分和を集約したが、$\Sigma S_X\ne0$ が $132$ 組、$\Sigma S_Y\ne0$ が $248$ 組、$\Sigma S_Z\ne\Sigma U$ が $380$ 組残った（$\Sigma S_Y\ne0$ の最初の例は台 $F=\{v(0,1),v(1,1)\}$、$(a,b)=(0,0)$ で $\Sigma S_Y=1$）。一方、非後退置換が一つも無い分解 $F=D\sqcup E$（$E$ 偶）は零件で、空のファイバーの場合分けは一辺二では現れない。従って分類（接触なし・切り替え可能・全対切り替え不能）を保つ符号反転対合は、相殺の単位を固定ファイバーから台へ粗くしても存在しない。観察を `note_kac_ward_claim_fully_unswitchable_contacts_witness_doubled_edges_same_support_aggregation` と SageMath `same-support-fiber-phase-sums` に固定した。

次 tick は、分類を保つ対合の探索を打ち切り、三分類を混ぜてファイバー位相和の全体と選択和を直接結ぶ数え上げ（接触対の個数に関する帰納や母関数的関係）を構成する。

前進前レビューでは、前 tick の反転対の直接除去の反例（ノート・overview・検算・台帳・MEMORY）を照合した。検査件数 $2{,}912$・非後退 $32$・$D$ 一辺減 $0$ は検算の assert と一致し、対象ラベルは実在し、ノートであって主張ブロックではなく、「何も言っていない主張」や記号・住処・参照・検算の不一致は無かったため、レビュー修正は無かった。

並列の式変形統一は、姉妹側の「$\varepsilon$ 射影子の性質（冪等・直交・和が単位行列・像が固有空間）」の証明で、五つの鎖の直後に置かれた参照段落を削り、参照を実際に使う各式変形行の行末の `\blkref` へ移した。内容・式変形・根拠・参照は不変である。姉妹側 check・PDF 360 ページも通した。

Lambda 側 check（940 ブロック・相互参照 $3{,}955$ 件すべて解決）・linkage（421 検算）・対象 SageMath（台 $104$ 個×四スピン構造 $416$ 組、実行 5 分 24 秒）を通した。新しい本文主張は置かず観察ノートを追加したため、Lean の追加対象はない。
