...
**2026-08-11 の tick 109 は、レビューで直すところが無く、根を持つ多項式が一次式を
因子に持つことを四層すべてで完了させた。**

1. **レビュー**: 因数定理の本文 1 ブロック、SageMath `qbar-factor-theorem`、Lean の具体版・
   必要十分版・導出を突き合わせる。
2. **次に進めるセクションは「零でない多項式の根は次数を超えないこと」**。
   状態台帳のセクション表が正本であり、論法は次数についての帰納法 1 本である。
