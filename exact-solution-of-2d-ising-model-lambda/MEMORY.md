...
移行中に踏んだ落とし穴を 2 つ記録する。次に同じ移行をするとき同じ所で止まる。

1. **本文全体を 1 つのリテラルとして型検査させると、TypeScript の予算を超える。** 章 11 個を
   個別に検査すると全部通るのに、まとめると通らず、しかも「グループの前置き要素の 2 番目が
   1 番目の型に一致しない」という成り立ちようのない不一致を報告する。章ごとに `defineSection` で
   型を確定させ、束ねるところでは再検査しない（`compileDocumentStructure` も content 側では呼ばない）。
2. **木を書き出す道具は、テンプレートリテラルの中の行に触れてはならない。** 字下げを揃えるつもりで
   行頭 2 文字を削り、数式の `\begin{cases}` を `egin{cases}` に壊した。生成後は `String.raw` の
   中身を移行前と突き合わせて確認する。
