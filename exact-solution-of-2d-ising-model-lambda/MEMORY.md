...
2026-08-18 の tick 431 は、前進前レビューで tick 430 の原因記述を一次情報の範囲へ限定した。段階別ログが確定するのは `candidate_distance.minpoly()` が 480 秒内に完了しなかったことまでで、「PARI の数体統合が原因」という機構の断定は出ないため、観測上の律速を `AA` の最小多項式計算とする表現へ観察ノート・overview・台帳・MEMORY で統一した。「何も言っていない主張」は新たに見つからなかった。
