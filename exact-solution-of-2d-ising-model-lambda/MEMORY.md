...
| 層 | 状態 |
| --- | --- |
| 記述（構造化テキスト） | ラベルの数は合計 214 件（定義・主張・定理・注意）。`npm run check` と `npm run build:pdf` が全通過 |
...
| Lean 具体版 | 上記の定義と主張と定理に対応する形式化（2026-08-11 の tick 85 で `claim_shift_matrix_eigenvalue_root_of_unity`（シフト行列の固有値が 1 の $L$ 乗根であること）の Lean を置いた。未着手の主張は無い）。`lake build` と `check-no-sorry.sh`（定理 520 件を登録）が通る |
...
次は、シフト行列の固有空間へ転送行列を分けること（対角化）である。
