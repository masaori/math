...
## 全六研究監督: 離散Whitney形式化の次対象を維持（2026-09-22 21:02 JST）

実行識別子 `7058aadb-fd8e-41f1-adef-9056db75e90b`。前回読解終点以後にremote defaultへ統合された数学本文・証明・候補評価の変更はない。LLMによる検証では、固定原始特徴多項式探索の運用停止と進行可能な離散Whitney形式化を分け、次のLean配線を三周期比較の整数補題に維持した。21:02:47のプログラミングによる観測ではlaunchd登録あり・非実行中・last exit 0だが、直近20:35:26は共有作業ツリーの未コミット変更による見送りだった。


## 全六研究監督: 三周期比較の整数補題を次対象として維持（2026-09-22 15:01 JST）

実行識別子 `4407661c-15c0-4656-9b0f-904ca4b494df`。LLMによる検証では、前回読解終点以後に
remote defaultへ統合された数学本文・証明・段取りの前進はない。一側閉包の射影回転数から
三周期比較の整数補題をLean二版へ接続する順序を維持し、固定原始特徴探索だけの運用停止と区別した。
15:01:25のプログラミングによる観測ではlaunchd登録あり・非実行中・last exit 0だが、直近14:35:25は
作業ツリーの未コミット変更による見送りであり、研究実行の成功とは扱わない。

## 全六研究監督: 三周期比較の整数補題を維持（2026-09-22 11:13 JST）

実行識別子 `7005d8ee-2e7f-4384-b77c-d0e247cc692d`。LLMによる検証では前回監督以後の研究上の前進はなく、一側閉包の射影回転数の次にある三周期比較の整数補題をLean二版へ接続する対象を維持した。固定原始特徴探索だけに対する運用停止と、進行対象である離散Whitney形式化を区別し、段取りを変更していない。11:13:22のプログラミングによる観測ではlaunchd登録あり・非実行中・last exit 0だが、直近実ログは11:05:30に作業ツリーの未コミット変更を理由として見送られており、正常終了とは読み替えない。

## 全六研究監督: 三周期比較の整数補題を維持（2026-09-22 03:04 JST）

実行識別子 `abe2fce3-53cb-4316-b044-419b2750581f`。LLMによる検証では前回監督以後の研究上の前進はなく、一側閉包の射影回転数の次にある三周期比較の整数補題をLean二版へ接続する対象を維持した。固定原始特徴探索の運用停止と段取りを変更していない。03:03:56のプログラミングによる観測ではlaunchd登録あり・非実行中・last exit 0だが、直近実ログは02:35:34に作業ツリーの未コミット変更を理由として見送られており、正常終了とは読み替えない。

## 全六研究監督: 三周期比較の整数補題へ進む（2026-09-21 21:00 JST）

実行識別子 `35cea172-3027-43d1-ac6b-5e842132f49f`。LLMによる検証では、内外連結性の局所仮定緩和、境界頂点の対角接触排除、平面閉多角形と一側閉包射影の循環総回転数をLean二版へ接続したことを研究上の前進と判定した。次は三周期比較の整数補題であり、固定原始特徴探索の運用停止は維持する。段取り変更はない。20:56:25のプログラミングによる観測ではlaunchd登録あり・非実行中・last exit 0だが、直近実ログは20:35:31に作業ツリーの未コミット変更を理由として見送られており、正常終了とは読み替えない。

## 一側閉包の射影回転数を Lean 二版へ配線（2026-09-21 の 370 回目）

一側閉包の頂点単純性と、射影辺列の平面持ち上げが閉包の一定平行移動であることから、持ち上げ点の相異性を導いて離散 Whitney の正負四の結論へ渡す具体版を追加した。必要十分版は単射な点列・一定平行移動・単射性を受ける結論だけへ抽出し、導出版を具体版へ揃えた。SageMath は非零巻き付きの頂点単純閉路 3,464 本・一側閉包 45,216 本・射影後の循環総回転数 45,216 件で再実行して通過した。次は「三周期比較の整数補題」を Lean 二版へ配線する。

## 全スピン反転をレビュー（2026-09-21 の 374 回目）

本文・SageMath・Lean 定義・後続参照を LLM により照合した。有限配位上で各スピンへ整数の加法逆元を取る写像を定め、破れた辺の保存と後続の原像数え上げから反復参照されるため、空の四則主張ではない。記号・住処・値域に不一致はなく、SageMath の一辺一から四の 2,106,500 本もプログラミングにより再実行して通過した。次のレビューと表記確認は「全スピン反転は各辺の破れを保つ」から続ける。

## 平面閉多角形の回転数定理の仮定緩和を Lean 二版へ配線（2026-09-21 の 369 回目）

「持ち上げ点が相異なる零巻き付き閉路の循環総回転数は正負 4 のいずれかである」を Lean へ配線した。具体版は Euler 等式、頂点数と辺数の分割、頂点・辺の接続数の二重計数を合成し、凸頂点数と凹頂点数の差を四としたうえで循環向きの二場合を処理した。必要十分版は五等式・Euler 等式・符号の二場合だけへ抽出し、導出版を具体版へ揃えた。SageMath は長さ十以下の持ち上げ点相異の閉歩道 704 本を再実行して通過した。次は「一側閉包の循環総回転数は正負 4」の Lean 二版へ進む。

## 有限格子の Fisher 零点の代数性をレビュー（2026-09-21 の 373 回目）

LLM による検証で「有限格子の Fisher 零点は代数的である」の本文・SageMath・Lean 二版・導出版を照合した。係数和から $Z_L(1)=2^{L^2}\ne0$ を導き、零でない整係数多項式の根の証人を与える実質的な主張なので、空の四則主張ではない。記号・住処・式変形に不一致はなく、本文の書き換えも不要だった。次のレビューと表記確認は「全スピン反転」から続ける。

## 境界頂点の対角接触排除の仮定緩和を Lean 二版へ配線（2026-09-21 の 368 回目）

「持ち上げ点が相異なる閉路で頂点まわりの内側セルが対角の二セルだけになることは無い」の具体版から、格子点ごとの訪問回数高々一という局所仮定を除いた。任意の格子点について一周期中の訪問点の存在で場合分けし、訪問点なら持ち上げ点相異性を前段の四セル弧へ渡し、非訪問点なら有限和の各項を零として訪問回数を零へ落とした。必要十分版は四つの零一値が巡回弧を作るか全て等しいという有限命題のまま保ち、導出版を新しい具体版へ揃えた。次は「平面閉多角形の回転数定理の仮定の緩和」の Lean 二版へ進む。

## 有限格子の Fisher 零点をレビュー（2026-09-21 の 372 回目）

LLM による検証で「有限格子の Fisher 零点」の本文・SageMath・Lean 定義・後続参照を照合した。分配多項式を代数的数で評価した根の集合を定め、代数性・双対性・零点の有限性・詰め寄りから反復参照されるため、空の四則主張ではない。記号・住処・評価写像との対応に不一致はなく、定義ブロックなので式変形の書き換えも不要だった。次のレビューと表記確認は「有限格子の Fisher 零点は代数的である」から続ける。

## 外側帯と補集合連結性の仮定緩和を Lean 二版へ配線（2026-09-21 の 367 回目）

「持ち上げ点が相異なる閉路の歩道沿いの外側帯は空でなく辺連結である」と「内側セル集合の補集合は辺連結である」の具体版を、一周期の持ち上げ点の相異性へ接続した。前段の四セル弧を各訪問頂点で呼び、内側と外側がともに現れる有限命題から局所外側セル集合の非空性を導くことで、従来の局所非空性の仮定を両定理から除いた。局所連結性・隣接弧の橋・外接長方形外との重なり・任意の外側セルから二つの核への到達は既存の必要十分版へ渡し、導出版も新しい具体版へ揃えた。次は「境界頂点の対角接触排除の仮定の緩和」の Lean 二版へ進む。

## 整係数多項式の代数的数における値をレビュー（2026-09-21 の 371 回目）

LLM による検証で「整係数多項式の代数的数における値」の本文・SageMath・Lean 定義・後続参照を照合した。整数係数を代数的数へ埋め込んだ有限和による評価写像を定め、Fisher 零点・双対性・持ち上げ評価から反復参照されるため、空の四則主張ではない。記号・住処・有限和による定義に不一致はなく、定義ブロックなので式変形の書き換えも不要だった。次のレビューと表記確認は「有限格子の Fisher 零点」から続ける。

## 内側帯と内側セル連結性の仮定緩和を Lean 二版へ配線（2026-09-21 の 366 回目）

「持ち上げ点が相異なる閉路の歩道沿いの内側帯は空でなく辺連結である」と「内側セル集合は辺連結である」の具体版を、一周期の持ち上げ点の相異性へ接続した。前段の四セル弧を各訪問頂点で呼び、内側と外側がともに現れる有限命題から局所内側セル集合の非空性を導くことで、従来の局所非空性の仮定を両定理から除いた。局所連結性・隣接弧の橋・内側セルから帯への到達は既存の必要十分版へ渡し、導出版も新しい具体版へ揃えた。次は「外側帯と補集合連結性の仮定の緩和」の Lean 二版へ進む。

## 固有空間分解をレビュー（2026-09-21 の 370 回目）

LLM による検証で「シフト行列の固有空間たちは列ベクトルの全体を張る」の本文・SageMath・Lean 具体版・必要十分版・導出版を照合した。射影像の固有空間所属、スカラー倍閉性、有限和による復元を合成する実質的な主張であり、空の四則主張ではない。式変形行ですでに引いた復元定理を証明末尾に名前だけ再掲していたため、禁止されている参照一覧として削除した。次のレビューと表記確認は「整係数多項式の代数的数における値」から続ける。

## 全六研究監督: 内側帯と内側セル連結性の仮定緩和へ進む（2026-09-21 09:00 JST）

実行識別子 `855f8450-f011-4aad-9d8a-210ccabf73ff`。LLMによる検証では、離れた反復横断階段の分離から一側閉包、トーラス射影、射影持上げ、同奇偶セル弧の仮定緩和までを研究上の前進と判定した。次のLean配線は内側帯と内側セル連結性の仮定緩和である。固定原始特徴探索の運用停止は維持する。09:00:25のプログラミングによる観測ではlaunchd登録あり・非実行中で、08:50:37正常終了だった。

## 頂点まわりの同奇偶セルの弧の仮定緩和を Lean 二版へ配線（2026-09-21 の 365 回目）

「持ち上げ点が相異なる閉路の各訪問頂点で内側セルと外側セルはそれぞれ一つの弧をなす」の Lean 具体版を、訪問回数一の局所仮定から一周期の持ち上げ点の相異性へ接続した。具体版は相異性から指定した持ち上げ点の訪問回数が一であることを有限和で導き、既存の四辺通過数と四セルの奇偶弧の計算へ渡す。必要十分版は四つの境界辺で内外が二回だけ切り替わる有限命題のまま保ち、導出版を新しい具体版へ揃えた。SageMath は持ち上げ点相異の閉じた非後退歩道 704 本・訪問頂点 6,672 個で再実行して通過した。次は「内側帯と内側セル連結性の仮定の緩和」の Lean 二版へ進む。

## 固有空間のスカラー倍閉性をレビュー（2026-09-21 の 369 回目）

LLM による検証で「固有空間はスカラー倍で閉じる」の本文・SageMath・Lean 具体版・必要十分版・導出版を照合した。作用の斉次性とスカラー積の結合則・可換性を合成する九段の鎖であり、固有空間分解から反復参照されるため空の四則主張ではない。記号・住処・零スカラー倍を許す理由に不一致はなく、式変形も一続き・一行一操作・行末の根拠へ統一済みだった。次のレビューと表記確認は「固有空間分解」から続ける。

## 射影の平面持ち上げの一致と巻き付き零を Lean 二版へ配線（2026-09-21 の 364 回目）

「射影の平面持ち上げは元の平面閉路の平行移動で巻き付きは零である」を Lean へ配線した。具体版は二点列の一歩差の一致を帰納法で足し上げ、閉路の終点一致と巻き付き対の終点差公式から二成分を零へ戻した。必要十分版は加法可換群・一歩差・閉性・巻き付き対の零反映性だけへ抽出し、導出版で整数格子へ戻した。SageMath は頂点単純な閉単位格子路 144 本・射影の歩 3,216 件・持ち上げ点 3,648 件で再実行して通過した。次は「頂点まわりの同奇偶セルの弧の仮定の緩和」の Lean 二版へ進む。

## 固有空間の和閉性をレビュー（2026-09-21 の 368 回目）

LLM による検証で「固有空間は和で閉じる」の本文・SageMath・Lean 具体版・必要十分版・導出版を照合した。作用の加法性と分配則を固有空間の加法閉性へ合成する実質的な主張であり、固有空間分解の土台なので空の四則主張ではない。七段の等式鎖・記号・住処に不一致はなく、式変形も一続き・一行一操作・行末の根拠へ統一済みだった。次のレビューと表記確認は「固有空間はスカラー倍で閉じる」から続ける。

## 平面単純閉路のトーラス射影を Lean 二版へ配線（2026-09-21 の 363 回目）

「平面単純閉路のトーラス射影」を Lean へ配線した。具体版は平面格子点列と実際のトーラス頂点・向き付き辺・反転写像について、四方向の場合分けが与える端点整合、反転から二歩戻ること、頂点単純性を合成して閉性と非後退性を固定した。必要十分版は点・頂点・辺の型に構造を要求せず、端点整合・反転復元・点列の単射性・二歩先の添字が異なることだけへ抽出し、導出版で整数格子とトーラス辺へ戻した。SageMath は頂点単純な閉単位格子路 144 本・射影の歩 3,216 件・持ち上げ点 3,648 件を再実行して通過した。次は「射影の平面持ち上げの一致と巻き付き零」の Lean 二版へ進む。

## 代数的数を成分とする行列の固有空間をレビュー（2026-09-21 の 367 回目）

LLM による検証で「代数的数を成分とする行列の固有空間」の本文・SageMath・Lean 定義・後続参照を照合した。零ベクトルを含む作用等式の解集合を定め、和・スカラー倍閉性と固有空間分解から反復参照されるため、空の四則主張ではない。記号・住処・零ベクトルを除外しない理由に不一致はなく、定義ブロックなので式変形の書き換えも不要だった。次のレビューと表記確認は「固有空間は和で閉じる」から続ける。

## 一側閉包の頂点単純性を Lean 二版へ配線（2026-09-21 の 362 回目）

「一側閉包の頂点単純性」を Lean へ配線した。具体版は周期持ち上げ・上り階段・反転平行階段・下り階段を共有端点が重複しない半開区間で読み、各部分の単射性と六つの部分間非交差から終点を除く全頂点の相異なりを固定した。必要十分版は点の型に構造を要求せず、四つの点族の局所的な単射性と相互非交差だけへ抽出し、導出版で整数格子へ戻した。SageMath は非零巻き付きの頂点単純閉路 3,464 本・一側閉包 45,216 本・頂点相異性 1,210,464 件を再実行して通過した。次は「平面単純閉路のトーラス射影」の Lean 二版へ進む。

## 代数的数を成分とする行列の固有値をレビュー（2026-09-21 の 366 回目）

LLM による検証で「代数的数を成分とする行列の固有値」の本文・SageMath・Lean 定義・後続参照を照合した。非零固有ベクトルが存在するという行列に対する実質的な存在条件であり、シフト行列の固有値と固有空間分解から反復参照されるため、空の四則主張ではない。記号・住処・存在量化に不一致はなく、定義ブロックなので式変形の書き換えも不要だった。次のレビューと表記確認は「代数的数を成分とする行列の固有空間」から続ける。

## 一側閉包の定義と閉性を Lean 二版へ配線（2026-09-21 の 361 回目）

「一側閉包の定義と閉性」を Lean へ配線した。具体版は周期持ち上げ・帯外へ上がる横断階段・逆向きの平行階段列・始点へ降りる横断階段の四部分を連結し、境界値の一致と各部分の単位歩から閉じた単位格子路を固定した。必要十分版は四つの有限路・三つの境界接続・終点と始点の一致・各部分の許容された一歩だけへ抽出し、導出版で整数格子へ戻した。SageMath は非零巻き付きの頂点単純閉路 3,464 本・一側閉包 45,216 本・単位歩 1,210,464 件を再実行して通過した。次は「一側閉包の頂点単純性」の Lean 二版へ進む。

## 代数的数を成分とする行列の固有ベクトルをレビュー（2026-09-21 の 365 回目）

LLM による検証で「代数的数を成分とする行列の固有ベクトル」の本文・SageMath・Lean 定義・後続参照を照合した。作用とスカラー倍の等式に零ベクトルでない条件を加えて固有値の住処を確定し、固有値・固有空間・行列冪の作用から反復参照される実質的な定義であるため、空の四則主張ではない。記号・住処・非零条件に不一致はなく、定義ブロックなので式変形の書き換えも不要だった。次のレビューと表記確認は「代数的数を成分とする行列の固有値」から続ける。

## 平行階段の横断座標の幅と帯外条件を Lean 二版へ配線（2026-09-21 の 360 回目）

「平行階段の横断座標の幅と帯外に留まる条件」を Lean へ配線した。具体版は積の符号による場合分けで横断座標差を $-L|w_{\mathrm h}w_{\mathrm v}|$ 以上 $0$ 以下に評価し、帯外条件から周期持ち上げとの非交差を導いた。必要十分版は加法的な整数値座標・有限路の上下界・比較点族の上界だけへ抽出し、導出版で整数格子へ戻した。SageMath は非零巻き付きの頂点単純閉路 3,464 本・平行階段頂点 18,336 個・帯比較 133,560 件を再実行して通過した。次は「一側閉包の定義と閉性」の Lean 二版へ進む。

## 零ベクトルをレビュー（2026-09-21 の 364 回目）

LLM による検証で「零ベクトル」の本文・SageMath・Lean 定義・後続参照を照合した。有限添字集合上の列ベクトルの加法単位元を成分ごとに固定し、固有ベクトル・固有空間・零スカラー倍から反復参照されるため、空の四則主張ではない。記号・住処・可算性に不一致はなく、定義ブロックなので式変形の書き換えも不要だった。次のレビューと表記確認は「代数的数を成分とする行列の固有ベクトル」から続ける。

## 巻き付きベクトルの平行階段を Lean 二版へ配線（2026-09-21 の 359 回目）

「巻き付きベクトルの平行階段」を Lean へ配線した。具体版は積の符号で縦横の順序を選ぶ有限階段について、各差が四つの単位格子ベクトルの一つで平行座標を真に増やし、全頂点が相異なることを固定した。必要十分版は二つの歩の順序選択・有限歩数・各歩を正へ送る加法準同型だけへ抽出し、導出版で整数格子へ戻した。SageMath は非零巻き付きの頂点単純閉路 3,464 本・平行階段 14,872 歩を再実行して通過した。次は「平行階段の横断座標の幅と帯外に留まる条件」の Lean 二版へ進む。

## 代数的数を成分とする列ベクトルのスカラー倍をレビュー（2026-09-21 の 363 回目）

LLM による検証で「代数的数を成分とする列ベクトルのスカラー倍」の本文・SageMath・Lean 定義・後続参照を照合した。代数的数の積と列ベクトルの演算を区別して成分ごとに定め、作用の斉次性と固有空間のスカラー倍閉性から反復参照されるため、空の四則主張ではない。記号・住処・可算性に不一致はなく、定義ブロックなので式変形の書き換えも不要だった。次のレビューと表記確認は「零ベクトル」から続ける。

## 閉包の周期数の一般化を Lean 二版へ配線（2026-09-21 の 358 回目）

「閉包の周期数の一般化」を Lean へ配線した。具体版は整数格子の非零な一周期並進の正の自然数倍が非零であることと、四部分の長さが $N=2h+2cm$ になることを固定した。必要十分版は加法的ねじれのない可換群における正の自然数倍と自然数の長さ等式だけへ抽出し、導出版で整数格子へ戻した。SageMath は周期数一・二・三について閉包 226,080 本と三周期比較 75,360 件を再実行して通過した。二側閉包の周期差で順向きと逆向きの回転寄与が相殺する反証は既に固定済みなので、次は「巻き付きベクトルの平行階段」の Lean 二版へ進む。

## 代数的数を成分とする列ベクトルの和をレビュー（2026-09-21 の 362 回目）

LLM による検証で「代数的数を成分とする列ベクトルの和」の本文・SageMath・Lean 定義・後続参照を照合した。代数的数を成分とする列ベクトルどうしの演算を成分ごとに定め、代数的数の加法と記号を分けて、後続の作用の加法性と固有空間の加法閉性から反復参照されるため、空の四則主張ではない。記号・住処・可算性に不一致はなく、定義ブロックなので式変形の書き換えも不要だった。次のレビューと表記確認は「代数的数を成分とする列ベクトルのスカラー倍」から続ける。

## 接続階段の周期並進による単純閉路を Lean 二版へ配線（2026-09-21 の 357 回目）

「接続階段とその周期並進は二つの持ち上げを単純閉路へ閉じる」を Lean へ配線した。具体版は共有端点を一方にだけ残した四部分を合成し、各部分の単射性と六組の像の分離から終点以外の頂点の相異なりを導いた。必要十分版は四つの点族・各単射性・六つの相互分離・閉性・許容された一歩だけへ抽出し、導出版で整数格子へ戻した。SageMath は非零巻き付きの頂点単純閉路 3,464 本・閉包 226,080 本・閉包頂点 7,752,240 個で再実行して通過した。次は「閉包の周期数の一般化」の Lean 二版へ進む。

## 代数的数を成分とする行列と作用をレビュー（2026-09-21 の 361 回目）

LLM による検証で「代数的数を成分とする行列と作用」の本文・SageMath・Lean 具体版・必要十分版・導出版・後続参照を照合した。有限添字集合上の代数的数係数行列・列ベクトル・作用を定め、行列積と作用の両立を8段で示して後続から反復参照されるため、空の四則主張ではない。記号・住処・有限和の順序に不一致はなく、修正点は無かった。式変形は一続き・一行一操作・行末の根拠へ統一済みだった。次のレビューと表記確認は「代数的数を成分とする列ベクトルの和」から続ける。

## 最初に当たる接続階段を Lean 二版へ配線（2026-09-21 の 356 回目）

「接続階段は元の持ち上げと始点でのみ、移動後の持ち上げと終点でのみ交わる」を Lean へ配線した。具体版は反復横断階段の単射性、元の持ち上げを正の歩数で避ける既証明、最初の当たり歩数の最小性、移動後の持ち上げの横断座標下界を合成した。必要十分版は単射な点列・上下端を持つ二点族・一定の座標移動・正の最初の当たりだけへ抽出し、導出版で具体版へ戻した。SageMath は非零巻き付きの頂点単純閉路 3,464 本・上端基点 37,680 個・接続階段頂点 236,600 個・持ち上げ所属 510,640 件で再実行して通過した。次は「帯外で接続路と二つの周期持ち上げを合成する単純閉路」の Lean 二版へ進む。

## 代数的数と 1 の冪根をレビュー（2026-09-21 の 360 回目）

LLM による検証で「代数的数と 1 の冪根」の本文・SageMath・Lean 定義・具体版・必要十分版・導出版・後続参照を照合した。代数閉包と冪根集合の可算な住処を定め、約数に沿う包含を四段の冪計算で示して後続の固有値論から参照されるため、空の四則主張ではない。記号・住処・指数法則の向きに不一致はなく、修正点は無かった。式変形も一続き・一行一操作へ統一済みだった。次のレビューと表記確認は「代数的数を成分とする行列と作用」から続ける。

## 平行座標が離れた二つの反復横断階段の分離を Lean 二版へ配線（2026-09-21 の 355 回目）

「基点の平行座標が幅を超えて離れた二つの反復横断階段は交わらない」を Lean へ配線した。具体版は各階段点の平行座標の基点からのずれを同じ区間へ評価し、二基点の座標差が区間幅を超えることと合成した。必要十分版は二つの点族・整数値座標・共通の上下界・離れた基点だけへ抽出し、導出版で具体版へ戻した。SageMath は非零巻き付き対 280 組・分離した基点対 1,120 組・二階段の頂点比較 315,952 件で再実行して通過した。次は「反対側の持ち上げも終点以外で避ける接続路」の Lean 二版へ進む。

## 行列式と特性多項式をレビュー（2026-09-21 の 359 回目）

LLM による検証で「行列式と特性多項式」の本文・SageMath・Lean 定義・後続参照を照合した。整数から整係数多項式、さらに二変数多項式への埋め込みを明示して有限行列の行列式・特性行列・特性多項式を定め、固有値と軌道分解から反復参照される実質的な定義群であり、空の四則主張ではない。記号・住処・置換の向き・二つの不定元の区別に不一致はなく、修正点は無かった。式変形を含まない定義群なので書き換えは不要だった。次のレビューと表記確認は「代数的数と 1 の冪根」から続ける。

## 全六研究監督: 離れた反復横断階段の分離へ進む（2026-09-21 03:17 JST）

実行識別子 `6ecff40d-b8ca-4256-a1b1-4ed48c3e89f6`。LLMによる検証では、セルEuler関係、対角接触、
ゲート横断、周期持上げ、横断階段、非零周期並進との分離、反復階段の平行座標幅までを本文・SageMath・
Lean二版へ接続したことを研究上の前進と判定した。次は「平行座標が離れた二つの反復横断階段の分離」の
Lean二版であり、段取り変更はない。
固定原始特徴の多項式探索に対する運用停止は別系列として維持する。02:57:37のプログラミングによる
観測ではlaunchd登録あり・非実行中で、02:35:31開始の実ログは02:55:26に正常終了した。

## 反復横断階段の平行座標幅を Lean 二版へ配線（2026-09-21 の 354 回目）

「反復横断階段の平行座標は基点から幅以内に収まる」を Lean へ配線した。具体版は周期方向の平行座標が零であることと、横断階段の二区間が端点 0 と $w_{\mathrm h}w_{\mathrm v}$ の間に収まることを合成した。必要十分版は反復有限経路、周期方向を零へ送る整数値加法準同型、一周期内の上下界だけへ抽出し、導出版で具体版へ戻した。SageMath は非零巻き付き対 280 組・平行座標のずれ 16,800 件で再実行して通過した。次は「平行座標が離れた二つの反復横断階段の分離」の Lean 二版へ進む。

## 置換・転倒数・符号をレビュー（2026-09-21 の 358 回目）

LLM による検証で「置換・転倒数・符号」の本文・SageMath・Lean 具体版・必要十分版・導出版・後続参照を照合した。有限線型順序集合上の転倒数から符号を定め、合成についての乗法性を導く実質的な定義と主張であり、行列式と後続の置換展開から反復参照されるため空の四則主張ではない。記号・住処・合成の向き・転倒対の数え上げに不一致はなく、式変形も既定の形へ統一済みだった。次のレビューと表記確認は「行列式と特性多項式」から続ける。

## 反復横断階段と非零周期並進の分離を Lean 二版へ配線（2026-09-21 の 353 回目）

「反復横断階段は非零の周期並進と交わらない」を Lean へ配線した。具体版は反復階段の平行座標幅と、$L(w_{\mathrm v}^2+w_{\mathrm h}^2)>|w_{\mathrm h}w_{\mathrm v}|$ である周期並進増分を合成した。必要十分版は座標幅を持つ点族と整数並進の一定増分だけへ抽出し、導出版で具体版へ戻した。SageMath は非零巻き付き対 280 組、階段頂点 16,800 個、非零周期並進との比較 908,544 件で再実行して通過した。次は「反復横断階段の平行座標の幅」の Lean 二版へ進む。

## 有限集合を添字とする行列をレビュー（2026-09-21 の 357 回目）

LLM による検証で「有限集合を添字とする行列」の本文・SageMath 実装・Lean 定義・後続参照を照合した。有限添字集合上の整数係数多項式行列と積・正の冪・トレースを定め、後続から反復参照される実質的な定義群であり、空の四則主張ではない。記号・住処・冪の始点・有限和に不一致はなく、証明の式変形を含まないため書き換えなかった。次のレビューと表記確認は「置換・転倒数・符号」から続ける。

## 整数帯の上端からの反復横断階段の分離を Lean 二版へ配線（2026-09-21 の 352 回目）

「整数帯の上端以上の基点からの反復横断階段は基点以外で周期持ち上げと交わらない」を Lean へ配線した。具体版は反復横断階段の歩数下界と周期持ち上げの横断座標上界を整数順序で合成した。必要十分版は整数値座標を持つ二つの点族・基点・上下界だけへ抽出し、導出版で具体版へ戻した。SageMath は非零巻き付きの頂点単純閉路 3,464 本、帯を超えた階段頂点 155,040 個、周期持ち上げとの比較 5,727,600 件で再実行して通過した。次は「反復横断階段とその非零周期並進の分離」の Lean 二版へ進む。

## 正の有理数の対数の順序同値をレビュー（2026-09-21 の 356 回目）

LLM による検証で「正の有理数の対数は順序を保ちかつ反映する」の本文・SageMath・Lean 具体版・必要十分版・導出版・後続参照を照合した。正の有理数と対数順序群の順序を双方向に移送し、後続の不等式で反復参照される実質的な主定理であり、空の四則主張ではない。記号・住処・同値の向きに不一致はなく、同値の鎖も一続き・一行一操作・行末の根拠へ統一済みなので修正しなかった。次のレビューと表記確認は「有限集合を添字とする行列」から続ける。

## 横断平行移動した周期持ち上げの分離を Lean 二版へ配線（2026-09-21 の 351 回目）

「横断幅を超えて離した周期持ち上げは交わらない」を Lean へ配線した。具体版は横断移動後の座標を整数の分配則で計算し、周期持ち上げの横断座標の上下界と幅の不等式から二つの像を分離した。必要十分版は加法群の点族、整数移動方向、整数値加法準同型、元の点族の上下界だけへ抽出し、具体版をその特殊化として接続した。SageMath は非零巻き付きの頂点単純閉路 3,464 本、座標等式 520,632 件、分離した点対 21,566,088 組で再実行して通過した。次は「整数帯の上端からの反復横断階段と周期持ち上げの分離」の Lean 二版へ進む。

## 対数順序群の線形順序性をレビュー（2026-09-21 の 355 回目）

LLM による検証で「対数順序群の順序は線形順序である」の本文・SageMath・Lean 具体版・必要十分版・導出版・後続参照を照合した。有理数順序の引き戻しによる反射律・推移律・全順序性と、逆写像で元へ戻す反対称律を述べ、後続から反復参照される実質的な主張であり、空の四則主張ではない。記号・住処・四つの順序律に不一致はなく、反対称律の等号列も一続き・一行一操作・行末の根拠へ統一済みなので修正しなかった。次のレビューと表記確認は「対数順序群の順序と有理数の順序は同値である」から続ける。

## 反復横断階段を Lean 二版へ配線（2026-09-21 の 350 回目）

「横断階段を反復して整数帯の外へ運ぶ単純路」を Lean へ配線した。具体版は整数の商と余りで横断階段を終点方向へ反復し、剰余が末尾か否かの二場合で各差を元の階段の一歩へ戻した。その差が単位格子ベクトルで横断座標を 1 以上増やすこと、歩数による下界、全頂点の相異性を人手証明どおり合成した。必要十分版は正の長さを持つ有限階段、終点を周期とする反復、各歩を 1 以上へ送る整数値加法準同型だけへ抽出し、導出版で具体版へ戻した。SageMath は非零巻き付きの頂点単純閉路 3,464 本・反復階段 30,048 歩で再実行して通過した。次は「横断平行移動した周期持ち上げ同士の分離」の Lean 二版へ進む。

## 対数順序群の順序をレビュー（2026-09-21 の 354 回目）

LLM による検証で本文・Lean 定義・線形順序性・加法単調性・後続参照を照合した。正の有理数への写像で有理数の順序を引き戻して決定可能な関係を定め、後続から反復参照される実質的な定義であり、空の四則主張ではない。記号・住処・決定可能性に不一致はなく、式変形を含まない定義なので書き換えは不要だった。次のレビューと表記確認は「対数順序群の順序は線形順序である」から続ける。

## 巻き付きベクトルの横断階段を Lean 二版へ配線（2026-09-21 の 349 回目）

「巻き付きベクトルの横断階段」を Lean へ配線した。具体版は整数符号で定めた縦横二段階の階段について、各隣接差が四つの単位格子ベクトルの一つで整数横断座標を真に増やすことと、終点までの全頂点が相異なることを人手証明どおり形式化した。必要十分版は二つの歩、二つの歩数、各歩を正へ送る加法準同型だけへ抽出し、導出版で具体版へ戻した。次は「横断階段を反復して整数帯の外へ運ぶ単純路」の Lean 二版へ進む。

## 正の有理数の対数は全射であることをレビュー（2026-09-21 の 353 回目）

LLM による検証で本文・SageMath・Lean 具体版・必要十分版・導出版・後続参照を照合した。有限台指数ベクトルから正の有理数を有限積で構成し、対数の逆像を与える実質的な主張であり、空の四則主張ではない。記号・住処・正値性・全射性に不一致はなく、内部の式変形も一続き・一行一操作・行末の根拠へ統一済みなので修正しなかった。次のレビューと表記確認は「対数順序群の順序」から続ける。

## 巻き付きに平行な整数座標の一周期増加を Lean 二版へ配線（2026-09-21 の 348 回目）

「周期並進は平行座標を正の定数だけ増やす」を Lean へ配線した。具体版は平行座標を加法準同型として定義し、整数の商と余りによる周期延長を一周期ずらしたときの増分が $L(w_{\mathrm v}^2+w_{\mathrm h}^2)$ であり、非零巻き付きと $L\ge1$ の下でそれが正であることを人手証明どおり形式化した。必要十分版は基本点と周期方向を持つ任意の加法群と加法準同型へ抽出し、導出版で具体版へ戻した。次は「巻き付きベクトルの横断階段」の Lean 二版へ進む。

## 有限台指数ベクトルが定める正の有理数をレビュー（2026-09-21 の 352 回目）

LLM による検証で本文・Lean 定義・SageMath 検算・後続参照を照合した。有限台の整数指数から正の有理数を有限積で定め、対数の逆写像と可算側での閉性を後続が反復参照する実質的な定義であり、空の四則主張ではない。記号・住処・有限積の意味に不一致はなく、式変形を含まない定義なので書き換えは不要だった。次のレビューと表記確認は「正の有理数の対数は全射である」から続ける。

## 周期延長した持ち上げの有限幅の整数帯を Lean 二版へ配線（2026-09-20 の 347 回目）

「周期延長した持ち上げは有限幅の整数帯に入る」を Lean へ配線した。具体版は巻き付きに直交する整数座標を加法準同型として定義し、周期並進を零へ送る計算、一周期内の基本点との値の一致、値域の有限性を人手証明どおり合成した。必要十分版は加法準同型が周期方向を零へ送ることだけへ抽出し、導出版で具体版へ戻した。SageMath は非零巻き付きの頂点単純閉路 3,464 本・周期延長した持ち上げ点 123,960 個で再実行して通過した。次は「巻き付きベクトルに平行な整数座標と一周期ごとの増加」の Lean 二版へ進む。

## 正の有理数の対数をレビュー（2026-09-20 の 351 回目）

LLM による検証で本文・Lean 定義・後続参照を照合した。正の有理数の素因数指数差が有限台を持つことを示して Λ の元を定め、単射・全射・順序・有限自由エントロピーから反復参照される実質的な定義であり、空の四則主張ではない。記号・住処・有限台の閉性に不一致はなく、式変形を含まない定義なので書き換えは不要だった。次のレビューと表記確認は「有理数の対数から正の有理数を戻す写像」から続ける。

## 周期延長した平面持ち上げの単射性を Lean 二版へ配線（2026-09-20 の 346 回目）

「非零巻き付きの頂点単純閉路の周期延長した持ち上げ点は相異なる」を Lean へ配線した。具体版は整数の商と余りで周期延長を定義し、格子剰余による余りの一致、非零巻き付きによる商の一致、商余り表示による添字の一致を人手証明どおり合成した。必要十分版は周期方向を消す射影・余りの分離・周期方向の無ねじれ・商余り表示の単射性だけへ抽出し、導出版で具体版へ戻した。SageMath は非零巻き付きの頂点単純閉路 3,464 本・周期延長した持ち上げ点 123,960 個で再実行して通過した。次は「巻き付きベクトルの横断座標による有限幅の整数帯」の Lean 二版へ進む。

## 対数順序群をレビュー（2026-09-20 の 350 回目）

LLM による検証で本文・Lean 定義・後続参照を照合した。素数から整数への有限台写像、点ごとの加法、整数倍、形式生成元と一意な有限和表示を定め、正の有理数の対数・順序・有限自由エントロピーから反復参照される実質的な定義であり、空の四則主張ではない。記号・住処・有限台の閉性に不一致はなく、式変形を含まない定義なので書き換えは不要だった。次のレビューと表記確認は「正の有理数の対数」から続ける。

## 方向番号の門の横断数と回転数の等式を Lean 二版へ配線（2026-09-20 の 345 回目）

「循環総回転数は方向番号の門の符号付き横断数の 4 倍である」を Lean へ配線した。具体版は四方向の標準整数代表と正負の門横断指示子を定義し、各一歩の三場合から有限和の望遠和までを人手証明どおり合成した。必要十分版は各項の門横断分解と端点の一致だけへ抽出し、導出版で具体版へ戻した。SageMath は非後退な循環方向列 88,592 本・循環遷移 841,564 箇所で再実行して通過した。次は「周期延長した平面持ち上げの単射性」の Lean 二版へ進む。

## 素因数分解の指数をレビュー（2026-09-20 の 349 回目）

LLM による検証で本文・Lean 定義・SageMath の利用箇所・後続参照を照合した。有限個の素因数の指数から有限台を確定し、対数順序群・有理数の対数・有限自由エントロピーの土台として反復参照される実質的な定義であり、空の四則主張ではない。記号・住処・有限積の意味に不一致はなく、式変形を含まない定義なので書き換えは不要だった。次のレビューと表記確認は「対数順序群」から続ける。

## 境界頂点での対角接触の排除を Lean 二版へ配線（2026-09-20 の 344 回目）

「持ち上げ点が相異なる閉路で頂点まわりの内側セルが対角の二セルだけになることは無い」を Lean へ配線した。具体版は、頂点を一回訪問する場合には四セルの内外が二つの巡回弧を作る既証明を使い、訪問しない場合には四辺の通過回数が零なので四セルの内外が一致することを隣接セルの奇偶則から導いた。必要十分版は四つの 0--1 値が二回だけ切り替わるか全て等しいという条件へ抽出し、対角二セルを排除するとともに、二セルだけが内側なら辺隣接することまで固定した。SageMath は持ち上げ点相異の閉歩道 704 本・格子点 20,096 個で再実行して通過した。次は「方向番号の門の横断数と回転数の等式」の Lean 二版へ進む。

## 有限サイズスケーリングの条件付き読みをレビュー（2026-09-20 の 348 回目）

LLM による検証で本文・Lean 定義・SageMath の観察・後続予定を照合した。先頭距離と格子サイズを正の実数へ送り、実対数の比の ε--N 収束を「指数を読める」条件として固定する実質的な定義であり、存在・一意性・指数の値を主張していないため空の四則主張ではない。記号・量化・住処・実対数と極限による実数体への脱出に不一致はなく、書き換えは不要だった。次のレビューと表記確認は本文先頭の「素因数分解の指数」から続ける。

## 内側セル複体の Euler 数を Lean 二版へ配線（2026-09-20 の 343 回目）

「有限な辺連結セル集合の補集合も辺連結なら Euler 数は 1 である」を Lean へ配線した。具体版は、人手証明で得る面数がセル数より一つ多い等式と有限連結平面グラフの Euler 等式を、整数格子セル複体の頂点数・辺数・セル数へ特殊化した。必要十分版は四つの自然数とこの二等式だけへ抽出し、導出版で具体版へ戻した。SageMath は長さ十以下の頂点単純閉歩道 704 本・内側セル延べ 2,888 個で再実行して通過した。次は「境界頂点での対角接触の排除」を Lean 二版へ配線する。

## 実閉部分体の順序を保つ実現をレビュー（2026-09-20 の 347 回目）

LLM による検証で本文・Lean 定義・後続利用を照合した。可算側の実閉部分体から実数値へ脱出する入力として環準同型と狭義順序保存を固定し、後続の先頭距離の正値性が両条件を実際に使うため、空の四則主張ではない。定義ブロックなので式変形の対象ではなく、記号・量化・住処・実数体への脱出理由にも不一致は無かった。次のレビューと表記確認は「有限サイズスケーリングの条件付き読み（実数体への脱出: 実対数・極限）」から続ける。

## セル集合の複体量と一セル追加の増分を Lean 二版へ配線（2026-09-20 の 342 回目）

整数格子セルに付随する四頂点と四辺を有限集合として具体化し、セル集合へ一セルを加えたときの Euler 数の増分を、合併分解と有限集合の包除から導いた。必要十分版は二つの付随有限集合の合併分解・単体の個数四・包除だけへ抽出し、導出版で具体版へ戻した。SageMath の一セル追加 3,328 件と単一セル 17 件、Lean 全体を含むプログラミングによる検証を通した。次は「内側セル複体の Euler 数が 1 に等しいこと」を Lean 二版へ配線する。

## 先頭距離の詰め寄りによる述語導出をレビュー（2026-09-20 の 346 回目）

LLM による検証で本文・Lean 具体版・必要十分版・導出版・SageMath・後続利用を照合した。任意の正有理上界から Fisher 零点と正有理点の距離上界を構成して相転移の可算な言明を導く主張であり、空の四則主張ではない。四層に不一致はなく、本文の書き換えは不要だった。QQ の記号等式と AA の厳密合成も再実行して通過した。

## 全六研究監督: 離散Whitney論証をセル複体量へ進める（2026-09-20 20:57 JST）

実行識別子 `811465fd-4507-46a0-ab2c-38cbe3e537f7`。LLMによる検証では、頂点通過偶奇、
内外セル帯、内側セル集合、外側セル補集合の辺連結性を本文・SageMath・Lean二版へ接続したことを
研究上の前進と判定した。次はセル集合の複体量と一セル追加の増分等式をLeanへ配線する。固定原始特徴の
多項式探索に対する既存停止は維持し、別の既定形式化ストリームを再開とは扱わない。20:55:46の
プログラミングによる観測ではlaunchd登録あり・非実行中、直近実ログは20:51:11正常終了だった。

## 内側セル集合の補集合の辺連結性を Lean 二版へ配線（2026-09-20 の 341 回目）

離散 Whitney 論証の「持ち上げ点が相異なる閉路の内側セル集合の補集合は辺連結である」を Lean へ配線した。具体版は外接長方形の外側と歩道沿いの外側帯を交わる二つの連結な核とし、任意の外側セルからその合併へ至る有限列を合成した。必要十分版は、交わる二つの連結集合と全点からその合併への到達だけへ抽出し、導出版で具体版へ戻した。SageMath は持ち上げ点相異の閉じた非後退歩道 704 本・外側セル 864 個・外側帯への到達 432 件・外接長方形の外側への到達 432 件で再実行して通過した。次の Lean 配線は「セル集合の複体量の定義と一セル追加の増分等式」から続ける。

## 挟み込み区間から取る臨界点の有理近似をレビュー（2026-09-20 の 345 回目）

「挟み込み区間から取る臨界点の有理近似」の本文・Lean 具体版・必要十分版・導出版・SageMath・後続利用を照合した。有理分割の挟み込みと実閉部分体内の平方証人から、平方誤差を任意の正有理数未満へ落とす存在主張であり、後続の零点詰め寄り証明から直接参照されるため空の四則主張ではない。四層に不一致はなく、四つの正有理誤差に対するプログラミングによる検証も通過した。修正点は SageMath 検証の実行日更新だけだった。次のレビューと表記確認は「先頭距離の詰め寄りは詰め寄りの述語を導く」から続ける。

## 歩道沿いの外側帯の辺連結性を Lean 二版へ配線（2026-09-20 の 340 回目）

離散 Whitney 論証の「持ち上げ点が相異なる閉路の歩道沿いの外側帯は空でなく辺連結である」を Lean へ配線した。具体版は各訪問頂点に接する外側セル集合とその合併を整数格子上で定義し、各局所集合の非空性・辺連結性と隣接集合の共通セルを有限連鎖の合成補題へ渡した。必要十分版は対称関係で連結な非空集合の有限鎖へ抽出し、導出版で具体版へ戻した。SageMath は持ち上げ点相異の閉じた非後退歩道 704 本・橋 6,672 箇所・外側帯セル延べ 9,488 個で再実行して通過した。次は「外側セルの行方向の到達と補集合の辺連結性の合成」の Lean 二版へ進む。

## 相転移の可算な言明をレビュー（2026-09-20 の 344 回目）

「相転移の可算な言明」の本文・Lean 定義・SageMath・後続利用を照合した。これは正の有理数全体にわたる零点の詰め寄りを一つの述語として固定し、後続の先頭距離からの導出が参照する定義であるため、空の四則主張ではない。記号・住処・量化に不一致はなく、240 比較・48 非零対と有限標本のプログラミングによる検証も通過した。修正点は SageMath 検証の実行日更新だけだった。次のレビューと表記確認は「正の有理数による臨界点の平方近似」から続ける。

## 内側セル集合の辺連結性を Lean 二版へ配線（2026-09-20 の 339 回目）

「持ち上げ点が相異なる閉路の内側セル集合は辺連結である」を Lean へ配線した。具体版は歩道沿いの内側帯を連結な核とし、任意の内側セルから帯へ至る内側セルだけの有限列を前後につないだ。必要十分版は、対称関係で連結な非空部分集合へ全点が集合内で到達できれば全体が連結になることだけへ抽出し、導出版で具体版へ戻した。SageMath は持ち上げ点相異の閉じた非後退歩道 704 本・内側セル 2,888 個・行方向の帯への到達 2,888 件で再実行して通過した。次の Lean 配線は「歩道沿いの外側帯の辺連結性」から続ける。

## 零点と正の有理点の距離の二乗の非零性をレビュー（2026-09-20 の 343 回目）

LLM による検証で本文・Lean 具体版・必要十分版・導出版・SageMath・後続利用を照合した。距離の零性から零点と正の有理点の一致を導き、正の有理点が Fisher 零点でないことと矛盾させる非自明な主張で、相転移の可算な言明から直接参照されるため空の四則主張ではない。四層に不一致はなく、48 組に対するプログラミングによる検証も通過した。背理法と集合所属の矛盾による証明なので一続きの等号変形へ変えず、本文中の主張を適用箇所で参照していることを確認した。次のレビューと表記確認は「相転移の可算な言明」から続ける。

## 歩道沿いの内側帯の辺連結性を Lean 二版へ配線（2026-09-20 の 338 回目）

「持ち上げ点が相異なる閉路の歩道沿いの内側帯は空でなく辺連結である」を Lean へ配線した。具体版は各訪問頂点に接する内側セル集合とその合併を整数格子上で定義し、各局所集合の辺連結性と隣接集合の共通セルから帯全体を合成した。必要十分版は対称関係で連結な非空集合の有限鎖へ抽出し、導出版で具体版へ戻した。SageMath は持ち上げ点相異の閉じた非後退歩道 704 本・橋 6,672 箇所・帯セル延べ 2,888 個で再実行して通過した。次の Lean 配線は「内側セルの辺連結性の合成」から続ける。

## 臨界点への距離の二乗の零性をレビュー（2026-09-20 の 342 回目）

LLM による検証で本文・Lean 具体版・必要十分版・導出版・SageMath・後続利用を照合した。平方和の零性から虚部係数を零へ絞り、実部係数を臨界点へ一致させる非自明な主張で、先頭距離の正値性から直接参照されるため、空の四則主張ではない。四層に不一致はなく、18 組に対するプログラミングによる検証も通過した。二つの含意と背理法を含むため一続きの式変形へ変えず、内部の等式鎖が一行一操作・行末の根拠を持つことを確認した。次のレビューと表記確認は「Fisher 零点上では臨界点への距離の二乗は零でない」から続ける。

## 頂点まわりの同奇偶セルの弧を Lean 二版へ配線（2026-09-20 の 337 回目）

「持ち上げ点が相異なる閉路の各訪問頂点で内側セルと外側セルはそれぞれ一つの弧をなす」を Lean へ配線した。具体版は接する四辺の総通過回数が二で各辺が高々一回であることと、共有辺の通過奇偶が隣接セルの内外の切替に一致することを合成した。必要十分版は四つの境界のうち相異なる二つだけで状態が切り替わる有限巡回列へ抽出し、導出版で具体版へ戻した。SageMath は持ち上げ点相異の閉じた非後退歩道 704 本・訪問頂点 6,672 個で再実行して通過した。次は「歩道沿いの内側帯の辺連結性」の Lean 二版へ進む。

## 零点と臨界点の距離の二乗をレビュー（2026-09-20 の 341 回目）

LLM による検証で「零点と臨界点の距離の二乗」の本文・Lean 定義・後続利用を照合した。代数的数の一意表示と臨界点の実閉部分体所属から、距離の二乗を実閉部分体の元として確定し、後続の零性・最小距離・詰め寄りから反復参照されるため、空の四則主張ではない。記号の濫用と住処の不一致は無く、定義ブロックなので式変形の書き換えは不要だった。関連する零性同値の SageMath 18 組も再実行して通過した。次のレビューと表記確認は「臨界点への距離の二乗の零性は一致と同値である」から続ける。

## 頂点に接する四辺の通過総和を Lean 二版へ配線（2026-09-20 の 336 回目）

`claim_vertex_incident_edge_traversal_even` を本文対応の具体版・必要十分版・導出版へ形式化した。具体版は各単位格子辺を四方向で場合分けし、接する四辺の指示値を辺の始点・終点の指示値へ書き換える。必要十分版は閉じた有限列の始点指示値と終点指示値の巡回的な添字付け替えだけを抽出し、各区間の両端への寄与総和が訪問数の二倍になることを示す。導出版で具体版が同じ手順の特殊化であることを固定した。次は「頂点まわりの同奇偶セルの弧」の Lean 二版へ進む。

## 自己双対点の平方根と臨界点の実閉部分体所属をレビュー（2026-09-20 の 340 回目）

`claim_critical_point_mem_real_closed` の本文・Lean 具体版・SageMath・後続利用を照合した。一意表示から虚部が零であることを導き、臨界点を実閉部分体へ戻す主張は後続の距離と順序から反復参照されるため、空の四則主張ではない。四層に不一致はなく、厳密計算のプログラミングによる検証も通過した。式変形は場合分けを含むため一列へ無理に変えず、内部の展開と係数比較の根拠が保たれていることを確認した。

## 内側セルの行区間が奇数回通過の縦辺で終わることを Lean 二版へ配線（2026-09-20 の 335 回目）

離散 Whitney 論証の「内側セルは行に沿って奇数回通過の縦辺まで届く」を Lean へ配線した。具体版は右方で交差数が零になる位置の存在から最初の外側セルを取り、その直前までのセルが内側であることと、隣接セルの交差数の等式から境界縦辺の通過回数が奇数かつ正であることを示した。必要十分版は開始位置の奇数性・右方に偶数位置があること・隣接二位置の個数差が境界個数であることだけを抽出し、導出版で具体版へ戻した。SageMath は 5,340 閉歩道・3,912 内側セルで再実行して通過した。次の Lean 配線は「頂点に接する四辺の通過総和の偶数性」から続ける。

## 二以上の有限格子の Fisher 零点集合の非空性をレビュー（2026-09-20 の 339 回目）

LLM による検証で本文・Lean 具体版・必要十分版・導出版・SageMath・後続利用を照合した。一頂点だけ反転した配位が正次数の非零係数を与え、代数閉性から根を得るため、空の四則主張ではない。四層に不一致はなく、$L=1,2,3$ に対するプログラミングによる検証も通過した。存在構成なので無理に等号列へ変えず、内部の係数評価と零点評価が適用箇所の根拠を持つことを確認した。次のレビューと表記確認は「自己双対点の平方根と臨界点は実閉部分体の元である」から続ける。

## 隣接セルの交差奇偶と共有辺の通過奇偶を Lean 二版へ配線（2026-09-20 の 334 回目）

離散 Whitney 論証の「隣接セルの交差奇偶と共有辺の通過奇偶」を Lean へ配線した。具体版は横に隣り合うセルについて右半直線上の縦辺を共有辺とその右側へ互いに素に分割し、上下に隣り合うセルについて閉じた指示値列の境界横断数が偶数であることと、単位格子辺の横断を下側・上側・共有辺へ分割することから合同式を導いた。必要十分版は整数位置の右側条件を次の位置とその右へ分ける有限和の等式と、偶数総数の三分割だけを抽出し、導出版で具体版へ戻した。SageMath の二検算はそれぞれ 5,340 閉歩道・166,232 隣接セル対で再実行して通過した。次の Lean 配線は「内側セルの行区間が奇数回通過の縦辺で終わること」から続ける。

## 実閉部分体の有限集合の最小元をレビュー（2026-09-20 の 338 回目）

LLM による検証で本文・Lean 具体版・必要十分版・導出版・SageMath・後続利用を照合した。三分法と推移律から有限集合の最小元の存在一意性を導き、先頭距離の定義から直接参照されるため、空の四則主張ではない。四層に不一致はなく、63 個の有限集合に対するプログラミングによる検証も通過した。存在は有限集合への帰納法、一意性は三分法の排他性であり、一続きの等号変形へ変える対象ではない。次のレビューと表記確認は「二以上の有限格子の Fisher 零点集合は空でない」から続ける。

## 右半直線交差による内側セルの有限性を Lean 二版へ配線（2026-09-20 の 333 回目）

離散 Whitney 論証の「右半直線との交差奇偶で定めた内側セルは有限個である」を Lean へ配線した。具体版では平面持ち上げの整数座標列から右半直線交差数と奇数支持を定義し、閉路の水準横断数から外接矩形の左側で交差数が偶数、残る三方向の外側で零になることを示して有限性を得た。必要十分版は四つの境界条件だけを抽出し、導出版は具体版の境界補題を渡して同じ結論へ戻した。次の Lean 配線は「隣接セルの交差奇偶と共有辺の通過奇偶」から続ける。

## 実代数的数の狭義順序の推移律をレビュー（2026-09-20 の 337 回目）

LLM による検証で本文・Lean 具体版・必要十分版・導出版・SageMath・後続利用を照合した。平方和の閉性と零性から順序の推移を導き、後続の最小元証明から反復参照されるため、空の四則主張ではない。本文の三段の等式鎖は一続き・一行一操作・行末の根拠へ統一済みで、四層に不一致は無かった。有限標本に対するプログラミングによる検証も通過した。次のレビューと表記確認は「実閉部分体の空でない有限集合は最小元をちょうど 1 つ持つ」から続ける。

## 整数列の水準横断数を Lean 二版へ配線（2026-09-20 の 332 回目）

離散 Whitney 論証の先頭「隣接差が高々 1 の整数列の水準横断数の恒等式」を Lean へ配線した。具体版では整数列を自然数上の関数の先頭 $n+1$ 項で表し、上下横断数を有限和で数える。隣接差が $-1,0,1$ のいずれかなら水準指示値の一歩差が上下横断指示値の差に一致することを示し、望遠鏡和から端点の三場合を得た。必要十分版は任意の加法可換群で隣接差の有限和が終点と始点の差になる部分だけを抽出し、導出版は具体側の指示値同定からこの補題へ接続する。Lean 全体ビルドと未証明公理検査を通過した。次の Lean 配線は「右半直線交差による内側セルの定義と有限性」から続ける。

## 実閉部分体の 2 と -2 の平方性を二主張へ分割（2026-09-20 の 336 回目）

LLM による検証で本文・Lean・SageMath・後続利用を照合した。$2$ の平方性は平方の和の閉性から $R$ 内の平方根を取り、$-2$ の非平方性は平方三分法の排他性から導く。二つは後続から別々に参照されるため、1 ブロック 1 主張の規則に従って本文ブロックとラベルを分割し、$-2$ の枝を排除する後続参照を新しい非平方性のラベルへ付け替えた。どちらも空の四則主張ではない。対応する厳密計算のプログラミングによる検証は通過した。次のレビューと表記確認は「実代数的数の狭義順序は推移的である」から続ける。

## 実閉部分体の平方の和のレビューと表記確認（2026-09-20 の 335 回目）

LLM による検証で本文・Lean 具体版・必要十分版・導出版・SageMath・後続利用を照合した。代数閉性と一意表示から成分を取り出し、Gauss の恒等式で平方の和の閉性を与え、後続から反復参照されるため、空の四則主張ではない。四層に不一致はなく、対応する厳密計算のプログラミングによる検証も通過した。一意表示から成分を取り出す準備と恒等式への代入は一続き・一行一操作であるため、本文は書き換えなかった。次のレビューと表記確認は「実閉部分体では 2 が平方であり、-2 は平方でない」から続ける。

## 全六研究監督: 固定原始特徴の多項式探索を終了（2026-09-20 15:02 JST）

実行識別子 `8b419085-2dcb-4d5f-9198-3aaef3b9d55a`。LLMによる検証では、次数二・三以下の有限反証と
四分の一回転の非対応を研究上の前進と判定した一方、次数二の非可解後に次数を追加しないrunbookと
実行が矛盾していた。統合済み証拠は保存し、回転比較を固定原始特徴の多項式探索の終端とした。
回転後の不一致分解、次数四、新特徴へは進まず、一般辺長で定義できOnsager閉形式へ接続する別候補の
採用判断を分離する。15:04:04のプログラミングによる観測では登録あり・非実行中で、直近実ログは
15:00:48正常終了だった。

## 実閉部分体の二つの平方の和の零性のレビューと表記確認（2026-09-20 の 334 回目）

LLM による検証で本文・Lean 具体版・必要十分版・導出版・SageMath・後続利用を照合した。平方和を虚数単位による二因子へ分解し、零因子の不在と実閉部分体の一意表示から両成分の零性を導く主張で、順序の推移・有理数の平方性・臨界点の符号判定などから反復参照されるため、空の四則主張ではない。四層に不一致はなく、有限標本に対するプログラミングによる検証も通過した。場合分けを含むため式変形へ無理につなげず、既存の因数分解鎖を維持した。次のレビューと表記確認は「実閉部分体では二つの平方の和がまた平方である」から続ける。

## 次数三以下の二つの矛盾証拠の四分の一回転比較（2026-09-20 の 331 回目）

端点だけの支持係数側の十六巡回軌道を、内部語・二端点の所属・切断旗ごと正負の四分の一回転で移した。プログラミングによる検証では正方向の像十四軌道、逆方向の像十一軌道が一辺二・三の有限宇宙に現れたが、語位置・端点の支持係数側の十六軌道との共通部分はいずれも零だった。上下・左右各八件という方向分布の一致は回転対称性の像ではない。次は回転後の不一致を内部語・端点所属・切断旗・選択集合所属へ分解し、次数四や新しい原始特徴へは進まない。

## 実代数的数の狭義順序の三分法のレビューと表記確認（2026-09-20 の 333 回目）

LLM による検証で本文・Lean 具体版・必要十分版・導出版・SageMath・後続利用を照合した。差の平方三分法を順序・等号の排他的な三場合へ移す主定理で、後続の最小元・距離比較・臨界点の符号判定から反復参照されるため、空の四則主張ではない。有限標本に対するプログラミングによる検証も通過し、修正点は無かった。場合分けなので式変形へ書き換えなかった。次のレビューと表記確認は「実閉部分体の二つの平方の和が零なら、両方が零である」から続ける。

## 次数三以下の二つの矛盾証拠の構造比較（2026-09-20 の 330 回目）

次数三以下の二つの左核矛盾証拠を観測・弧型・巡回軌道・候補記述・活性原始特徴ベクトルへ分解した。プログラミングによる検証では各証拠が十六個の相異なる観測・弧型・巡回軌道からなり、二証拠間の共有数は全て零だった。端点だけの支持係数側は上下方向各八件、語位置・端点の支持係数側は左右方向各八件だった。直接の共通部分はないが直交する方向軸に同じ分布を持つため、次は正方格子の四分の一回転が前者の巡回軌道を後者へ運ぶか判定する。次数四や新しい原始特徴へは進まない。

## 実代数的数の広義順序のレビューと表記確認（2026-09-20 の 332 回目）

LLM による検証で本文定義・Lean 具体版・必要十分版・SageMath・後続利用を照合した。狭義順序と等号から区間・不等式で反復利用する関係を定めるため、空の四則主張ではない。有限標本に対するプログラミングによる検証も通過し、修正点は無かった。次のレビューと表記確認は「実代数的数の狭義順序の三分法」から続ける。

## 固定原始特徴の次数三以下ブール式の有限反証（2026-09-20 の 329 回目）

絶対境界方向の四指示関数、相対端点十成分、境界基準の巡回三文字 29,012 種の出現回数の偶奇を原始特徴に固定し、定数・一次・相異なる二特徴積・相異なる三特徴積を全て許す $\mathbb F_2$ 線形系を二係数について一括判定した。プログラミングによる検証では 11,841 観測で現れた係数列 460,111 本（うち三次列 362,140 本）の行列階数は 11,584、二係数の拡大階数はいずれも 11,585 で非可解だった。それぞれ 16 観測の左核矛盾証拠を保存し、行の XOR が零・標的の XOR が一であることを再計算した。これは辺長二・三の有限宇宙における固定候補クラスの棄却であり、一般の辺長の不可能性ではない。次は二証拠の共有構造を比較し、次数四や新しい原始特徴へは自動的に進まない。

## 実代数的数の狭義順序のレビューと表記確認（2026-09-20 の 331 回目）

LLM による検証で本文定義・Lean 具体版・必要十分版・SageMath・後続利用を照合した。差 $b-a$ が可算側の集合 $R$ に属することを明示し、後続の三分法・最小元・距離比較から反復参照されるため、空の四則主張ではない。有限標本に対するプログラミングによる検証も通過した。次のレビューと表記確認は「実代数的数の広義順序」から続ける。

## 次数二有限系の標的列を $\mathbb F_2$ へ修正（2026-09-20 の 331 回目）

既存出力のレビューで、次数二以下の有限線形系が行を XOR する一方、二つの標的列を整数加算していた実装誤りを見つけた。標的加算・左核証拠の再計算・可解時の後退代入を全て法二へ直した。プログラミングによる再実行後も係数行列の階数 8,779、両標的の拡大階数 8,780、保存済み左核証拠は変わらなかったため、既存の非可解という結論は維持される。

## 二つの次数二以下矛盾証拠が共有する三次相互作用（2026-09-20 の 328 回目）

保存済み二証拠の定数・一次・二次の全単項式の相殺を再計算した。二証拠は 16 観測を共有し、両方で同時に奇数回現れる相異なる三特徴積は 34 列だった。内訳は絶対境界方向・巡回三文字頻度・相対端点の積 14 本、絶対境界方向・相対端点二つの積 6 本、巡回三文字頻度・相対端点二つの積 9 本、相対端点三つの積 5 本である。これは三次式の存在証明ではない。次は同じ固定原始特徴の次数三以下全体を二係数について一括判定し、非可解でも次数四や新しい原始特徴へ自動的に進まない。

## 零元でない実代数的数の平方に関するレビュー（2026-09-20 の 330 回目）

LLM による検証で本文・Lean 二版・導出版・SageMath・後続二箇所を照合した。平方三分法の排他性を取り出して距離二乗の零性から虚部係数の零性を導くため、空の四則主張ではない。四層に修正点はなく、有限標本に対するプログラミングによる検証も通過した。次のレビューと表記確認は「実代数的数の狭義順序」から続ける。

## 絶対境界方向つき次数二以下ブール式の有限反証（2026-09-20 の 327 回目）

絶対境界方向の四指示関数、相対端点十成分、境界基準の巡回三文字 29,012 種の出現回数の偶奇を原始特徴に固定し、定数・一次・相異なる二特徴積を全て許す $\mathbb F_2$ 線形系を 11,841 観測で一括判定した。プログラミングによる検証では係数行列の階数 8,779 に対し、端点だけの支持係数と語位置・端点の支持係数はいずれも拡大階数 8,780 で非可解だった。各係数の左核矛盾証拠を保存し、観測行の XOR が零・標的値の XOR が一であることを再計算した。有限宇宙における候補クラスの棄却であり、一般の辺長の不可能性とは呼ばない。次は二つの矛盾証拠の共通構造を調べ、次数または原始特徴を一項ずつ増やす反復には進まない。

## 有理数は実代数的数である主張のレビューと表記確認（2026-09-20 の 329 回目）

LLM による検証で本文・Lean 具体版・必要十分版・導出版・SageMath・後続の利用箇所を照合した。部分体が有理数像を含むことは値の所属と可算側での閉性を確定し、後続から反復参照されるため、空の四則主張ではない。自然数・整数・有理数の順に閉性を使う本文と Lean 二版の論理手順は一致し、記号の濫用と未証明の付加主張は無かった。有限標本に対するプログラミングによる検証は通過した。次のレビューと表記確認は「零元でない実代数的数の平方は -1 にならない」から続ける。

## 全六研究監督: ブール式の有限候補クラスを固定（2026-09-20 09:07 JST）

実行識別子 `65e3693f-9257-40eb-aed3-30e0daef8cda`。LLMによる検証では、絶対境界方向を戻すと二係数の衝突が有限全データで消える一方、任意のブール式は真理値表で必ず作れるため閉形式の判定にならない。runbookとstateを、絶対方向・相対端点・巡回三文字頻度の偶奇からなる次数二以下のF2多項式を一括判定する段取りへ修正した。09:07:19のプログラミングによる観測では登録あり・実行中で、現行回の成否は未確認。

## 実閉部分体と虚数単位のレビューと表記確認（2026-09-20 の 328 回目）

LLM による検証で本文・Lean 具体版・必要十分版・導出版・SageMath・後続の利用箇所を照合した。組 $(R,\omega)$ の五条件は実代数的順序・距離・臨界点の符号を固定して後続から反復参照され、$\omega^4=1$ も 1 の冪根として使われるため、空の四則主張ではない。SageMath 冒頭コメントだけが第五条件追加前の「4 条件」を残していたので「5 条件」へ直した。帰結 $\omega^4=1$ の三段は一行一操作へ統一済みである。五条件の有限標本に対するプログラミングによる検証は通過した。次のレビューと表記確認は「有理数は実代数的数である」から続ける。

## 正の有理点は Fisher 零点でない主張のレビューと表記確認（2026-09-20 の 327 回目）

LLM による検証で本文・Lean 具体版・必要十分版・導出版・SageMath・後続の利用箇所を照合した。分配多項式の正の有理点での正値性から有限格子の Fisher 零点を排除し、後続の零点詰め寄り述語から参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無く、式変形も一続き・一行一操作・適用箇所の根拠へ統一済みなので書き換えなかった。格子サイズ一から三、正の有理点十一個に対するプログラミングによる検証は通過した。次のレビューと表記確認は「実閉部分体と虚数単位」から続ける。

## 絶対境界方向を戻すと二つの係数が有限全データ上で決定（2026-09-20 の 326 回目）

境界方向を上方向へ正規化した巡回三文字頻度と相対端点 10 成分へ、候補ごとの正規化前の絶対境界方向を戻した。プログラミングによる検証では、9,739 弧型の分割が 7,427 記述から 9,735 記述へ細分され、端点だけの支持の 220 種、語位置と端点の支持の 260 種の係数衝突はいずれも 0 になった。LLM による検証では、追加した特徴が絶対境界方向だけなので、現有限データで従来の衝突を解く不足情報はこの方向依存に局在する。次は絶対境界方向つき記述から、二つの係数を与える有限なブール式を抽出できるか判定する。

## 双対な点どうしの自由エントロピーの関係のレビューと表記確認（2026-09-20 の 326 回目）

LLM による検証で本文・Lean 具体版・必要十分版・導出版・SageMathを照合した。分配多項式の値の双対分解を対数順序群の等式へ移す主定理であり、対数を適用する各量の正値性も定義域を保証する内容を持つため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無く、式変形も一続き・一行一操作・適用箇所の根拠へ統一済みなので書き換えなかった。格子サイズ一から三、有理点六個に対するプログラミングによる検証は通過した。次のレビューと表記確認は「正の有理点は Fisher 零点でない」から続ける。

## 境界方向を共通基準にした全窓長でも係数未決定（2026-09-20 の 325 回目）

各実現可能な境界入射候補の境界方向を上方向へ送る一意な回転を、現有限データに実在する未評価の巡回窓長四から八へ共通に作用させた。プログラミングによる検証では、境界基準の順序付き窓は長さ四から八で 47,848、59,704、64,308、65,512、65,676 種へ増えたが、全窓長で 9,739 弧型の分割は 7,427 記述、係数衝突は端点だけの支持で 220 種、語位置と端点の支持で 260 種のままだった。LLM による検証では、各窓長の分割が直前の分割を細分し、記述数が等しいので窓長三から八までの分割は一致する。境界基準の局所窓頻度候補を有限範囲で棄却した。次は正規化で落とした絶対境界方向を候補記述へ戻し、その依存だけで係数衝突が消えるかを判定する。

## 分配多項式の値の双対分解のレビューと表記確認（2026-09-20 の 325 回目）

LLM による検証で「分配多項式の値の双対分解」の本文・Lean 具体版・必要十分版・導出版・SageMath・後続の利用箇所を照合した。四つのセクター双対恒等式を分配多項式へ合成し、後続の自由エントロピー双対関係から参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無く、修正点は無かった。式変形は一続き・一行一操作・適用箇所の根拠へ統一済みなので書き換えなかった。格子サイズ一から三、有理点六個に対するプログラミングによる検証は通過した。次のレビューと表記確認は「双対な点どうしの自由エントロピーの関係」から続ける。

## 境界方向を共通基準にした連続三文字でも係数未決定（2026-09-20 の 324 回目）

各実現可能な境界入射候補の境界方向を上方向へ送る一意な回転を、巡回内部語の順序付き連続三文字へ共通に作用させた。プログラミングによる検証では 29,012 種の順序付き三文字と相対端点 10 成分の組により 9,739 弧型は 7,427 記述へ分かれたが、端点だけの支持で 220 種、語位置と端点の支持で 260 種の係数衝突が残った。三文字記述は二文字記述を細分する一方で記述数が同じなので有限範囲では分割が一致する。連続する窓長二・三で分割と衝突集合が変わらないため、次は実在する未評価の窓長四から八を一括判定する。

## セクター多項式の値の双対関係のレビューと表記確認（2026-09-20 の 324 回目）

LLM による検証で本文・Lean 具体版・必要十分版・導出版・SageMath・後続の利用箇所を照合した。有限和の各項で双対重みを移し共通因子を括り出す主定理であり、後続の自由エントロピー双対分解から参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無く、式変形も一続き・一行一操作へ統一済みなので書き換えなかった。格子サイズ一から三、有理点六個、四セクターに対するプログラミングによる検証は通過した。次のレビューと表記確認は「分配多項式の値の双対分解」から続ける。

## 境界方向を共通基準にした隣接二文字の出現回数でも係数未決定（2026-09-20 の 323 回目）

各実現可能な境界入射候補の境界方向を上方向へ送る一意な回転を、巡回内部語の順序付き隣接二文字へ共通に作用させた。プログラミングによる検証では 7,856 種の順序対と相対端点 10 成分の組により 9,739 弧型は 7,427 記述へ分かれたが、端点だけの支持で 220 種、語位置と端点の支持で 260 種の係数衝突が残った。この分割と衝突数は各隣接窓を独立に回転同値でまとめた既存判定と一致するため、有限範囲では境界基準への変更が情報を増やさない。次は同じ境界基準で連続三文字の順序を保った出現回数を判定する。

## 双対変換の有理開区間保存のレビューと表記確認（2026-09-20 の 323 回目）

LLM による検証で「双対変換は零と一の間の有理数を零と一の間の有理数へ写す」の本文・Lean 具体版・必要十分版・導出版・SageMath・後続の利用箇所を照合した。有理数内での双対変換の定義域と値域を確定して後続のセクター双対関係から参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無く、値の有理数性・正値性・上端との比較も一続きの式変形へ統一済みなので書き換えなかった。分母 2 から 40 の既約分数 489 点に対するプログラミングによる検証は通過した。次のレビューと表記確認は「セクター多項式の値の双対関係」から続ける。

## 境界方向を共通基準にした単一文字出現回数でも係数未決定（2026-09-20 の 322 回目）

各実現可能な境界入射候補の境界方向を上方向へ送る一意な回転を内部語の全ての文字へ共通に作用させ、回転後の文字出現回数と候補ごとの相対端点 10 成分を対応づけた。プログラミングによる検証では 9,739 弧型が 6,068 記述へ分かれたが、端点だけの支持で 310 種、語位置と端点の支持で 380 種の係数衝突が残った。LLM による検証では、局所窓ごとに回転を忘れる前段と異なり、全語に同じ端点境界基準を使っても単一文字の個数は順序を失うため不足すると判断した。次は同じ境界基準で隣接二文字の順序を保った出現回数を判定する。

## 零と一の間の有理数の集合のレビューと表記確認（2026-09-20 の 322 回目）

LLM による検証で「零と一の間の有理数の集合」の本文・Lean 定義・SageMath・後続の利用箇所を照合した。有理数の可算な定義域を確定し、Kramers--Wannier 双対変換・有限系の自由エントロピー・高温側の有理切断から反復参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無く、定義ブロックなので式変形の書き換えも不要だった。分母 2 から 40 の既約分数 489 点に対するプログラミングによる検証は通過した。次のレビューと表記確認は「双対変換は零と一の間の有理数を零と一の間の有理数へ写す」から続ける。

## 未評価の巡回窓長を一括判定して局所窓頻度候補を棄却（2026-09-20 の 321 回目）

現有限データの内部語の最大長は 8 であり、未評価だった巡回窓長 5、6、7、8 を有限集合として一括評価した。プログラミングによる検証では、相対回転族は順に 14,926、16,077、16,378、16,419 族へ増えたが、相対端点 10 成分との組による分割は全て長さ 4 と同じ 7,427 記述で、端点だけの支持の 220 種、語位置と端点の支持の 260 種の係数衝突も全て残った。各長さの分割が直前の分割を細分することと記述数の一致から、長さ 4 から 8 の分割は同じである。有限範囲で局所窓頻度の候補クラスを棄却した。次は端点の境界方向を共通基準として内部文字を相対化した出現回数が二つの係数を決めるかを判定する。

## 自己双対点のセクター対応のレビューと表記確認（2026-09-20 の 321 回目）

LLM による検証で「自己双対点では高温側と低温側のセクター変数が一致する」の本文・Lean 具体版・SageMath・利用関係を照合した。自己双対点で双対変換が不動になることを有限系のセクター双対恒等式へ特殊化する内容を持つ主定理であり、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無く、一続きの式変形と各行の根拠も揃っているため書き換えなかった。格子サイズ 1 から 3 と四セクターに対するプログラミングによる検証は通過した。次のレビューと表記確認は「零と一の間の有理数の集合」から続ける。

## 連続四文字の相対回転は三文字より情報を増やさない（2026-09-20 の 320 回目）

巡回内部語の連続四文字へ同じ四回転を作用させ、絶対方向名を除きつつ四文字間の局所基準の相対回転を保つ 11,962 四文字族へまとめた。プログラミングによる検証では、相対端点 10 成分との組による 9,739 弧型の分割は三文字の場合と同じ 7,427 記述で、端点だけの支持の 220 種、語位置と端点の支持の 260 種の係数衝突も全て残った。LLM による検証では、四文字頻度は先頭三文字が一致する族を足せば三文字頻度を復元できる一方、分割数が増えないため、この有限範囲では両記述の分割は一致すると判断した。次は現有限データに実在する未評価の巡回窓長を有限集合として列挙し、一括評価により最初に分割を厳密に細分化する最小長、または局所窓頻度の候補クラス全体の不成立を判定する。

## 全六研究監督: 巡回窓長を有限候補群として一括判定する（2026-09-20 00:13 JST）

実行識別子 `9bd8381d-5644-4ffe-bd00-547947a24239`。LLMによる検証では、三文字窓が二文字窓と
同じ7,427記述、同じ220種・260種の係数衝突を残す成果を前進と認定した。一長ずつ増やす反復には
停止規則がないため、現有限データに実在する未評価窓長を有限集合として一括評価する段取りへ変更した。
00:13:27のプログラミングによる観測ではlaunchd登録あり・実行中だが現行ログに終端がなく、成否は
未確認とした。有限結果を一般語長へ外挿していない。
## 代数的数の点におけるセクター双対関係のレビューと表記確認（2026-09-20 の 320 回目）

LLM による検証で「代数的数の点における高温側と低温側のセクター双対関係」の本文・Lean 具体版・SageMath・後続の利用箇所を照合した。有限なセクター和の各項を Kramers--Wannier 双対変換で移し、共通因子を括り出す内容を持ち、自己双対点への特殊化から参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無く、一続きの式変形と各行の根拠も揃っているため書き換えなかった。格子サイズ 1 から 3、四セクター、三つの代数的数に対するプログラミングによる検証は通過した。次のレビューと表記確認は「自己双対点では高温側と低温側のセクター変数が一致する」から続ける。

## 連続三文字の相対回転は隣接二文字より情報を増やさない（2026-09-18 の 319 回目）

巡回内部語の連続三文字へ同じ四回転を作用させ、絶対方向名を除きつつ三文字間の局所基準の相対回転を保つ 7,253 三文字族へまとめた。プログラミングによる検証では、相対端点 10 成分との組による 9,739 弧型の分割は隣接二文字の場合と同じ 7,427 記述で、端点だけの支持の 220 種、語位置と端点の支持の 260 種の係数衝突も全て残った。LLM による検証では、三文字頻度は先頭二文字が一致する族を足せば二文字頻度を復元できる一方、分割数が増えないため、この有限範囲では両記述の分割は一致すると判断した。次は連続四文字へ同じ四回転を作用させた巡回窓を加える。

## 臨界点のレビューと表記確認（2026-09-18 の 319 回目）

LLM による検証で「臨界点」の本文・Lean 定義・SageMath・後続の利用箇所を照合した。臨界点を二次体の正錐に属する自己双対方程式の一意な根として指定し、後続の分配多項式評価・双対性・実閉部分体上の距離から反復参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無く、自己双対性に至る代数的な含意にも適用箇所の根拠が揃っているため書き換えなかった。二つの平方根と有理表示 98 組に対するプログラミングによる検証は通過した。次のレビューと表記確認は「代数的数の点における高温側と低温側のセクター双対関係」から続ける。

## 全六研究監督: 局所要約が失う係数情報を確認（2026-09-18 09:00 JST）

実行識別子 `7d0b041e-373d-4814-8dcc-0cae6a050196`。LLMによる検証では、二支持の正準係数式が
合成され、内部語軌道、端点10成分、一文字の局所所属、相対スロット、隣接二文字の相対回転では
有限データ上の係数衝突が残ると分かった。現行段取りは妥当で、次は連続三文字の共同四回転巡回窓を
有限全データで判定する。09:00:01のプログラミングによる観測ではlaunchd登録あり・非実行中・
runs 169・last exit 0で、実ログの08:52:38正常終了を確認した。

## 隣接二文字の相対回転でも係数未決定（2026-09-18 の 318 回目）

巡回内部語の隣接二文字へ同じ四回転を作用させ、絶対方向名を除きつつ二文字の局所基準の相対回転を保つ 1,964 隣接族へまとめた。プログラミングによる検証では、相対端点 10 成分との組で 9,739 弧型は 7,427 記述へ分かれたが、端点だけの支持で 220 種、語位置と端点の支持で 260 種の係数衝突が残った。LLM による検証では、個々の文字内と隣接二文字間の相対配置を保っても衝突するため、次は連続三文字へ同じ四回転を作用させた巡回窓を加える。

## 自己双対方程式の正根一意性のレビューと表記確認（2026-09-18 の 318 回目）

LLM による検証で「自己双対方程式の正の根は −1+s に限る」の本文・Lean 具体版・必要十分版・導出版・SageMath・後続の利用箇所を照合した。二根の網羅と負根の正錐非所属から正根を一意に確定し、臨界点の定義と後続の双対性から反復参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無く、二根の場合分けと負根の正錐所属・非所属の矛盾も一続き・一行一操作・適用箇所の根拠へ統一済みなので書き換えなかった。二つの平方根と有理表示 98 組に対するプログラミングによる検証は通過した。次のレビューと表記確認は「臨界点」から続ける。

## 単一辺基準の相対スロット族でも係数未決定（2026-09-18 の 317 回目）

内部語の各文字について、単一辺・二重辺・切断旗の四スロット所属を四回転で同時に移し、時計回り・反時計回りを保ったまま絶対方向名を除く 55 種の相対スロット族へまとめた。プログラミングによる検証では、相対端点 10 成分との組で 9,739 弧型は 6,021 記述へまとまったが、端点だけの支持で 312 種、語位置と端点の支持で 381 種の係数衝突が残った。LLM による検証では、個々の文字内の相対配置を保っても文字間の局所基準の回転関係を失うため、次は隣接する文字間で局所基準の回転がどう移るかを加える。

## 根 −1−s は正でない主張のレビューと表記確認（2026-09-18 の 317 回目）

LLM による検証で本文・Lean 具体版・必要十分版・導出版・SageMath・後続の利用箇所を照合した。二次体の正錐三条件を全て排除し、直後の正根一意性から参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無く、表示 $(-1,-1)$ から三条件を排除する計算も一続き・一行一操作・適用箇所の根拠へ統一済みなので書き換えなかった。次のレビューと表記確認は「自己双対方程式の正の根は −1+s に限る」から続ける。

## 局所所属の述語族への集約を反証（2026-09-18 の 316 回目）

内部語の各文字を、単一辺の局所向きが直進か曲がりか、二重辺と切断旗の個数、および単一辺・二重辺・切断旗の二重・三重交差の個数からなる方向名に依らない 34 種の局所所属族へまとめた。プログラミングによる検証では、相対端点 10 成分との組で 9,739 弧型は 5,753 記述へまとまったが、端点だけの支持で 315 種、語位置と端点の支持で 388 種の係数衝突が残った。LLM による検証では、98 文字の任意列挙を自然な有限個の局所述語へ置き換える方向は維持できる一方、所属の個数だけではスロットの相対配置を失いすぎると判断した。次は単一辺の局所向きを基準に、二重辺と切断旗のスロット相対配置を述語族へ加える。

## 根 −1−s の表示のレビューと表記確認（2026-09-18 の 316 回目）

LLM による検証で本文・Lean 具体版・必要十分版・導出版・SageMath・後続の利用箇所を照合した。二次体の表示を一意に確定し、直後の正錐判定から反復参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無く、表示の写像と表示の一意性を適用する一行も適用箇所の根拠へ統一済みなので書き換えなかった。次のレビューと表記確認は「根 −1−s は正でない」から続ける。

## 文字出現回数を最小 98 成分へ縮約（2026-09-18 の 315 回目）

前段の相対端点 10 成分を固定し、内部語に現れる 216 種の文字の出現回数から、二つの正準巡回支持の係数をともに決める最小成分集合を有限全データで判定した。プログラミングによる検証では、係数対が異なる 38,352 弧型対から 36,603 種の相違成分集合を作り、全てを横切る最小集合が 98 文字であることを二値整数計画で確定した。この 98 出現回数と相対端点 10 成分により 9,739 弧型は 8,969 記述へ分かれ、両支持の係数衝突は零になった。LLM による検証では、有限データ上の最小性は得たが 98 文字の列挙は一般の閉じた係数式ではないと判断した。次は選ばれた 98 文字を、文字をなす局所所属の述語族へまとめる。

## 根 −1−s の所属のレビューと表記確認（2026-09-18 の 315 回目）

LLM による検証で本文・Lean 具体版・必要十分版・導出版・SageMath・後続の利用箇所を照合した。根を二次体の台集合へ持ち上げ、後続の表示と正錐判定から反復参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無く、根を $(-1)+(-1)\cdot s$ へ運ぶ計算も一続き・一行一操作・適用箇所の根拠へ統一済みなので書き換えなかった。二つの平方根と有理表示 98 組に対するプログラミングによる検証は通過した。次のレビューと表記確認は「根 −1−s の表示」から続ける。

## 内部語の文字出現回数で二つの軌道係数を決定（2026-09-18 の 314 回目）

前段の相対端点 10 成分に、内部語の長さ $k$ の巡回窓の多重集合を加え、二つの正準巡回支持の係数をともに決める最小の共通窓長を有限全データで判定した。プログラミングによる検証では、内部語を捨てると端点だけの支持で 238 種、語位置・端点の支持で 305 種の係数衝突が残る一方、$k=1$、すなわち各文字の出現回数を加えると 9,739 弧型は 9,735 記述へ分かれ、両支持の衝突が零になった。LLM による検証では、この候補族の最小窓長は 1 だが、文字種ごとの出現回数を全て残しているため、まだ閉じた最小係数式ではないと判断した。次は各文字の出現回数を係数決定に必要な最小の成分へ縮約する。

## 根 −1+s の表示のレビューと表記確認（2026-09-18 の 314 回目）

LLM による検証で本文・Lean 具体版・必要十分版・導出版・SageMath・後続の利用箇所を照合した。二次体の表示を一意に確定し、直後の正錐判定から反復参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無く、表示の写像と表示の一意性を適用する一行も適用箇所の根拠へ統一済みなので書き換えなかった。二つの平方根と有理表示 98 組に対するプログラミングによる検証は通過した。次のレビューと表記確認は「根 −1−s の所属」から続ける。

## 相対端点 10 成分だけのブール式の反証（2026-09-18 の 313 回目）

前段で選んだ相対端点 10 成分だけで二つの正準巡回支持の軌道係数を決められるかを、同じ 9,739 弧型の有限全データで判定した。各弧型について実現可能な境界入射候補が与える 10 ビット列を全て残しても記述は 601 種に縮まり、端点だけの支持では 238 種、語位置・端点の支持では 305 種が係数 0 と 1 の両方を含んだ。プログラミングによる検証で各衝突の反例を保存した。LLM による検証では、候補集合全体は 10 成分から作る任意の集約より多い情報を持つため、この衝突により 10 成分だけの全ブール式が有限範囲で反証され、内部語の情報は必要だと判断した。次は内部語から係数を分ける最小の巡回不変量を判定する。

## 根 −1+s の所属のレビューと表記確認（2026-09-18 の 313 回目）

LLM による検証で本文・Lean 具体版・必要十分版・導出版・SageMath・後続の利用箇所を照合した。根を二次体の台集合へ持ち上げ、後続の表示と正錐判定から反復参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無く、根を $(-1)+1\cdot s$ へ運ぶ計算も一続き・一行一操作・適用箇所の根拠へ統一済みなので書き換えなかった。二つの平方根と有理表示 98 組に対するプログラミングによる検証は通過した。次のレビューと表記確認は「根 −1+s の表示」から続ける。

## 相対端点局所量の最小数（2026-09-18 の 312 回目）

二つの正準巡回支持の係数対が異なる 2,640 弧型対について、各切断候補の境界方向を基準に、整列端点と反対端点の所属を境界・時計回り・反対・反時計回りへ並べ直した。プログラミングによる検証では 1,037 種の相違成分集合を得て、両係数を保つには相対端点 32 成分のうち少なくとも 10 成分が必要であることを二値整数計画で確定した。選んだ 10 成分により 9,739 弧型は 9,735 記述へ圧縮され、両支持とも係数衝突は 0 件だった。LLM による検証では、絶対方向に依存した前段の 18 成分を相対 10 成分へ縮約できたが、記述数はほぼ弧型数のままなので閉じた係数式ではないと判断した。次はこの 10 成分のブール式として二つの係数を表せるかを判定する。

## 臨界点は Fisher 零点でない主張のレビューと表記確認（2026-09-18 の 312 回目）

LLM による検証で本文・Lean 具体版・必要十分版・導出版・SageMath・後続の利用箇所を照合した。臨界点での分配多項式の値が正錐に属することから有限格子の零点でないことを確定し、後続の先頭零点距離の非零性から参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無く、背理法の仮定から零評価・零表示・正錐三条件の否定へ至る計算も一続き・一行一操作・適用箇所の根拠へ統一済みなので書き換えなかった。6 組のプログラミングによる検証は通過した。次のレビューと表記確認は「根 −1+s の所属」から続ける。

## 両軌道係数を保つ端点局所成分の最小数（2026-09-18 の 311 回目）

巡回・反転で正規化した内部語が同じで、二つの正準支持の係数対が異なる弧型対を全て作り、各対を区別する端点局所成分の最小集合を有限全データで判定した。

プログラミングによる検証では、2,640 対から 1,605 種の相違成分集合を得て、これらを全て横切るには端点32成分のうち少なくとも18成分が必要であることを二値整数計画で確定した。選んだ18成分により9,739弧型は9,660記述へ圧縮され、両支持とも係数衝突は0件だった。LLMによる検証では、端点情報は減ったが絶対方向と端点順に依存するため、まだ任意の内部語に対する閉じた係数式ではないと判断した。次は18成分を端点方向に沿う相対局所量へまとめ、両係数を保つ候補式を判定する。

## 分配多項式の臨界点値のレビューと表記確認（2026-09-18 の 311 回目）

LLMによる検証で本文・Lean具体版・必要十分版・導出版・SageMath・後続の利用箇所を照合した。有限和が二次体と正錐に留まり、臨界点が有限格子のFisher零点でないことへ反復参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無く、正の初項を確保して有限部分和を帰納する計算は一続き・一行一操作・適用箇所の根拠へ統一済みなので書き換えなかった。6組のプログラミングによる検証は通過した。次のレビューと表記確認は「臨界点はFisher零点でない」から続ける。

## 内部語と完全端点署名は二つの軌道係数を決める（2026-09-18 の 310 回目）

巡回・反転で正規化した内部語と、二端点の完全な局所署名との組が、二つの正準巡回支持の軌道係数を決めるかを有限全データで判定した。

プログラミングによる検証では、9,739 弧型から 9,739 個の異なる組が得られ、端点だけの支持と語位置・端点の支持のどちらでも係数衝突は 0 件だった。LLM による検証では、完全端点署名を残せばこの有限範囲の係数は決まる一方、弧型数と鍵数が同数なのでまだ閉じた係数式ではないと判断した。次は端点署名の各成分を一つずつ落とし、両係数を保つ最小の端点局所量を判定する。

## 正錐の元の自然数倍のレビューと表記確認（2026-09-18 の 310 回目）

LLM による検証で本文・Lean 具体版・必要十分版・導出版・SageMath・後続の利用箇所を照合した。自然数倍が二次体に留まり、零倍と正の倍を分けて正錐所属を確定し、後続の臨界点値の有限和から反復参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無く、場合分けと各式変形は一続き・一行一操作・適用箇所の根拠へ統一済みなので書き換えなかった。48 組のプログラミングによる検証は通過した。次のレビューと表記確認は「分配多項式の臨界点での値は正錐の元である」から続ける。

## 内部語だけでは二つの軌道係数を決められない（2026-09-18 の 309 回目）

二つの正準巡回支持の軌道係数が、内部語の巡回・反転軌道だけの関数になるかを有限全データで判定した。

プログラミングによる検証では、9,739 弧型を 5,421 内部語軌道へ射影し、同じ内部語で係数が 0 と 1 に分かれる軌道が、端点だけの支持で 192、語位置・端点の支持で 257 あることを確定し、各反例を保存した。LLM による検証では、この有限範囲では内部語から作るどの巡回不変量も係数を決められず、端点データが必要であると判断した。次は内部語と端点局所量の組が各係数を決めるかを判定する。

## 正錐の元の冪のレビューと表記確認（2026-09-18 の 309 回目）

LLM による検証で本文・Lean 具体版・必要十分版・導出版・SageMath・後続の利用箇所を照合した。冪が二次体と正錐に留まることを確定し、分配多項式の臨界点値で反復参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無く、出発点と帰納段は一続きの式変形・一行一操作・適用箇所の根拠へ統一済みなので書き換えなかった。72 帰納法ステップのプログラミングによる検証は通過した。次のレビューと表記確認は「正錐の元の自然数倍は零元または正錐の元である」から続ける。

## 二つの正準巡回支持の係数式（2026-09-18 の 308 回目）

二つの正準巡回支持の切断前係数を $\mathbb F_2$ 上で加えた対称差について、任意の内部語の軌道ごとに、二つの軌道係数の和を実現可能な最小切断だけへ戻す係数式を本文へ追加した。同じ論理手順の Lean 具体版・必要十分版・導出版を配線した。

プログラミングによる検証では、保存済みの 462 軌道と 616 軌道が 227 軌道を同じ切断位置で共有し、そこが $1+1=0$ で相殺して対称差に 624 軌道が残ること、その支持が係数式の復元と一致することを厳密検査した。LLM による検証では、切断位置の表引きは消えたが、二つの軌道係数自体はまだ保存済み支持で与えていると判断した。次はこの二つの軌道係数を、内部語の巡回不変な局所量で書けるか判定する。

## 正の有理数は正錐の元である主張のレビューと表記確認（2026-09-18 の 308 回目）

LLM による検証で本文・Lean 具体版・必要十分版からの導出・SageMath・後続の利用箇所を照合した。有理数の二次体と正錐への所属を確定し、後続の冪と自然数倍から反復参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無く、表示 $(q,0)$ への鎖と正錐の第一条件は一続きの式変形と行末の根拠へ統一済みなので書き換えなかった。二つの平方根と正の有理数 6 個・非正 4 個に対するプログラミングによる検証は通過した。次のレビューと表記確認は「正錐の元の冪は正錐の元である」から続ける。

## 全六研究監督: 支持係数の閉じた式への接続を維持（2026-09-18 03:00 JST）

実行識別子 `34e9a830-e636-4e40-a398-ba140f7bf332`。LLMによる検証では、開始成果以後に語長依存の
衝突分離から境界入射、二次元座標復元、実現可能候補の選択へ進み、巡回軌道係数と最小切断から
切断前の支持係数を一意に再構成する一般命題まで四層化された。候補が二つとも実現可能でも最小選択で
復元できるため、本質は候補の一意性でなく軌道係数の保存と正準選択にある。段取りは妥当で、次は任意の
内部語に対する閉じた係数式への合成である。03:00:48のプログラミングによる観測では登録あり・
非実行中・runs 158・last exit 0、実ログの02:54:08正常終了を確認した。

## 実現可能な最小切断による支持係数の再構成（2026-09-18 の 307 回目）

各巡回・反転軌道で、実現可能候補集合の最小元以外の切断係数が零なら、候補係数の $\mathbb F_2$ 和である軌道係数を最小候補へ戻すことで、切断前の全係数が一意に復元される一般命題を本文へ追加した。同じ論理手順の Lean 具体版・必要十分版・導出版を配線した。

プログラミングによる検証では、二つの正準巡回支持のうち二候補が方向上連結する 489 項について、座標上実現可能な候補を再計算し、保存済み係数が「軌道係数を実現可能な最小候補へだけ戻す」再構成と全て一致した。LLM による検証では、実現可能候補の一意性を仮定せず、両候補が実現可能な 162 項も同じ規則で復元できると判断した。次は再構成した二支持の係数を、任意の内部語に対する閉じた係数式へ合成する。

## 正錐と乗法の両立のレビューと表記確認（2026-09-18 の 307 回目）

LLM による検証で本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath・後続の利用箇所を照合した。九つの符号条件の組を尽くして積の正錐所属を確定し、後続の自然数倍と有限和から反復参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無かった。表示の転送は一続きの四段の式変形で、九場合の根拠も適用箇所から参照されているため書き換えなかった。有理数の表示 69,696 組に対するプログラミングによる検証は通過した。次のレビューと表記確認は「正の有理数は正錐の元である」から続ける。

## 保存済み切断位置は実現可能候補集合の最小元と一致する（2026-09-18 の 306 回目）

二つの正準巡回支持のうち、方向上連結する二候補を持つ 489 項について、座標上の実現可能性と保存済み切断位置を再計算した。候補 0 だけが実現可能なものは 174 項、候補 1 だけは 153 項、両候補とも実現可能なものは 162 項だった。従って「実現可能候補は一つだけ」という仮説は成り立たない。一方、保存済み切断位置は全 489 項で実現可能であり、実現可能候補集合の最小元と一致した。

重い有限合同系を再構成して得た上記の集計を `certificate.json` へ保存し、その証拠だけを読む検査入口 `verify-certificate.sage` を追加した。重い再計算と保存証拠だけの検査はいずれも終了コード 0 で通過し、後者の実測は 3.34 秒だった。次は一意性を仮定せず、本文の「復元候補集合の最小元を選ぶ」規則を二つの正準巡回支持へ適用して、支持係数を再構成する一般命題へ進む。

## 正錐と加法の両立のレビューと表記確認（2026-09-18 の 306 回目）

LLM による検証で本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath・後続の利用箇所を照合した。九つの符号条件の組を尽くして和の正錐所属を確定し、後続の有限和の帰納法から参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無かった。表示の転送は一続きの三段の式変形で、九場合の各根拠も適用箇所から参照されているため書き換えなかった。有理数の表示 69,696 組に対するプログラミングによる検証は通過した。次のレビューと表記確認は「正錐と乗法の両立」から続ける。

## 支持係数再構成の前提検査は未完了（2026-09-18 の 305 回目）

二つの正準巡回支持について、復元した実現可能候補が保存済みの切断位置を一意に返すかを全支持で再計算する診断を開始した。既存の検算ファイルを読み込む経路も、保存済み証拠を読む経路も、依存する巨大な有限合同系を先に再構成する実装になっており、後者も実測で 1 分を超えて計算が継続した。01:40 のまとめ締切までに四層を完成できないため、数学命題と検算ファイルへの着手は行わず、リポジトリ限定の安全な終了手順で診断プロセスを停止した。

次は、`parity-identity-simple-cycle-arc-orientation-length-four-cyclic-relation-arc-lifts/certificate.json` の保存済み支持と、切断候補の実現可能性判定に必要な構成だけを、有限合同系の再構成なしで読み出せる検査入口へ分離する。その入口で二支持の各軌道に実現可能候補が一つだけあり、保存済み切断位置と一致することを確定してから、支持係数の再構成命題を本文・SageMath・Lean 二版へ上げる。

## 二次体の積の表示のレビューと表記確認（2026-09-18 の 305 回目）

LLM による検証で本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath・後続の利用箇所を照合した。積の係数対を一意に確定し、正錐と乗法の両立から反復参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無かった。積の係数対へ至る十四段の鎖を前段の閉性から引用し、表示の一意性へ渡す二段は一続きの式変形・一行一操作・適用箇所の根拠へ統一済みなので書き換えなかった。乗法の両立 49 組と補助鎖・乗法鎖・積表示 4,802 組に対するプログラミングによる検証は通過した。次のレビューと表記確認は「正錐の加法による閉性」から続ける。

## 復元切断旗による実現可能候補の選択（2026-09-18 の 304 回目）

二つの切断候補について、内部語の合同条件から復元した始点が生成する反対端点切断旗への所属を候補ごとに判定し、幾何的に実現可能な候補集合と復元候補集合が一致する一般命題を本文へ追加した。復元候補集合が非空なら最小候補が実現可能であることも示し、Lean 具体版・必要十分版・導出版を配線した。

プログラミングによる検証では、既存の 1,036 境界入射を再計算し、651 入射を残して実現不能な 385 入射を除く判定と一致した。LLM による検証では、二候補の濾過は観測済み支持の表引きではなく、任意の有限内部語に対する復元完全性から従うと判断した。次はこの選択規則を二つの正準巡回支持の固定関係へ適用し、実現可能な候補だけから支持係数を再構成する一般命題へ進む。

## 二次体の台集合の乗法による閉性のレビューと表記確認（2026-09-18 の 304 回目）

LLM による検証で本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath・後続の利用箇所を照合した。積が二次体の台集合に留まることを確定し、直後の積の表示と正錐の乗法閉性から反復参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無かった。三本の補助鎖と十四段の乗法鎖は一続きの式変形・一行一操作・適用箇所の根拠へ統一済みなので書き換えなかった。乗法の両立 49 組と補助鎖・乗法鎖・積表示 4,802 組に対するプログラミングによる検証は通過した。次のレビューと表記確認は「二次体の積の表示」から続ける。

## 二次元座標列と境界延長候補の完全性（2026-09-18 の 303 回目）

行と列の累積変位・切断旗へ一座標の始点復元命題を独立に適用し、整合する二次元座標列が復元始点集合の直積から得る列に一致し、一辺延長後の切断旗候補もその像に一致することを本文へ追加した。同じ論理手順の Lean 具体版・必要十分版・導出版を配線した。

プログラミングによる検証では、辺長二から四・語長一から四の 13,920 合同系、13,920 座標直積、27,840 境界延長、1,036 境界入射が全て一致した。LLM による検証では、始点全列挙に依存しない二次元復元の完全性が一般に閉じていると判断した。次はこの完全性を正準巡回支持の二つの切断候補へ適用し、実現可能な候補を選ぶ一般規則を本文命題へ上げる。

## 二次体の和の表示のレビューと表記確認（2026-09-18 の 303 回目）

LLM による検証で本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath・後続の利用箇所を照合した。和の係数対を一意に確定し、正錐の加法閉性から反復参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無かった。和の係数対へ至る七段の鎖と表示写像への帰結は一続きの式変形・一行一操作・適用箇所の根拠へ統一済みなので書き換えなかった。和の両立 49 組と和の鎖・表示 4,802 組に対するプログラミングによる検証は通過した。次のレビューと表記確認は「二次体の台集合の乗法による閉性」から続ける。

## 切断旗の一般始点復元命題（2026-09-17 の 302 回目）

辺長、累積変位、各位置の零側・末尾側の切断旗に対し、正の旗が始点剰余類を強制し、負の旗が剰余類を除外することを有限集合の等式として本文へ追加した。同じ論理手順の Lean 具体版・必要十分版・必要十分版からの導出を配線し、既存の辺長二から四・語長一から四の 13,920 合同系、27,840 境界延長、1,036 境界入射の SageMath 検算を新しい本文ラベルへ対応させた。

LLM による検証: 一座標の始点復元は一般に閉じた。次は行・列への独立適用から二次元座標列と境界延長候補の完全性を本文命題へ上げる。

## 二次体の台集合の加法による閉性のレビューと表記確認（2026-09-17 の 302 回目）

LLM による検証で本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath・後続の利用箇所を照合した。和が二次体の台集合に留まることを確定し、直後の表示写像と正錐の加法閉性から参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無かった。和を二次体の標準表示へ運ぶ七段の鎖と台集合への所属は一続きの式変形・一行一操作・適用箇所の根拠へ統一済みなので書き換えなかった。和の両立 49 組と和の鎖・表示 4,802 組に対するプログラミングによる検証は通過した。次のレビューと表記確認は「二次体の和の表示」から続ける。

## 切断旗の合同条件による始点復元（2026-09-17 の 301 回目）

内部方向列の行・列の累積変位から、各切断旗を始点剰余類の等式または除外条件へ変換し、行と列で独立に解いて始点座標を復元する規則へ書き換えた。辺長二から四・語長一から四の 13,920 合同系で始点全列挙との一致を確認し、27,840 件の境界延長と既存 1,036 境界入射の一致も維持した。

プログラミングによる検証は終了コード 0。LLM による検証: 始点の全列挙は構成から除けたが、一般の辺長と語長についての本文命題と Lean 二版は未着手である。次は合同条件による始点復元を一般命題として本文へ上げる。

## 二次体の三分律（高々一つ）のレビューと表記確認（2026-09-17 の 301 回目）

LLM による検証で本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath・後続の利用箇所を照合した。正錐・零元・加法逆元の正錐所属が二つ同時に成り立たないことを九つの場合で確定し、後続の順序定義から参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無かった。場合分けは分離され、零元の加法逆元の鎖と平方比較の矛盾は一行一操作・適用箇所の根拠へ統一済みなので書き換えなかった。高々一つの判定 1,458 組、零表示 3 件、書き直し 729 組、排他 6,561 組に対するプログラミングによる検証は通過した。次のレビューと表記確認は「二次体の台集合の加法による閉性」から続ける。

## 有限内部語の一般境界延長規則（2026-09-17 の 300 回目）

辺長、空でない有限内部語、連結向きを入力とし、最初のトーラス頂点を列挙して内部出方向に沿う座標列の切断旗を照合した後、境界を一辺延長して反対端点の切断旗を返す規則を定式化した。辺長二から四・語長一から四の 13,920 本の座標路、27,840 件の境界延長、13,234 種の辺長付き内部語で規則の完全性を有限検算し、既存の一辺二・三の 1,036 境界入射の判定とも完全に一致した。

プログラミングによる検証は終了コード 0。LLM による検証: 語長や観測済み支持に依存しない有限規則へ移したが、始点を全列挙する形なので一般証明の命題にはまだなっていない。次は始点列挙を方向列と切断旗の合同条件へ書き換える。

## 二次体の三分律（少なくとも一つ）のレビューと表記確認（2026-09-17 の 300 回目）

LLM による検証で本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath・後続の利用箇所を照合した。表示係数の四場合分けから正錐・零元・加法逆元の正錐所属の少なくとも一つを確定し、後続の排他性と正錐の閉性から反復参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無かった。場合分けは分離され、混合符号の場合の平方比較は一続きの式変形・一行一操作・行末の根拠へ統一済みなので書き換えなかった。少なくとも一つの判定 1,458 組、場合分け 729 組、加法逆元表示 1,458 組、混合符号 338 組に対するプログラミングによる検証は通過した。次のレビューと表記確認は「二次体の三分律（高々一つ）」から続ける。

## 実現可能な境界入射による切断候補の分離（2026-09-17 の 299 回目）

二候補とも方向上連結する 489 項について、座標上実現可能な境界入射だけで候補記述を作り直した。1,036 入射から実現不能な 385 入射を除いた 651 入射は、端点ビットを使わない 94 種の記述だけで選択順位の衝突が零になった。

プログラミングによる検証は終了コード 0。LLM による検証: 前段の五つの切断旗と一つの選択辺所属は、方向上だけ連結していて座標上は実現不能な入射を混ぜたため必要に見えていた。これは一辺二・三の観測済み支持上の有限判定であり、一般語長の規則ではない。次は実現可能入射による有限選択を一般の内部語へ運べる規則として定式化する。

## 有理数の平方は二倍の平方にならない定理のレビューと表記確認（2026-09-17 の 299 回目）

LLM による検証で本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath・後続の利用箇所を照合した。有理数の平方が二にならない定理を比に適用する非存在定理で、三分律と正錐の乗法閉性から反復参照されるため、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無かった。比の平方を二へ運ぶ六段の鎖と非存在定理による矛盾は一続きの式変形・一行一操作・行末の根拠へ統一済みなので書き換えなかった。640,800 組に対するプログラミングによる検証は通過した。次のレビューと表記確認は「二次体の三分律（少なくとも一つ）」から続ける。

## 切断線幾何で実現可能な境界入射の切り分け（2026-09-17 の 298 回目）

六ビット最小集合を内部語と切断線の幾何へ引き戻すため、方向上連結する 1,036 入射を一辺二・三のトーラス座標へ持ち上げた。385 入射は切断旗と内部接続方向を同時に満たす座標列が無く、既存の方向上の連結判定には幾何的に実現不能な入射が含まれる。651 入射は実現可能で、そのうち 78 入射は二通りの反対端点切断旗候補を持つが、誤った端点だけを返す入射は零だった。

プログラミングによる検証は終了コード 0。LLM による検証: 六ビットを既存の全入射へ直接読み替えることはできず、まず幾何的に実現可能な入射へ絞る必要がある。次はその入射だけで選択衝突と必要な端点ビットを再計算する。

## 二次体の加法逆元の表示のレビューと表記確認（2026-09-17 の 298 回目）

LLM による検証で本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath・後続の利用箇所を照合した。表示写像の加法逆元での値を確定し、三分律から反復参照されるため、空の四則主張として削除してはならない。記号の濫用、未証明の付加主張、四層の不一致は無かった。加法逆元の表示を求める鎖と表示写像への帰結は一続きの式変形・一行一操作・行末の根拠へ統一済みなので書き換えなかった。加法逆元 19 組、零元表示 722 組、表示の鎖 722 組に対するプログラミングによる検証は通過した。次のレビューと表記確認は「有理数の平方は二倍の平方にならない（混合符号の排除）」から続ける。

## 境界方向外の端点ビットの最小集合（2026-09-17 の 297 回目）

二候補とも連結する 489 項で、境界入射記述が同じなのに候補順位が反対となる 839 組を比較した。境界方向外の二重辺・単一辺・選択辺所属だけでは 35 組を区別できず、切断旗が必要だった。二端点の全二十四ビットを戻した 393 記述では衝突が零になった。

衝突を零にする最小集合は一意で、整合端点の時計回り・反時計回りの切断旗、反対端点の時計回り・反対・反時計回りの切断旗、反対端点の反時計回りの選択辺所属からなる六ビットだった。プログラミングによる検証は終了コード 0。LLM による検証: 観測済み支持上の有限選択条件であり、一般語長の規則ではない。次は六ビットの条件を内部語と切断線の幾何から導く。

## 二次体の台集合の加法逆元による閉性のレビューと表記確認（2026-09-17 の 297 回目）

LLM による検証で本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath・後続の利用箇所を照合した。加法逆元後も二次体の台集合に留まることを確定し、三分律から反復参照されるため、空の四則主張として削除してはならない。記号の濫用、未証明の付加主張、四層の不一致は無かった。加法逆元の表示と台集合への所属は一続きの式変形・一行一操作・行末の根拠へ統一済みなので書き換えなかった。加法逆元 19 組、零元表示 722 組、表示の鎖 722 組に対するプログラミングによる検証は通過した。次のレビューと表記確認は「二次体の加法逆元の表示」から続ける。

## 連結する二候補の境界方向と端点所属（2026-09-17 の 296 回目）

二候補とも連結する 489 項で、連結向きの最初の入方向または最後の出方向を、整合する端点の同方向へ引き戻した。観測した 1,036 個の境界入射は全て単一辺に属し二重辺には属さなかった。一方、選択辺所属と切断旗の四通りは、実際に選ばれた候補と選ばれなかった候補の双方に現れた。

境界入射だけを残すと 117 種の記述のうち 20 種が相反する候補順位を要求した。二端点の全所属を戻した 393 種の記述には衝突が無かった。プログラミングによる検証は終了コード 0。LLM による検証: 選択辺所属または切断旗の一ビットだけでは切断位置を選べず、境界方向以外の端点所属が必要である。次は 20 個の衝突を分ける端点所属ビットを切り分ける。

## 二次体の零元の表示による特徴づけのレビューと表記確認（2026-09-17 の 296 回目）

LLM による検証で本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath・後続の利用箇所を照合した。零元と係数対 $(0,0)$ の同値を確定し、三分律など後続から反復参照されるため、空の四則主張として削除してはならない。記号の濫用、未証明の付加主張、四層の不一致は無かった。二方向の証明は一続きの式変形・一行一操作・行末の根拠へ統一済みなので書き換えなかった。加法逆元 19 組、零元表示 722 組、表示の鎖 722 組に対するプログラミングによる検証は通過した。次のレビューと表記確認は「二次体の台集合の加法逆元による閉性」から続ける。

## 内部語の連結条件による切断候補と境界奇偶（2026-09-16 の 295 回目）

内部語の各位置で二本の単一辺を使い切り、直前の出方向の反対向きから次位置へ入る連結条件を定めた。観測済み支持 781 項の端点境界候補 1,562 個のうち 292 個は連結不能で、残る一候補は全て実際の切断位置だった。候補順の連結可否二ビットと端点ビットを合わせた入力に相反する選択は無い。全ての連結した向き 1,429 通りで、四方向奇偶は最初の入方向・最後の出方向・内部接続辺の反対方向対から復元できた。

プログラミングによる検証は終了コード 0。LLM による検証: 前段の語長三指示と四方向奇偶を内部語の連結構造へ戻した。ただし二候補とも連結する 489 項の一般選択規則は未構成である。次は境界の入出方向と二端点の非圧縮な単一辺・二重辺・選択辺所属を対応づける。

## 二次体の台集合への零元の所属のレビューと表記確認（2026-09-16 の 295 回目）

LLM による検証で本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath・後続の零元定義を照合した。零元が二次体の台集合に属することと表示の well-defined 性を確定する主張なので、空の四則主張として削除してはならない。記号の濫用、未証明の付加主張、四層の不一致は無かった。三段の鎖は一続きの式変形・一行一操作・行末の根拠へ統一済みなので書き換えなかった。加法逆元 19 組、零元表示 722 組、表示の鎖 722 組に対するプログラミングによる検証は通過した。次のレビューと表記確認は「二次体の零元の表示による特徴づけ」から続ける。

## 内部語の向き奇偶による切断衝突の分離（2026-09-16 の 294 回目）

端点ビットで衝突する語長三と語長五の内部語について、各語位置が持つ四つの向きビットを座標ごとに排他的論理和した。語長三側は二語とも $(1,1,1,1)$、語長五側は $(0,1,1,0)$ と $(0,1,0,1)$ だった。「四座標が全て奇数」という一ビットは語順の巡回移動と反転で不変であり、端点情報と合わせると観測済み支持 781 項の入力 611 種に衝突は無かった。

プログラミングによる検証は終了コード 0。LLM による検証: 語長三という外形的な指示を巡回語の構造から導く有限述語へ置き換えた。ただし一般語長で切断位置を選ぶ閉じた規則ではない。次はこの奇偶と弧の端点境界・選択位置の関係を内部語の連結条件から導く。

## 根の取り替えと正錐のレビューと表記確認（2026-09-16 の 294 回目）

LLM による検証で「根の取り替えと正錐」の本文・Lean の台集合等式・SageMath・後続の利用箇所を照合した。選んだ平方根を反転したときの台集合、表示係数、正錐の対応を明示し、後続で正の根を選ぶ際の入力を分離する説明なので、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無かった。このブロックは注意で独立した証明の式変形を含まないため、表記の書き換えも行わなかった。対応 SageMath の正値性 361 組、根の取り替え 722 組、表示の一意性 1,200 組に対するプログラミングによる検証は通過した。次の本文レビューと式変形の表記統一は「二次体の台集合への零元の所属」から続ける。

## 語長三の指示ビットによる端点ビット衝突の分離（2026-09-16 の 293 回目）

端点ビットが同じなのに候補順位が衝突する二つの証拠は、どちらも語長三と語長五が反対の順位を要求する組だった。両者はともに奇数なので語長の奇偶では分離できない。一方、端点ビットへ「語長が三である」という一ビットを加えると、観測済み支持 781 項から得る入力 612 種の衝突は零になった。

プログラミングによる検証は終了コード 0。LLM による検証: 任意の選択規則は少なくとも語長三と語長五を区別する必要があり、語長三の指示ビットは観測済み支持上では十分である。ただしこれは有限表の分離であり、一般語長の閉じた規則ではない。次は衝突証拠の語長三と語長五の内部語を比較し、この一ビットを巡回語の構造から導く述語を探す。

## 二次体の正錐のレビューと表記確認（2026-09-16 の 293 回目）

LLM による検証で「二次体の正錐」の本文・Lean 具体版・SageMath・後続の利用箇所を照合した。有理係数対への三つの条件で正順序を定め、選んだ平方根の所属と後続の閉包性・三分律から反復参照される定義なので、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無かった。このブロックは定義で式変形を含まないため、表記の書き換えも行わなかった。対応 SageMath の三条件と正値性 361 組、根の取り替え 722 組、表示の一意性 1,200 組に対するプログラミングによる検証は通過した。次の本文レビューと式変形の表記統一は「根の取り替えと正錐」から続ける。

## 端点ビット述語の反例（2026-09-16 の 292 回目）

語長二以上の正準弧型支持 781 項について、二端点の全所属・切断旗と二候補の反転正規化前の向き・境界位置・端点順を固定長 51 ビットへ展開した。候補対のビット列 607 種のうち 2 種が同じ入力に順位 0 と順位 1 の両方を要求した。従って端点ビットだけから切断候補を選ぶ述語は存在しない。語長を入力へ戻すと 614 種に衝突は無かった。

プログラミングによる検証は終了コード 0。LLM による検証: 観測済み支持上でも端点だけでは不十分であり、一般語長の規則には語長に依存する追加情報が必要である。次は二つの衝突証拠を比較し、必要な語長述語を切り分ける。

## 二次体の表示の写像のレビューと表記確認（2026-09-16 の 292 回目）

LLM による検証で「二次体の表示の写像」の本文・Lean 具体版・SageMath・後続の利用箇所を照合した。二次体の元と有理係数対を同一視せず、一意な表示を well-defined な写像として取り出して後続から反復参照する定義なので、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無かった。このブロックは定義で式変形を含まないため、表記の書き換えも行わなかった。次の本文レビューと式変形の表記統一は「二次体の正錐」から続ける。

## 有向切断候補の端点局所選択写像（2026-09-16 の 291 回目）

語長二以上の正準弧型支持 781 項について、二候補を区別した有向記述から内部語を捨てた。反転正規化前の向き、端点が語の先頭・末尾のどちらに合うか、端点の全所属・切断旗・端点順からなる 614 個の局所記述では、同じ記述が要求する候補順位に衝突が無く、全支持項の切断位置を復元できた。

プログラミングによる検証は終了コード 0。LLM による検証: 観測済み支持上の有限選択写像であり、一般語長の閉じた式ではない。次は 614 個の有限表を端点ビットの述語へ圧縮する。

## 二次体の台集合のレビューと表記確認（2026-09-16 の 291 回目）

LLM による検証で「二次体の台集合」の本文・Lean 定義・SageMath・後続の利用箇所を照合した。有理係数二つと二の平方根による表示を持つ代数的数の集合を定め、可算側の閉包先として後続から反復参照される定義なので、空の四則主張ではない。記号の濫用、未証明の付加主張、四層の不一致は無かった。このブロックは定義で式変形を含まないため、表記の書き換えも行わなかった。次の本文レビューと式変形の表記統一は「二次体の表示の写像」から続ける。

## 反転正規化前の向きによる切断候補の分離（2026-09-16 の 290 回目）

巡回・反転軌道代表から二つの切断候補を復号するとき、巡回移動後の内部語とその反転のどちらが辞書式最小かを一ビットで記録した。前段で端点の未圧縮な選択集合所属まで一致した 162 項は、この反転正規化前の向きによって全て分かれた。二つの正準支持で共有される軌道をまとめた 620 件の有限要件にも、相反する選択は無かった。

プログラミングによる検証は 781 支持項で終了コード 0。LLM による検証: この一ビットは反転同一視で失われる有向情報を正確に補うが、有限支持上で候補を区別しただけであり、一般語長で実際の切断位置を選ぶ閉じた規則は未構成である。次は有向記述から実際の切断を選ぶ規則を抽出する。

## 二次体の表示の一意性のレビューと表記確認（2026-09-16 の 290 回目）

LLM による検証で「二次体の表示の一意性」の本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath・後続の利用箇所を照合した。一と $s$ の一次独立性から係数対の一意性を導き、正錐の well-defined 性と後続の係数計算から反復参照される実質的な主張なので、空の四則主張ではない。未証明の付加主張と四層の不一致は無く、式変形も一続きの鎖と行末の根拠に統一済みだった。対応 SageMath の表示不一致 4,704 組、等式鎖 4,802 組、同一表示 98 組に対するプログラミングによる検証は通過した。次の本文レビューと式変形の表記統一は「二次体の台集合」から続ける。

## 非圧縮端点所属による切断候補識別の反例（2026-09-16 の 289 回目）

語長二以上の正準弧型支持 781 項について、巡回・反転軌道代表を二つの端点境界候補で復号し、整合する端点の選択集合所属を未圧縮の四方向ビットとして付加した。二候補の有限記述が同一になる項が 162 項あった。残る 619 項でも実際の切断位置は辞書順最小 281 項・最大 338 項に分かれたため、この情報だけでは一般語長の選択規則にならない。

プログラミングによる検証では、二つの支持で重複する軌道をまとめた 620 件の有限要件に矛盾が無いことも確かめ、証明書へ固定した。LLM による検証: 端点の選択集合所属を圧縮前へ戻すだけでは反転同一視で失った向きを復元できない。次は反転同一視の前の有向境界情報を一つ加え、同値な 162 候補対を先に分離する。

## 一と s の一次独立性のレビューと表記確認（2026-09-16 の 289 回目）

LLM による検証で「一と s の一次独立性」の本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath・後続の利用箇所を照合した。有理数の平方が二にならないことから $s$ の有理性を排除する実質的な主張で、「二次体の表示の一意性」から参照されるため、空の四則主張ではない。未証明の付加主張と四層の不一致は無く、式変形も一続きの鎖と行末の根拠に統一済みだった。対応 SageMath の 4,416 非零係数組に対するプログラミングによる検証は通過した。次の本文レビューと式変形の表記統一は「二次体の表示の一意性」から続ける。

## 正準弧型支持の切断位置の整合性（2026-09-16 の 288 回目）

二つの正準弧型支持の和集合 851 軌道と共通部分 227 軌道を比較し、共通部分で切断位置が全て一致することを確定した。従って観測済みの支持上では、支持の種類に依存しない有限選択写像として切断位置を扱える。ただし、語長一の 297 項は端点境界候補が一つであるのに対し、語長二以上の 781 項は全て二候補を持つ。端点が語の境界に来るという条件だけでは一般語長の選択規則にならない。

プログラミングによる検証では、各支持項の切断位置が端点境界候補に属すること、語長ごとの候補数、共有軌道の一致、全選択写像のハッシュを固定した。LLM による検証: 有限集合上の整合性は得られたが、一般語長へ運ぶには二候補を分ける追加条件が必要である。次は端点の非圧縮所属と軌道代表からその条件を抽出する。

## 有理数の平方の非二性のレビューと表記確認（2026-09-16 の 288 回目）

LLM による検証で「有理数の平方は二にならない」の本文・Lean 二版・SageMath・利用箇所を照合した。素数二の指数の奇偶による実質的な主張で後続から反復参照されるため、空の四則主張ではない。四層の不一致と未証明の付加主張は無かった。式変形も一続きの鏖と行末の根拠に統一済みだったため、書き換えは行わなかった。次の本文レビューと式変形の表記統一は「一と s の一次独立性」から続ける。

## 正準弧型支持の巡回軌道符号化（2026-09-16 の 287 回目）

端点対を固定したまま弧の内部語を巡回移動し、既存の反転同一視を施した軌道の最小代表と、元の切断位置へ戻す最小巡回移動量で二つの正準弧型支持を符号化した。端点特徴代表の支持 462 項は 462 軌道、語位置・端点積代表の支持 616 項は 616 軌道へ単射に写り、両支持が共有する軌道は 227 個だった。

プログラミングによる検証では全項を軌道代表と切断位置から復号し、支持との完全一致、軌道への射影の単射性、語長別の軌道サイズと切断位置分布を確かめた。LLM による検証: 軌道代表と切断位置による符号化は任意の空でない有限語へ同じ式で運べる。一方、切断位置は語長と端点特徴だけでは一意に決まらないため、一般語長の関係式には端点署名と軌道代表から切断位置を選ぶ追加規則が必要である。次はその選択規則を抽出する。

## 自己双対方程式の二根の表記統一（2026-09-16）

「自己双対方程式の因数分解と根の全体」の残りを LLM により検証し、今回分割した因数分解と二根の導出を含め、一続きの式変形・一行一操作・行末の根拠へ統一され、本文・SageMath・Lean 二版の中間式が一致していることを確認した。次の本文レビューと式変形の表記統一は「有理数の平方は二にならない」から続ける。

## 自己双対方程式の二根のレビュー（2026-09-16 の 287 回目）

LLM による検証で「自己双対方程式の因数分解と根の全体」の本文・Lean 二版・SageMath・利用箇所を照合した。因数分解と二根の網羅を確定し、正の自己双対根の一意性から参照されるため、空の四則主張ではない。本文と Lean 二版に残っていた複数操作を同じ行へまとめた箇所を一ステップ一操作へ分け、SageMath の対応も同じ中間式へ揃えた。次の本文レビューは「有理数の平方は二にならない」から続ける。次の式変形の表記統一は「自己双対方程式の因数分解と根の全体」の残りから続ける。

## 全六研究監督: 混合関係の弧型支持圧縮を次対象として維持（2026-09-16 08:55 JST）

実行識別子 `8dc58408-74c1-4443-b24a-23d5f3782a7e`。LLMによる検証では、許容一次・二次特徴の
有限候補全体で系は可解だが非一意であり、巡回不変な特徴だけの固定線型形式は零、弧型を含む
混合関係商は339次元だった。二つの疎な特徴代表を弧型側へ持ち上げられたため、一般化可能な構造は
特徴単独でなく混合関係にある。次は弧型支持を巡回軌道で圧縮する現行段取りを維持する。
08:55:05のプログラミングによる観測ではlaunchd登録あり・非実行中・runs 79・last exit 0、
実ログは08:39:57正常終了と成果 `8aab421b` の包含を記録した。

## 語長四の疎な巡回関係の弧型係数側（2026-09-16 の 286 回目）

語長四の端点二次代表と語位置・端点積代表について、元の有限合同系の行結合を解き、特徴部分を保った巡回不変な混合関係へ持ち上げた。弧型だけの固定関係は 4,876 次元である。この空間の既約階段基底により弧型係数を標準化すると、端点二軌道代表は生の弧型支持重み 774 から 462、四語位置へ巡回する語位置・端点積代表は 870 から 616 へ縮約され、どちらの固定値も 1 になった。

プログラミングによる検証では、元の関係空間への所属、巡回型係数の消滅、特徴部分の完全一致、弧型だけの関係基底のピボット座標の消滅、保存済み解での固定値を $\mathbb F_2$ 上で確かめ、証明書へ保存した。LLM による検証: 二代表の弧型側を具体化した有限結果であり、支持 462・616 項の巡回軌道構造、一般の辺長・全語長の命題、Lean 二版は未追加である。次は弧型支持を巡回軌道で圧縮し、一般語長へ運べる規則を抽出する。

## 臨界点の非零点性のレビュー（2026-09-16 の 286 回目）

- LLM による検証で「臨界点は Fisher 零点でない」の本文、Lean 具体版、必要十分版、必要十分版からの導出、SageMath、後続の利用箇所を照合した。臨界点での評価値が正錐に属し、零元の表示が正錐の三条件を全て破ることから有限格子で零点にならない主張は実質を持ち、後続の先頭距離の正値性から参照されるため、空の四則主張ではない。
- 本文には、このブロックからは従わない「零点は臨界点へ詰め寄る」という文があった。有限格子で臨界点と一致しないことと、零点列が臨界点へ近づくことは別の主張なので、この未証明の文だけを削除した。Lean と SageMath の主張は元から有限格子での非零点性だけであり、変更は不要だった。
- プログラミングによる検証: `critical-point-not-fisher-zero/check.sage` は二つの平方根と一辺一から三の六組で通過した。

## 語長四の巡回不変な混合関係の疎な代表（2026-09-16 の 285 回目）

語長四の巡回不変な混合関係商から、端点特徴だけに支持を持つ 144 次元と語位置・端点積だけに支持を持つ 115 次元の基底を取り出した。既約階段基底を、基底性を保ち Hamming 重みを真に減らす行加算で決定的に疎化した。端点だけでは総重みが 663 から 521 へ、語位置・端点積だけでは 334 から 316 へ減った。

疎化基底内の最小代表は、端点だけでは `end0_c_down*end0_wrap_col0 + end0_c_right*end0_wrap_col0` の二軌道、語位置・端点積だけでは `step0_wrap_col0*end0_wrap_col0` の一軌道だった。後者は四つの語位置へ巡回移送した四特徴の和を表す。各基底の階数、元の 339 次元商空間への持ち上げ、全基底行の所属を有限体上で検査し、証明書を保存した。

LLM による検証: ここでの最小は決定的に疎化した基底行の中での最小であり、部分空間全体の最小距離ではない。弧型係数側、一般の辺長・全語長の命題、Lean 二版は未追加である。次はこの二代表を混合関係へ持ち上げ、弧型係数側を復元する。

## 臨界点での分配多項式値のレビューと二の平方根の表記確認（2026-09-16 の 285 回目）

LLM による検証で「分配多項式の臨界点での値は正錐の元である」の本文・Lean 二版・SageMath・利用箇所を照合した。正の初項と、零元または正錐の元である後続項の有限和が正錐に留まることを確定し、臨界点が有限格子の Fisher 零点でないことから参照されるため、空の四則主張には該当しない。修正点は無く、対応 SageMath の六組に対するプログラミングによる検証は通過した。

「二の平方根の存在」の二次係数四段と根の評価十一段も LLM により照合し、すでに一続きの式変形と行末の根拠へ統一され、本文・SageMath・Lean 二版の段数と順序が一致しているため書き換えなかった。次の表記確認は「自己双対方程式の因数分解と根の全体」。

## 語長四の巡回不変な混合関係の支持分類（2026-09-16 の 284 回目）

語長四の巡回不変な混合関係商 339 次元を、3,240 特徴の 1,212 巡回軌道座標上で支持構造により分類した。一次特徴の射影は零だった。二特徴積だけに支持を持つ部分空間は、同じ語位置 9 次元・隣接語位置 16 次元・反対語位置 14 次元・語位置と端点 115 次元・端点どうし 144 次元である。語位置間隔三種を合わせると 42 次元なので 3 次元は間隔を混ぜ、全 339 次元のうち 38 次元は語位置だけ・端点だけ・語位置端点積の三群をまたぐ。証明書の保存を含むプログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 一般語長へ運びやすい支持群は分離できたが、最小支持の代表、一般の辺長・全語長の命題、Lean 二版は未追加である。次は端点だけの 144 次元と語位置・端点積だけの 115 次元から最小支持の代表を抽出する。

## 正錐の自然数倍のレビューと自己双対条件の表記統一（2026-09-16 の 284 回目）

LLM による検証で「正錐の元の自然数倍は零元または正錐の元である」の本文・Lean 二版・SageMath・利用箇所を照合した。二次体への所属と零倍・正の倍数の場合分けを確定し、後続から反復参照されるため空の四則主張には該当しない。修正点は無く、対応 SageMath の 48 組に対するプログラミングによる検証は通過した。

「自己双対条件は二次方程式と同値」の複数操作をまとめた式変形を一ステップ一操作へ分け、SageMath と Lean 二版も同じ鎖へ揃えた。次の本文レビューは「分配多項式の臨界点での値は正錐の元である」から、次の式変形の表記統一は「二の平方根の存在」から続ける。

## 語長四の弧型係数を含む巡回不変関係の判定（2026-09-16 の 283 回目）

語長四の一次特徴と全二特徴積を許した可解な有限合同系で、自由な弧型係数と特徴係数だけに台を持つ固定線型関係を分類した。弧型 $7{,}609$ 係数と特徴 $3{,}240$ 係数に台を制限した関係は $6{,}434$ 次元、弧型係数だけでは $4{,}876$ 次元、語位置の巡回移動で不変な関係は $5{,}215$ 次元であり、弧型だけの関係を除いた商は $339$ 次元だった。証明書の保存を含むプログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 特徴だけでは零だった巡回核が弧型との混合で現れることを確定したが、$339$ 次元の支持の類別、一般の辺長・全語長の命題、Lean 二版は未追加である。次はこの商空間を支持の構造で類別し、一般の語長へ運べる候補を抽出する。

## 正錐の冪閉性のレビューと双対変換の対合性の表記統一（2026-09-16 の 283 回目）

LLM による検証で「正錐の元の冪は正錐の元である」の本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath・利用箇所を照合した。自然数帰納法で正錐の乗法閉性を冪へ持ち上げ、後続の臨界点での分配多項式評価から反復参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の二つの平方根と正錐三条件を覆う四表示、72 帰納段に対するプログラミングによる検証は通過した。

「双対変換の対合性」では、積の結合則と可換則を一行で同時適用していた変形を、結合則と可換則の二段へ分けた。LLM による検証で逆元の使用条件、二つの補助等式、零因子が無いことによる結論、全参照を保持したと判断した。

次の本文レビューは「正錐の元の自然数倍は零元または正錐の元である」から続ける。次の式変形の表記統一は「自己双対条件は二次方程式と同値」から続ける。

## 語長四の巡回不変な固定線型形式の判定（2026-09-16 の 282 回目）

語長四の一次特徴と全二特徴積を許した可解な有限合同系で、特徴だけからなる固定線型形式の空間を左核条件から全て求めると 491 次元だった。3,240 特徴を語位置四箇所の巡回移動で 1,212 軌道へ分け、軌道和空間との共通部分を取ると零次元だった。固定形式空間とその巡回移送を順に交差させた次元も 491、6、0、0 であり、全移送が固定される非零の核は無い。証明書の保存を含むプログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 特徴係数だけの巡回的な核は棄却できたが、弧型係数を混ぜた共変関係、一般の辺長・全語長の命題、Lean 二版は未追加である。次は弧型係数も含む固定線型関係を巡回移動で類別する。

## 正の有理数の正錐所属のレビューと双対変換の表記統一（2026-09-16 の 282 回目）

LLM による検証で「正の有理数は正錐の元である」の本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath・利用箇所を照合した。有理数を係数対 $(q,0)$ で二次体へ埋めて正錐への所属と住処を確定し、後続の冪と自然数倍から反復参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の符号二通り、正の有理数六個と非正の有理数四個に対するプログラミングによる検証は通過した。

「双対変換は零と一の間の有理数を零と一の間の有理数へ写す」では、値の有理数性・正値性・上端との比較を隔てていた散文を除き、三つの計算を根拠が行末にある一つの整列数式へまとめた。LLM による検証で逆元の定義、値の住処、二つの狭義不等式、全参照を保持したと判断した。

次の本文レビューは「二次体の台集合の乗法による閉性」から続ける。次の式変形の表記統一は「双対変換は対合である」から続ける。

## 語長四の解空間で固定される特徴係数の判定（2026-09-16 の 281 回目）

語長四の一次特徴と全二特徴積を許した可解な有限合同系の被約行階段形から、全解で固定される座標を取り出した。係数行列は 11,208 列・階数 6,793、解空間の次元は 4,415 で、3,240 個の特徴係数に固定座標は無かった。従って行階段形が選んだ 248 項は列順に依存する一代表であり、個々の係数から語位置の巡回移動に沿う非零の核は抽出できない。プログラミングによる検証は終了コード 0。

LLM による検証: 有限候補クラス内の係数ごとの核は棄却できたが、固定される線形結合、一般の辺長・全語長の命題、Lean 二版は未追加である。次は解空間上で固定される線形結合を語位置の巡回移動で類別する。

## 正錐と乗法の両立のレビュー（2026-09-16 の 281 回目）

LLM による検証で「正錐と乗法の両立」の本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath・利用箇所を照合した。九つの符号条件の組を尽くして後続から反復参照されるため、空の四則主張には該当しない。転送計算で乗法の可換則と加法の可換則を一つの等号へ同時適用していたため二段へ分け、Lean と SageMath も同じ鎖へ揃えた。対応 SageMath の九場合 69,696 組に対するプログラミングによる検証は通過した。

次の本文レビューは「正の有理数は正錐の元である」から続ける。次の式変形の表記統一は「双対変換は零と一の間の有理数を零と一の間の有理数へ写す」から続ける。

## 自己双対点のセクター対応の表記統一（2026-09-16）

「自己双対点では高温側と低温側のセクター変数が一致する」で、自己双対性の参照を式変形前の散文から実際に適用する等号の行末へ移した。LLM による検証で自己双対点の非零性、セクター双対関係への代入、有限系だけという射程を保持したと判断した。

## 語長四の一次・二次特徴候補クラス全体の有限判定（2026-09-16 の 280 回目）

語長四の完全署名をなす 80 個の一次特徴と同じ特徴どうしを含む全二特徴積を合わせた 3,240 項を列挙し、先行二十項以外の 3,220 項を全 7,085 鍵の合同系へ一括投入した。係数行列と拡大行列の階数はいずれも 6,793 で可解であり、行階段形が選ぶ代表は一次 10 項・二次 238 項を持つ。元の合同系への再代入を含むプログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 二次項 238 項のうち 237 項が先頭語位置の特徴を一方の因子に持つが、これは列順に依存する代表の観測であり、一般の語長の公式とはみなさない。候補クラス全体の可解性は確定した一方、一般の辺長・全語長の命題と Lean 二版は未追加である。次は解空間で固定される係数を分離し、語位置の移動に沿う一般化可能な核があるかを判定する。

## 二次体の積の表示のレビューとセクター双対関係の表記統一（2026-09-16）

LLM による検証で「二次体の積の表示」の本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath・利用箇所を照合した。積の表示の二係数を一意に確定し、後続の正錐の乗法閉性から再利用されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の和と積の両立 49 組、補助等式・積の等式鎖・表示 4,802 組に対するプログラミングによる検証は通過した。

「代数的数の点における高温側と低温側のセクター双対関係」では、積の可換則と結合則を一行で同時適用していた変形を、結合・可換・結合の三段へ分けた。LLM による検証で双対変換の代入、逆元の使用条件、有限和への合成、全参照を保持したと判断した。

次の本文レビューは「正錐と乗法の両立」から続ける。次の式変形の表記統一は「自己双対点では高温側と低温側のセクター変数が一致する」から続ける。

## 十九特徴追加後に残る次の語長四障害（2026-09-16 の 279 回目）

位置ビット閉式へ先行十九障害を除く特徴を加えた有限合同系から、36 鍵を台に持つ次の左核矛盾証拠を取り出した。36 行を足すと自由未知数の 7,968 弧型と先行十九特徴は相殺し、固定済みの語長四弧型 30 種だけが奇数回残る。候補値の和は一、鍵の右辺の和は零である。プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 一次特徴と二特徴積を全数比較した 1,140 個の相殺しない特徴のうち、第二語位置の反対側の列切断旗 `step1_wrap_collast` が最初の一次特徴として今回の証拠との積を一にする。ただしこの列も加えた全 7,085 鍵の系は階数 6,322・拡大階数 6,323 で非可解なので、二十特徴でも閉式には足りず、さらに独立な障害が残る。一般命題と Lean 二版は未追加である。次は監督方針に従い、許容した一次特徴・二特徴積の残りを一括投入して候補クラス全体を有限判定する。

## 二次体の乗法閉性のレビューと正の自己双対根の一意性の表記統一（2026-09-16）

LLM による検証で「二次体の台集合の乗法による閉性」の本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath・利用箇所を照合した。積の表示係数を証人として積が二次体の台集合に留まることを確定し、後続の積の表示・正錐の乗法閉性・正錐の元の自然数倍から反復参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の和と積の両立 49 組、補助等式・積の等式鎖・表示 4,802 組に対するプログラミングによる検証は通過した。

続けて「自己双対方程式の正の根は −1+s に限る」で、二根への場合分けと負の根の場合の正錐所属・非所属の矛盾を、散文と箇条書きから各根拠を行末に持つ整列数式へ移した。LLM による検証で二根の網羅、正錐所属の仮定、負の根の非所属、全参照を保持したと判断した。

次の本文レビューは「二次体の積の表示」から続ける。次の式変形の表記統一は「代数的数の点における高温側と低温側のセクター双対関係」から続ける。

## 全六研究監督: 語長四の候補クラス全体を有限判定する（2026-09-16 03:01 JST）

実行識別子 `6b9a5fbc-a636-4b5b-9cd3-2098874b867d`。LLMによる検証では、読解終点
`d84683d8a35a20348ed2b2c7549f9ed958cf1eec` までに第十三から第十八までの特徴が別々の左核障害を
除いた一方、各追加後も係数行列と拡大行列の階数差一が残った。03時までにremote defaultへ入った
第十九特徴でも同じ形が続いたため、一特徴ずつの反復には候補クラス全体の成否を決める有限な終点がない。
次は許容した一次特徴・二特徴積の残りを一括投入し、可解なら正準代表を抽出、非可解なら左核証拠で
候補クラスを棄却する段取りへ本文・runbook・stateを同期した。03:01:06のプログラミングによる観測では
launchd登録あり・非実行中・runs 70・last exit 0、実ログは02:56:05正常終了と成果包含を記録した。

## 十八特徴追加後に残る次の語長四障害（2026-09-16 の 278 回目）

位置ビット閉式へ先行十八障害を除く特徴を加えた有限合同系から、26 鍵を台に持つ次の左核矛盾証拠を取り出した。26 行を足すと自由未知数の 7,968 弧型と先行十八特徴は相殺し、固定済みの語長四弧型 26 種だけが奇数回残る。候補値の和は零、鍵の右辺の和は一である。プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 一次特徴と二特徴積を全数比較した 1,149 個の相殺しない特徴のうち、第二語位置の反対側の行切断旗 `step1_wrap_rowlast` が最初の一次特徴として今回の証拠との積を一にする。ただしこの列も加えた全 7,085 鍵の系は階数 6,321・拡大階数 6,322 で非可解なので、十九特徴でも閉式には足りず、さらに独立な障害が残る。一般命題と Lean 二版は未追加である。次は十九特徴を加えた系から残る左核証拠を取り出し、相殺しない語長四特徴を比較する。

## 二次体の和の表示のレビューと根 −1−s の非正値性の表記統一（2026-09-16）

LLM による検証で「二次体の和の表示」の本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath・利用箇所を照合した。二次体の元の和に対する表示係数を確定し、後続の正錐の加法閉性から反復参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の和の両立 49 組と和の等式鎖・表示 4,802 組に対するプログラミングによる検証は通過した。

続けて「根 −1−s は正でない」で、表示係数の決定と正錐の三条件の否定を、散文と箇条書きから根拠を行末に持つ一つの整列数式へ移した。LLM による検証で表示、三条件の否定、正錐の定義、全参照を保持したと判断した。

次の本文レビューは「二次体の台集合の乗法による閉性」から続ける。次の式変形の表記統一は「自己双対方程式の正の根は −1+s に限る」から続ける。

## 十七特徴追加後に残る次の語長四障害（2026-09-16 の 277 回目）

位置ビット閉式へ先行十七障害を除く特徴を加えた有限合同系から、24 鍵を台に持つ次の左核矛盾証拠を取り出した。24 行を足すと自由未知数の 7,968 弧型と先行十七特徴は相殺し、固定済みの語長四弧型 24 種だけが奇数回残る。候補値の和は零、鍵の右辺の和は一である。プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 一次特徴と二特徴積を全数比較した 1,137 個の相殺しない特徴のうち、第二語位置の下辺所属 `step1_e_down` が最初の一次特徴として今回の証拠との積を一にする。ただしこの列も加えた全 7,085 鍵の系は階数 6,320・拡大階数 6,321 で非可解なので、十八特徴でも閉式には足りず、さらに独立な障害が残る。一般命題と Lean 二版は未追加である。次は十八特徴を加えた系から残る左核証拠を取り出し、相殺しない語長四特徴を比較する。

## 二次体の加法閉性のレビューと根 −1+s の正値性の表記統一（2026-09-16）

LLM による検証で「二次体の台集合の加法による閉性」の本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath・利用箇所を照合した。有理係数の和を証人として台集合への所属を確定し、後続の和の表示と正錐の加法閉性から反復参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の和の両立 49 組と和の等式鎖・表示 4,802 組に対するプログラミングによる検証は通過した。

続けて「根 −1+s は正である」で、表示係数の決定、正錐の第三条件をなす三つの有理数不等式、正錐への所属を、散文と二つの計算から根拠を行末に持つ一つの整列数式へ移した。LLM による検証で表示、三条件、正錐の定義、全参照を保持したと判断した。

次の本文レビューは「二次体の和の表示」から続ける。次の式変形の表記統一は「根 −1−s は正でない」から続ける。

## 十六特徴追加後に残る次の語長四障害（2026-09-16 の 276 回目）

位置ビット閉式へ先行十六障害を除く特徴を加えた有限合同系から、20 鍵を台に持つ次の左核矛盾証拠を取り出した。20 行を足すと自由未知数の 7,968 弧型と先行十六特徴は相殺し、固定済みの語長四弧型 20 種だけが奇数回残る。候補値の和は一、鍵の右辺の和は零である。プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 一次特徴と二特徴積を全数比較した 1,089 個の相殺しない特徴のうち、第二語位置の左辺所属 `step1_e_left` が最初の一次特徴として今回の証拠との積を一にする。ただしこの列も加えた全 7,085 鍵の系は階数 6,319・拡大階数 6,320 で非可解なので、十七特徴でも閉式には足りず、さらに独立な障害が残る。一般命題と Lean 二版は未追加である。次は十七特徴を加えた系から残る左核証拠を取り出し、相殺しない語長四特徴を比較する。

## 二次体の三分律の排他のレビューと根 −1−s の表示の表記統一（2026-09-16）

LLM による検証で「二次体の三分律（高々一つ）」の本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath・利用箇所を照合した。零との排他と正錐・加法逆元側の正錐の九通りの排他を有理数の順序だけで示し、三分律の相互排他性を与える非自明な主張なので、空の四則主張には該当しない。修正点は無かった。対応 SageMath の高々一つの判定 1,458 組、零表示 3 件、書き直し 729 組、排他 6,561 組に対するプログラミングによる検証は通過した。

続けて「根 −1−s の表示」で、表示写像の一意性による係数対の決定を散文から根拠を行末に持つ整列数式へ移した。LLM による検証で根の表示、表示写像の仕様、一意性、全参照を保持したと判断した。

次の本文レビューは「二次体の台集合の加法による閉性」から続ける。次の式変形の表記統一は「根 −1+s は正である」から続ける。

## 十五特徴追加後に残る次の語長四障害（2026-09-16 の 275 回目）

位置ビット閉式へ先行十五障害を除く特徴を加えた有限合同系から、36 鍵を台に持つ次の左核矛盾証拠を取り出した。36 行を足すと自由未知数の 7,968 弧型と先行十五特徴は相殺し、固定済みの語長四弧型 26 種だけが奇数回残る。候補値の和は一、鍵の右辺の和は零である。プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 一次特徴と二特徴積を全数比較した 1,016 個の相殺しない特徴のうち、第二語位置のこちら側の行切断旗 `step1_wrap_row0` が最初の一次特徴として今回の証拠との積を一にする。ただしこの列も加えた全 7,085 鍵の系は階数 6,318・拡大階数 6,319 で非可解なので、十六特徴でも閉式には足りず、さらに独立な障害が残る。一般命題と Lean 二版は未追加である。次は十六特徴を加えた系から残る左核証拠を取り出し、相殺しない語長四特徴を比較する。

## 二次体の三分律のレビューと根 −1−s の所属の表記統一（2026-09-16）

LLM による検証で「二次体の三分律（少なくとも一つ）」の本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath を照合した。正錐・零元・加法逆元の正錐所属を有理係数の四場合分けで覆い、後続の順序構造から反復参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の少なくとも一つの判定 1,458 組、場合分け 729 組、加法逆元表示 1,458 組、混合符号 338 組に対するプログラミングによる検証は通過した。

続けて「根 −1−s の所属」で、係数の有理数体・代数的数体への所属と二次体への所属を、散文から各根拠を行末に持つ整列数式へ移した。LLM による検証で根の表示、係数の住処、二次体の定義、全参照を保持したと判断した。

次の本文レビューは「二次体の三分律（高々一つ）」から続ける。次の式変形の表記統一は「根 −1−s の表示」から続ける。

## 全六研究監督: 十三特徴後の残存障害を次対象として維持（2026-09-15 21:32 JST）

実行識別子 `a32da8e9-de9d-4b5b-91ae-b654d01aa00f`。LLMによる検証では、開始成果
`c4ff950329a8dfd9869e54819684f3d12b9d2bf9` から、語長四の独立障害を十三特徴まで切り分けたことを
研究上の前進とした。十二特徴後の37鍵左核証拠には上側の倍化辺所属が相殺しないが、それを加えた全系も
階数6315・拡大階数6316で非可解であり、一般閉式とLean二版は未到達である。次は十三特徴後に残る
左核証拠と相殺しない語長四特徴の比較で、段取り変更はない。20:59:27のプログラミングによる観測では
launchd登録あり・非実行中・runs 56・last exit 0、実ログは20:53:26正常終了を記録した。

## 十四特徴追加後に残る次の語長四障害（2026-09-15 の 274 回目）

位置ビット閉式へ先行十四障害を除く交差所属積 `step0_e_left*step0_d_up`、上辺・下辺・左辺所属 `step0_e_up`・`step0_e_down`・`step0_e_left`、列切断旗 `step0_wrap_col0`・`step0_wrap_collast`・`step1_wrap_col0`、四側での倍化辺所属 `step0_d_left`・`step0_d_right`・`step0_d_down`・`step0_d_up`、両側の行切断旗 `step0_wrap_rowlast`・`step0_wrap_row0`、第二語位置の上辺所属 `step1_e_up` を加えた有限合同系から、15 鍵を台に持つ次の左核矛盾証拠を取り出した。15 行を足すと自由未知数の 7,968 弧型と先行十四特徴は相殺し、固定済みの語長四弧型 18 種だけが奇数回残る。候補値の和は一、鍵の右辺の和は零である。プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 一次特徴と二特徴積を全数比較した 845 個の相殺しない特徴のうち、第二語位置の上側での倍化辺所属 `step1_d_up` が最初の一次特徴として今回の証拠との積を一にする。ただしこの列も加えた全 7,085 鍵の系は階数 6,317・拡大階数 6,318 で非可解なので、十五特徴でも閉式には足りず、さらに独立な障害が残る。一般命題と Lean 二版は未追加である。次は十五特徴を加えた系から残る左核証拠を取り出し、相殺しない語長四特徴を比較する。

## 有理平方と二倍平方の不一致のレビューと根 −1+s の表示の表記統一（2026-09-15）

LLM による検証で「有理数の平方は二倍の平方にならない（混合符号の排除）」の本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath を照合した。平方根二の有理数不存在へ還元して等しい場合を排除し、後続の三分律と混合符号積から反復参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の主張と等式鎖 640,800 組に対するプログラミングによる検証は通過した。

続けて「根 −1+s の表示」で、表示写像の一意性による係数対の決定を散文から根拠を行末に持つ整列数式へ移した。LLM による検証で根の表示、表示写像の仕様、一意性、全参照を保持したと判断した。

次の本文レビューは「二次体の三分律（少なくとも一つ）」から続ける。次の式変形の表記統一は「根 −1−s の所属」から続ける。

## 十三特徴追加後に残る次の語長四障害（2026-09-15 の 273 回目）

位置ビット閉式へ先行十三障害を除く交差所属積 `step0_e_left*step0_d_up`、上辺・下辺・左辺所属 `step0_e_up`・`step0_e_down`・`step0_e_left`、列切断旗 `step0_wrap_col0`・`step0_wrap_collast`・`step1_wrap_col0`、四側での倍化辺所属 `step0_d_left`・`step0_d_right`・`step0_d_down`・`step0_d_up`、両側の行切断旗 `step0_wrap_rowlast`・`step0_wrap_row0` を加えた有限合同系から、36 鍵を台に持つ次の左核矛盾証拠を取り出した。36 行を足すと自由未知数の 7,968 弧型と先行十三特徴は相殺し、固定済みの語長四弧型 28 種だけが奇数回残る。候補値の和は零、鍵の右辺の和は一である。プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 一次特徴と二特徴積を全数比較した 1,051 個の相殺しない特徴のうち、第二語位置の上辺所属 `step1_e_up` が最初の一次特徴として今回の証拠との積を一にする。ただしこの列も加えた全 7,085 鍵の系は階数 6,316・拡大階数 6,317 で非可解なので、十四特徴でも閉式には足りず、さらに独立な障害が残る。一般命題と Lean 二版は未追加である。次は十四特徴を加えた系から残る左核証拠を取り出し、相殺しない語長四特徴を比較する。

## 二次体の加法逆元表示のレビューと根 −1+s の所属の表記統一（2026-09-15）

LLM による検証で「二次体の加法逆元の表示」の本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath を照合した。係数表示を加法逆元の下で一意に追跡し、後続の三分律から反復参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の加法逆元の両立 19 組、零元の特徴づけ 722 組、加法逆元の表示 722 組に対するプログラミングによる検証は通過した。

続けて「根 −1+s の所属」で、係数対の有理数体への所属と二次体への所属を、散文から各根拠を行末に持つ整列数式へ移した。LLM による検証で根の表示、係数の住処、二次体の定義、全参照を保持したと判断した。

次の本文レビューは「有理数の平方は二倍の平方にならない（混合符号の排除）」から続ける。次の式変形の表記統一は「根 −1+s の表示」から続ける。

## 十二特徴追加後に残る次の語長四障害（2026-09-15 の 272 回目）

位置ビット閉式へ先行十二障害を除く交差所属積 `step0_e_left*step0_d_up`、上辺・下辺・左辺所属 `step0_e_up`・`step0_e_down`・`step0_e_left`、列切断旗 `step0_wrap_col0`・`step0_wrap_collast`・`step1_wrap_col0`、左右下側での倍化辺所属 `step0_d_left`・`step0_d_right`・`step0_d_down`、両側の行切断旗 `step0_wrap_rowlast`・`step0_wrap_row0` を加えた有限合同系から、37 鍵を台に持つ次の左核矛盾証拠を取り出した。37 行を足すと自由未知数の 7,968 弧型と先行十二特徴は相殺し、固定済みの語長四弧型 26 種だけが奇数回残る。候補値の和は零、鍵の右辺の和は一である。プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 一次特徴と二特徴積を全数比較した 921 個の相殺しない特徴のうち、先頭語位置の上側での倍化辺所属 `step0_d_up` が最初の一次特徴として今回の証拠との積を一にする。ただしこの列も加えた全 7,085 鍵の系は階数 6,315・拡大階数 6,316 で非可解なので、十三特徴でも閉式には足りず、さらに独立な障害が残る。一般命題と Lean 二版は未追加である。次は十三特徴を加えた系から残る左核証拠を取り出し、相殺しない語長四特徴を比較する。

## 二次体の加法逆元閉性のレビューと臨界点の非零点性の表記統一（2026-09-15）

LLM による検証で「二次体の台集合の加法逆元による閉性」の本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath を照合した。加法逆元が二次体の台集合に留まることを確定し、後続の三分律から反復参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の加法逆元の両立 19 組、零元の特徴づけ 722 組、加法逆元の表示 722 組に対するプログラミングによる検証は通過した。

続けて「臨界点は Fisher 零点でない」で、背理法の仮定から零評価・零表示へ至る含意と正錐三条件の否定を、散文と箇条書きから各根拠を行末に持つ整列数式へ移した。LLM による検証で正錐所属との矛盾、Fisher 零点の定義、全参照を保持したと判断した。

次の本文レビューは「二次体の加法逆元の表示」から続ける。次の式変形の表記統一は「根 −1+s の所属」から続ける。

## 十一特徴追加後に残る次の語長四障害（2026-09-15 の 271 回目）

位置ビット閉式へ先行十一障害を除く交差所属積 `step0_e_left*step0_d_up`、上辺・下辺・左辺所属 `step0_e_up`・`step0_e_down`・`step0_e_left`、列切断旗 `step0_wrap_col0`・`step0_wrap_collast`・`step1_wrap_col0`、左右での倍化辺所属 `step0_d_left`・`step0_d_right`、両側の行切断旗 `step0_wrap_rowlast`・`step0_wrap_row0` を加えた有限合同系から、35 鍵を台に持つ次の左核矛盾証拠を取り出した。35 行を足すと自由未知数の 7,968 弧型と先行十一特徴は相殺し、固定済みの語長四弧型 26 種だけが奇数回残る。候補値の和は一、鍵の右辺の和は零である。プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 一次特徴と二特徴積を全数比較した 1,067 個の相殺しない特徴のうち、先頭語位置の下側での倍化辺所属 `step0_d_down` が最初の一次特徴として今回の証拠との積を一にする。ただしこの列も加えた全 7,085 鍵の系は階数 6,314・拡大階数 6,315 で非可解なので、十二特徴でも閉式には足りず、さらに独立な障害が残る。一般命題と Lean 二版は未追加である。次は十二特徴を加えた系から残る左核証拠を取り出し、相殺しない語長四特徴を比較する。

## 二次体の零元表示のレビューと臨界点での値の表記統一（2026-09-15）

LLM による検証で「二次体の零元の表示による特徴づけ」の本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath を照合した。零元と表示係数対 $(0,0)$ の同値を与え、後続の三分律などから反復参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の零判定 722 組と等式鎖 722 組に対するプログラミングによる検証は通過した。

続けて「分配多項式の臨界点での値は正錐の元である」で、正の初項の確保と帰納法の出発点を、散文から各根拠を行末に持つ二つの整列数式へ移した。LLM による検証で定数配位、多重度、正錐の冪と自然数倍、有限和の帰納法、全参照を保持したと判断した。

次の本文レビューは「二次体の台集合の加法逆元による閉性」から続ける。次の式変形の表記統一は「臨界点は Fisher 零点でない」から続ける。

## 十特徴追加後に残る次の語長四障害（2026-09-15 の 270 回目）

位置ビット閉式へ先行十障害を除く交差所属積 `step0_e_left*step0_d_up`、上辺・下辺・左辺所属 `step0_e_up`・`step0_e_down`・`step0_e_left`、列切断旗 `step0_wrap_col0`・`step0_wrap_collast`・`step1_wrap_col0`、左側での倍化辺所属 `step0_d_left`、両側の行切断旗 `step0_wrap_rowlast`・`step0_wrap_row0` を加えた有限合同系から、35 鍵を台に持つ次の左核矛盾証拠を取り出した。35 行を足すと自由未知数の 7,968 弧型と先行十特徴は相殺し、固定済みの語長四弧型 26 種だけが奇数回残る。候補値の和は零、鍵の右辺の和は一である。プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 一次特徴と二特徴積を全数比較した 1,049 個の相殺しない特徴のうち、先頭語位置の右側での倍化辺所属 `step0_d_right` が最初の一次特徴として今回の証拠との積を一にする。ただしこの列も加えた全 7,085 鍵の系は階数 6,313・拡大階数 6,314 で非可解なので、十一特徴でも閉式には足りず、さらに独立な障害が残る。一般命題と Lean 二版は未追加である。次は十一特徴を加えた系から残る左核証拠を取り出し、相殺しない語長四特徴を比較する。

## 二次体の零元所属のレビューと正錐の自然数倍の表記統一（2026-09-15）

LLM による検証で「二次体の台集合への零元の所属」の本文・Lean 具体版・必要十分版・SageMath を照合した。有理係数の証人 $(0,0)$ により零元の所属先を確定し、可算側の閉包先を与えて後続から反復参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の符号反転 19 組、零元の特徴づけ 722 組、等式鎖 722 組に対するプログラミングによる検証は通過した。

続けて「正錐の元の自然数倍は零元または正錐の元である」で、零の場合の台集合への所属証人と正の場合の有理数・台集合・正錐への所属を、散文から各根拠を行末に持つ二つの整列数式へ移した。LLM による検証で自然数の場合分け、有理数の順序、二次体と正錐の乗法閉性、全参照を保持したと判断した。数学証明の記述規則に従い、一ステップごとの根拠と記号の所属先を数式内に明示した。

次の本文レビューは「二次体の零元の表示による特徴づけ」から続ける。次の式変形の表記統一は「分配多項式の臨界点での値は正錐の元である」から続ける。

## 九特徴追加後に残る次の語長四障害（2026-09-15 の 269 回目）

位置ビット閉式へ先行九障害を除く交差所属積 `step0_e_left*step0_d_up`、上辺・下辺・左辺所属 `step0_e_up`・`step0_e_down`・`step0_e_left`、列切断旗 `step0_wrap_col0`・`step0_wrap_collast`、左側での倍化辺所属 `step0_d_left`、両側の行切断旗 `step0_wrap_rowlast`・`step0_wrap_row0` を加えた有限合同系から、4 鍵を台に持つ次の左核矛盾証拠を取り出した。4 行を足すと自由未知数の 7,968 弧型と先行九特徴は相殺し、固定済みの語長四弧型 4 種だけが奇数回残る。候補値の和は一、鍵の右辺の和は零である。プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 一次特徴と二特徴積を全数比較した 338 個の相殺しない特徴のうち、第二語位置のこちら側の列切断旗 `step1_wrap_col0` が最初の一次特徴として今回の証拠との積を一にする。ただしこの列も加えた全 7,085 鍵の系は階数 6,312・拡大階数 6,313 で非可解なので、十特徴でも閉式には足りず、さらに独立な障害が残る。一般命題と Lean 二版は未追加である。次は十特徴を加えた系から残る左核証拠を取り出し、相殺しない語長四特徴を比較する。

## 根の取り替えと正錐のレビューと正錐の冪閉性の表記統一（2026-09-15）

LLM による検証で「根の取り替えと正錐」の本文・Lean・SageMath を照合した。二根が同じ台集合を与え、係数の符号反転で選んだ根を正とする正錐同士が対応することを述べ、根の選択が構造を特別扱いしない根拠なので、空の四則主張には該当しない。修正点は無かった。対応 SageMath の根の交換 722 組と Lean の台集合の等号に対するプログラミングによる検証は通過した。

続けて「正錐の元の冪は正錐の元である」で、帰納法の出発点と一歩における台集合・正錐への所属を、散文から各根拠を行末に持つ二つの整列数式へ移した。LLM による検証で自然数帰納法、代数的数の冪、台集合と正錐の乗法閉性、全参照を保持したと判断した。

次の本文レビューは「二次体の台集合への零元の所属」から続ける。次の式変形の表記統一は「正錐の元の自然数倍は零元または正錐の元である」から続ける。

## 八特徴追加後に残る次の語長四障害（2026-09-15 の 268 回目）

位置ビット閉式へ先行八障害を除く交差所属積 `step0_e_left*step0_d_up`、上辺・下辺・左辺所属 `step0_e_up`・`step0_e_down`・`step0_e_left`、列切断旗 `step0_wrap_col0`・`step0_wrap_collast`、左側での倍化辺所属 `step0_d_left`、反対側の行切断旗 `step0_wrap_rowlast` を加えた有限合同系から、9 鍵を台に持つ次の左核矛盾証拠を取り出した。9 行を足すと自由未知数の 7,968 弧型と先行八特徴は相殺し、固定済みの語長四弧型 6 種だけが奇数回残る。候補値の和は一、鍵の右辺の和は零である。プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 一次特徴と二特徴積を全数比較した 554 個の相殺しない特徴のうち、先頭語位置のこちら側の行切断旗 `step0_wrap_row0` が最初の一次特徴として今回の証拠との積を一にする。ただしこの列も加えた全 7,085 鍵の系は階数 6,311・拡大階数 6,312 で非可解なので、九特徴でも閉式には足りず、さらに独立な障害が残る。一般命題と Lean 二版は未追加である。次は九特徴を加えた系から残る左核証拠を取り出し、相殺しない語長四特徴を比較する。

## 二次体の正錐のレビューと正の有理数の正錐所属の表記統一（2026-09-15）

LLM による検証で「二次体の正錐」の本文・Lean 具体版・SageMath を照合した。有理係数の三条件だけで正値性を定め、後続から 49 回参照される順序構造の定義なので、空の四則主張には該当しない。修正点は無かった。対応 SageMath の三条件と正値性 361 組、根の交換 722 組、表示の一意性 1,200 組に対するプログラミングによる検証は通過した。

続けて「正の有理数は正錐の元である」で、有理数の二次体への所属、表示の一意性、正錐の第一条件を散文から各根拠を行末に持つ三つの整列数式へ移した。LLM による検証で有理数と代数的数の住処、正錐の三条件、全参照を保持したと判断した。

次の本文レビューは「根の取り替えと正錐」から続ける。次の式変形の表記統一は「正錐の元の冪は正錐の元である」から続ける。

## 七特徴追加後に残る次の語長四障害（2026-09-15 の 267 回目）

位置ビット閉式へ先行七障害を除く交差所属積 `step0_e_left*step0_d_up`、上辺・下辺・左辺所属 `step0_e_up`・`step0_e_down`・`step0_e_left`、列切断旗 `step0_wrap_col0`・`step0_wrap_collast`、左側での倍化辺所属 `step0_d_left` を加えた有限合同系から、34 鍵を台に持つ次の左核矛盾証拠を取り出した。34 行を足すと自由未知数の 7,968 弧型と先行七特徴は相殺し、固定済みの語長四弧型 24 種だけが奇数回残る。候補値の和は零、鍵の右辺の和は一である。プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 一次特徴と二特徴積を全数比較した 934 個の相殺しない特徴のうち、先頭語位置の反対側の行切断旗 `step0_wrap_rowlast` が最初の一次特徴として今回の証拠との積を一にする。ただしこの列も加えた全 7,085 鍵の系は階数 6,310・拡大階数 6,311 で非可解なので、八特徴でも閉式には足りず、さらに独立な障害が残る。一般命題と Lean 二版は未追加である。次は八特徴を加えた系から残る左核証拠を取り出し、相殺しない語長四特徴を比較する。

## 二次体表示写像のレビューと非負有理数の平方比較の表記統一（2026-09-15）

LLM による検証で「二次体の表示の写像」の本文・Lean 具体版・SageMath を照合した。二次体の各元に唯一の有理係数対を対応させ、元と係数対の同一視を避けて後続の正錐と演算表示から反復参照されているため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の表示の一意性の対偶 1,200 組に対するプログラミングによる検証は通過した。

続けて「非負有理数の平方の大小から大小」で、背理法の二段の不等式鎖と仮定をつなぐ矛盾を、散文中の不等式から各根拠を行末に持つ二つの整列数式へ移した。LLM による検証で非負元による乗法単調性、順序の推移律、狭義順序の非反射性、全参照を保持したと判断した。

次の本文レビューは「二次体の正錐」から続ける。次の式変形の表記統一は「二次体の台集合への正の有理数の所属」から続ける。

## 六特徴追加後に残る第七の語長四障害（2026-09-15 の 266 回目）

位置ビット閉式へ先行六障害を除く交差所属積 `step0_e_left*step0_d_up`、上辺・下辺・左辺所属 `step0_e_up`・`step0_e_down`・`step0_e_left`、列切断旗 `step0_wrap_col0`・`step0_wrap_collast` を加えた有限合同系から、29 鍵を台に持つ第七の左核矛盾証拠を取り出した。29 行を足すと自由未知数の 7,968 弧型と先行六特徴は相殺し、固定済みの語長四弧型 18 種だけが奇数回残る。候補値の和は一、鍵の右辺の和は零である。プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 一次特徴と二特徴積を全数比較した 823 個の相殺しない特徴のうち、先頭語位置の左側での倍化辺所属 `step0_d_left` が最初の一次特徴として第七証拠との積を一にする。ただしこの列も加えた全 7,085 鍵の系は階数 6,309・拡大階数 6,310 で非可解なので、七特徴でも閉式には足りず、さらに独立な障害が残る。一般命題と Lean 二版は未追加である。次は七特徴を加えた系から第八の左核証拠を取り出し、残る障害で相殺しない語長四特徴を比較する。

## 二次体の台集合のレビューと積の表示の表記統一（2026-09-15）

LLM による検証で「二次体の台集合」の本文・Lean 宣言・SageMath を照合した。有理係数二つと $s$ による表示を持つ代数的数の集合を定め、所属・可算側の閉包先を与えて後続から反復参照されるため、空の四則主張には該当しない。修正点は無かった。対応する構造化テキスト一括検査・Lean 全体と SageMath の二次体関連検査によるプログラミングによる検証は通過した。

続けて「二次体の積の表示」で、積の表示と表示写像の一意性による結論を、散文中の等式から各根拠を行末に持つ二つの整列数式へ移した。LLM による検証で係数の住処、積の所属、表示の一意性、全参照を保持したと判断した。

次の本文レビューは「二次体の表示の写像」から続ける。次の式変形の表記統一は「非負有理数の平方の大小から大小」から続ける。

## 五特徴追加後に残る第六の語長四障害（2026-09-15 の 265 回目）

位置ビット閉式へ先行五障害を除く交差所属積 `step0_e_left*step0_d_up`、上辺・下辺所属 `step0_e_up`・`step0_e_down`、列切断旗 `step0_wrap_col0`・`step0_wrap_collast` を加えた有限合同系から、29 鍵を台に持つ第六の左核矛盾証拠を取り出した。29 行を足すと自由未知数の 7,968 弧型と先行五特徴は相殺し、固定済みの語長四弧型 18 種だけが奇数回残る。候補値の和は一、鍵の右辺の和は零である。プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 一次特徴と二特徴積を全数比較した 817 個の相殺しない特徴のうち、先頭語位置の左辺所属 `step0_e_left` が最初の一次特徴として第六証拠との積を一にする。ただしこの列も加えた全 7,085 鍵の系は階数 6,308・拡大階数 6,309 で非可解なので、六特徴でも閉式には足りず、さらに独立な障害が残る。一般命題と Lean 二版は未追加である。次は六特徴を加えた系から第七の左核証拠を取り出し、残る障害で相殺しない語長四特徴を比較する。

## 二次体表示の一意性のレビューと二次体の乗法閉性の表記統一（2026-09-15）

LLM による検証で「二次体の表示の一意性」の本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath を照合した。一と $s$ の一次独立性から二次体の係数表示を一意化し、後続の表示写像と正錐の well-defined 性から反復参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の表示の異なる 4,704 組、等式鎖 4,802 組、同一表示 98 組に対するプログラミングによる検証は通過した。

続けて「二次体の台集合の乗法による閉性」で、三つの補助等式を途中の散文で分断せず一つの整列数式へまとめ、主計算の適用根拠を補助等式の名前ではなく各等式そのものへ置き換えた。LLM による検証で積の十五段の鎖、係数の住処、二次体への所属、全参照を保持したと判断した。

次の本文レビューは「二次体の台集合」から続ける。次の式変形の表記統一は「二次体の積の表示」から続ける。

## 四特徴追加後に残る第五の語長四障害（2026-09-15 の 264 回目）

位置ビット閉式へ先行四障害を除く交差所属積 `step0_e_left*step0_d_up`、上辺所属 `step0_e_up`、列切断旗 `step0_wrap_col0` と `step0_wrap_collast` を加えた有限合同系から、18 鍵を台に持つ第五の左核矛盾証拠を取り出した。18 行を足すと自由未知数の 7,968 弧型と先行四特徴は相殺し、固定済みの語長四弧型 14 種だけが奇数回残る。候補値の和は零、鍵の右辺の和は一である。プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 一次特徴と二特徴積を全数比較した 975 個の相殺しない特徴のうち、先頭語位置の下辺所属 `step0_e_down` が最初の一次特徴として第五証拠との積を一にする。ただしこの列も加えた全 7,085 鍵の系は階数 6,307・拡大階数 6,308 で非可解なので、五特徴でも閉式には足りず、さらに独立な障害が残る。一般命題と Lean 二版は未追加である。次は五特徴を加えた系から第六の左核証拠を取り出し、残る障害で相殺しない語長四特徴を比較する。

## 一と s の一次独立性のレビューと二次体の和の表示の表記統一（2026-09-15）

LLM による検証で「一と s の一次独立性」の本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath を照合した。有理数の平方が二にならないことから $s$ の有理性を排除し、後続の「二次体の表示の一意性」で参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の 4,416 非零係数組と式変形に対するプログラミングによる検証は通過した。

続けて「二次体の和の表示」で、表示の仕様による二つの元の展開、加法閉性、和の表示の一意性を、散文から各根拠を行末に持つ一つの整列数式へ移した。LLM による検証で係数の住処、和の所属、表示の一意性、全参照を保持したと判断した。

次の本文レビューは「二次体の表示の一意性」から続ける。次の式変形の表記統一は「二次体の台集合の乗法による閉性」から続ける。

## 全六研究監督: 四特徴後の第五の語長四障害へ進む（2026-09-15 15:02 JST）

実行識別子 `26f33afc-f560-4787-851c-501e03d64e96`。LLMによる検証では、語長二の交差二項と
強制係数核、語長三位置式、語長四での反証と四つの独立障害を研究上の前進とした。四特徴を加えても
有限合同系は階数6306・拡大階数6307で非可解であり、一般命題とLean二版は未達である。段取り変更なし。
次は第五の左核証拠と非直交な語長四特徴の比較。14:58:42のプログラミングによる観測ではlaunchd登録あり・
非実行中・runs 43・last exit 0、実ログは14:54:14正常終了だった。

## 三特徴追加後に残る第四の語長四障害（2026-09-15 の 263 回目）

位置ビット閉式へ先行三障害を除く交差所属積 `step0_e_left*step0_d_up`、上辺所属 `step0_e_up`、列切断旗 `step0_wrap_col0` を加えた有限合同系から、28 鍵を台に持つ第四の左核矛盾証拠を取り出した。28 行を足すと自由未知数の 7,968 弧型と先行三特徴は相殺し、固定済みの語長四弧型 16 種だけが奇数回残る。候補値の和は零、鍵の右辺の和は一である。プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 一次特徴と二特徴積を全数比較した 828 個の相殺しない特徴のうち、最初の語位置の反対側の列切断旗 `step0_wrap_collast` が最初の一次特徴として第四証拠との積を一にする。ただしこの列も加えた全 7,085 鍵の系は階数 6,306・拡大階数 6,307 で非可解なので、四特徴でも閉式には足りず、さらに独立な障害が残る。一般命題と Lean 二版は未追加である。次は四特徴を加えた系から第五の左核証拠を取り出し、残る障害で相殺しない語長四特徴を比較する。

## 有理数の平方のレビューと二次体の加法閉性の表記統一（2026-09-15）

LLM による検証で「有理数の平方は二にならない」の本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath を照合した。素数二の指数の加法性から整数の二倍が一になる矛盾を導き、後続の「一と s の一次独立性」と「有理数の平方は二倍の平方にならない」で参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の 3,200 有理数と整数の偶奇に対するプログラミングによる検証は通過した。

続けて「二次体の台集合の加法による閉性」で、有理係数の和二つの所属、二次体の定義による表示の所属、元の和への戻しを、散文から各根拠を行末に持つ一続きの数式へ移した。LLM による検証で七段の加法の鎖、係数の住処、二次体への所属、全参照を保持したと判断した。

次の本文レビューは「一と s の一次独立性」から続ける。次の式変形の表記統一は「二次体の和の表示」から続ける。

## 二特徴追加後に残る第三の語長四障害（2026-09-15 の 262 回目）

位置ビット閉式へ先行二障害を除く交差所属積 `step0_e_left*step0_d_up` と上辺所属 `step0_e_up` を加えた有限合同系から、28 鍵を台に持つ第三の左核矛盾証拠を取り出した。28 行を足すと自由未知数の 7,968 弧型と先行二特徴は相殺し、固定済みの語長四弧型 16 種だけが奇数回残る。候補値の和は零、鍵の右辺の和は一である。プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 一次特徴と二特徴積を全数比較した 830 個の相殺しない特徴のうち、最初の語位置の列切断旗 `step0_wrap_col0` が最初の一次特徴として第三証拠との積を一にする。ただしこの列も加えた全 7,085 鍵の系は階数 6,305・拡大階数 6,306 で非可解なので、三特徴でも閉式には足りず、さらに独立な障害が残る。一般命題と Lean 二版は未追加である。次は三特徴を加えた系から第四の左核証拠を取り出し、残る障害で相殺しない語長四特徴を比較する。

## 自己双対方程式の二根のレビューと二次体の三分律の表記統一（2026-09-15）

LLM による検証で「自己双対方程式の因数分解と根の全体」の本文・Lean 具体版・必要十分版・必要十分版からの導出・SageMath を照合した。因数分解と零因子の不在から二根を尽くし、後続の正の根の一意性で参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の二つの平方根と 19 検査点に対するプログラミングによる検証は通過した。

続けて「二次体の三分律（高々一つ）」で、零元と負元の排他に使う負元の表示を、散文中の二等式から各根拠を行末に持つ整列式へ移した。LLM による検証で三通りの排他、九つの場合分け、全参照を保持したと判断した。

次の本文レビューは「有理数の平方は二にならない」から続ける。次の式変形の表記統一は「二次体の台集合の加法による閉性」から続ける。

## 一項追加後に残る第二の語長四障害（2026-09-15 の 261 回目）

位置ビット閉式へ最初の五鍵障害を除く交差所属積 `step0_e_left*step0_d_up` を加えた有限合同系から、27 鍵を台に持つ次の左核矛盾証拠を取り出した。27 行を足すと自由未知数の 7,968 弧型と最初の交差所属積は相殺し、固定済みの語長四弧型 14 種だけが奇数回残る。候補値の和は零、鍵の右辺の和は一である。プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 一次特徴と二特徴積を全数比較した 652 個の相殺しない特徴のうち、先頭語位置の上辺所属 `step0_e_up` が最も単純な一次特徴として第二証拠との積を一にする。ただしこの列も加えた全 7,085 鍵の系は階数 6,304・拡大階数 6,305 で非可解なので、二特徴でも閉式には足りず、さらに独立な障害が残る。一般命題と Lean 二版は未追加である。次は二特徴を加えた系から第三の左核証拠を取り出し、残る障害で相殺しない語長四特徴を比較する。

## 二の平方根の存在のレビューと二次体の三分律の表記統一（2026-09-15）

LLM による検証で「二の平方根の存在」の本文・Lean 二版・SageMath を照合した。代数閉性から $s\cdot s=2$ を満たす代数的数の存在を供給し、後続から反復参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の二根に対するプログラミングによる検証は通過した。

続けて「二次体の三分律（少なくとも一つ）」で、非正係数の加法逆元と零判定、および混合符号の場合の積の結合を、散文中の連鎖から各根拠を行末に持つ整列式へ移した。LLM による検証で四つの場合分け、正錐の三条件、全参照を保持したと判断した。

次の本文レビューは「自己双対方程式の因数分解と根の全体」から続ける。次の式変形の表記統一は「二次体の三分律（高々一つ）」から続ける。

## 語長四候補を反証する五鍵で落ちた特徴（2026-09-15 の 260 回目）

位置ビット閉式を反証した左核の五鍵を比較した。五行を足すと自由未知数の 7,968 弧型は全て偶数回現れて消え、固定した語長四弧型は二種類だけが奇数回残る。二種類は両端点の完全署名が同じで、候補値もともに零だが、五鍵の右辺の和は一である。プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 二種類は、先頭内部頂点の横向き単辺に対して二重辺が上か下かという、異なる軸の単辺・二重辺所属の相対配置を区別する。交差所属積 `step0_e_left*step0_d_up` は二種類で一、零となり、五鍵証拠との積は一である。ただしこの一項を全 7,085 鍵へ加えた系は階数 6,303・拡大階数 6,304 で非可解なので、閉式を完成させる十分条件ではなく別の独立な障害が残る。一般命題と Lean 二版は未追加である。次はこの一項を加えた系の左核証拠を取り出し、残る障害で相殺しない語長四特徴を比較する。

## 自己双対条件のレビューと有理平方の混合符号排除の表記統一（2026-09-15）

LLM による検証で「自己双対条件は二次方程式と同値」の本文・Lean 二版・SageMath を照合した。双対変換の不動点条件を代数的な二次方程式へ移し、後続の自己双対点の導出で参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の 17 検査点に対するプログラミングによる検証は通過した。

続けて「有理数の平方は二倍の平方にならない（混合符号の排除）」で、背理法の計算結果と「有理数の平方は二にならない」の適用を、参照を行末に持つ二つの数式として並べた。LLM による検証で乗法逆元による六段の鎖、矛盾、全参照を保持したと判断した。

次の本文レビューは「二の平方根の存在」から続ける。次の式変形の表記統一は「二次体の三分律（少なくとも一つ）」から続ける。

## 位置ビット閉式の語長四候補は有限合同系と両立しない（2026-09-15 の 259 回目）

一辺二・三から得た 7,085 鍵の有限合同系で、語長四の弧型 2,130 種の値を、各語位置の同スロット $E/D$ 所属積と、二つの内部語位置へ反復した切断旗の八項規則の和へ固定した。他の弧型・閉路型 7,968 種を自由未知数に残した系は、係数行列の階数 6,302、拡大係数行列の階数 6,303 で非可解だった。台が五つの鍵の式からなる左核の矛盾証拠について、係数行列との積が零、右辺との積が一であることも確認した。プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 語長三で強制された切断旗八項を内部語位置へ反復するだけでは、既存の有限鍵に現れる語長四を同時に書けない。この候補を一般語長の式としては採用しない。これは有限データによる反証であり、一般の辺長の恒等式そのものを否定したものではない。一般命題と Lean 二版は未追加である。次は五つの鍵の矛盾証拠を比較し、候補が落とした語長四の特徴を切り分ける。

## 双対変換の対合性のレビューと二次体の加法逆元表示の表記統一（2026-09-15）

LLM による検証で「双対変換の対合性」の本文・Lean 二版・SageMath を照合した。双対変換の定義域保存を使って二回適用が元へ戻ることを示し、後続のセクター値双対から参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の 16 検査点に対するプログラミングによる検証は通過した。

続けて「二次体の加法逆元の表示」で、表示の一意性を適用する結論を「上の鎖」という散文参照から、使う等式と主張ラベルを行末に置いた数式行へ移した。LLM による検証で三段の加法逆元の式変形、係数の住処、表示の一意性、全参照を保持したと判断した。

次の本文レビューは「自己双対条件は二次方程式と同値」から続ける。次の式変形の表記統一は「有理数の平方は二倍の平方にならない（混合符号の排除）」から続ける。

## 固定切断旗係数の位置ビット閉式（2026-09-15 の 258 回目）

語長三で有限合同系の全解に固定された異軸の切断旗係数 16 個を、語位置旗の軸 $a$、境界側 $s$、端点番号 $e$、端点旗の境界側 $t$ という四ビットで整理した。非零 8 項は一意な代数標準形 $1+s+as+e+ase+at+set\in\mathbb F_2$ と一致する。座標四ビットの置換と反転 384 通りを全数検査すると、この係数表を保つものは恒等写像だけだった。プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 左スロットの正準二項を選んだことで非自明な位置対称性が破れており、この有限データだけから対称性に基づく一般式は導けない。一般語長への候補として、両端を除く各語位置へ同じ 8 項規則を置く。語長三では固定 8 項へ正確に戻るが、一般の辺長・全語長での十分性と強制性は未確定であり、一般命題と Lean 二版は未追加。次はこの候補を語長四へ課した有限合同系の可解性を判定する。

## 双対変換の定義域保存のレビューと二次体の加法逆元閉性の表記統一（2026-09-15）

LLM による検証で「双対変換の値は分母を零にしない」の本文・Lean 二版・SageMath を照合した。双対変換を再適用できるという定義域の保存を述べ、後続の対合性が参照する well-defined 性なので、空の四則主張には該当しない。修正点は無かった。対応 SageMath の 16 検査点に対するプログラミングによる検証は通過した。

続けて「二次体の台集合の加法逆元による閉性」で、有理係数の加法逆元による閉性、二次体の定義、表示の等式から結論の所属へ至る含意を、一続きの数式へ移した。LLM による検証で加法逆元の三段の等式、係数の住処、二次体への所属、全参照を保持したと判断した。

次の本文レビューは「双対変換の対合性」から続ける。次の式変形の表記統一は「二次体の加法逆元の表示」から続ける。

## 有限合同系が強制する係数核の同定（2026-09-15 の 257 回目）

左スロットの正準二項を固定した一辺二・三の有限合同系で、係数射影の右核が動かさない 22 係数を名前へ戻した。非零に固定される 10 項は、語長二の `step0_e_up*end0_d_left`、`step0_d_up*end0_e_left` と、語長三の切断旗どうしの積 8 項である。零に固定される 12 項も全て語長三の切断旗どうしの積だった。縮約代表 768 項のうち残る非零 758 項は右核で変更できる代表依存項である。全 7,085 鍵の復元と名前付き係数の assertion によるプログラミングによる検証は終了コード 0 で通過した。

LLM による検証: これは一辺二・三の有限データの解空間に対する切り分けである。固定非零 10 項が一般の辺長と全語長でも強制されること、その 10 項が一般式の十分条件になることは未確定であり、一般命題と Lean 二版は未追加。次は語長三の固定切断旗係数を位置対称性で整理し、一般の語長へ延長する閉式候補を作る。

## 双対変換のレビューと二次体の零元表示の表記統一（2026-09-15）

LLM による検証で「双対変換」の定義・本文・Lean 二版・SageMath を照合した。代数的数体上の逆元で変換を定め、後続の対合性・自己双対点・セクター値双対から反復参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の 16 検査点に対するプログラミングによる検証は通過した。

続けて「二次体の零元の表示による特徴づけ」で、第一の方向が使う零元表示の根拠を「準備の鎖」という散文参照から、その行が使う等式 $0=0+0\cdot s$ そのものへ移した。LLM による検証で二方向の含意、表示の一意性、全参照を保持したと判断した。

次の本文レビューは「双対変換の値は分母を零にしない」から続ける。次の式変形の表記統一は「二次体の台集合の加法逆元による閉性」から続ける。

## 左スロットの正準二項を固定した係数式の縮約（2026-09-15 の 256 回目）

所属向きが異なる左スロットの正準二項 `step0_e_up*end0_d_left` と `step0_d_up*end0_e_left` だけを直前の語長二候補へ加える系を固定し、保存解を係数座標へ射影した右核で縮約した。右核は 1,521 次元、係数射影は 1,386 次元で、22 係数が全解で固定され、うち 10 項が非零だった。既約階段基底の正準代表 881 項を同じ基底順の決定的な貪欲縮約で 768 項（語長一・二・三で 106・318・344 項）へ縮めた。縮約後の係数式を全 7,085 鍵へ再代入するプログラミングによる検証は終了コード 0 で通過した。

LLM による検証: これは一辺二・三の有限データに対する閉じた二次式候補である。768 項が全解中の最小支持台であること、一般の辺長と全語長での成立、一般命題と Lean 二版は未確定である。次は全解で固定される 22 係数（うち非零 10 項）を名前で同定し、有限データに固有な代表依存項と一般式へ残る核を切り分ける。

## 高温展開の四セクター分解のレビュー（2026-09-15）

LLM による検証で「高温展開の多項式は四つのセクター多項式の和である」の本文・Lean 二版・SageMath を照合した。偶部分グラフの四セクターへの一意分割を有限和の分割へ使い、後続の双対恒等式で再利用されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の一辺一から三の整数係数多項式に対するプログラミングによる検証は通過した。

続けて「二次体の台集合への零元の所属」で、表示の証人から得る所属の結論を後続の散文から同じ式の鎖へ移し、二次体の定義を所属行の行末から参照する形へ揃えた。LLM による検証で有理係数二つの所属、零元の表示、二次体への所属を保持したと判断した。

次の本文レビューは「双対変換」から続ける。次の式変形の表記統一は「二次体の零元の表示による特徴づけ」から続ける。

## 所属向きが異なれば先頭端点の左右によらず二項で足りる（2026-09-15 の 255 回目）

語長一の全純二次項と語長三の固定項を保ち、語長二の直前の 160 項へ加える候補を、先頭語位置 `step0` の上スロットと先頭端点 `end0` の左右スロットを結ぶ異軸両所属混合積の二項に絞った。`step0` 側が単一辺所属で `end0` の左スロットを使う項と、`end0` 側が単一辺所属で右スロットを使う項を組む系、および左右を反転した系は、いずれも 12,220 行・13,356 未知数で両階数 11,835 となり可解だった。各系の厳密解を保存し、全 7,085 鍵を復元した。プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 前回までに同じ端点スロットを使う二項族二つも可解と確定しているため、四つの候補項から所属向きが異なるように選ぶ四通りの二項族は全て十分であり、先頭端点の左右は可解性を左右しない。単一辺所属が `step0` 側だけまたは `end0` 側だけの各 8 項族が非可解である既検査と合わせ、この候補系では所属向きが異なる二項を選ぶことが必要十分で、最小項数は 2 である。一般の辺長と全語長は未判定であり、一般命題と Lean 二版は未追加。次は四通りのうち左スロットだけを使う正準な二項を固定し、他の自由係数を縮約して閉じた候補式を抽出する。

## 高温展開のセクター多項式のレビューと二次体表示の表記統一（2026-09-15）

LLM による検証で「高温展開のセクター多項式」の本文・Lean 定義・SageMath を照合した。有限セクター上の整数係数多項式とその住処を定め、後続の四セクター分解・混合双対恒等式・四セクター値双対で反復参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の一辺一から三の有限多項式に対するプログラミングによる検証は通過した。

続けて「二次体の表示の一意性」で、係数差が零であることを使う二本の等式の根拠を「上で得た」という散文参照から、その行が使う等式 `α=0` と `β=0` そのものへ移した。LLM による検証で十四段の係数差の消去、一次独立性の適用、元の係数へ戻す二本の六段の鎖、全参照を保持したと判断した。

次の本文レビューは「高温展開の多項式は四つのセクター多項式の和である」から続ける。次の式変形の表記統一は「二次体の台集合への零元の所属」から続ける。

## 全六研究監督: 可解性を回復する二項族まで絞り込み（2026-09-15 08:59 JST）

実行識別子 `f5cb34ff-b5ce-439f-af5e-afd4e43a91bd`。LLMによる検証では、語長二の有限F2系で
可解性を回復する候補が、先頭語位置の上スロットと先頭端点の左右いずれかを結ぶ二項族まで
絞られた。単一所属側の八項族は非可解なので一項では足りないが、一般の辺長と全語長は未判定である。
次は所属向きの異なる項を左右から一つずつ選ぶ二つの交差組を判定する既存順序を維持した。
08:59のプログラミングによる観測ではlaunchd登録あり・非実行中・runs 34・last exit 0で、
08:05開始分は08:39:50に正常終了した。

## 先頭端点の左右どちらの二項でも足りる（2026-09-15 の 254 回目）

語長一の全純二次項と語長三の固定項を保ち、語長二の直前の 160 項へ加える先頭語位置 `step0` の上スロットと先頭端点 `end0` の横軸スロットを結ぶ異軸両所属混合積 4 項を、`end0` の左スロットを使う 2 項族と右スロットを使う 2 項族へ分けた。二つの系はいずれも 12,220 行・13,356 未知数で、係数行列と拡大係数行列の階数はともに 11,835 となり可解だった。各系について保存した厳密解で全 7,085 鍵を復元した。プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 左右どちらの 2 項族も単独で可解性を回復するため、先頭端点の片側のスロットは必要条件ではない。既検査では単一辺所属が `step0` 側だけの 8 項族と `end0` 側だけの 8 項族がともに非可解なので、単項では足りず、この候補系で追加する混合積の最小項数は 2 である。一般の辺長と全語長は未判定であり、一般命題と Lean 二版は未追加。次は、所属向きの異なる二項を `end0` の左右から一つずつ選ぶ二つの交差組を判定する。

## 低温展開の自明セクター表示のレビューと一・s の一次独立性の表記統一（2026-09-15）

LLM による検証で「低温展開の自明セクター表示」の本文・Lean 二版・SageMath を照合した。破れ辺集合と自明セクターの間の全単射による有限和の添字替えを低温展開の多項式恒等式へ接続し、後続の混合双対恒等式・四セクター値双対・自由エントロピー双対関係で反復参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の一辺一から三の有限多項式に対するプログラミングによる検証は通過した。

続けて「一と s の一次独立性」で、逆元の等式と既に得た三つの等式の根拠を「準備」「上で得た」という散文参照から、各行が使う等式そのものへ移した。LLM による検証で非有理性による背理法、残った係数の消去、全参照を保持したと判断した。

次の本文レビューは「高温展開のセクター多項式」から続ける。次の式変形の表記統一は「二次体の表示の一意性」から続ける。

## 先頭語位置の上スロットを使う四項で足りる（2026-09-15 の 253 回目）

語長一の全純二次項と語長三の固定項を保ち、語長二の直前の 160 項へ加える可解な縦軸 8 項を、先頭語位置 `step0` の上スロットを使う 4 項族と下スロットを使う 4 項族へ分けた。上スロットの 4 項だけの系は 12,220 行・13,358 未知数、係数行列と拡大係数行列の階数がともに 11,835 で可解だった。保存した厳密解で全 7,085 鍵を復元した。下スロットの 4 項だけの系は階数 11,835・拡大階数 11,836 で非可解となり、左核の矛盾証拠を保存した。初回と保存証拠を使った再実行のプログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 上スロットの 4 項だけで可解性が回復するため、下スロットの 4 項は全て同時に除ける。また、単一辺所属が `step0` 側または `end0` 側にあるより大きい 8 項族がどちらも非可解だった既検査から、上スロットの 4 項には両方の所属向きが必要である。個別項の必要性、最小項数、一般の辺長と全語長は未判定であり、一般命題と Lean 二版は未追加。次は上スロットの 4 項を、`end0` の左スロットと右スロットを使う二つの 2 項族へ分けて判定する。

## セクター生成多項式のレビューと有理平方の表記統一（2026-09-15）

LLM による検証で「セクターごとの生成多項式」の本文・Lean 定義・SageMath を照合した。有限セクターの整数係数生成多項式を定め、後続の低温展開・双対関係・符号付きセクター和で反復参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の一辺一から三の全偶部分グラフに対するプログラミングによる検証は通過した。

続けて「有理数の平方は二にならない」で、素数二の指数を数える鎖が再利用する等式の根拠を「準備で得た」という散文参照から、その行が使う等式 $r\cdot r=2$ そのものへ移した。LLM による検証で正の有理数への場合分け、指数の加法性、整数の偶奇による矛盾、全参照を保持したと判断した。

次の本文レビューは「低温展開の自明セクター表示」から続ける。次の式変形の表記統一は「一と s の一次独立性」から続ける。

## 先頭語位置と先頭端点の異軸混合積は縦軸の八項で足りる（2026-09-15 の 252 回目）

語長一の全純二次項と語長三の固定項を保ち、語長二の直前の 160 項へ加える先頭語位置 `step0` と先頭端点 `end0` の異軸両所属混合積を、単一辺所属の側と `step0` の辺スロットの軸で四つの 4 項族へ分けた。所属側が異なる族どうしの四通りのうち、両族とも `step0` の縦軸スロットを使う 8 項だけが 12,220 行・13,362 未知数、係数行列と拡大係数行列の階数 11,836 で可解だった。保存した厳密解で全 7,085 鍵を復元した。他の三組は非可解で、左核の矛盾証拠を保存した。プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 直前の 16 項のうち `step0` の横軸スロットを使う 8 項は全て同時に除ける。個別項の必要性、最小項数、一般の辺長と全語長は未判定であり、一般命題と Lean 二版は未追加。次は可解な縦軸の 8 項を、`step0` の上スロットと下スロットによる二つの 4 項族へ分けて判定する。

## 実現できる双対像のレビューと自己双対方程式の根の表記統一（2026-09-15）

LLM による検証で「実現できる破れた辺集合の双対像は自明セクターの全体である」の本文・Lean 二版・SageMath を照合した。双対破れ辺像の自明セクター所属と自明セクターからの配位復元を二つの包含へ使い、後続の低温展開で再利用されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の一辺一から三の有限集合の一致に対するプログラミングによる検証は通過した。

続けて「自己双対方程式の因数分解と根の全体」で、二方向の含意が使う因数分解の根拠を「準備の等式」という散文参照から、各行が使う因数分解の等式そのものへ移した。LLM による検証で因数分解、二つの根からの代入、零因子のないことによる逆向きの含意、全参照を保持したと判断した。

次の本文レビューは「セクターごとの生成多項式」から続ける。次の式変形の表記統一は「有理数の平方は二にならない」から続ける。

## 先頭語位置と先頭端点の異軸混合積には両方の所属向きが必要である（2026-09-15 の 251 回目）

語長一の全純二次項と語長三の固定項を保ち、語長二の直前の 160 項へ、先頭語位置 `step0` と先頭端点 `end0` を結ぶ異なる軸の両所属混合積のうち、単一辺所属が `step0` 側にある 8 項だけ、または `end0` 側にある 8 項だけを加えた。二つの系はいずれも 12,220 行・13,362 未知数で、係数行列の階数は 11,835、拡大係数行列の階数は 11,836 となり非可解だった。両方の左核の矛盾証拠を保存し、プログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 直前の 16 項全体を許す系は可解なので、十分な部分族には単一辺所属が `step0` 側の項と `end0` 側の項を少なくとも一つずつ含む必要がある。個別項の必要性、最小項数、一般の辺長と全語長は未判定であり、一般命題と Lean 二版は未追加。次は単一辺所属の側と `step0` の辺スロットの軸による四つの 4 項族へ分け、所属の側が異なる 4 項族どうしの組を判定する。

## 自明セクターからの配位復元のレビューと自己双対条件の表記統一（2026-09-15）

LLM による検証で「自明セクターの偶部分グラフから配位を復元できる」の本文・Lean 二版・SageMath を照合した。自明セクターの巻き付き条件から配位を構成し、その双対破れ辺像が元の偶部分グラフに一致することと、原像が全スピン反転の二配位に限ることを示すため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の一辺一から三で各ファイバーが全スピン反転対になることのプログラミングによる検証は通過した。

続けて「自己双対条件は二次方程式と同値」で、逆元の等式と二方向の含意で再利用する積の等式の根拠を「準備」「上の計算」という散文参照から、各行が使う等式そのものへ移した。LLM による検証で準備の積計算、二方向の含意、零因子のないことによる結論、全参照を保持したと判断した。

次の本文レビューは「実現できる破れた辺集合の双対像は自明セクターの全体である」から続ける。次の式変形の表記統一は「自己双対方程式の因数分解と根の全体」から続ける。

## 先頭語位置と先頭端点を結ぶ異なる軸の両所属混合積で両立する（2026-09-15 の 250 回目）

語長一の全純二次項と語長三の固定項を保ち、語長二の直前の 160 項へ、先頭語位置 `step0` と先頭端点 `end0` を結ぶ異なる軸の辺スロットの両所属混合積 16 項だけを加えた。12,220 行・13,370 未知数の系は階数と拡大階数がともに 11,836 で可解だった。保存した厳密解で全 7,085 鍵を復元するプログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 直前の 160 項系は非可解なので、この 16 項の少なくとも一つは必要である。一方、この 16 項だけで可解性が回復するため、残る三つの位置ブロック対の 48 項は全て同時に除ける。この 16 項の個別項の必要性、項数の最小値、一般の辺長と全語長は未判定であり、一般命題と Lean 二版は未追加。次は単一辺所属が語位置側にある 8 項と端点側にある 8 項へ分けて判定する。

## 双対破れ辺像の巻き付き偶奇のレビューと双対変換の対合性の表記統一（2026-09-15）

LLM による検証で「破れた辺の双対像の二つの巻き付き偶奇は零である」の本文・Lean 二版・SageMath を照合した。周期閉路上の二値変化が偶数回であることから自明セクター所属を定め、直後の配位復元で再利用されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の一辺一から四の全配位に対するプログラミングによる検証は通過した。

続けて「双対変換の対合性」で、逆元の等式と既に得た二つの積の等式の根拠を「準備」「上の計算」という散文参照から、各行が使う等式そのものへ移した。LLM による検証で二つの分子計算、分母を掛けた二本の鎖、零因子のないことによる結論、全参照を保持したと判断した。

次の本文レビューは「自明セクターの偶部分グラフから配位を復元できる」から続ける。次の式変形の表記統一は「自己双対条件は二次方程式と同値」から続ける。

## 語位置と端点を結ぶ異なる軸の両所属混合積で両立する（2026-09-15 の 249 回目）

語長一の全純二次項と語長三の固定項を保ち、語長二の直前の 160 項へ、語位置と端点を結ぶ異なる軸の辺スロットの両所属混合積 64 項だけを加えた。12,220 行・13,418 未知数の系は階数と拡大階数がともに 11,836 で可解だった。保存した厳密解で全 7,085 鍵を復元するプログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 二つの語位置を結ぶ 16 項と二つの端点を結ぶ 16 項はどちらも単独では足りなかったが、語位置と端点を結ぶ 64 項で可解性が回復する。従って、異なる軸を結ぶ 96 項のうち前二族 32 項は全て同時に除ける。64 項の個別項の必要性、項数の最小値、一般の辺長と全語長は未判定であり、一般命題と Lean 二版は未追加。次は 64 項を位置ブロックの組で分け、個別族の必要性を判定する。

## 双対破れ辺像のレビューと双対変換の定義域保存の表記統一（2026-09-15）

LLM による検証で「破れた辺の双対像は偶部分グラフである」の本文・Lean 二版・SageMath を照合した。格子面境界の四辺に沿うスピン積から各双対頂点の偶次数を導き、後続で反復参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の一辺一から三の全配位に対するプログラミングによる検証は通過した。

続けて「双対変換の値は分母を零にしない」で、逆元の等式と背理法で得た零等式の根拠を「準備」「上で得た」という散文参照から、各行が使う等式そのものへ移した。LLM による検証で四段の双対変換の計算、零因子のないことを使う背理法、全参照を保持したと判断した。

次の本文レビューは「破れた辺の双対像の二つの巻き付き偶奇は零である」から続ける。次の式変形の表記統一は「双対変換の対合性」から続ける。

## 全六研究監督: 二つの16項族を反証し残る64項へ進む段取りを確認（2026-09-15 04:40 JST）

実行識別子 `e73ddff5-bbd6-471e-ac42-54c751cae720`。LLMによる検証では、160項系の非可解性と全256項系の可解性に加え、異なる軸の
語位置間16項と端点間16項を個別に加えた二系も非可解だと有限証拠で確定した。個別項・最小項数・一般の
辺長と全語長は未判定のまま、次は語位置と端点を結ぶ残り64項を加える既存段取りを維持した。
プログラミングによる04:40観測ではlaunchd登録あり・非実行中・runs 28・last exit 0で、04:35実ログも正常終了を記録する。

## 二つの端点を結ぶ異なる軸の両所属混合積だけでは足りない（2026-09-15 の 248 回目）

語長一の全純二次項と語長三の固定項を保ち、語長二の直前の 160 項へ、二つの端点を結ぶ異なる軸の辺スロットの両所属混合積 16 項だけを加えた。12,220 行・13,370 未知数の系は階数 11,835・拡大階数 11,836 で非可解だった。鍵の式 1,086 本と追加制約 2,204 本による左核の矛盾証拠を保存し、初回と保存証拠を使った再実行のプログラミングによる検証はいずれも終了コード 0 で通過した。

LLM による検証: 二つの語位置を結ぶ 16 項だけの系も非可解であり、異なる軸の全 96 項を許す系は可解なので、語位置と端点を結ぶ残り 64 項または複数族の併用が必要である。個別項の必要性、項数の最小値、一般の辺長と全語長は未判定であり、一般命題と Lean 二版は未追加。次は語位置と端点を結ぶ異なる軸の残り 64 項だけを直前の 160 項へ加えた系を判定する。

## 双対辺写像のレビューと高温展開のセクター分割の表記統一（2026-09-15）

LLM による検証で「正方格子の双対辺写像」と「双対辺写像は全単射である」の本文・Lean 二版・SageMath を照合した。明示した両側逆写像から全単射性を示し、後続で反復参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の一辺一から五の全辺に対するプログラミングによる検証は通過した。

続けて「高温展開の多項式は四つのセクター多項式の和である」で、重なりのない合併による有限和の分割の根拠を「準備」という散文参照から、セクター定義と一意分割のブロック参照へ移した。LLM による検証で四段の多項式等式、添字集合の分割、全参照を保持したと判断した。

次の本文レビューは「破れた辺の双対像は偶部分グラフである」から続ける。次の式変形の表記統一は「双対変換の値は分母を零にしない」から続ける。

## 二つの語位置を結ぶ異なる軸の両所属混合積だけでは足りない（2026-09-15 の 247 回目）

語長一の全純二次項と語長三の固定項を保ち、語長二の直前の 160 項へ、二つの語位置を結ぶ異なる軸の辺スロットの両所属混合積 16 項だけを加えた。12,220 行・13,370 未知数の系は階数 11,834・拡大階数 11,835 で非可解だった。鍵の式 1,046 本と追加制約 2,114 本による左核の矛盾証拠を保存し、初回のプログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 異なる軸の全 96 項を加えた系は可解なので、残る二つの端点を結ぶ 16 項または語位置と端点を結ぶ 64 項の少なくとも一つが必要である。個別項の必要性、項数の最小値、一般の辺長と全語長は未判定であり、一般命題と Lean 二版は未追加。次は二つの端点を結ぶ異なる軸の両所属混合積 16 項だけを直前の 160 項へ加えた系を判定する。

## 四セクター分割のレビューと低温展開の表記統一（2026-09-15）

LLM による検証で「偶部分グラフは四つの巻き付きセクターへ一意に分かれる」の本文・Lean 二版・SageMath を照合した。二つの巻き付き偶奇が定める唯一の所属を後続二箇所で使う主張であり、体の四則だけを述べる空の主張には該当しない。修正点は無かった。対応 SageMath の一辺一から三の全偶部分グラフのプログラミングによる検証は通過した。

続けて「低温展開の自明セクター表示」で、双対像の個数保存を写像 $\Delta_L$ の定義から始まる二段の等号へまとめ、主計算でも同じ写像を通して有限和の添字を取り替える形に揃えた。LLM による検証で全単射の構成、個数保存、五段の多項式等式、全参照を保持したと判断した。

次の本文レビューは「正方格子の双対辺写像」から続ける。次の式変形の表記統一は「高温展開の多項式は四つのセクター多項式の和である」から続ける。

## 異なる軸の両所属混合積を全て加えると両立する（2026-09-15 の 246 回目）

語長一の全純二次項と語長三の固定項を保ち、語長二へ異なる位置ブロックの異なる軸の辺スロットを結ぶ残り 96 項を加えた。これにより単一辺所属・二重辺所属の全 256 混合積を許す系となり、12,220 行・13,450 未知数の系は階数と拡大階数がともに 11,836 で可解だった。保存済みの厳密解を新しい構成へ掛け直し、全 7,085 鍵を復元するプログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 直前の 160 項系の非可解性と合わせ、異なる軸を結ぶ 96 項の少なくとも一つが必要である。96 項の個別項の必要性、項数の最小値、一般の辺長と全語長は未判定であり、一般命題と Lean 二版は未追加。次は 96 項のうち二つの語位置を結ぶ 16 項だけを直前の 160 項へ加えた系を判定する。

## 双対破れ辺像のレビューと実現可能像の両包含の表記統一（2026-09-15）

LLM による検証で「破れた辺の双対像は偶部分グラフである」と「破れた辺の双対像の二つの巻き付き偶奇は零である」の本文・Lean 二版・SageMath を照合した。格子面境界のスピン積と周期閉路上の二値変化を使って自明セクターへの所属を確定し、後続から反復参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の一辺一から四のプログラミングによる検証は通過した。

続けて「実現できる破れた辺集合の双対像は自明セクターの全体である」の右から左への包含で、復元した配位の破れ辺集合を双対写像へ代入する二段の等号を一続きにし、各行へ根拠を置いた。LLM による検証で両包含、存在の構成、全参照を保持したと判断した。

次の本文レビューは「偶部分グラフは四つの巻き付きセクターへ一意に分かれる」から続ける。次の式変形の表記統一は「低温展開の自明セクター表示」から続ける。

## 反対向き同軸辺スロットを結ぶ両所属混合積でも足りない（2026-09-06 の 245 回目）

語長一の全純二次項と語長三の固定項を保ち、語長二には単一辺所属同士・二重辺所属同士の積 240 項、同じ位置ブロック内の両所属混合積 64 項、異なる位置ブロックの同名辺スロットを結ぶ 48 項と反対向き同軸辺スロットを結ぶ 48 項を許した。12,220 行・13,354 未知数の系は階数 11,833・拡大階数 11,834 で非可解だった。鍵の式 1,086 本と追加制約 2,204 本による左核の矛盾証拠を保存し、初回と保存証拠を使った再実行のプログラミングによる検証は終了コード 0 で通過した。

LLM による検証: この 160 項でも既存の有限鍵と両立しない。混合積 256 項を全て許す系は可解なので、異なる位置ブロックの異なる軸の辺スロットを結ぶ項の少なくとも一つが必要である。個別項の必要性、一般の辺長と全語長は未判定であり、一般命題と Lean 二版は未追加。
次は異なる軸の辺スロットを結ぶ残り 96 項も加えた系を判定する。

## 低温展開恒等式のレビューと自明セクターからの復元の表記統一（2026-09-06）

LLM による検証で「分配多項式は破れた辺の集合の生成多項式の二倍である」の本文・Lean 二版・SageMath を照合した。破れた辺集合の各原像が全スピン反転の二配位であることから有限和を類別する低温展開の恒等式で、後続から参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の一辺一から三の全原像と多項式恒等式のプログラミングによる検証は通過した。

続けて「自明セクターの偶部分グラフから配位を復元できる」で、任意の行・列の辺所属和を零へ運ぶ二本の鎖を、行和・列和の巡回不変性と巻き付き偶奇の定義を各等号の行末から引く形へ直した。LLM による検証で既存の構成、原像の二元性、参照を保持したと判断した。

次の本文レビューは「配位の双対破れ辺像は自明セクターに属する」から続ける。次の式変形の表記統一は「実現できる破れた辺集合の双対像は自明セクターの全体である」から続ける。

## 全六研究監督: 未統合160項系を保全して候補包含順を維持（2026-09-14 21:01 JST）

実行識別子 `91e3e4bb-b5cd-45c0-a338-37ebc13d5f4e`。LLMによる検証では、専用worktreeの
160項有限F2系と矛盾証拠は未コミットなので研究上の前進を未判定とした。既存検査・台帳同期・統合後だけ
残る96項へ進む段取りを維持し、checkpointには触れていない。プログラミングによる21:01観測では
launchd登録あり・非実行中・last exit 0だが、20:35実ログは未コミット差分保護による見送りである。

## 全六研究監督: 未統合160項系を保全して候補包含順を維持（2026-09-14 15:02 JST）

実行識別子 `35cb2607-6d12-4f2b-a439-7b6aece2a410`。LLMによる検証では、専用worktreeの
160項有限F2線型系の非可解性と矛盾証拠は未コミットなので研究上の前進を未判定とし、既存検査・
台帳同期・統合後だけ異なる軸の残り96項へ進む段取りを維持した。プログラミングによる15:02観測では
launchd登録あり・非実行中・last exit 0だが、14:47の実ログは未コミット差分保護による見送りである。
これは人間の研究停止指示ではなく運用文であり、保持中差分には触れていない。

## 全六研究監督: 未統合160項系を保全して候補包含順を維持（2026-09-14 09:02 JST）

実行識別子 `7907f348-ccf0-47ad-81fa-a80222f688d9`。LLMによる検証では、専用worktreeの160項有限F2線型系の非可解性と矛盾証拠は未コミットなので研究上の前進を未判定とし、既存検査・台帳同期・統合後だけ異なる軸の残り96項へ進む段取りを維持した。プログラミングによる観測ではlaunchd登録あり・非実行中・last exit 0だが、08:35の実ログは未コミット差分保護による見送りである。人間の研究停止指示ではなく運用文である。

## 全六研究監督: 未統合160項系の保全と候補包含順を維持（2026-09-14 02:58 JST）

実行識別子 `0a88b0e4-b340-4857-a657-5389f7dd095c`。専用worktreeには反対向き同軸スロットまで
含む160項有限系の非可解性と矛盾証拠が未コミットで保持される。LLMによる検証では未統合なので
研究成果を未判定とし、統合後だけ異なる軸を結ぶ残り96項へ進む候補包含順を維持した。
プログラミングによる観測ではlaunchd登録あり・02:35の呼出しは未コミット差分保護による見送り。
監督からcheckpointも研究起動状態も変更していない。

## 全六研究監督: 未統合160項系の保全と候補包含順を維持（2026-09-13 15:00 JST）

実行識別子 `b4c7b222-1bc7-4947-84a9-417b0c1fc324`。専用worktreeには反対向き同軸スロットまで
含む160項系の非可解性と矛盾証拠が未コミットで保持される。LLMによる検証では、既存検査・台帳同期・
統合を閉じた後だけ異なる軸の残り96項へ進む段取りが妥当であり、未統合成果を証明済みへ数えない。
プログラミングによる観測ではlaunchd登録あり・非実行中・last exit 0だが、14:35の実ログは未コミット
差分保護による見送りである。研究停止の人間指示ではなく運用文であり、段取り変更はしない。

## 全六研究監督: 反対向き同軸候補の未コミット成果を保持（2026-09-13 09:03 JST）

LLMによる検証では、前回追記の読解終点以後に新しい統合済み数学成果はない。共有checkoutに保持中の160項系の非可解性と矛盾証拠は未コミットなので、研究上の前進は未判定のまま維持した。既存検査・台帳同期・統合を終えた後だけ異なる軸を結ぶ残り96項へ進む段取りを維持し、未コミット成果と停止条件を変更していない。
前回追記の読解終点 `82201de514e266d8435ad8e0b68b471e09043fd3` から `db3541d70c088eb80c99dd04db23d1c9a4bfd365` までに当研究の新しい統合成果はない。プログラミングによる実行観測は09:03:28、登録あり・非実行中・runs 53・last exit 0。08:35:11の最新呼出しは未コミット差分保護による運用上の見送りであり、研究実行の成功ではない。保持中差分には手を触れていない。

## 全六研究監督: 反対向き同軸候補のcheckpointを未判定で保持（2026-09-10 03:02 JST）

LLMによる検証では、remote defaultに新しい統合済み数学成果はない。別worktreeには反対向き同軸辺スロット48項を加えた160項の有限F2線型系が非可解であることと矛盾証拠が未コミットで保持されている。全256項では可解なので、次候補は異なる軸を結ぶ残り96項だが、checkpointの検査・統合前は研究上の前進を未判定とする。段取り変更はない。
プログラミングによる実行観測は03:02:49、登録あり・非実行中・runs 128・last exit 1。02:35の起動はアカウント割当前に失敗し、02:05:12までは未コミット差分を保護して見送られている。

## 全六研究監督: 反対向き同軸辺スロットの固定候補を維持（2026-09-09 02:58 JST）

LLMによる検証では、開始成果以後に新しい統合済み数学成果はない。既存112項へ反対向き同軸辺スロット48項を加える有限F2線型系の判定を次対象として維持し、段取り変更は行っていない。研究停止の人間指示はなく、実行見送りは未コミット差分を保護する運用判定である。
プログラミングによる実行観測は02:58:25、登録あり・非実行中・runs 80・last exit 0。ただし直近02:35:13は未コミット差分による見送りで、研究成功へ数えていない。

## 全六研究監督: 反対向き同軸辺スロット候補を継続（2026-09-08 15:05 JST）

LLMによる検証では、前回読解終点以後に新しい統合済み数学成果はない。既存112項へ反対向きの同軸辺スロット48項を加える有限F2線型系を次対象として維持し、段取り変更・研究再開は行っていない。
プログラミングによる実行観測は15:05:37、登録あり・非実行中・last exit 0。ただし直近15:05:11は未コミット差分を守る見送りで、研究実行の成功や研究上の前進ではない。

## 全六研究監督: 反対向き同軸辺スロット候補を維持（2026-09-07 16:28 JST）

LLMによる検証では、前回読解終点以後に新しい統合済み数学成果はない。反対向きの同軸辺スロット48項を加える有限F2線型系を次対象として維持し、段取り変更・研究再開は行っていない。
プログラミングによる実行観測は16:28:10、登録あり・非実行中・last exit 0。ただし直近16:05:16は未コミット差分を守る見送りで、研究実行の成功や研究上の前進ではない。

## 全六研究監督: 次の有限F2候補を維持（2026-09-06 20:59 JST）

LLMによる検証では、前回読解終点以後に新しい統合済み数学成果はなく、反対向きの同軸辺スロット48項を加える有限系を次対象として維持した。段取り変更・研究再開なし。
プログラミングによる実行観測は20:59:50、登録あり・非実行中・last exit 0。直近20:35:12は未コミット差分による見送りで、09:35開始分の終了値は未確認のまま分離した。

## 異なる位置ブロックの同名辺スロットを結ぶ両所属混合積でも足りない（2026-09-06 の 244 回目）

語長一の全純二次項と語長三の固定項を保ち、語長二には単一辺所属同士・二重辺所属同士の積 240 項、同じ位置ブロック内の両所属混合積 64 項、異なる位置ブロックの同名辺スロットを結ぶ両所属混合積 48 項だけを許した。12,220 行・13,306 未知数の系は階数 11,825・拡大階数 11,826 で非可解だった。鍵の式 1,016 本と追加制約 2,062 本による左核の矛盾証拠を保存し、初回と保存証拠を使った再実行のプログラミングによる検証は終了コード 0 で通過した。

LLM による検証: この 112 項でも既存の有限鍵と両立しない。混合積 256 項を全て許す系は可解なので、異なる位置ブロックの異なる名前の辺スロットを結ぶ項の少なくとも一つが必要である。個別項の必要性、一般の辺長と全語長は未判定であり、一般命題と Lean 二版は未追加。
次は反対向きの同軸辺スロットを結ぶ残り 48 項も加えた系を判定する。

## 破れた辺集合の生成多項式のレビューと双対像の巻き付き偶奇の表記統一（2026-09-06）

LLM による検証で「破れた辺の集合の生成多項式」の本文・Lean・SageMath を照合した。有限集合族の次数生成多項式として住処を定め、後続の低温展開恒等式から参照されるため、空の四則主張には該当しない。修正点は無かった。対応 SageMath の一辺一から三の全原像と多項式恒等式のプログラミングによる検証は通過した。

続けて「破れた辺の双対像の二つの巻き付き偶奇は零である」で、縦向き巻き付き偶奇の最初の等式に巻き付き・双対辺写像・全単射性の根拠を行末参照として補い、零への帰着で使う二値符号化と有限和の分配を明記した。LLM による検証で横向きと縦向きの閉路和、既存参照、主張を保持したと判断した。

次の本文レビューは「分配多項式は破れた辺の集合の生成多項式の二倍である」から続ける。次の式変形の表記統一は「自明セクターの偶部分グラフから配位を復元できる」から続ける。

## 語長二の位置ブロック内の両所属混合積だけでは足りない（2026-09-06 の 243 回目）

語長一の全純二次項と語長三の固定項を保ち、語長二には単一辺所属同士・二重辺所属同士の積 240 項と、同じ語位置または同じ端点の四スロット間に属する両所属混合積 64 項だけを許した。12,220 行・13,258 未知数の系は階数 11,808・拡大階数 11,809 で非可解だった。鍵の式 1,026 本と追加制約 2,120 本による左核の矛盾証拠を保存し、初回と保存証拠を含む再実行のプログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 同じ位置ブロック内の混合積 64 項では既存の有限鍵と両立しない。混合積 256 項を全て許す系は可解なので、異なる位置ブロックを結ぶ項の少なくとも一つが必要である。個別項の必要性、一般の辺長と全語長は未判定であり、一般命題と Lean 二版は未追加。
次は 64 項に、異なる位置ブロックにある同じ名前の辺スロットを結ぶ混合積 48 項を加えた系の可解性を判定する。

## 実現できる破れた辺集合のレビューと双対像の偶性の表記統一（2026-09-06）

LLM による検証で「実現できる破れた辺の集合の全体」の本文・Lean・SageMath を照合した。有限集合族としての住処を定め、後続から反復参照される定義なので、空の四則主張には該当しない。修正点は無かった。続けて「破れた辺の双対像は偶部分グラフである」で、破れ指示変数の符号等式が破れた辺集合の定義を使うことと、次数の偶性から自然数の二倍表示を得る根拠を該当数式の行末へ移した。LLM による検証で四辺の端点消去、偶性の導出、既存参照を保存したと判断した。対応 SageMath の一辺一から三、本文一括検査・PDF・534 検算の対応、Lean 全体ビルドと登録 1,635 定理の未証明依存のプログラミングによる検証は全て通過した。

次の本文レビューは「破れた辺の集合の生成多項式」から続ける。次の式変形の表記統一は「破れた辺の双対像の巻き付きは零である」から続ける。

## 語長二の同じ署名位置の両所属混合積だけでは足りない（2026-09-06 の 242 回目）

語長一の全純二次項と語長三の固定項を保ち、語長二には単一辺所属同士・二重辺所属同士の積 240 項と、同じ語位置または同じ端点スロットに属する単一辺所属・二重辺所属の混合積 16 項だけを許した。12,220 行・13,210 未知数の系は階数 11,797・拡大階数 11,798 で非可解だった。鍵の式 1,002 本と追加制約 2,112 本による左核の矛盾証拠を保存し、初回と保存証拠を含む再実行のプログラミングによる検証は終了コード 0 で通過した。

LLM による検証: 同じ署名位置の混合積 16 項を戻しても既存の有限鍵とは両立しない。直前の系と階数・矛盾証拠の大きさが一致したことだけから、追加した各列が不要だとは結論しない。一般の辺長、全語長、異なる位置を結ぶ個別項の必要性は未判定であり、一般命題と Lean 二版は未追加。
次は両所属の混合積を、同じ語位置または同じ端点の四スロット間に属する 64 項へ広げ、異なる語位置・端点を結ぶ混合積を除いた系の可解性を判定する。

## 破れた辺集合の定義のレビューと生成多項式恒等式の表記統一（2026-09-06）

LLM による検証で「配位のもとで破れている辺の集合」の本文・Lean・SageMath を照合した。有限部分集合への帰属と破れた辺数との一致を定め、後続から反復参照される定義なので、空の四則主張には該当しない。修正点は無かった。続けて「分配多項式は破れた辺の集合の生成多項式の二倍である」で、全スピン反転・二原像性・破れた辺数の適用根拠を該当する数式の行末へ移した。LLM による検証で原像の構成、相異なる二配位、有限和の類別、全参照を保存したと判断した。対応 SageMath の一辺一から三の全原像と多項式恒等式のプログラミングによる検証は通過した。

次の本文レビューは「実現できる破れた辺の集合の全体」から続ける。次の式変形の表記統一は「双対辺集合は偶部分グラフである」から続ける。

## 破れた辺集合の二原像性の表記統一（2026-09-06）

有限判定の成果 `ea068cdbd` は main への包含を確認した。
「同じ破れた辺の集合を与える配位は全スピン反転を除いて一意である」で、端点での一致の同値と基点への伝播の根拠を各数式の行末へ移し、経路の自然数代表を準備へまとめた。LLM による検証で二値性・二方向の帰納法・基点での場合分け・定義への参照を保持したと判断した。対応 SageMath の一辺一から三、本文一括検査・PDF・532 検算の対応、Lean 全体ビルドと登録 1,635 定理の未証明依存のプログラミングによる検証は全て通過した。
次の本文レビューは「配位のもとで破れている辺の集合」から続ける。次の式変形の表記統一は「分配多項式は破れた辺の集合の生成多項式の二倍である」から続ける。

## 語長二の両所属混合積は全て同時には除けない（2026-09-06 の 241 回目）

語長一の全純二次項と語長三の固定項を保ち、語長二の非切断項を単一辺所属同士・二重辺所属同士の積 240 項だけへ制限すると、12,220 行・13,194 未知数、階数 11,797・拡大階数 11,798 で非可解だった。
鍵の式 1,002 本と追加制約 2,112 本による左核の矛盾証拠を `sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-separated-membership-joint/certificate.json` に保存した。初回と保存証拠を使う再実行のプログラミングによる検証は終了コード 0。
LLM による検証: 直前の可解性と合わせると、語長二の単一辺所属と二重辺所属の混合積 256 項のうち少なくとも一つは必要である。個別項の必要性と項数の最小性、一般の辺長と全語長については未判定。一般命題と Lean 二版は未追加。
次は同じ署名位置に属する単一辺所属・二重辺所属の混合積だけを戻し、異なる位置を結ぶ混合積を除いた系の可解性を判定する。

## 前進前レビュー（2026-09-06 の 241 回目）

LLM による検証で「同じ破れた辺の集合を与える配位は全スピン反転を除いて一意である」の本文・Lean 二版・SageMath を照合した。主張は格子の連結性とスピン値の二値性から破れた辺集合の原像を高々二つへ絞り、後続の低温展開で再利用されるため、空の四則主張には該当しない。修正点は無かった。プログラミングによる検証は一辺一から三の全 530 配位で通過した。

次の本文レビューは「配位のもとで破れている辺の集合」から続ける。

## 全スピン反転の同値変形の準備を整理（2026-09-06）

有限判定の成果 `9bf381084` は main への包含を確認した。
「全スピン反転は各辺の破れを保つ」で、整数の等式を否定して得る同値の説明を式変形前の準備へ移し、補助変数の整数への所属を明記した。LLM による検証で二段の同値変形・定義への参照・整数の単射性の根拠を保持したと判断した。プログラミングによる検証で表示数式・参照・主張の不変を確認した。
次の本文レビューは「同じ破れた辺の集合を与える配位は全スピン反転を除いて一意である」から続ける。

対応 SageMath の一辺一から四、本文一括検査・PDF・531 検算の対応、Lean 全体ビルドと登録 1,635 定理の未証明依存のプログラミングによる検証は全て通過した。

## 語長二の非切断項から選択辺所属を除去（2026-09-06 の 240 回目）

語長一の全純二次項と語長三の固定項を保ち、語長二の非切断項を単一辺所属・二重辺所属だけからなる全ての積へ広げると、12,220 行・13,450 未知数、両階数 11,836 で可解となり、全 7,085 鍵を復元した。
保存先は `sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-unselected-membership-joint/certificate.json`。初回のプログラミングによる検証は終了コード 0。
...
本文レビューに修正点は無く、前進前の修正コミットは不要だった。作業場所の登録コマンドは担当窓限定のため終了コード 1。この tick は既存の定期起動用 worktree で実行した。
...
## 前進前レビュー（2026-09-06 の 240 回目）

LLM による検証で「全スピン反転」の定義と「全スピン反転は各辺の破れを保つ」の本文・Lean 二版・SageMath を照合した。定義は配位への所属を確定し、主張は破れた辺集合の原像の数え上げで再利用されるため、空の四則主張には該当しない。修正点は無かった。プログラミングによる検証は一辺一から四の全 2,106,500 本の配位つき辺で通過した。

次の本文レビューは「同じ破れた辺の集合を与える配位は全スピン反転を除いて一意である」から続ける。

## Fisher 零点の根の条件の表記統一（2026-09-06）

LLM による検証で、有限判定の帰結が語長二も純二次式で書く候補に限ることを明記した。高次の式の不可能性までは含めない。本文・台帳・検算概要を同じ範囲へ揃えた。

有限判定の成果 `dfe66ed4f` は main への包含を確認した。
「有限格子の Fisher 零点は代数的である」で、根の定義と所属条件の適用根拠を評価式の行末へ移した。LLM による検証で非零評価の鎖・根の所属条件・証拠の選択・全参照を保持したと判断した。
次の本文レビューは「全スピン反転」から続ける。

対応 SageMath の全 20 零点、本文一括検査・PDF・530 検算の対応、Lean 全体ビルドと登録 1,635 定理の未証明依存のプログラミングによる検証は全て通過した。

## 語長二へ単一辺所属と二重辺所属の積を追加（2026-09-06 の 239 回目）

語長一の全純二次項と語長三の固定項を保ち、語長二の非切断項に単一辺所属どうしの積と単一辺・二重辺所属の積を許す系は、12,220 行・13,330 未知数、階数 11,833・拡大階数 11,834 で非可解だった。
鍵の式 1,044 本と追加制約 2,068 本による左核の矛盾証拠を保存した。保存先は `sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-mixed-membership-joint/certificate.json`。初回のプログラミングによる検証は終了コード 0。
LLM による検証: 語長一の純二次式と語長三の固定項を保ち、語長二も純二次式で書く範囲では、語長二に二重辺所属どうしの積か、選択辺所属を含む非切断項の少なくとも一方も必要である。個別種類・個別項の必要性は未判定。一般命題と Lean 二版は未追加。
前進前のレビュー修正 `fb7b9f4ac` は main への包含を確認した。
...
## 前進前レビュー（2026-09-06 の 239 回目）

実行記録: 全根を掛け合わせる追加判定は合成体の resultant 計算に入った（sample で確認）。この実行だけを safe-kill で停止し（終了コード 143）、全根の完全性と同値な、各根の形式微分による正確な重複度・相異性・次数との和の一致へ書き直した。有限例の範囲・厳密計算・期待する完全性は保つ。

LLM による検証で多項式評価・Fisher 零点の定義と代数性の本文・Lean 二版・SageMath を照合した。非零な整係数多項式の証拠と可算側の帰属を確定する内容なので本文は維持した。必要十分版は完成した証拠の言い直しだったため、試験点での非零評価から多項式の非零性を導く本文と同じ手順へ直した。SageMath に係数和による評価と形式微分による各根の正確な重複度と次数との和の一致を追加した。
実行記録: SageMath 初回と対象・全体 Lean ビルドの各初回はリポジトリ直下を指定したため、それぞれ対象ファイル・プロジェクト設定を解決できず終了コード 2・1・1。各プロジェクトの基準ディレクトリを指定して同じ経路を再実行した。

プログラミングによる検証は一辺一から三の全 20 零点、本文一括検査・PDF・529 検算の対応、Lean 全体ビルドと登録 1,635 定理の未証明依存で全て通過した。

## 固有空間の分解の表記統一（2026-09-06）

有限判定の成果 `4ecf2dbee` は main への包含を確認した。
「シフト行列の固有空間たちは列ベクトルの全体を張る」で、族の定義の説明を準備の冒頭へ置き、所属と復元の式の間にあった説明を除いた。復元公式の根拠は式の行末と直後の参照へ揃えた。LLM による検証で構成・所属・復元の各段と全参照の保存を確認した。
次の本文レビューは「整係数多項式の代数的数における値」から続ける。
対応 SageMath の一辺一から四、表示数式と参照の保存、本文一括検査・PDF・529 検算の対応、Lean 全体ビルドと登録 1,635 定理の未証明依存のプログラミングによる検証は全て通過した。

## 語長二の非切断項を単一辺所属の積へ制限（2026-09-06 の 238 回目）

語長一に全ての純二次項を許しても、語長二の非切断項を単一辺所属どうしの積だけへ制限し、語長三の固定項を保つ系は、12,220 行・13,074 未知数、階数 11,764・拡大階数 11,765 で非可解だった。
鍵の式 980 本と追加制約 2,018 本による左核の矛盾証拠を `sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-single-membership-joint/certificate.json` に保存した。初回のプログラミングによる検証は終了コード 0。
LLM による検証: 語長一の純二次式と語長三の固定項を保つ限り、語長二には単一辺所属どうしの積以外の非切断項も必要である。どの種類・個別項が必要かは未判定。一般命題と Lean 二版は未追加。
前進前のレビュー修正 `510df1bbc` は main への包含を確認した。
...
## 前進前レビュー（2026-09-06 の 238 回目）

LLM による検証で「シフト行列の固有空間たちは列ベクトルの全体を張る」の本文・Lean 二版・SageMath を照合した。本文は固有空間への所属と全体の分解を主張するため維持する。Lean 必要十分版が完成した族の存在を言い直すだけだったため、射影像の所属からスカラー倍を経て族を構成する本文と同じ手順へ直し、具体版への特殊化も更新した。SageMath に冪根集合の完全性と規格化前の射影像の所属を追加し、プログラミングによる検証は一辺一から四で通過した。
本文一括検査・PDF・528 検算の対応、Lean 全体ビルドと登録 1,635 定理の未証明依存のプログラミングによる検証は通過した。
初回の Lean ビルドは特殊化側の添字型を推論できず終了コード 1。冪根の型を明記して同じビルドを再実行し、通過した。

## 復元公式の表記統一（2026-09-06）

有限判定の成果 `a9312ae9c` は main への包含を確認した。
「固有空間へ落とす写像から列ベクトルを復元できる」で、準備に使う冪和の等式の根拠と適用対象を数式の行末へ移した。LLM による検証で場合分け・復元の鎖・所属・参照の保存を確認した。
次の本文レビューは「シフト行列の固有空間たちは列ベクトルの全体を張る」から続ける。
対応 SageMath の一辺一から四、本文一括検査・PDF・528 検算の対応、Lean 全体ビルドと登録 1,635 定理の未証明依存のプログラミングによる検証は全て通過した。

## 語長一・二の単一辺所属の積への同時制限（2026-09-06 の 237 回目）

語長一・二の非切断項をともに単一辺所属どうしの積だけへ制限し、語長三の固定項を保つ系は、12,220 行・12,644 未知数、階数 11,692・拡大階数 11,693 で非可解だった。
鍵の式 938 本と追加制約 1,930 本による左核の矛盾証拠を `sagemath/check/parity-identity-simple-cycle-arc-orientation-short-single-membership-joint/certificate.json` に保存した。初回のプログラミングによる検証は終了コード 0。
LLM による検証: 語長一を単一辺所属の積に制限したままでは、語長二の非切断項から他の五種類を同時に除けない。語長一の制限を外した場合の必要性や個別の項の必要性は未判定。 一般命題と Lean 二版は未追加。
前進前のレビュー修正 `850a13f2b` は main への包含を確認した。
...
## 前進前レビュー（2026-09-06 の 237 回目）

復元公式の SageMath に冪根列の相異性・根への所属・独立な全根集合との一致判定を追加した。プログラミングによる検証は一辺一から四で通過した。
LLM による検証: 本文と Lean 二版は冪和の消滅による復元を扱い、後続の固有空間の分解に使うため維持する。
本文一括検査・PDF・527 検算の対応、Lean 全体ビルドと登録 1,635 定理の未証明依存のプログラミングによる検証も通過した。

## 冪和公式の表記統一（2026-09-06）

有限判定の成果 `df676b387` は main への包含を確認した。
「1 の冪根の全体にわたる冪の和の値」で、二つの結論と非自明冪を持つ根の存在の根拠を数式の行末へ移した。LLM による検証で場合分け・既存三主張の適用・所属・参照の保存を確認した。
次の本文レビューは「固有空間へ落とす写像から列ベクトルを復元できる」から続ける。
実行記録: 表記統一後の SageMath 初回は基準ディレクトリを誤り、対象ファイルが無く終了コード 2。プロジェクト直下で同じコマンドを再実行した。

対応 SageMath の 144 組、本文一括検査・PDF・527 検算の対応、Lean 全体ビルドと登録 1,635 定理の未証明依存のプログラミングによる検証は全て通過した。

## 語長一の非切断項を単一辺所属の積へ制限（2026-09-06 の 236 回目）

語長一の非切断項を単一辺所属どうしの積 66 項だけへ制限し、語長二の純二次式と語長三の固定項を保つ系は、12,220 行・13,304 未知数、両階数 11,836 で可解となり、全 7,085 鍵を復元した。
実装は `sagemath/check/parity-identity-simple-cycle-arc-orientation-length-one-single-membership-joint/`。13,304 成分の解を保存した。初回のプログラミングによる検証は終了コード 0。
LLM による検証: 語長一の非切断項のうち、単一辺所属以外の五種類は同時に除ける。一解で非切断項10項が残るが最小性は主張しない。一般命題と Lean 二版は未追加。
前進前のレビュー修正 `cc7b6df77` は main への包含を確認した。
...
## 前進前レビュー（2026-09-06 の 236 回目）

冪和公式の SageMath に相異性・根への所属・独立な全根集合との一致を補った。プログラミングによる検証は全 144 組で通過した。
LLM による検証: 本文と Lean 二版は場合分けと既存三主張の合成で一致し、射影からの復元で使う公式として維持した。
本文一括検査・PDF・526 検算の対応、Lean 全体ビルドと登録 1,635 定理の未証明依存のプログラミングによる検証も通過した。

## 冪根の非自明冪の存在の表記統一（2026-09-06）

有限判定の成果 `7cab64b84` は main への包含を確認した。
必須の表記統一は一件だけ行い、準備に使う根の等式と定義の参照を式の行末へ移した。
LLM による検証: 除法・背理法・冪の七段の鎖・個数による矛盾・所属・参照を保持した。
対応 SageMath の 93 組、本文一括検査・PDF・526 検算の対応、Lean 全体ビルドと登録 1,635 定理の未証明依存のプログラミングによる検証は全て通過した。
次の本文レビューは「1 の冪根の全体にわたる冪の和の値」から続ける。

## 語長一の純二次式と語長二の局在形の両立判定（2026-09-06 の 235 回目）

語長二の局在形と語長三の固定項を保つ限り、語長一に全ての純二次項を許しても、12,220 行・12,954 未知数の系は階数 11,727・拡大階数 11,728 で非可解だった。
実装は `sagemath/check/parity-identity-simple-cycle-arc-orientation-length-one-pure-quadratic-joint/`。一般命題と Lean 二版は未追加。
初回のプログラミングによる検証は終了コード 0。鍵の式 992 本と追加制約 2,042 本による左核の矛盾証拠を保存した。保存証拠を含む再実行のプログラミングによる検証も終了コード 0 で通過し、両階数と左核の矛盾を再現した。
LLM による検証: 既存の語長一局在系の非可解性と合わせ、語長三の固定項を保つ短い二語長の純二次形では、語長一・二のどちらも非切断項を全て零にはできない。個別の項の必要性は主張しない。
次は語長一の非切断項を単一辺所属どうしの積だけへ制限し、語長二の純二次式と語長三の固定項を保った系の可解性を同じ有限鍵集合で判定する。
前進前のレビュー修正 `ab8bbf7b5` は main への包含を確認した。
本文一括検査・PDF・526 検算の対応・構成分離、Lean 全体ビルドと登録 1,635 定理の未証明依存のプログラミングによる検証は全て通過した。

## 前進前レビュー（2026-09-06 の 235 回目）

冪根の非自明冪の存在について、SageMath に根への所属と独立な全根集合との一致判定を補った。
追加後のプログラミングによる検証は 93 組で通過した。
本文一括検査・PDF・525 検算の対応、Lean 全体ビルドと登録 1,635 定理の未証明依存のプログラミングによる検証も通過した。
LLM による検証: 本文と Lean 二版は除法・冪の鎖・根の個数の矛盾という同じ論法であり、冪和公式で使う主張として維持した。

## 冪根の倍数指数の和の表記統一（2026-09-06）

有限判定の成果 `aefcab813` は main への包含を確認した。
必須の表記統一を一件行い、準備に使う冪根の個数の等式と参照を式の行末へ移した。
LLM による検証: 仮定・有限和の四段の鎖・全単射の構成・所属・参照を保持した。
対応 SageMath の 104 組、本文一括検査・PDF・525 検算の対応、Lean 全体ビルドと登録 1,635 定理の未証明依存のプログラミングによる検証は全て通過した。
次の本文レビューは「指数が根の次数の倍数でないとき、冪が 1 でない 1 の冪根が存在する」から続ける。


## 語長一・二の純二次式と語長三の固定項の両立（2026-09-06 の 234 回目）

語長三の固定項を保ち、語長一・二の両方に全ての純二次項を許すと、12,220 行・13,734 未知数の系は両階数 11,843 で可解となり、全 7,085 鍵を復元した。
実装は `sagemath/check/parity-identity-simple-cycle-arc-orientation-short-pure-quadratic-fixed-third/`。初回のプログラミングによる検証は終了コード 0。
13,734 成分の解を保存した。保存解を含む再実行のプログラミングによる検証も終了コード 0 で通過し、両階数と全 7,085 鍵の復元を再現した。
一解の二次項は語長一が 206 項（切断旗あり 136、なし 70）、語長二が 413 項（切断旗あり 259、なし 154）、語長三が切断旗あり 326 項と固定項。
LLM による検証: 前回の非可解性と合わせ、語長三の固定項と語長一・二の純二次形を保つ系では、語長一の非切断項を全て零にすることはできない。一解に現れた個別項の必要性・項数の最小性は主張しない。
一般命題と Lean 二版は未追加。次は語長一に全ての純二次項を許し、語長二を切断旗の項だけへ戻しても、語長三の固定項と両立するかを同じ有限鍵集合で判定する。
レビュー修正 `4f769d69f` は前進前に main への包含を確認した。
本文一括検査・PDF・525 検算の対応・構成分離、Lean 全体ビルドと登録 1,635 定理の未証明依存のプログラミングによる検証も通過した。


## 前進前レビュー（2026-09-06 の 234 回目）

冪根の倍数指数の和について、SageMath の列の長さだけによる個数判定を補った。
相異性・根への所属・多項式から独立に求めた全根との集合一致を追加し、104 組で通過した。
LLM による検証: 本文と Lean 二版は同じ有限和の鎖であり、冪和公式に使うので主張を維持した。
本文一括検査・PDF・524 検算の対応、Lean 全体ビルドと登録 1,635 定理の未証明依存のプログラミングによる検証も通過した。
構成分離の補助検査は必須引数なしの呼出しで終了コード 2。前進で対象ファイルを指定して実行する。


## 冪根の倍数指数の表記統一（2026-09-06）

有限判定の成果 `58836024e` は main への包含を確認した。
必須の表記統一を一件行い、準備に使う根の等式と定義の参照を数式の行末へ移した。
LLM による検証: 仮定・四段の鎖・所属・参照を保持した。
対応 SageMath の 468 組、本文一括検査・PDF・524 検算の対応、Lean 全体ビルドと登録 1,635 定理の未証明依存のプログラミングによる検証は全て通過した。
次の本文レビューは「指数が根の次数の倍数のとき、冪の和は根の次数の与える代数的数である」から続ける。

## 語長二の純二次式による三語長の両立判定（2026-09-06 の 233 回目）

語長一の局在形と語長三の固定項を保つ限り、語長二に全ての純二次項を許しても、12,220 行・13,238 未知数の系は階数 11,826・拡大階数 11,827 で非可解だった。
語長二の非切断二次項を全て解放しても障害は除けない。語長二単独の非存在は結論しない。
実装は `sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-pure-quadratic-joint/`。
初回と保存証拠を含む再実行のプログラミングによる検証は終了コード 0 で通過した。
鍵の式 1,003 本と追加制約 2,094 本を足す左核の矛盾証拠を保存し、両階数と矛盾を再現した。
本文一括検査・PDF・524 検算の対応・構成分離のプログラミングによる検証も通過した。
一般命題と Lean 二版は未追加。次は語長三の固定項を保ち、語長一・二の両方に非切断項も含む純二次式を許した単一系の可解性を判定する。
LLM による検証: 語長一・三の制約は保持し、語長二の定数・一次項は許していない。

前進前の LLM による検証で冪根の倍数指数の本文・Lean 二版・SageMath の同じ論法を確認した。
後続の冪和公式で繰り返し使う主張なので削除しない。修正点は無し。
プログラミングによる検証は対応 SageMath の 468 組と前回の保存解による全鍵の復元が通過した。
Lean 全体ビルドと登録 1,635 定理の未証明依存のプログラミングによる検証も通過した。

## 冪根の和の消滅の表記統一（2026-09-06）

有限判定の成果 `983fd5f5a` は main への包含を確認した。
必須の表記統一を一件行い、冪和が零元になる結論の根拠を式の行末へ明示した。
LLM による検証: 仮定・四段の鎖・零因子不存在の適用・参照を保存した。
対応 SageMath・本文一括検査・PDF・523 検算の対応、Lean 全体ビルドと登録 1,635 定理の未証明依存のプログラミングによる検証は全て通過した。
次の本文レビューは「指数が根の次数の倍数ならば 1 の冪根の冪は 1 である」から続ける。

## 語長二の局在形と語長三の固定項の両立（2026-09-06 の 232 回目）

語長一の制約を外し、語長二の局在形と語長三の固定非切断項だけを課す系も可解で、10,971 行・12,008 未知数の両階数は 10,625、全 7,085 鍵を復元した（`parity-identity-simple-cycle-arc-orientation-length-two-three-fixed-interior`）。既存の語長一・二だけ、語長一・三だけの可解性と合わせ、三つの条件は二つずつなら両立し、三つ同時では両立しない。
一解の切断旗を含む積は語長二で 304 項、語長三で 197 項。最小性・一意性は主張しない。
初回のプログラミングによる検証は終了コード 0。保存証拠による再実行も終了コード 0 で通過し、両階数と全鍵の復元を再現した。本文一括検査・PDF・523 検算の対応・構成分離、Lean 全体ビルドと登録 1,635 定理の未証明依存のプログラミングによる検証も通過した。
本文検査の初回起動はリポジトリ直下に package.json が無く終了コード 254。構造化テキストのディレクトリで同じコマンドを再実行した。
Lean 全体ビルドと登録 1,635 定理の未証明依存のプログラミングによる検証は通過した。
一般命題と Lean 二版は未追加。次は語長一の局在形と語長三の固定項を保ち、語長二には非切断項も含む純二次式を許した単一系の可解性を判定する。
LLM による検証: 別々の二語長系の可解性と、三語長を同時に課す系の可解性は区別した。

前進前の LLM による検証では、冪和の消滅の本文・Lean 二版・SageMath は同じ論法で、
後続の冪和公式で使うため空の四則主張には該当しなかった。修正点は無し。
対応 SageMath と前回の保存解による全鍵の復元のプログラミングによる検証も通過した。

## 冪根の個数の表記統一（2026-09-06）

有限判定の成果 `9d8bd5f32` は main への包含を確認した。
必須の表記統一を一件行い、冪根の個数の上下界と最後の等号の根拠を各式の行末へ明示した。
LLM による検証: 上下界の合成・仮定・式・参照を保存している。
対応 SageMath・本文一括検査・PDF・522 検算の対応、Lean 全体ビルドと登録 1,635 定理の未証明依存のプログラミングによる検証は全て通過した。
次の本文レビューは「冪が 1 でない 1 の冪根があるとき、冪の和は零元である」から続ける。

## 語長一の局在形と語長三の固定項の両立（2026-09-06 の 231 回目）

一辺二・三の既存 7,085 鍵では、語長二の制約を外すと語長一の局在形と語長三の固定項は両立する。
10,378 行・11,698 未知数の両階数は 10,011。解を保存し、全鍵の値を復元した。
一解の切断旗を含む積は語長一が 122 項、三が 242 項。項数の最小性は主張しない。
実装は `sagemath/check/parity-identity-simple-cycle-arc-orientation-length-one-three-fixed-interior/`。
初回のプログラミングによる検証は終了コード 0。保存した証拠を読む再実行も終了コード 0 で通過し、両階数と全鍵の復元を再現した。
LLM による検証: 語長二は自由未知数のままであり、三語長同時の両立まで結論しない。
一般命題と Lean 二版は未追加。次は語長一の制約を外し、語長二・三だけの両立を判定する。
レビュー修正は `d60b41b2d` を main へ push し、fetch 後の包含を確認した。
本文一括検査・PDF・522 検算の対応・構成分離、Lean 全体ビルドと登録 1,635 定理の
未証明依存のプログラミングによる検証は通過した。報告本文の組み立ては、未対応の `--help` 指定で
終了コード 2 となった後、正規の二引数で成功した。送信はしていない。

## 前進前レビュー（2026-09-06 の 231 回目）

LLM による検証: 冪根の個数の本文・Lean 二版・SageMath は上界と相異なる根の下界を
合成しており、後続で繰り返し使うため空の四則主張としては削除しない。
本文末尾の段階表だけが前回の同時制約を判定中としていたため、非可解という保存済みの結果へ揃えた。
プログラミングによる検証: 冪根の個数の SageMath、本文一括検査・PDF・対応検査、
Lean 全体ビルドと登録 1,635 定理の未証明依存検査は通過。

## 根の順次分解の表記統一（2026-09-06）

有限探索の成果 `5650890b7` は main への包含を確認した。
必須の表記統一を一件行い、因数定理で得る商の分解の根拠と適用対象を等式の行末へ明示した。
LLM による検証: 帰納法・仮定・式・参照は保持されている。
対応 SageMath・本文一括検査・PDF・521 検算の対応・Lean 全体ビルドと登録 1,635 定理の未証明依存のプログラミングによる検証は全て通過した。
次の本文レビューは「1 の冪根の全体はちょうど指数個の元を持つ」から続ける。

## 語長一・二の局在形と語長三の固定項の両立（2026-09-06 の 230 回目）

語長一・二の局在形を保って語長三だけに固定項を加える系も、同じ行列サイズと階数 11,612・拡大階数 11,613 で非可解だった。鍵の式 947 本と追加制約 1,942 本の左核の矛盾証拠を保存した（`parity-identity-simple-cycle-arc-orientation-length-three-fixed-interior-joint`）。次は語長二の制約を外し、語長一の局在形と語長三の固定非切断項だけの両立を判定する。
初回のプログラミングによる検証は終了コード 0 で通過した。保存した証拠を含む再実行も終了コード 0 で通過した。本文一括検査・PDF・521 検算の対応・構成分離、Lean 全体ビルドと登録 1,635 定理の未証明依存のプログラミングによる検証も通過した。
LLM による検証: 右辺の語長別の零・固定項の区別が候補式に一致し、結論は同時制約の非可解性に限る。
一般命題と Lean 二版は未追加。前進前のレビュー修正 `0f47307c1` は main への包含を確認した。

## 前進前レビュー（2026-09-06 の 230 回目）

根の多項式の相異なる一次因子の取り出しについて、SageMath を本文の途中の等式へ対応させた。
LLM による検証: 本文と Lean 二版は同じ帰納法であり、根の個数を供給する主張として維持した。
プログラミングによる検証: 対応 SageMath、本文一括検査・PDF・520 検算の対応、Lean 全体ビルドと登録 1,635 定理の未証明依存検査が通過した。

## 指定因子の取り出しの表記統一（2026-09-06）

有限探索の成果 `2d58cfae5` は main への包含を確認した。
必須の表記統一は一件だけ行い、帰納法の仮定から得る分解の根拠を等式の行末へ明示した。
LLM による検証: 帰納法・二場合・係数上界・適用定理の参照を保持している。
対応 SageMath・本文一括検査・PDF・520 検算の対応・Lean 全体ビルドと登録 1,635 定理の未証明依存のプログラミングによる検証は全て通過した。
次の本文レビューは「根の多項式は相異なる根の一次因子を順に取り出せる」から続ける。

## 短い三語長への非切断項の同時固定（2026-09-06 の 229 回目）

一辺二・三の既存 7,085 鍵では、語長一・二・三すべてに「上辺所属と左辺所属の積の語位置和」を
固定し、残差を各語長の切断旗を含む二次式で表す単一系は非可解だった。
12,220 行・12,458 未知数の系で階数 11,612・拡大階数 11,613。
鍵の式 947 本と追加制約 1,942 本を足す左核の矛盾証拠を `obstruction.json` へ保存した。
実装は `sagemath/check/parity-identity-simple-cycle-arc-orientation-short-word-fixed-interior/`。
初回のプログラミングによる検証は通過し、観測固定と保存証拠を含む全再実行も終了コード 0 で通過した。
一般命題と Lean 二版は未追加。どの語長が単独で矛盾するかは未判定である。
LLM による検証: 右辺の固定項と本文の候補式は対応し、非可解性の射程はこの制約系に限る。
次は語長一・二の非切断項を零とする既存の局在形を保ち、語長三にだけ固定項を加える単一系を判定する。
レビュー修正 `ae91244f3` は前進前に main への包含を確認した。
本文一括検査・PDF・520 検算の対応・構成分離、Lean 全体ビルドと登録 1,635 定理の
未証明依存のプログラミングによる検証も通過した。

## 前進前レビュー（2026-09-06 の 229 回目）

指定した一次因子の取り出しの SageMath を本文の各等式へ対応させた。
LLM による検証: 本文と Lean 二版は同じ帰納法であり、係数上界の供給に使う主張なので維持した。
プログラミングによる検証: 対応 SageMath、本文一括検査・PDF・519 検算の対応、
Lean 全体ビルドと登録 1,635 定理の未証明依存検査が通過した。
SageMath の初回は作業ディレクトリ違いで終了コード 2、プロジェクト直下で同じ処理を再実行して復旧した。

## 取り出した根の相異性の表記統一（2026-09-06）

今回の有限計算は `7b61efe1c` を main へ push し、fetch 後の包含を確認した。
続いて必須の表記統一を一件行い、背理法で使う残りの因子の非零性の根拠を行末へ移した。
LLM による検証: 仮定・等式の鎖・論法・参照は保存されている。
プログラミングによる検証: 対応 SageMath、構造化テキスト一括検査、PDF、519 検算の対応、
Lean 全体ビルドと登録 1,635 定理の未証明依存検査が通過した。
次の本文レビューは、一次因子の積から指定した因子を取り出す証明から続ける。

## 語長三の方向対係数の位置共通化（2026-09-06 の 228 回目）

語長三の非切断項を語位置内に限り、同じ方向対の係数を三位置で共通にしても、
一辺二・三の既存 7,085 鍵を復元できた。9,129 行・11,254 未知数の両階数は 8,817。
得た一解の非切断項は各位置の上辺所属と左辺所属の積の和だけで、切断旗を含む積は 101 項。
一般命題と Lean 二版は未追加。観測固定後の全再実行も終了コード 0 で通過し、同じ階数・共通係数・全鍵の復元を再現した。
実装は `sagemath/check/parity-identity-simple-cycle-arc-orientation-length-three-shared-direction-coefficients/`。
プログラミングによる検証: 構造化テキスト一括検査・PDF・519 検算の対応・構成分離、
Lean 全体ビルドと登録 1,635 定理の未証明依存検査も通過した。
観測固定用の編集は初回に相対パスの誤りで失敗し、作業ディレクトリを正して復旧した。
次は語長一・二・三でこの非切断項を固定し、残差を各語長の切断旗を含む二次式で同時に表せるかを判定する。
LLM による検証: 端点内の零固定と語位置間の係数一致を区別して式と構成を照合した。
方向対どうしの一致、係数の一意性、項数の最小性は主張しない。
レビュー修正は `6961be19a` で main に反映し、fetch 後の包含を確認した。
前回の有限計算のプログラミングによる検証も、両階数 8,819 と全鍵の復元を再現した。

## 前進前レビュー（2026-09-06 の 228 回目）

残りの因子の非零性について、SageMath の準備を本文の係数四段・評価五段へ揃え、
最後の商の非零性も直接判定するようにした。従来の終点の assertion は保持した。
Lean 具体版と必要十分版への特殊化の係数計算も同じ四段へ展開した。
LLM による検証: 本文の仮定・論法・参照は維持し、根の相異性を支えるため主張を残した。
プログラミングによる検証: 対応 SageMath、本文一括検査と PDF、518 検算の対応、
Lean 全体ビルドと登録 1,635 定理の未証明依存検査が通過した。

## 分解の残りの因子の非零性の表記統一（2026-09-06）

有限計算の前進は `a0acce809` を main へ push し、fetch 後の包含を確認した。
続いて必須の表記統一を一件行い、残りの因子の値の一致と非零性を一続きにした。
LLM による検証では、既存の準備・等式・仮定・適用定理を保存したことを確認した。
対応する SageMath、構造化テキスト一括検査、PDF、518 検算の対応、Lean 全体ビルドと
登録 1,635 定理の未証明依存のプログラミングによる検証は全て通過した。

## 語長三の同じ署名位置への制限（2026-09-06 の 227 回目）

一辺二・三の既存 7,085 鍵で、切断旗を含む 1,150 項と同じ署名位置内の
単一辺所属同士の 30 項だけを許した系は可解だった。9,129 行・11,278 未知数の
両階数は 8,819 で、全鍵の値を復元した。異なる位置間の非切断項 160 項を同時に除ける。
実装は `sagemath/check/parity-identity-simple-cycle-arc-orientation-length-three-same-position-sufficiency/`。
一解は切断旗を含む積 104 項と語の同じ位置の単一辺所属同士の積 5 項で、端点内の非切断項は零。
語は始点を含み終点を含まず、語と端点対を別々に正準化するので、五つの特徴ブロック間に
実頂点の同定を追加しない。各許可積は一頂点内であり、同じ頂点内の積による十分性が従う。
一般命題と Lean 二版は未追加。次は非切断項を語の三位置だけに限り、
同じ四方向の組の係数を三位置で共通にしても可解かを判定する。

LLM による検証: 一次因子の積の係数上界は後続で使われ、本文・Lean 二版・SageMath は
同じ帰納法を持つ。空の四則主張の削除対象ではなく、前進前の修正点は無し。
プログラミングによる検証: 前回計算の両階数 8,825 と全鍵復元、係数上界の SageMath、
構造化テキスト一括検査、518 検算の対応、新規対象の構成 48 本と検算 1 本の分離、
Lean 全体ビルドと登録 1,635 定理の未証明依存検査は通過した。
今回計算の初回と観測固定後の全再実行は終了コード 0 で成功した。変更後の本文の一括検査と PDF 生成も通過した。
PDF は 412 ページで、未解決参照・組めない文字・版面外の行は零。
初回起動の終了コード 2 は作業ディレクトリと相対パスの不一致によるもので、
プロジェクト直下で同じ SageMath スクリプトを実行して復旧した。
本文の npm 起動も最初はリポジトリ直下で終了コード 254（package.json 不在）となり、構造化テキストのディレクトリで再実行して復旧した。

## 語長三の単一辺所属の積だけで足りるか（2026-09-05 の 226 回目）

前進前の回収・レビュー修正はコミット `22767580a` を main へ反映し、fetch 後の包含を確認した。
今回の対象は同じ 7,085 鍵での一つの十分性判定である。切断旗を含む 1,150 項と
単一辺所属同士の 190 項だけを残し、他の五種類 938 項を同時に零固定する。
実装は `sagemath/check/parity-identity-simple-cycle-arc-orientation-length-three-single-membership-sufficiency/`。
初回の有限計算は両階数 8,825 の可解な系を与え、全 7,085 鍵の値を復元した。
得た一解は切断旗を含む積 83 項と単一辺所属同士の積 21 項からなる。最小項数とは言わない。
従って他の五種類の非切断項は同時に除ける。切断旗と二重辺・選択辺所属との積は残るので、
それらの所属ビット自体が不要だとは言えない。観測を assertion に固定した全再実行も終了コード 0 で通過し、両階数 8,825 と全 7,085 鍵の復元を再現した。
一般命題と Lean 二版は未追加。次は、切断旗を含まない積を同じ内部頂点または同じ切断端点の単一辺所属同士だけへ制限しても可解かを判定する。
プログラミングによる検証: 517 検算の本文対応、新規対象の構成 47 本と検算 1 本の分離、
既存証明の SageMath、Lean 全体ビルドと登録 1,635 定理の未証明依存検査が通過した。
更新後の構造化テキスト一括検査と PDF 生成も通過した。PDF は 412 ページで、
未解決参照・組めない文字・版面外の行は零。台帳からの報告本文の組み立ても通過した。
LLM による検証: 係数を固定した一解の削減ではなく、残る未知数についての存在判定である。
式変形の統一は一次因子の積の係数上界一件で、適用対象を行末へ移して帰納法・量化・式・参照を保存した。
初回は作業ディレクトリと相対パスの不一致でファイル作成と SageMath 起動が失敗した。
相対パスを正し、同じ処理を再実行した。

## 前回成果の回収と前進前レビュー（2026-09-05 の 226 回目）

前回の未コミット成果を保持し、六種類の独立除去計算を固定 assertion 付きで再実行した。
全 assertion が終了コード 0 で通過した。初回と同じ階数・五系の全鍵の復元・一系の左核証拠を再現した。
LLM による検証: 「一次因子は消去できる」は商の一意性と重複度上界から繰り返し使われ、
空の四則主張ではない。Lean 二版と SageMath が本文の準備を短縮していたため、
一次式の係数三本・積の係数五段・帰納法の一歩六段へ揃えた。
必要十分版は積の可換性を要さず、環の仮定で同じ論法が通る形にした。
SageMath のプログラミングによる検証は各行の追加後も全段が通過した。
プログラミングによる検証: Lean 二版と後続を含む全体ビルド、登録 1,635 定理の
未証明依存検査が通過した。構造化テキスト一括検査・PDF・516 検算の対応・
構成分離も通過した。前進前に回収成果とレビュー修正を `22767580a` で main へ保存し、包含を確認した。

## 語長三の純二次式で必要な所属の積（2026-09-05）

一辺二・三の既存 7,085 鍵では、切断旗を含まない単一辺所属同士の積 190 項を
全て除くと純二次式の系は非可解になる。9,129 行・12,186 未知数、階数 8,826・
拡大階数 8,827。鍵の式 40 本と弧型の式 28 本を足す左核ベクトルが矛盾を与える。
他の五種類（選択辺同士・選択辺と二重辺・選択辺と単一辺・二重辺同士・二重辺と単一辺）は、
それぞれ単独では全て除ける。各系の両階数は 8,828 で、全鍵の値を復元できた。
実装は `sagemath/check/parity-identity-simple-cycle-arc-orientation-length-three-quadratic-kind-removal/`。
次は、切断旗を含まない積を単一辺所属同士だけに制限しても足りるかを判定する。

LLM による検証: 六つの単独除去を同時除去と混同せず、必要なのはこの種類から
少なくとも一項を残すことであって個々の係数の不可欠性ではないことを明記した。
一般の辺長の命題と Lean 二版は未追加。式変形の統一は「一次因子は消去できる」の
最後の等号一件で、論法・量化・参照を保存した。
プログラミングによる検証: 初回は六系の階数、可解な五系の全鍵の復元、非可解な系の
左核の証拠が通過した。階数を assertion へ固定した全再実行も終了コード 0 で通過し、六系の階数・五系の全鍵の復元・一系の左核証拠を再現した。
構成と assertion の分離、516 検算の対応、一次因子の消去の SageMath、
Lean 全体ビルドと登録 1,635 定理の未証明依存検査も通過した。
補助編集と対応検査の初回には作業ディレクトリに対して誤った相対パスを指定した。
対象パスを直して同じ操作を実行し、本文編集と対応検査を完了した。

## 前進前レビュー（2026-09-05 の 225 回目）

「一次因子との積の係数は、上の番号で零である」の Lean 具体版・必要十分版を、
本文の準備三段と本体六段に対応する計算へ直した。従来は積の係数を短い公式へ
畳んでいた。必要十分版では積の可換性を使わないため、仮定を可換環から環へ落とした。
LLM による検証: この係数上界は一次因子の積と重複度上界の後続証明で繰り返し
使われており、空の四則主張ではない。前回の純二次式の記録は有限データに限る結論と整合する。
プログラミングによる検証: 前回の純二次式計算は階数 8,828 と全 7,085 鍵の復元を再現した。
係数上界の SageMath、構造化テキスト一括検査、515 検算の本文対応が通過した。
Lean の初回コンパイルは反対角和から区間和への書換えの照合に失敗した。
同じ和の組添字版補題へ書換えを明示し、数学的主張と本文は変更していない。
修正後の Lean 全体ビルドと登録 1,635 定理の未証明依存検査、PDF 生成も通過した。

## 語長三の定数項と一次項を除いた存在判定（2026-09-05）

一辺二・三の既存 7,085 鍵では、語長三の値を相異なる二成分の積だけで表せる。
定数項・一次項の 69 列を除いた系は 9,129 行・12,376 未知数で、係数行列と
拡大係数行列の階数はともに 8,828。各弧型を式で評価し、全鍵の値を復元した。
実装は `sagemath/check/parity-identity-simple-cycle-arc-orientation-length-three-pure-quadratic-existence/`。
得た一解の 109 項中、切断旗を含む積は 67 項。項数の最小性や各係数の不可欠性は未判定。
前回の局在系の非可解性と合わせ、定数項・一次項は不要だが、切断旗を含まない二次項を
全て除くことはできない。次はそれらの項を特徴の種類ごとに零固定して切り分ける。

LLM による検証: 列の除去が前回から変更した条件の全てであり、他の語長と閉路型の
値は自由に残ることを確認した。有限探索であり、一般命題と Lean 二版は未追加。
式変形の統一は冪根の集合の逆包含の一件で、論法と参照を保存した。
プログラミングによる検証: 初回の階数・解の残差・各弧型と全鍵の値の復元は通過。
階数の assertion を追加した全再実行も終了コード 0 で通過し、同じ階数と全鍵の復元を再現した。
構造化テキスト一括検査、PDF、515 検算の本文対応、構成と assertion の分離、冪根集合の SageMath、
Lean 全体ビルドと登録 1,635 定理の未証明依存検査が通過した。
PDF は 412 ページで、未解決参照・組めない文字・版面外の行は零。報告本文のローカル組み立ても通過した。

## 前進前レビュー（2026-09-05 の 224 回目）

LLM による検証で、台帳末尾に残った固定アカウント指定を、起動口が渡した
`CODEX_HOME` を保持する runbook と実装へ揃えた。前回の二次式存在判定は
列挙した有限集合に射程を限定しており、一般証明の完了とは記していない。
「1 乗して 1 になる代数的数は 1 だけである」は集合の確定と根の個数の帰納法の
出発点を担うため維持した。
本文末尾の段取りに残っていた「署名の圧縮を切り分けている」という現在地も、
語長三の二次式の項の必要性を切り分ける段へ更新した。
プログラミングによる検証: 前回の二次式存在判定は階数 8,828 の一致と全 7,085 鍵の
復元を再現した。構造化テキスト一括検査、PDF、514 検算の本文対応、冪根の集合の
SageMath、Lean 全体ビルドと登録 1,635 定理の未証明依存検査が通過した。
起動アカウントを保持する実装の試験も通過した。
...
## 語長三の局在を要求しない二次式の存在判定（2026-09-05）

語長三だけについて、定数項・一次項・全ての相異なる二成分の積を許す有限存在判定を
`sagemath/check/parity-identity-simple-cycle-arc-orientation-length-three-quadratic-existence/` に追加した。
他の語長と閉路型の値は自由に残し、9,129 行・12,445 未知数の系を構成する。
係数行列と拡大係数行列の階数はともに 8,828 で、得た解から全 7,085 鍵の値を復元できた。
従って次数二そのものではなく、前回の係数の零固定条件が障害だった。一般命題と Lean 二版は未追加。
初回と観測を固定する assertion の追加後の全再実行がともに終了コード 0 で通過した。
次は定数項・一次項を零に固定して、全ての二次項だけで可解かを判定する。

LLM による検証: 前回の有限局在系とその矛盾証拠、今回の候補が次数二以下の全ての
多項式を含むことを確認した。既存の冪根の商の同定と一次因子の消去は後続から使われる
ため維持し、レビュー修正は無し。式変形の統一は、商の根での非零性の準備三行を対象とした。
プログラミングによる検証: 前回の語長三単独の局在検算は階数 8,816・拡大階数 8,817 と
同じ左核の台を再現した。変更後の構造化テキスト一括検査、PDF、514 検算の本文対応、
変更した証明の SageMath、Lean 全体ビルドと登録 1,635 定理の未証明依存検査は通過した。
新規スクリプトの初回の SageMath は、系の構成・階数・解の残差・全鍵の値の復元が通過した。
階数を固定する assertion の追加後の全再実行でも、同じ階数と全鍵の復元を再現した。
PDF は 412 ページ、未解決参照・組めない文字・版面外の行は全て零。
...
（観測の出力と assertion）に分ける。**下流は `construction.sage` だけを読む。**

分ける前は `check.sage` が上流の `check.sage` を読んでいたため、弧署名の検算を一本走らせるだけで
先行 36 本の assertion が毎回再実行され、読み込みだけで 13 分かかっていた。自動ループの tick は
これで 2700 秒の上限に当たり exit 124 で落ちた（2026-09-05 07:05 の tick で実際に起きた）。
**原因は外部データの取得ではなく、検査コードと再利用構成が未分離だったことである。**
分離後、同じ連鎖の読み込みは 2 分 44 秒になった。

守ること。

- `construction.sage` に**関数の外の `assert` と `print` を置かない**（関数の中の `assert` は残す）。
- `check.sage` は自分の `construction.sage` を読み、**上流の `check.sage` は読まない**。
- 構成へ移した文にもとから `assert` が付いていたら、その文を原文のまま `check.sage` へも置く。
  **assertion を減らさない**（先行検算は日次監査が `check.sage` を全数で回して維持する）。
- 合同系の行は、列を全て並べた組ではなく**奇数回現れる型の台**だけを作る。
  7,085 行 × 約 1 万列を Python の組で作ると、それだけで四変種が 30 分を超える。
- 形が保たれているかは `bash scripts/verify-upstream-load-and-roadmap.sh --published` が検査する
  （検査器自身の回帰試験は `scripts/test-verify-upstream-load-and-roadmap.sh`）。
...
## 前進前レビュー（2026-09-05 の 222 回目）

台帳末尾に残っていた旧モデル交互実行・上限時切替を、runbook の固定モデル指定へ修正した。
LLM による検証: 前回の語長一・二・三の同時局在系と左核の矛盾証拠を読み、
有限データに限る結論と一致することを確認した。次の本文レビュー対象である
「零でない代数的数を正の個数だけ足した有限和は零でない」は、標数零と零因子の不存在を
合成して後続の重根排除を支えるので、空の四則操作としては削除しない。
プログラミングによる検証: 前回の同時局在検算を再実行し、階数と左核の矛盾証拠を再現した。
構造化テキスト一括検査、PDF、512 検算の本文対応、Lean 全体ビルドと
登録 1,635 定理の未証明依存検査が通過した。

## 前進前レビュー（2026-09-05 の 221 回目）

LLM による検証で、前回の合同系は拘束前 10,176 行・拘束後 11,554 行であるのに、
台帳が拘束前を 11,554 行としていた不整合を見つけて修正した。観察ノートにも、
語長二の局在解と語長一・二の同時局在解の存在が欠けていたため、有限検算の射程を明記して反映した。
対象の本文「行列式の位相和は反転対と単純通過で層別される」とその位相付き寄与の定義を読み、
層別公式と値の帰属を述べているため「何も言っていない主張」の削除対象ではないと判断した。

プログラミングによる検証: 前回未実施だった語長一・二の同時局在検算を assertion 固定後に再実行し、
全 assertion が通過した。構造化テキスト一括検査、PDF、SageMath 対応、Lean 全体ビルド、
登録 1,635 定理の未証明公理への非依存検査も通過した。

## 現在の到達点（2026-09-05 の 222 回目の tick 時点）

一辺二・三の既存 7,085 鍵で、語長三だけでも切断旗を含む二次項だけの表示は存在しない。
他の語長と閉路型の値を自由未知数のまま残した系は 9,129 行・11,248 未知数、
係数行列と拡大係数行列の階数は 8,816 と 8,817。左核の証拠は鍵の式 12 本と
弧型の局在式 8 本を足し、全係数が零・右辺が一となることを示す。
実装と再現手順は `sagemath/check/parity-identity-simple-cycle-arc-orientation-length-three-wrap-localization/`。

LLM による検証: 結論はこの有限データにおける局在二次式の不存在に限られ、
局在させない一般の二次式の不存在を意味しない。次は語長三だけで定数項・一次項・
全ての二次項を許し、局在条件と次数二のどちらが障害かを切り分ける。
一般命題と Lean 二版は未追加。式変形の統一は反復和の非零性の根拠を適用行末へ移した一件。
プログラミングによる検証: 初回の有限計算、構造化テキスト一括検査、PDF、513 検算の対応、
構成 45 本と検算 1 本の分離、反復和の SageMath、Lean ビルドと登録 1,635 定理の
未証明依存検査が通過。観測を assertion へ固定した後の再実行も終了コード 0 で通過し、
階数と矛盾証拠の台を再現した。観察ノート反映後の構造化テキスト一括検査と PDF も通過。
PDF は 412 ページ、未解決参照・組めない文字・版面外の行はいずれも零。
台帳からの報告本文の組み立てもローカルで通過した（送信はしていない）。

## 現在の到達点（2026-09-05 の 221 回目の tick 時点）

一辺二・三の既存 7,085 鍵について、語長一・二・三を同時に切断旗を含む二次項だけの和で
書く候補は非可解である。語長三の対象弧型は 2,044 種、特徴 68 ビット、許す二次単項式は
1,150 個。零固定係数を未知数から除いた系は 12,220 行・12,458 未知数で、係数行列と
拡大係数行列の階数は 11,612 と 11,613。左核ベクトルと係数行列の積が零、右辺との積が一で
あることもプログラミングによる検証で確認した。
実装と再現手順は `sagemath/check/parity-identity-simple-cycle-arc-orientation-length-three-joint-localization/`。

LLM による検証では、零固定係数の除去が従来の拘束系と同値であり、語長三だけの局在の
不存在までは結論できないことを確認した。次は語長三だけに局在を課し、単独の障害か、
語長一・二との同時拘束の障害かを切り分ける。有限探索の結果であり、一般命題と Lean 二版は未追加。
式変形の統一は「単位元を正の個数だけ足した有限和は零でない」の前置き参照を行末へ移した一件。
プログラミングによる検証: 語長三の初回と assertion 固定後の再実行が通過した。
構造化テキスト一括検査、512 検算と本文ラベルの対応、構成 45 本と検算 1 本の分離、
有限和の非零性の SageMath、Lean 全体ビルドと登録 1,635 定理の未証明依存検査も通過した。
PDF は 412 ページ、未解決参照・組めない文字・版面外へ出た行はいずれも零。
台帳からの報告本文の組み立てもローカルで通過した（送信はしていない）。

## 現在の到達点（2026-09-05 の 220 回目の tick 時点）

**語長 $1$ と語長 $2$ の切断旗局在の拘束を同時に課しても連立系は可解で、両語長の弧型の値が
同時に「切断旗を含む二次単項式だけの和」で書ける単一の解が存在する。**
弧型ごとの値と両語長の式の係数を同時に未知数とする単一の連立 $\mathbb F_2$ 線型系
（orient_d 合同系 $7{,}085$ 行＋語長 $1$ の各弧型での「値＝二次式」の等式 $1{,}249$ 行＋
語長 $2$ の同じ等式 $1{,}842$ 行。未知数 $12{,}686$）に、両ブロックの定数項・一次係数・
切断旗非関与の二次係数（語長 $1$: $946$ 中 $496$、語長 $2$: $1{,}540$ 中 $780$）を全て $0$ に
固定する拘束行 $1{,}378$ 本を積んでも可解（階数 $11{,}194$）で、強制された係数座標は固定した
拘束座標にちょうど一致し、拘束の外に新たな強制は生じない。拘束後の正準代表は語長 $1$ が
定数項 $0$・一次項 $0$ 個・二次項 $16$ 個、語長 $2$ が定数項 $0$・一次項 $0$ 個・二次項 $184$ 個で、
いずれも全て切断旗を含む積である（語長 $2$ の二次項数は単独判定の $52$ 個から増え、
step0 を含む項も現れる。語長 $1$ の拘束を同時に課すと核が縮み、正準化で消去できる方向が
減るためで、局在そのものは保たれる）
（`parity-identity-simple-cycle-arc-orientation-length-one-two-joint-localization`。観測を assert へ
固定した。一回の実行は約 $13$ 分で、固定後の再実行は tick の締切に収まらず未実施。
日次監査が `check.sage` を全数で回して確認する）。
**次は同じ局在拘束を語長 $3$ の弧型へ延ばし、切断旗局在の形が全語長で同時に成り立つかを調べる。**

## 現在の到達点（2026-09-05 の 219 回目の tick 時点）

**切断旗非関与の二次項を消せる解は解空間に存在し、語長 $2$ の局在の破れは正準化の副作用である。**
連立 $\mathbb F_2$ 線型系（orient_d 合同系＋語長 $2$ の各弧型での「値＝二次式」の等式）に、
切断旗非関与の二次係数 $780$ 個（相異なる積 $1{,}540$ 個中）を $0$ に固定する拘束行を積んでも
可解（階数 $9{,}390$）で、強制された係数座標は固定した $780$ 個にちょうど一致し、拘束の外に
新たな強制は生じない。拘束後の正準代表は定数項 $0$・一次項 $0$ 個・二次項 $52$ 個で、
全て切断旗を含む積（step0 を含む項は $0$ 個）。さらに定数項・一次係数・切断旗非関与の
二次係数を同時に $0$ に固定（階数 $9{,}447$）しても可解なので、語長 $2$ の弧型の値は
語長 $1$ と同じ「切断旗を含む二次単項式だけの和」で書ける解が存在し、局在した二次式の形は
語長を跨いで保たれる
（`parity-identity-simple-cycle-arc-orientation-length-two-wrap-localization`。観測を assert へ
固定し、固定後に再実行して一致を確認した。一回の実行は約 $4$ 分）。
**次は語長 $1$ と語長 $2$ の切断旗局在の拘束を同時に課した単一の解の存在を判定し、
成り立てば語長 $3$ 以上へ延ばす。**

## 現在の到達点（2026-09-05 の 218 回目の tick 時点）

**語長 $2$ でも二次式で書ける解は存在するが、切断旗への完全な局在は保たれない。**
語長 $1$ の二次式の正準化を語長 $2$ の弧型（弧型 $1{,}842$ 種・閉路型 $16$ 種）へ延ばした。
二つのステップと両端点の $56$ 特徴ビットについて、一次式（定数項＋ $56$ 係数。
未知数 $10{,}155$・階数 $8{,}520$）は非可解、相異なる二成分の積 $1{,}540$ を加えた二次式
（未知数 $11{,}695$・階数 $8{,}622$）は可解である。係数座標へ射影した核（次元 $1{,}468$）で
消去した正準代表（剰余類だけで決まる）は定数項 $0$・一次項 $0$ 個・二次項 $39$ 個だが、
切断旗を含む積は $25$ 個に留まり（$14$ 項が切断旗非関与）、項は反転正準化の後側
（step1・end1）に偏って step0 を含む項は $0$ 個である。強制された係数は $0$ 個で、
正準代表の二次式を合同系へ代入しても残りは可解である
（`parity-identity-simple-cycle-arc-orientation-length-two-quadratic-solution`。観測を assert へ
固定し、固定後に再実行して一致を確認した。一回の実行は約 $6$ 分）。
**次は解空間の側で切断旗非関与の二次項を消せる解が存在するか（局在が正準化の副作用か、
解空間の障害か）を判定する。**

## 現在の到達点（2026-09-05 の 217 回目の tick 時点）

**語長 $1$ の弧型の値を書く二次式の正準代表は、切断旗を含む二次単項式 $16$ 個の和である。**
係数座標（定数項 $1$・一次 $44$・相異なる積 $946$ の計 $991$）へ射影した核（次元 $946$）の
既約階段基底で特殊解の係数部分を消去した正準代表（剰余類だけで決まり、解の取り方に
依存しない）は、定数項 $0$・一次項 $0$ 個・二次項 $16$ 個で、全ての二次項が切断旗を含む積
（端点第二成分の切断旗を含むものが全 $16$ 項。端点同士 $10$・ステップ×端点 $6$）である。
核のどのベクトルも動かさない強制された係数は $0$ 個なので個々の係数は解空間の不変量では
ないが、この二次式が定める語長 $1$ の値を合同系へ代入しても残りは可解である
（`parity-identity-simple-cycle-arc-orientation-length-one-quadratic-solution`。観測を assert へ
固定し、固定後に再実行して一致を確認した。一回の実行は約 $4$ 分）。
**次は同じ正準化を語長 $2$ の弧型へ延ばし、切断旗に局在した二次式の形が語長を跨いで
保たれるかを調べる。**

## 現在の到達点（2026-09-05 の 216 回目の tick 時点）

**語長 $1$ の制限は、一次式では解空間のどの解でも書けないが、二次式で書ける解は存在する。**
代表を固定せず、弧型ごとの値と式の係数を同時に未知数とする連立 $\mathbb F_2$ 線型系
（orient_d 合同系＋語長 $1$ の各弧型での「値＝定数項つき低次式」の等式）で判定すると、
一次式（定数項＋ $44$ 係数。未知数 $10{,}143$・階数 $7{,}988$）は非可解であり、
語長 $1$ の制限が一次式になる解は解空間のどこにも存在しない（代表の取り方の問題ではない）。
一方、相異なる二成分の積 $946$ を加えた二次式（$990$ 特徴。未知数 $11{,}089$・階数 $8{,}020$）は
可解で、語長 $1$ の弧型の値が単一ステップと両端点の特徴ビットの二次式で書ける解が存在する
（`parity-identity-simple-cycle-arc-orientation-length-one-existence`。観測を assert へ固定し、
固定後に再実行して一致を確認した。一回の実行は約 $3$ 分）。
**次はその二次式の解を一つ構成して係数の形を読み、語長 $2$ 以上の弧型の値の規則へ
延ばせるかを調べる。**

## 現在の到達点（2026-09-05 の 215 回目の tick 時点）

**語長 $1$ の弧型では、縮約代表の値の規則は特徴ビットの低次式では書けない。**
orient_d 署名の解の支持台を語長 $1$ へ層別すると、語長 $1$ の弧型は全 $1{,}249$ 種
（閉路型は無い）で、うち縮約代表の支持台は $222$ 種。単一ステップの向き・切断旗・
$D$ 所属と両端点の完全署名の $44$ 特徴ビットは直接衝突を持たないが、値は定数項つきの
$\mathbb F_2$ 一次式（階数 $37$）でも、相異なる二成分の積を加えた二次式
（$990$ 特徴・階数 $439$）でも書けない
（`parity-identity-simple-cycle-arc-orientation-length-one-rule`。観測を assert へ固定し、
固定後に再実行して一致を確認した。一回の実行は約 $7$ 分半）。値は解空間の代表の
取り方（貪欲な縮約の順序）に依存するので、**次は代表を固定せず、語長 $1$ の制限が
低次式になる解が解空間に存在するかを、式の係数も未知数に加えた連立 $\mathbb F_2$
線型系で判定する。**

## 現在の到達点（2026-09-05 の 214 回目の tick 時点）

**orient_d 署名の解の支持台は、縮約してもそのほとんどが内部頂点の $D$ 所属と切断旗の両方に関与する。**
前 tick の打ち切りで未追跡のまま残っていた検算
`parity-identity-simple-cycle-arc-orientation-support` を拾って完走させた。核ベクトルの貪欲な縮約で
生の解の $2{,}904$ 弧型（核の次元 $3{,}299$）が $2{,}519$ 弧型（閉路型 $87$・弧型 $2{,}432$）へ縮まり、
語長は $1$ から $8$ まで全域に分布（最多は語長 $4$ の $531$ 種）、内部の $D$ 所属が全て空の型は $79$、
切断旗が全て空の型は $20$ に留まる。観測を assert へ固定し、固定後に再実行して一致を確認した
（一回の実行は約 $7$ 分）。従って弧型ごとの値の閉じた式の候補は向きだけでは書けない。
**次は支持台を語長で層別し、語長の短い弧型（語長 $1$ が $222$ 種）から値の規則を読み取る。**

## 現在の到達点（2026-09-05 の 213 回目の tick 時点）

**頂点項の弧分解に必要なのは、弧に沿った向きと $D$ 所属の組の並びであり、選択 $C$ の情報は落とせる。**
完全署名の側から落とす四変種で、スロット名順の $E$ 所属（向き）・切断旗に $D$ 所属を組で添えた
orient_d は型 $10{,}098$ 種・階数 $6{,}799$・直接衝突なしで合同 $\mathbb F_2$ 線型系に解を持ち、
$C$ 所属も添えた orient_dc と型の種数・階数が一致する（$C$ 所属は向き・切断旗・$D$ 所属から決まる
情報を加えない）。$C$ 所属だけの orient_c は型 $10{,}061$・階数 $6{,}771$・直接衝突 $16$ 件で非可解、
向き・両所属を保って切断旗だけを落とした full_nowrap は型 $9{,}787$・階数 $6{,}491$・直接衝突なしで
可解である（`parity-identity-simple-cycle-arc-orientation-membership`。観測を assert へ固定し、
固定後に再実行して一致を確認した。四変種の一括実行は 212 秒）。
**次は orient_d 署名の解の支持台を調べ、弧型ごとの値を閉じた式へ近づける。**

## 現在の到達点（2026-09-05、上流入れ子の解消後）

**内部頂点へ所属情報を戻す四変種は、どれも頂点項の分解を保たない。** 曲がり/直進の型と切断旗に、
四スロットの $D$ 所属を加えると型 $10{,}059$ 種・階数 $6{,}760$・直接衝突 $25$ 件、$C$ 所属を加えると
$10{,}030$・$6{,}740$・$48$、両方を加えると $10{,}075$・$6{,}776$・$16$、型を四スロットの $E$ 所属へ
置き換えて向きだけを戻すと $10{,}061$・$6{,}771$・$16$ で、**四変種とも合同 $\mathbb F_2$ 線型系は非可解**
である（`parity-identity-simple-cycle-arc-interior-membership`。観測と同じ実行で assert へ固定した）。
従って足りないのは所属だけでも向きだけでもなく、**向きと所属を同時に保つ完全署名との残差**である。
次はその残差、すなわち完全署名から何を落とすと壊れるのかを、弧の内部での向きと所属の組の
並び方に絞って調べる。

実行が締切に収まらなかった原因は取り除いた（上の「検算の『構成』と『assertion』を分ける」）。
四変種の実行は $446$ 秒で終わる。
## 現在の到達点（2026-09-05 の 212 回目の tick 時点）

> 追記: **このキャッシュはこの取り込みで外した。** 上流連鎖を `construction.sage`（構成）と `check.sage`（assertion）へ分けたことで、上流の読み込みが 2 分 44 秒になり、四変種の実行が 446 秒で終わるようになったので、キャッシュが解く問題そのものが無くなった。手で消して作り直す必要のある派生ファイルを検証層へ置かない方がよい（消し忘れると古い上流の結果で PASS してしまう）。辞書索引による行構築の方は、奇数回現れる弧型の台だけを作る形として残っている。

**内部所属四変種の検算を上流再実行から切り離す割り直しを実装した。観測値はまだ無い。** `parity-identity-simple-cycle-arc-interior-membership/check.sage` は、鍵ごとの弧の生の署名語（圧縮前の頂点署名列・切断頂点の署名対・頂点項）を git 管理外の `arc-words-cache.sobj` へ保存し、キャッシュがあれば上流を読み込まずに使う（無ければ自身が上流を読み込んで同じコードで再生成する。**上流の検算を変更したらキャッシュを削除すること**）。行ベクトルの構築も、全型に対する `count` 走査（型 1 万種 × 鍵 7,085 件の二乗的走査が 29 分打ち切りの主因）から辞書索引の偶奇反転へ置き換えた。キャッシュ経路の正しさは、旧実装で観測済みの `interior_d` の値（型 $10{,}059$ 種・階数 $6{,}760$・直接衝突 $25$ 件・非可解）への一致 assert が検査する。初回のキャッシュ生成は上流連鎖の再実行を含み、並行する監査ジョブと CPU が競合した今回は締切に収まらず打ち切った。**次の tick はこの check を前面で実行してキャッシュを生成し（環境変数なしで全変種が走る。キャッシュ生成後の各変種は数分の見込みではなく実測で確かめること）、`interior_c`・`interior_dc`・`interior_orient` の観測値を overview へ記録して assert を確定させる。** 並列の式変形統一は締切のため見送った。

## 現在の到達点（2026-09-05 の 211 回目の tick 時点）

**`interior_c` 一変種の検算を前面で実行したが、約 29 分の実行が 10:05 のまとめ開始時刻までに完了せず、観測値の無いまま打ち切った。** 前進前レビューでは、210 回目の観察ノート追記が overview・台帳・MEMORY と一致しており、修正すべき点は無かった。一変種ずつへ割り直しても、上流検算の読み込み（実測 13 分）を毎回やり直す構造では締切に収まらない。**次の tick はまず、上流検算の読み込み結果（`joint_keys`: 鍵と頂点項の一覧）を一度ファイルへ保存し、変種実行がそれを読むだけで始められる形への割り直しを行うこと。** そのうえで `interior_c` から観測値と assert を確定させる。並列の式変形統一は締切のため見送った。

## 現在の到達点（2026-09-05 の 210 回目の tick 時点）

**`interior_d` の有限検査結果が観察ノートに欠けていた不整合を修正した。** `interior_c` 一変種の検算は上流検査を通過中に 09:10 のまとめ開始時刻へ達したため中断し、未完の観測値は記録していない。次の tick も `interior_c` の観測値と assert の確定から続ける。

## 現在の到達点（2026-09-05 の 209 回目の tick 時点）

**内部所属四変種のうち、内部頂点へ $D$ 所属だけを戻しても頂点項の弧分解は保たれないことを有限検査で確定した。** `interior_d` は型 $10{,}059$ 種・階数 $6{,}760$・直接衝突 $25$ 件で、合同 $\mathbb F_2$ 線型系は非可解だった。残る `interior_c`・`interior_dc`・`interior_orient` は 07:40 のまとめ開始時刻までに完了しなかったため未実行である。次の tick は検算を一変種ずつ実行できる形へ割り直し、`interior_c` から観測値と assert を確定させる。

## 前進前レビュー（2026-09-05 の 210 回目）

`interior_d` の有限検査結果は検算 overview・台帳・MEMORY に記録されていたが、観察ノート（`structured-latex/notes/kac-ward.ts`）だけが「内部所属情報のどの位置が必要かを切り分ける」で止まっていた。型 $10{,}059$ 種・階数 $6{,}760$・直接衝突 $25$ 件で非可解という帰結と、次に $C$ 所属だけを検査することを観察ノートへ追記した。検算実装の一変種選択・期待値照合と overview・台帳・MEMORY は一致していた。「何も言っていない主張」は無かった。

## 前進前レビュー（2026-09-05 の 208 回目）

直前 tick は三水準圧縮の帰結（三水準とも直接衝突あり・合同線型系非可解）を overview と台帳の「現在地」へは記録したが、観察ノート（`structured-latex/notes/kac-ward.ts`）が「次は圧縮する」で止まったままだったので、帰結の段落を追記した。台帳のセクション表の備考も「三水準圧縮は実装済み・未実行」のまま古かったので現状へ直した。「何も言っていない主張」は無かった。

## 現在の到達点（2026-09-05 の 208 回目の tick 時点）

**内部頂点の所属情報のどの位置が必要かを切り分ける四変種の弧署名検算を実装した。実行は次の tick が行う。** turnwrapword（内部頂点は曲がり/直進の型と切断旗だけ。非可解）と完全署名（解あり）の間に、interior_d（四スロットの $D$ 所属を加える）、interior_c（$C$ 所属を加える）、interior_dc（両方を加える）、interior_orient（型を四スロットの $E$ 所属（名前順）へ置き換え、曲がり/直進で落ちる向きの情報だけを戻す）の四水準を置き、各水準の直接衝突の有無と合同 $\mathbb F_2$ 線型系の可解性を観測する（`parity-identity-simple-cycle-arc-interior-membership`。overview に**未実行**と明記済み）。実行を開始したが、上流検算の読み込み 13 分に四変種の行列計算が加わって締切に収まらないと観測されたため打ち切った。**次の tick はレビュー後、最初にこの検算を実行し、観測値を overview へ記録して assert を確定させること**（assert は観測直後に固定し、再実行の確認は日次監査に委ねてよい。その場合は overview へその旨を書く）。切り分けの読み方: interior_d と interior_c の片方だけが可解ならその所属が必要な位置、両方非可解で interior_dc が可解なら組が必要、interior_dc も非可解で interior_orient が可解ならスロット名つきの向きが本体、全て非可解なら完全署名との残差（向き＋所属の同時保持）が必要である。

本文主張と Lean の追加対象はない。並列の式変形統一は締切のため見送った。

## 前進前レビュー（2026-09-05 の 207 回目）

直前 tick の `parity-identity-simple-cycle-arc-signature-compression` は、弧の寄与を半開区間で数える一方、圧縮統計で「両端頂点の完全署名」としていた後端へ終点直前の頂点署名を渡していた。寄与を数える半開区間は保ち、端点対だけを二つの切断頂点の署名へ修正した。「何も言っていない主張」は無かった。

## 現在の到達点（2026-09-05 の 207 回目の tick 時点）

**弧署名の三水準圧縮はいずれも頂点項の分解を保たないことを有限検査で確定した。** counts は型 $8{,}890$ 種・階数 $5{,}898$・直接衝突 $243$ 件、turnword は型 $9{,}225$ 種・階数 $6{,}198$・直接衝突 $160$ 件、turnwrapword は型 $9{,}998$ 種・階数 $6{,}708$・直接衝突 $65$ 件で、全て合同線型系に解が無かった（`parity-identity-simple-cycle-arc-signature-compression`）。とくに内部頂点の $D/C$ 所属だけを落とす turnwrapword も失敗するため、閉じた弧公式には所属情報の一部を残す必要がある。観測直後に結果を assert へ固定し、overview を PASS へ更新した。本文主張と Lean の追加対象はない。並列の式変形統一では、姉妹側の「$S_1^{(\pm)}$ と $S_2$ は実対称」にある三つの Pauli 行列積について、段落の参照を実際に行列表示を使う各等号の行末へ移した。次は直接衝突を比較し、内部所属情報のどの位置が必要かを切り分ける。

## 前進前レビュー（2026-09-05 の 206 回目）

直前 tick の `parity-identity-simple-cycle-boundary-arc-decomposition` を再実行し、弧型 $6{,}421$ 種・階数 $5{,}983$ で解あり・直接衝突なし・一つの解の支持台 $2{,}769$ 弧型という記録どおりの PASS を確認した（実測 13 分 22 秒）。検算コード・overview・観察ノート・台帳の射程は一致し、「何も言っていない主張」は無かった。修正は無かった。

## 現在の到達点（2026-09-05 の 206 回目の tick 時点）

**弧署名を粗い統計へ圧縮しても頂点項の分解が保たれるかを判定する三水準の検算を実装した。実行は次の tick が行う。** 水準は counts（弧の長さ・曲がり型頂点数・切断旗総数・両端頂点の完全署名の対）、turnword（曲がり/直進の型の列（反転同一視）・切断旗総数・端点署名対）、turnwrapword（頂点ごとの曲がり型と切断旗の列（反転同一視）・端点署名対。内部頂点の $D/C$ 所属だけを落とす）で、各水準の直接衝突の有無と合同 $\mathbb F_2$ 線型系の可解性を観測する（`parity-identity-simple-cycle-arc-signature-compression`。overview に**未実行**と明記済み）。上流検算の読み込みだけで 13 分と実測され、tick の締切内に実行と assert の確定が収まらないため、実装のみで止めた。**次の tick はレビュー後、最初にこの検算を実行し、観測値を overview へ記録して assert を確定させること**（再実行の重さを考え、初回の観測とassert 確定を同じ実行で済ませたければ、観測値の print を見てから assert を書き足し、確認の再実行は日次監査に委ねてもよい——ただしその場合は overview に「assert は観測直後に固定、再実行は監査で確認」と書く）。

本文主張と Lean の追加対象はない。並列の式変形統一は締切のため見送った。

## 前進前レビュー（2026-09-05 の 205 回目）

直前 tick の `parity-identity-simple-cycle-selector-membership` を再実行し、選択所属を加えた合同系が署名 $452$ 種・階数 $408$ で非可解、一辺二は解あり・一辺三は署名 $432$ 種・階数 $378$ で解なしという記録どおりの PASS を確認した。検算コード・overview・観察ノート・台帳の射程は一致し、「何も言っていない主張」は無かった。修正は無かった。

## 現在の到達点（2026-09-05 の 205 回目の tick 時点）

**単純閉路を $\partial D$ の頂点で切った完全な弧署名により、頂点項を弧型ごとの値の和へ分解できることを有限検査で確定した。** 辞書式最小の選択 $C$ を含む頂点署名を閉路順に並べ、向きの反転を同じ弧型とすると、一辺二・三の全 $7{,}085$ 鍵に現れる弧型は $6{,}421$ 種、所属行列の階数は $5{,}983$ で、合同線型系は解を持ち、直接衝突は無かった。一つの解の支持台は $2{,}769$ 弧型である（`parity-identity-simple-cycle-boundary-arc-decomposition`）。従って頂点局所署名に欠けていた閉路順序を弧の有限列として保持すれば分解できる。ただし完全な列を使う存在確認であり、閉じた公式ではない。次は弧の長さ・曲がり・端点の $D/C$ 配置・切断横断数へ署名を圧縮する。

新しい成果は有限系の存在検査と観察ノートであり、本文主張と Lean の追加対象はない。

並列の式変形統一では、姉妹側の「エルミート行列の指数は正定値」で、共役転置・可換な行列指数の積・零行列の指数への後置き参照を、実際に適用する各等号の行末へ移した。内容・式変形・根拠・参照は変えていない。

## 前進前レビュー（2026-09-05 の 204 回目）

直前 tick の `parity-identity-simple-cycle-joint-duplicate-adjacency` を再実行し、重複隣接旗を加えた合同系が署名 $394$ 種・階数 $323$ で非可解、一辺二は解あり・一辺三は署名 $270$ 種・階数 $234$ で解なしという記録どおりの PASS を確認した。検算コード・overview・観察ノート・台帳の射程は一致し、「何も言っていない主張」は無かった。修正は無かった。

## 現在の到達点（2026-09-05 の 204 回目の tick 時点）

**選択集合の所属を署名へ加えても、一辺三の単純閉路鍵の頂点項は頂点局所署名の関数の頂点和で書けないことを確定した。** 辞書式最小の選択 $C$（`key_terms` と同じ規約）への所属 $c_s$ を上下左右の四スロットへ加えた $16$ 変数の頂点署名でも、直接衝突は無いが合同系は署名 $452$ 種・階数 $408$ で解を持たず、一辺三は署名 $432$ 種・階数 $378$ で単独でも非可解だった（一辺二は署名 $172$ 種・階数 $122$ で解あり。`parity-identity-simple-cycle-selector-membership`）。従って不足は選択の頂点単位の情報ではなく、閉路に沿って伝わる大域情報にある。次は閉路を $\partial D$ の頂点で切った弧ごとの寄与など、頂点より粗い局所単位での分解を検査する。

新しい成果は有限系の非存在証明と観察ノートであり、本文主張と Lean の追加対象はない。締切残時間を考慮し並列の式変形統一は見送った。

## 前進前レビュー（2026-09-05 の 203 回目）

直前 tick の `parity-identity-simple-cycle-joint-local-sign-formula` を再実行し、合同系が署名 $274$ 種・階数 $246$ で非可解という記録どおりの PASS を確認した。検算コード・overview・観察ノート・台帳の射程は一致し、対象の本文主張は実質的な行列式の層別等式であって「何も言っていない主張」ではなかった。修正は無かった。

## 現在の到達点（2026-09-05 の 203 回目の tick 時点）

**重複隣接旗を加えても、一辺二と一辺三の単純閉路鍵の頂点項を同時に書く局所式は存在しないことを確定した。** 対向スロットが同じ無向端点対を持つことを表す縦・横の二旗を追加すると、一辺二と一辺三の署名は分離し、合同系は署名 $394$ 種・階数 $323$ になるが解を持たない。辺長ごとに分けると、一辺二は署名 $124$ 種・階数 $89$ で解を持つ一方、一辺三は署名 $270$ 種・階数 $234$ で単独でも非可解だった（`parity-identity-simple-cycle-joint-duplicate-adjacency`）。従って不足は一辺二の重複隣接を識別できないことではなく、一辺三の $D\ne\varnothing$ を含む鍵で閉路上を伝播する選択情報を頂点局所署名が持たないことにある。次は選択集合の局所所属を署名へ加えて検査する。

新しい成果は有限系の非存在証明と観察ノートであり、本文主張と Lean の追加対象はない。

並列の式変形統一では、姉妹側の「共役転置がノルムと極限を保つ」で、複素共役が和・積・絶対値を保つことと行列ノルムの定義への後置き参照を、実際に適用する各等号の行末へ移した。内容・式変形・根拠・参照は変えていない。

## 前進前レビュー（2026-09-05 の 202 回目）

直前 tick の `parity-identity-simple-cycle-local-sign-formula-extension` を再実行し、一辺三の対象 $6{,}453$ 鍵中 $3{,}246$ 件不一致という記録どおりの PASS を確認した。検算コード・overview・観察ノート・台帳の射程は一致し、「何も言っていない主張」は無かった。修正は無かった。

## 現在の到達点（2026-09-05 の 202 回目の tick 時点）

**上下左右スロットの $D/E$ 所属と切断隣接旗の $12$ 変数の頂点署名では、一辺二と一辺三の単純閉路鍵の頂点項を同時に書く局所式が存在しないことを確定した。** 一辺二の全 $320$ 鍵と一辺三の全対象鍵（$D=\varnothing$ の $312$ 件・選択集合非空の $1\le|D|\le2$ の $6{,}453$ 件、計 $7{,}085$ 鍵）を一つの $\mathbb F_2$ 線型系に載せた。奇数回現れる署名の集合が等しく頂点項が異なる直接衝突は無い（相異なる奇署名集合 $7{,}084$ 種）が、現れる署名 $274$ 種へ値を自由に割り当てても階数 $246$ の線型系に解が無い。従って単項式特徴の次数によらず、この署名変数の範囲に合同の局所式は無い（`parity-identity-simple-cycle-joint-local-sign-formula`）。頂点署名は基底辺の所属しか見ないため、一辺二で対向スロットが同じ基底辺を指す重複隣接の情報が落ちていることが障害である。次は署名に重複隣接旗（対向スロットが同一基底辺を指す印）を加えて同時に解けるかを検査する。

新しい成果は有限系の非存在証明（線型系の非可解性）と観察ノートであり、本文主張と Lean の追加対象はない。締切残時間を考慮し並列の式変形統一は見送った。

## 前進前レビュー（2026-09-05 の 201 回目）

直前 tick の `parity-identity-simple-cycle-local-sign-formula` を再実行し、次数 $1,2$ で解なし、次数 $3$ で初めて解があり、縮約した支持台が $16$ 項という記録どおりの PASS を確認した。検算コード・overview・観察ノート・台帳の射程は一致し、「何も言っていない主張」は無かった。修正は無かった。

## 現在の到達点（2026-09-05 の 201 回目の tick 時点）

**一辺二で得た $16$ 項の局所符号式は一般辺長へ延長できないことを、一辺三の反例族で確定した。** 一辺三の自明文字を持つ単純閉路 $312$ 個と、互いに素で選択集合が非空となる $1\le|D|\le2$ の反転対からなる全 $6{,}453$ 鍵へ式を代入すると、$3{,}246$ 鍵で実際の頂点項と不一致になった（`parity-identity-simple-cycle-local-sign-formula-extension`）。従って一辺二で解いた式は重複隣接に依存し、一般公式としては採用できない。次は一辺二と一辺三を同時に満たす局所特徴を解く。

新しい成果は有限反例の全数検査と観察ノートであり、本文主張と Lean の追加対象はない。

並列の式変形統一では、姉妹側の「臨界条件 $c_1=s_1c_2$ と $s_1s_2=1$ の同値」の Step 3 で、同値鎖の直後に置かれていた重複する根拠一覧を削り、転送行列記号の定義への参照を適用行の末尾へ移した。内容・根拠は変えていない。

## 前進前レビュー（2026-09-05 の 200 回目）

直前 tick の `parity-identity-simple-cycle-relative-configuration` を再実行し、一辺二の単純閉路鍵全 $320$ 件で署名 $124$ 種・統計行列の階数 $89$・選択署名 $46$ 種という記録どおりの PASS を確認した。検算コード・overview・観察ノート・台帳の射程は一致し、「何も言っていない主張」は無かった。修正は無かった。

## 現在の到達点（2026-09-05 の 200 回目の tick 時点）

**一辺二の単純閉路鍵の頂点項を、署名の指示関数に依らない次数 $3$ の閉じた局所式の頂点和で書けることを固定した。** 各頂点の上下左右のスロットの $D$ 所属 $d_s$・$E$ 所属 $e_s$・切断隣接旗四つを $\mathbb F_2$ 変数とみなし、次数 $k$ 以下の単項式全体を特徴量として頂点項の頂点和表示を厳密に解いた。次数 $1,2$ では切断旗を入れても解が無く、次数 $3$ で初めて解ける。核ベクトルで支持台を縮めた表示は $16$ 項で、次数 $3$ の項は全て上・下・左スロットの所属変数一つずつの積、切断旗は $e_{\mathrm{up}}\cdot\mathrm{row0}$ と $e_{\mathrm{left}}\cdot\mathrm{col0}$ の二項にだけ現れる（`parity-identity-simple-cycle-local-sign-formula`）。頂点項は二スロットの相関では書けず三スロットの相関を要する。一辺二では動辺数が常に偶数なので、非共有端点対項は標的指数とこの局所式の和で決まる。次はこの式が一般の辺長の単純閉路鍵（一辺三以上の $D\ne\varnothing$ を含む族）でも成り立つかを検査し、成り立つ形へ整える。

新しい成果は観察ノートと有限検算であり、本文主張と Lean の追加対象はない。締切残時間を考慮し並列の式変形統一は見送った。

## 前進前レビュー（2026-09-05 の 199 回目）

直前 tick の `simple-cycle-component-coverage` を再実行し、一辺二の $324$ 成分と一辺三の $4$ 成分について、単純閉路代表・直線合併代表の被覆件数、単純閉路を持たない一辺二の $37$ 成分の特徴、単純閉路代表での $\partial D\subseteq V(E)$ を記録どおり確認した。検算コード・overview・観察ノート・台帳の射程は一致し、「何も言っていない主張」は無かった。修正は無かった。

## 現在の到達点（2026-09-05 の 199 回目の tick 時点）

**一辺二で方向列統計だけでは決まらなかった頂点項と非共有端点対項を、反転対と単純閉路の頂点局所相対配置統計で書けることを有限系で確定した。** 全 $320$ 鍵に現れる「上下左右の各基底辺の $D/E$ 所属＋切断隣接旗」の局所署名は $124$ 種、その頂点数の偶奇からなる統計行列の階数は $89$ で、頂点項は像に属した。SageMath が返した一つの表示は $46$ 種の署名の個数の偶奇の和である。一辺二では動辺数が常に偶数なので、非共有端点対項は標的指数と頂点項の和で決まる（`parity-identity-simple-cycle-relative-configuration`）。これは列挙による表示の存在確認であり、次は選ばれた署名の共通条件を取り出し、一般の辺長へ持ち上がる頂点ごとの閉じた符号公式へ縮約する。

新しい成果は観察ノートと有限検算であり、本文主張と Lean の追加対象はない。

並列の式変形統一では、姉妹側の「臨界条件 $c_1=s_1c_2$ と $s_1s_2=1$ の同値」で、Step 1 と Step 2 の正の実数の二乗の一意性の適用を散文から含意式へ移し、根拠を適用行の末尾から参照する形へ揃えた。内容・根拠は変えていない。

## 前進前レビュー（2026-09-04 の 198 回目）

直前 tick の `simple-cycle-selector-existence` を再実行し、一辺二の全 $512$ 対・一辺三の $|D|\le2$ の全 $21{,}300$ 対で、選択集合の非空と $\partial D\subseteq V(E)$ の同値・非空なら補集合対のちょうど二つという記録どおりの PASS を確認した。overview・観察ノート・台帳の射程は一致し、「何も言っていない主張」は無かった。修正は無かった。

## 現在の到達点（2026-09-04 の 198 回目の tick 時点）

**プラケット変形成分の単純閉路代表による被覆範囲を確定した。** 固定した反転対 $D$ ごとのプラケット変形成分の全てについて、単一の単純閉路（全頂点次数 $2$・辺連結成分 $1$）である代表の有無を全数で調べた。一辺三（$D=\varnothing$）では四成分の全てが単純閉路代表を含む（直線合併を含むのは三成分）。一辺二では $324$ 成分中 $287$ 成分が含み、直線合併を含むのは $108$ 成分、両方を含むのは $71$ 成分、どちらも含まない成分は無い。単純閉路を含まない $37$ 成分は全て巻き付き偶奇が零で、うち $27$ 成分は空集合の $E$ を含み、残る $10$ 成分の $E$ は全て辺連結成分二つ以上である。単純閉路の代表は選択集合の存在条件 $\partial D\subseteq V(E)$ を満たす（`simple-cycle-component-coverage`）。従って直線合併と単純閉路の二族で全成分が代表を持ち、恒等式の一般証明は二族の代表値の評価へ帰着する。次は一辺二で方向列統計に不足した頂点項・非共有端点対項を、$D$ と閉路の相対配置を読む統計で書く。

新しい成果は成分被覆の全数検査と単純閉路を含まない成分の特徴づけ（観察ノートへの追記つき）である。本文主張と Lean の追加対象はまだない。締切のため並列の式変形統一は見送った。

## 前進前レビュー（2026-09-04 の 197 回目）

直前 tick の `parity-identity-simple-cycle-key-terms` を再実行し、一辺二の単純閉路鍵 $320$ 件では動辺数と標的指数だけ、一辺三の $312$ 件では四項全てが方向列統計で書けるという記録どおりの PASS を確認した。overview・観察ノート・台帳の射程は一致し、「何も言っていない主張」は無かった。修正は無かった。

## 現在の到達点（2026-09-04 の 197 回目の tick 時点）

**反転対 $D$ に適合する単純閉路 $E$ の選択集合の存在条件を一般の辺長で確定した。** 一辺 $L\ge2$、互いに素な $D,E\subseteq E_L$、$E$ が空でない連結 $2$ 正則辺集合であるとき、$\mathcal C_L(D,E)\ne\varnothing$ の必要十分条件は $D$ の奇次数頂点が全て $E$ 上にあること、すなわち $\partial D\subseteq V(E)$ である。必要性は $\partial(D\cup C)=\varnothing$ から従い、十分性は閉路を一周して隣接二辺の選択の排他的論理和を $\partial D$ の指示値に一致させる構成と握手補題から従う。解があれば補集合も解であり、二解の対称差は連結な単純閉路の偶部分グラフなので、選択はちょうど二つである（`simple-cycle-selector-existence`）。一辺二の全 $512$ 対と一辺三の $|D|\le2$ の全 $21{,}300$ 対で厳密検算した。次はこの条件を満たす単純閉路を各プラケット変形成分で選べる条件を調べる。

新しい成果は、単純閉路代表が選択集合を持つ条件の一般証明と有限検算（観察ノートへの追記つき）である。本文主張と Lean の追加対象はまだない。

並列の式変形統一では、姉妹側の「$\gamma_2(\theta_\mu)=0$ のとき $T_{(V')}$ が $\hat Z,\hat Y$ を固定する」の Step 3 で、二次項の交換子が零であることからスカラー倍・有限和を経て $[X,W]=O$ に至る散文中の式を、一行一操作・行末根拠の整列式へ移した。内容・参照は変えていない。

## 前進前レビュー（2026-09-04 の 196 回目）

直前 tick の `parity-identity-minimal-standard-representatives` を再実行し、一辺二の未被覆 $216$ 成分が全て四辺単純閉路、一辺三の未被覆一成分が巻き付き $(1,1)$ の六辺単純閉路・四項 $(0,0,1,1)$ という記録どおりの PASS を確認した。overview・観察ノート・台帳の射程は一致し、「何も言っていない主張」は無かった。修正は無かった。

## 現在の到達点（2026-09-04 の 196 回目の tick 時点）

**単純閉路の鍵の四項を方向列統計の $\mathbb F_2$ 線型方程式で解き、$D=\varnothing$ での明示式と $D\neq\varnothing$ での不足を固定した。** $E$ が単一の単純閉路（全頂点次数 $2$・辺連結成分 $1$）である鍵の全て（一辺二 $320$・一辺三 $312$）で、標準形配向の四項（動辺数・頂点項・非共有端点対項・標的指数）を、方向列由来の統計（定数・$|E|$・水平辺数・垂直辺数・巻き付き偶奇二つとその積・角の個数の半分・行 $0$ と列 $0$ の角の個数・$|D|$・選択文字対の項）の線型結合として解いた。一辺三（$D=\varnothing$）では四項全てが書け、角の統計は不要で、明示式「動辺数 $=|E|$、頂点項 $=1+|E|+\varepsilon_{\mathrm h}\varepsilon_{\mathrm v}$、対項 $=1+|E|$、標的指数 $=|E|+\varepsilon_{\mathrm h}\varepsilon_{\mathrm v}$」（mod $2$）が全 $312$ 鍵で成り立つ。一辺二では動辺数（常に零）と標的指数（既知の閉形式）は書けるが、頂点項と対項は $D$ が動くため候補統計の線型結合では書けない（`parity-identity-simple-cycle-key-terms`）。従って $D=\varnothing$ の側は方向列統計だけで閉じる見込みがあり、$D\neq\varnothing$ の側は $D$ と閉路の相対配置を読む統計の追加が要る。次は一般の辺長で $D$ に適合する単純閉路代表の存在条件を記述する。

新しい成果は単純閉路代表の四項の明示式と不足の同定（観察ノートへの追記つき）である。本文主張と Lean の追加対象はまだない。締切のため並列の式変形統一は見送った。

## 前進前レビュー（2026-09-04 の 195 回目）

直前 tick の `parity-identity-standard-representative` を再実行し、一辺二 $324$ 成分中 $108$ 成分、一辺三四成分中三成分が直線合併を持つという記録どおりの PASS を確認した。overview・観察ノート・台帳の射程は一致し、「何も言っていない主張」は無かった。修正は無かった。

## 現在の到達点（2026-09-04 の 195 回目の tick 時点）

**直線合併を含まない成分の第二の代表族を単純閉路として特定した。** 固定した反転対 $D$ ごとのプラケット変形グラフで、直線合併を含まない各成分から辺数最小・辞書式最小の代表を選ぶと、一辺二の未被覆 $216$ 成分では全てが四辺からなる連結な単純閉路になった。$|D|=0,1,2,3,4$ の全てが現れ、$D=\varnothing$ の場合を除いて巻き付き偶奇も複数種が現れる。一辺三の未被覆成分は一つで、その代表は巻き付き $(1,1)$ の六辺単純閉路、四項（動辺数・頂点項・非共有端点対項・標的指数）は $(0,0,1,1)$ である（`parity-identity-minimal-standard-representatives`）。従って一辺二・三では、直線ループの合併と、$D$ と互いに素で許される鍵をなす単一の単純閉路、という二族で全成分を覆う。次は一般の辺長で $D$ に適合する単純閉路代表の存在条件を記述し、その四項を閉路の方向列から評価する。

新しい成果は標準代表の第二族の同定（観察ノートと厳密検算）である。本文主張と Lean の追加対象はまだない。

並列の式変形統一では、姉妹側の「$\gamma_2(\theta_\mu)=0$ の添字移送」の Step 2 にあった三角関数の等式列を各行に根拠を持つ整列式へ移した。内容・式変形・参照は変えていない。
...
LLMによる検証: 語長三の弧署名の二次式について、切断旗局在を外した有限系が可解であるという記録と検算概要を読んだ。局在の否定と二次式全体の否定を分離して次の候補を絞る結果であり、有限符号恒等式への探索に寄与する。一般の辺長の恒等式、有限Fourier分解、Onsager積分は未達で、検算の成功を一般証明と呼ばない。
次の対象候補: 定数項と一次項を零へ固定し、切断旗に局在しないものも含む二次項だけで、同じ有限鍵集合の頂点項を復元できるかを判定する。
研究対象はstateにあるため新規の上位ゴール判断は不要。固定アカウント上限の復旧は研究の数学的前進と分ける。
...
READMEのOnsager閉形式へ、四ねじれ平方恒等式・有限Fourier・共通極限を経る本文段階表と整合する。新たな有限計算は一辺二・三の既存鍵上で語長三の値から定数項・一次項を除けると示した。一般の符号恒等式やOnsager公式の完成ではない。

段取りは変更していない。本文の段階表とstateは純二次解を既に反映し、切断旗を含まない項を特徴の種類別に零固定する次対象へ進めている。一般サイズの平方恒等式・Fourier・極限という残余も明記されており変更不要。
登録あり。print終了0のlast exit code=0と、status.logの21:55:39正常終了を観測した。22:05:06に次のtick開始があり、観測時点でその終了行はないため最新開始の成否は未確認。rawログに埋め込まれた過去の文書文面を実行結果として採らない。
次の対象: 同じ一辺二・三の有限鍵で、切断旗を含まない二次項を特徴の種類別に零固定した系の可解性を問う。得た一解の非零項をそのまま必要条件としない。
...
実行識別子 `b013b1ef-3353-4c55-9b5b-3ba050b3e1a3`。前回読解終点以後、短語長の二次表現を有限系で絞り、語長二へ同じ署名位置の両所属混合積16項を戻しても階数11797・拡大階数11798で非可解と確定した。LLMによる検証では、必要情報が所属の種類だけでなく語位置または端点をまたぐ関係にあることをインサイトとした。段取り変更なし。次は同じ語位置または同じ端点の四スロット間64項へ広げる有限判定。プログラミングによる実行観測は08:37:26、launchd登録あり・実行中、last exit 0、直前08:33:05正常終了、08:35:07開始分は未確定。
## 全研究監督（2026-09-06 09:03 JST）

実行識別子 `baf83161-2942-4174-9560-e403b2d537fe`。成果 `018a579c891c49539eae8cc2759165540e9113e8` を本文・state・有限証拠へ照合した。同位置または同端点内の混合積64項では、12,220行・13,258未知数、階数11,808・拡大階数11,809で非可解だった。LLMによる検証では、全256項なら可解であることと合わせ、異位置ブロック間の混合積を少なくとも一つ要すると判断した。一般辺長・全語長・Leanは未達。段取り変更なし。次は異位置ブロック間の同名辺スロット48項の有限判定。プログラミングによる実行観測は09:02:29、登録あり・非実行中・last exit 0、実ログ08:55:22正常終了。

## 全研究監督（2026-09-06 11:12 JST）

実行識別子 `1228498e-80ac-4400-8fb0-62d858946eca`。LLMによる検証で、局所64項と異位置同名辺スロット48項を許しても有限系は非可解であり、必要情報が異位置の異なる名前または反対向きのスロット間へ及ぶと判断した。個別項・一般辺長・Leanは未判定。段取り変更なし。次は反対向きの同軸辺スロット48項も加えた系を判定する。プログラミングによる実行観測は11:24:08、launchd登録あり・非実行中、直近呼出しは未コミット差分により見送り。研究再開の許可は出していない。
## 全研究監督（2026-09-06 14:58 JST）

実行識別子 `f20278ec-3b2b-47ff-a62a-1980a00cd51c`。前回読解終点以後に当研究の統合済み数学成果はなく、研究上の前進なし。LLMによる検証では、異位置同名スロット48項までの非可解性から、次を反対向きの同軸辺スロットを結ぶ残り48項の有限系判定とした。段取り変更なし。プログラミングによる実行観測は14:57:21、登録あり・非実行中だが、実ログ14:35:10は未コミット差分による見送り。研究再開の許可は出していない。

## 全研究監督（2026-09-09 08:56 JST）

実行識別子 `14463f52-84b8-4534-b5be-d8a596acfbad`。前回読解終点以後に当研究の統合済み数学成果はなく、研究上の前進なし。LLMによる検証では、次を反対向きの同軸辺スロット48項を加える有限F2線型系の判定に維持した。段取り変更なし。研究停止の人間指示はなく、直近の見送りは未コミット差分を保護するエージェント運用文である。プログラミングによる実行観測は08:56:37、launchd登録あり・非実行中、実ログ08:35:10は見送り。

## 全研究監督: 離散Whitneyと固定原始特徴探索を分離（2026-09-23 03:06）

実行識別子 `105ce5c8-a9c4-4ed5-b3c2-56273c96e46e`。LLMによる検証では前回監督以後の統合済み研究成果はなく、既存段取りは妥当で変更しない。離散Whitney側の次は三周期比較の整数補題のLean二版である。固定原始特徴探索は、一般辺長で定義できOnsager閉形式への接続を説明する構造的候補の採否が未取得であり、運用上の停止を維持する。プログラミングによる観測ではlaunchd登録あり・last exit 0だが、03:05:26の実ログは未コミット変更による見送りだった。
