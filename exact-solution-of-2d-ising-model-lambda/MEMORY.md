...
1. **レビュー**: 低温展開の自明セクター表示の Lean 三層を本文と突き合わせ、有限和の添字交換に
   仮定の過不足が無いか、具体版が双対辺写像の往復と辺数保存を正しく使っているかを見る。
2. **次に進めるセクションは「高温展開の生成多項式を四つのセクターの生成多項式の和へ分解する」**。
   状態台帳の先頭行だけを進める。
