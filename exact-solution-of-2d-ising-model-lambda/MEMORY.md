...
**記述と SageMath 検証（$L=1,\dots,6$）までである。Lean は未着手**（2026-08-10 の tick 51）。
ここにも $\mathbb{R}/\mathbb{C}$ は現れない。
...
残るのは、そこから符号が $(-1)^{\lvert O\rvert-1}$ であること、
...
3. **並列の作業ストリーム（式変形の書き方の統一）は今 tick は行っていない**
   （締切に間に合わないため。次 tick で 1 件進める）。
