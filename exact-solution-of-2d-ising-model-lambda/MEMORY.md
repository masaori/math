...
`sagemath/check/finite-size-scaling-observation/l3-enlarged-stack-experiment.sage` で PARI スタックを 8 GiB（上限 16 GiB）へ拡張して tick 427 と同じ全根分離経路を再実行したが、スタック超過は再発せず 540 秒の時間切れで打ち切られた。限界はスタック容量ではなく `AA` の最小多項式計算の時間であり、$L=3$ を通すには数体の統合を避ける経路の置き換え（$\mathbb Q[x]$ の終結式で距離の多項式を直接作る等）が要る。overview と観察ノートに追記した。本文・Lean への変更なし。todo は引き続き空（命題候補が立つまで本文へ項目を追加しない）。
