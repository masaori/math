...
**大域的組み替えの規則は「ファイバーごとに偶部分グラフを一つ選ぶ」までは単純化できないことを一辺二の全数検査で確定した。** 巻き付き文字が非自明な全 64 ファイバー中 48 には、ファイバー内の全置換が位相反転候補を持つ一様被覆の偶部分グラフが存在し、その全てで候補対だけの完全マッチングも成立する。しかし残り 16 ファイバー（最初の例は $D=\varnothing$、$E=\{h(1,0),h(1,1),v(0,1),v(1,1)\}$）には一様被覆する偶部分グラフが無い（`even-subgraph-fiberwise-uniform-matching` とノート `note_kac_ward_claim_selection_even_subgraph_action_character_fiberwise_uniform_matching` に固定）。従って一般の辺長への持ち上げは、置換ごとに偶部分グラフを選ぶ割り当てを保つ形で構成する。次は覆えない 16 ファイバーで割り当てが従う構造（軌道分解・接触対の配置）を調べる。
