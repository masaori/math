...
- **3 次元へ進むときの方針は
  ...
  にある**（2026-08-13 に作成）。要点は、3 次元では臨界点が代数的である保証がないので、
  「臨界点そのもの」ではなく「臨界点の切断を判定する有限体積の証明書」を可算側の標的にすること。
  高温側の判定量（Duminil-Copin–Tassion）は $x=e^{-2\beta J}$ の整数係数多項式の比になるため
  有理数で決定可能であり、その校正を 2 次元（$x_c=\sqrt2-1$ が既知）で先に行う。
  この校正は本プロジェクトの零点・$\Lambda$ の道具立てをそのまま使う。
