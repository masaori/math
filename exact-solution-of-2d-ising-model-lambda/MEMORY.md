...
- Ising に依存しないブロックは 123 件あるが、道具の章に入れたのは 60 件である。残りは
  二次体と正錐・実閉部分体・代数的数係数の多項式・有理係数の対数順序群などにあり、
  節の中に本論のブロックが混ざっているため節ごと動かせない。節を割れば道具の章へ移せる。
- 行き止まりの主張（下流数 0 の非主定理）の切り分け。引用漏れか不要かを 1 件ずつ読んで判断する。
