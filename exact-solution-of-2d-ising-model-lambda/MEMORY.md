...
前進前レビューでは、前 tick の置換符号の軌道分解（コミット `b5d17d2f`）を本文・SageMath で照合し、「何も言っていない主張」を含む修正対象が無いことを確認した（上記の互換符号の欠落はレビュー時点では見落とし、前進の調査で発見した）。締切のため並列の式変形統一はこの tick の状況次第で記録する。
