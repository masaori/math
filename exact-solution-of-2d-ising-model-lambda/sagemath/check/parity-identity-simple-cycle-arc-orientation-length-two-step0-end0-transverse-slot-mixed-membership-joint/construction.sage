"""同軸候補に、語位置 step0 と端点 end0 を結ぶ異軸の両所属混合積を加える。"""
...
def step0_end0_transverse_slot_mixed_membership(first_name, second_name):
    first = first_name.split("_")
    second = second_name.split("_")
    return ({first[0], second[0]} == {"step0", "end0"}
            and word_position_endpoint_transverse_slot_mixed_membership(
                first_name, second_name))


def length_two_step0_end0_transverse_slot_mixed_membership_joint_system(
                               ...
                               or ({names[i].split("_")[1], names[j].split("_")[1]} == {"e", "d"}
                                   and (same_membership_position_block(names[i], names[j])
                                        ...
                                        or cross_block_opposite_collinear_slot_mixed_membership(
                                            names[i], names[j])
                                        or step0_end0_transverse_slot_mixed_membership(
                                            names[i], names[j]))))))
        start = next_column
