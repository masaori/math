"""既存 7,085 鍵で、語位置 step0 と端点 end0 を結ぶ異軸の両所属混合積 16 項を加える有限判定。
...
system, system_rhs, blocks = \
    length_two_step0_end0_transverse_slot_mixed_membership_joint_system(
...
for length, (types, names, pairs, start) in blocks.items():
    ...
    expected_step0_end0 = {
        ...
        if step0_end0_transverse_slot_mixed_membership(names[i], names[j])}
    ...
    if length == 2:
        ...
        assert all({names[i].split("_")[0], names[j].split("_")[0]}
                   == {"step0", "end0"} for i, j in expected_step0_end0)
...
certificate_path = Path("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-step0-end0-transverse-slot-mixed-membership-joint/certificate.json")
...
print("PASS: length-two step0/end0 transverse-slot mixed-membership joint decision and certificate", flush=True)
