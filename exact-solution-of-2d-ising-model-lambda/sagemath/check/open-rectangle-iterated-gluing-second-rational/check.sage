...
def check_iterated_second(a, b, k, q):
    single = partition_value(a, b, q)
    joined = partition_value(a, k * b, q)
    boundary_factor = q ** ((k - 1) * a)
    # 主張の二場合（q = 1 は両方に属し、両方を見る）。
    if q <= 1:
        assert boundary_factor * single ** k <= joined <= single ** k
    if q >= 1:
        assert single ** k <= joined <= boundary_factor * single ** k
    # 帰納段: 第二座標の長さ kb と b の二長方形への接合不等式（正の有理点）と帰納法の仮定の積。
    if k >= 2:
        prev = partition_value(a, (k - 1) * b, q)
        prev_factor = q ** ((k - 2) * a)
        assert q ** ((k - 1) * a) == q ** a * prev_factor            # ka = a + (k-1)a
        if q <= 1:
            assert q ** a * prev_factor * single ** (k - 1) * single <= q ** a * prev * single
            assert q ** a * prev * single <= joined <= prev * single
            assert prev * single <= single ** (k - 1) * single == single ** k
        if q >= 1:
            assert single ** (k - 1) * single <= prev * single <= joined
            assert joined <= q ** a * prev * single
            assert q ** a * prev * single <= q ** a * prev_factor * single ** (k - 1) * single
...
for a, b, k in sizes:
    for q in test_points:
        check_iterated_second(a, b, k, q)
        total += 1

print(f"開境界長方形の第二座標方向の反復接合不等式（正の有理点。QQ で厳密）: {total} 組 OK")
