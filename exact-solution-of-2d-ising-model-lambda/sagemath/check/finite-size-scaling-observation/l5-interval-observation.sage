...
# L=5 の区間観察: L=4 と同じ区間経路（最小多項式計算を含まない）で、
# 分配多項式の構成・全根の QQbar での分離・距離の二乗の 256 bit 包含区間・
# 先頭候補の分離検証を実行し、先頭距離 d_1(5) の包含区間と、
# 実対数へ脱出した読み a_rho(5) を得る。
#
# 分配多項式の構成だけは L=4 までと方法が違う。定義どおりの全配位の足し上げは
# 2^25 個の配位を要して現実的でないため、本文で証明済みの跡定理
# theorem_partition_polynomial_is_trace（Z_L = Tr T^L）を引いて、
# 転送行列 T in M_{2^5}(ZZ[x]) の 5 乗の跡として構成する。
# これは観察のための計算であり、定義との一致の検証ではない
# （一致の検証は check.sage が L<=3 で行っている）。
# 各段階を経過秒つきでログファイルへ永続化する（打ち切られても残る）。
# 実行例: timeout 480 sage l5-interval-observation.sage
...
_log_path = os.path.join(
    _dir, 'l5-stage-log-%s.txt' % datetime.date.today().isoformat())
...
log('stage: sanity check passed, Z_5(1) = 2^25')
...
log('d_1(5) interval =', candidate_interval)

# a_rho(5) の表示だけ実対数による実数体への脱出である。
real_field = RealBallField(128)
scaling_reading = (-real_field(candidate_interval.center()).log()
                   / (2 * real_field(L).log()))
log('a_rho(5) in RealBallField(128) =', scaling_reading)
log('=== run complete')
