...
# L=8 の区間観察: L=7 と同じ区間経路（最小多項式計算を含まない）で、
# 分配多項式の構成・全根の QQbar での分離・距離の二乗の 256 bit 包含区間・
# 先頭候補の分離検証を実行し、先頭距離 d_1(8) の包含区間と、
# 実対数へ脱出した読み a_rho(8) を得る。
#
# 分配多項式の構成は L=5,6,7 と同じく、定義どおりの全配位の足し上げ（2^64 配位）が
# 現実的でないため、本文で証明済みの跡定理
# theorem_partition_polynomial_is_trace（Z_L = Tr T^L）を引いて、
# 転送行列 T in M_{2^8}(ZZ[x]) の 8 乗の跡として構成する。
# これは観察のための計算であり、定義との一致の検証ではない
# （一致の検証は check.sage が L<=3 で行っている）。
# 各段階を経過秒つきでログファイルへ永続化する（打ち切られても残る）。
# 実行例: timeout 1500 sage l8-interval-observation.sage
...
_log_path = os.path.join(
    _dir, 'l8-stage-log-%s.txt' % datetime.date.today().isoformat())
...
log('stage: sanity check passed, Z_8(1) = 2^64')
...
log('d_1(8) interval =', candidate_interval)

# a_rho(8) の表示だけ実対数による実数体への脱出である。
real_field = RealBallField(128)
scaling_reading = (-real_field(candidate_interval.center()).log()
                   / (2 * real_field(L).log()))
log('a_rho(8) in RealBallField(128) =', scaling_reading)
log('=== run complete')
