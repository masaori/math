...
# L=7 の区間観察: L=6 と同じ区間経路（最小多項式計算を含まない）で、
# 分配多項式の構成・全根の QQbar での分離・距離の二乗の 256 bit 包含区間・
# 先頭候補の分離検証を実行し、先頭距離 d_1(7) の包含区間と、
# 実対数へ脱出した読み a_rho(7) を得る。
#
# 分配多項式の構成は L=5,6 と同じく、定義どおりの全配位の足し上げ（2^49 配位）が
# 現実的でないため、本文で証明済みの跡定理
# theorem_partition_polynomial_is_trace（Z_L = Tr T^L）を引いて、
# 転送行列 T in M_{2^7}(ZZ[x]) の 7 乗の跡として構成する。
# これは観察のための計算であり、定義との一致の検証ではない
# （一致の検証は check.sage が L<=3 で行っている）。
# 各段階を経過秒つきでログファイルへ永続化する（打ち切られても残る）。
# 実行例: timeout 900 sage l7-interval-observation.sage
...
_log_path = os.path.join(
    _dir, 'l7-stage-log-%s.txt' % datetime.date.today().isoformat())
...
log('stage: sanity check passed, Z_7(1) = 2^49')
...
log('d_1(7) interval =', candidate_interval)

# a_rho(7) の表示だけ実対数による実数体への脱出である。
real_field = RealBallField(128)
scaling_reading = (-real_field(candidate_interval.center()).log()
                   / (2 * real_field(L).log()))
log('a_rho(7) in RealBallField(128) =', scaling_reading)
log('=== run complete')
