"""既存 7,085 鍵で語長二の局在形と語長三の固定非切断項を両立させる有限判定。

対象ラベル: claim_kac_ward_determinant_fiber_stratified_phase_sum
一般の辺長と全語長についての命題ではない。
"""
print("LOAD: constructing finite key data", flush=True)
load("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-three-fixed-interior/construction.sage")
...
system, system_rhs, blocks = length_two_three_fixed_interior_system(entries, rhs, all_types, column_index)
assert system.dimensions() == (10971, 12008)
assert tuple(len(blocks[n][0]) for n in (2, 3)) == (1842, 2044)
assert tuple(len(blocks[n][2]) for n in (2, 3)) == (760, 1150)
...
certificate_path = Path("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-three-fixed-interior/certificate.json")
...
print("PASS: length-two-three fixed interior decision and certificate", flush=True)
