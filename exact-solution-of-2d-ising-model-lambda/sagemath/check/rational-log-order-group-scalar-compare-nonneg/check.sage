...
for nu in vectors:
    # 仮定: 0 ≤_{Λ_Q} ν
    if not rational_log_order_le(zero, nu):
        continue
    count_nonneg += 1
    for r in scalars:
        for s in scalars:
            if not r <= s:
                continue
            # 主張: r ≤ s、0 ≤ ν ⟹ r·ν ≤_{Λ_Q} s·ν（決定手続きで判定）
            assert rational_log_order_le(qsmul(r, nu), qsmul(s, nu)), (nu, r, s)
            count_claims += 1
            # 証明の鎖を段ごとに検査する
            c = s - r
            assert c >= 0                                                        # Q の順序と減法
            assert rational_log_order_le(qsmul(c, zero), qsmul(c, nu))           # 非負有理数倍の順序保存（λ:=0, μ:=ν）
            assert qsmul(c, zero) == zero                                        # c·0 = 0
            assert rational_log_order_le(zero, qsmul(c, nu))                     # 0 ≤ c·ν
            lhs = qadd(zero, qsmul(r, nu))
            rhs = qadd(qsmul(c, nu), qsmul(r, nu))
            assert rational_log_order_le(lhs, rhs)                               # 加法単調性（λ:=0, μ:=c·ν, ν:=r·ν）
            assert qsmul(r, nu) == lhs                                           # r·ν = 0 + r·ν（零写像は単位元）
            assert rhs == qsmul(c + r, nu)                                       # 分配則 (c+r)·ν = c·ν + r·ν
            assert c + r == s and qsmul(c + r, nu) == qsmul(s, nu)               # c + r = s
            count_chain += 1

print("非負のベクトル数:", count_nonneg)
print("主張の検査件数:", count_claims)
print("鎖の検査件数:", count_chain)
print("PASS: claim_rational_log_order_group_scalar_compare_nonneg")
