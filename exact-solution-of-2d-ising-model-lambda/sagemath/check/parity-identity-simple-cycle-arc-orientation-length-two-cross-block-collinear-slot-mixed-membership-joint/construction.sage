"""位置内の両所属混合積に、異なる位置ブロックの同名・反対向き同軸スロットを結ぶ積を加える。"""
...
def cross_block_opposite_collinear_slot_mixed_membership(first_name, second_name):
    first = first_name.split("_")
    second = second_name.split("_")
    opposite_slots = {frozenset(("up", "down")), frozenset(("left", "right"))}
    return (first[0] != second[0]
            and frozenset((first[2], second[2])) in opposite_slots
            and {first[1], second[1]} == {"e", "d"})


def length_two_cross_block_collinear_slot_mixed_membership_joint_system(
