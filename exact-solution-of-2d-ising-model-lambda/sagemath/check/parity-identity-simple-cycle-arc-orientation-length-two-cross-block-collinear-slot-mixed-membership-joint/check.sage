"""既存 7,085 鍵で、異なる位置ブロックの同名・反対向き同軸スロットを結ぶ両所属混合積を加える有限判定。

対象ラベル: claim_kac_ward_determinant_fiber_stratified_phase_sum
一般の辺長と全語長についての命題ではない。
"""
print("LOAD: constructing finite key data", flush=True)
load("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-cross-block-collinear-slot-mixed-membership-joint/construction.sage")
...
system, system_rhs, blocks = length_two_cross_block_collinear_slot_mixed_membership_joint_system(
    entries, rhs, all_types, column_index)
assert system.dimensions() == (12220, 13354)
assert tuple(len(blocks[n][0]) for n in (1, 2, 3)) == (1249, 1842, 2044)
assert tuple(len(blocks[n][2]) for n in (1, 2, 3)) == (946, 1160, 1150)
...
for length, (types, names, pairs, start) in blocks.items():
    ...
    if length == 2:
        assert len(expected_position) == 64
        assert len(expected_same) == 48
        assert len(expected_opposite) == 48
        assert expected_position.isdisjoint(expected_same)
        assert expected_position.isdisjoint(expected_opposite)
        assert expected_same.isdisjoint(expected_opposite)
        assert sum(i in single and j in single for i, j in pairs) == 120
        assert sum(i in doubled and j in doubled for i, j in pairs) == 120
        assert sum(i not in wrap and j not in wrap for i, j in pairs) == 400
        opposite_slots = {frozenset(("up", "down")), frozenset(("left", "right"))}
        assert all(names[i].split("_")[0] != names[j].split("_")[0]
                   and frozenset((names[i].split("_")[2], names[j].split("_")[2]))
                   in opposite_slots
                   for i, j in expected_opposite)
...
certificate_path = Path("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-cross-block-collinear-slot-mixed-membership-joint/certificate.json")
...
print("PASS: length-two cross-block collinear-slot mixed-membership joint decision and certificate", flush=True)
