...
#
# 同じ開境界長方形を第一座標方向へ k 個接いだ値の上下評価を QQ 上で厳密に検査する。
# 浮動小数点・RR・CC は使わない。
...
def check_iterated_first(a, b, k, t):
    single = partition_value(a, b, t)
    joined = partition_value(k * a, b, t)
    boundary_factor = t ** ((k - 1) * b)
    if t <= 1:
...
for a, b, k in sizes:
    for t in test_points:
        check_iterated_first(a, b, k, t)
        total += 1

print(f"開境界長方形の第一座標方向の反復接合不等式（QQ で厳密）: {total} 組 OK")
