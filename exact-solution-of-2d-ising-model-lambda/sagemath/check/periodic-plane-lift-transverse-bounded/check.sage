                    ...
                    if (w_h, w_v) != (ZZ(0), ZZ(0)):
                        ...
                        for q in range(-2, 3):
                            for r in range(len(walk)):
                                ...
                                assert transverse(extended, w_h, w_v) \
                                    == transverse(points[r], w_h, w_v)
                                extended_values.add(transverse(extended, w_h, w_v))
                                point_total += 1
                        assert extended_values == base_values
                        assert min(base_values) <= max(base_values)
                        cycle_total += 1
