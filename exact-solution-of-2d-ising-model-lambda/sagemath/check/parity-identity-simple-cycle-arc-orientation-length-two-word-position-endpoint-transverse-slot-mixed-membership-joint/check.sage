"""既存 7,085 鍵で、語位置と端点を結ぶ異なる軸の両所属混合積 64 項を加える有限判定。

対象ラベル: claim_kac_ward_determinant_fiber_stratified_phase_sum
一般の辺長と全語長についての命題ではない。
"""
print("LOAD: constructing finite key data", flush=True)
load("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-word-position-endpoint-transverse-slot-mixed-membership-joint/construction.sage")
...
system, system_rhs, blocks = \
    length_two_word_position_endpoint_transverse_slot_mixed_membership_joint_system(
        entries, rhs, all_types, column_index)
assert system.dimensions() == (12220, 13418)
assert tuple(len(blocks[n][0]) for n in (1, 2, 3)) == (1249, 1842, 2044)
assert tuple(len(blocks[n][2]) for n in (1, 2, 3)) == (946, 1224, 1150)
...
for length, (types, names, pairs, start) in blocks.items():
    ...
    expected_opposite = {(min(i, j), max(i, j)) for i in single for j in doubled
                         if cross_block_opposite_collinear_slot_mixed_membership(
                             names[i], names[j])}
    expected_position_endpoint = {
        (min(i, j), max(i, j)) for i in single for j in doubled
        if word_position_endpoint_transverse_slot_mixed_membership(names[i], names[j])}
    assert pairs == tuple((i, j) for i in range(len(names)) for j in range(i + 1, len(names))
                          ...
                          or (length == 2 and ((i in single and j in single)
                                             ...
                                             or (i, j) in expected_position_endpoint)))
    if length == 2:
        assert tuple(map(len, (expected_position, expected_same, expected_opposite,
                               expected_position_endpoint))) == (64, 48, 48, 64)
        ...
        assert sum(i not in wrap and j not in wrap for i, j in pairs) == 464
        assert all(((names[i].split("_")[0] in {"step0", "step1"}
                     and names[j].split("_")[0] in {"end0", "end1"})
                    or (names[j].split("_")[0] in {"step0", "step1"}
                        and names[i].split("_")[0] in {"end0", "end1"}))
                   for i, j in expected_position_endpoint)
...
certificate_path = Path("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-word-position-endpoint-transverse-slot-mixed-membership-joint/certificate.json")
...
print("PASS: length-two word-position/endpoint transverse-slot mixed-membership joint decision and certificate", flush=True)
