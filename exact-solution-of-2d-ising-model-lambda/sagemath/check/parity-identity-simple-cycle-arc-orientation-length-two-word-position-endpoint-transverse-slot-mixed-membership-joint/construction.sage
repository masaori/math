"""同軸候補に、語位置と端点を結ぶ異なる軸の両所属混合積を加える。"""
...
def word_position_endpoint_transverse_slot_mixed_membership(first_name, second_name):
    first = first_name.split("_")
    second = second_name.split("_")
    vertical_slots = {"up", "down"}
    horizontal_slots = {"left", "right"}
    transverse = ((first[2] in vertical_slots and second[2] in horizontal_slots)
                  or (first[2] in horizontal_slots and second[2] in vertical_slots))
    position_endpoint = ((first[0] in {"step0", "step1"}
                          and second[0] in {"end0", "end1"})
                         or (second[0] in {"step0", "step1"}
                             and first[0] in {"end0", "end1"}))
    return (position_endpoint
            and transverse
            and {first[1], second[1]} == {"e", "d"})


def length_two_word_position_endpoint_transverse_slot_mixed_membership_joint_system(
                               ...
                               or ({names[i].split("_")[1], names[j].split("_")[1]} == {"e", "d"}
                                   and (same_membership_position_block(names[i], names[j])
                                        ...
                                        or cross_block_opposite_collinear_slot_mixed_membership(
                                            names[i], names[j])
                                        or word_position_endpoint_transverse_slot_mixed_membership(
                                            names[i], names[j]))))))
        start = next_column
