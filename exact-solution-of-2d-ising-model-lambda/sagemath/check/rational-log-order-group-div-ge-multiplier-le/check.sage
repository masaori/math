...
# 仮定 μ ≤ n·ε の μ は、ε の非負倍・ε 自身・ε より小さい元などから作る。
# 一般性のため、すべての (μ, ε) の組で「μ ≤ n·ε が成り立つとき」だけ主張と鎖を検査する。
count_eps = 0
count_claims = 0
count_chain = 0
count_negative = 0
for eps in vectors:
    # 仮定: 0 ≤_{Λ_Q} ε
    if not rational_log_order_le(zero, eps):
        continue
    count_eps += 1
    for mu in vectors:
        for n in range(0, 4):
            # 仮定: μ ≤_{Λ_Q} n·ε
            if not rational_log_order_le(mu, qsmul(QQ(n), eps)):
                continue
            for a in range(1, 6):
                if not n <= a:
                    # 仮定 n ≤ a を満たさない a: 主張は要求しない（否定側の例が実在することだけ数える）
                    if not rational_log_order_le(qsmul(QQ(1) / a, mu), eps):
                        count_negative += 1
                    continue
                # 主張: (1/a)·μ ≤_{Λ_Q} ε
                assert rational_log_order_le(qsmul(QQ(1) / a, mu), eps), (mu, eps, n, a)
                count_claims += 1
                # 準備: 0 ≤ 1/a、n/a ≤ 1（Q の順序）
                assert QQ(0) <= QQ(1) / a
                assert QQ(n) / a <= QQ(1)
                # 第一段: 非負有理数倍の順序保存（c := 1/a、λ := μ、μ := n·ε）
                s1_l = qsmul(QQ(1) / a, mu)
                s1_r = qsmul(QQ(1) / a, qsmul(QQ(n), eps))
                assert rational_log_order_le(s1_l, s1_r)
                # 第二段: 有理数倍の結合則 (1/a)·(n·ε) = ((1/a)·n)·ε
                assert s1_r == qsmul((QQ(1) / a) * QQ(n), eps)
                # 第三段: Q の四則 (1/a)·n = n/a
                assert (QQ(1) / a) * QQ(n) == QQ(n) / a
                assert qsmul((QQ(1) / a) * QQ(n), eps) == qsmul(QQ(n) / a, eps)
                # 第四段: 非負の元の係数比較（r := n/a、s := 1、ν := ε）
                assert rational_log_order_le(qsmul(QQ(n) / a, eps), qsmul(QQ(1), eps))
                # 第五段: 1·ε = ε
                assert qsmul(QQ(1), eps) == eps
                count_chain += 1

print("非負の ε の個数:", count_eps)
print("主張の検査件数:", count_claims)
print("鎖の検査件数:", count_chain)
print("n ≤ a を外すと主張が落ちる例の個数:", count_negative)
assert count_negative > 0
print("PASS: claim_rational_log_order_group_div_ge_multiplier_le")
