"""端点の境界方向を共通基準にした連続三文字の出現回数を判定する。
...
三文字へ作用させ、順序付き三文字の出現回数と、その候補の相対端点十成分を
...
boundary_framed_triples = tuple(sorted({
    (
        transform_letter(word[index], rotation),
        transform_letter(word[(index + 1) % len(word)], rotation),
        transform_letter(word[(index + 2) % len(word)], rotation),
    )
...
boundary_framed_triple_index = {
    triple: index for index, triple in enumerate(boundary_framed_triples)
}


def boundary_framed_triple_candidate_descriptor(arc_type):
...
descriptors = {
    arc_type: boundary_framed_triple_candidate_descriptor(arc_type)
    for arc_type in arc_types
}
...
certificate = {
    "kind": "cyclic-boundary-framed-triple-count-decision",
    ...
    "boundary_framed_triple_count": len(boundary_framed_triples),
...
if all(
    ...
    certificate["conclusion"] = (
        "candidate-wise counts of ordered consecutive internal-letter triples "
...
print(
    "BOUNDARY-FRAMED TRIPLES: %d" % len(boundary_framed_triples),
    flush=True,
)
...
print("PASS: boundary-framed triple-count decision completed", flush=True)
