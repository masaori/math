"""語長二の同所属積に、同じ語位置・端点内の両所属混合積を加える。"""

load("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-three-joint-localization/construction.sage")


def same_membership_position_block(first_name, second_name):
    first = first_name.split("_")
    second = second_name.split("_")
    return first[0] == second[0]


def length_two_position_block_mixed_membership_joint_system(entries, rhs, all_types, column_index):
