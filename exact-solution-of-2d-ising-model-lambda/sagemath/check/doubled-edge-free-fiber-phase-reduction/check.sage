"""反転対の無いファイバーで位相反転部分集合が相殺することを厳密検査する。

対象: claim_doubled_edge_free_fiber_phase_reduction。

一辺 L=2 の全非後退置換を (D,E_1) のファイバーに分け、D が空の各ファイバーを
接触なし、B_L、A_L^(-4)、A_L^(4) の四集合へ分類する。四つのスピン構造ごとに
B_L の位相寄与の総和が零であり、ファイバー全体の和が残る三集合の和に等しい
ことを Q(zeta_8) の等号だけで検査する。浮動小数点は使わない。
"""
...
for phi in nonbacktracking_permutations:
    ...
    if doubled:
        continue
    key = frozenset(single)
    fibers.setdefault(key, []).append(phi)

classified = 0
cancelled = 0
equalities = 0
for single, fiber in fibers.items():
    parts = {"contact_free": [], "phase_reversing": [], -4: [], 4: []}
    for phi in fiber:
        if not contact_pairs(phi):
            ...
            assert is_switchable_contact_pair(phi, pair[0], pair[1])
...
assert classified > 0
print("PASS: L=%d の反転対の無い %d ファイバー、置換 %d 個を四集合へ分類し、"
      "四スピン構造で位相反転部分の零和と簡約等式 %d 件を検査（零和の被加数延べ %d 個）"
      % (L, len(fibers), classified, equalities, cancelled))
