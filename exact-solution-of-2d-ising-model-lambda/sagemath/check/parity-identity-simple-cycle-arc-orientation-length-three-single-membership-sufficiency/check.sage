"""一辺二・三の既存鍵で、単一辺所属の積を残すことの十分性を判定する。
...
pairs = single_membership_quadratic_pairs(names)
start = len(all_types)
assert (len(joint_keys), len(types), len(names), len(pairs), start) == (7085, 2044, 68, 1340, 10098)
assert Counter(pair_membership_kind(names, pair) for pair in pairs) == {None: 1150, ("e", "e"): 190}
...
assert (rank, augmented_rank, solvable) == (8825, 8825, True)
...
if solvable:
    ...
    assert active_kinds[("e", "e")] > 0
    assert active_kinds == {("e", "e"): 21, None: 83}
    values = {}
...
print("PASS: single-membership sufficiency decision and witness", flush=True)
