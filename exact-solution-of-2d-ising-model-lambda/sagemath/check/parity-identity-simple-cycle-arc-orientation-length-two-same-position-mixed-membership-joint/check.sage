"""既存 7,085 鍵で、語長二の両所属混合積を同じ署名位置だけへ制限する有限判定。

対象ラベル: claim_kac_ward_determinant_fiber_stratified_phase_sum
一般の辺長と全語長についての命題ではない。
"""
print("LOAD: constructing finite key data", flush=True)
load("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-same-position-mixed-membership-joint/construction.sage")
...
system, system_rhs, blocks = length_two_same_position_mixed_membership_joint_system(
    entries, rhs, all_types, column_index)
assert system.dimensions() == (12220, 13210)
assert tuple(len(blocks[n][0]) for n in (1, 2, 3)) == (1249, 1842, 2044)
assert tuple(len(blocks[n][2]) for n in (1, 2, 3)) == (946, 1016, 1150)
...
for length, (types, names, pairs, start) in blocks.items():
    ...
    expected_mixed = {(i, j) for i in single for j in doubled
                      if i != j and same_membership_position(names[i], names[j])}
    expected_mixed.update((j, i) for i in doubled for j in single
                          if i != j and same_membership_position(names[i], names[j]))
    expected_mixed = {(min(i, j), max(i, j)) for i, j in expected_mixed}
    ...
    for arc_type in types:
        ...
        assert system_rhs[row_index] == GF(2)(
            sum(bits[12 * k] * bits[12 * k + 2] for k in range(length)) if length == 3 else 0)
        row_index += 1
...
certificate_path = Path("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-same-position-mixed-membership-joint/certificate.json")
...
if solvable:
    ...
    for length, (types, names, pairs, start) in blocks.items():
        ...
        mixed_count = sum({names[i].split("_")[1], names[j].split("_")[1]} == {"e", "d"}
                          for i, j in active)
        print("WITNESS: length=%d terms=%d wrap_terms=%d nonwrap_terms=%d mixed_terms=%d" %
              (length, len(active), wrap_count, len(active) - wrap_count, mixed_count), flush=True)
...
print("PASS: length-two same-position mixed-membership joint decision and certificate", flush=True)
