"""既存 7,085 鍵で、二つの端点を結ぶ異なる軸の両所属混合積 16 項を加える有限判定。

対象ラベル: claim_kac_ward_determinant_fiber_stratified_phase_sum
一般の辺長と全語長についての命題ではない。
"""
print("LOAD: constructing finite key data", flush=True)
load("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-endpoint-transverse-slot-mixed-membership-joint/construction.sage")
...
system, system_rhs, blocks = length_two_endpoint_transverse_slot_mixed_membership_joint_system(
...
for length, (types, names, pairs, start) in blocks.items():
    ...
    expected_opposite = {(min(i, j), max(i, j)) for i in single for j in doubled
                         if cross_block_opposite_collinear_slot_mixed_membership(
                             names[i], names[j])}
    expected_endpoint = {(min(i, j), max(i, j)) for i in single for j in doubled
                         if endpoint_transverse_slot_mixed_membership(names[i], names[j])}
    assert pairs == tuple((i, j) for i in range(len(names)) for j in range(i + 1, len(names))
                          ...
                          or (length == 2 and ((i in single and j in single)
                                             ...
                                             or (i, j) in expected_endpoint)))
    if length == 2:
        assert tuple(map(len, (expected_position, expected_same, expected_opposite,
                               expected_endpoint))) == (64, 48, 48, 16)
        families = (expected_position, expected_same, expected_opposite, expected_endpoint)
        ...
        assert all({names[i].split("_")[0], names[j].split("_")[0]} == {"end0", "end1"}
                   for i, j in expected_endpoint)
...
certificate_path = Path("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-endpoint-transverse-slot-mixed-membership-joint/certificate.json")
...
print("PASS: length-two endpoint transverse-slot mixed-membership joint decision and certificate", flush=True)
