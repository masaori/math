"""既存 7,085 鍵で語長二の非切断項だけを単一辺所属の積に制限する有限判定。

対象ラベル: claim_kac_ward_determinant_fiber_stratified_phase_sum
一般の辺長と全語長についての命題ではない。
"""
print("LOAD: constructing finite key data", flush=True)
load("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-single-membership-joint/construction.sage")
...
system, system_rhs, blocks = length_two_single_membership_joint_system(entries, rhs, all_types, column_index)
assert system.dimensions() == (12220, 13074)
assert tuple(len(blocks[n][0]) for n in (1, 2, 3)) == (1249, 1842, 2044)
assert tuple(len(blocks[n][2]) for n in (1, 2, 3)) == (946, 880, 1150)
...
for length, (types, names, pairs, start) in blocks.items():
    ...
    if length == 2:
        assert sum(i not in wrap and j not in wrap for i, j in pairs) == 120
...
assert (rank, augmented_rank, solvable) == (11764, 11765, False)
print("RESULT: rank=%d augmented_rank=%d solvable=%s" % (rank, augmented_rank, solvable), flush=True)
import json
import sys
from pathlib import Path
record = "--record" in sys.argv
certificate_path = Path("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-single-membership-joint/certificate.json")
...
print("PASS: length-two single-membership joint decision and certificate", flush=True)
