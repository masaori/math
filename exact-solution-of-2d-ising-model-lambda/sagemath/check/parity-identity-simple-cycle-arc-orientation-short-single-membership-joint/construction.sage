...
def short_single_membership_joint_system(entries, rhs, all_types, column_index):
    ...
    for length in (1, 2, 3):
        ...
        pairs = tuple((i, j) for i in range(len(names)) for j in range(i + 1, len(names))
                      if "wrap" in names[i] or "wrap" in names[j]
                      or (length in (1, 2) and names[i].split("_")[1] == "e"
