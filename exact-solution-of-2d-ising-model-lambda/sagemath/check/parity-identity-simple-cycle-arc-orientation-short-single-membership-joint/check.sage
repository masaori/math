"""既存 7,085 鍵で語長一・二の非切断項を単一辺所属の積に制限する有限判定。

対象ラベル: claim_kac_ward_determinant_fiber_stratified_phase_sum
一般の辺長と全語長についての命題ではない。
"""
print("LOAD: constructing finite key data", flush=True)
load("sagemath/check/parity-identity-simple-cycle-arc-orientation-short-single-membership-joint/construction.sage")
...
system, system_rhs, blocks = short_single_membership_joint_system(entries, rhs, all_types, column_index)
assert system.dimensions() == (12220, 12644)
assert tuple(len(blocks[n][0]) for n in (1, 2, 3)) == (1249, 1842, 2044)
assert tuple(len(blocks[n][2]) for n in (1, 2, 3)) == (516, 880, 1150)
...
for length, (types, names, pairs, start) in blocks.items():
    ...
    assert pairs == tuple((i, j) for i in range(len(names)) for j in range(i + 1, len(names))
                          ...
                          or (length in (1, 2) and i in single and j in single))
    if length in (1, 2):
        assert sum(i not in wrap and j not in wrap for i, j in pairs) == (66 if length == 1 else 120)
...
assert (rank, augmented_rank, solvable) == (11692, 11693, False)
print("RESULT: rank=%d augmented_rank=%d solvable=%s" % (rank, augmented_rank, solvable), flush=True)
import json
import sys
from pathlib import Path
record = "--record" in sys.argv
certificate_path = Path("sagemath/check/parity-identity-simple-cycle-arc-orientation-short-single-membership-joint/certificate.json")
...
print("PASS: short-word single-membership joint decision and certificate", flush=True)
