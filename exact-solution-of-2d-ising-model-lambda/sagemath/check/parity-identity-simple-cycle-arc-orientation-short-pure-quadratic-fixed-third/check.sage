"""既存 7,085 鍵で語長三の固定項を保ち語長一・二に純二次式を許す有限判定。

対象ラベル: claim_kac_ward_determinant_fiber_stratified_phase_sum
一般の辺長と全語長についての命題ではない。
"""
print("LOAD: constructing finite key data", flush=True)
load("sagemath/check/parity-identity-simple-cycle-arc-orientation-short-pure-quadratic-fixed-third/construction.sage")
...
system, system_rhs, blocks = short_pure_quadratic_fixed_third_system(entries, rhs, all_types, column_index)
assert system.dimensions() == (12220, 13734)
assert tuple(len(blocks[n][0]) for n in (1, 2, 3)) == (1249, 1842, 2044)
assert tuple(len(blocks[n][2]) for n in (1, 2, 3)) == (946, 1540, 1150)
...
for length, (types, names, pairs, start) in blocks.items():
    ...
    assert pairs == tuple((i, j) for i in range(len(names)) for j in range(i + 1, len(names))
                          if length in (1, 2) or i in wrap or j in wrap)
...
certificate_path = Path("sagemath/check/parity-identity-simple-cycle-arc-orientation-short-pure-quadratic-fixed-third/certificate.json")
...
print("PASS: short pure quadratic fixed-third decision and certificate", flush=True)
