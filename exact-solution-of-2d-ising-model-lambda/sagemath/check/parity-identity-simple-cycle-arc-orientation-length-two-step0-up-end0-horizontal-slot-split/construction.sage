"""先頭語位置の上スロットを使う四項を、先頭端点の左右スロットで二分する。"""
...
def step0_up_end0_horizontal_slot_pair(first_name, second_name, end0_slot):
    if end0_slot not in {"left", "right"}:
        raise ValueError("not a horizontal end0 slot: %s" % end0_slot)
    if not step0_end0_vertical_slot_pair(first_name, second_name, "up"):
        return False
    by_block = {name.split("_")[0]: name for name in (first_name, second_name)}
    return by_block["end0"].split("_")[-1] == end0_slot


def length_two_step0_up_end0_horizontal_slot_system(
        entries, rhs, all_types, column_index, end0_slot):
    assert end0_slot in {"left", "right"}
                               ...
                               or ({names[i].split("_")[1], names[j].split("_")[1]} == {"e", "d"}
                                   and (same_membership_position_block(names[i], names[j])
                                        ...
                                        or cross_block_opposite_collinear_slot_mixed_membership(
                                            names[i], names[j])
                                        or step0_up_end0_horizontal_slot_pair(
                                            names[i], names[j], end0_slot))))))
        start = next_column
