...
#
# 検査すること（claim_open_square_free_entropy_density_upper_bound の証明の中身）:
#   準備の第一: Z^op_{L,L}(q) ∈ QQ_{>0}（ZZ[x] の開境界分配多項式への代入と配位和の一致も見る）、
#               Z^op_{L,L}(q) ≤ 2^{L²}(1+q)^{2L²}。
#   準備の第二: log 2 = ℓ_2（各素数での四段の鎖）。
#   準備の第三: n·ι(ν) = ι(nν)（n = L², 2L²、ν = ℓ_2, log(1+q)）。
#   Λ の鎖: log Z^op_{L,L}(q) ≤_Λ log(2^{L²}(1+q)^{2L²}) = log 2^{L²} + log (1+q)^{2L²}
#            = L² log 2 + 2L² log(1+q) = L² ℓ_2 + 2L² log(1+q)（≤_Λ は rat_Λ を通した QQ の比較）。
#   Λ_Q の鎖: Ψ^op_L(q) = (1/L²)·ι(log Z^op_{L,L}(q)) ≤_{Λ_Q} (1/L²)·ι(L²ℓ_2 + 2L² log(1+q))
...
#   周期境界の finite-free-entropy-density-upper-bound と同じ項目を、開境界正方形の値で検査する。
...
for L in L_RANGE:
    inv = QQ(1) / QQ(L * L)
    Zop = open_partition_polynomial(L, L)
    for q in Q_SAMPLES:
        # 準備の第一（値は ZZ[x] の開境界分配多項式への代入。配位和と一致することも見る）
        value = QQ(Zop(x=q))
        assert value == sum(q ** open_broken_bond_count(L, L, sigma) for sigma in open_configurations(L, L))
        ...
        assert qsmul(L * L, psi) == iota(phi) and qsmul(L * L, s1) == iota(top)  # N = L² は共通分母、証人は log Z^op、top
...
print("PASS: open-square-free-entropy-density-upper-bound (%d checks)" % count)
