"""可解だった縦軸八項を、先頭語位置の上・下スロットの四項族へ分けて判定する。

対象ラベル: claim_kac_ward_determinant_fiber_stratified_phase_sum
一般の辺長と全語長についての命題ではない。
"""
print("LOAD: constructing finite key data", flush=True)
load("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-step0-end0-vertical-slot-split/construction.sage")
...
certificate_path = Path("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-step0-end0-vertical-slot-split/certificate.json")
saved = {} if record else json.loads(certificate_path.read_text())
observed_all = {}

for step0_slot in ("up", "down"):
    print("VARIANT: step0_%s" % step0_slot, flush=True)
    system, system_rhs, blocks = length_two_step0_end0_vertical_slot_system(
        entries, rhs, all_types, column_index, step0_slot)
    assert system.dimensions() == (12220, 13358)
    assert tuple(len(blocks[n][0]) for n in (1, 2, 3)) == (1249, 1842, 2044)
    assert tuple(len(blocks[n][2]) for n in (1, 2, 3)) == (946, 1164, 1150)
    ...
    for length, (types, names, pairs, start) in blocks.items():
        ...
        expected_slot = {
            ...
            if step0_end0_vertical_slot_pair(names[i], names[j], step0_slot)}
        ...
        if length == 2:
            ...
            assert sum(i not in wrap and j not in wrap for i, j in pairs) == 404
...
print("PASS: step0 vertical-slot split decisions and certificates", flush=True)
