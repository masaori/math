"""同軸候補に、二つの語位置を結ぶ異なる軸の両所属混合積を加える。"""
...
def word_position_transverse_slot_mixed_membership(first_name, second_name):
    ...
    return ({first[0], second[0]} == {"step0", "step1"}
            and transverse
            and {first[1], second[1]} == {"e", "d"})


def length_two_word_position_transverse_slot_mixed_membership_joint_system(
                               ...
                               or ({names[i].split("_")[1], names[j].split("_")[1]} == {"e", "d"}
                                   and (same_membership_position_block(names[i], names[j])
                                        ...
                                        or cross_block_opposite_collinear_slot_mixed_membership(
                                            names[i], names[j])
                                        or word_position_transverse_slot_mixed_membership(
                                            names[i], names[j]))))))
        start = next_column
