"""既存 7,085 鍵で語長一の局在形と語長三の固定非切断項を両立させる有限判定。

対象ラベル: claim_kac_ward_determinant_fiber_stratified_phase_sum
一般の辺長と全語長についての命題ではない。
"""
print("LOAD: constructing finite key data", flush=True)
load("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-one-three-fixed-interior/construction.sage")
...
system, system_rhs, blocks = length_one_three_fixed_interior_system(entries, rhs, all_types, column_index)
assert system.dimensions() == (10378, 11698)
assert tuple(len(blocks[n][0]) for n in (1, 3)) == (1249, 2044)
assert tuple(len(blocks[n][2]) for n in (1, 3)) == (450, 1150)
...
certificate_path = Path("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-one-three-fixed-interior/certificate.json")
...
print("PASS: length-one-three fixed interior decision and certificate", flush=True)
