"""既存 7,085 鍵で語長一の局在形と語長三の固定項を保ち語長二に純二次式を許す有限判定。

対象ラベル: claim_kac_ward_determinant_fiber_stratified_phase_sum
一般の辺長と全語長についての命題ではない。
"""
print("LOAD: constructing finite key data", flush=True)
load("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-pure-quadratic-joint/construction.sage")
...
system, system_rhs, blocks = length_two_pure_quadratic_joint_system(entries, rhs, all_types, column_index)
assert system.dimensions() == (12220, 13238)
assert tuple(len(blocks[n][0]) for n in (1, 2, 3)) == (1249, 1842, 2044)
assert tuple(len(blocks[n][2]) for n in (1, 2, 3)) == (450, 1540, 1150)
...
certificate_path = Path("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-pure-quadratic-joint/certificate.json")
...
print("PASS: length-two pure quadratic joint decision and certificate", flush=True)
