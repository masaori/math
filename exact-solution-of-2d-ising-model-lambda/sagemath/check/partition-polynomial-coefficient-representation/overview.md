...
この検証は、その各 Step を小さい格子で実際に数え上げて固定する。

1. 類 $A_m=\{\sigma\in\Sigma_L\mid b(\sigma)=m\}$ を総当たりで作り、
   合併が $\Sigma_L$ に一致すること（被覆）と、$m\ne m'$ で共通元を持たないこと（互いに素）を確かめる。
   本文の主張「配位全体は破れボンド数の値ごとに類別される」の Step 1・Step 2 に対応する。
2. $\Omega_L(m)=|A_m|$ を確かめる（係数表示の Step 1）。
3. 各 $m$ について、類 $A_m$ の上を走る和が $\Omega_L(m)\,x^m$ に等しいことを確かめる
   （係数表示の Step 4）。
4. 定義どおりの和として作った $Z_L$ と、多重度から作った $\sum_m\Omega_L(m)x^m$ が
   多項式として一致し、係数も $m$ ごとに一致することを確かめる（係数表示の Step 5）。
