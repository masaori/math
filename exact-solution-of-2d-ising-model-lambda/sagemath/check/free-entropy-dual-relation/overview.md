...
- 2 行目: 冪の指数法則 $2^{L^2}\cdot2=2^{L^2+1}$
- 3 行目: 四つの $H^{a,b}_L(q)$ を独立に数え上げ、混合双対恒等式の代入と一致すること
- 4 行目: セクター値双対 $H^{a,b}_L(q)=(1+q)^{2L^2}G^{a,b}_L(\mathrm{KW}(q))$ の四つの適用
- 5 行目: 分配則による $(1+q)^{2L^2}$ の括り出し
