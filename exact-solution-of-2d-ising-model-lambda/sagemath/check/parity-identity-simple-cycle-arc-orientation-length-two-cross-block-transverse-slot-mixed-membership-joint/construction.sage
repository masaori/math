"""同軸候補に、異なる位置ブロックの異なる軸のスロットを結ぶ両所属混合積を加える。"""
...
def cross_block_transverse_slot_mixed_membership(first_name, second_name):
...
def length_two_cross_block_transverse_slot_mixed_membership_joint_system(
                               ...
                               or ({names[i].split("_")[1], names[j].split("_")[1]} == {"e", "d"}
                                   and (same_membership_position_block(names[i], names[j])
                                        ...
                                        or cross_block_opposite_collinear_slot_mixed_membership(
                                            names[i], names[j])
                                        or cross_block_transverse_slot_mixed_membership(
                                            names[i], names[j]))))))
        start = next_column
