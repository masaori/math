...
# 確かめる。あわせて人手証明の鎖の 6 段を 1 段ずつ確かめる。
#
...
#   3. 鎖の 6 段（作用の定義・1 項を分ける・単位行列の定義・単位元と零元との積・
#      零元だけの有限和・零元との和）。
...
for L in [1, 2, 3, 4]:
    ...
    for t in R:
        ...
        # 第 4 段: 単位元との積と、零元との積
        step4 = v[t] + sum(QQbar(0) for t1 in R if t1 != t)
        assert step3 == step4
    ...
    # 4. 主張そのもの（写像の等号）
    assert act(R, I, v) == v
    print("L =", L, ": I·v = v を", len(R), "成分すべてで確認（鎖の 6 段も一致）")
