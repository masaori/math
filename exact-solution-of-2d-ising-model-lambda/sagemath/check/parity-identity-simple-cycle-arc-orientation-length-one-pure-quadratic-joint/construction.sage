...
def length_one_pure_quadratic_joint_system(entries, rhs, all_types, column_index):
    ...
    for length in (1, 2, 3):
        ...
        pairs = (tuple((i, j) for i in range(len(names)) for j in range(i + 1, len(names)))
                 if length == 1 else wrap_quadratic_pairs(names))
        start = next_column
