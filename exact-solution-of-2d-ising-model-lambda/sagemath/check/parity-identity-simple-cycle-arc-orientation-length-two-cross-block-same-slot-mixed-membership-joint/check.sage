"""既存 7,085 鍵で、異なる位置ブロックの同名辺スロットを結ぶ両所属混合積を加える有限判定。

対象ラベル: claim_kac_ward_determinant_fiber_stratified_phase_sum
一般の辺長と全語長についての命題ではない。
"""
print("LOAD: constructing finite key data", flush=True)
load("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-cross-block-same-slot-mixed-membership-joint/construction.sage")
...
system, system_rhs, blocks = length_two_cross_block_same_slot_mixed_membership_joint_system(
    entries, rhs, all_types, column_index)
assert system.dimensions() == (12220, 13306)
assert tuple(len(blocks[n][0]) for n in (1, 2, 3)) == (1249, 1842, 2044)
assert tuple(len(blocks[n][2]) for n in (1, 2, 3)) == (946, 1112, 1150)
...
for length, (types, names, pairs, start) in blocks.items():
    ...
    expected_cross = {(min(i, j), max(i, j)) for i in single for j in doubled
    ...
    if length == 2:
        ...
        assert len(expected_cross) == 48
        assert expected_position.isdisjoint(expected_cross)
        assert sum(i in single and j in single for i, j in pairs) == 120
        assert sum(i in doubled and j in doubled for i, j in pairs) == 120
        assert sum(i not in wrap and j not in wrap for i, j in pairs) == 352
        assert all(names[i].split("_")[0] != names[j].split("_")[0]
                   and names[i].split("_")[2] == names[j].split("_")[2]
                   for i, j in expected_cross)
...
certificate_path = Path("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-cross-block-same-slot-mixed-membership-joint/certificate.json")
...
print("PASS: length-two cross-block same-slot mixed-membership joint decision and certificate", flush=True)
