# SageMath Check: 開境界長方形の第一座標方向の反復接合不等式（正の有理点）
...
$(a,b,k)$ を $a\in\{1,2,3\}$、$b\in\{1,2\}$、$k\le4$ のうち接合後の頂点数が 8 以下の 14 通り、
...
- 本文の二場合の上下評価（$0<q\le1$ では $q^{(k-1)b}Z^{\mathrm{op}}_{a,b}(q)^k\le Z^{\mathrm{op}}_{ka,b}(q)
  \le Z^{\mathrm{op}}_{a,b}(q)^k$、$1\le q$ ではその逆向き。$q=1$ は両方に属し、両方を見る）。
- 帰納段（$k\ge2$）: $kb=b+(k-1)b$ の指数の等式、帰納法の仮定に $q^b$ と一枚の値を掛けた評価、
  一辺 $(k-1)a$ と $a$ の二長方形への接合不等式（正の有理点。
...
`openPartitionValueRat_iteratedGlueFirst_bounds_of_le_one/of_one_le`、必要十分版
...
sage sagemath/check/open-rectangle-iterated-gluing-first-rational/check.sage
```
