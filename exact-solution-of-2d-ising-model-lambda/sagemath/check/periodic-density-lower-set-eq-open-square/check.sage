...
#
# 検査すること（0 < q ≤ 1）。集合の等号 A^per(q) = A^op(q) を外延性（両向きの所属）で検査する。
#   候補 μ は、各 L0 の Ψ_{L0}(q)・Ψ^op_{L0}(q) から正の候補 ε を引いた元。
#   (i)  有限範囲で μ ∈ A^per(q) を示せる証人 (ε, N) があるなら、同じ証人で μ ∈ A^op(q)
#        （claim_periodic_density_lower_set_subset_open_square_le_one）。
#   (ii) 有限範囲で μ ∈ A^op(q) を示せる証人 (ε, N) があるなら、証人 (ε' := (1/2)·ε, N' := N + n)
#        （n は Archimedes 性の倍率）で μ ∈ A^per(q)（claim_open_square_density_lower_set_subset_periodic_le_one）。
#        密度の列は L ≤ L_MAX までしか作れないので、N' ≤ L_MAX の証人だけを数える。
...
for q in test_points:
    ...
    assert rational_log_order_le(zero, two_delta)
    total += 1
    for L0 in range(1, L_MAX + 1):
        for base in (seq_per[L0], seq_op[L0]):
            for eps in eps_candidates:
                mu = lamq_add(base, qneg(eps))
                for N in range(1, L_MAX + 1):
                    # (i) A^per(q) ⊂ A^op(q): 同じ証人
                    if in_lower_set_with_witness(seq_per, mu, eps, N):
                        per_members += 1
                        assert in_lower_set_with_witness(seq_op, mu, eps, N)
                        total += 1
                    # (ii) A^op(q) ⊂ A^per(q): 証人 (ε', N')
                    if in_lower_set_with_witness(seq_op, mu, eps, N):
                        op_members += 1
...
print(f"周期境界の密度の下組と開境界正方形の密度の下組は等しい（q は 1 以下。ZZ/QQ と素因数分解で厳密）: A^per の証人 {per_members} 組、A^op の証人 {op_members} 組（うち N' ≤ L_MAX が {op_nonvacuous} 組）、{total} 検査 OK")
