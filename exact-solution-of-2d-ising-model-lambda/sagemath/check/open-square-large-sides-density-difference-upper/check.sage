...
#
# 検査すること（claim_open_square_large_sides_density_difference_upper_le_one の証明の中身）:
#   上端（claim_open_square_large_side_density_upper_vs_base_side_le_one を辺 a, L で読む）: Ψ^op_L ≤ U + Ψ^op_a、
#     U := (2/a)ι(ℓ_2) + (4/a)ι(log(1+q))。
#   下端（claim_open_square_large_side_density_lower_vs_base_side_le_one を辺 a, M で読む）:
#     (D + Ψ^op_a) + (−(2/a)C) ≤ Ψ^op_M、D := (4/a)ι(log q)、C := ι(ℓ_2) + 2ι(log(1+q))。
#   準備: 下端の両辺に (−D) + (2/a)C を足すと左辺は Ψ^op_a（結合則・交換則・逆元・単位元）、
#     よって Ψ^op_a ≤ Ψ^op_M + ((−D) + (2/a)C)。
#   本体: Ψ^op_L + (−Ψ^op_M) ≤ (U + Ψ^op_a) + (−Ψ^op_M) = Ψ^op_a + ((−Ψ^op_M) + U)
#     ≤ (Ψ^op_M + ((−D) + (2/a)C)) + ((−Ψ^op_M) + U) = (Ψ^op_M + (−Ψ^op_M)) + (U + ((−D) + (2/a)C))
#     = 0 + (U + ((−D) + (2/a)C)) = U + (−D) + (2/a)C、推移律の結論。
...
def check_difference_upper(a, L, M, q):
    ...
    # 上端と下端（既に検証済みの主張を、この形で読み直す）
    upper_rhs = lamq_add(U, psi)
    lower_lhs = lamq_add(lamq_add(D, psi), lamq_neg(tC))
    assert rational_log_order_le(psi_L, upper_rhs)
    assert rational_log_order_le(lower_lhs, psi_M)
    checks += 2

    # 準備: 下端の両辺に ν := (−D) + (2/a)C を足す
    nu = lamq_add(lamq_neg(D), tC)
    prep_lhs = lamq_add(lower_lhs, nu)
    prep_rhs = lamq_add(psi_M, nu)
    assert rational_log_order_le(prep_lhs, prep_rhs)                                   # 加法単調性
    rearranged = lamq_add(psi, lamq_add(lamq_add(D, lamq_neg(D)), lamq_add(lamq_neg(tC), tC)))
    assert prep_lhs == rearranged                                                       # 結合則・交換則
    assert lamq_add(D, lamq_neg(D)) == zero and lamq_add(lamq_neg(tC), tC) == zero      # 逆元
    assert rearranged == lamq_add(psi, lamq_add(zero, zero))
    assert lamq_add(psi, lamq_add(zero, zero)) == psi                                   # 単位元
    assert rational_log_order_le(psi, prep_rhs)                                         # 準備の結論
    checks += 6

    # 本体
    lhs = lamq_add(psi_L, lamq_neg(psi_M))
    step1 = lamq_add(upper_rhs, lamq_neg(psi_M))
    step2 = lamq_add(psi, lamq_add(lamq_neg(psi_M), U))
    step3 = lamq_add(prep_rhs, lamq_add(lamq_neg(psi_M), U))
    step4 = lamq_add(lamq_add(psi_M, lamq_neg(psi_M)), lamq_add(U, nu))
    step5 = lamq_add(zero, lamq_add(U, nu))
    rhs = lamq_add(lamq_add(U, lamq_neg(D)), tC)
    assert rational_log_order_le(lhs, step1)     # 上端に −Ψ^op_M を足す（加法単調性）
    assert step1 == step2                        # 結合則・交換則
    assert rational_log_order_le(step2, step3)   # 準備の結論に (−Ψ^op_M) + U を足す（加法単調性）
    assert step3 == step4                        # 結合則・交換則
    assert lamq_add(psi_M, lamq_neg(psi_M)) == zero and step4 == step5   # 逆元
    assert step5 == rhs                          # 単位元と結合則
    assert rational_log_order_le(lhs, rhs)       # 推移律の結論（主張）
    checks += 7
    return checks
...
for a, L, M in shapes:
    for q in test_points:
        total += check_difference_upper(a, L, M, q)

print(f"基準辺の平方以上の二つの辺の密度の差の一様な上からの評価（q は 1 以下。ZZ/QQ と素因数分解で厳密）: {total} 検査 OK")
