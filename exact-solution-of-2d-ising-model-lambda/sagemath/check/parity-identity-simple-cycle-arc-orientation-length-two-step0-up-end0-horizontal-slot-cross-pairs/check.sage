"""所属向きの異なる二項を先頭端点の左右から一つずつ選ぶ。

対象ラベル: claim_kac_ward_determinant_fiber_stratified_phase_sum
一般の辺長と全語長についての命題ではない。
"""
print("LOAD: constructing finite key data", flush=True)
load("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-step0-up-end0-horizontal-slot-cross-pairs/construction.sage")
...
certificate_path = Path("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-two-step0-up-end0-horizontal-slot-cross-pairs/certificate.json")
saved = {} if record else json.loads(certificate_path.read_text())
observed_all = {}

variants = (("left", "right"), ("right", "left"))
for step0_single_slot, end0_single_slot in variants:
    variant = "step0_single_end0_%s__end0_single_end0_%s" % (
        step0_single_slot, end0_single_slot)
    print("VARIANT: %s" % variant, flush=True)
    system, system_rhs, blocks = length_two_step0_up_end0_horizontal_slot_cross_system(
    ...
    for length, (types, names, pairs, start) in blocks.items():
        ...
        expected_opposite = {(min(i, j), max(i, j)) for i in single for j in doubled
                             if cross_block_opposite_collinear_slot_mixed_membership(
                                 names[i], names[j])}
        expected_step0_single = {
            (min(i, j), max(i, j)) for i in single for j in doubled
            if step0_up_end0_slot_membership_pair(
                names[i], names[j], "step0", step0_single_slot)}
        expected_end0_single = {
            (min(i, j), max(i, j)) for i in single for j in doubled
            if step0_up_end0_slot_membership_pair(
                names[i], names[j], "end0", end0_single_slot)}
        expected_cross = expected_step0_single | expected_end0_single
        assert pairs == tuple((i, j) for i in range(len(names)) for j in range(i + 1, len(names))
                              ...
                              or (length == 2 and ((i in single and j in single)
                                                 ...
                                                 or (i, j) in expected_cross)))
        if length == 2:
            assert tuple(map(len, (expected_position, expected_same, expected_opposite,
                                   expected_step0_single, expected_end0_single))) == (64, 48, 48, 1, 1)
            families = (expected_position, expected_same, expected_opposite,
                        expected_step0_single, expected_end0_single)
            assert all(families[i].isdisjoint(families[j])
                       for i in range(len(families)) for j in range(i + 1, len(families)))
            assert len(expected_cross) == 2
            assert sum(i not in wrap and j not in wrap for i, j in pairs) == 402
            selected_names = {
                tuple(sorted((names[i], names[j]))) for i, j in expected_cross}
            assert selected_names == {
                tuple(sorted(("step0_e_up", "end0_d_%s" % step0_single_slot))),
                tuple(sorted(("step0_d_up", "end0_e_%s" % end0_single_slot))),
            }
...
print("PASS: opposite-membership horizontal-slot cross-pair decisions and certificates", flush=True)
