"""既存 7,085 鍵で語長一・二の局在形と語長三の固定非切断項を両立させる有限判定。

対象ラベル: claim_kac_ward_determinant_fiber_stratified_phase_sum
一般の辺長と全語長についての命題ではない。
"""
print("LOAD: constructing finite key data", flush=True)
load("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-three-fixed-interior-joint/construction.sage")
...
system, system_rhs, blocks = length_three_fixed_interior_joint_system(entries, rhs, all_types, column_index)
...
certificate_path = Path("sagemath/check/parity-identity-simple-cycle-arc-orientation-length-three-fixed-interior-joint/certificate.json")
...
print("PASS: length-three fixed interior joint decision and certificate", flush=True)
