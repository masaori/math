...

  {
    ...
    statement: [
      ...
      paragraph([
        ...
        " に対し、算術の基本定理（1 以上の整数は素数の積として、順序を除いてただ一通りに書ける）により、",
        math(String.raw`n`),
        " は",
      ]),
      displayMath(String.raw`n=\prod_{p\in\mathcal{P}}p^{\,v_p(n)}`),
      paragraph([
        "の形にただ一通りに書ける。ここで各 ",
      ...
      paragraph([
        ...
        " である（",
        math(String.raw`1`),
        " は空積であり、どの素数も因子に持たない）。第二に、1 以上の整数 ",
        math(String.raw`m,n`),
        " に対し",
      ]),
      ...
      paragraph([
        "が成り立つ（指数の加法性）。実際、",
        math(String.raw`m=\prod_p p^{\,v_p(m)}`),
        " と ",
        math(String.raw`n=\prod_p p^{\,v_p(n)}`),
        " を掛けると ",
        math(String.raw`mn=\prod_p p^{\,v_p(m)+v_p(n)}`),
        " となり、これは ",
        math(String.raw`mn`),
        " を素数の積として書いたものである。算術の基本定理の一意性より、この表示の指数は ",
        math(String.raw`v_p(mn)`),
        " に一致する。",
      ]),
      paragraph([
...

  {
    ...
    statement: [
      ...
      paragraph([
        "名前に「順序」が入っているとおり、",
        math(String.raw`\Lambda`),
        " には正の有理数の大小と整合する全順序を入れることができる。",
        "本章ではその順序を使わないので、ここでは加法群としての構造だけを定める。",
        "順序が必要になる箇所（零点の詰め寄りを述べるとき）で、そこで必要な形として導入する。",
      ]),
    ],
  },
