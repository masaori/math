...
export default defineNotes([
  {
    ...
    body: [
      ...
      paragraph([
        "追試として、PARI スタックを 8 GiB へ拡張して同じ経路を再実行したところ、スタック超過は再発しなかったが 540 秒でも完了しなかった。",
        "限界はスタック容量ではなく実代数的数の最小多項式計算の時間であり、",
        math(String.raw`L=3`),
        " を通すには数体の統合を避ける経路の置き換え（有理係数多項式環の終結式で距離の多項式を直接作る等）が要る。",
      ]),
    ],
  },
]);
