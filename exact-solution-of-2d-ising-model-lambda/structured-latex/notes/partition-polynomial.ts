...
export default defineNotes([
  {
    ...
    body: [
      paragraph([
        ...
        " と定義する。エネルギーを ",
        math(String.raw`\mathcal{E}(\sigma)=-J|E_L|+2J\,b(\sigma)`),
        " と書けば、全体因子を外して ",
        math(String.raw`x=e^{-2\beta J}`),
        " とおくことで ",
        math(String.raw`\sum_\sigma x^{b(\sigma)}`),
        " が得られる。",
      ]),
      paragraph([
        "この経路は本文では採用しない。",
  ...
  },
  {
    ...
    body: [
      paragraph([
        ...
        "破れボンド数の多重度は ",
        math(String.raw`\Omega_{C_4}(0)=2`),
        "、",
        math(String.raw`\Omega_{C_4}(2)=12`),
        "、",
        math(String.raw`\Omega_{C_4}(4)=2`),
        " であり、分配多項式は",
      ]),
      displayMath(String.raw`Z_{C_4}=2+12x^2+2x^4\in\mathbb{Z}[x]`),
      paragraph([
        "となる。係数の総和は ",
      ...
      paragraph([
        "本文の格子で ",
        math(String.raw`L=2`),
        " とした場合は、辺の番号が 8 個あり（4 サイクルの各辺が 2 重になる）",
        math(String.raw`Z_2=2+12x^4+2x^8`),
        " となる。指数が 4 サイクルのちょうど 2 倍なのはそのためである。",
        math(String.raw`L=3`),
        " では ",
        math(String.raw`Z_3=2+18x^4+48x^6+198x^8+144x^{10}+102x^{12}`),
        " で、係数の総和は ",
