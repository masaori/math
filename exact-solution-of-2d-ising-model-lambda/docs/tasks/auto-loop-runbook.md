...
8. **式変形の書き方の統一を 1 件進める**（下の「並列の作業ストリーム」）。本プロジェクトの前進とは別の
   ...
   **ただし、実数体への脱出の後ろ倒しが済むまでは、この手順を飛ばす**（下記の一時停止。
   ユーザー指示 2026-08-15「先に直して」）。
...
> **いまは一時停止中**（2026-08-15〜）。ユーザーが「実数体への脱出の後ろ倒しを先に直せ」と
> 指示したため、セクション台帳の「切断による ℝ への一度きりの脱出」と「旧実数値経路を撤去する」が
> 済むまで、このストリームは進めない。tick の持ち時間はすべて本文の前進へ充てる。
> 済んだら、この段落を消して再開する（到達点は下記の台帳に残してある）。
