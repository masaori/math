...
- **URL を決め打ちしない**（リポジトリの所有が移って決め打ちの URL が 404 になった実例がある。
  ...
  公開に成功したら Slack へ流す（同じ版では二度送らない）。**本文は台帳の「現在地」の先頭に
  ある太字の要点**——つまりその tick が何をしたか——を 240 字までで送る。コミットの件名ではない
  （件名では中身が分からない）し、箇条書き全体でもない（実装の詳細まで送ると長すぎる)。
  台帳の節の名前、直近の tick を先頭に置く並び、要点を太字で囲う書き方を変えると、この通知が壊れる。
- **このプロジェクトの中身が動いていなければ、公開も通知もしない。** 同じリポジトリで
  3 次元 Ising 側のループも push しているため、版が変わっただけでは論文は変わらない。
  最後に通知した版から `exact-solution-of-2d-ising-model-lambda/` に差分が無ければ何もせず終える
  （実測 2026-08-14: この判定が無く、姉妹側の push で Slack が 2 通ずつ届いていた）。
...
**tick は Slack へ通知しない**（`slack-notification` skill も curl も使わない）。
前進した tick の報告は `scripts/publish-artifact.sh` が公開処理の中で 1 通だけ送る。
**tick 側も送ると 1 通の事象に 2 通届く**（実測 2026-08-14）。
**リポジトリ全体の「作業完了時は Slack へ報告する」規約より、このループではこちらが優先する。**

- 通知の本文は台帳の「現在地」の先頭項目である。**だから「現在地」の先頭に、その tick で
  何をしたかを 1〜2 文で簡潔に書く**（通知は先頭 240 字で切る）。
- 例外は **止まったとき**だけである（下の「止まってよい場合」に該当し人間の判断を待つとき）。
  このときは `slack-notification` skill で何が論点かを報告する。
- 検証が落ちて自分で直した、レビューで誤りを直した、といったことは台帳に書けばよい。
