...
- **公開スクリプトは Slack へ送らない**（ユーザー指示 2026-08-15）。人間への報告は tick 側の
  1 通に一本化してある。tick は最後に自分で公開スクリプトを走らせ、ログ末尾の「OK: 公開した」の
  行から URL を読み、何をしたかを 1〜2 文書いて URL を添えて送る（下の「報告（Slack）」）。
...
**報告は tick が `slack-notification` skill で 1 通だけ送る**（ユーザー指示 2026-08-15。
公開スクリプトからは送らない。**1 つの事象に 2 通届くのを避けるため、送り手はどちらか一方に
固定する**——実測 2026-08-14 に両方から送って重複した）。

- **tick の最後に必ず送る。** 送らなければ誰にも届かない。
- 本文は **その tick で何をしたか**を 1〜2 文で具体的に書く。台帳の文章をそのまま貼らない
  （長すぎて読まれない）。末尾に論文の公開 URL を添える。
- **URL は `logs/publish-artifact.log` 末尾の「OK: 公開した（版 …）→ …」から読む。**
  決め打ちしない。その行が今回の版でなければ（公開が走らなかったので）URL は添えない。
- 検証が落ちて自分で直した、レビューで誤りを直した、といったことは台帳に書けばよく、通知に含めない。
- **止まったとき**（下の「止まってよい場合」に該当し人間の判断を待つとき）も同じ skill で、
  何が論点かを報告する。
