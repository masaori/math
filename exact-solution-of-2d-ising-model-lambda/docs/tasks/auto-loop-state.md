...
（これより古い 252 件は [auto-loop-archive.md](auto-loop-archive.md) へ移した。）
...
| 章 | セクション | 状態 | 備考 |
|---|---|---|---|
| 有限系の自由エントロピー | 対数順序群の順序 | todo | $\lambda\le\mu:\iff\log^{-1}(\lambda)\le\log^{-1}(\mu)$（$\mathbb{Q}$ の順序）。反射・推移・反対称・三分律は $\mathbb{Q}$ から移す。加法単調性は $\log$ の加法性（対数の加法性）で正の有理数の積の単調性へ落とす。決定可能（有理数の比較） |
