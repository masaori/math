...
（これより古い 227 件は [auto-loop-archive.md](auto-loop-archive.md) へ移した。）
...
| 章 | セクション | 状態 | 備考 |
|---|---|---|---|
| 熱力学極限 | 接合不等式・破れボンド数の接合面分解の Lean | todo | 接合の全単射の段は済（`OpenRectangleGluing.lean`）。本文は記述と SageMath 済 |
| 熱力学極限 | 接合不等式・実数評価の上下評価の Lean | todo | 分解と併せて claim の具体版・必要十分版・導出を閉じ、セクション完了にする |
...
（これより古い 238 件は [auto-loop-archive.md](auto-loop-archive.md) へ移した。）
