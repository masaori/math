...
| 章 | セクション | 状態 | 備考 |
|---|---|---|---|
| 熱力学極限 | 零点密度: 一次因子の冪どうしが互いに素であること（$(t-\widehat w)^{k}$ と $(t-\widehat{w'})^{m}$ が Bezout 恒等式で結ばれること、$w\ne w'$） | todo | `claim_qbar_bezout_power_propagation` を $a:=t-\widehat w$、$b:=t-\widehat{w'}$ と、$a,b$ を入れ替えて二度使う（$k,m$ それぞれの冪へ伝播させる） |
