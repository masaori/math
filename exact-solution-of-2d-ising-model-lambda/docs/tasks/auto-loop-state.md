...
- **2026-08-15 の tick 292 は、「倍数でない辺への拡張」を六つに割り直し、その先頭
  ...
  「この先に書くこと」に済んだ項目が残っていたのを消しただけ。次は「開境界長方形の値の配位数による
  上からの評価」。
...
| 章 | セクション | 状態 | 備考 |
|---|---|---|---|
| 熱力学極限 | 開境界長方形の値の配位数による上からの評価 | todo | $0<t\le1$ で $Z^{\mathrm{op}}_{a,b}(t)\le2^{ab}$、$1\le t$ で $Z^{\mathrm{op}}_{a,b}(t)\le2^{ab}t^{2ab}$（各項の底と指数を上げ、配位数で数える） |
...
- 2026-08-15（tick 292）: **割り直し**: 「倍数でない辺への拡張」は、値の評価二本・部分正方形との
  ...
  正の元の冪の非負性だけ）、導出版。sorry 検査 1056 件。
