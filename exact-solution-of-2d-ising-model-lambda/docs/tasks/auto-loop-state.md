...
| 章 | セクション | 状態 | 備考 |
|---|---|---|---|
| 臨界指数を零点列で書く | 有限サイズスケーリングの条件付き読み（実対数・極限による ℝ 脱出） | todo | `def_real_closed_realization` のデータを固定し、$d_1(L)$ は距離の二乗なので $-\log_{\mathbb R}(\rho(d_1(L)))/(2\log_{\mathbb R}L)$ が $1/\nu$ へ収束することを「指数 $\nu$ を読める」条件として定義する。その極限の存在も 2 次元 Ising で $\nu=1$ となることも主張しない |
