...
| 章 | セクション | 状態 | 備考 |
|---|---|---|---|
| 熱力学極限 | 実数体への脱出を極限の一点へ後ろ倒しする（設計変更。**先にこれを片付ける**） | todo | 人間の判断（2026-08-15）。いまは章頭で ℝ を宣言しているが、宣言している性質（順序体・ℚ からの順序を保つ埋め込み）は ℚ も満たすので、その 2 つだけで済む主張は可算側で書ける。$\log_{\mathbb Q}$ は存在しない（狭義単調な準同型 $\mathbb{Q}_{>0}\to\mathbb{Q}$ があると $f(3)/f(2)=\log3/\log2$ が有理数になって矛盾）が、$\Lambda$ 値の対数は既にある。有限系は $\Phi_L$ と $\Lambda\otimes\mathbb{Q}$ で通す |
