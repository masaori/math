...
| 章 | セクション | 状態 | 備考 |
|---|---|---|---|
| Onsager 閉形式への接続 | 四つの Kac--Ward 行列式による有限トーラス公式 | 軌道列の頂点単純閉路分解まで記述と SageMath | `claim_reversal_free_orbit_vertex_simple_decomposition` の本文・SageMath が揃った（Lean 未着手）。次はこの主張の Lean 二版を書き、その後に平方恒等式 $D^{a,b}_L=(Q^{a,b}_L)^2$ へまとめる。 |
