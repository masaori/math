...
| 章 | セクション | 状態 | 備考 |
|---|---|---|---|
| Onsager 閉形式への接続 | 四つの Kac--Ward 行列式による有限トーラス公式 | 軌道列の台の辺の相異なりは記述と SageMath まで | `claim_reversal_free_orbit_support_edges_distinct`（軌道列が接触点分割の仮定を満たすこと）を本文と SageMath で固定した。次はその Lean 二版を書き、軌道列へ接触点分割を適用して平方恒等式 $D^{a,b}_L=(Q^{a,b}_L)^2$ へまとめる。 |
