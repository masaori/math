...
| 章 | セクション | 状態 | 備考 |
|---|---|---|---|
| 臨界指数を零点列で書く | 挟み込み区間から臨界点への有理近似を取る | todo | 正の有理端点 $q:=(k+1)/N$ を取り、$q-x_c\le_Rh_N$（差の保存）と平方の単調性（差の因数分解と二平方和の二性質）、$h_N^2<\delta$ の $<_R$ への移送（正の有理数の正値性）から $(x_c-q)^2<_R\delta$ を出す |
