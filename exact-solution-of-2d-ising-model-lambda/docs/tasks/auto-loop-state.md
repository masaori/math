...
- 次に進めるセクション: **転送行列 / 行配位と、破れボンド数の行内・行間への分解**（台帳の todo の先頭）
- 前回のレビュー到達点: 章「分配多項式」の全体（前々 tick まで）に加え、章「有限系の自由エントロピー」の
  定義 4 件・主張 2 件と、対応する Lean 5 ファイル・SageMath 検証を一巡した。
  次 tick は、この tick で書いた同章の主張 3 件（対数の加法性・対数の冪の法則・
  すべての配位を等しく数える点での自由エントロピー $\Phi_L(1)=L^2\ell_2$）を読み直すところから始める。
  特に見るべき点は 3 つある。
  第一に、対数順序群の定義へ今回足した整数倍（素数ごとの積として定めた）が、冪の法則の本文で
  使っている $k\,\log q$ と、Lean 側の `k • ·`（$k$ 個の和）と、三者で食い違っていないか。
  第二に、$\Phi_L(1)$ の証明が引く 2 つの主張（係数表示・多重度の総和）の適用が、
  Lean 側の `partitionPolynomial_eval_one` の中の 2 つの書き換えと 1 対 1 に対応しているか。
  第三に、$\Phi_L(1)=L^2\ell_2$ を「1 頂点あたり $\log 2$」と読み替える記述が本文へ漏れていないか
  （その読み替えは実数への脱出と密度を含むので、ノート `notes/free-entropy.ts` へ隔離してある）。
- 最終更新: 2026-08-08（tick 6 実行）
