...
- 前回のレビュー到達点: `content/partition-polynomial.ts` の定義 4 件と主張 1 件、
  および `sagemath/_shared/defs.sage` と検証まで見た。次 tick は `notes/` と `README`・`MEMORY` の
  記述が現行の定義と合っているかから見る。
- 最終更新: 2026-08-08（tick 1 実行。レビューと修正まで済み、前進は未了）
