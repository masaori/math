...
| 章 | セクション | 状態 | 備考 |
|---|---|---|---|
| 熱力学極限 | 有理係数の対数順序群の順序の定義と線形順序性 | todo | 有限台の有理係数へ共通分母を掛けて $\Lambda$ へ戻し、$w:\mathbb{Q}_{>0}\to\Lambda$ の逆像の有理数比較で定める。共通分母の取り方に依らないこと、三分律・推移律・加法単調性、決定可能性を示す。有限系密度の分母消去は直前のセクションで済んだ |
