...
| 章 | セクション | 状態 | 備考 |
|---|---|---|---|
| Onsager 閉形式への接続 | 四つの Kac--Ward 行列式による有限トーラス公式 | 符号付き偶部分グラフ多項式は記述と SageMath まで | `def_signed_even_subgraph_polynomial` と `claim_signed_even_subgraph_sector_sum`（$Q^{a,b}_L=\sum_{(c,d)}\pm G^{c,d}_L$）を本文と SageMath で固定した。次はこの主張の Lean 二版を書き、その後に非後退置換和と横断消去・接触点分割を結んで平方恒等式 $D^{a,b}_L=(Q^{a,b}_L)^2$ へまとめる。 |
