...
| 章 | セクション | 状態 | 備考 |
|---|---|---|---|
| Onsager 閉形式への接続 | 四つの Kac--Ward 行列式による有限トーラス公式 | 符号付き偶部分グラフ多項式は四層完了 | `claim_signed_even_subgraph_sector_sum`（$Q^{a,b}_L=\sum_{(c,d)}\pm G^{c,d}_L$）の Lean 具体版・必要十分版まで配線した。次は非後退置換和と横断消去・接触点分割を結んで平方恒等式 $D^{a,b}_L=(Q^{a,b}_L)^2$ へまとめる。 |
