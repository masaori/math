...
| 章 | セクション | 状態 | 備考 |
|---|---|---|---|
| 有限系の自由エントロピー | 正の有理数の対数は全射である | todo | $\lambda\in\Lambda$ に対し $q_\lambda:=\prod_{\lambda(p)>0}p^{\lambda(p)}\big/\prod_{\lambda(p)<0}p^{-\lambda(p)}$ を作り、素数ごとの指数を読んで $\log q_\lambda=\lambda$。単射性（済）と合わせて $\log$ は全単射。逆写像に名前を付ける |
