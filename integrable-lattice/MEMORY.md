...
- **cycle 37（進行中）step 1: 命題 W\* の残り 2 件のうち 1 件を埋め、1 件を半分埋めた。件数は動いていない（残り 21 件）。**
  **$\det G=\pm N(\eta)$ の可約な場合は埋まった**（`EulerDualBasis.det_weightedGram`）——
  **判別式を経由しなければ体は要らない**。段 6 の $C\,G=M_\eta$ の行列式を取り、$\det C=\pm1$ を使うだけである。
  **冪基底の仮定の当てはめも埋まった**（`WStarPowerBasisInstance.lean`）。
  **$\eta$ が零因子でないことは $N(\eta)\neq0$ と同値である**ことを書き（mathlib の
  `Algebra.norm_ne_zero_iff` は $A$ が整域であることを要求するので、行列の側から書き直した）、
  $\det G\neq0$ から出る形にした。**残るのは、$\rho$ が無平方であることから $\det G\neq0$ を出す段だけである。**
  止めた理由は素材で、無平方性を $\mathbb{Z}[x]$ から $\mathbb{Q}[x]$ へ移す Gauss 型の補題が mathlib に無い
  （2026-08-05 実測、`logs/mathlib-gap-survey-cycle37-squarefree.log`）。素材そのものは在るので書ける見込みはある。

- **cycle 37（進行中）step 2: 外部定理「Kirchhoff の matrix-tree 定理」が完了した（残り 21 → 20 件）。本文側の 16 件は動いていない。**
  cycle 36 が挙げた見立て（後半には頂点の部分集合を引数に持つ形への書き直しが要る）は**当たり**、
  書き直したら帰納法はそのまま通った（`SpanningConnectivity` の段 6）。
  芯は「葉を取り除いても残りは根から届く」で、素直に書くと回らないので
  **「葉へ着いたらその辺のもう一方の端まで戻す」と主張を強めて回した。**
  Kirchhoff の本体（根の行を落としたラプラシアンの行列式＝全域木の個数）まで書いた。
  **閉路の型は最後まで作っていない**（本数を $|V|-1$ に固定すると「連結」と同値になる）。
  **振り分けを 1 件直した**——指標分解はこの外部定理の残りではない（本文が「指標による対角化と
  Kirchhoff の matrix-tree 定理から」と 2 つを並べて引いている）ので、本文の主張の側へ移した。
  **この読み替えで残りが 1 件減っている。基準を緩めたのではない。**
  この壁が塞いでいた本文の主張 5 件は、5 件とも matrix-tree は外れたが、
  いずれも別の残りを持つので 1 件も完了していない。

- **cycle 37（進行中）step 3: 命題 C′ と 命題 C″ が共有していた壁を書いた（件数は動かず 20 件）。**
  **cycle 19 から 18 サイクル持ち越されていた「素材が無い」が誤りだった**——
  「$p^{w^*}G^{-1}$ が $p$ 進整数成分をもつ」に**整数行列の単因子は要らない**。
  要るのは $G$ の像についての `IsPLevel` だけで、それは cycle 29 の `isLeast_isPLevel` が
  **部分加群の**適合基底から既に与えていた（mathlib に在るのは部分加群の Smith 標準形である。
  「行列の単因子が無い」という実測は正しく、**誤っていたのは「だから書けない」という推論**である）。
  **「素材が無い」が「素材ではなく書き方の問題だった」に化けたのはこれで 3 件目**（cycle 35・36 に続く）。
  命題 C′ の残りは 2 → 1、命題 C″ の残りは 3 → 2 になったが、どちらも完了はしていない。
