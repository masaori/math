...
- **cycle 40 step 1（2026-08-05）: 命題 W\* の掲げた残り 1 件（$\chi$ から因子の族を取り出す段）を書いた。
  件数は動いていない（残り 19 件のまま）。そう書く。**
  芯は「相異なるモニックな素元は互いに割らない（同伴なモニック多項式は等しいから）」で、
  一意分解の因子をモニック化して数え上げれば重複度になる。整数行列の特性多項式へ当てるところまで書いた。
  検査 E の実出力は 受け取る 3 件 → **0 件**。
  **それでも完了しない。書いてみて、残っているものの中身が違うことが分かった**——
  **cycle 39 が記号を取り違えていた。** cycle 39 が「本文の $\mu$」と書いた `multWeight` は $\chi'/h$ であり、
  本文の記号では $\eta$ である（本文は $\eta=\mu\,\rho'(\theta)$ と書き、$\mu$ は成分ごとに重複度をとる別の元）。
  したがって本文の $G$（重みが $\mu$）についての主張はまだ無い。**残りは 2 つになった。**
  併せて cycle 39 の記録の数の誤りを 1 件見つけた（`check-no-sorry.sh` の列挙は 654 でなく 656）。
