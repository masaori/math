...
- **実測**: 日本語版 18 ページ / 英語版 27 ページ。どちらも未解決参照 0・組めない文字 0・版面外 0。
...
### この作業で見つかった日本語正本の誤り 7 件（すべて修正済み）
...
- **生成器の地の文 Unicode 変換表に θ が無く、cycle 19 の新章が入った時点で
  日本語版の PDF ビルドが「組めない文字」で落ちていた**（main 側で PDF を作っていないため未発見）。
  θ を含むギリシャ文字 18 字を補った。英語版は同じ表を import している。
...
- **英語版は main の f45f7c8 時点の日本語正本に対応する。** main は自動ループで進み続けるので、
