...
## 完了（2026-07-27）: 構造化テキスト基盤に生成器を足し、企画書 002 の確定命題を移設した

**論文 001（ℝ/Λ 双対）が PDF として出るところまで到達した。** ツール側の作業であり、
数学的判断（証明の再構成・成立条件の確定）は一切していない。

- **生成器を移植**: `tools/build-latex.ts` / `latex-escape.ts` / `verify-no-notes-in-output.ts` を
  Ising 側から移植し、本プロジェクト固有の要求に合わせた。**住処（`habitat`）と ℝ 脱出（`realEscape`）を
  PDF にも印字する**（データに持つだけでは読者に伝わらないため）。見出しは level 1 を章（`\section`）へ写す。
- **散文中の Unicode 数学記号を LaTeX へ写す** `tools/unicode-math.ts` を追加。
  ℝ/ℤ/ℚ/μ が PDF から**無言で消えていた**のを、Ising 側から移植した「組めない文字はビルドを落とす」検査が
  実際に検出した。content のデータは書き換えていない。
- **複製が腐らない仕組み**: `tools/verify-shared-tools-in-sync.ts` を追加（`npm run check` に組込み）。
  土台 4 ファイルが複製元とバイト一致していること、固有化した 6 ファイルが一致して**いない**ことを検査する。
  判断の根拠（共有ライブラリにしなかった理由と実測差分）は [docs/structured-latex-decision.md](docs/structured-latex-decision.md)。
- **企画書 002 の確定命題を移設**: 定義 5 件（スペクトル曲線・周期点数・Massieu Φ・p 素点の有限手続き・π(p,k)）と
  命題 A・B・C・N・L・V・T・W、および ℝ 脱出の隔離を宣言する注意 1 件。
  **主張文の転記のみで、証明本体は未移設**（各 `proof` に report のパスを `todo` で置いた）。
  移していないもの（双対命題 D の記述の食い違い、(∞ 素点) の主張、λ の計算可能性 ほか 6 件）と
  その理由は [docs/paper-001-migration-status.md](docs/paper-001-migration-status.md)。
- **実測**: ブロック 17 件・ラベル 14 件・相互参照 8 件すべて型で解決・SageMath 検証リンク 10 件すべて実在・
  PDF 5 ページ・未解決参照 0・組めない文字 0・版面外へ出た行 0・**ℝ 脱出の宣言は 1 件**（企画書と一致）。
- **レビューで見つけて直した実害**: (1) ℚ̄ が結合マクロンを落として **PDF で ℚ に化けていた**
  （意味が変わる。合成文字を先に処理し、対応表に無い結合文字はビルドを落とす）。
  (2) ℝ 脱出の注意で「有限 L の主張」の列挙に企画書に無い命題 V・W を足していた（企画書どおりへ戻し、
  含めるかは判断待ちとしてノートへ）。(3) 足場が最終成果物の第 1 章に出て、偽の ℝ 脱出宣言を印字していた
  （実ブロックが入ったので削除）。(4) 地の文の Unicode ℝ/ℂ が可算宣言の検査を素通りしていた。
- **Typst**: `_old/typst/` に退避済みで追跡対象に `.typ` は 0 件。退避物も確認し、`main.typ`（17 行）に
  定理環境は 0 件、`parts/` は `.gitkeep` のみ。**移行漏れは原理的に起こらない**ので
  Ising 側の `verify-no-lost-proofs.ts` は移植しない（理由を docs に記録）。
- **Ising 側は 1 ファイルも変更していない**（並行セッションを壊さないため）。あちらの検証と PDF 生成が
  通り続けることを確認済み（278 ブロック・1674 参照すべて解決・PDF 236 ページ）。

**次にやること（担当セッション）**: 証明本体の移設、双対命題 D の記述の食い違いの解消、
`lean/` の新設と `lean` フィールドへの紐づけ（現在 0 件。Lean 定理名の実在検査は Lean プロジェクトが
出来るまで機械検査できない）。
