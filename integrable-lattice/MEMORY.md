...
- 現在の被覆（2026-08-04 実測。cycle 29 step 3 時点の検査 F の実出力）:
  主張 **24 件**のうち **完了 5・部分的 17・未着手 2**、`lean/` の宣言 **479 件**。
  **全数形式化まで残り 19 件。**
...
- **cycle 29 step 1 は配線側から 3 件を実際に形式化したが、残りは 19 件のまま動いていない。**
  ...
  **step 2・step 3 でも残りは 19 件のままである。cycle 28 から 3 サイクル連続で、
  着手はしたが全数への距離は縮まっていない。**
