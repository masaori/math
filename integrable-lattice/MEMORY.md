...
- 現在の被覆（2026-08-04 実測）: 主張 **24 件**のうち **完了 5・部分的 16・未着手 3**。
  **全数形式化まで残り 19 件。**
...
- **未着手 3 件は、いずれも mathlib の欠落が理由である**（命題 LSW と双対命題 D が多変数の
  Mahler 測度、命題 G が matrix-tree 定理）。**こちら側の未着手はもう無い。**
...
- **cycle 29 step 1 は配線側から 3 件を実際に形式化したが、残りは 19 件のまま動いていない。**
  ...
  **cycle 28 に続き 2 サイクル連続で、着手はしたが全数への距離は縮まっていない。**
