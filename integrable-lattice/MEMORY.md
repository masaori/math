...
- **cycle 39 進行中（2026-08-05 起こし）: step 1 で外部定理「Newton 多面体の加法性（Ostrowski の定理）」が完了した。**
  **全数までの残りは 20 → 19 件**（本文 16 ＋ 自分で証明する外部定理 3）。
  **本文側の 16 件はまだ動いていない。4 サイクル続けてである。そう書く。**
  cycle 38 が入れた「書けない理由」の検査が挙げた候補をそのまま着手して書けたので、
  **予告そのものは当たった。ただし当たり方が違った**——予告された素材 `convexHull_add` は現に入ったが、
  それが済ませたのは主張の片側だけで、証明の量の大半は素材と関係のない組合せの議論だった。
  **この形が予告できるのは「着手してよい」ことまでで、「どれだけ書けば済むか」ではない**（検査の限界へ書き足した）。
  実際に効いたのは **$\mathbb{R}$ を使わない道を選んだこと**である——凸包を頂点へ落とす標準の道（Krein–Milman）は
  mathlib では $\mathbb{R}$ の位相を要求するので、使うと主張は可算側にあるのに証明が $\mathbb{R}$ へ出る。
  分離定理を使わず「頂点でない点は中点として書ける」という組合せの事実で代えた。
