...
* **命題 C″ (1) のしきい値 $w^*+1$ の最良性**（$k\le w^*$ では階段が偽であること）は書いていない。
  ...
  **この 2 行は cycle 40 の時点で古くなっていた。cycle 49 step 1 の全数の突き合わせで直す。そう書く**——
  最良性そのものは cycle 40 step 3 が反例つきで書いた
  （`TracePeriodThresholdSharp.ladder_fails_at_two`。本文にもその反例を書き足してある）。
  **いま残っているのはその続きで、全ての $k$ で破れる形と、一般の $T$ についての配線である。**
