...
2. $\mu$（$\chi=\prod_i f_i^{a_i}$ の重複度 $a_i$ を成分ごとにとる元。
...
* **$\chi$ から族 $f_i$ と重複度 $a_i$ を取り出す段は書いていない。** 本ファイルは族を受け取る形で、
  $\rho=\prod_i f_i$ をその族から作っている。本文は $\rho=\mathrm{rad}(\chi)$ と
  $\chi$ から定義しているので、一意分解から族を取り出す段（およびモニックな $\chi$ の因子が
  モニックに取れること）が残る。**掲げた残り 2 件を書いてもこの主張が完了しないのは、これが理由である。
  そう書く。**
-/
import Mathlib
