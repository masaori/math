...
* **$\Gamma=\mathbb{Z}/N\times\mathbb{Z}/N'$（2 変数）の場合**は書いていない。本文の $\mathbb{Z}_\ell^2$ 塔は
  こちらである。巡回の場合を 2 回重ねれば出るはずだが、重ねる段は書いていない。
* **導来グラフの構成と、そのラプラシアンがブロック巡回であること**は書いていない。
  本ファイルが書いたのは「ブロック巡回ならばブロック対角化できる」ところまでで、
  導来グラフのラプラシアンがブロック巡回であることの当てはめは別である。
* **$\widehat L(j)$ が voltage ラプラシアン $L(z,w)$ の $z=\zeta^{j}$ での評価値であること**も、
  上と同じ理由で書いていない（voltage ラプラシアン自体を型として持っていない）。
-/
import Mathlib
