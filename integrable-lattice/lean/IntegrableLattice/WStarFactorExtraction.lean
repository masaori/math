...
* 残っているのは $G$ の同定のほうである。成分ごとに $a_i$ をとる元は $A_\mathbb{Q}$ に住むので
  $A=\mathbb{Z}[x]/(\rho)$ の中では作れないが、**その $\mu$ の構成は cycle 41 step 1 で
  `WStarMuGram.lean` に書いた**（$\rho'(\theta)$ が $A_\mathbb{Q}$ の単元であることから $\eta$ を割る）。
  本文の $G$ は整数行列 $(\operatorname{Tr}T^{i+j})$ なので、代数側の Gram 行列と同じであることを
  言うには $\operatorname{Tr}T^N$ が冪和であることが要る。
