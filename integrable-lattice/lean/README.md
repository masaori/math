...
**2026-08-04 のユーザー判断で、対象に本論文が引用しているだけの外部定理も含めることになった。**
引用で済ませる道は採らない。線引きの正本は
[`docs/external-theorem-criterion.md`](../docs/external-theorem-criterion.md)、
振り分けの台帳は `structured-latex/tools/external-theorem-coverage.ts`
（検査 F が毎回印字する）。cycle 31 step 1 の実測で、引用している外部定理は全 27 件、
うち**自分で証明することにしたのが 7 件**（matrix-tree／Cauchy–Binet／可換環の上の Euler の双対基底公式／
Newton 多面体の加法性／Skolem–Mahler–Lech／Monsky／Cuoco–Monsky）、
mathlib から引くのが 7 件、$\mathbb{R}$ 脱出として隔離するのが 2 件、対象外が 11 件である。

**現在の被覆（2026-08-04 実測）: 本論文の主張 24 件のうち 完了 6・部分的 16・未着手 2。**
**全数まで残り 24 件**（本文の未完了 18 件 ＋ 自分で証明する外部定理のうち未完了 6 件）。
cycle 31 で対象が広がって件数は 18 → 25 へ増え、
**cycle 32 step 1 で Cauchy–Binet が完了して 25 → 24 になった**
（外部定理として初めて完了した 1 件である。**本文側の 18 件は動いていない**）。
なお cycle 31 までの台帳には**完了を書く場所そのものが無く**、
何段書いても件数が動かない造りだった。cycle 32 step 1 で状態（完了／部分的／未着手）を持たせた。
