...
# This file was *autogenerated* from the file lower_order.sage
from sage.all_cmdline import *   # import sage library

_sage_const_78 = Integer(78); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_2 = Integer(2); _sage_const_42 = Integer(42); _sage_const_38 = Integer(38); _sage_const_4 = Integer(4); _sage_const_3 = Integer(3); _sage_const_5 = Integer(5); _sage_const_7 = Integer(7); _sage_const_11 = Integer(11); _sage_const_13 = Integer(13); _sage_const_6 = Integer(6); _sage_const_24 = Integer(24); _sage_const_40 = Integer(40); _sage_const_34 = Integer(34); _sage_const_8 = Integer(8); _sage_const_17 = Integer(17)# cycle 16 / T3 Pure: 低位項の係数（とくに定数項 nu）の明示公式と、退化点の整理。
#
...
cnt1 = bad1 = _sage_const_0 
for (name, m, edges) in EX:
    for ell in PRIMES:
        inv = invariants(m, edges, ell)
        if inv is None or inv['zeros'] or inv['J0'] != _sage_const_1 :
            continue
        ords = usable_prefix(tower_ords(m, edges, ell, NMAX[ell], tag='Step1 ' + name))
        if len(ords) < _sage_const_2 :
            print("  %-42s ell=%2d | 計算できた段が %d 段のみ -> 除外"
                  % (name[:_sage_const_42 ], ell, len(ords)))
            continue
        nmax = len(ords) - _sage_const_1 
        ok = all(ords[n] == predicted_ord(inv, ell, QQ(_sage_const_0 ), n) for n in range(nmax + _sage_const_1 ))
        cnt1 += _sage_const_1 
        if not ok:
            bad1 += _sage_const_1 
        print("  %-42s ell=%2d mu=%d k=%d v(kX)=%d | n=0..%d 全段一致=%s"
              % (name[:_sage_const_42 ], ell, inv['mu'], inv['k'], inv['vkX'], nmax, ok))
        if not ok:
            print("      実測 %s" % ords)
            print("      予言 %s" % [predicted_ord(inv, ell, QQ(_sage_const_0 ), n) for n in range(nmax + _sage_const_1 )])
...
cnt2 = bad2 = _sage_const_0 
for (name, m, edges) in EX:
    for ell in PRIMES:
        inv = invariants(m, edges, ell)
        if inv is None or inv['zeros'] or inv['J0'] < _sage_const_2 :
            continue
        ords = usable_prefix(tower_ords(m, edges, ell, NMAX[ell], tag='Step2 ' + name))
        n0chk = inv['J0'] - _sage_const_1 
        if len(ords) < n0chk + _sage_const_1 :
            print("  %-38s ell=%2d J0=%d | 計算できた段が %d 段のみ（n>=%d が要る）-> 除外"
                  % (name[:_sage_const_38 ], ell, inv['J0'], len(ords), n0chk))
            continue
        nmax = len(ords) - _sage_const_1 
        Delta = delta_correction(inv['D'], ell, inv['mu'], inv['k'], inv['J0'])
        if Delta is None:
            continue
        n0 = inv['J0'] - _sage_const_1 
        ok = all(ords[n] == predicted_ord(inv, ell, Delta, n) for n in range(n0, nmax + _sage_const_1 ))
        # n < n0 でも成立するか（成立開始段の実測）
        nstart = None
        for n in range(nmax + _sage_const_1 ):
            if all(ords[t] == predicted_ord(inv, ell, Delta, t) for t in range(n, nmax + _sage_const_1 )):
                nstart = n; break
        cnt2 += _sage_const_1 
        if not ok:
            bad2 += _sage_const_1 
        print("  %-38s ell=%2d mu=%d k=%d J0=%d Delta=%s | n>=%d 一致=%s (実測の成立開始段 n=%s)"
              % (name[:_sage_const_38 ], ell, inv['mu'], inv['k'], inv['J0'], Delta, n0, ok, nstart))
        if not ok:
            print("      実測 %s" % ords)
            print("      予言 %s" % [predicted_ord(inv, ell, Delta, n) for n in range(nmax + _sage_const_1 )])
...
nz = [t for t in seen_delta if t[_sage_const_4 ] != _sage_const_0 ]
for (name, ell, J0, k, Dl) in seen_delta:
    print("     %-38s ell=%2d J0=%d k=%d Delta=%s" % (name[:_sage_const_38 ], ell, J0, k, Dl))
...
def parallel_bundle(ell):
    ...
    volts = [(_sage_const_0 , _sage_const_0 ), (_sage_const_1 , _sage_const_0 ), (_sage_const_0 , _sage_const_1 ), (_sage_const_1 , _sage_const_1 ), (_sage_const_2 , _sage_const_1 ), (_sage_const_1 , _sage_const_2 ), (_sage_const_2 , _sage_const_3 ), (_sage_const_3 , _sage_const_1 ),
             (_sage_const_1 , _sage_const_3 ), (_sage_const_2 , _sage_const_2 ), (_sage_const_0 , _sage_const_2 ), (_sage_const_3 , _sage_const_2 ), (_sage_const_2 , _sage_const_0 )]
    return (_sage_const_2 , [(_sage_const_0 , _sage_const_1 , volts[i % len(volts)]) for i in range(ell)])


CAND3 = []
for ell in [_sage_const_5 , _sage_const_7 , _sage_const_11 , _sage_const_13 ]:
    # (i) mu > 0
    for (name, m, edges) in EX[:_sage_const_6 ]:
        (m2, e2) = repeat_edges(m, edges, ell)
        CAND3.append(('%s を %d 重化 (mu>0 狙い)' % (name[:_sage_const_24 ], ell), m2, e2, ell))
    # (ii) v_ell(kappa(X)) > 0
    (mb, eb) = parallel_bundle(ell)
    CAND3.append(('2 頂点 %d 重平行辺 (v(kX)>0 狙い)' % ell, mb, eb, ell))

found = []
rej = {'inv=None((H)不成立等)': _sage_const_0 , '退化(z_H>0)': _sage_const_0 , 'mu=0 かつ v(kX)=0': _sage_const_0 , '段数不足': _sage_const_0 }
for (name, m, edges, ell) in CAND3:
    inv = invariants(m, edges, ell)
    if inv is None:
        rej['inv=None((H)不成立等)'] += _sage_const_1 ; continue
    if inv['zeros']:
        rej['退化(z_H>0)'] += _sage_const_1 ; continue
    if inv['mu'] == _sage_const_0  and inv['vkX'] == _sage_const_0 :
        rej['mu=0 かつ v(kX)=0'] += _sage_const_1 ; continue
    ords = usable_prefix(tower_ords(m, edges, ell, NMAX[ell], tag='Step3 ' + name))
    if len(ords) < _sage_const_2 :
        rej['段数不足'] += _sage_const_1 ; continue
...
nmu = len([_sage_const_1  for f in found if f[_sage_const_4 ]['mu'] > _sage_const_0 ])
nkx = len([_sage_const_1  for f in found if f[_sage_const_4 ]['vkX'] > _sage_const_0 ])
print("  内訳: mu>0 が %d 件、v_ell(kappa(X))>0 が %d 件" % (nmu, nkx))
print()

cnt3 = bad3 = _sage_const_0 
for (name, m, edges, ell, inv, ords) in found:
    Delta = QQ(_sage_const_0 ) if inv['J0'] == _sage_const_1  else delta_correction(inv['D'], ell, inv['mu'], inv['k'], inv['J0'])
    if Delta is None:
        continue
    n0 = inv['J0'] - _sage_const_1 
    nmax = len(ords) - _sage_const_1 
    ...
    print("  %-40s ell=%2d mu=%d k=%d v(kX)=%d(kX=%d) J0=%d Delta=%s | n=%d..%d 一致=%s"
          % (name[:_sage_const_40 ], ell, inv['mu'], inv['k'], inv['vkX'], inv['kX'], inv['J0'], Delta,
             n0, nmax, ok))
    if not ok:
        print("      実測 %s" % ords)
        print("      予言 %s" % [predicted_ord(inv, ell, Delta, n) for n in range(nmax + _sage_const_1 )])
print("  %s 対象 %d 件、破れ %d 件" % (el(), cnt3, bad3))
if cnt3 == _sage_const_0 :
...
def direction_of(ell, n, a, b):
    ...
    N = ell**n
    va = n if a % N == _sage_const_0  else ZZ(a % N).valuation(ell)
    vb = n if b % N == _sage_const_0  else ZZ(b % N).valuation(ell)
...
for ell in [_sage_const_2 , _sage_const_3 , _sage_const_5 ]:
    for n in range(_sage_const_1 , {_sage_const_2 : _sage_const_6 , _sage_const_3 : _sage_const_4 , _sage_const_5 : _sage_const_3 }[ell] + _sage_const_1 ):
        N = ell**n
        cntd = {}
        for a in range(N):
            for b in range(N):
                ...
                cntd[d] = cntd.get(d, _sage_const_0 ) + _sage_const_1 
        exp = ZZ(ell**(_sage_const_2  * n) - _sage_const_1 ) / (ell + _sage_const_1 )
        ok = (len(cntd) == ell + _sage_const_1 ) and all(v == exp for v in cntd.values())
        if not ok:
            bad4a += _sage_const_1 
...
PTMAX = {_sage_const_2 : _sage_const_4 , _sage_const_3 : _sage_const_2 , _sage_const_5 : _sage_const_2 }
cnt4 = bad4 = _sage_const_0 
for (name, m, edges) in EX:
    for ell in [_sage_const_2 , _sage_const_3 , _sage_const_5 ]:
        inv = invariants(m, edges, ell)
        if inv is None:
        ...
        zeros = set(inv['zeros'])
        for n in range(J0, nmaxpt + _sage_const_1 ):
            N = ell**n
            mism = _sage_const_0 ; nband = _sage_const_0 ; nexc = _sage_const_0 ; tot = _sage_const_0 
            for a in range(N):
                for b in range(N):
                    if a == _sage_const_0  and b == _sage_const_0 :
                        continue
            ...
            print("     %-34s ell=%d n=%d k=%d J0=%d z_H=%d | 対象点 %d, Band %d, 例外 %d, 不一致 %d"
                  % (name[:_sage_const_34 ], ell, n, k, J0, len(zeros), tot, nband, nexc, mism))
...
bad4c = _sage_const_0 
for (name, m, edges) in EX[:_sage_const_8 ]:
    for ell in [_sage_const_2 , _sage_const_3 ]:
        inv = invariants(m, edges, ell)
        if inv is None:
            continue
        zeros = set(inv['zeros']); zH = len(zeros)
        for n in range(_sage_const_1 , {_sage_const_2 : _sage_const_6 , _sage_const_3 : _sage_const_4 }[ell] + _sage_const_1 ):
            N = ell**n
            cb = _sage_const_0 
            for a in range(N):
                for b in range(N):
                    ...
                    if d in zeros:
                        cb += _sage_const_1 
            exp = zH * ZZ(ell**(_sage_const_2  * n) - _sage_const_1 ) / (ell + _sage_const_1 )
            if cb != exp:
                bad4c += _sage_const_1 
...
def ph(e, M):
    ...
    if e == M - _sage_const_1 :
        return oo
    return QQ(_sage_const_2 )**(_sage_const_1  + e - M)

print()
print("  (5a) 点ごとの付値: 位数がともに 2^M（M >= 2）の (zeta,xi) = (g^a, g^b) について")
print("       v_2(D) = min(2, ph(s) + ph(t)),  s = v_2(b-a), t = v_2(b+a)")
bad5a = _sage_const_0 ; cnt5a = _sage_const_0 
for M in range(_sage_const_1 , _sage_const_7 ):
    (K, g, P, e) = cyc_data(_sage_const_2 , M)
    bad = _sage_const_0 ; tot = _sage_const_0 
    for a in range(_sage_const_1 , _sage_const_2 **M, _sage_const_2 ):
        for b in range(_sage_const_1 , _sage_const_2 **M, _sage_const_2 ):
            v = point_val(Dtor, _sage_const_2 , M, a, b)
            s = M if (b - a) % _sage_const_2 **M == _sage_const_0  else ZZ((b - a) % _sage_const_2 **M).valuation(_sage_const_2 )
            t = M if (b + a) % _sage_const_2 **M == _sage_const_0  else ZZ((b + a) % _sage_const_2 **M).valuation(_sage_const_2 )
            ps = ph(s, M); pt = ph(t, M)
            pred = QQ(_sage_const_2 ) if (ps is oo or pt is oo) else min(QQ(_sage_const_2 ), ps + pt)
            tot += _sage_const_1 
            if v != pred:
                bad += _sage_const_1 
    ...
    print("     M=%d: 点 %d 個、公式から外れた点 %d 個%s"
          % (M, tot, bad, "（M=1 は例外。本文 5.3 参照）" if M == _sage_const_1  else ""))
...
for M in range(_sage_const_1 , _sage_const_8 ):
    ...
    for a in range(_sage_const_1 , _sage_const_2 **M, _sage_const_2 ):
        for b in range(_sage_const_1 , _sage_const_2 **M, _sage_const_2 ):
            ...
            t = M if (b + a) % _sage_const_2 **M == _sage_const_0  else ZZ((b + a) % _sage_const_2 **M).valuation(_sage_const_2 )
            ps = ph(s, M); pt = ph(t, M)
            tot += QQ(_sage_const_2 ) if (ps is oo or pt is oo) else min(QQ(_sage_const_2 ), ps + pt)
    if M == _sage_const_1 :
        tot = QQ(_sage_const_3 )     # M = 1 は (a,b) = (1,1) の 1 点のみで v = 3（(5a) 参照）
    pred = (M + _sage_const_1 ) * _sage_const_2 **M - _sage_const_4 
    ok = (tot == pred)
    if M >= _sage_const_2  and not ok:
        bad5b += _sage_const_1 
...
ords_tor2 = usable_prefix(tower_ords(TOR[_sage_const_0 ], TOR[_sage_const_1 ], _sage_const_2 , NMAX[_sage_const_2 ], tag='Step5c torus ell=2'))
bad5c = _sage_const_0 
for n in range(_sage_const_0 , len(ords_tor2)):
    pred = _sage_const_2  * n * _sage_const_2 **n + _sage_const_4  * _sage_const_2 **n - _sage_const_6  * n - _sage_const_1 
    ok = (ords_tor2[n] == pred)
    if n >= _sage_const_1  and not ok:
        bad5c += _sage_const_1 
...
for (name, m, edges) in EX:
    for ell in [_sage_const_2 , _sage_const_3 ]:
        ...
        if len(ords) < _sage_const_6 :
            print("  %-38s ell=%d | 計算できた段 %d (フィットに 6 段必要) -> 除外"
                  % (name[:_sage_const_38 ], ell, len(ords)))
            continue
        nmax = len(ords) - _sage_const_1 
        ...
        def fitval(n):
            return sum(co[t] * v for t, v in
                       enumerate([ell**(_sage_const_2  * n), n * ell**n, ell**n, n, _sage_const_1 ]))
        # n = 0 は kappa_0 = kappa(X) で、定理 D2 など閉形式の射程外（n >= 1）である。
        # よって out-of-sample は n >= 1 のものを本判定とし、n = 0 は参考として別に出す。
        oos_all = [n for n in range(nmax - _sage_const_4 )]
        oos = [n for n in oos_all if n >= _sage_const_1 ]
        ...
        if not integral or okoos is False:
            ...
            typ = 'I'
        elif co[_sage_const_1 ] > _sage_const_0 :
            typ = 'III'
        elif co[_sage_const_1 ] == _sage_const_0 :
            typ = 'II'
        else:
        ...
        print("  %-38s ell=%d mu=%d k=%d z_H=%d %-22s" % (name[:_sage_const_38 ], ell, inv['mu'], inv['k'], zH,
                                                          ','.join(inv['zeros'])))
        print("      a=%s b=%s c=%s d=%s e=%s | 係数が整数=%s | oos(n>=1, n=%s)=%s | oos に n=0 も含めると=%s"
              % (co[_sage_const_0 ], co[_sage_const_1 ], co[_sage_const_2 ], co[_sage_const_3 ], co[_sage_const_4 ], integral, oos, okoos, okoos0))
        print("      -> %s" % typ)
ngood = len([r for r in rows6 if r[_sage_const_8 ] in ('I', 'II', 'III')])
nII = len([r for r in rows6 if r[_sage_const_8 ] == 'II'])
nIII = len([r for r in rows6 if r[_sage_const_8 ] == 'III'])
...
print()
bad7 = _sage_const_0 ; cnt7 = _sage_const_0 
for ell in [_sage_const_5 , _sage_const_13 , _sage_const_17 ]:
    inv = invariants(TOR[_sage_const_0 ], TOR[_sage_const_1 ], ell)
    ords = usable_prefix(tower_ords(TOR[_sage_const_0 ], TOR[_sage_const_1 ], ell, NMAX.get(ell, _sage_const_1 ),
                                    tag='Step7 torus ell=%d' % ell))
    print("  ell=%2d: k=%d mu=%d z_H=%d %s, 退化帯の割合 = %s/%d, 計算できた段 n=0..%d"
          % (ell, inv['k'], inv['mu'], len(inv['zeros']), inv['zeros'],
             len(inv['zeros']), ell + _sage_const_1 , len(ords) - _sage_const_1 ))
    for n in range(len(ords)):
        nd = predicted_ord(inv, ell, QQ(_sage_const_0 ), n)
        h61 = QQ(_sage_const_2  * ell + _sage_const_6 ) / (ell - _sage_const_1 ) * (ell**n - _sage_const_1 ) - _sage_const_2  * n
        ok61 = (ords[n] == h61)
        if n >= _sage_const_1 :
            cnt7 += _sage_const_1 
            if not ok61:
...
if not SKIPS:
    ...
    for (tag, m, edges, ell, n, budget) in SKIPS:
        print("  %-34s ell=%2d n=%d で打ち切り（m=%d edges=%s）" % (tag[:_sage_const_34 ], ell, n, m, list(edges)))
