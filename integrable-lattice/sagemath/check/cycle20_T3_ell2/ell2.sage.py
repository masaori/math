...
# This file was *autogenerated* from the file ell2.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0 = Integer(0); _sage_const_7500p0 = RealNumber('7500.0'); _sage_const_1 = Integer(1); _sage_const_60 = Integer(60); _sage_const_78 = Integer(78); _sage_const_13 = Integer(13); _sage_const_3 = Integer(3); _sage_const_5 = Integer(5); _sage_const_7 = Integer(7); _sage_const_9 = Integer(9); _sage_const_15 = Integer(15); _sage_const_2 = Integer(2); _sage_const_4 = Integer(4); _sage_const_8 = Integer(8); _sage_const_6 = Integer(6); _sage_const_12 = Integer(12); _sage_const_16 = Integer(16); _sage_const_10 = Integer(10); _sage_const_11 = Integer(11); _sage_const_31 = Integer(31); _sage_const_32 = Integer(32); _sage_const_24 = Integer(24); _sage_const_300 = Integer(300); _sage_const_20 = Integer(20); _sage_const_120 = Integer(120)# cycle 20 / T3 Pure: ell = 2 の決着（ell_equals_2）
#
...
PQ_FULL = [(p, q) for p in range(_sage_const_1 , _sage_const_13 ) for q in range(_sage_const_1 , _sage_const_13 )]
PQ_POINT = [(_sage_const_1 , _sage_const_1 ), (_sage_const_1 , _sage_const_3 ), (_sage_const_3 , _sage_const_5 ), (_sage_const_1 , _sage_const_7 ), (_sage_const_5 , _sage_const_3 ), (_sage_const_1 , _sage_const_5 ), (_sage_const_3 , _sage_const_1 ), (_sage_const_5 , _sage_const_7 ), (_sage_const_9 , _sage_const_7 ), (_sage_const_1 , _sage_const_15 ),
            (_sage_const_1 , _sage_const_2 ), (_sage_const_2 , _sage_const_1 ), (_sage_const_1 , _sage_const_4 ), (_sage_const_3 , _sage_const_4 ), (_sage_const_2 , _sage_const_3 ), (_sage_const_4 , _sage_const_3 ), (_sage_const_8 , _sage_const_3 ), (_sage_const_1 , _sage_const_6 ), (_sage_const_6 , _sage_const_1 ), (_sage_const_2 , _sage_const_5 ),
            (_sage_const_4 , _sage_const_5 ), (_sage_const_12 , _sage_const_5 ), (_sage_const_16 , _sage_const_3 ), (_sage_const_10 , _sage_const_3 ), (_sage_const_2 , _sage_const_2 ), (_sage_const_2 , _sage_const_6 ), (_sage_const_4 , _sage_const_4 ), (_sage_const_6 , _sage_const_10 ), (_sage_const_2 , _sage_const_2  * _sage_const_7 ),
            (_sage_const_1 , _sage_const_9 ), (_sage_const_9 , _sage_const_1 ), (_sage_const_7 , _sage_const_9 ), (_sage_const_11 , _sage_const_5 ), (_sage_const_3 , _sage_const_13 ), (_sage_const_1 , _sage_const_31 ), (_sage_const_32 , _sage_const_3 ), (_sage_const_3 , _sage_const_32 ), (_sage_const_24 , _sage_const_5 )]

NMAX_A = _sage_const_4           # Step A の全点網羅の段数
NMAX_B2 = _sage_const_4          # Step B2（塔の値）の段数
NMAX_B3 = _sage_const_8          # Step B3（円分体 Theta_m 経路）の段数
...
for (p, q) in PQ_POINT:
    (m, edges) = fam(p, q)
    D = detL(m, edges)
    mu = mu_content(D, _sage_const_2 )
    Ev = E_of(D, _sage_const_2 , mu)
    for n in range(_sage_const_1 , NMAX_A + _sage_const_1 ):
        for a in range(_sage_const_2 **n):
            for b in range(_sage_const_2 **n):
                ...
                act = point_val(Ev, _sage_const_2 , n, a, b)
                BRANCH[br] = BRANCH.get(br, _sage_const_0 ) + _sage_const_1 
                npts += _sage_const_1 
...
for (p, q) in PQ_POINT:
    ...
    for n in range(_sage_const_1 , _sage_const_8 ):
        S = QQ(_sage_const_0 )
        for a in range(_sage_const_2 **n):
            for b in range(_sage_const_2 **n):
                if a == _sage_const_0  and b == _sage_const_0 :
        ...
        got = mu * (_sage_const_4 **n - _sage_const_1 ) - _sage_const_2  * n + S
...
nb2 = _sage_const_0 
b2_cut = _sage_const_0 
for (p, q) in PQ_POINT:
    (m, edges) = fam(p, q)
    ords = tower_ords(m, edges, _sage_const_2 , NMAX_B2, tag='fam(%d,%d)' % (p, q), budget=_sage_const_300 )
    got = usable_prefix(ords)
    b2_cut += (NMAX_B2 + _sage_const_1  - len(got))
    for n in range(_sage_const_1 , len(got)):
...
nb3 = _sage_const_0 
b3_cut = _sage_const_0 
for (p, q) in PQ_POINT[:_sage_const_20 ]:
    (m, edges) = fam(p, q)
    got = ords_via_sigma(m, edges, _sage_const_2 , NMAX_B3, budget=_sage_const_60 )
    if got is None:
        b3_cut += _sage_const_1 
        continue
    b3_cut += (NMAX_B3 + _sage_const_1  - len(got))
    for n in range(_sage_const_1 , len(got)):
...
for (p, q) in [(_sage_const_1 , _sage_const_1 ), (_sage_const_1 , _sage_const_3 ), (_sage_const_1 , _sage_const_2 ), (_sage_const_1 , _sage_const_4 ), (_sage_const_2 , _sage_const_2 )]:
    (mu, pp, qq, case, par) = fam_invariants(p, q)
    print('    (p,q)=(%2d,%2d) mu=%d case=%s par=%s : %s'
          % (p, q, mu, case, par, [Y_closed(p, q, n) for n in range(_sage_const_0 , _sage_const_7 )]))
...
for (p, q) in PQ_FULL:
    ...
    mu = mu_content(D, _sage_const_2 )
    Ev = E_of(D, _sage_const_2 , mu)
    lines = exceptional_lines(Ev, _sage_const_2 )
    (H, zh) = (None, None)
    Hh = lowest_form(D, _sage_const_2 )
    report('C1 (p,q)=(%d,%d)' % (p, q), len(lines) >= _sage_const_1 ,
...
for (p, q) in PQ_FULL:
    ...
    if case == 'A':
        (lam0, lamm) = par
        want = sorted([((_sage_const_1 , _sage_const_1 ), lam0), ((_sage_const_1 , -_sage_const_1 ), lam0)])
        want = sorted((u if u[_sage_const_0 ] > _sage_const_0  or (u[_sage_const_0 ] == _sage_const_0  and u[_sage_const_1 ] > _sage_const_0 ) else (-u[_sage_const_0 ], -u[_sage_const_1 ]), l)
                      for (u, l) in want)
    else:
        (lam1, w, swapped) = par
        want = [((_sage_const_0 , _sage_const_1 ), lam1)] if swapped else [((_sage_const_1 , _sage_const_0 ), lam1)]
...
for (p, q) in PQ_FULL:
    (mu, pp, qq, case, par) = fam_invariants(p, q)
    ok_all = all(X_prime_closed(p, q, _sage_const_2 , n) == Y_closed(p, q, n) for n in range(_sage_const_1 , _sage_const_7 ))
    if case == 'A':
        key = 'A lam0=%s' % ('1' if par[_sage_const_0 ] == _sage_const_1  else '>=2')
    else:
        key = 'B lam1=%s' % ('1' if par[_sage_const_0 ] == _sage_const_1  else '>=2')
    agree_cases.setdefault(key, [_sage_const_0 , _sage_const_0 ])
    agree_cases[key][_sage_const_0  if ok_all else _sage_const_1 ] += _sage_const_1 
    want = (case == 'B' and par[_sage_const_0 ] >= _sage_const_2 )
    report('C3 (p,q)=(%d,%d)' % (p, q), ok_all == want,
           'X\'=%s Y\'=%s case=%s par=%s'
           % ([X_prime_closed(p, q, _sage_const_2 , n) for n in range(_sage_const_1 , _sage_const_4 )],
              [Y_closed(p, q, n) for n in range(_sage_const_1 , _sage_const_4 )], case, par))
    nc3 += _sage_const_1 
print('  %s C3 照合 %d 組。場合ごとの内訳（定理 X\' と一致 / 不一致）:' % (el(), nc3))
for k in sorted(agree_cases):
    print('    %-12s 一致 %3d 組 / 不一致 %3d 組' % (k, agree_cases[k][_sage_const_0 ], agree_cases[k][_sage_const_1 ]))
...
nd1 = _sage_const_0 
for m in range(_sage_const_2 , _sage_const_10 ):
    bad = _sage_const_0 
    minne1 = _sage_const_0 
    for ap in range(_sage_const_1 , _sage_const_2 **m, _sage_const_2 ):
        for bp in range(_sage_const_1 , _sage_const_2 **m, _sage_const_2 ):
            s = nu2(ap - bp, _sage_const_2 **m)
            t = nu2(ap + bp, _sage_const_2 **m)
            if s is not Infinity and s < _sage_const_1 :
                bad += _sage_const_1 
            if t is not Infinity and t < _sage_const_1 :
                bad += _sage_const_1 
            if min(s, t) != _sage_const_1 :
                minne1 += _sage_const_1 
    report('D1 m=%d' % m, bad == _sage_const_0  and minne1 == _sage_const_0 ,
...
for m in range(_sage_const_1 , _sage_const_8 ):
    both = sum(_sage_const_1  for ap in range(_sage_const_1 , _sage_const_2 **m, _sage_const_2 ) for bp in range(_sage_const_1 , _sage_const_2 **m, _sage_const_2 )
               if (ap - bp) % _sage_const_2 **m == _sage_const_0  and (ap + bp) % _sage_const_2 **m == _sage_const_0 )
    report('D2 m=%d' % m, (both == _sage_const_0 ) == (m >= _sage_const_2 ),
    ...
    for n in range(_sage_const_1 , _sage_const_6 ):
        for a in range(_sage_const_2 **n):
            for b in range(_sage_const_2 **n):
                ...
                phin = _sage_const_2 **(n - _sage_const_1 )
                na = nu2(A, _sage_const_2 **n)
                nb = nu2(B, _sage_const_2 **n)
                Ua = Infinity if na is Infinity else lam1 * phin + _sage_const_2 **(na + _sage_const_1 )
                Ub = Infinity if nb is Infinity else _sage_const_2 **(nb + _sage_const_1 )
                if Ua is Infinity or Ub is Infinity or Ua != Ub:
                    continue
                tie_shapes[(lam1, na - n, nb - n)] = tie_shapes.get((lam1, na - n, nb - n), _sage_const_0 ) + _sage_const_1 
                act = point_val(Ev, _sage_const_2 , n, a, b)
                naive = QQ(Ua) / phin
                report('D3 (p,q)=(%d,%d) n=%d (a,b)=(%d,%d)' % (p, q, n, a, b),
                       act == QQ(_sage_const_2  + w) and act > naive,
                       'actual=%s want=%s naive(min)=%s' % (act, _sage_const_2  + w, naive))
                nd3 += _sage_const_1 
print('  %s D3 打ち消し点 %d 件を照合。起きた (lam1, v_2(A)-n, v_2(B)-n) の形:' % (el(), nd3))
for k in sorted(tie_shapes):
    print('    lam1=%d, v_2(A)=n%+d, v_2(B)=n%+d : %d 件' % (k[_sage_const_0 ], k[_sage_const_1 ], k[_sage_const_2 ], tie_shapes[k]))
...
for (P1, P2) in [((_sage_const_1 , _sage_const_2 ), (_sage_const_1 , _sage_const_6 )), ((_sage_const_3 , _sage_const_2 ), (_sage_const_1 , _sage_const_2 )), ((_sage_const_5 , _sage_const_2 ), (_sage_const_1 , _sage_const_2 )), ((_sage_const_2 , _sage_const_3 ), (_sage_const_2 , _sage_const_1 ))]:
    (mu1, _, _, c1, pr1) = fam_invariants(*P1)
    (mu2, _, _, c2, pr2) = fam_invariants(*P2)
    print('    (p,q)=%s: case=%s par=%s  ord=%s' % (P1, c1, pr1, [Y_closed(P1[_sage_const_0 ], P1[_sage_const_1 ], n) for n in range(_sage_const_1 , _sage_const_6 )]))
    print('    (p,q)=%s: case=%s par=%s  ord=%s' % (P2, c2, pr2, [Y_closed(P2[_sage_const_0 ], P2[_sage_const_1 ], n) for n in range(_sage_const_1 , _sage_const_6 )]))
...
""")
for (p, q) in [(_sage_const_1 , _sage_const_1 ), (_sage_const_1 , _sage_const_3 ), (_sage_const_3 , _sage_const_5 )]:
    (mu, pp, qq, case, par) = fam_invariants(p, q)
    print('  (p,q)=(%d,%d) case=%s lam0=%s' % (p, q, case, par[_sage_const_0 ]))
    for m in range(_sage_const_2 , _sage_const_8 ):
        sub = {}
        for ap in range(_sage_const_1 , _sage_const_2 **m, _sage_const_2 ):
            for bp in range(_sage_const_1 , _sage_const_2 **m, _sage_const_2 ):
                ...
                sub[key] = sub.get(key, QQ(_sage_const_0 )) + v
        tot = sum(sub.values())
        want = ((m + _sage_const_1 ) * _sage_const_2 **m - _sage_const_4 ) if par[_sage_const_0 ] == _sage_const_1  else _sage_const_2 **m * (m - _sage_const_1  + par[_sage_const_0 ])
...
for (p, q) in PQ_POINT:
    ...
    (bstar, det) = jstar_sum(Ev, _sage_const_2 )
    # (B*): 定理 B' の最小点が一意か（レベル <= 4）
    bstar_ok = True
    for Mp in range(_sage_const_1 , _sage_const_5 ):
        (th, ok) = Theta_level(coeffs, _sage_const_2 , Mp)
        if not ok:
            bstar_ok = False
    key = (case, 'lam=%s' % par[_sage_const_0 ], 'B*ok' if bstar_ok else 'B*broken')
    f_stat[key] = f_stat.get(key, _sage_const_0 ) + _sage_const_1 
    report('F b=sum j* (p,q)=(%d,%d)' % (p, q), bstar == _sage_const_2 ,
...
g_ok = g_shape_miss = g_short = g_cut = g_degen = g_hfail = _sage_const_0 
g_short_names = []
g_n0tab = {}
g_shape_names = []
g_bmatch = g_bmiss = _sage_const_0 
g_btab = {}
g_dtab = {}
g_cut_names = []
g_bmiss_ex = []
...
for (name, m, edges) in POP:
    ...
    if mu is Infinity:
        ...
        continue
    Ev = E_of(D, _sage_const_2 , mu)
    ords = ords_via_sigma(m, edges, _sage_const_2 , NMAX_G, budget=_sage_const_120 )
    if ords is None:
        g_hfail += _sage_const_1           # 仮定 (H) が破れる（ある段で E が消え v = oo）
        continue
    if len(ords) < NMAX_G + _sage_const_1 :
        g_short += _sage_const_1           # 45 秒の段あたり上限で n = %d まで届かなかった
        g_short_names.append((name, len(ords) - _sage_const_1 ))
        continue
    sol = fit_shape(ords, mu, [_sage_const_6 , _sage_const_7 , _sage_const_8 , _sage_const_9 ])
    if sol is None:
        g_shape_miss += _sage_const_1 
        g_shape_names.append((name, ords, None))
        continue
    (b, c, d, e) = sol
    def hits(n):
        return ords[n] == mu * (_sage_const_4 **n - _sage_const_1 ) + b * n * _sage_const_2 **n + c * _sage_const_2 **n + d * n + e
    oos = all(hits(n) for n in [_sage_const_4 , _sage_const_5 ])
    n0 = _sage_const_6 
    while n0 >= _sage_const_1  and hits(n0 - _sage_const_1 ):
        n0 -= _sage_const_1 
    g_n0tab[n0] = g_n0tab.get(n0, _sage_const_0 ) + _sage_const_1 
    if not oos:
        g_shape_miss += _sage_const_1 
        g_shape_names.append((name, ords, sol))
        continue
    g_ok += _sage_const_1 
    g_btab[b] = g_btab.get(b, _sage_const_0 ) + _sage_const_1 
    g_dtab[d] = g_dtab.get(d, _sage_const_0 ) + _sage_const_1 
    (bstar, det) = jstar_sum(Ev, _sage_const_2 )
    if bstar == b:
        g_bmatch += _sage_const_1 
    else:
        g_bmiss += _sage_const_1 
        if len(g_bmiss_ex) < _sage_const_8 :
            g_bmiss_ex.append((name, ords, b, bstar, det, sinf_brute(Ev, _sage_const_2 , _sage_const_6 )))
...
if g_short_names:
    print('    時間で届かなかった塔（到達段数）: %s%s'
          % (', '.join('%s(n<=%d)' % t for t in g_short_names[:_sage_const_10 ]),
             ' ...' if len(g_short_names) > _sage_const_10  else ''))
if g_cut_names:
    print('    打ち切った塔（%d 個）: %s%s'
          % (len(g_cut_names), ', '.join(g_cut_names[:_sage_const_12 ]),
             ' ...' if len(g_cut_names) > _sage_const_12  else ''))
...
for (p, q) in [(_sage_const_1 , _sage_const_1 ), (_sage_const_1 , _sage_const_3 ), (_sage_const_1 , _sage_const_2 ), (_sage_const_1 , _sage_const_6 ), (_sage_const_8 , _sage_const_3 )]:
    (mu, pp, qq, case, par) = fam_invariants(p, q)
...
for (p, q) in PQ_POINT[:_sage_const_20 ]:
    ...
    D = detL(m, edges)
    Ev = E_of(D, _sage_const_2 , mu_content(D, _sage_const_2 ))
    cand = sorted(tuple(u) for (u, lam, th) in exceptional_lines(Ev, _sage_const_2 ))
    brute = sinf_brute(Ev, _sage_const_2 , _sage_const_8 )

    def norm(u):
        (a, b) = (ZZ(u[_sage_const_0 ]), ZZ(u[_sage_const_1 ]))
        return (a, b) if (a > _sage_const_0  or (a == _sage_const_0  and b > _sage_const_0 )) else (-a, -b)
...
for (p, q) in [(_sage_const_1 , _sage_const_1 ), (_sage_const_1 , _sage_const_3 ), (_sage_const_1 , _sage_const_2 ), (_sage_const_1 , _sage_const_4 )]:
    (mu, pp, qq, case, par) = fam_invariants(p, q)
    print('        (p,q)=(%d,%d) case=%s par=%s : %s'
          % (p, q, case, par, [Y_closed(p, q, n) for n in range(_sage_const_1 , _sage_const_7 )]))
...
for (p, q) in [(_sage_const_1 , _sage_const_2 ), (_sage_const_1 , _sage_const_6 ), (_sage_const_3 , _sage_const_2 )]:
    (mu, pp, qq, case, par) = fam_invariants(p, q)
    (lam1, w, swapped) = par
    nm = _sage_const_2  * _sage_const_1  * _sage_const_2  + _sage_const_2  * _sage_const_1  - _sage_const_1  + _sage_const_2  * w
    print('      (p,q)=(%d,%d) w=%d: n>=2 の式を n=1 に当てると %d、真の値は %d'
          % (p, q, w, nm, Y_closed(p, q, _sage_const_1 )))
