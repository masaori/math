...
# This file was *autogenerated* from the file carryover.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_78 = Integer(78); _sage_const_2 = Integer(2); _sage_const_3 = Integer(3); _sage_const_9 = Integer(9); _sage_const_700p0 = RealNumber('700.0'); _sage_const_4 = Integer(4); _sage_const_5 = Integer(5); _sage_const_8 = Integer(8); _sage_const_6 = Integer(6); _sage_const_19 = Integer(19); _sage_const_61 = Integer(61); _sage_const_167 = Integer(167)# cycle 26 / T3 Pure: 3 サイクル持ち越しの未検証 2 件を閉じる。
#
...
def hdr(s):
    print(''); print('=' * _sage_const_78 ); print(s, el26()); print('=' * _sage_const_78 ); sys.stdout.flush()
...
BUDGET_D = _sage_const_700p0 
BUDGET_K = _sage_const_700p0 
...
zero = [r for r in DATA if r['co'][_sage_const_5 ] == _sage_const_0 ]
nonzero = [r for r in DATA if r['co'][_sage_const_5 ] != _sage_const_0 ]
print('Tdef = 0 の塔 %d / Tdef != 0 の塔 %d' % (len(zero), len(nonzero)))

# Tdef = 0 の塔を「全 delta_M が 0」と「0 でない delta_M があるのに総和が 0」に分ける。
allzero = [r for r in zero if all(x == _sage_const_0  for x in r['ds'])]
cancel = [r for r in zero if any(x != _sage_const_0  for x in r['ds'])]
...
mixed = []
for r in DATA:
    sg = set(sign(x) for x in r['ds'] if x != _sage_const_0 )
    if len(sg) > _sage_const_1 :
        mixed.append(r)
print('')
print('delta_M の符号が混ざる塔（正と負の両方が出る）: %d 件' % len(mixed))
for r in mixed[:_sage_const_8 ]:
    print('  ell=%d %-30s M*=%s delta=%s Tdef=%s'
          % (r['ell'], r['nm'], r['Ms'], [str(x) for x in r['ds']], r['co'][_sage_const_5 ]))
mixed_zero = [r for r in mixed if r['co'][_sage_const_5 ] == _sage_const_0 ]
print('  そのうち Tdef = 0 のもの（＝証明中の反例が実在の塔で実現する）: %d 件' % len(mixed_zero))
for r in mixed_zero[:_sage_const_8 ]:
...
nchk = _sage_const_0 ; nmis = _sage_const_0 ; nskip = _sage_const_0 
n0_hits = _sage_const_0         # 予言も実測も「一致する」
n0_miss = _sage_const_0         # 予言も実測も「一致しない」
...
for r in DATA:
    ...
    (a, b, c, d, e, tdef) = r['co']
    ds = r['ds']
    for n in range(_sage_const_0 , NMAX[ell] + _sage_const_1 ):
        ...
        if says_equal != is_equal:
            ...
            n0_hits += _sage_const_1 
        else:
            n0_miss += _sage_const_1 
...
c_ok = _sage_const_0 ; c_ng = _sage_const_0 ; c_zero_ok = _sage_const_0 
for r in DATA:
    ell = r['ell']
    (a, b, c, d, e, tdef) = r['co']
    kx = kappa_derived(r['m'], r['ed'], _sage_const_1 , _sage_const_1 )
    if kx == _sage_const_0 :
        continue
    actual = ZZ(kx).valuation(ell)
    pred = a + c + e          # n=0 での (1.1)
    if (tdef == _sage_const_0 ) != (actual == pred):
        c_ng += _sage_const_1 
...
ell = _sage_const_2 
ed = [(_sage_const_0 , _sage_const_0 , (_sage_const_1 , _sage_const_0 )), (_sage_const_0 , _sage_const_0 , (_sage_const_0 , _sage_const_1 ))]
...
if P is None:
    ...
    want = (QQ(_sage_const_0 ), QQ(_sage_const_2 ), QQ(_sage_const_4 ), QQ(-_sage_const_6 ), QQ(-_sage_const_1 ))
    if (a, b, c, d, e) != want:
    ...
    DV = [_sage_const_5 , _sage_const_19 , _sage_const_61 , _sage_const_167 ]     # DuBose-Vallieres の数列（cycle 14 §7.3・cycle 16 定理 D2）
    print('')
    print('  レベルごとの判定と実測:')
    for n in range(_sage_const_0 , _sage_const_5 ):
        Sn = sum(ds[:n]) if n <= len(ds) else sum(ds)
        says_equal = (Sn == tdef)
        kx = kappa_derived(_sage_const_1 , ed, ell**n, ell**n)
        actual = ZZ(kx).valuation(ell)
        pred = a * ell**(_sage_const_2  * n) + b * n * ell**n + c * ell**n + d * n + e
        is_equal = (actual == pred)
        mark = ''
        if _sage_const_1  <= n <= _sage_const_4 :
            mark = ' / DuBose-Vallieres=%s' % DV[n - _sage_const_1 ]
            if actual != DV[n - _sage_const_1 ]:
                fail('ell=2 トーラス n=%d: 塔の値 %s が DuBose-Vallieres の %s と違う'
                     % (n, actual, DV[n - _sage_const_1 ]))
