...
> **注意（救済時の追記）**: 以下が参照する `inputs/queries/seed-queries.md`,
> `inputs/seeds/{operations,axes,canonical-papers}.md` は、コミット `c7fe283`
> 「cycle 0 成果と文献バイアス seed を全削除(Λ再定義のためクリーンスタート)」で
> **意図的に削除済み**であり、現在の main には存在しない。実装（下記スクリプト）は
> クエリファイルのパスを引数で受け取るので、走らせる場合は Λ-statement の選別基準
> （`inputs/seeds/lambda-statement-program.md`）に沿ったクエリファイルを新規に用意すること。
> 旧クエリをそのまま復元すると、`docs/tasks/auto-loop-runbook.md` が禁じている
> 「文献の厳密可解分類での整理」へ戻ってしまう。
