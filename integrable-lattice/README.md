...
作業前に必ず [docs/tasks/auto-loop-runbook.md](docs/tasks/auto-loop-runbook.md) の
「証明の書き方」節を読むこと。要点:

1. **できる限り Λ の言葉で証明を記述し、可算の世界と非可算の世界を分別しながら証明する。**
   可算（ℕ/ℚ/Λ/ℚ̄）と非可算（ℝ/ℂ）を混ぜず、**ℝ へ脱出した箇所を必ず明示する**。
...
2. **証明は構造化 LaTeX（`structured-latex/`）の形式で残す。新規に Typst で書かない。**
   ...
   移行時は**証明の中身が原本から確実に運ばれたか**を突き合わせて確認すること。
