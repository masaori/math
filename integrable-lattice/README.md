...
可積分格子模型の周辺で、**Λ/ℚ̄ で決定可能・ℝ脱出隔離・形式検証可能な statement** を広く発掘し、論文候補の母集団を作る。

土台は `docs/discussion/対数順序群上の統計力学/` の立場（統計力学をできるだけ可算な対数順序群 $\Lambda$ と代数的数 $\overline{\mathbb{Q}}$ で定式化し、$\mathbb{R}/\mathbb{C}$ への移行を不可欠な一点に隔離する）。収集の整理軸・選別軸はこの **決定可能性の梯子（ℕ⊂ℚ⊂Λ⊂ℚ̄ ⊂ ℝ）と四軸（帰属／計算可能性／複雑性／可解性）** に置く。文献の「厳密可解」分類（determinant か character か）では整理しない。詳細は `inputs/seeds/lambda-statement-program.md`。
