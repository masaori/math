...
cycle 21 step 4 で日本語正本へ命題 G″ を足した際に確定した語。**§1 の表と同じ拘束力を持つ**。

| 日本語 | 英語 | 根拠 |
|---|---|---|
| 飽和（$v_2(4)$ で止まる枝） | **saturation** | 本プロジェクトの造語（cycle 16 注 5.8 が名付け、cycle 20 step 3 が機構として同定した）。逐語訳。$1+h^N=0$ により最小が定数項へ移り、そこで止まる現象。初出で「$\ell$ 奇には無い枝である」ことを必ず添える |
| 打ち消し（$U_e=U_o$ の同点） | **cancellation** | §1.5 追記の「打ち消し = cancellation」と同語。**命題 R の (J2) の打ち消しとは別の障害**（こちらは (J3) の最小点の tie）なので、初出でどちらかを明示する |
| 係数の次の $2$ 進桁 | **the next 2-adic digit of the coefficient** | 補題 Y2 の $w$。**"the next binary digit" にしない**（本文は付値の言葉で書いており、2 進表記の話ではない） |
| 例外なく型 III | **of type III without exception** | (G″2)。**"always type III" と言い切らない**（族に固有の性質であって $\ell=2$ 一般の性質ではない、という限定が本文の主張の一部だから） |
| 排反かつ網羅 | **exclusive and exhaustive** | (G″1)。場合分けの健全性の主張。標準語 |
| 母集団 | **population** | (G″ の限界)。統計の標準語。**"sample" と混ぜない**（本文は全走査であって標本抽出ではないと明記している） |
| 全走査 | **a full sweep** | 同上。「宣言した有限母集団を尽くした」の意 |

### 訂正: 記号 ℓ は地の文へ書ける（上の「記号 ℓ を地の文へ書けないこと」は現状では誤り）

上の節は「U+2113 `ℓ` は日本語版 `tools/unicode-math.ts` の対応表に無い」と書いているが、
**現在の `integrable-lattice/structured-latex/tools/unicode-math.ts` には `"ℓ": "$\\ell$"` の項が実在する**
（cycle 21 step 4 で grep により確認）。実際、命題 G″ の英語版タイトルは地の文に `ℓ` を含むが、
`npm run build:pdf` は「組めない文字 0 件」で通る（同 step で実測）。
