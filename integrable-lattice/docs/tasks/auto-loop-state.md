...
current_cycle: 37           # cycle 0-36 完了。cycle 37 は同日 rank:cycle37 で総括した（掲げた 5 点はすべて潰れ、全数までの残りは 21 → 20 件へ減った＝外部定理「Kirchhoff の matrix-tree 定理」が完了。**本文側の 16 件は 2 サイクル続けて動いていない**）。本サイクルの主題は「『書けない理由』として記録されていたものが 3 つとも誤りだった」ことである（判別式を経由しなければ体は要らない／行列の単因子は要らない〈18 サイクル持ち越し〉／頂点集合を引数にすれば帰納法は通る）。
