...
| 3b-旧 | — | （当初の指示） | — | | **本サイクルの締め。主張を 1 つ完了させ、残り 19 件を実際に減らす。** step 1〜3 で 4 件が動いたが、動いたのは段であって主張の数ではない。最も完了に近いのは周期点数の終結式表示（一般の $d$ だけが残り、**素材の欠落は無い**）。呼び出し元が `MvPolynomial.finSuccEquiv`（`Mathlib/Algebra/MvPolynomial/Equiv.lean:647`）の実在を確認しており、これが「$d$ について再帰で反復多項式環を作る」ことそのものである＝step 1 が「こちらが持っていない」と書いた型は mathlib に在る。第二目標は命題 W\* の整数への降下（step 3 が「書けば通る見込み」と書いたが書いていない段）。 |
