...
current_cycle: 28           # cycle 27 完了。cycle 28 は 2026-08-04 に cycle 27 総括の「cycle 28 の焦点（案）」7 点から起こした。着手時に 7 点すべての前提を実測し、全点が一次情報と一致することを確認してから起こしている。最優先はユーザー方針の全数 Lean 形式化。
...
| 2 | 運用 | expand_definition_order_ledger | todo | | 検査 O の台帳を本文の用語・記号の全数走査で広げる。どこまでを本論文が定義する語とみなすかを決める。 |
| 3 | 運用 | reread_emphasis_removed | todo | | 強調を外して意味が落ちた箇所が無いかを実際に読んで確かめる。 |
| 4 | 運用 | back_extremum_nonempty | todo | | 検査 M の「構成から空でない」17 件を SageMath か Lean で裏取りできるか検討し、取れるものは取る。 |
| 5 | 運用 | port_pr69_pisano | todo | | 救済 PR #69 の 1 コミット分の差分だけを現在の main の上へ移植する（そのままマージしない）。移植不要と判断したなら根拠を PR へ書いて閉じる。 |
| 6 | 運用 | coverage_number_consistency | todo | | 本文の被覆の数値と台帳の数が一致することを検査にするか、本文から数を落として台帳を指すかを決めて実施する。 |
| 7 | — | rank:cycle28 | todo | | 総括・MEMORY・cycle 29 の焦点。 |
