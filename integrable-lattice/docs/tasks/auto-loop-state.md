...
current_cycle: 37           # cycle 0-36 完了。cycle 37 は 2026-08-05 に cycle 36 総括の「cycle 37 の焦点（案）」5 点から起こした。着手時に 5 点の前提を実測し、**5 点とも一次情報と一致した**（前提の食い違い 0 件。cycle 36 に続いて 2 サイクル連続）。ただし cycle 36 の教訓は「記録を測るだけでは足りず、中に入るまで分からない層がある」ことなので、一致したことを根拠に中の見立てを信じない。最優先はユーザー方針の全数 Lean 形式化で、**本サイクルは本文側を動かすことを主目的とする**（本文側 16 件は cycle 36 で動かなかった）。
