...
current_cycle: 36           # cycle 0-35 完了。cycle 36 は同日 rank:cycle36 で総括した（掲げた 5 点はすべて潰れ、全数までの残りは 22 → 21 件へ減った＝外部定理「可換環の上の Euler の双対基底公式」が完了。**本文側の 16 件は動いていない**）。本サイクルは着手時の実測では前提の食い違いが 0 件だったが、**中に入ってから 2 件の食い違いが出た**——「残り 1 件」と書かれていた 命題 W\* と matrix-tree が、どちらも残り 1 件ではなかった。
