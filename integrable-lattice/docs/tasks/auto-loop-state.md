...
current_cycle: 22           # cycle 0-21 完了。cycle 22 は 2026-08-01 に cycle 21 総括の「cycle 22 の焦点（案）」4 点から起こした。
...
| # | track | step | status | done日 | 観察メモ |
|---|------|------|--------|--------|----------|
| 1 | T3 Pure | cuoco_monsky_attribution | todo | | 命題 K への Cuoco–Monsky 帰属を本文（日英）へ書き足し、**CM §2 以降を読んで定理 W3・W4 の既出性を決着**させる。本文を触ってよい唯一の step。 |
| 2 | 運用 | transcription_guard | todo | | 本文ブロックと根拠 report の対応を機械検証する道具を作り、転記事故（3 回目）を検出可能にする。**例外表が日英対応検証に開ける穴**も塞ぐ。検査道具を触ってよい唯一の step。 |
| 3 | T3 Pure | coefficients_d_e | todo | | 係数 $d,e$ に必要な追加情報を特定し、可能なら明示式にする。取れないなら**何が原理的に妨げているか**を反例つきで確定させる。 |
| 4 | 運用 | lean_cycle21_theorems | todo | | 定理 Q1・G4 を Lean に通して検算（7 サイクル目）。**併せて cycle 21 step 3 の `lake build` を負荷の低いときに再実行**して 8674 jobs／196 定理 sorryAx 非依存を独立に確認する。 |
| 5 | — | rank:cycle22 | todo | | cycle 22 総括。cycle 23 の焦点を挙げる。 |
...
4. **cycle 21 の新定理群（定理 Q1・G4）の Lean 化**。7 サイクル目の検算。
   **合わせて cycle 21 step 3 の `lake build` を負荷の低いときに再実行して確認する**
   （本サイクルではマシンの負荷平均が 432 に達し呼び出し元が完走を確認できなかった。下記 step 3 の欄）。
