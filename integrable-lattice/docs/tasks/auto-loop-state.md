...
current_cycle: 25           # cycle 0-25 完了。cycle 25 は 2026-08-01 に cycle 24 総括の「cycle 25 の焦点（案）」4 点から起こし、同日 rank:cycle25 で総括した。
...
- 訂正 6 件はすべて塞がり、**新しい $c_1$ の証明に `Real` が 1 つも現れない**ことで
  ℝ 脱出の除去が型の上で確認された。
...
- **レートリミットによる中断が複数回起きたが、成果は 1 つも失われていない。** 実働子の本体は生き残り、
  worktree のコミットとブランチが成果を保持していた。**中断からの復帰は「生存確認」ではなく
  「コミット・ファイル更新・トランスクリプト成長という前進の一次情報」で判定した。**
- **同じ作業ツリーで依存取得（`lake exe cache get`）が二重に走る事故が起きた。**
  停止は `pkill` を使わず **PID の系統で特定**して自分の分だけを止めた（cwd 一致で選ぶと相手を巻き込む）。
