...
| 5 | 運用 | runin_label_other_shapes | todo | | 検査 L が箇条書きの直前以外の形も拾えることを示す（0 件だったことと拾えることは別）。 |
| 6 | 運用 | coverage_match_liveness | todo | | 被覆の数値の照合が実際に効いていることを毎回示す形にする（空振りしても静かなままを塞ぐ）。 |
