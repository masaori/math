...
current_cycle: 39           # cycle 0-38 完了。cycle 39 は同日 rank:cycle39 で総括した（掲げた 5 点はすべて潰れ、**全数までの残りは 20 → 19 件へ減った**＝外部定理「Newton 多面体の加法性」が完了。**本文側の 16 件は 4 サイクル続けて動いていない**）。本サイクルの主題は 2 つで、**「残りが N 件」の外れ方が 4 件そろって 1 つの形に見えたこと**（残りを閉じたと書いた定理が実は含意で、その仮定を誰が与えるかを書き手が数えていない。数える対象を散文から署名へ移す検査を入れた）と、**人が書いた数の誤りが本サイクルだけで 2 件出たこと**（外部定理の内訳／検査の段数。どちらも両端が 1 ずつずれ、差し引きだけ合っていた）である。
