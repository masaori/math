...
current_cycle: 25           # cycle 0-24 完了。cycle 25 は 2026-08-01 に cycle 24 総括の「cycle 25 の焦点（案）」4 点から起こした。
...
| # | track | step | status | done日 | 観察メモ |
|---|------|------|--------|--------|----------|
| 1 | 運用 | fix_q5_c1_and_g2_cond32 | todo | | cycle 24 step 5 の新規検出 2 件を根拠 report で訂正する。 |
| 2 | 運用 | guard_missing_proof_and_rotten_refs | todo | | 証明の欠落と、本文・コメント中の腐ったツール参照を機械検出する。 |
| 3 | 運用 | lean_cycle25 | todo | | Lean 検算 10 サイクル目（訂正後の Q 系・本文へ入った命題 M・U・未検算の定理群）。 |
| 4 | 運用 | reflect_proofs_and_q_series | todo | | 命題 M・U の証明を本文へ運び、Q 系を訂正後の形で本文へ入れる。 |
| 5 | — | rank:cycle25 | todo | | 総括と cycle 26 の焦点。 |
