...
current_cycle: 27           # cycle 0-26 完了。cycle 27 は 2026-08-03 に cycle 26 総括の「cycle 27 の焦点（案）」5 点から起こした。着手時に 5 点すべての前提を実測し、4 点は一致・1 点（日本語版 PDF のアスタリスク「410 箇所」）は数え方が行数だったことを確認してから起こしている。
...
| # | track | step | status | done日 | 観察メモ |
|---|------|------|--------|--------|----------|
| 1 | 運用 | scan_min_empty_conventions | todo | | 本文の全 $\min$ / $\max$（日 129・英 130）について、空集合になりうるか・規約が書かれているかを機械で洗う。同型の事故が 2 例目（定理 G2 $(3.2)$・命題 G′）で、**どちらも $\ell=3$ だけが落ちるので目視では見つからない**。 |
| 2 | 運用 | lean_cycle25_remaining_six | todo | | cycle 25 が本文へ運んだ 7 件のうち未検算の 6 件（命題 G″・J・K・R・M・U）を Lean で検算する。 |
| 3 | 運用 | measure_ja_emphasis_only | todo | | 日本語版 PDF の素のアスタリスクの**計測と、変えた場合に何がどう変わるかの提示までを行う。本文の強調そのものは 1 箇所も書き換えない**——正本 PDF の見た目を変える判断はユーザーの価値判断であり、自動ループでは決めない。**判断が要る 1 点として管理役へ上げる。** |
| 4 | 運用 | transcription_covers_provenance | todo | | 転記検査に残る弱点を減らす。機械検証できない免除 8 件と、`covers` へ回した 28 件の passage（report 本文より弱い出所と照合している）。 |
| 5 | 運用 | back_lean_tactic_ledger | todo | | `field_simp` 直後の `ring` の台帳 10 件を、**実際に `ring` を外して落ちることで裏取りする**。いまの根拠は「ビルドが通っているから必要なはず」という論理であって実測ではない。 |
| 6 | — | rank:cycle27 | todo | | 総括・MEMORY・cycle 28 の焦点。 |
