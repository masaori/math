...
current_cycle: 40           # cycle 0-39 完了。cycle 40 は同日 rank:cycle40 で総括した（掲げた 5 点はすべて潰れたが、**全数までの残りは 19 件のまま動かなかった。本文側の 16 件は 5 サイクル続けて動いていない**）。本サイクルの主題は 2 つで、**段数を初めてサイクルをまたいで比べられたこと**（全体は 8 → 44 段だが数えられる欄が 6 → 14 件へ増えたためで、**比べられる 6 件では 8 → 10 段。減っていない**）と、**「書けない」「済み」と書かれていたものが測ると 3 つとも別の形で外れていたこと**（記号の取り違え／探した語が定理の名前だったこと／済みの根拠を名指せない部が 7 件）である。
