...
**実測の結論を先に書く。** 実数へ出ると閉じるのは**計算で裏を取る道**である
（SageMath の厳密計算と Lean の `decide` がそこで止まる）。**証明を検査する基準は変わらない**——
Lean の kernel も本プロジェクトの検査も、実数を使った証明を使わない証明と同じ規則・同じ基準で通し、
実数のために追加の公理も要らない。ただし裏取りの道が閉じるぶん、
**定義が縮退したまま証明が通ってしまったことを検算で捕まえられなくなる**（このプロジェクトで実際に起きた）。
そのうえで、線引きをどの案に変えても**動くのは外部定理 27 件のうち 2 件**である。
...
補題 Q5（例外的な点の個数がレベルに依らず有限であることを言う補題）に現れる定数 $c_1$ の
...
### 公理 — 実数を使っても、mathlib 標準の 3 公理の外へは出ない
...
785 件すべてに `#check` をかけて型を出したところ、**主張の型そのものに
$\mathbb{R}/\mathbb{C}$ が現れる定理は 10 件**であり、**その 10 件はすべて 3 公理すべてに依存する。**
一方、残る 775 件でも 641 件（83%）が同じ 3 公理に依存する。
すなわち実数を使えば 3 公理は確かに要るが、
**それは実数を使わない証明の大半でも同じであって、実数のために追加の公理が要るわけではない**
（3 公理の外にある公理は 785 件のどこにも現れない）。

（この切り方は主張の型に実数が現れるかで測っている。主張は可算側にあって証明の途中だけ
実数を経由する場合は、この測り方では拾えない。ファイル単位で粗く切ると
$\mathbb{R}/\mathbb{C}$ を使う 3 ファイルの 68 件中 53 件（78%）、残る 713 件中 597 件（84%）となり
実数側のほうが低く見えるが、これは 3 ファイルに実数を使わない定理が多数同居しているためで、
**定理単位で切ればその見かけは消える**。）
...
| `padicValNat_pow_sub_one_of_dvd`（命題 L で使う、冪の付値を持ち上げる補題） | 使わない（$\mathbb{N}$） | 3 公理すべて |
| `crudeBound_padicValInt_le_mul_logb`（補題 Q0＝アルキメデス粗上界の実対数版） | **使う** | 3 公理すべて |
...
**すなわち `Classical.choice` は実数だけが呼び出すものではない。** mathlib の補題はほとんどが
古典論理で書かれているので、$\mathbb{N}$ の中だけで閉じた主張でも、そうした補題を一つ引いた時点で
同じ 3 公理に依存する（上の表の matrix-tree・Newton 多面体・冪の付値がそれである）。
公理が減った 134 件はいずれも実数を使わない側にあるが、**実数を使わないことが公理の減る条件に
なっているわけではない**——使わない側でも 83% は 3 公理すべてに依存している。
上の表で公理が減っている 2 件は、いずれも証明が短く、古典的な補題を経由していない。
134 件すべてについてその原因を調べたわけではない。
...
### 検証の強さ — 検査の基準は同じ。ただし「通ること」が主張の意味まで保証するわけではない
...
**したがって、検査の基準としては、実数を使った Lean の証明は使わない証明と同じ強さで通る。**
Lean の kernel はどちらも同じ規則で検査し、この検査もどちらも同じ基準で通す。
実数を使うことで落ちるのは、上に書いた**計算による裏取り**の側だけである。

**ただし「検査を通ること」は「定義が意図した対象を指していること」までは保証しない。**
前節のジャンク値がその実例である。$c_1$ の旧定義は $b=0$ で真数が $0$ になり本来定義されないのに、
`Real.log 0 = 0` のおかげで値を返し、その値を含む主張が Lean を通ってしまう。
**この穴は実数に固有ではない**——`(3:ℕ) - 5 = 0` も `(3:ℚ)/0 = 0` も同じく Lean を通る（実行して確認した）。
実数側で厄介なのは、**穴に落ちたことを計算で検出する道が同時に閉じている**ことである。
訂正後の $\mathbb{N}$ 版では 8 通りの表を `by decide` で機械に検算させて縮退が無いことを確かめられた
（`Q5_c1_table_check`）が、実対数版に同じ検算はできない。
これは前節の「計算で裏を取る道が閉じる」が、証明の側へ跳ね返る経路である。
...
この 3 ファイルには検査対象の定理が 68 件宣言されているが、
**主張の型に $\mathbb{R}/\mathbb{C}$ が現れるのはそのうち 10 件である**
（残りは実数を使わない定理が同じファイルに同居しているだけである）。
...
短所は二つある。第一に、上の実測のとおり**検査の基準としては実数を使っても弱くならない**ので、
②（可算側の内容を担う）は検査の強さを守るための条件ではない
（ただし、実数側では定義の縮退を計算で検出できないという弱さは残る。前節のジャンク値の実例）。
第二に、隔離できているかの判定は人の読みであり、
...
後者（成果の出し方）は、この二つが「同じ境界線を別方向から指している」ことを
この研究方針の骨格として挙げている。**今回の実測は、その一致が成り立つ層を限定する。**
...
**「等号が決定可能か」は計算の層の境界であって、証明の層を検査する基準の境界ではない。**
ただし二つの層は完全に独立ではない。計算の道が閉じると、
**定義が縮退したまま証明が通ったことを検算で捕まえられなくなる**（ジャンク値の実例）。
実数の影響は、検査の基準ではなく、検査を通った主張を疑う手段の側に出る。
