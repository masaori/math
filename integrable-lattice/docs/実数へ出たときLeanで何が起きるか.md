...
このうち、**主張の式そのものに $\mathbb{R}/\mathbb{C}$ が現れる定理は 11 件**であり、
**その 11 件はすべて 3 公理すべてに依存する。** 一方、残る 774 件でも 640 件（83%）が
同じ 3 公理に依存する。すなわち実数を使えば 3 公理は確かに要るが、
**それは実数を使わない証明の大半でも同じであって、実数のために追加の公理が要るわけではない**
（3 公理の外にある公理は 785 件のどこにも現れない）。

（この切り方は主張の式に実数が現れるかで測っている。主張は可算側にあって証明の途中だけ
実数を経由する場合は、この測り方では拾えない。ファイル単位で粗く切ると
$\mathbb{R}/\mathbb{C}$ を使う 3 ファイルの 68 件中 53 件（78%）、残る 717 件中 598 件（83%）となり、
実数側のほうが低く見えるが、これは 3 ファイルに実数を使わない定理が多数同居しているためである。）
...
**主張の式に $\mathbb{R}/\mathbb{C}$ が現れるのはそのうち 11 件である**
