...
**より広い族への拡張（観察 T′）**。$(9.1)$ は $L=2^{n}$ の 7 点だけの一致だが、$L=2^n$ に限らず
**任意の偶数 $L$** について $s:=v_2(L)$ とおくと

$$v_2(\tau(L))=(2s+4)\,L-(6s+1) \tag{9.1$'$}$$

が成り立つ、という観察が得られる。$L=2^{n}$（$s=n$）を代入すると
$(2n+4)2^{n}-(6n+1)=2n\,2^{n}+4\cdot2^{n}-6n-1$ となり、$(9.1)$ を**特殊化として含む**。
$(9.1')$ の内容は「$v_2(\tau(L))$ は $L$ の奇部分に依らず $s=v_2(L)$ と $L$ だけで決まる」ことであり、
$L$ が 2 冪でない偶数（$L=6,10,12,\dots$）を含む点が $(9.1)$ より広い。

確認範囲: $L=2,4,\dots,160$ の**全偶数 80 例**で $(9.1')$ が成立（不一致 0 件）。
$s$ ごとの標本は $s=1$ が 40 例（$L=2,6,10,\dots,158$）、$s=2$ が 20 例、$s=3$ が 10 例、
$s=4$ が 5 例、$s=5$ が 3 例、$s=6,7$ が各 1 例。
計算は `sagemath/check/cycle13_T1_tau_v2/tau_v2_verify.sage` の (6)（実行ログ同 `.out`）で、
$\tau(L)$ を $\mathrm{Res}\bigl((x^{L}-1)/(x-1),\,D(x)\bigr)$ の**厳密整数**として求めている
（浮動小数点を使わない。同スクリプトの (7) で OEIS A212800 の $a(2..11)$ とも厳密一致を確認済み）。

**位置づけ: $(9.1')$ は数値観察であって証明ではない。** cycle13 の観察 T（奇 $L$）の証明は
2 が $\mathbb{Z}[\zeta_L]$ で不分岐であること・$v_2(L)=0$ を本質的に使っており、
偶 $L$ ではこれらが両方壊れる（`sagemath/check/cycle13_T1_tau_v2/` の (4)）。
したがって $(9.1')$ に対する証明は現時点で無く、$L>160$ の偶数については何も主張しない。
$(9.1)$ と同じく、$\ell^{2n}$ 型の主要項をもたない（$\mu=0$）ことと整合する。
