...
| **補題 Q5（悪い点は $O(1)$ 個）**: 上の条件を満たさない点の個数は $M$ に依らず $r\ell^{c_1}$ 以下（$c_1$ は明示） | **証明した**（§5.2）。実測でも $M$ について有界（§8）。**2026-08-01（cycle 25 step 1）訂正: $c_1$ は「$2b<(\ell-1)\ell^{c}$ を満たす最小の自然数」＝$\mathbb{N}$ 上の決定可能な最小元である**（初稿の実対数による定義は $b=0$ で定義されなかった。§5.2 の訂正） |
| **補題 Q0（粗上界）**: $\hat\theta_M(P)\le\varphi(\ell^M)\log_\ell C_0$（$C_0=\sum|c_{pq}|$） | **証明した**（§5.1）。**本 report で $\mathbb{R}$（アルキメデス素点）へ脱出する唯一の箇所**。§7.3 で隔離を明示する（**この「唯一」は cycle 25 step 1 の訂正で初めて正しくなった**——初稿は補題 Q5 の $c_1$ に実対数を使っており、そこも脱出していた。§7.3 の訂正） |
...
> c_1:=\min\bigl\{\,c\in\mathbb{Z}_{\ge0}\ :\ 2b<(\ell-1)\,\ell^{c}\,\bigr\}. \tag{5.2}$$
> *とくに $|\mathcal{B}_M|$ は $M$ に依らない。*

**$c_1$ が定まること（$\mathbb{N}$ の中だけで済む）.** $\ell\ge2$ なので $(\ell-1)\ell^{c}$ は $c$ について
狭義単調増加で非有界、よって条件を満たす $c$ は存在する。条件 $2b<(\ell-1)\ell^{c}$ は**整数の比較ひとつ**であり
$\mathbb{N}$ 上で決定可能だから、条件を満たす $c$ の集合は空でない $\mathbb{N}$ の部分集合として最小元を持つ。
**$b=0$ のときは $c=0$ が $0<\ell-1$ を満たすので $c_1=0$。**
実対数も切り上げも現れない＝**この定数の定義で $\mathbb{R}$ へ脱出しない**（§7.3）。

**証明.** まず $b=0$ の場合を片付ける。$(1.2)$ の $m_i$ は $\ge1$ なので $b=\sum_im_i=0\iff r=0$ であり、
このとき $B$ は空積 $=1$、したがって全ての $P$ で $\beta_P=0$ である。$P\in\mathcal{B}_M$ は
$\theta_G^{\max}\ge\varphi(\ell^M)$ を要求するが、仮定 $\varphi(\ell^M)\ge2\theta_G^{\max}$ と合わせると
$\theta_G^{\max}\ge2\theta_G^{\max}$ すなわち $\theta_G^{\max}=0$、よって $0\ge\varphi(\ell^M)\ge1$ で矛盾。
ゆえに $\mathcal{B}_M=\emptyset$ で、$(5.2)$ は $0\le r\ell^{c_1}=0$ として成り立つ。

以下 $b\ge1$ とする。$P\in\mathcal{B}_M$ なら $\beta_P\ge\varphi(\ell^M)-\theta_G^{\max}\ge\varphi(\ell^M)/2$。
$\rho_{\max}:=\max_i\rho_i(P)$ とする。ある $i$ で $\rho_i(P)=M$ なら $\rho_{\max}=M\ge M-c_1$ で以下は自明なので、
全ての $i$ で $\rho_i(P)<M$ とする。このとき $\beta_P\le b\,\ell^{\rho_{\max}}$ なので
$\ell^{\rho_{\max}}\ge\varphi(\ell^M)/(2b)=\ell^{M-1}(\ell-1)/(2b)$。
$c_1$ の定義（狭義不等式 $2b<(\ell-1)\ell^{c_1}$）より $(\ell-1)/(2b)>\ell^{-c_1}$ だから

$$\ell^{\rho_{\max}}\ >\ \ell^{M-1}\cdot\ell^{-c_1}=\ell^{M-c_1-1},$$

すなわち $\rho_{\max}>M-c_1-1$ であり、$\rho_{\max}$ は整数なので $\rho_{\max}\ge M-c_1$。
**ここで使ったのは狭義不等式だけで、対数も切り上げも使っていない。**
...
> **（このとき書き足した段落は、次の訂正で $c_1$ の定義ごと差し替わった。
> $+1$ の役割と反例が新定義の下でどう読み替わるかは、次の訂正ブロックに書いた。）**

> **【訂正 2026-08-01（cycle 25 step 1）— $c_1$ の定義そのものを差し替えた】**
>
> **初稿（および cycle 24 step 1 の訂正後）の定義は**
> $$c_1:=\max\Bigl(0,\ \Bigl\lceil 1+\log_\ell\frac{2b}{\ell-1}\Bigr\rceil\Bigr)$$
> **であった。これは $b=0$ で $\log_\ell 0$ を含み、定義されない。**
> 定理 Q1 は仮定 (H) しか置かず $b$ に下限を課していないので、$b=0$ は定理の適用範囲の中にある。
> しかも **$b=0$ は仮想の場合ではない**——同じ cycle 21 の**系 G6（$S_\infty=\emptyset$）がまさにこの場合**であり
> （`cycle21_T3_general_closed_form.md` §5.5。そこで $b=0$）、
> 本 report の母集団でも **461 組の $(\text{塔},\ell)$ のうち 317 組（68.8%）が $b=0$** である（下記の検算）。
> 検出は cycle 24 step 5 の Lean 検算 `cycle24_ops_lean_cycle23_corrections.md` §2.1
> （`Q5_logb_junk_at_b_zero`。mathlib の `Real.log 0 = 0` というジャンク値のせいで、
> 式が根拠なく「動いて」しまうことまで型に出されている）。
>
> **差し替えの中身**: 補題 Q5 の証明が実際に使うのは**狭義不等式 $2b<(\ell-1)\ell^{c_1}$ だけ**なので、
> それを満たす**最小の自然数**を $c_1$ の定義に採った（上の $(5.2)$）。これで
> (i) $b=0$ の縮退が消え（$c_1=0$）、(ii) **実対数＝$\mathbb{R}$ 脱出が消える**（$\mathbb{N}$ 上の決定可能な述語の最小元）。
> 同型の書き換えは cycle 24 step 4 が定理 G3 の $\lceil\log_\ell(e_{j^*}+1)\rceil$ に対して既に行っている。
>
> **cycle 24 step 1 が書き足した「$+1$ の役割」は、新定義の下でこう読み替わる。**
> 旧定義の $+1$ は「切り上げだけでは $\frac{2b}{\ell-1}$ が $\ell$ の冪ちょうどのときに等号
> $2b=(\ell-1)\ell^{c_1}$ が起きてしまう」ことへの手当てだった。
> **新定義は狭義不等式そのものを定義に採るので、この手当てが定義の中へ吸収される**——
> 等号が起きる $c$ はそもそも $c_1$ の候補集合に入らない。
> したがって **$+1$ という補正項は新定義には存在しない**（役割が消えたのではなく、定義の側へ移った）。
>
> **cycle 24 step 1 が挙げた反例（$\ell=2$, $b=1$, $c_1=1$, $M=3$, $\rho_{\max}=1$）は、
> 新定義の下では「$c_1=1$ という値がそもそも取れない」という形になる。**
> $\ell=2,b=1$ では $2b=2$ に対し $(\ell-1)\ell^{1}=2$ で狭義不等式が破れるので $c=1$ は候補に入らず、
> $c=2$（$2<4$）が最小＝新定義は $c_1=2$ を返す。旧定義も $\max(0,\lceil1+\log_2 2\rceil)=2$ を返すので、
> **この場合は両定義の値が一致する。反例が排除される機構だけが「証明の中の注意」から「定義」へ移った。**
>
> **新旧の値の関係（証明つき）**: $b\ge1$ のとき旧定義の $c_1^{\text{旧}}$ は
> $c_1^{\text{旧}}\ge1+\log_\ell\frac{2b}{\ell-1}$ より $(\ell-1)\ell^{c_1^{\text{旧}}}\ge2b\ell>2b$ を満たす、
> すなわち**候補集合の元**である。よって $c_1^{\text{新}}\le c_1^{\text{旧}}$ で、
> **上界 $r\ell^{c_1}$ は決して悪化せず、$\frac{2b}{\ell-1}$ が $\ell$ の冪でないときは $\ell$ 倍タイトになる。**
> 定理 Q1 $(6.1)$ の $C$ は $c_1$ について単調増加なので、$C$ も悪化しない。
>
> **検算（cycle 25 step 1 が自分で実行した。ℝ を使わない整数計算だけ）**:
> $\ell\in\{2,3,5,7,11\}\times b\in\{0,\dots,20\}$ の全 105 組で、新定義が狭義不等式を満たすこと・最小であること・
> $c_1^{\text{新}}\le c_1^{\text{旧}}$ を確認（違反 0）。さらに母集団 124 塔 $\times$ 素数 5 個の
> **461 組**で $(b,r,\theta_G^{\max})$ を計算し直し、補題 Q5 の仮定
> $\varphi(\ell^M)\ge2\theta_G^{\max}$ かつ $M>c_1$ を満たす **1620 組の $(\text{塔},\ell,M)$** について
> **結論 $|\mathcal{B}_M|\le r\ell^{c_1}$ と、証明の中間段 $\rho_{\max}\ge M-c_1$ を各点で確認した（違反 0）。**
> うち **938 組が $b=0$ で、すべて $\mathcal{B}_M=\emptyset$**（上の $b=0$ の場合分けの実測）。
> **上界が等号で達成される塔が実在する**（例: `FAM p=5 q=6`、$\ell=3$、$b=2$, $r=1$, $c_1=1$ で
> $|\mathcal{B}_M|=3=r\ell^{c_1}$ が $M=2,\dots,6$ で成立）ので、
> **$c_1$ をこれ以上小さく取ると補題 Q5 は実際に破れる**＝新定義の最小性は必要である。
> 数値は `cycle25_ops_fix_q5_c1_and_g2_cond32.md` §3 に全部載せた。
...
> **【追記 2026-08-01（cycle 25 step 1）— $b=0$ のときの読み方】**
> 補題 Q5 の $c_1$ を「$2b<(\ell-1)\ell^{c}$ を満たす最小の自然数」に直した（§5.2）ので、
> **定理 Q1 は $b=0$ でも文字どおり意味を持つ。** 具体的には $c_1=0$、$r=0$ となり
> - 仮定「$M>c_1$」は「$M\ge1$」＝**全てのレベル**になる（$b=0$ のとき定理の適用範囲は狭まらない）、
> - $(6.1)$ の定数は $C=\theta_G^{\max}\dfrac{\ell+1}{\ell}$ に退化する（$b=0,r=0$ を代入）、
> - 主張は $|\Theta_M|\le\theta_G^{\max}\dfrac{\ell+1}{\ell}\ell^M=\theta_G^{\max}(\ell+1)\ell^{M-1}$ になる。
>
> **これは正しい。** $b=0$ なら $\mathcal{B}_M=\emptyset$（§5.2 の場合分け）かつ全点で $\beta_P=0$ なので
> $\Theta_M=\sum_P\theta_G(P)\le\theta_G^{\max}\cdot|\mathbb{P}^1(\mathbb{Z}/\ell^M)|=\theta_G^{\max}(\ell+1)\ell^{M-1}$
> であり、**上の $C\ell^M$ はこの自明な数え上げとちょうど一致する**（$(6.1)$ の第 2 項は元々この項である）。
> すなわち $b=0$ での定理 Q1 は「$\Theta_M$ は $\ell^M$ のオーダー」という主張になり、
> $(6.2)$ は $\mathrm{ord}_\ell(\kappa_n)=\mu(\ell^{2n}-1)+O(\ell^n)$（$n\ell^n$ の項が消える）を与える。
> **これは系 G6（$S_\infty=\emptyset$）が閉形式で与える $b=0$ と整合する**
> （`cycle21_T3_general_closed_form.md` §5.5。そこでは誤差項の中身まで開いて $c=\frac{\ell}{\ell-1}A_{\mathrm{gen}}$ が出る）。
> 初稿の $c_1$ ではこの場合に定数の定義自体が壊れており、**定理の主張が書けなかった**。
...
> **【訂正 2026-08-01（cycle 25 step 1）— 初稿の「ただ 1 箇所」は正しくなかった】**
> 初稿の本節は「$\mathbb{R}$ へ脱出するのは補題 Q0 ただ 1 箇所である」と書いていたが、
> **補題 Q5 の $c_1$ の定義 $\max\bigl(0,\lceil1+\log_\ell\frac{2b}{\ell-1}\rceil\bigr)$ も実対数を使っており、
> そこも脱出していた。**すなわち初稿の脱出箇所は 2 つで、本節はそれを数え落としていた。
> 検出は cycle 24 step 5 の Lean 検算（`cycle24_ops_lean_cycle23_corrections.md` §1.5・§2.1。
> Lean 側でも `Real.logb` / `Real.rpow_logb` が現れるのは $c_1$ の扱いのためだけで、
> **脱出の出どころは Lean ではなく report の定義そのものだ**と明記されている）。
> **cycle 25 step 1 の訂正（§5.2）で $c_1$ から実対数を除去したので、
> 本節の「補題 Q0 ただ 1 箇所」は現在の記述に対しては正しい。**
> 新しい $c_1$ は「$2b<(\ell-1)\ell^{c}$ を満たす最小の自然数」＝整数の比較だけで決まる量なので、
> 上の箇条書きに次を足す:
> - **$c_1$（補題 Q5 の定数）**: $\mathbb{N}$ 上の決定可能な述語の最小元。**可算・決定可能。$\mathbb{R}$ を使わない。**
>
> なお定理 Q1 の $C$ にはなお $\log_\ell C_0$ が残るが、これは補題 Q0 由来であり、
> **$\mathbb{R}$ 脱出は依然として補題 Q0 の 1 箇所に隔離されている**（$C$ は誤差項の係数にしか現れず、
> $b$ の値にも $b$ の決定手続きにも入らない）。
...
> **【訂正 2026-08-01（cycle 25 step 1）】** $c_1$ を新定義に直す（§5.2）と上界 $r\ell^{c_1}$ は
> 場合により $\ell$ 倍タイトになるので、「実測はこの上界よりずっと小さい」という上の観察は**弱まる**。
> 124 塔 $\times$ 素数 5 個・仮定を満たす 1620 組で測り直したところ、
> **上界が等号で達成される塔が実在した**（`FAM p=5 q=6`、$\ell=3$、$b=2,r=1,c_1=1$ で
> $|\mathcal{B}_M|=3=r\ell^{c_1}$、$M=2,\dots,6$。$\ell=11$ では $c_1=0$ で $|\mathcal{B}_M|=2=r\ell^{0}$）。
> **上界は破れていない（違反 0 件）が、「ずっと小さい」は新定義の下では一般には言えない。**
> これは新定義の最小性が必要であることの実測でもある。
