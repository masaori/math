...
| **定理 K（一般の予言アルゴリズム）**: 上の 2 つを合わせると、$\mathrm{ord}_\ell(\kappa_n)$ は $D$ の係数だけからの有限計算で決まる（塔の値も円分体の計算も使わない） | **証明した**（§4.3）。母集団走査で **2128 段**を塔の値と照合し**全一致**（検証 Step D）。ただし $\ell=5$ は母集団 430 個のうち先頭 254 個までで、残り 176 個は時間上限で打ち切った（§9.1） |
...
| **定理 J7（$\theta=\infty$ ⇒ 型 III）**: $n\ell^n$ 項の係数は $b=\sum_{P\in S_\infty}j^*(P)$、$S_\infty=\{P\in\mathbb{P}^1(\mathbb{Z}_\ell):\theta(P)=\infty\}$ | **証明した**（§5.2、仮定つき。仮定 (F) は系 J10 で不要になった。残る仮定は $\mathrm{argmin}$ の一意性）。**和は $\mathbb{P}^1(\mathbb{F}_\ell)$ の方向ではなく $\mathbb{P}^1(\mathbb{Z}_\ell)$ の点についてである**（ここを取り違えたのが本サイクル最大の誤り。§11.2）。**$n\ell^n$ を生むのは点そのものではなく各点の $\ell$ 進近傍である**（§5.6 (b)、§11.4） |
| **定理 J8（型 III 塔の族と閉形式）**: $\ell$ 奇素数、bouquet に $(1,0)$ を $\ell-1$ 本・$(0,1)$ を 1 本置くと、**全ての $n\ge0$** で $\mathrm{ord}_\ell(\kappa_n)=2n\ell^n+2\ell^n-2$。この族では $S_\infty=\{(1{:}1),(1{:}-1)\}$ が**厳密に決定できる** | **証明した**（§5.3）。**ただしこれは step 2 定理 X′ の $(p,q)=(\ell-1,1)$ の場合であり、族としての正本は step 2 側である**（§5.6 (a)）。本 report での意義は、定理 J7 という別の道具から独立に同じ値へ到達したことである |
...
| **補題 J9（指数の一次独立性）**: 相異なる $\gamma_1,\dots,\gamma_k\in\mathbb{Z}_\ell$ に対し $(1+x)^{\gamma_1},\dots,(1+x)^{\gamma_k}\in\mathbb{F}_\ell[[x]]$ は $\mathbb{F}_\ell$ 上一次独立 | **証明した**（§5.5）。定理 J2 の桁安定性がそのまま効く |
| **系 J10（$S_\infty$ の決定）**: $S_\infty$ は**有限**で、その点はすべて**有理点**であり、$\mathrm{supp}(\bar{\tilde E})$ の差ベクトルからの有限計算で候補が尽きる | **証明した**（§5.5）。**定理 J7 の仮定 (F) は仮定ではなくなった。§7.2 に未解決として残していた問いが解けた** |
| **step 2（`cycle19_T3_theta_infinity.md`）との関係**: 定理 J8 は step 2 定理 X′ の $(p,q)=(\ell-1,1)$ の場合。$\theta=\infty$ の族については **step 2 の定理 X′ が正本**であり、本 report は $b=\sum j^*$ の側からそれを再現する | **突き合わせ済み**（§5.6）。$\ell\in\{3,5,7\}$ の 146 組・例外直線 196 本で $b=2$ と閉形式の一致を機械照合（検証 Step H） |
| 残る障害（打ち消し $J(r)$ 非一意、$S_\infty$ を決める**判定**の完全な手続き、$\ell=2$） | **未解決**（§7）。何が足りないかを具体化し、打ち消しの実例は奇 $\ell$ でも示した |
| 新規性 | **主張しない**（§10） |
...
> **先に断っておく（§5.6 (a)）**: 以下の定理 J8 は、同じ cycle の step 2
> （`cycle19_T3_theta_infinity.md`）定理 X′ が族 $p(1,0)+q(0,1)$ について証明した閉形式の
> $(p,q)=(\ell-1,1)$ の場合であり、**族としての一般形は step 2 側が正本**である。
> ここに残すのは、下の証明が定理 J7（$b=\sum j^*$）という**別の道具**から独立に
> 同じ値へ到達しており、2 つの step の相互検証になっているからである。
...
### 5.5 補題 J9・系 J10（$S_\infty$ は有限で、すべて有理点である）

定理 J7 は仮定 **(F)（$S_\infty$ が有限）** を置いており、§7.2 はそれを未解決として残していた。
**これは解決した。** 決め手は、cycle 19 step 2（`cycle19_T3_theta_infinity.md` 命題 3）が
Newton 多面体の Minkowski 分解で「**$\mathbb{Z}^2$ の**例外直線は有限本」を示していたことと、
そこに残っていた唯一の隙間 —「$\mathbb{Z}_\ell\setminus\mathbb{Q}$ の方向にも $\theta=\infty$ がありうるか」—
を次の補題が塞ぐことである。

> **補題 J9（指数の一次独立性）.** *$\gamma_1,\dots,\gamma_k\in\mathbb{Z}_\ell$ が相異なれば、
> $(1+x)^{\gamma_1},\dots,(1+x)^{\gamma_k}\in\mathbb{F}_\ell[[x]]$ は $\mathbb{F}_\ell$ 上一次独立である。*

**証明.** $\sum_ia_i(1+x)^{\gamma_i}=0$（$a_i\in\mathbb{F}_\ell$）とする。$\gamma_i$ は相異なるので、
$L$ を十分大きく取れば $n_i:=\gamma_i\bmod\ell^L\in\{0,\dots,\ell^L-1\}$ も相異なる。
補題 J0 より $m<\ell^L$ に対し $\binom{\gamma_i}{m}\bmod\ell$ は $\gamma_i\bmod\ell^L$ だけで決まるから、
$$(1+x)^{\gamma_i}\equiv(1+x)^{n_i}\pmod{x^{\ell^L}}.$$
よって $\sum_ia_i(1+x)^{n_i}\equiv0\pmod{x^{\ell^L}}$。左辺は次数 $\max_in_i\le\ell^L-1<\ell^L$ の
**多項式**なので、恒等的に $0$ である。ところが $(1+x)^{n_i}$ たちの次数 $n_i$ は相異なるので、
最大の $n_i$ をもつ項の $x^{n_i}$ の係数は $a_i$ であり、これが $0$。同じ議論を繰り返して全 $a_i=0$。$\blacksquare$

> **系 J10（$S_\infty$ の決定）.** *$S:=\mathrm{supp}(\bar{\tilde E})\subset\mathbb{N}^2$（$\bar{\tilde E}\neq0$ より $S\neq\emptyset$）とする。
> 原始的な $(a,b)\in\mathbb{Z}_\ell^2$ について*
> $$\theta(a,b)=\infty\iff
> \text{任意の}\ \gamma\in\mathbb{Z}_\ell\ \text{で}\ \sum_{\substack{(p,q)\in S\\ pa+qb=\gamma}}\bar c_{pq}=0. \tag{5.8}$$
> *とくに $\theta(a,b)=\infty$ ならば、相異なる $(p,q),(p',q')\in S$ で $pa+qb=p'a+q'b$ となるものがある。
> したがって*
> $$S_\infty\ \subseteq\ \bigl\{\,(\Delta q:-\Delta p)\ :\ (p,q)\neq(p',q')\in S,\ (\Delta p,\Delta q):=(p-p',q-q')\,\bigr\}, \tag{5.9}$$
> *すなわち $S_\infty$ は**高々 $\binom{|S|}2$ 点の有限集合**であり、その点はすべて
> $\mathbb{P}^1(\mathbb{Q})\subset\mathbb{P}^1(\mathbb{Z}_\ell)$ に属し、候補は $D$ の係数からの有限計算で尽きる。*

**証明.** $\overline{\Phi_{(a,b)}}=\sum_{(p,q)\in S}\bar c_{pq}(1+x)^{pa+qb}$（単元の単項式倍を除く。§3.1 と同じ
ずらしをしてよい）。指数 $\gamma=pa+qb$ の値ごとに項をまとめると、補題 J9 より
異なる $\gamma$ の $(1+x)^\gamma$ たちは一次独立なので、和が $0$ であることと
各 $\gamma$ の類の係数和が $0$ であることは同値。これが $(5.8)$。

$S\neq\emptyset$ だから、少なくとも 1 つの類は空でない。1 元だけの類はその係数 $\bar c_{pq}\neq0$ が
そのまま係数和になるので $(5.8)$ を破る。よって**すべての類が 2 元以上**、とくに
$\Delta p\,a+\Delta q\,b=0$ なる $(\Delta p,\Delta q)\neq(0,0)$（整数ベクトル）が存在する。
$(a,b)$ は原始的なので、この 1 次方程式の $\mathbb{P}^1(\mathbb{Z}_\ell)$ での解は
$(a:b)=(\Delta q:-\Delta p)$ ただ 1 点であり、これは整数ベクトルの定める有理点である。$\blacksquare$

**注 5.5（step 2 の命題 3 との一致）.** $(5.9)$ の候補 $u=(\Delta q,-\Delta p)$ は
$u^\perp=-(\Delta p,\Delta q)\in\mathrm{Newt}(\bar{\tilde E})-\mathrm{Newt}(\bar{\tilde E})$ を意味する。
これは step 2 命題 3 が Ostrowski の定理（Newton 多面体の加法性）から出した候補集合と
**同じ集合**である。両者は独立な証明で同じ有限集合に到達している。
違いは、step 2 が $\mathbb{Z}^2$ のベクトルについて述べているのに対し、
$(5.9)$ は $\mathbb{Z}_\ell^2$ のベクトルについて述べていることである。
**定理 J7 が必要としていたのは後者であり、そこが補題 J9 で埋まった。**

> **系 J10′.** *定理 J7 の仮定 (F) は不要である（$S_\infty$ は常に有限）。
> また (N) の「球が互いに素になる $r_0$」も、有限個の相異なる点なので必ず存在する。
> 残る仮定は (N) の後半（$\mathrm{argmin}$ の一意性）と (B\*) だけである。*

**証明.** 系 J10 より $S_\infty$ は有限。相異なる有限個の点は正の最小距離をもつので、
$\ell^{-r_0}$ をそれより小さく取れば球は互いに素。$\blacksquare$

**機械照合**: 検証 Step H1・H2。母集団 $\times\ \ell\in\{2,3,5,7\}\times$ 原始整数ベクトル
$|a|,|b|\le5$ の全走査で、(H1)「指数がすべて相異なるなら $\theta<\infty$」と
(H2)「$\theta=\infty$ の点は必ず $(5.9)$ の候補集合に入る」を確認した（件数はログ）。

### 5.6 cycle 19 step 2 との突き合わせ（重複と食い違いの処理）

同じ cycle の並行 step である `outputs/reports/cycle19_T3_theta_infinity.md`（以下 step 2）は、
$\theta=\infty$ の場合を正面から扱っている。読んだうえで、重なる主張を次のように整理した。

**(a) 定理 J8 は step 2 定理 X′ の特別な場合である（重複。step 2 が正本）.**
step 2 命題 8・定理 X′ は、族 $p(1,0)+q(0,1)$（bouquet）について
$$\mathrm{ord}_\ell(\kappa_n)=\mu(\ell^{2n}-1)+2n\ell^n+\Lambda(\ell^n-1),\qquad
\Lambda=\begin{cases}2v_\ell(p'+q')&\ell\mid p'+q'\\ v_\ell(p')&\ell\mid p'\\ v_\ell(q')&\ell\mid q'\end{cases}$$
を**すべての奇素数 $\ell$・すべての $(p,q)$** について証明している。
本 report の定理 J8 は $(p,q)=(\ell-1,1)$ の場合であり、$p'+q'=\ell$ から $\Lambda=2$、
$(5.1)$ の右辺は $2n\ell^n+2(\ell^n-1)$ となって $(5.3)$ と**一致する**。
**したがって「奇素数の型 III 族」という結果自体は step 2 の定理 X′ が正本であり、
本 report の $(5.3)$ はその 1 点にすぎない。§0 の表・§8.3 の「奇素数ごとに 1 族」という書き方は
step 2 の一般形に劣る。** 本 report で定理 J8 を残すのは、それが
**定理 J7（$b=\sum_{P\in S_\infty}j^*(P)$）という別の道具から独立に導かれている**ためで、
2 つの step が同じ値に到達していることが相互検証になるからである。

**(b) 食い違いに見えるもの — 「$n\ell^n$ の出所」— は食い違いではない.**
step 2 命題 7 は「例外直線**上の点そのもの**の寄与は $\lambda(\ell^n-1)+n\theta^*$ で、
$n\ell^n$ 項を作らない」と述べ、同 注 5.4 は「$n\ell^n$ は $\theta=\infty$ の点ではなく
**その $\ell$ 進近傍**（$\theta=1+\ell^r$ と深くなる点の集積）から出る」と結論している。
一方、本 report §5.2 の見出しは「$\theta=\infty$ の $\mathbb{Z}_\ell$ 点が $n\ell^n$ 項を生む」である。
**証明の中身を見ると両者は同じことを言っている**: 定理 J7 の証明 (b) は、
$M'\ell^{M'}$ 項を出すのは $v=R'-1,\dots,M'-2$ の層（すなわち $\beta\neq0$ の**近傍**）であり、
$P$ を含む最内球は $\ell^{M'}$ の定数倍と定数しか出さない、と明示している。
$b=\sum_{P\in S_\infty}j^*(P)$ は「$S_\infty$ の点ごとに、**その近傍から** $j^*(P)$ ずつ寄与する」
という数え方であって、点そのものの付値を足しているのではない。
**§5.2 の見出しの言い方（「点が生む」）は誤解を招くので、ここで訂正する。**
正しくは「$n\ell^n$ 項は $S_\infty$ の各点の $\ell$ 進近傍から出て、その係数が $j^*(P)$ で決まる」である。

**(c) 数値で決着させた（検証 Step H3）.** 上の (a)(b) は読み合わせによる整合だが、
食い違いが残っていないことは計算で確かめた。$\ell\in\{3,5,7\}$、$1\le p,q\le2\ell$ で
例外直線をもつ **146 組**（例外直線は延べ 196 本）について、

- 本 report の道具（$\psi_j$, $e_j$, $j^*$）で $S_\infty$ と $b=\sum j^*$ を計算 → **全 146 組で $b=2$**、
- step 2 定理 X′ の閉形式を塔の値（Matrix–Tree 定理による独立計算、フィット 0 個）と照合 → **全一致**、
- 系 J10 の候補集合が step 2 命題 8 の例外直線を含むこと → **全一致**

を確認した。とくに step 2 の場合 [B]（$\ell\mid p'$）では例外直線が **1 本しかない**のに
$b=2$ でなければならず、実際そこでは $j^*=2$ になる（$\bar{\tilde E}=-q'z(w-1)^2$ で
$\psi_1\equiv0$、$e_2=0$）。**2 本 $\times\,j^*{=}1$ と 1 本 $\times\,j^*{=}2$ が同じ $b=2$ に到達するという、
自明でない一致である。**

**(d) 2 つの step は競合ではなく相補である（検証 `j7_outside_family.out`）.**
step 2 §9.1 は「定理 X′ の**形**は族の外へ延長できない」ことを反例で確定させている
（$\ell=3$、bouquet $(1,0),(1,-1),(1,2)$ で実測 $0,10,50$ に対し X′ の素朴な延長は $0,8,44$）。
**その反例の塔で、定理 J7 の $b=\sum_{P\in S_\infty}j^*(P)$ が当たるかを確かめた。**
系 J10 の候補集合から $S_\infty=\{(1{:}0)\}$、そこで $e_j=(\infty,1,0,0,0)$ すなわち $j^*=1$、
よって定理 J7 の予言は $b=1$。他方、塔の値を一切使わずに $\Theta_{M'}$ を
レベルごとに厳密計算すると（$M'=1,\dots,6$ で $12,42,142,478,1594,5266$。
どのレベルでも定理 B′ の最小点は全点で一意なので、これらは下界ではなく値である）、
$\Theta_{M'}=\beta M'\ell^{M'}+c\ell^{M'}+eM'+d$ の 4 係数を**深いレベル $3,4,5,6$ だけ**から解いて
$$\beta=\tfrac23,\quad c=\tfrac{29}9,\quad e=0,\quad d=1,$$
$\beta=b(1-\ell^{-1})=b\cdot\tfrac23$ より **$b=1$。定理 J7 の予言と一致する。**
さらに $e=0$ を課した 3 係数版をレベル $3,4,5$ だけから解くと同じ $\beta$ が出て、
**当てはめに使っていないレベル $6$（予言 $5266$ = 実測 $5266$）とレベル $2$（$42=42$）で当たる。**
レベル $1$ は外れる（予言 $38/3$、実測 $12$）が、これは定理 J7 が
$M'\ge\max(r_0,L_U,n_1)$ を要求していることと整合する。

**まとめると: step 2 の定理 X′ は「族については閉形式の全体」を与え、
本 report の定理 J7 は「一般の塔について $n\ell^n$ の係数だけ」を与える。
族の外では X′ の形は崩れるが、J7 の主要項は崩れない。** これが 2 つの step の関係である。

**注 5.6（何を数値に頼っているか）.** 定理 J7 自体には §5.2 に証明がある。
上の (d) で数値に頼っているのは (i) この塔で定理 J7 の仮定（(N) の $\mathrm{argmin}$ 一意性、(B\*)）が
実際に成り立っていること、(ii) $\Theta_{M'}$ が $\beta M'\ell^{M'}+c\ell^{M'}+eM'+d$ の形に乗ること、
の 2 点である。(ii) は当てはめなので、**当てはめに使っていないレベルで当たったことだけが証拠**であり、
上ではレベル $6$ と $2$ がそれにあたる。
$\ell=5$ の例（bouquet $3\times(1,0),(0,1),(1,2)$、step 2 §9.1 のもう 1 つの反例）も同様で、
$S_\infty=\{(1{:}-1)\}$、$e_j=(\infty,1,1,\infty,0)$ より $j^*=1$、定理 J7 の予言は $b=1$。
$\Theta_{M'}$（$M'=1,\dots,5$ で $18,106,626,3626,20626$）から $e=0$ を課してレベル $2,3,4$ だけで解くと
$\beta=\tfrac45=b(1-5^{-1})$ すなわち $b=1$ で、out-of-sample のレベル $5$（$20626=20626$）と
レベル $1$（$18=18$）の両方で当たる。数値は `j7_outside_family.out`（FAIL 0 件）に全件出す。
**すなわち $b=\sum j^*$ は $b=2$ 以外の値（$b=1$）でも、族の外の塔でも当たっている。**
...
### 7.2 $\{\theta=\infty\}$ の位相 — **解決した**（§5.5・§5.6 を参照）

**当初この節は「$S_\infty$ が有限かは決めていない」と書いていた。これは解決した。**
系 J10 により $S_\infty$ は有限（高々 $\binom{|S|}2$ 点）で、すべて有理点であり、
候補は $\mathrm{supp}(\bar{\tilde E})$ の差ベクトルからの有限計算で尽きる。
したがって定理 J7 の仮定 (F) は不要になり、(N) の前半（球の分離）も自動になる（系 J10′）。
なお $\{\theta=\infty\}$ が閉集合であること（$\theta<\infty$ の点は局所定数性から開近傍を持つ、系 J3）は
そのまま正しく、系 J10 はそれを「有限個の点」まで絞ったことになる。

**残る未解決**: 候補集合 $(5.9)$ の各点で $\theta=\infty$ かどうかを**判定**する手続きは、
$(5.8)$ の形（各 $\gamma$ 類の係数和が消えるか）で有限計算になるが、
本 report ではそれを一般の塔について実装・検証していない
（検証 Step H2 は「$\theta=\infty$ の点は候補集合に入る」という**包含**だけを確かめている。
候補集合の各点での判定は、族については step 2 命題 8 が完全に解いている）。
また、打ち消し（§7.1）と (B\*) の破れは依然として残る障害である。

$\theta=\infty$ の点で $E$ を $\ell$ で割って取り直す段階的処理（$(5.6)$ で 1 段だけ使った）を
一般に何段必要か、という問いは cycle 19 step 2 の対象である。
**step 2 補題 2.2 は「制限した 1 変数多項式の内容で 1 回割るだけで必ず終わる（反復不要）」を
証明しており、この問いは step 2 側で解決済みである。**
...
系 J5 は $\theta(P)\le\ell$ を真に含む。母集団 430 個 $\times\ \ell\in\{3,5,7\}$ の**全走査**
（検証ログ Step G2。ここには時間上限を置いておらず、打ち切りは無い）での内訳は、
判定条件が通る塔のうち **cycle 18 定理 C でも扱えるもの 614 個、本サイクルで新たに扱えるもの 117 個**である。
...
| 型 III の実例 | $\ell=2$ トーラス 1 個 | 同左 | 同左 | 奇素数ごとに 1 族（定理 J8）。**ただしこれは step 2 定理 X′ の特別な場合であり、族としての正本は step 2 側である**（§5.6 (a)） |
| 型 III の原因 | 不明 | 不明 | 不明（$\theta=\infty$ は「第 3 の破れ方」とだけ） | **$S_\infty$（$\theta=\infty$ の $\mathbb{Z}_\ell$ 点、有限）の各点の $\ell$ 進近傍。係数 $b=\sum_{P\in S_\infty}j^*(P)$**（定理 J7・系 J10） |

### 8.4 step 2 との分担（この cycle の中での位置）

同じ cycle の step 2（`cycle19_T3_theta_infinity.md`）は $\theta=\infty$ を正面から扱い、
族 $p(1,0)+q(0,1)$ について閉形式を完全に決めた（定理 X′）。本 step との分担は次のとおり。

| | step 2（$\theta=\infty$） | 本 step（$\theta\ge\ell+1$） |
|---|---|---|
| $\theta$ の定義域 | $\mathbb{Z}^2$ のベクトル（例外直線） | $\mathbb{P}^1(\mathbb{Z}_\ell)$ の点 |
| $S_\infty$ の有限性 | $\mathbb{Z}^2$ の直線として有限（命題 3、Newton 多面体） | $\mathbb{P}^1(\mathbb{Z}_\ell)$ の点として有限（系 J10、補題 J9）。**両者を合わせて完全になる** |
| 型 III の族 | 閉形式の**全体**（定理 X′。族に限る） | $n\ell^n$ の**係数だけ**（定理 J7。族の外でも成り立つ。§5.6 (d)） |
| 型 II の判定 | 扱わない | 系 J5・定理 J6（$\theta$ が有限レベルで止まるか） |
...
**検出力（明示。完走したログ `theta_padic.out` の実数値）**: 母集団は
bouquet 2–3 ループ 210 個と 2 頂点 3 重辺 220 個（voltage 9 種＋自明）の計 **430 個**、
$\ell\in\{2,3,5,7\}$。塔の照合段数と内訳は次の通りで、**予言と実測の食い違い（FAIL）は 0 件**である。
...
$95\%$ の確度で除外できるのは $p\gtrsim3/N$ までである。
段を単位に取れば $N=2128$ で $p\gtrsim0.14\%$、$\ell$ ごとに見れば
$N=374,537,627,590$ でそれぞれ $p\gtrsim0.80\%,0.56\%,0.48\%,0.51\%$ までしか除外できない。
これは「この母集団のこの $\ell$ の範囲で」の話であって、**一般の塔についての主張ではない。**
とくに、母集団の外（$d\ge3$、頂点数の多いグラフ、大きい voltage、大きい $\ell$）で
打ち消しが起きないという主張はしていない。

**$\ell=5$ の打ち切り（隠さず書く）**: $\ell=5$ では $\ell$ ごとの壁時計上限 900 秒に達し、
母集団 430 個のうち**先頭 254 個までしか処理していない**。未処理の **176 個はすべて
2 頂点 3 重辺の族**であり（bouquet 210 個は全て処理済み）、内訳は
「第 1 辺の voltage が $(1,0)$ で第 2 辺以降が列挙の後半に来るもの 11 個」と
「第 1 辺の voltage が $(1,0)$ 以外のもの 165 個」である。
したがって上表の $\ell=5$ の行は、**母集団の 59%（254/430）に対する走査**であって全走査ではない。
影響を受けるのは次の 2 点に限られる。

- **$\ell=5$ での「最小点の一意性が母集団で広く成り立つ」という観察の射程**が、
  2 頂点 3 重辺の族の 4/5 を欠いた範囲に狭まる。$\ell=5$ の tie 0 件は
  「処理した 254 個の中で 0 件」であって、母集団全体で 0 件だとは言えない。
- **定理 K の予言と塔の値の照合**が $\ell=5$ ではその 254 個ぶんに留まる。
  未処理の 176 個については、定理 K が当たるとも外れるとも本 report は述べていない。

打ち切りは定理・命題の**証明**には影響しない（§2–§6 の証明は有限個の例に依らない）。
影響するのは本節（数値支持どまり）の射程だけである。
打ち切りはログ末尾の「打ち切った計算」欄に 1 件として全件記録されている。

**なお §5.6 (c)(d) の突き合わせは数値支持どまりではない。** そこで数値が担っているのは
「証明済みの 2 つの命題（定理 J7 と step 2 定理 X′）が同じ塔で同じ値を出すこと」の確認であり、
どちらの命題も証明を持つ。$\ell\in\{3,5,7\}$・$1\le p,q\le2\ell$ の 146 組は
**宣言した母集団の全走査**であって標本抽出ではない。
ただし §5.6 (d) の $\Theta_{M'}$ の形（$\beta M'\ell^{M'}+c\ell^{M'}+eM'+d$）は当てはめなので、
そこは out-of-sample のレベルで当たったことだけを根拠にしている（注 5.6）。
...
### 11.4 「$\theta=\infty$ の点が $n\ell^n$ を生む」という見出しの言い方が不正確だった

§5.2 の見出しを「$\theta=\infty$ の $\mathbb{Z}_\ell$ 点が $n\ell^n$ 項を生む」と書いたが、
定理 J7 の証明 (b) 自身が示しているとおり、$M'\ell^{M'}$ 項を出すのは
$P$ を含む最内球ではなく **その外側の層（$\beta\neq0$、$\theta$ が $\Lambda(r)$ に沿って深くなる点）**である。
step 2 命題 7・注 5.4 が「$n\ell^n$ の出所は点そのものではなく近傍である」と明示していたのを読んで、
自分の見出しが証明の中身とずれていることに気づいた。§5.6 (b) で訂正した。
**結論の数値（$b=\sum j^*$）は変わらないが、言い方が「点の付値を足している」と読めるのは誤りである。**

### 11.5 $S_\infty$ の有限性を「未解決」として出しかけた

§7.2 は当初「$S_\infty$ が有限かは決めていない」で締めていた。
しかし step 2 命題 3 が $\mathbb{Z}^2$ の側でこれを解いており、
残る隙間（$\mathbb{Z}_\ell$ の無理な方向）は補題 J9 という**この report の §2 で既に証明した
桁安定性（補題 J0・定理 J2）の直接の帰結**で塞がる。
つまり必要な道具は最初から手元にあり、自分の未解決リストが 1 項目分だけ水増しされていた。
**並行 step の成果を読み合わせる前に「未解決」を確定させてはいけない。**

### 11.6 $\theta$ と $\hat\theta$ の混同
...
8. **「定理 J8 の族は $\ell$ ごとにグラフが変わるので、1 個の塔の結果を並べただけではないか」**
   ...
   **さらに、この族は step 2 定理 X′ の $(p,q)=(\ell-1,1)$ の場合にすぎない**（§5.6 (a)）。
   本 report 単独では新しい族を出したように読めてしまうので、そう書かないよう §0・§8.3 を直した。
9. **「定理 J7 と step 2 命題 7 は $n\ell^n$ の出所について反対のことを言っていないか」**
   → 言っていない。両方とも「出所は $S_\infty$ の点の**近傍**」である。
   本 report の見出しの言い方が悪かっただけで、証明の中身は一致している（§5.6 (b)、§11.4）。
10. **「$b=\sum_{P\in S_\infty}j^*(P)$ は、たまたま定理 J8 の族で $b=2$ に合っただけではないか」**
   → 違う。(i) 検証 Step H3 は $\ell\in\{3,5,7\}$ の 146 組で、例外直線が 2 本のとき（$j^*=1$ ずつ）と
   1 本のとき（$j^*=2$）という**構造の異なる 2 通り**が同じ $b=2$ に到達することを確かめている。
   (ii) さらに `j7_outside_family.out` は、**step 2 が定理 X′ の反例として挙げた塔**
   （$\ell=3$、bouquet $(1,0),(1,-1),(1,2)$、$b=1$）で $b=\sum j^*$ が当たることを、
   当てはめに使っていないレベルでの検証つきで示している。$b=2$ 以外の値でも当たっている。
11. **「系 J10 は $S_\infty$ を有限と言うが、$\mathbb{Z}_\ell$ の無理な点を本当に排除できているか」**
   → できている。補題 J9 は $\gamma\in\mathbb{Z}_\ell$（有理とは限らない）について述べており、
   $(5.8)$ から出る条件 $\Delta p\,a+\Delta q\,b=0$ は**整数係数**の 1 次方程式なので、
   $\mathbb{P}^1(\mathbb{Z}_\ell)$ での解は有理点 1 個に限られる。
   有限桁の計算では $\theta=\infty$ を確かめられない（§7.2）が、
   **候補が有限個の有理点に限られること自体は有限計算で分かる**、というのが系 J10 の内容である。
...
- **Step H**: 補題 J9・系 J10（$S_\infty$ の有限性・有理性）の照合と、
  **cycle 19 step 2（定理 X′）との突き合わせ**（§5.5・§5.6）。
  H1・H2 は母集団 $\times\ \ell\in\{2,3,5,7\}\times$ 原始整数ベクトル $|a|,|b|\le5$ の全走査、
  H3 は $\ell\in\{3,5,7\}$・$1\le p,q\le2\ell$ の族 146 組で $b=2$ と定理 X′ の閉形式を照合、
  H4 は「$n\ell^n$ の出所」の内訳を数値で分離する。

別ファイル `j7_outside_family.sage` / `.out`:
**step 2 §9.1 が定理 X′ の反例として挙げた塔**（$\ell=3$ の bouquet $(1,0),(1,-1),(1,2)$ と
$\ell=5$ の bouquet $3\times(1,0),(0,1),(1,2)$）で、定理 J7 の $b=\sum_{P\in S_\infty}j^*(P)$ が
当たることを、当てはめに使っていないレベルでの検証つきで確かめる（§5.6 (d)）。

**Step A–H は証明済み命題の照合である。§9 の 2 件だけが数値支持どまりであり、本文にもそう書いた。**

### 実行結果（完走したログの要約）

`theta_padic.sage`: 所要 **1327.2 秒**で完走。**FAIL 0 件**、打ち切り **1 件**。
`j7_outside_family.sage`: 完走。**FAIL 0 件**、打ち切り **0 件**。
Step ごとの実数値は `RESULTS.md` に、生ログは `theta_padic.out` / `j7_outside_family.out` にある。
...
今回実際に打ち切ったのは 1 件で、**Step D の $\ell=5$**（壁時計上限 900 秒に達し、
母集団 430 個のうち **176 個を未実施**。すべて 2 頂点 3 重辺の族）である。
何が落ちたか・どの主張の射程が狭まるかは **§9.1 に明記した**。他の Step に打ち切りは無い。
