...
### 訂正履歴（後のサイクルが検出した誤り。隠さず残す）

| 日付 | 箇所 | 訂正 | 検出したのは |
|---|---|---|---|
| 2026-08-01（cycle 23 step 1） | §5.3 条件 2 | $M\ge r^\sharp+\max K+1$ → **$M\ge r^\sharp+\max K$**（1 つ強すぎた）。同じ report の §6.1 が自分でこの条件を破っており、**正しいのは §6.1 の側**だった | cycle 22 step 4（Lean で成立条件を型に出した） |
| 2026-08-01（cycle 23 step 1） | 注 4.2 | 「$K$ は上界でよい」の**理由**（$K\to K+1$ で $(5.3)$$(5.4)$ の増分が相殺すること）を書き足した。主張自体は正しい | 同上 |
| 2026-08-01（cycle 23 step 1） | §6.3 | 「$n=1$ から完全に一致する」は定理 G4 の保証範囲（$n\ge M^*-1$、この塔では $n\ge2$）の**外**であることを明記した。また書かれていなかった $e=-1$ の導出を書き足した | 同上 |

**いずれも定理 G1・G2・G3・G4、系 G5・G6 の主張と §7 の照合結果を変えない**
（§5.3 は $M^*$ を小さくする方向の訂正、注 4.2 と §6.3 は理由と射程の書き足しである）。
...
> **補足（cycle 23 step 1 で書き足した。初稿は理由を書いていなかった）.**
> $(5.3)$$(5.4)$ は $-\frac{(\ell-1)j^{*}(K+r^\sharp)}{\ell}$ と $-e_{j^*}\ell^{K}$ という
> **$K$ に直接依存する項**を含むので、「$K$ を大きめに取ってもよい」と言うためには、
> 大きく取り直した $K$ を代入しても値が変わらないことが要る。実際に $K\to K+1$ で確かめる。
> 深さ $K+1$ の層は定理 G3 より非飽和なので $(4.2)$ が使え、
> $\Lambda_{K+1}=j^{*}/\varphi(\ell^{K+1})$、$\theta^\sharp_{K+1}=e_{j^*}$ である。
> - $(5.4)$: 括弧の第 3 項が $\varphi(\ell^{K+1})e_{j^*}$ だけ増え、第 1 項は $-e_{j^*}\ell^{K}$ から
>   $-e_{j^*}\ell^{K+1}$ になる。変化は $e_{j^*}\bigl((\ell^{K+1}-\ell^{K})-\ell^{K+1}+\ell^{K}\bigr)=0$。
> - $(5.3)$: 第 3 項の $\bigl(\Lambda_0+\sum_k\varphi(\ell^k)\Lambda_k\bigr)$ が
>   $\varphi(\ell^{K+1})\cdot\frac{j^{*}}{\varphi(\ell^{K+1})}=j^{*}$ だけ増え、第 2 項は
>   $-\frac{(\ell-1)j^{*}(K+r^\sharp)}{\ell}$ から $-\frac{(\ell-1)j^{*}(K+1+r^\sharp)}{\ell}$ になる。
>   変化は $\frac{\ell-1}{\ell}j^{*}-\frac{\ell-1}{\ell}j^{*}=0$。
>
> **したがって注 4.2 は正しく、その理由は「増分がちょうど相殺すること」である。**
> 一般の $K'\ge K$ への帰納は cycle 22 step 3 の命題 D1a
> （`cycle22_T3_coefficients_d_e.md` §2.3）が書いている。
> **この打ち消しが書かれていないことは cycle 22 step 4（同 report §3.2）が指摘した。**
...
2. $M\ge r^\sharp+\max_{P_0}K(P_0)$（(b) の閉形式が成り立つ）。
3. $\varphi(\ell^{M})>\theta^{\max}_U-2$（(a)）。
4. 各 $P_0$、各 $k\le K$ で $M\ge2k$ かつ $\varphi(\ell^{M})>(\theta^\sharp_k-m^\sharp_k)\varphi(\ell^{k})$（定理 G2 $(3.2)$）。
5. 各 $P_0$ で $e_{j^*}+j^{*}\ell^{\,M-K-1}-2<\varphi(\ell^{M})$（定理 G3、$k=K+1$ の最外層。$M=r^\sharp+K$ ではこの層が無いので条件は空である）。

いずれも $D$ の係数からの有限計算である。

> **訂正（cycle 23 step 1、2026-08-01）.** **条件 2 は初稿では $M\ge r^\sharp+\max_{P_0}K(P_0)+1$ と
> 書かれており、1 つ強すぎた。** (b) が使う等式
> $\sum_{r=r^\sharp}^{M-K-1}\varphi(\ell^{M-r})=\sum_{s=K+1}^{M-r^\sharp}\varphi(\ell^{s})=\ell^{M-r^\sharp}-\ell^{K}$
> が成り立つ条件は $K\le M-r^\sharp$、すなわち $M\ge r^\sharp+K$ である。
> $M=r^\sharp+K$ では (b) の層は**空**だが、両辺とも $0$ なので等式は成り立ち、
> $(2.1)$ も成り立つ（空の層の寄与が $0$ であること自体は障害ではない）。
> 破れるのは $M<r^\sharp+K$ で、そこで初めて右辺が負になる。
> 初稿の括弧内の理由「(b) の層が空でない」も、正しくは「(b) の閉形式が成り立つ」である。
> **この食い違いは cycle 22 step 4（`cycle22_ops_lean_cycle21_theorems.md` §1）が
> Lean（`GeneralTower.sum_totient_Ico` / `layer_b_boundary`）で成立条件を型に出して検出した。**
> 検出の決め手は、**同じ report の §6.1 が $r^\sharp=1$, $K=0$ の下で $M^*=1$ を使っており、
> 初稿の条件 2（$M\ge2$）を自分で破っていた**ことである。正しいのは §6.1 の側であり、
> ここ（§5.3）を直した。$M^*$ は $(2.1)$ の十分条件であって鋭さを主張しないので（§9.2）、
> **定理 G4 の結論も §7 の照合も変わらない**（$M^*$ が小さくなる方向の訂正であり、
> $M\ge M^*$ で照合した §7 の全件はそのまま有効である）。
...
> **注（cycle 23 step 1）.** ここで使っている $M^*=1$ は $r^\sharp+K=1+0=1$ であり、
> **訂正後の §5.3 条件 2（$M\ge r^\sharp+\max K$）を満たす。**
> 訂正前の条件 2（$+1$ つき）はこれを満たさず、同じ report の中で矛盾していた。
> 正しいのはこちら（§6.1）の側である（§5.3 の訂正注を参照）。
...
> **訂正（cycle 23 step 1、2026-08-01）— 2 点。**
>
> **(i) 「$n=1$ から一致する」は定理 G4 の保証範囲の外である。** 定理 G1 の結論が保証されるのは
> $n\ge M^*-1$ であり、この塔では $r^\sharp=2$、$K=1$ なので §5.3 条件 2（訂正後）から
> $M^*\ge3$、したがって**保証範囲は $n\ge2$ である**（他の条件 1・3–5 もこの塔では $M=3$ で満たされるので
> $M^*=3$）。$n=1$ での一致は**保証の外で起きた一致**であって、定理 G4 の検証としては
> 「保証範囲の外でも一致した」と書くのが正しい。一致すること自体は事実である。
> これは cycle 21 step 3 が系 Y″ について検出した「保証範囲外の小さい $n$ の一致・不一致を根拠に使う」
> のと同型の問題であり、**cycle 22 step 4（`cycle22_ops_lean_cycle21_theorems.md` §5）が指摘した。**
>
> **(ii) 初稿は $e=-1$ をどう出したかを書いていなかった。** 書いておく。$M^*=3$ なので
> $(2.3)$ の角括弧は $0$ ではない。角括弧は cycle 22 step 3 の定理 D2 の**過渡欠損**
> $T_{\mathrm{def}}=\sum_{M\ge1}\bigl(\Theta_M-(\alpha M\ell^M+\beta\ell^M+\gamma)\bigr)$ に等しく、
> この塔では $T_{\mathrm{def}}=3$（`cycle22_T3_coefficients_d_e.md` §5.1・検証 Step F）である。
> また 1 頂点 bouquet なので $\kappa(X)=1$、$v_2(\kappa(X))=0$、$\mu=0$。よって
> $$e=v_2(\kappa(X))-\mu+T_{\mathrm{def}}+\frac{\ell}{(\ell-1)^2}\alpha-\frac{\ell}{\ell-1}\beta
> =0-0+3+(2\cdot1-2\cdot3)=-1 .$$
> 同じ値は定理 D2 の $e=v_\ell(\kappa(X))-a-c+T_{\mathrm{def}}=0-0-4+3=-1$ からも出る。
