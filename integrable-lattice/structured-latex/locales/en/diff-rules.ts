 ...
 * この形なら必ず違反として出る（`tools/verify-localization-detection-test.ts` が再現データで確かめている）。
 */
...
export type DiffRule =
  ...
  /**
   * 英語版にだけ `cite` ノードが増えている。日本語版は書誌を地の文に書く方針で
   * `cite` を 1 件も使わない（日本語版の生成器は `cite` を出力できず例外を投げる）。
   * **`cite` ノードにしか適用しない**ので、これを口実に数式・参照・画像を増減できない。
   */
  | "citation-added"
  /**
   * 英訳で段落の切り方・語順が変わり、骨格ノードの**入れ子と並び**が原文と違う
   * （英語では 1 文が 2 段落へ割れる、数式の前後が入れ替わる、等）。
   *
   * 骨格ノードを平らにした**多重集合**が一致することを実際に検査するので、
   * 「組み替えた」を口実にノードを落とす／足すことはできない。
   * これは移行前の自前検査が持っていた強さ（数式の多重集合一致）と同じ水準であり、
   * **位置の一致まで見るシステム既定より弱い**。弱めた事実をここに明示しておく。
   */
  | "translation-reflowed"
  /**
   * 同じ数式が、原文では行内（`math`）、英語版では別行立て（`displayMath`）になっている
   * （その逆も同じ）。**tex は他の規則の下で一致することを別に検査する**ので、
   * これを口実に数式の中身を変えることはできない。
   *
   * 移行前の自前検査は数式を「tex の多重集合」でしか見ておらず、この差を検出できなかった。
   * システムの構造照合へ載せ替えたことで初めて見えた差である。
   */
  | "math-display-mode-changed";
