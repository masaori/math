...
// cycle 26 step 4: 条件文が 1 文も取れなかった passage は台帳の `covers` を照合対象へ回す。
// **黙って回すのではなく件数を毎回出す**（回っていること自体は正常だが、
// 「report 本文ではなく台帳の要約と照合している」ことは読む側が知っているべきである）。
const fallbacks = coverage.reduce((n, r) => n + r.coversFallbacks, 0);
console.log(
  `  うち **台帳の \`covers\` へ回した passage ${fallbacks} 件**` +
    "（条件文が 1 文も取れなかったもの。cycle 26 step 4 でこの仕組みを入れるまで、" +
    "これらは何も照合していなかった）",
);
