...
export const FORMALIZATION_COVERAGE: readonly CoverageEntry[] = [
  ...
  { block: "paper_061_theorem_V", state: "完了", note: "命題 V は $d=1$ と $d=2$。" },
  {
    ...
    remaining:
      ...
      "**cycle 49 step 1 で全数の突き合わせをすると、この欄が数えていない段が見つかった。そう書く**——" +
      "`PropTHenselLift.lean` が挙げている 段 4（Newton 多角形）の $v(m_j)=1$ そのもの である。" +
      "`PropT.lean` は段 4 の組合せ核を書いているが、$v(m_j)=1$ は外から来る事実として" +
      "仮定の形で受け取っており、その事実自体は書かれていない。" +
      "**`lean/` 側は cycle 42 から挙げていたのに、欄は 1 度も数えていなかった。**" +
      "**step 1 は同じ箇条書きの前半（剰余体の側）も数えていない段と読んだが、" +
      "step 3 で開けると二重に数えていたことが分かった。そう書く**——" +
      "剰余体の側そのものは cycle 44 step 2 で入っており" +
      "（`PropTResidueRoot.isPrimitiveRoot_residue_of_odd`。剰余標数 $2$ の Hensel 的局所環で、" +
      "$L$ が奇なら原始 $L$ 乗根の像がまた原始 $L$ 乗根である）、" +
      "残っていたのは舞台の同定だけで、それは (1) ではなく 付値の位相の項目と同じものである。" +
      "**step 1 の 4 件の見立てのうち、1 件はこの二重計上だった。開けるまで分からなかった。そう書く。**" +
      "**同 step 3 で舞台そのものを $\\mathfrak m$ 進完備化の側から書き下した**" +
      "（`PropTAdicStage.lean`。Noether 局所環の $\\mathfrak m$ 進完備化は Hensel 的な局所環であり" +
      "〔mathlib の局所環の完備化の章にある instance と cycle 43 step 3 の橋の引き合わせ〕、" +
      "その剰余体は元の環の剰余体と同型で〔`residueFieldEquiv`〕、" +
      "**cycle 42 step 2 の持ち上げと cycle 44 step 2 の根の側が、仮定なしでそこへ当たる**" +
      "〔`exists_root_quadratic_adicCompletion`〕）。" +
      "**したがって残る形が変わった。残っているのは、本文の付値による完備化がこの $\\mathfrak m$ 進完備化と" +
      "同じものであることである。** cycle 48 が測った 付値の位相が $\\mathfrak m$ 進位相であること は" +
      "その同定の中身であり、測定そのものは正しい（2026-08-05 に測り直した——" +
      "`lean/logs/mathlib-gap-survey-cycle49-adic-stage.log`。付値の位相が $\\mathfrak m$ 進位相であることを" +
      "述べた宣言は 3 段とも 0 件、非アルキメデス的局所体のクラスは定義ファイルの外に 0 件である）。" +
      "**変わったのは、その 1 本を渡した先に何が待っているかである——渡した先はもう空ではない。**" +
      "**したがってこの主張の残りは 2 つである。**" +
      "**cycle 50 step 3 でそのうち 1 つを取った。段 4（Newton 多角形）の $v(m_j)=1$ そのものである。そう書く**" +
      "（`PropTNewtonStage.lean`）。**Newton 多角形を組み立てる必要は無かった。ここが本 step の主題である**——" +
      "本文の 2 次方程式 $\\zeta^{j}m^{2}+(3\\zeta^{j}+\\zeta^{-j}-4)m-2(1-\\zeta^{j})(1-\\zeta^{-j})=0$ の 3 係数は、" +
      "$z=\\zeta^{j}$ と置くと $z$・$z^{-1}(z-1)(3z-1)$・$2z^{-1}(z-1)^{2}$ と因数分解でき、" +
      "**剰余体の標数が 2 で $z-1$ が単元なら、はじめの 2 つは単元で 3 つ目はちょうど 2 の単元倍である**" +
      "（本文が「3 係数の付値は $(0,0,1)$」と書いていることの中身がこれである）。" +
      "段 3 の Hensel 持ち上げが $r\\equiv\\zeta^{j}$ を与えるので $m$ は極大イデアルに属し、" +
      "**方程式を $m(a_2m+a_1)=-a_0$ と読み替えるだけで $m$ が 2 の単元倍であることが出る**" +
      "（$a_2m+a_1$ は極大イデアルの元と単元の和なので単元である。`eq_two_mul_unit_of_root`）。" +
      "**2 根の付値の対を作る必要も、付値写像を導入する必要も無い。**" +
      "併せて二項展開の側（$r^{L}-1=m\\cdot$単元。$L$ 奇が効く）も書き、" +
      "$r^{L}-1$ が 2 の単元倍であるところまで繋いだ（`pow_sub_one_eq_two_mul_unit_of_root`）。" +
      "**$\\mathbb{R}$ へも $\\overline{\\mathbb{Q}}$ へも 1 度も出ない**（可換局所環の単元と極大イデアルの計算だけである）。" +
      "**したがってこの主張の残りは 1 つになった。件数は動いていない。そう書く**——" +
      "残っているのは 付値の位相が $\\mathfrak m$ 進位相であること の 1 本である。" +
      "**$v(2)=1$ を読むには 2 が素元である舞台が要るが、それはその 1 本の内側である**" +
      "（舞台そのものは `PropTMixedWitness.lean` が混標数で与えている）。",
    remainingItems: [
  ...
  },
  {
    ...
    remaining:
      ...
      "**したがって残りは 3 つである。**" +
      "**cycle 48 step 4 で積公式の段を測った。2 つの道具だけでは組み立たない。そう書く**" +
      "（2026-08-05 実測、`lean/logs/mathlib-gap-survey-cycle48-productformula.log`。" +
      "走査 script は `lean/scripts/mathlib-gap-survey-cycle48-productformula.sh`）。" +
      "**足りないものが 1 つ実測で出た**——本文の $(★_2)$ の左辺は塔の全域木数 $\\kappa(X_{N,N^{\\prime}})$、" +
      "右辺は指標ごとの行列式の積である。指標分解が与えるのはラプラシアン全体の行列式であって、" +
      "**全域木数はその「1 行 1 列を落とした余因子」の側なので、両者を繋ぐ段が別に要る**" +
      "（非零固有値の積を頂点数で割る形。同じことだが、まず全余因子が等しいことが要る）。" +
      "**それは mathlib にも本プロジェクトにも無い**——mathlib はラプラシアンの余因子についても" +
      "全域木数の固有値形についても被覆グラフの積公式についても 3 段とも 0 件で、" +
      "本プロジェクトの `adjugate` を含む宣言は Jacobi の公式と Newton の初期値の側だけである。" +
      "**ただし土台は在る**——ラプラシアンの各行の和が $0$ であることは cycle 30 step 2 で書いてある" +
      "（`MultigraphLaplacian.lapMatrix_row_sum`。cycle 30 の時点で" +
      "「全余因子が等しいことの土台になる」と書かれている）。" +
      "**したがってこの段は「在る道具を組み立てるだけ」ではない。3 つ目の道具が要る。そう書く。**" +
      "**この step は測っただけで 1 件も形式化していない。残り項目は 3 件のまま、中身が具体化した**——" +
      "1 つ目は 積公式の段（全域木数と行列式を繋ぐ余因子の側が要る） である。" +
      "**cycle 49 step 4 でその 3 つ目の道具に着手した。着手して測ると、この段は 2 つに割れた。そう書く**" +
      "（`AllCofactorsEqual.lean`）——**代数の側と、連結性の側である。**" +
      "**代数の側は書いた**——行の和が $0$ なら定数ベクトルが核に入るので $\\det M=0$ であり、" +
      "そこから $M\\operatorname{adj}M=0$ と $\\operatorname{adj}M\\,M=0$ が出て、" +
      "余因子行列の各列が核に、各行が左核に入る。" +
      "**したがって核と左核が定数ベクトルだけなら、余因子行列は定数行列になる**" +
      "（`adjugate_const_of_kernel_const`。すなわち どの 1 行 1 列を落としても余因子が等しい）。" +
      "**ここまで連結性を 1 度も使っていない。**係数は可換環のままで、体も整域も使わない" +
      "（$\\mathbb{R}$ へも $\\overline{\\mathbb{Q}}$ へも出ない）。" +
      "併せて、ラプラシアンが対称であることと列の和も $0$ であることを書いた" +
      "（cycle 30 step 2 の行の和から出る。あちらが「全余因子が等しいことの土台になる」と書いていた当のものである）。" +
      "**残っているのは連結性の側である**——核が実際に定数ベクトルだけであること、" +
      "すなわち階数が $|V|-1$ であることで、グラフの連結性から出る" +
      "（代数の側は仮定として受け取っており、それがこの 積公式の段 に残っている中身である）。" +
      "**したがってこの残り項目は閉じない。中身が狭まっただけである。そう書く。**" +
      "**cycle 50 step 1 でその連結性の側を書いた。3 つ目の道具は 2 つの側とも入った。そう書く**" +
      "（`LaplacianKernelConnected.lean`）。**階数を経由しない。中身は平方和ひとつである**——" +
      "$L=D\\,D^{\\mathsf T}$ と書けているので $Lx=0$ から $x^{\\mathsf T}Lx=0$ が出て、" +
      "左辺は $D^{\\mathsf T}x$ の成分の平方和である。整数の平方は負にならないので各項が $0$、" +
      "すなわち各辺の両端で $x$ が等しく、到達可能性の鎖に沿って運べば $x$ は定数である" +
      "（`ker_eq_const_of_reachOn`）。**「階数が $|V|-1$」という言い方は同じことを述べているが、" +
      "そこを経由すると係数を体に取る必要が出る。平方和で書けば係数は $\\mathbb{Z}$ のままで済む。**" +
      "**$\\mathbb{R}$ へも $\\overline{\\mathbb{Q}}$ へも 1 度も出ない。**" +
      "左核も同じである（ラプラシアンは対称なので核に一致する。`leftKer_eq_const_of_reachOn`）。" +
      "**これで cycle 49 の代数の側が仮定として受け取っていたものが埋まり、" +
