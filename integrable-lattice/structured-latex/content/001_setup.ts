...
export default defineBlocks([
  ...
  },
  {
    ...
    statement: [
      ...
      displayMath(String.raw`a_L := \prod_{z_1^{L}=\dots=z_d^{L}=1} P(z_1,\dots,z_d) \in \mathbb{Z}`),
      paragraph([
        "と定める（",
        math(String.raw`L`),
        " 等分点すべてにわたる積）。零点を含む場合に備えて、零点となる因子を除いた",
        "「簡約周期点数」も併せて用いる:",
      ]),
      displayMath(
  ...
  },
  {
    ...
    statement: [
      ...
      displayMath(String.raw`\Phi_L := \log|a_L| = \sum_p v_p(a_L)\, \ell_p \in \Lambda`),
      paragraph([
        "と定める。ここで ",
        math(String.raw`v_p`),
        " は ",
  ...
  },
  {
    ...
    statement: [
      paragraph([
        ref("def_massieu_phi"),
        " の係数 ",
        math(String.raw`v_p(a_L) \in \mathbb{Z}_{\geq 0}`),
        " は、任意の素数 ",
        math(String.raw`p`),
        " と任意の ",
        math(String.raw`L`),
