...
export default defineBlocks([
  {
    ...
    kind: "heading",
    level: 1,
    sourcePath: "structured-latex/content/001_intro.ts",
    sourceOrdinal: 1,
    title: { text: "序論 — 整数スペクトル曲線の二素点と、Λ 側の決定可能性" },
    labels: [],
  ...
  },
  {
    ...
    sourcePath: "structured-latex/content/001_intro.ts",
    sourceOrdinal: 2,
    title: { text: "本論文の位置づけ（最初に明示する）" },
    labels: ["paper_positioning"],
    habitat: "mixed",
    realEscape:
      "本ブロックは位置づけの宣言であり、ℝ は「Lehmer 問題が ℝ 側に固有である」という寄与 (d) の " +
      "記述の中で名指しとして現れるだけである。ℝ の元を構成したり ℝ 上の議論をしたりはしていない。" +
      "本論文が実際に ℝ を使うのは第 3 章の L → ∞ の極限ただ一点である。",
    statement: [
      paragraph([
        "本論文は**既知数学の再框（reframe）**である。新しい定理・新しい厳密解・新しい深い数論を",
        "一切主張しない。扱う数学的内容はすべて既知であり、出典を第 3 章と第 5 章で文献名・",
        "命題番号つきに特定する。",
      ]),
      paragraph([
        "本論文が与えるのは次の 4 点だけである。",
      ]),
      list([
        [
          "**(a) 二素点の辞書**: 同一の整数スペクトル曲線 ",
          math(String.raw`P`),
          " について、アルキメデス素点での量（自由エネルギー密度＝Mahler 測度）と",
          "各 ",
          math(String.raw`p`),
          " 進素点での量（有限サイズ Massieu 自由エントロピーの素因数構造）を並置し、",
          "統計力学の量へ対応づける明示的な辞書。",
        ],
        [
          "**(b) 可算化の精密化**: ",
        ...
        [
          "**(c) 決定可能命題群の確定**: ",
          math(String.raw`\Lambda`),
          " 側の有限・初等・決定可能な顔を、具体的な決定手続きと witness をもつ命題として確定する",
          "（第 4 章の命題 A・B・C・N・L と、第 6 章の命題 T・V・W）。",
        ],
        [
          "**(d) 決定可能性非対称の地図**: Lehmer 問題が ",
          math(String.raw`\mathbb{R}`),
          " 側に固有であり、",
          math(String.raw`\Lambda`),
          " 側には対応する未解決の連続ギャップが存在しないことの整理（第 7 章）。",
        ],
      ]),
      paragraph([
...
\ \subset\ \underbrace{\mathbb{R}/\mathbb{C}}_{\text{決定不能}}`,
      ...
      paragraph([
        math(String.raw`\Lambda`),
        " での**等号は素因数分解の一致**、**順序は指数ベクトルの整数比較**であり、どちらも有限手続きで",
        "決定できる。",
