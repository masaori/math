/**
 * 本プロジェクトが使う構造化テキストスキーマの**具体化**。
 *
 * **入力言語（ブロック・ノード・ラベル・ノートの語彙）の定義はここには無い。**
 * 正本はリポジトリ直下の `structured-latex/`（システム）が 1 つだけ持つ。
 * このファイルがやるのは次の 3 つだけである（システム README「使う側がやること」）。
 *
 ...
 *
 * 以前はここに Ising 版から複製した入力言語の定義そのものが置かれていた。同じ言語が
 * 複数箇所で定義される状態を解消するのがこの移行の目的である。
 *
 * 型で捕まえること（コンパイル時）:
 *   - 存在しないラベルへの `ref()` / ノートの `targets`、未登録ラベルの宣言（システム）
 *   - id・ラベル・ノート id の重複、kind ごとに許されるフィールド、`targets` の空配列（システム）
 ...
 *   - 住処と `realEscape` の対応（型を迂回した値のための保険。`assertHabitation`）
 */
