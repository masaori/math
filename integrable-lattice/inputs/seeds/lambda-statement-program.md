...
- **(iv) 単一の説明可能なずれ**: 既知アンカーから 1 つの軸だけ動かした形。軸は次の6本とする。
  - **対象軸**（既知結果そのものを動かす）: 境界・rank・模型・複雑性・可解性。
  - **メタ軸**（対象は動かさず、**どの集合の上でどの手続きで語るか**を動かす）: 可算化・決定可能性・形式検証可能性。
    T1 Reframe（`docs/themes.md` の本流。「既知の再框」を first-class 成果とする）は定義上このメタ軸だけを
    動かすトラックなので、メタ軸を軸として認めないと T1 の成果が (iv) を満たせなくなる。
    メタ軸を動かした statement は、**対象軸を 1 本も動かしていないこと**（＝数学的内容は既知のまま）を
    明示すること。これは新規性の主張を制限する条件であって、緩和ではない。
  （2026-07-26 追記: cycle 12 step 1 の paper-plan 002 で、メタ軸が軸リストに無く (iv) の1本判定が
  文言どおり適用できないことが判明したため、`docs/themes.md` の T1 の位置づけに合わせて明文化した。）
