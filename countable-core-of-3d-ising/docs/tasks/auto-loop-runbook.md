...
2026-09-05の停止解消指示と既存の可算コア同定の目的に基づき、
[next-research-target.md](next-research-target.md)の固定候補の適合性・採否・残る不足を評価した。
本文の自由境界分配多項式を対象とし、READMEに残る外側固定境界と混同しない。
両者の位置づけを最初に本文と方針文書へ照合し、定義の一致を確認してから式を使う。
これは候補評価の実行の採用であり、候補不等式や第二点の存在の採用ではない。

台帳の「現在の研究対象」の一件だけを進める。この対象を閉じるまで下記の過去の並行ストリームを
再開せず、cityの対分けや一般列の必要条件を追加しない。候補が不適合または不足なら、
入力・判定根拠・残る条件を記録して候補を閉じる。補題や点を自動で追加しない。
全項目は閉じており、次の対象は共通監督と所有repoの評価へ戻した。
新しい対象が採用されるまでは、終了済み候補の再レビューを台帳やMEMORYへ追記せず、
本文・検算・PDFの全体検査も繰り返さない。定期起動の実行結果は外側の実ログだけに残す。
過去の停止文書は auto-loop-paused.md に履歴として保存する。
元発言とエージェントが加えた許可待ちの区別は共通の研究停止の元指示と復旧条件を参照する。


状態は [auto-loop-state.md](auto-loop-state.md) に保存する。判断の根拠はこれらの手順・状態と、
リポジトリ内の一次情報（docs/context/、この研究のREADME、可算コア同定の方針文書）とする。
...
3. **対象の有無を判定**: 「現在の研究対象」に未完了項目がなければ、終了済み候補を再評価せず、
   ファイル変更・検査・コミットを行わず終了する。新しい候補や補題を自動採用しない。
4. **レビューと修正**（未完了対象がある tick では必須。下の「レビュー観点」）。
   直したものがあれば、その時点でコミットし、**その場で push まで済ませる**。
   前進の作業に入る前に `origin/main` へ入れること。
5. **前進**: `auto-loop-state.md` の**最初の未完了セクション**（`done` でないもの）を 1 つ選び、
...
6. **検証**（下の「検証コマンド」）。すべて通るまで直す。落ちたら本文を直す
   （検証を主張に合わせて緩めない）。
7. **台帳と MEMORY の更新**: セクションの四層の状態、観察、日付を書く。
   **あわせて `進捗の要約.md` を更新する**（下の「進捗の要約を更新する」）。
8. **main へ push**し、`git merge-base --is-ancestor <commit> origin/main` で反映を確認する。
9. **未完了対象を進めた tick で PDF を作り直す**（`cd structured-latex && npm run build:pdf`）。
   未完了対象が無く手順 3 で終了する tick では行わない。
...
10. **並行ストリームを 1 件進める**（下の「二本立てで進める」）。本流とは別枠なので、
   「1 tick 1 セクション」の上限には数えない。締切に当たったらここは飛ばしてよい。
11. **止まる。** 次のセクションへ進まない。次の tick を待つ。
...
固定候補の評価は終了した。採否と残る不足は同文書の評価結果を参照する。
第二の有理点の極限存在は未解決であり、次の対象選定は共通監督と所有repoへ戻す。
未完了の対象がない tick では、冒頭と実行手順の規則どおり、終了済み候補を再評価せず、
台帳・MEMORY・本文・検算を変更せず、全体検査・PDF再生成・コミットも行わない。
定期起動の成否は外側の実ログだけへ残し、閉じた評価の再実行や小主張・別候補の追加を行わない。
