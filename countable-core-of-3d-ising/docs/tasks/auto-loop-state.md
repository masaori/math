...
- 2026-08-20 03:04: 交差べき等式から極限量へ渡す最終束ね定理を、モノイド上の対象二元でのべき単射性と位相空間上の収束移送だけへ抽象化した Lean 必要十分版を閉じた。
