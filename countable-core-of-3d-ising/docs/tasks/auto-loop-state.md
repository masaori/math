...
| **【本流】最小性: 有限箱値の交差べき等式は極限量に対して十分である** | ゴール文書の自由エネルギー密度についての最小性から 2026-08-19 14:34 に引き直した。二つの正有理数列 $A_L,B_L$ と正の自然数列 $N_L,M_L$ に対し、各箱で $A_L^{M_L}=B_L^{N_L}$ なら正の乗根 $A_L^{1/N_L}=B_L^{1/M_L}$ が一致し、したがって一方の箱サイズ極限が存在すれば他方も存在して一致することを示す。交差べき等式は有限整数算術で決定可能であり、値そのものの尾部一致より粗い。2026-08-19 15:33 に先頭の代数的主張を記述した。$x=A^{1/N},y=B^{1/M}$ と置き、$x^{NM}=A^M=B^N=y^{NM}$ と正の自然数乗の単射性から $x=y$ を得る。次に箱サイズ極限へ渡す十分性の主張へ分ける。2026-08-19 16:03 に先頭の主張の SageMath 検証を通した（check `cross-power-equality-implies-root-equality/`：乗根が正有理数に取れる三例で証明の各行を `QQ` で厳密比較、正の自然数乗の単射性の有限標本、負例の検出。linkage 43 件）。2026-08-19 16:32 に Lean 具体版 `cross_power_equality_implies_posRoot_equality` を本文と同じ六段の計算と正の自然数乗の単射性で形式化した。2026-08-19 17:03 に Lean 必要十分版を二つへ割った：先頭は「べき乗が単射になる順序構造だけを仮定に残した抽象版 `cross_power_equality_implies_root_equality_abstract` を置けるか判断して形式化（既存の必要十分版の再利用可否も判断）」、次に「具体版をその特殊化として導出し status を上げる」。2026-08-19 17:05 に割った先頭を閉じた：抽象版 `cross_power_equality_implies_root_equality_abstract` をモノイドと `N*M` 乗写像の単射性だけで形式化（sorry 検査 249 件）。 | Lean 具体版まで（次は具体版を抽象版の特殊化として導出し status を上げる） |
