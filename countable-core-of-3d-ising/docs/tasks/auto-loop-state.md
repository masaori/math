...
| **【本流】最小性: 有限箱量の列の有限接頭部を忘れる粗視化は極限量に対して十分である** | ゴール文書「極限側で問う言明」の「自由エネルギー密度を決める十分な最小の層はどこか」から 2026-08-18 18:02 に引き直した。二つの有限箱量の列が、ある $N\in\mathbb N$ 以降のすべての添字で項ごとに一致することを「有限接頭部を忘れた値の一致」と定め、その一致から両列の箱の大きさの極限が一致することを示す。これは直前の末尾ずらし反例が示した「有限個の箱の値は必要でない」を、十分な粗視化の候補（尾部同値類）として正面から書くものである。2026-08-18 18:35 に先頭を二つへ割った：先頭の「定義ブロックの記述——有限箱の列の組に対する尾部同値関係（ある $N\in\mathbb N_{\ge1}$ が存在して、$N$ 以上のすべての添字で項ごとに一致する）を $\mathbb N\times\Lambda$ 値の列の上の同値関係として定義する」は 18:36 に `def_tail_equivalence_of_finite_box_sequences` として記述済み。残りは「十分性の主張ブロックの記述——尾部同値な二列は、一方の極限量が存在すれば他方も存在して一致する（既存の末尾ずらしの極限一意性の論法を一般の $N$ ずらしへ広げて引く）」。 | 記述の途中（定義まで） |
