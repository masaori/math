...
| **【本流】潰れる候補: Galois 群は極限量に効かないか** | ずらした自由族 $Z'_L:=Z_{L+1}$ を使う。極限量の一致は既存の末尾ずらし定理から従うので、ある $L$ で $Z_L$ と $Z_{L+1}$ の分解体上の Galois 群が同型でないことを有限の厳密計算で示す。帰無モデルの超八面体群上限は再発見として数えず、群の同型型そのものを比較する。$L=2$（$Z_2$ 対 $Z'_2=Z_3$）で、$Z_2$ の分解体の群は位数 4、$Z_3$ は既約 40 次因子を持つので群の位数が 40 の倍数となり、非同型が有限判定できる。主張 `claim_shifted_free_family_galois_group_does_not_determine_limit_quantity` を記述済み。SageMath check `galois-group-shifted-free-family-nonisomorphic/` を 2026-08-18 08:36 に通した（$Z_2=2(x+1)^4(x^2+1)^2(x^4-4x^3+8x^2-4x+1)$、根基の分解体は次数 4 で群は位数 4・可換・非巡回、$Z_3=c(x+1)^{14}g$ で $g$ 既約 40 次、$40\nmid4$ の判定。linkage 37 件）。Lean 具体版は「有限位数比較」と「推移的作用・極限一致との束ね」に割り、先頭 `no_equiv_of_card_four_of_forty_dvd_card` を 2026-08-18 09:04 に形式化した。残りの「推移的作用・極限一致との束ね」を 2026-08-18 09:35 にさらに二つへ割った：先頭は「既約 40 次因子から位数の可除性を得る補題（mathlib の Galois 理論で、既約・分離的な $g\in\mathbb Q[X]$ の分解体の Galois 群が $g$ の根の集合へ推移的に作用することから $\deg g\mid\#\mathrm{Gal}(g)$ を出す。使える定理は軌道・固定部分群の数え上げ `MulAction.card_orbit_dvd_card`／`Polynomial.Gal.galAction` 系を候補として同定する。標準の道具で閉じられない場合は可除性 $40\mid\#G_3$ を仮定に置く形へ後退し、その判断と理由を台帳へ書く）」、次に「位数比較 `no_equiv_of_card_four_of_forty_dvd_card` と既存の末尾ずらし極限定理（`TailShiftLimit` 系）を束ねて主張全体の Lean 具体版を閉じる」。 | Lean 具体版の途中（2026-08-18 09:04。09:35 に残りを二つへ割った） |
