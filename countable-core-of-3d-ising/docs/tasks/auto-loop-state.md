...
| **【本流】潰れる候補: 判別式は極限量に効かない、を判定できる形に定義する** | ゴール文書「極限側で問う言明」の未検討項（判別式・Galois 群は極限量に効くか）と「最初の三手」の三手目から引き直した標的（2026-08-17 17:02）。割った先頭は定義だけ：$q$ に依らない列 $L\mapsto\mathrm{disc}(Z_L)\in\mathbb Z$（判別式は整係数多項式の係数の整数多項式なので可算側）を粗視化として置き、既存の極限量 $\alpha(q)$ に対する「十分」「必要でない」の判定が何を意味するかを本文の定義ブロックに書く（候補：二つの有理点 $q,q'$ ではなく、同じ $q$ で判別式の列が異なる二つの多項式族を比べる枠が要るなら、その族の取り方を定義に含める）。判定できる形にならなければ、その旨を台帳へ書いて次の候補（Galois 群）へ移る。**2026-08-17 17:30 に先頭の定義を記述**（`def_constant_coarse_graining_from_q_independent_invariant`。有理点を動かす枠では判定が退化することを示し、判定枠を「二つの分配多項式の族の組」へ置き直した）。**2026-08-17 18:00 に残りを二つに割った**：自由族と周期族の判別式差を有限計算で閉じる部分と、極限量の一致を健全性の橋へ落とす部分。 | 記述と SageMath まで（定義と $L=3$ の自由・周期判別式差）。極限量の一致は未完了。**2026-08-17 21:30** に周期族との比較は極限一致が未証明、ずらした自由族は $\mathrm{disc}(Z_4)$ が要ると判明。**2026-08-18 00:55** に後者の計算を「密行列を作らない自由境界の整数点評価核」へ割り、Kronecker 積の butterfly による高速層転送を実装。$L=2,3$・$x=0,1,2$ で既存の厳密多項式と一致。**2026-08-18 01:35 に $Z_4$ の係数復元をさらに二つへ割った**：(先頭) 法素数 $p$ 上で butterfly 核により整数点 $\#E_4+1=145$ 点（$\#E_4=3\cdot4^2\cdot3=144$ が次数上界）を評価し Lagrange 補間で $Z_4 \bmod p$ を復元、次数 $144$ が保たれることと $\gcd(Z_4,Z_4')=1 \pmod p$ を確認する check。ある一つの素数 $p$ で square-free かつ次数が保たれれば $\mathrm{disc}(Z_4)\not\equiv0 \pmod p$、したがって $\mathrm{disc}(Z_4)\ne0$ が $\mathbb Z$ で従う（判別式の非零性はこの一つの素数で閉じる。$\mathbb Z$ 上の全係数復元は不要）。(次) その結果を本文の主張ブロック（ずらした自由族 $Z'_L:=Z_{L+1}$ が判定枠の反例になる）として書き、check の対象ラベルを付け替える。 |
