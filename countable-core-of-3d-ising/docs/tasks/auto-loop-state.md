...
| **【本流】潰れる候補: Galois 群は極限量に効かないか** | ずらした自由族 $Z'_L:=Z_{L+1}$ を使う。極限量の一致は既存の末尾ずらし定理から従うので、ある $L$ で $Z_L$ と $Z_{L+1}$ の分解体上の Galois 群が同型でないことを有限の厳密計算で示す。帰無モデルの超八面体群上限は再発見として数えず、群の同型型そのものを比較する。先頭は $Z_3,Z_4$ の厳密データと SageMath で利用できる Galois 群決定の条件を照合し、実行可能な最初の箱の組を固定する。**2026-08-18 07:36 に照合を完了し、組を $L=2$（$Z_2$ 対 $Z'_2=Z_3$）へ固定した**：$Z_2=2(x+1)^4(x^2+1)^2(x^4-4x^3+8x^2-4x+1)$ の分解体の Galois 群は $C_2\times C_2$（位数 4。`splitting_field` と `galois_group` の厳密計算）、$Z_3=c\,(x+1)^{14}\,g$（$g$ は既約 40 次。厳密な層転送と `factor`）の分解体の群は完全決定不能（polgalois は次数 11 まで）だが 40 個の根への推移性から位数が 40 の倍数なので、位数の比較だけで非同型が有限判定できる。$Z_4$ は $\mathbb Z$ 上の厳密係数が未取得（法 65537 のみ）なので組に使わない。次の小分け：(1) 本文の主張ブロック（$L=2$ で $Z_2$ と $Z'_2=Z_3$ の分解体の Galois 群が非同型——位数 4 対「位数は 40 の倍数」——かつ末尾ずらしで極限量が一致するので、Galois 群は判定枠の意味で極限量に効かない）を記述する。(2) SageMath check（$Z_2$ の分解と分解体の群、$Z_3$ の既約分解と 40 次因子の既約性）を置き対象ラベルを付ける。(3) Lean の扱い（有限群の位数の可除性の段だけ形式化できるか）を判断して割る。 | todo（2026-08-18 07:02 にゴール文書から引き直した。07:36 に先頭の照合を完了、次は主張ブロックの記述） |
