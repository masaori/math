...
| **【本流】潰れる候補: 既約分解の型は極限量に効かないか** | ゴール文書「極限側で問う言明」の「既約分解の型は効くか」から 2026-08-18 11:33 に引き直した。判別式・Galois 群と同じずらした自由族 $Z'_L:=Z_{L+1}$ の判定枠を使う。極限量の一致は既存の末尾ずらし定理（`TailShiftLimit` 系）から従うので、残る有限判定は「ある $L$ で $Z_L$ と $Z_{L+1}$ の既約分解の型（原始的で最高次係数が正の既約因子の次数と重複度の多重集合）が異なること」。$L=2$ で $Z_2=2(x+1)^4(x^2+1)^2(x^4-4x^3+8x^2-4x+1)$ の型は $\{(1,4),(2,2),(4,1)\}$、$Z_3=c\,(x+1)^{14}\,g$（$g$ 既約 40 次）の型は $\{(1,14),(40,1)\}$ で異なる（因数分解は check `galois-group-shifted-free-family-nonisomorphic/` で厳密計算済み）。本文の主張 `claim_shifted_free_family_factorization_type_does_not_determine_limit_quantity` を記述済み。SageMath は check `factorization-type-shifted-free-family-differs/` で二つの多重集合の決定と非一致を厳密検証済み（linkage は 1 check につき 1 ラベルなので既存 check への追記ではなく新設した）。次は Lean で既存の有限不変量抽象版 `finite_invariant_does_not_determine_limit_quantity` の再利用を判断する | 記述と SageMath まで |
