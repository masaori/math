...
## 現在地
- 2026-08-23 06:04: 本流「三つの切り詰めは衝突の判定の系である」を三つに割り、その先頭「有限個の素数への切り詰めは値の衝突を持つ」を記述の層まで進めた。素指数データを有限個の素数の集合 $P$ へ切り詰める写像 $\pi_P$ を置き、$P$ に属さない素数 $r$ を取って $1$ と $r$ が衝突を与えることを示し、あわせて注意ブロックで、有限個の素数での指数だけを見る粗視化の非十分性が値の衝突の判定（`claim_colliding_coarse_graining_is_not_sufficient_for_limit_quantity`）の特殊化として得られ独立な論法を要さないことを明記した。衝突の判定の主張が終域の集合に $S$ を使っているため、切り詰める素数の集合には $P$ を用いて記号の衝突を避けた。`npm run check` は 130 ブロック・261 参照すべて解決、linkage 53 件。次 tick はこのセクションの SageMath 検証から進める。レビューは着手前に作業ツリーが `origin/main` と一致して汚れていないことを確認し、直前 tick で閉じた衝突の判定の主張・証明を読み直したが、許されない脱出・記号の濫用・未証明の扱いの誤りは見つからず修正はしていない。並行ストリームは締切のため今 tick は見送った。
