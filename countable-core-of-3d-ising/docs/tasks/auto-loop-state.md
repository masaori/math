...
| **【本流】潰れる候補: 判別式は極限量に効かない、を判定できる形に定義する** | ゴール文書「極限側で問う言明」の未検討項（判別式・Galois 群は極限量に効くか）と「最初の三手」の三手目から引き直した標的（2026-08-17 17:02）。割った先頭は定義だけ：$q$ に依らない列 $L\mapsto\mathrm{disc}(Z_L)\in\mathbb Z$（判別式は整係数多項式の係数の整数多項式なので可算側）を粗視化として置き、既存の極限量 $\alpha(q)$ に対する「十分」「必要でない」の判定が何を意味するかを本文の定義ブロックに書く（候補：二つの有理点 $q,q'$ ではなく、同じ $q$ で判別式の列が異なる二つの多項式族を比べる枠が要るなら、その族の取り方を定義に含める）。判定できる形にならなければ、その旨を台帳へ書いて次の候補（Galois 群）へ移る。**2026-08-17 17:30 に先頭の定義を記述**（`def_constant_coarse_graining_from_q_independent_invariant`。有理点を動かす枠では判定が退化することを示し、判定枠を「二つの分配多項式の族の組」へ置き直した）。**2026-08-17 18:00 に残りを二つに割った**：自由族と周期族の判別式差を有限計算で閉じる部分と、極限量の一致を健全性の橋へ落とす部分。 | 記述と SageMath まで（定義と $L=3$ の自由・周期判別式差）。極限量の一致は未完了。**2026-08-17 21:30** に周期族との比較は極限一致が未証明、ずらした自由族は $\mathrm{disc}(Z_4)$ が要ると判明。**2026-08-18 00:55** に後者の計算を「密行列を作らない自由境界の整数点評価核」へ割り、Kronecker 積の butterfly による高速層転送を実装。$L=2,3$・$x=0,1,2$ で既存の厳密多項式と一致。**2026-08-18 02:03** に法 $65537$ 上で 145 点を評価・補間し、$Z_4\bmod65537$ が次数 144 を保ち square-free であることを確認したので、$\mathrm{disc}(Z_4)\ne0$ が $\mathbb Z$ 上で従う。**2026-08-18 03:08** に反例の主張 `claim_shifted_free_family_discriminant_does_not_determine_limit_quantity` を本文へ記述、**03:31** に check `discriminant-free-vs-periodic-differ` の対象ラベルをこの主張へ付け替えた。**04:04** に Lean 具体版を「末尾ずらしの極限一段」と「ずらした自由族への束ね」に分け、先頭 `tendsto_tail_one` を形式化した。**04:31 に束ねをさらに二つへ割った**：先頭は「ずらした自由族の有限箱量の列が元の列の末尾ずらしに項ごとに一致する補題」、次に「その等式と `tendsto_tail_one` の合成で $\alpha'(q)=\alpha(q)$ を導き Lean 具体版を閉じる定理」。**05:03 に先頭の補題 `shiftedFreeFiniteBoxQuantitySeq_eq_tail` を形式化。2026-08-18 05:33 に合成定理 `shiftedFreeFiniteBoxQuantitySeq_tendsto`・`shiftedFreeFiniteBoxQuantitySeq_limit_eq`（`TailShiftLimit.lean`）で Lean 具体版を閉じた**（lake build・sorry 検査 222 件通過）。次は Lean 必要十分版（末尾ずらしの極限一致は既に一般の実数列 `tendsto_tail_one` で書かれているので、`NecSuf/` 側へ何を置けば必要十分になるかを判断して割る）。 |
...
| **【並行】測定量の事前予言: 2 次元の閉形式から代数的命題を 1 つ導く** | 2 次元の既知の閉形式から、$\widetilde R$ について**測定の前に**検証可能な代数的命題（全称命題）を 1 つ導出する。**導出できないと分かった場合は、その事実を記録してこの測定量を候補から落とす**（ゴール文書の否定判定「2 次元から事前に予言が出ない量は候補から落とす」）。落とすことも成果である。**2026-08-18 02:06 に当初候補を棄却して割り直した**：台帳にあった「$R^{(2)}_{L,L'}$ は $2^{\#V}$ 倍と単項式倍で Pfaffian」は、辺重み $X_e^{[\sigma_u\ne\sigma_v]}$ の展開と係数が合わない。まず各辺の有限恒等式を展開して、$A=A^{(2)}_{L,L'}$ と偶部分グラフ $F\subseteq A$ について $$R^{(2)}_{L,L'}=2^{\#V^{(2)}_L-\#A}\sum_{F\ \mathrm{even}}\prod_{e\in F}(1-X_e)\prod_{e\in A\setminus F}(1+X_e)$$ を本文で証明する（負の 2 冪を係数として誤読しないよう、必要なら両辺へ正の 2 冪を掛けた整数多項式の等式として書く）。その後、この偶部分グラフ和に平面グラフの Kasteleyn–Fisher 表示を適用できるかを、一次文献の正確な定理と照合する。 | 記述まで。**2026-08-18 03:34 に三つへ割った**：先頭は整数多項式の偶部分グラフ和、次に小箱の SageMath 検証、最後に Kasteleyn–Fisher 表示の一次文献照合。**04:04** に先頭 `claim_two_dimensional_boundary_response_even_subgraph_sum` を記述した（辺ごとの有限恒等式の展開と、奇数次数のスピンを反転する対合）。次は SageMath 検証。 |
...
- 2026-08-17 21:30: 「潰れる候補: 判別式」の (b) を検討。自由族×周期族の組は $\alpha(q)=\alpha^{\mathrm{per}}(q)$ 自体が未証明で閉じず、自由族をずらした組 $Z'_L:=Z_{L+1}$ は極限の一致は自明だが $\mathrm{disc}(Z_2)=\mathrm{disc}(Z_3)=0$ で反例にならないと判明（$\mathrm{disc}(Z_4)$ が要る）。本文未変更、台帳に次点二案を記録。
- 2026-08-17 19:30: 「潰れる候補: 判別式」の (a) を本文の主張 `claim_discriminant_free_vs_periodic_differ_at_L3` として閉じ、check の対象ラベルを付け替えた。
- 2026-08-17 19:00: 「潰れる候補: 判別式」の (a) の先頭。共通 sage 定義（箱の分配多項式）と check `discriminant-free-vs-periodic-differ` を追加（$L=2$ 一致・$L=3$ 不一致を厳密計算で確認）。本文未変更。
...
- 2026-08-18 00:55: 直近の主定理表示変更と「$L=3$ で自由族と周期族の判別式が異なる」の本文・SageMath 対応を点検。`npm run check`（99 ブロック・163 参照）・linkage 35 件・判別式 check が通り、修正なし。
- 2026-08-17 21:30: `npm run check`・`npm run build:pdf` の再実行のみ（99 ブロック・163 参照・28 ページ、前 tick と一致）。修正なし。
- 2026-08-17 20:30: 開始が締切 8 分前。`npm run check`・`npm run build:pdf` の再実行のみ（99 ブロック・163 参照・28 ページ、前 tick と一致）。修正なし。
- 2026-08-17 17:30: 粗視化の定義・対称化した極限量の定義・必要でないことの主張を読み直し、脱出・記号の濫用ともに不備なし。修正なし。
