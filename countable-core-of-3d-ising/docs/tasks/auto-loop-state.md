...
| **【本流】最小性: 有限箱量の列の有限接頭部を忘れる粗視化は極限量に対して十分である** | ゴール文書「極限側で問う言明」の「自由エネルギー密度を決める十分な最小の層はどこか」から 2026-08-18 18:02 に引き直した。二つの有限箱量の列が、ある $N\in\mathbb N$ 以降のすべての添字で項ごとに一致することを「有限接頭部を忘れた値の一致」と定め、その一致から両列の箱の大きさの極限が一致することを示す。これは直前の末尾ずらし反例が示した「有限個の箱の値は必要でない」を、十分な粗視化の候補（尾部同値類）として正面から書くものである。2026-08-18 19:03 に、定義ブロックへ混在していた同値関係の主張を独立させ、`claim_tail_agreement_is_equivalence_relation` として反射性・対称性・推移性を記述した。2026-08-18 19:32 に十分性の主張 `claim_tail_agreement_is_sufficient_for_limit_quantity` を記述し、記述の三分割（定義・同値関係・十分性）が揃った。2026-08-18 20:03 に可算側の段を SageMath で厳密検証した。2026-08-19 10:34 に Lean 具体版の先頭 `tailAgreement_tendsto` を形式化した。2026-08-19 11:05 に有限箱量の列への特殊化と束ね（`rootSeq_tailAgreement_of_tailAgreement`・`tailAgreement_limitQuantity_tendsto`・`tailAgreement_limitQuantity_eq`）で Lean 具体版を閉じた。残りは Lean 必要十分版。 | Lean 具体版まで |
