...
| **【本流】最小性: ずらした自由族は有限箱の分配多項式値が極限量に必要でないことの反例である** | ゴール文書「極限側で問う言明」の「自由エネルギー密度を決める十分な最小の層はどこか」から 2026-08-18 14:02 に引き直した。自由族 $Z_L$ とずらした自由族 $Z'_L:=Z_{L+1}$ を $q=1$ で比べると、$L=2$ で $Z_2(1)=2^8\ne2^{27}=Z'_2(1)$ だが、`claim_partition_value_at_one` により各箱の正規化値は $2$ であり、両族の極限量は一致する。まず本文でこの反例を一主張として記述し、その後 SageMath・Lean 具体版・Lean 必要十分版へ進める。**2026-08-18 14:31 に記述を二つへ割った**：先頭は「有限側の主張の記述——$q=1$ で `claim_partition_value_at_one`（$Z_L(1)=2^{\#V_L}$）から $Z_2(1)=2^8\ne2^{27}=Z'_2(1)$ だが、有限箱量は全 $L$ で $a_L(1)=a'_L(1)=2$ と項ごとに一致すること」、次に「極限側の束ね——末尾ずらし定理（`TailShiftLimit` 系）による極限量の一致と併せ、有限箱の分配多項式値が極限量に必要でない反例として一主張に束ねる」。**2026-08-18 15:13 に先頭を `claim_shifted_free_family_partition_values_differ_but_finite_box_quantities_agree` として記述した。** | 記述まで（**2026-08-18 15:32 に極限側の束ね `claim_shifted_free_family_partition_value_does_not_determine_limit_quantity` を記述し、主張の記述が揃った**。次は SageMath 検証） |
...
| **【並行】測定量の事前予言: 2 次元の閉形式から代数的命題を 1 つ導く** | 2 次元の既知の閉形式から、$\widetilde R$ について**測定の前に**検証可能な代数的命題（全称命題）を 1 つ導出する。まず辺ごとの有限恒等式を展開し、偶部分グラフの整数多項式有限和を導いた。その後、この和へ Kasteleyn–Fisher 表示を適用できるかを一次文献の正確な定理と照合する。 | 記述と SageMath まで。`claim_two_dimensional_boundary_response_even_subgraph_sum` を記述し、`sagemath/check/two-dimensional-boundary-response-even-subgraph-sum/` で $L'=1,L=2$ の全配位・全辺部分集合を直接比較して PASS。Kasteleyn 側は Cimasoni 講義録から二次文献の格付け付きで記録した。**2026-08-18 08:06 に Fisher 1966 の原論文本文も確認し**、expanded lattice・terminal lattice の構成、polygon と dimer の一対一対応、内部重み 1・外部重み $v_{ij}^{-1}$、平面性を `文献と確認状況.md` へ記録した。次は、こちらの偶部分グラフ和 $\sum_{F\,\mathrm{even}}\prod_F(1-X_e)\prod_{A\setminus F}(1+X_e)$ を $v_e=(1-X_e)/(1+X_e)$ と正規化したとき、Fisher の単項式因子と合わせて分母が消え、多項式環内の Pfaffian 恒等式として書けるかを判定する。満たすなら本文へ予言として書き、満たさないなら候補から落として記録する。 **2026-08-18 10:45 に分母消去の判定を台帳上で行った（本文未記述・SageMath 未検証）**: 偶部分グラフ和は $\prod_{e\in A}(1+X_e)\cdot\sum_{F\,\mathrm{even}}\prod_{e\in F}v_e$（$v_e=(1-X_e)/(1+X_e)$）と書け、Fisher–Kasteleyn 側の Pfaffian は terminal lattice の完全マッチングの符号付き有限和で、各マッチングは各外部辺の重みを高々 1 回しか含まない（dimer 配置は辺集合の部分集合。多重線形）。ゆえに各項の分母は $\prod_{e\in A}(1\pm X_e)$ を割り、その積を掛ければ各項が $\mathbb Z[X_e]$ の元になるので、**分母は消えると判定した（候補から落とさない）**。ただしこの判定は「Pfaffian のマッチング展開」という文献の定理の言明に依存しており、まだ有限例で検証していない。**2026-08-18 14:04 に有限検証を二つへ割った**。先頭は $L'=1,L=2$ の terminal lattice と polygon–dimer 対応を有限集合として構成し、完全マッチングとの全単射を確認する check。次に、その同じグラフへ Kasteleyn 向き付けと $v_e=(1-X_e)/(1+X_e)$ を入れ、分母消去後の Pfaffian 多項式が $R^{(2)}_{2,1}$ と一致することを確認する。**2026-08-18 15:13 に先頭の check を同じ検証ディレクトリへ追加し、偶部分グラフ 2 個・完全マッチング 2 個を全列挙して往復写像が互いに逆であることを確認した。** 次は Kasteleyn 向き付けと分母消去後の多項式一致。両方が通った後に本文へ予言命題として記述する。 |
