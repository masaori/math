...
| **【本流】最小性: 有限箱値の交差べき等式は極限量に対して十分である** | ゴール文書の自由エネルギー密度についての最小性から 2026-08-19 14:34 に引き直した。二つの正有理数列 $A_L,B_L$ と正の自然数列 $N_L,M_L$ に対し、各箱で $A_L^{M_L}=B_L^{N_L}$ なら正の乗根 $A_L^{1/N_L}=B_L^{1/M_L}$ が一致し、したがって一方の箱サイズ極限が存在すれば他方も存在して一致することを示す。交差べき等式は有限整数算術で決定可能であり、値そのものの尾部一致より粗い。2026-08-19 15:33 に先頭の代数的主張を記述した。$x=A^{1/N},y=B^{1/M}$ と置き、$x^{NM}=A^M=B^N=y^{NM}$ と正の自然数乗の単射性から $x=y$ を得る。次に箱サイズ極限へ渡す十分性の主張へ分ける。2026-08-19 16:03 に先頭の主張の SageMath 検証を通した（check `cross-power-equality-implies-root-equality/`：乗根が正有理数に取れる三例で証明の各行を `QQ` で厳密比較、正の自然数乗の単射性の有限標本、負例の検出。linkage 43 件）。2026-08-19 16:32 に Lean 具体版 `cross_power_equality_implies_posRoot_equality` を本文と同じ六段の計算と正の自然数乗の単射性で形式化した。2026-08-19 17:03 に Lean 必要十分版を二つへ割った。17:33 のレビューで、抽象版の大域的な累乗単射性は偶数乗の実数上で成り立たないため、対象二元での局所的な単射性へ修正した。17:35 に具体版を修正済み抽象版の特殊化 `cross_power_equality_implies_posRoot_equality_fromNecSuf` として導出した。 | Lean 必要十分版まで（次の十分性の主張の記述を 2026-08-19 18:04 に二つへ割った。先頭は「各箱で交差べき等式 $A_L^{M_L}=B_L^{N_L}$ が成り立つ二組の列について、乗根列 $A_L^{1/N_L}$ と $B_L^{1/M_L}$ が全ての $L$ で項別一致するという可算側の主張の記述（乗根一致の主張の列への適用）」、次に「項別一致は尾部一致の特別な場合なので、既存の尾部一致の十分性の主張と合成して、一方の箱サイズ極限が存在すれば他方も存在して一致するという極限側の主張の記述」） |
...
| **【並行】測定量の事前予言: 2 次元の閉形式から代数的命題を 1 つ導く** | 2 次元の既知の閉形式から、$\widetilde R$ について測定前の全称命題を導く。偶部分グラフの有限和を記述・検証し、Kasteleyn–Fisher 表示の仮定を文献と照合した。2026-08-19 10:37 に、Fisher terminal graph の内部辺へ重み $1$、元の辺に対応する外部辺へ $(1+X_e)/(1-X_e)$ を置き、Kasteleyn 向き付けから得る Pfaffianへ $\prod_e(1-X_e)$ を掛ければ $2^{\#A}R^{(2)}_{L,L'}$ に符号と $2^{\#V}$ の因子を除いて一致する全称命題を本文へ記述した。正実数重みの絶対値等式は使わず、完全マッチングの符号一致と Pfaffian の有限展開だけを使う。$L'=1,L=2$ の terminal lattice では polygon–dimer 全単射を全列挙し、分母消去後の一致を多変数整数係数多項式として厳密検証した。2026-08-19 14:35 に Lean 具体版を二つへ分割した。2026-08-19 15:37 に先頭の一定符号有限展開を `Prediction.constantSign_finiteExpansion` として形式化した。残りは Fisher terminal graph への特殊化・polygon–dimer 全単射・分母消去の束ね。 | Lean 具体版の途中（次は terminal graph への特殊化と分母消去） |
...
## 前進の記録（新しい 5 件まで）
- 2026-08-19 14:34: 自由エネルギー密度の最小性から、有限箱値そのものの尾部一致より粗い新標的として、正有理数値の交差べき等式が対応する乗根列を決め、極限量に対して十分であることを選定した。次は可算側の交差べき等式から乗根一致までを記述する。
- 2026-08-19 13:33: 零点集合の反例の Lean 具体版を閉じた。最小多項式次数集合の有限な不一致と、末尾ずらしによる極限量の一致を一つの定理に束ねた。次は Lean 必要十分版。
- 2026-08-19 13:04: 零点集合の反例の SageMath 検証を通した。$X^2+1$ が $Z_2$ を割って既約であること、$Z_3$ の既約因子次数が 1 と 40 だけであること、$\gcd(Z_3,X^2+1)=1$ を `QQ` 上で厳密判定した。次は Lean 具体版。
- 2026-08-19 12:32: ずらした自由族では零点の集合が異なっても極限量が一致する主張を本文へ記述した。次数 2 の最小多項式を持つ $Z_2$ の零点が、最小多項式次数 1 または 40 の零点しか持たない $Z'_2=Z_3$ には属さないことを有限判定した。
- 2026-08-19 12:03: ゴール文書の自由エネルギー密度についての最小性から、新標的「ずらした自由族は零点の集合が極限量に必要でないことの反例」を台帳へ追加した。零点の集合の相違は既存の厳密因数分解から有限判定できる。次は本文の主張ブロックの記述。
- 2026-08-19 10:37: 並行で、2 次元境界応答多項式を Fisher terminal graph の分母消去済み Kasteleyn Pfaffian として表す全称命題を記述し、最小の有限例を厳密再検証した。
- 2026-08-19 10:34: 尾部一致した二つの実数列の一方の収束を他方へ移す一般補題を Lean で形式化した。次は有限箱量の列への特殊化と十分性の主張の束ね。
- 2026-08-18 20:03: 尾部一致の十分性について、有限接頭部の相違、尾部の列の項、正有理数値、有限箱量を特徴づける有限べきの等式を SageMath で厳密検証した。次は Lean 具体版。
- 2026-08-18 19:32: 尾部一致から極限量の存在と一致を得る十分性の主張を記述し、記述の三分割が揃った。次は SageMath 検証。
- 2026-08-18 19:03: 尾部一致の定義から同値関係であるという主張を分離し、反射性・対称性・推移性を独立した本文主張として記述した。次は尾部一致から極限量の存在と一致を得る十分性の主張を記述する。
- 2026-08-18 18:02: 自由エネルギー密度についての最小性から、有限箱量の列の有限接頭部を忘れた尾部同値類を十分な粗視化の候補として引き直した。次は同値関係と十分性を本文の一定理として記述する。
- 2026-08-18 17:05: 分配多項式値反例の Lean 具体版を閉じた。有限不変量の不一致、末尾との項別一致、極限量の一致を一つの定理に束ねた。
- 2026-08-18 16:35: 分配多項式値反例の Lean 具体版の先頭、$Z_2(1)=2^8\ne2^{27}=Z_3(1)$ の不一致補題を、既形式化の 1 での値と点数公式に帰着して形式化した。
- 2026-08-18 16:03: ずらした自由族の分配多項式値が極限量に必要でない反例について、有限値の不一致と正規化値の有限べき等式を `ZZ` 上で検証し、SageMath 層を閉じた。
- 2026-08-18 14:02: ゴール文書の自由エネルギー密度についての最小性から、ずらした自由族を使って「有限箱の分配多項式値は極限量に必要でない」を判定する新標的を台帳へ追加した。先頭は $q=1,L=2$ の有限値の不一致と極限一致を束ねた本文記述。
- 2026-08-18 13:05: 既約分解型の反例の Lean 具体版を閉じた。二つの正規化済み因子型リストの不一致を決定計算し、末尾ずらしによる極限一致と一つの定理に束ねた。
- 2026-08-18 11:04: Galois 群非同型主張の Lean 必要十分版を閉じた。有限型の位数の非可除性と、フィルタを保つ添字写像・項別一致・極限一意性だけへ抽象化し、具体版をその特殊化として導出した。
...
## レビュー記録（新しい 5 件まで）
- 2026-08-19 17:33: 交差べき等式の抽象版が `N*M` 乗写像のモノイド全体での単射性を要求しており、偶数乗では実数上の具体版へ特殊化できなかった。証明が実際に使うのは対象の二元 `x,y` における含意だけなので、仮定を `x^(NM)=y^(NM) → x=y` へ縮めた。
- 2026-08-19 14:32: 直前の零点集合反例の Lean 必要十分版導出をレビューし、実数列の収束仮定を持つにもかかわらずファイル冒頭が「ℝ への脱出は無い」としていた不整合を修正した。非可算への脱出は二つの有限箱量列について箱の大きさの極限を取る仮定だけであると明記した。
- 2026-08-19 12:31: 直前に選定した零点集合の反例を、既約分解の厳密 check、既約分解型から零点の最小多項式次数を得る既存主張、末尾ずらしによる極限一致と照合した。次数 2 の零点の有無で集合の不一致が従い、許されない脱出や未証明依存は無いため修正事項なし。
- 2026-08-19 10:32: 尾部一致の本文を有限箱列・素指数データからの復元・極限量の定義・SageMath check と照合し、許されない脱出や参照不備がないことを確認した。`npm run check` は 108 ブロック・201 参照ですべて通過し、修正事項なし。
- 2026-08-18 20:01: 十分性の本文を尾部一致の定義・素指数データからの復元・極限量の定義と照合し、許されない脱出や参照不備がないことを確認した。`npm run check` は 108 ブロック・201 参照ですべて通過し、修正事項なし。
- 2026-08-18 19:02: 直前の尾部一致の定義ブロックが、反射性・対称性・推移性という別の主張と証明を同じブロックへ含めていたため、定義だけへ戻した。同値関係であることは四層管理する独立主張として先頭に分割する。
- 2026-08-18 18:01: 直前の分配多項式値反例の本文と Lean 必要十分版を照合し、値の不一致・末尾ずらしによる項別一致・極限一意性の対応に不備がないことを確認した。`npm run check` は 105 ブロック・189 参照ですべて通過し、修正事項なし。
- 2026-08-18 17:03: 直前の有限不変量の不一致補題を人手証明、既形式化の $Z_L(1)=2^{\#V_L}$、点数公式と照合し、`npm run check`（105 ブロック・189 参照）を再実行した。修正事項なし。
- 2026-08-18 16:33: `npm run check`（105 ブロック・189 参照、すべて解決）を再実行し、前 tick の出力に修正事項なし。
- 2026-08-18 16:02: 直前に記述した有限側・極限側の二主張を判定枠と照合し、`npm run check`（105 ブロック・189 参照）を再実行した。修正事項なし。
- 2026-08-18 15:06: 箱の定義だけが $L\ge2$ に限定されていた一方、極限列・既存の $L=1$ 校正・台帳はいずれも $L\ge1$ を用いていた不整合を確認した。$L=1$ では辺集合が空になるだけで後続の定義と主張は成立するため、`def_box` の定義域を $L\ge1$ へ修正した。`npm run check` は修正前にも通っており、この意味上の不整合は型検査の範囲外だった。
- 2026-08-18 14:01: 直前の既約分解型の反例について、本文・SageMath check・Lean 具体版／必要十分版を定義済みの判定枠と照合し、`npm run check`（103 ブロック・177 参照）を再実行した。修正事項なし。
- 2026-08-18 13:01: 直近の既約分解型の本文・SageMath check・台帳を再照合し、`npm run check`、対象 check、linkage 38 件を再実行した。修正事項なし。
- 2026-08-18 11:32: `npm run check`（102 ブロック・214 参照、すべて解決）を再実行し、前 tick の出力に修正事項なし。
- 2026-08-18 11:02: `npm run check`（102 ブロック・173 参照、すべて解決）を再実行し、直前の Galois 群具体版と末尾ずらし必要十分版を突き合わせて修正事項なし。
