...
| **【並行】測定量の事前予言: 2 次元の閉形式から代数的命題を 1 つ導く** | 2 次元の既知の閉形式から、$\widetilde R$ について**測定の前に**検証可能な代数的命題（全称命題）を 1 つ導出する。**導出できないと分かった場合は、その事実を記録してこの測定量を候補から落とす**（ゴール文書の否定判定「2 次元から事前に予言が出ない量は候補から落とす」）。落とすことも成果である。**2026-08-18 02:06 に当初候補を棄却して割り直した**：台帳にあった「$R^{(2)}_{L,L'}$ は $2^{\#V}$ 倍と単項式倍で Pfaffian」は、辺重み $X_e^{[\sigma_u\ne\sigma_v]}$ の展開と係数が合わない。まず各辺の有限恒等式を展開して、$A=A^{(2)}_{L,L'}$ と偶部分グラフ $F\subseteq A$ について $$R^{(2)}_{L,L'}=2^{\#V^{(2)}_L-\#A}\sum_{F\ \mathrm{even}}\prod_{e\in F}(1-X_e)\prod_{e\in A\setminus F}(1+X_e)$$ を本文で証明する（負の 2 冪を係数として誤読しないよう、必要なら両辺へ正の 2 冪を掛けた整数多項式の等式として書く）。その後、この偶部分グラフ和に平面グラフの Kasteleyn–Fisher 表示を適用できるかを、一次文献の正確な定理と照合する。 | todo（誤った Pfaffian 係数形を棄却し、先頭を有限の偶部分グラフ和の記述へ差し替えた。本文未変更）。**2026-08-18 03:34 に三つへ割った**：先頭は「主張ブロックの記述（$2^{\#A}R^{(2)}_{L,L'}=2^{\#V^{(2)}_L}\sum_{F\subseteq A\ \mathrm{even}}\prod_{e\in F}(1-X_e)\prod_{e\in A\setminus F}(1+X_e)$ を整数多項式の等式として書く。証明は辺ごとの恒等式 $2X_e^{[\sigma_u\ne\sigma_v]}=(1+X_e)+(\sigma_u\sigma_v)(1-X_e)$ の展開とスピン和で奇数次項が消える 1 論法）」、次に「SageMath 検証（小さい内箱・外箱で両辺を多変数環で厳密比較）」、最後に「Kasteleyn–Fisher 表示を偶部分グラフ和へ適用できるかの一次文献照合」。 |
