...
| **【本流】最小性: 三つの切り詰めは衝突の判定の系である** | 上の判定条件から、既に個別に示した三つの切り詰め（有限個の素数へ制限・指数の大きさで頭打ち・素数が現れるかどうかだけ）の非十分性が、それぞれ衝突を 1 つ挙げるだけで従うことを示す（特殊化 3 本。新しい論法は使わない）。**2026-08-23 に上のセクションから割り出した** | todo |
| **【本流】最小性: 定数列の族に限れば、衝突を持たない粗視化は十分である** | 逆向き。$\pi$ が値の衝突を持たなければ、定数列の族の上では $\pi$ の値から元の値が定まり、極限量が一致することを示す。上の二つと合わせて、**定数列の族に限れば「十分」と「衝突を持たない」が同値**になり、可算コアの下限が一つの必要十分条件として書ける。**2026-08-23 に上のセクションから割り出した** | todo |
