...
| **【本流】潰れる候補: Galois 群は極限量に効かないか** | ずらした自由族 $Z'_L:=Z_{L+1}$ を使う。極限量の一致は既存の末尾ずらし定理から従うので、ある $L$ で $Z_L$ と $Z_{L+1}$ の分解体上の Galois 群が同型でないことを有限の厳密計算で示す。帰無モデルの超八面体群上限は再発見として数えず、群の同型型そのものを比較する。先頭は $Z_3,Z_4$ の厳密データと SageMath で利用できる Galois 群決定の条件を照合し、実行可能な最初の箱の組を固定する。 | todo（2026-08-18 07:02 にゴール文書から引き直した） |
...
| **【並行】測定量の事前予言: 2 次元の閉形式から代数的命題を 1 つ導く** | 2 次元の既知の閉形式から、$\widetilde R$ について**測定の前に**検証可能な代数的命題（全称命題）を 1 つ導出する。まず辺ごとの有限恒等式を展開し、偶部分グラフの整数多項式有限和を導いた。その後、この和へ Kasteleyn–Fisher 表示を適用できるかを一次文献の正確な定理と照合する。 | 記述と SageMath まで。`claim_two_dimensional_boundary_response_even_subgraph_sum` を記述し、`sagemath/check/two-dimensional-boundary-response-even-subgraph-sum/` で $L'=1,L=2$ の全配位・全辺部分集合を直接比較して PASS。次の Kasteleyn–Fisher 表示の一次文献照合は 2026-08-18 06:35 に三つへ割った（先頭から順に）：(1) 一次文献の定理の正確な言明を `文献と確認状況.md` へ写す——Kasteleyn の「平面グラフの偶部分グラフ和（dimer 型の母関数）が Pfaffian で書ける」定理と、Fisher の「平面 Ising の高温展開を dimer 被覆へ写す」構成について、**どのグラフ・どの辺重み・どんな向き付け（Pfaffian orientation）を仮定しているか**を原文の言明どおりに記録する（現在 `文献と確認状況.md` に Kasteleyn の項は無い）。(2) こちらの偶部分グラフ和 $\sum_{F\,\mathrm{even}}\prod_F(1-X_e)\prod_{A\setminus F}(1+X_e)$ がその仮定を満たすか（辺重みが $\frac{1-X_e}{1+X_e}$ の形へ正規化できるか、正規化が多項式環の中で書けるか＝分母を持ち込まないか）を判定する。(3) 満たすなら $R^{(2)}_{L,L'}$ の Pfaffian 表示を代数的命題（測定前の予言）として本文へ書き、満たさないならこの候補を落として記録する。 |
