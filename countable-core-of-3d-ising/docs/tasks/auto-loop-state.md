...
| **【本流】最小性: 有限箱値の交差べき等式は極限量に対して十分である** | ゴール文書の自由エネルギー密度についての最小性から 2026-08-19 14:34 に引き直した。二つの正有理数列 $A_L,B_L$ と正の自然数列 $N_L,M_L$ に対し、各箱で $A_L^{M_L}=B_L^{N_L}$ なら正の乗根 $A_L^{1/N_L}=B_L^{1/M_L}$ が一致し、したがって一方の箱サイズ極限が存在すれば他方も存在して一致することを示す。交差べき等式は有限整数算術で決定可能であり、値そのものの尾部一致より粗い。2026-08-19 15:33 に先頭の代数的主張を記述した。$x=A^{1/N},y=B^{1/M}$ と置き、$x^{NM}=A^M=B^N=y^{NM}$ と正の自然数乗の単射性から $x=y$ を得る。2026-08-19 16:03 に先頭の主張の SageMath 検証を通し、16:32 に Lean 具体版、17:35 に Lean 必要十分版からの導出を閉じた。2026-08-19 18:32 に、各箱の交差べき等式へ先頭主張を適用して、二つの正の乗根列が全ての箱で項別一致する主張を記述した。2026-08-19 19:32 に、項別一致から一致開始添字 $1$ の尾部一致の等式条件を得る主張を記述した。 | 記述まで（次は、既存の実数列の収束移送を適用し、一方の箱サイズ極限の存在から他方の存在と一致が従う極限側の束ね主張の記述） |
