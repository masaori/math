...
| **【本流】最小性: ずらした自由族は零点の集合が極限量に必要でないことの反例** | ゴール文書「極限側で問う言明」の自由エネルギー密度についての最小性から引き直した（2026-08-19 12:03）。決定関係の表の最上層である零点の集合（$\overline{\mathbb Q}$ の有限部分集合。最高次係数と重複度を除いて多項式を決める）が極限量に必要でないことを、ずらした自由族 $Z'_L=Z_{L+1}$ で示す。$L=2$ で $Z_2$ は 2 次の既約因子を持つ一方、$Z_3=c\,(x+1)^{14}g$（$g$ 既約 40 次）の零点の最小多項式次数は 1 か 40 だけなので零点の集合は異なるが、末尾ずらしにより極限量は一致する。2026-08-19 12:32 に、この有限判定と既存の極限一致を束ねた主張を本文へ記述した。2026-08-19 13:04 に SageMath 検証 `sagemath/check/root-set-shifted-free-family-differs/`（$X^2+1\mid Z_2$・既約、$Z_3$ の既約因子次数 $\{1,40\}$、$\gcd(Z_3,X^2+1)=1$）を PASS。 | 記述と SageMath まで（次は Lean 具体版） |
