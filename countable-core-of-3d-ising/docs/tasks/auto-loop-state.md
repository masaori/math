...
| **【並行】測定量の事前予言: 2 次元の閉形式から代数的命題を 1 つ導く** | 2 次元の既知の閉形式から、$\widetilde R$ について測定前の全称命題を導く。偶部分グラフの有限和を記述・検証し、Kasteleyn–Fisher 表示の仮定を文献と照合した。$L'=1,L=2$ の terminal lattice では polygon–dimer 全単射を全列挙し、さらに面境界との不一致が奇数本となる向き付けについて、外部辺重み $(1+X_e)/(1-X_e)$ の Pfaffianへ $\prod_e(1-X_e)$ を掛けると、符号を除いて $R^{(2)}_{2,1}$ の偶部分グラフ和に一致することを多変数整数係数多項式として厳密検証した。 | 記述と SageMath まで（有限例による導出可能性の判定は PASS。次はこの Pfaffian 表示を全称の予言命題として本文へ記述） |
