...
| 内箱と外箱の間の辺変数を 1 に置かない測定量の定義 | 内箱と外箱の間の辺の変数を 1 に置かず保持する測定量（例: 内箱に接する辺の変数だけを保持し、それ以外を 1 に置く有限代入の像）を一つ定義し、well-defined 性（有限代入が環準同型であること）を示す（有限の箱の比較にとどめる） | todo |
