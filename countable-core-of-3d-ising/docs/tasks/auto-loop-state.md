...
- 次の tick は、先頭未完了セクション「箱の定義から整数の算術を落とせるか」の Lean 必要十分版に
  着手する。具体版と同じ証明順を保ちつつ、後続写像とその単射性、方向の添字を落とし、
  有限な辺型、二つの端点写像、二色塗り分けだけが十分であることを形式化する。
...
| 箱の定義から整数の算術を落とせるか | 箱を「有限集合＋方向ごとの後続の部分写像＋2 色の塗り分け」として定義し直し、回文性の証明が通るかを問う。通れば整数の加法・順序・座標和は本質的でなかったことになり、通らなければどこで整数が要るかが判明する（`docs/discussion/3次元Isingを可算側で書く/不要な構造を持ち込まない.md`） | Lean 具体版まで |
