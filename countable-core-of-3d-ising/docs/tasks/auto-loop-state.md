...
- 2026-08-14: 「箱の定義から整数の算術を落とせるか」の Lean 必要十分版を完了し、
  ...
  二つの端点写像を備えた有限な辺型、両端で色が異なる二色塗り分け、値の反転の三性質
  （対合と、反転後の不一致が反転前の一致と同値であることの両向き）だけを仮定して、
  具体版と同じ六段の順で多重度の回文性を示した。具体版がその特殊化であることも導出した。
  新規 2 定理を検査対象へ登録して登録は計 30 件になった。
  次の先頭未完了セクションは「帰無モデル: 値と台の恒等式」で、最初の層は記述である。
  セクションは複数の主張（Z_L(1) の値、係数の非負性、台の両端、Ω の偶数性）を含むので、
  着手時に runbook の割り方の規則で 1 つの論法ごとに割り直すこと。
...
- 次の tick は、先頭未完了セクション「帰無モデル: 値と台の恒等式」に着手する。
  まずセクションを 1 つの論法ごとに割り直してから、最初の主張の記述を書く。
...
- 2026-08-14: 有限二部後続系の回文性の Lean 必要十分版と、具体版がその特殊化である
  ...
  二つの端点写像を備えた有限な辺型・二色塗り分け・値の反転の三性質だけを仮定する。
