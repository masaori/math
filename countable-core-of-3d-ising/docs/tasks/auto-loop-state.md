...
| **【本流】最小性: ずらした自由族は有限箱の分配多項式値が極限量に必要でないことの反例である** | ゴール文書「極限側で問う言明」の「自由エネルギー密度を決める十分な最小の層はどこか」から 2026-08-18 14:02 に引き直した。自由族 $Z_L$ とずらした自由族 $Z'_L:=Z_{L+1}$ を $q=1$ で比べると、$L=2$ で $Z_2(1)=2^8\ne2^{27}=Z'_2(1)$ だが、`claim_partition_value_at_one` により各箱の正規化値は $2$ であり、両族の極限量は一致する。本文の有限側と極限側の束ねを記述済み。2026-08-18 16:03 に $Z_2(1)$ と $Z'_2(1)$ の不一致、および有限箱量が $2$ になる根拠の有限べき等式を `ZZ` 上で検証した。 | Lean 具体版まで（2026-08-18 17:05。`partition_value_does_not_determine_limit_quantity` が有限不変量の不一致、ずらした有限箱量と元の列の末尾との項別一致、極限量の一致を束ねる。次は Lean 必要十分版——2026-08-18 17:35 に二つへ割った：先頭は既存の `value_invariant_does_not_determine_limit_quantity` の再利用判断と特殊化導出 `_fromNecSuf`、次に status を `done` へ） |
