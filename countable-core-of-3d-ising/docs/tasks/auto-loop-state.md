...
| **【本流】潰れる候補: 判別式は極限量に効かない、を判定できる形に定義する** | ゴール文書「極限側で問う言明」の未検討項（判別式・Galois 群は極限量に効くか）と「最初の三手」の三手目から引き直した標的（2026-08-17 17:02）。割った先頭は定義だけ：$q$ に依らない列 $L\mapsto\mathrm{disc}(Z_L)\in\mathbb Z$（判別式は整係数多項式の係数の整数多項式なので可算側）を粗視化として置き、既存の極限量 $\alpha(q)$ に対する「十分」「必要でない」の判定が何を意味するかを本文の定義ブロックに書く（候補：二つの有理点 $q,q'$ ではなく、同じ $q$ で判別式の列が異なる二つの多項式族を比べる枠が要るなら、その族の取り方を定義に含める）。判定できる形にならなければ、その旨を台帳へ書いて次の候補（Galois 群）へ移る。**2026-08-17 17:30 に先頭の定義を記述**（`def_constant_coarse_graining_from_q_independent_invariant`。有理点を動かす枠では判定が退化することを示し、判定枠を「二つの分配多項式の族の組」へ置き直した）。**2026-08-17 18:00 に残りを二つに割った**：(a) 二つ目の族＝周期族 $Z^{\mathrm{per}}_L$（本文の `def_periodic_multiplicity` から作る）で、判別式が異なる $L$ の存在を有限計算の主張＋SageMath で閉じる（未着手）。(b) $\alpha(q)=\alpha^{\mathrm{per}}(q)$ は極限側の言明で有限計算では判定できない——健全性の橋の定理として有限箱側の挟み込みへ割れるかを検討（未着手） | 記述まで（定義なので SageMath は対象外。(a)(b) は todo）。**2026-08-17 19:00 に (a) の先頭（共通定義＋check `discriminant-free-vs-periodic-differ`）を完了。$L=2$ は判別式一致（両方 $0$）、$L=3$ は不一致。次は $L=3$ の不一致を主張ブロックとして本文に書く**。**2026-08-17 19:30 に (a) を本文の主張 `claim_discriminant_free_vs_periodic_differ_at_L3` として閉じた（記述と SageMath まで。有限計算の主張なので Lean は対象外とする）。残りは (b)**。**2026-08-17 21:30 に (b) を検討し二つの構成が今のデータでは閉じないことを確認した（詳細は「現在地」）: 自由族と周期族を組にとる案は $\alpha(q)=\alpha^{\mathrm{per}}(q)$ 自体が未証明で挟み込みの論法が無い。自由族をずらした族 $Z'_L:=Z_{L+1}$ を組にとる案は極限の一致は自明に閉じるが $\mathrm{disc}(Z_2)=\mathrm{disc}(Z_3)=0$ のため反例にならず、$\mathrm{disc}(Z_4)$ が要る（層転送は状態数 $2^{16}$ でこの tick では計算できず）。次点は層転送の高速化を先に用意するか、Galois 群へ標的を変える |
...
| **【並行】測定量の事前予言: 2 次元の閉形式から代数的命題を 1 つ導く** | 2 次元の既知の閉形式から、$\widetilde R$ について**測定の前に**検証可能な代数的命題（全称命題）を 1 つ導出する。**導出できないと分かった場合は、その事実を記録してこの測定量を候補から落とす**（ゴール文書の否定判定「2 次元から事前に予言が出ない量は候補から落とす」）。落とすことも成果である **2026-08-17 17:36 に割った（本文未変更）**：(1) 閉形式の適用対象の特定——周期境界の単変数 $Z^{(2)}_L(X)$ の行列式表示（Kaufman）は境界応答多項式（辺ごとの変数・自由境界・遠い辺だけ 1）に直接は適用できない。適用できるのは平面グラフの任意の辺重み付き分配多項式の Pfaffian 表示（Kasteleyn–Fisher）であり、そこから出る候補命題は「$R^{(2)}_{L,L'}$ は成分が $\mathbb Z[x_e]$ の交代行列の Pfaffian の $2^{\#V^{(2)}_L}$ 倍と単項式倍で書ける（したがって $R^{(2)}_{L,L'}{}^2$ は行列式）」。(2) この候補を有限の $L,L'$（$L=3,4$）で SageMath により厳密検算し、通れば全称命題として本文へ書く（証明は Kasteleyn の定理を引くので既知性の確認と併せる）。(3) 通らなければ候補から落として記録する。次の並行 tick は (1) の候補命題を本文の主張として書き下すところから | todo（(1) の割り出しと候補の特定まで。本文未変更） |
...
- 2026-08-17: 「潰れる候補: 判別式は極限量に効かない、を判定できる形に定義する」の先頭の定義 `def_constant_coarse_graining_from_q_independent_invariant` を記述（status `todo` → `記述まで`）。
- 2026-08-17: 「極限量に対して必要でない粗視化を一つ同定する」を、割った 2 つが四層で揃ったことを確認して `done` にした。本流のセクション表に未完了が無くなった。
...
- 2026-08-17: 前 tick の Lean 必要十分版の後の本文・検証対応を `npm run check` の再実行でレビュー（締切のため最小）。不一致なし。
- 2026-08-17: 前 tick の Lean 必要十分版の合成の後の本文・検証対応を `npm run check` の再実行でレビュー（締切のため最小）。不一致なし。
