...
- 次の tick は、先頭未完了セクション「箱の定義から整数の算術を落とせるか」の SageMath 検証に
  着手する（有限二部後続系の小さい具体例で、色反転の対合、破れ辺集合の補集合化、破れ数の補数、
  多重度の回文性を一段ずつ厳密に確かめる）。
...
| 箱の定義から整数の算術を落とせるか | 箱を「有限集合＋方向ごとの後続の部分写像＋2 色の塗り分け」として定義し直し、回文性の証明が通るかを問う。通れば整数の加法・順序・座標和は本質的でなかったことになり、通らなければどこで整数が要るかが判明する（`docs/discussion/3次元Isingを可算側で書く/不要な構造を持ち込まない.md`） | 記述まで |
...
- 2026-08-14: 有限二部後続系を構造化テキストに定義し、二色の片側だけを反転する全単射により
  破れ辺集合が補集合へ移り、多重度が回文になることを証明した。整数の加法・順序・座標和を
  使わず、部分後続写像の単射性も回文性には不要であることを証明中で特定した。
- 2026-08-14: 帰無モデル「奇数周期では回文でない」の三番目の主張（多重度は回文でない）を
  Lean の具体版・必要十分版・導出で形式化し、新規 3 定理を検査対象へ登録した（登録は計 27 件）。
  これで同セクションの四層が揃い、完了した。
- 2026-08-14: 帰無モデル「奇数周期では回文でない」の二番目の主張
  （奇数周期では全周期辺を破れない）を Lean の具体版・必要十分版・導出で形式化し、
  新規 3 定理を検査対象へ登録した（登録は計 24 件）。
- 2026-08-14: 帰無モデル「奇数周期では回文でない」の最初の主張（定数配位は周期辺を破らない）を、
...
- 2026-08-14（整数算術を落とす記述の tick）: 前 tick で追加した「奇数周期では多重度は回文でない」
  の Lean 具体版・必要十分版・導出を人手証明と突き合わせた。前二主張から得る二つの自然数を
  「一方は 1 以上、他方は 0」だけへ落とす仮定、等しいと仮定して得る矛盾、具体版への特殊化、
  入口 import と検査登録に不備はなく、修正なし。
- 2026-08-14（奇数周期の Lean 多重度非回文 tick）: 前 tick で追加した全辺破れ不在の Lean 3 ファイルを
  人手証明の該当ブロックと突き合わせた。一周の点と辺の定義、末尾で 0 へ巻き戻る第二端点、
  全辺破れから隣接値の不一致を得る段、奇数個の ±1 の輪の積の鎖（(-1)^L と二乗の同時成立）、
  必要十分版の仮定の理由、導出、検査登録に不備はなく、修正なし。
- 2026-08-14（奇数周期の Lean 全辺破れ不在 tick）: 前 tick で追加した定数配位の Lean 3 ファイルを
  人手証明の該当ブロックと突き合わせた。周期辺・周期端点写像・破れ数・多重度の具体定義、
  定数配位が破れ数 0 の水準集合の証人になる論法、必要十分版の仮定、導出、検査登録に不備はなく、修正なし。
