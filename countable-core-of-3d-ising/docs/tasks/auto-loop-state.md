...
| 辺変数を 1 に置かない境界応答多項式の各辺変数についての次数は高々 1 | 任意の $e_0\in E_L$ について、$\widetilde R_{L,L'}$ の $X_{e_0}$ についての次数が高々 1 であること（各配位の単項式が相異なる不定元の集合上の積であること、有限和の次数が各項の次数の最大値以下であること）を示す。真の依存（次数がちょうど 1）の上半分。**2026-08-16 に「増えた辺の変数に真に依存する」から割り出した** **2026-08-16 に記述**（`claim_full_boundary_response_degree_at_most_one`） | 記述まで |
...
- 2026-08-16: 「辺変数を 1 に置かない境界応答多項式の外箱依存性」の Lean 必要十分版と導出を形式化した（可換半環へ一般化。status `done`）。
- 2026-08-16: 「辺変数を 1 に置かない境界応答多項式の外箱依存性」の Lean 具体版を形式化した（環準同型による有限和の分配。status `Lean 具体版まで`）。
- 2026-08-16: 「辺変数を 1 に置かない境界応答多項式の外箱依存性」を SageMath で検証した（3 箱の有限和と代入の直接計算。status `記述と SageMath まで`）。
- 2026-08-16: 「境界応答多項式は外箱に依存しない」の Lean 必要十分版と具体版からの導出を形式化した（status `done`）。
- 2026-08-16: 「境界応答多項式は外箱に依存しない」の Lean 具体版を形式化した（安定性の二度適用と配位数の積。status `Lean 具体版まで`）。
- 2026-08-16: 「境界応答多項式は外箱に依存しない」を SageMath で検証した（3 箱の有限和の直接計算。status `記述と SageMath まで`）。

- 2026-08-16: 「境界応答多項式は外箱に依存しない」を記述した（安定性の二度適用と自然数冪の積。status `記述まで`）。
...
- 2026-08-16: 「境界応答多項式は外箱に依存しない」の SageMath 検証をレビュー（再実行 PASS、検証対応 18 件、check 72 ブロック）。不一致なし。
- 2026-08-16: 「境界応答多項式は外箱に依存しない」の記述をレビュー（禁止記号・無限体積語の走査、安定性の参照 2 箇所、2 冪の積の段）。不一致なし。

- 2026-08-16: `npm run check`（71 ブロック・相互参照 66 件）、検証対応 17 件、立場違反語走査を再実行し不備なしを確認した（修正なし）。
