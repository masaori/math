...
## 現在地
- 2026-08-19 14:34: 本流の未完了が尽きたため、ゴール文書の自由エネルギー密度についての最小性から、新標的「有限箱値の交差べき等式は極限量に対して十分である」を台帳へ追加した。二つの正有理数値と二つの正の箱点数について、各箱で一方を他方の点数だけ累乗した値が逆向きの累乗と等しければ、対応する正の乗根が一致するため極限量も一致する、という条件を有限整数算術で判定する。次 tick はこの可算側の条件と乗根一致を本文の一主張として記述する。並行は Pfaffian 予言の Lean 具体版。
...
| **【並行】測定量の事前予言: 2 次元の閉形式から代数的命題を 1 つ導く** | 2 次元の既知の閉形式から、$\widetilde R$ について測定前の全称命題を導く。偶部分グラフの有限和を記述・検証し、Kasteleyn–Fisher 表示の仮定を文献と照合した。2026-08-19 10:37 に、Fisher terminal graph の内部辺へ重み $1$、元の辺に対応する外部辺へ $(1+X_e)/(1-X_e)$ を置き、Kasteleyn 向き付けから得る Pfaffianへ $\prod_e(1-X_e)$ を掛ければ $2^{\#A}R^{(2)}_{L,L'}$ に符号と $2^{\#V}$ の因子を除いて一致する全称命題を本文へ記述した。正実数重みの絶対値等式は使わず、完全マッチングの符号一致と Pfaffian の有限展開だけを使う。$L'=1,L=2$ の terminal lattice では polygon–dimer 全単射を全列挙し、分母消去後の一致を多変数整数係数多項式として厳密検証した。 | 記述と SageMath まで（全称の予言命題と有限例検証が揃った。次は Lean 具体版） |
