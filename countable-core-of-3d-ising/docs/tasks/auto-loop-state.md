...
- 次の tick は、先頭未完了セクション「帰無モデル: 奇数周期では回文でない」の SageMath 検証を行う
  （小さい奇数・偶数の箱で周期族の多重度を全数列挙し、奇数で回文が崩れ偶数で保たれることを
  ZZ の厳密計算で確かめる）。
...
| 帰無モデル: 奇数周期では回文でない | 周期境界の族を定義し、奇数周期では二部性が崩れて回文性が成り立たないことを反例で示す（数値では確認済み。2026-08-14 に上のセクションから割り出した） | 記述まで |
