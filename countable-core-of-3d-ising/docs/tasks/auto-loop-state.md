...
## 現在地
- 2026-08-20 05:08: 本流「交差べき等式は極限量に対して必要でない反例」の分割済み先頭を閉じ、評価点の厳密判定と破れの主張の記述を終えた。評価点 $q=2$・箱 $L=2$ で $Z_2(2)=36450=2\cdot3^6\cdot5^2$、$Z_3(2)=942223653336523266=2\cdot(\text{奇数})$ を厳密計算し、素因数 $2$ の指数が $27\ne8$ になることから $Z_2(2)^{27}\ne Z_3(2)^{8}$ を主張 `claim_shifted_free_family_cross_power_equality_fails_at_two` として本文へ記述した（すべて自然数の有限算術。habitat は可算側で脱出なし。$q=1$ は両辺一致で使えないことも厳密確認）。着手前レビューは `npm run check` 再実行（116 ブロック・221 参照すべて解決）で修正なし。記述後の check は 117 ブロック・225 参照すべて解決、PDF 35 ページ、linkage 44 件、Lean build・sorry 検査通過。次 tick はこの破れの主張と既存の末尾ずらしの極限一致を束ねる反例主張の記述、そのあと SageMath 検証へ。並行はまとめ締切のため見送り（次は Fisher 外部辺の有限積の等式を全完全マッチング重み和へ接続する特殊化）。
...
- 2026-08-20 02:03: 交差べき等式から乗根列の項別一致を得る補題と実数列の収束移送を合成し、最終束ね主張の Lean 具体版を閉じた。並行では各外部辺の分母消去等式を有限積へ束ねた。
