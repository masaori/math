...
## 現在地
- 2026-08-18 02:03: 本流「判別式が極限量に効かないことの判定」の先頭小分けを完了した。自由境界の高速 butterfly 層転送を法 $65537$ 上の一括計算へ拡張し、145 点の値から $Z_4\bmod65537$ を Lagrange 補間した結果、次数 144 が保たれ、$Z_4(1)=2^{64}$、$\gcd(Z_4,Z'_4)=1$ を確認した。したがって $\mathrm{disc}(Z_4)\ne0$ が $\mathbb Z$ 上で従う。レビューは直近の高速核と本文・検証対応を点検し、`npm run check`（99 ブロック・163 参照）が通過、修正なし。次は「ずらした自由族 $Z'_L:=Z_{L+1}$ が判別式を潰れる部分として示す反例」の本文主張ブロックを書き、check の対象ラベルを付け替える。
