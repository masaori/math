...
## 現在地
- 2026-08-19 15:33: 本流「有限箱値の交差べき等式は極限量に対して十分である」の先頭、交差べき等式から正の乗根一致までを本文へ記述した。正有理数 $A,B$ と正の自然数 $N,M$ について、有限整数算術で判定できる $A^M=B^N$ から、二つの正の乗根の $NM$ 乗が一致し、正の自然数乗の単射性により $A^{1/N}=B^{1/M}$ と結論する。本 tick のレビューは `npm run check` と直前の零点集合反例・Pfaffian 予言の照合で修正なし。次 tick の本流はこの主張の SageMath 検証、並行は Pfaffian の一定符号有限展開の Lean 形式化。
...
| **【並行】測定量の事前予言: 2 次元の閉形式から代数的命題を 1 つ導く** | 2 次元の既知の閉形式から、$\widetilde R$ について測定前の全称命題を導く。偶部分グラフの有限和を記述・検証し、Kasteleyn–Fisher 表示の仮定を文献と照合した。2026-08-19 10:37 に、Fisher terminal graph の内部辺へ重み $1$、元の辺に対応する外部辺へ $(1+X_e)/(1-X_e)$ を置き、Kasteleyn 向き付けから得る Pfaffianへ $\prod_e(1-X_e)$ を掛ければ $2^{\#A}R^{(2)}_{L,L'}$ に符号と $2^{\#V}$ の因子を除いて一致する全称命題を本文へ記述した。正実数重みの絶対値等式は使わず、完全マッチングの符号一致と Pfaffian の有限展開だけを使う。$L'=1,L=2$ の terminal lattice では polygon–dimer 全単射を全列挙し、分母消去後の一致を多変数整数係数多項式として厳密検証した。2026-08-19 14:35 に Lean 具体版を二つへ分割した。先頭は Pfaffian の有限展開で完全マッチングの符号が一定なら符号付き有限和が一つの符号とダイマー有限和の積になる補題、次に Fisher terminal graph への特殊化・polygon–dimer 全単射・分母消去を束ねる。 | 記述と SageMath まで（次は割った先頭の Lean 形式化） |
