...
- **「小さい箱での厳密測定」の対象が本文上で未定義。** 「2 次元からの事前予言」は四候補を落としたが
  「残った量」を書いていない。着手前に、測定対象を確定するセクション（2 次元の有限式から 3 次元自由
  境界族について具体的な全称命題を導ける量があるかを列挙し、無ければ「厳密測定の対象は無い」と
  本文に書き、台帳の同セクションを閉じるか割り直す）を台帳へ追加してから進む。
  候補として残りうるのは、次元によらず定義から得られる整係数分配多項式そのものと零点、および
  帰無モデルで既に示した性質（回文性・非負性・台の両端・偶数性）を超える構造の有無である。
