...
## 現在地
- 2026-08-18 18:35: 開始が締切の 8 分前。レビューは `npm run check`（105 ブロック・189 参照、すべて解決）の再実行で前 tick と一致、修正なし。本文は変えず、本流「有限箱量の列の有限接頭部を忘れる粗視化は極限量に対して十分である」の先頭を台帳で二つへ割った：先頭は「尾部同値関係（ある $N$ 以上のすべての添字で項ごとに一致）の定義ブロックの記述」、次に「十分性の主張ブロック（尾部同値な二列は極限量が存在すれば一致する）の記述」。PDF 再生成。次 tick は割った先頭の定義ブロックから着手する。並行（Pfaffian 予言の本文記述）は締切のため見送り。
