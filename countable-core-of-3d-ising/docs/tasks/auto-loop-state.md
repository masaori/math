...
| **【本流】健全性の橋: 有限の主張から極限量の言明へ渡す定理** | 有限箱ごとに成り立つ主張の族から、上で定義した極限量についての言明を導く定理を自作する。**脱出は一度きり**で、定理の仮定は有限箱の言明だけにする。これが架かってはじめて「十分／必要でない」を判定できる（判定はどちらも極限量を参照するため） | todo（上の 2 つに割った残り。両方が済んだら中身を再点検して残余があれば書く） |
| **【本流】健全性の橋: 十分性と必要でないことの判定を極限量へ具体化する** | ゴール文書の定義（すべての $L$ で粗視化の値が一致すれば極限量が一致＝十分／値が違っても極限量が一致する例がある＝必要でない）を、上で定義した極限量に対して書き下す。**潰れる部分を一つ厳密に同定することが、このゴール設定の最初の成果である** | todo |
| **【並行】測定量の事前予言: 2 次元での対応物を書き下す** | 辺変数を 1 に置かない境界応答多項式 $\widetilde R_{L,L'}$ の 2 次元版を、同じ定義（内箱・外箱・辺変数）で書き下す。**閉形式に頼る前に定義の対応を付ける**。有限の箱にとどめる | todo |
...
- 2026-08-16: 「増えた辺の変数を 1 に置いた境界応答多項式は配位ごとの元の外箱の破れ辺の単項式の有限和」を SageMath で検証した（4096 項 PASS。status `記述と SageMath まで`）。
