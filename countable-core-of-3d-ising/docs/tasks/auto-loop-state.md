...
## 現在地
- 2026-08-23 02:32: 本流「素指数の符号だけを見る粗視化は極限量に対して十分でない」の SageMath 検証を書いて通し、記述と SageMath の二層まで進めた。新設した検証 `sagemath/check/sign-of-prime-exponents-is-not-sufficient-for-limit-quantity/` は、符号を与える写像そのものが本文の定義どおりであること、素数 $2$ での素指数が $1$ と $2$ で異なる一方その符号が一致すること、$2$ 以外の素数では素指数も符号もともに $0$ であること、二つの列が定数列で候補値との差が $0$ であること、二つの候補値へ幅 $1/2$ で同時に近づけないこと、そして符号への潰しが高さ $N\ge2$ での切り詰めより粗いことを `ZZ`/`QQ` の厳密計算だけで確かめて全 PASS。`npm run check` 127 ブロック・254 参照すべて解決、linkage 52 件。レビューは前 tick で記述した本文と既存の大きさ切り詰めの検証を読み比べ、直すところは見つからなかった。**次 tick はこの標的の Lean 具体版から進める。**
