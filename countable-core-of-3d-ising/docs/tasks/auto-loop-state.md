...
- 次の tick は、先頭未完了セクション「箱の定義から整数の算術を落とせるか」の Lean 具体版に
  着手する（1 tick 1 主張の上限に従い、まず主張「回文性に整数の算術は要らない」の具体版から。
  有限二部後続系を有限集合・方向ごとの部分写像・二色塗り分けとして定義し、
  人手証明の四段と 1 対 1 に対応させる）。
...
| 箱の定義から整数の算術を落とせるか | 箱を「有限集合＋方向ごとの後続の部分写像＋2 色の塗り分け」として定義し直し、回文性の証明が通るかを問う。通れば整数の加法・順序・座標和は本質的でなかったことになり、通らなければどこで整数が要るかが判明する（`docs/discussion/3次元Isingを可算側で書く/不要な構造を持ち込まない.md`） | 記述と SageMath まで |
