...
| 外箱の拡大に対する境界応答多項式の安定性 | 同じ内箱 $V_{L'}$ に対する二つの外箱 $V_L\subset V_{L''}$ で、$V_{L'}$ に接する $E_{L''}$ の辺がすべて $E_L$ に含まれる（内箱の近傍が $V_L$ に収まる）とき、変数集合が $A_{L,L'}=A_{L'',L'}$ で一致し、$R_{L'',L'}=2^{\#V_{L''}-\#V_L}\,R_{L,L'}$ が成り立つことを、配位の有限和を $V_L$ 上の制限と外側の値へ分割する 1 論法で示す。有限の箱の比較にとどめ、無限体積の語は使わない | 記述まで |
...
- **次は「外箱の拡大に対する境界応答多項式の安定性」の SageMath 検証を行う**（記述は 02:45 の tick で
  済み。例えば $L'=1,L=2,L''=3$ の自由境界箱で、変数集合の一致と $R_{L'',L'}=2^{\#V_{L''}-\#V_L}R_{L,L'}$ を
  `ZZ` 上の有限和で一行ずつ確かめる）。四層が揃ったあとは、境界応答多項式が外箱に依存しないこと（測定量として極限情報を持たないこと）を
