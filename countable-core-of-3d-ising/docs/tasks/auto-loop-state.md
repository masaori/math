...
| 測定量の選び直し | 2 次元の有限式から事前の全称命題を得られなかった単変数の四候補に代わり、ゴール設定が要求する多変数分配多項式と箱の包含写像から、極限で潰れるかを問える測定量を一つに絞る | Lean 具体版まで |
...
- **次は境界応答多項式の Lean 必要十分版と具体版からの導出を行う。** 有限な辺型上の
  多変数多項式から、証明が実際に使う環準同型と指定した変数像だけを残し、具体版を特殊化する。
