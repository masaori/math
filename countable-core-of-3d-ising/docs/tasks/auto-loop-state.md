...
| **【本流】健全性の橋: 粗視化 $q\mapsto\varepsilon_{L,q}(\mathcal Z_L)$ は極限量に対して十分である** | 粗視化の値の一致 ⇒ 分配多項式の値の等式（`claim_coarse_graining_values_agree_implies_partition_values_agree`）と有限箱の等式の族の移送（`claim_finite_box_equalities_transfer_to_limit_quantity`）を繋いで、上の定義の意味での十分性を主張として書く | todo |
| **【本流】健全性の橋: 極限量に対して必要でない粗視化を一つ同定する** | 上の定義の意味で「値が違っても極限量が一致する」例を一つ作る（候補は判別式・Galois 群、または $q$ と $q'$ で $Z_L$ の値が異なるが乗根の極限が一致する組）。**潰れる部分を一つ厳密に同定することが、このゴール設定の最初の成果である** | todo |
