...
| 測定量の選び直し | 2 次元の有限式から事前の全称命題を得られなかった単変数の四候補に代わり、ゴール設定が要求する多変数分配多項式と箱の包含写像から、極限で潰れるかを問える測定量を一つに絞る | 記述まで |
...
- **次は境界応答多項式の SageMath 検証を行う。** $L'=1,L=2$ の自由境界箱について、
  多変数分配多項式の有限和、内箱に接しない辺変数を 1 に置く代入、加法・乗法・単位元の保存、
  得られる境界応答多項式を `ZZ` の多項式環で一行ずつ確認する。
