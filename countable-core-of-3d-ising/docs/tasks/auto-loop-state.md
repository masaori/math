...
## 前進の記録（新しい 5 件まで）
- 2026-08-22 14:52: 反例標的の Lean 具体版を閉じ、status を記述・SageMath・Lean 具体版までへ上げた。Z_2(2)・Z_3(2) を 2 で割った奇数部分への分解で表し、27 乗・8 乗後の偶奇の食い違いから交差べき等式の破れを示し、既存の末尾ずらし極限定理と束ねた。次は Lean 必要十分版。
- 2026-08-20 07:34: 反例標的の束ね主張の SageMath 検証を閉じ、status を記述と SageMath までへ上げた。末尾ずらしの項別一致（値・箱点数・特徴づけの有限べき等式の同一性）と破れの結論の独立再計算を整数上で全 PASS、対象ラベル付けと linkage も同時に閉じた。次は Lean 具体版。
- 2026-08-20 07:03: 締切 8 分前開始のためレビューと PDF 再生成のみ。反例標的の残りを「束ね主張の check 新設」と「対象ラベル付けと linkage 登録」の二つへ割った。次は割った先頭から。
- 2026-08-20 06:33: 反例標的の破れの主張の SageMath 検証を閉じた。評価点 2・箱 2 の二つの分配多項式値の厳密評価、素因数 2 の指数 27 対 8、交差べきの不一致、評価点 1 が使えないことを整数上で全 PASS。次は束ね主張側の検証と linkage。
- 2026-08-20 06:02: 締切 8 分前開始のためレビューと PDF 再生成のみ。反例標的の SageMath 検証を「評価点 2 での破れの check 新設」と「束ね主張の検証と linkage」の二つへ割った。次は割った先頭から。
