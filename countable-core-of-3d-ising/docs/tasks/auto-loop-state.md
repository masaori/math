...
- 次の tick は、先頭未完了セクション「帰無モデル: 分配多項式の 1 での値」の SageMath 検証を行う。
...
| 帰無モデル: 分配多項式の 1 での値 | 有限和で $Z_L(X)\in\mathbb Z[X]$ を定義し、破れ数ごとの水準集合が配位集合を分割することから $Z_L(1)=2^{\#V_L}$ を示す | 記述まで |
| 帰無モデル: 分配多項式の係数の非負性 | 各係数が多重度 $\Omega_L(m)\in\mathbb N$ であることを示す | todo |
...
- 2026-08-14: 有限二部後続系の多重度回文性を Lean の具体版で形式化した。
  人手証明の定義と五段を同じ順で実装し、最終定理を未証明公理なしで検査対象へ登録した。
  後続写像の単射性は系の定義には含むが、回文性の証明では使わない。
- 2026-08-14: 有限二部後続系の回文性の SageMath 検証を追加した
  （`sagemath/check/bipartite-successor-palindrome/`）。証明の四段と回文性を、
  道・整数の箱 L=2・単射でない星の三つの系の全数列挙で厳密に確かめ、
  箱の多重度が自由境界の検証と一致することも校正した。
...
- 2026-08-14（有限二部後続系の Lean 必要十分版 tick）: 前 tick の Lean 具体版を
  人手証明と突き合わせた。有限二部後続系の定義（部分集合上の単射・二色条件）、
  五段の対応コメント、単射性を証明で使わないこと、検査への登録に不備はなく、修正なし。
- 2026-08-14（有限二部後続系の Lean 具体版 tick）: 前 tick の SageMath 検証を本文と
  自由境界の既存検証へ突き合わせた。台帳と overview は「箱 L=2 の多重度が一致」と
  記録していたが、検証コードは辺数と配位総数しか比較していなかったため、自由境界で得た
  多重度の全係数との一致を明示的に検査するよう修正した。修正後は係数ごとの一致を確認した。
- 2026-08-14（有限二部後続系の SageMath tick）: 前 tick で追加した「箱から整数の算術を落とす」の
  記述（定義四つと主張）を突き合わせた。対合性・破れの反転・補集合の元数・水準集合の全単射の
  各段、ラベル参照、habitat、禁止された脱出の不使用に不備はなかった。主張文の
  「整数の算術は要らない」は、箱の二色塗り分けが本文の既証明（辺の両端の偶奇が異なる）で
  担保されているため過剰主張ではないと判断し、修正なし。
