...
| 各配位の単項式は増えた辺の変数を 1 に置く代入で単項式に写る | $V_L\subset V_{L''}$ と配位 $\sigma\colon V_{L''}\to\{-1,1\}$ に対し $\pi_{L'',L}\bigl(\prod_{e\in B(\sigma)}X_e\bigr)=\prod_{e\in B(\sigma)\cap E_L}X_e$（環準同型が有限積を保つこと、互いに素な有限集合上の有限積の分割、不定元の行き先の場合分けの 1 論法）。$\pi_{L'',L}$ が $\widetilde R_{L'',L'}$ の各項を単項式へ写し、外箱依存性の主張の和の分割の各項の形を与える。有限の箱の比較にとどめる。**2026-08-16 に「$\widetilde R$ の単項式構造」から割り出した（主標的表に todo が無かったため）** **2026-08-16 に記述**（`claim_full_boundary_response_monomial_maps_to_monomial_under_outer_edges_to_one`。`npm run check` 82 ブロック・相互参照 102 件） | 記述まで |
