...
| 測定量の選び直し | 2 次元の有限式から事前の全称命題を得られなかった単変数の四候補に代わり、ゴール設定が要求する多変数分配多項式と箱の包含写像から、極限で潰れるかを問える測定量を一つに絞る | todo |
...
- 2026-08-15: 「2 次元からの事前予言」の本文と台帳を再照合し、四候補を落とした後の測定対象が
  未定義だった不備を修正した。候補の列挙が四つで尽きることと、残る対象が無いことを明記した。
- 2026-08-15（レビューのみの tick）: 「2 次元からの事前予言」は四候補を落としただけで残った量を
  明示せず、次の「小さい箱での厳密測定」の対象が本文上で未定義であることを見つけた。
  本文の修正は時間内に収まらないため行わず、引き継ぎへ論点として記録した。
- 2026-08-15: 「零点と係数データが決める多項式」の本文と Lean 具体版・必要十分版・導出を再照合した。
  二つの反例と有限積復元は同順で、必要十分版に過剰な構造も禁止された脱出も無かった（修正なし）。
- 2026-08-15（レビューのみの tick）: 本文全体を禁止された脱出の記号・無限体積の語で機械的に走査し、
  構造化テキストの検査と検証対応の検査を再実行した。いずれも通り、不備なし。
- 2026-08-15（レビューのみの tick）: 「零点と係数データが決める多項式」の本文・Lean 三ファイルを
  再照合した。反例二つ、最高次係数と重複度込み零点からの復元、一意性が同順で対応し、不備なし。
- 2026-08-15（レビューのみの tick）: 「零点と係数データが決める多項式」の Lean 必要十分版と具体版からの
  導出を照合した。仮定は具体版が使う復元表示と反例の相違だけで、過剰な構造は無い（不備なし）。
- 2026-08-15（零点と係数データが決める多項式の Lean 必要十分版 tick）: 具体版の二つの反例と
  有限積復元を本文と照合し、証明順序・住処・禁止された脱出の不使用を確認した。不備なし。
- 2026-08-15: 「零点と係数データが決める多項式」の Lean 具体版を人手証明と照合した。反例二つ・
  有限積表示・一致の各段が対応し、mathlib の一般論への丸投げは無かった（不備なし）。
- 2026-08-15: 「零点と係数データが決める多項式」の本文と SageMath 検証 13 件を照合した。
  二つの反例と有限積表示の各段が対応し、禁止された脱出・浮動小数・空虚な検査は無かった（不備なし）。
- 2026-08-15: 「周期族から整数の算術を落とす」の本文、Lean 具体版、必要十分版、導出を照合した。
  奇数輪の有限積から端点多重度の不一致へ至る順序、具体版から必要十分版への仮定の縮約、
  禁止された脱出の不在を確認し、不備は見つからなかった。
- 2026-08-15（周期族の Lean 必要十分版 tick）: 前 tick の Lean 具体版を人手証明と再照合し、
  下端の正値性、奇数輪による全辺破れの否定、上端の零、端点多重度の不一致が同順で対応して
  いることを確認した。不備なし。
- 2026-08-15（周期族のレビューのみ tick）: 「周期族から整数の算術を落とす」の Lean 具体版を人手証明と
  照合した。四つの補題・定理が本文の四段と同順で対応し、奇数軌道の矛盾は既存の奇数閉路補題の再利用で
  mathlib への丸投げは無い。不備なし。
- 2026-08-15: 「周期族から整数の算術を落とす」の SageMath 検証 `check_odd_orbit_product.sage` が、
  奇数軌道では前件（全軌道辺が破れる）を満たす配位が存在しないため assert が一度も走らず空虚に
  通っていた。恒等式 $(-1)^L=-1$ の直接確認と、前件を満たす配位が 0 個であることの全配位確認に
  書き換え、overview の検証内容の記述も合わせた。
- 2026-08-15（周期族の SageMath tick）: 前 tick の「周期族から整数の算術を落とす」の記述を
  再照合し、禁止された脱出・未宣言記号・式変形の省略・参照の不備が無いことを確認した。
- 2026-08-15（周期族のレビューのみ tick）: 「周期族から整数の算術を落とす」の記述を照合した。
  軌道長一定の仮定のうち証明が使うのは $s_i^{\circ L}(a)=a$ だけであり、有限積の各段に根拠が
  付いている。不備なし。
- 2026-08-15（周期族から整数の算術を落とすの記述 tick）: 前 tick の「既約分解の型が決める
  零点の最小多項式次数」の Lean 必要十分版と具体版からの導出を再照合した。必要十分版の仮定は
  具体版が使う有限多重集合の濃度と次数写像だけであり、不備なし。
- 2026-08-15（レビューのみの tick）: 「既約分解の型が決める零点の最小多項式次数」の Lean
  必要十分版の多重集合結合・個数の段と、具体版最終定理の導出を照合した。仮定は具体版が使う
  性質だけで、不備なし。
- 2026-08-15（レビューのみの tick）: 「既約分解の型が決める零点の最小多項式次数」の Lean
  必要十分版が、具体版の最終定理（積多項式の零点多重集合の個数公式）を抽象化しておらず、
  導出もその段を経由していないことを確認した。次の tick で必要十分版へ追加する。
- 2026-08-15（既約分解の型が決める零点の最小多項式次数の Lean 具体版 tick）: 前 tick の
  有限積の零点結合補題を本文と照合し、因子の冪の零点を指数回反復して有限積へ結合する段に
  不備がないことを確認した。未完了だった個数公式への接続をこの tick で追加した。
- 2026-08-15（既約分解の型が決める零点の最小多項式次数の Lean 具体版途中の tick）:
  前 tick の差し戻しを再点検し、最終定理が積多項式そのものの根の多重集合を扱っていないことを確認した。
  差し戻しは正しく、その欠落を埋める有限積の根の結合補題を追加した。
- 2026-08-15（レビューのみの tick）: 前 tick の差し戻し判定を Lean 最終定理の本文で再確認した
  （零点対の数え上げのみ・重複度補題と零点非共有補題は未使用）。不備なし。検証も全通過。
- 2026-08-15（レビューのみの tick）: 「既約分解の型が決める零点の最小多項式次数」の Lean
  最終定理は、因子ごとにタグ付けした零点対の有限数え上げまでしか結合していなかった。
  本文の第四段で使う因子間の零点非共有と、因子指数が積多項式での代数的重複度になる段が
  最終定理から欠け、対応する既存補題も未使用なので、Lean 具体版完了の判定を取り消した。
- 2026-08-15（既約分解の型が決める零点の最小多項式次数の Lean 具体版完了 tick）: 前 tick の
  零点非共有補題を本文の第四段と突き合わせ、最小多項式の一意性による証明順序と一致し、禁止された
  脱出も無いことを確認した。不備なし。
- 2026-08-15（既約分解の型が決める零点の最小多項式次数の Lean 具体版途中の tick）: 前 tick の
  既約因子の各零点の重複度が高々 1 である補題を本文の分離性の段と突き合わせ、不備が無いことを確認した。
- 2026-08-15（既約分解の型が決める零点の最小多項式次数の Lean 具体版途中の tick）: 同ファイルで
  `namespace Ising3DCut.NullModel` が閉じずに入れ子で再宣言され、補題二本が二重名前空間に
  置かれて no-sorry 検査の登録名と実体が食い違っていた。入れ子を除去し登録を実体に合わせた。
  分離性の補題の内容自体は本文と一致（不備なし）。
- 2026-08-15（既約分解の型が決める零点の最小多項式次数の Lean 具体版途中の tick）: 前 tick に
  追加された最小多項式次数の補題を本文の第一段と突き合わせた。既約性・モニック性・零点の仮定と
  最小多項式の一意性による結論が対応し、禁止された脱出も無い。不備なし。
- 2026-08-15（既約分解の型が決める零点の最小多項式次数の Lean 具体版途中の tick）: 前 tick の
  記述と SageMath 検証を再点検し、最小多項式次数・相異なる零点数・代数的重複度・因子間の
  非共有の対応と、禁止された脱出が無いことを確認した。不備なし。
- 2026-08-15（レビューのみの tick）: 「既約分解の型が決める零点の最小多項式次数」の記述と
  SageMath 検証を突き合わせた。証明の四段（最小多項式の一意性・標数 0 での分離性・
  因子指数と代数的重複度・因子間で零点が共通しないこと）に検証項目が対応し、禁止された脱出は
  無い。sage 再実行 ALL PASS・npm run check・linkage 13 件も通過。不備なし。
...
- **次は測定量を選び直す。** 単変数の四候補は 2 次元の有限式から事前の全称命題を得られず、
  小さい箱での厳密測定の対象から全て落ちた。ゴール設定が要求する多変数分配多項式と箱の包含写像を
  一次データとして、極限で潰れるかを問える量を一つに絞る。
  次の tick で最初に検討する候補（ゴール設定の「族は境界条件で添字づける」の項から。決定ではない）：
  辺ごとに変数を持つ多変数分配多項式 $Z_L(X_e)$ と、箱 $L'\subset L$ の包含に伴う
  「$L'$ の外側の辺変数を $1$ に置く」写像を定義し、$Z_{L}$ から得た像と $Z_{L'}$ を比べる。
  一致しないなら、その差（境界の外側の和で潰れる部分）が「箱の包含で潰れる情報」の最初の
  厳密な候補になる。定義（1 ブロック）と、包含写像が環準同型であること（1 論法）で 1 セクションに割る。
