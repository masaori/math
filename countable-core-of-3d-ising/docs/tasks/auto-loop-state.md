...
- 次の tick は、先頭未完了セクション「帰無モデル: 分配多項式の 1 での値」の Lean 具体版を行う。
...
| 帰無モデル: 分配多項式の 1 での値 | 有限和で $Z_L(X)\in\mathbb Z[X]$ を定義し、破れ数ごとの水準集合が配位集合を分割することから $Z_L(1)=2^{\#V_L}$ を示す | 記述と SageMath まで |
