...
3. **二本立てで進める**（2026-08-16 のユーザー判断。runbook「二本立てで進める」が正本）。
   本流は「健全性の橋」——極限量を定義し、有限箱の主張から箱の大きさの極限を一度だけ使って
   極限量の言明へ渡す定理を自作する。並行ストリームは「測定量の事前予言」——辺変数を 1 に
   置かない境界応答多項式について、2 次元の閉形式から導かれる代数的命題を測定の前に導出する
   （導出できなければ候補から落とし、その記録を残す。それも成果である）。
   本流 1 件のあと、時間が残っていれば並行を 1 件進める（並行は上限に数えない）。
   **セクション表に未完了が無くなっても、小主張を自作して埋めてはならない。**
   戻り先は docs/discussion/3次元Isingを可算側で書く/可算コアの同定とは何か.md であり、
   そこの「最初の三手」「極限側で問う言明」「否定判定」から標的を引き直す。
   実測 2026-08-16: この規定が無く、多変数分配多項式の初等的な性質を自作して四層で
   証明し続けた。証明の本数は増えるがゴールへの距離は縮まない。
...
8. 本流を 1 セクション進めたら止まる（並行ストリームの 1 件を除く）。
...
CODEX_BLOCKED_MARK="$LOG_DIR/codex-blocked-until"
...
if [ "$agent" = "claude" ]; then
  if [ ! -d "$CLAUDE_TICK_CONFIG_DIR" ] || [ ! -s "$CLAUDE_TICK_TOKEN_FILE" ]; then
    ...
    # モデルは claude-sonnet-5。以前は claude-fable-5 だったが、このループ専用アカウント
    # （coding-agent-0004）の Fable 5 の枠が尽き、2026-08-20 07:36 の公開を最後に
    # 2 日以上すべての tick が 2 秒で異常終了し続けた（実測 60 回）。同じアカウントで
    # sonnet / opus / haiku は応答するのを確認したうえでの変更であり、実行時に別モデルへ
