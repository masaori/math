...
1. まず台帳の「現在の研究対象」に未完了項目があるかを確認する。
   未完了項目がなければ、終了済み候補を再レビューせず、ファイル変更・検査・
   PDF再生成・コミットを行わず正常終了する。新しい候補や補題を自動追加しない。
2. 未完了項目がある場合は、既存出力のレビューと修正を先に行う（飛ばして前進しない）。
   直したら、前進に入る前にコミットして push まで済ませる。
3. そのあと、台帳の「現在の研究対象」の未完了セクションを 1 つだけ進める。2 つ以上進めない。
...
4. 固定候補の評価は終了済みである。新しい対象が台帳に採用された場合は、
   その「現在の研究対象」だけを進め、過去の並行ストリームを再開しない。
   入力の適合性、候補の採否、存在条件への寄与または不足を記録して閉じる。
   一件を閉じたら小主張・評価点・別候補を自動追加しない。
5. このプロジェクトの立場を厳守する。許される非可算への脱出は箱の大きさの極限だけである。
   上限・下限・積分・微分・無限和・級数・指数関数・実対数・逆温度の記号を使わない。
   相・臨界温度・自発磁化などの無限体積の語を主張に使わない。
   立場を守れないと分かったら、黙って脱出せず、台帳へ論点を書いて報告して止まる。
6. 未完了対象を進めた場合は、検証（npm run check / build:pdf / sage / verify-check-linkage）を通す。
   検証が落ちたら本文を直す。検証を主張に合わせて緩めない。
7. 未完了対象を進めた場合は、台帳と MEMORY を更新し、main へ push して反映を確認する
