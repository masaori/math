...
2026-08-15 に「全スピン反転による多重度の偶数性」を記述した。全スピン反転が破れ辺集合と
破れ数を保つ対合であり、原点で値が必ず変わるため不動点を持たないことを示した。各破れ数の
水準集合は二元軌道へ分割されるので、各多重度は 2 と自然数の積になる。次の層は SageMath 検証である。
同日の次 tick は着手前レビューでこの記述に不備なしを確認し npm run check を再実行して通過したが、
残り時間が足りず SageMath 検証には着手しなかった（レビューのみ）。次 tick が SageMath 検証を行う。
