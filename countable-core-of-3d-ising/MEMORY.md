...
- 2026-08-18 18:35: 開始が締切 8 分前。レビュー（`npm run check` 再実行、105 ブロック・189 参照）修正なし。本文未変更、PDF 再生成。本流「有限接頭部を忘れる粗視化は十分」の先頭を二つへ割った：先頭は尾部同値関係（ある $N$ 以上の全添字で項ごとに一致）の定義ブロックの記述、次に十分性の主張ブロック（尾部同値な二列は極限量が存在すれば一致）の記述。次 tick は割った先頭から。並行は締切のため見送り。
