...
- 2026-08-17 06:45 tick: 本流「極限量に対して必要でない粗視化を一つ同定する」を 2 つに割った（先頭: 回文性から対称化した量 τ_L(q)=λ(Z_L(q))−(#E_L/2)λ(q) の q↔1/q 不変（有限箱の等式）、次: その箱の極限 α̃ に対して粗視化が必要でない）。対称化しない α には Z_L の単射性から反例が立たない論点を台帳に記録。本文変更なし。次は割った先頭の記述。
