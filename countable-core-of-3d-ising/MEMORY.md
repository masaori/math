...
分配多項式の代数的データの文献判定は 2026-08-14 に完了した。
Häggkvist ほか（2004）は 2 次元周期格子の厳密分配多項式計算に Galois 理論を使うが、
Ising 分配多項式自身の既約分解の型・判別式・Galois 群・零点の最小多項式次数を
箱の族として分類していない。同じ対象の定理は確認範囲では見つかっておらず、
主標的は維持する（非存在の主張ではない）。
同日、HTML と PDF の表題に 2 次元側の文言が残っていた誤りを修正し、
現ゴールの表題「3 次元 Ising 模型の可算コアを同定する」に統一した。
次は主標的の「帰無モデル: 二部性からの回文性」を記述する。
