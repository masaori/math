...
- 2026-08-24 05:33: 本流の記述層。有理点 2 では有限箱の量の列が定数列でないことを示した（`claim_finite_box_sequence_at_two_is_not_constant`）。$a_1(2)=2$、$a_2(2)>2$。有理点 1 で極限量の存在を閉じた定数列の経路が有理点 2 に使えないことが、観察ではなく定理になった。箱の大きさの極限は使っていない。次は同セクションの SageMath 検証。
