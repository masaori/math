...
- 2026-08-18 14:02: 本流の未完了が尽きたため、ゴール文書「極限側で問う言明」の自由エネルギー密度についての最小性から、新標的「ずらした自由族は有限箱の分配多項式値が極限量に必要でないことの反例」を台帳へ追加した。$q=1,L=2$ で $Z_2(1)=2^8\ne2^{27}=Z'_2(1)$ だが、正規化値と極限量は両族とも $2$。次 tick は本文の主張ブロックを記述する。レビューは既約分解型の本文・SageMath・Lean 二層を照合し、`npm run check`（103 ブロック・177 参照）で修正なし。並行は terminal lattice の有限例による Pfaffian 分母消去の SageMath 検証。
