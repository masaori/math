...
Lean では最初の主張「辺の両端の座標和の偶奇」と二番目の主張「奇数側だけ反転する写像は
全単射である」を、それぞれ具体版・必要十分版・導出で形式化済み（検査への登録は計 9 定理。
全単射の必要十分版は「値の反転が対合であること」だけを仮定する）。
次は同じセクションの三番目の主張「奇数側だけ反転する写像は各辺の破れを反転する」を
同じ三層で形式化する。
