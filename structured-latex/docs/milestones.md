...
- M3〜M8: **未着手**。次は M3。
- マイルストーンの枠外で先行して入ったもの: **リアルタイム Web プレビューのシステムへの吸収**
  （旧トップレベルアプリ `realtime-web-preview/` → `live-preview/`）。
  これは M5 / M6 の**達成ではない**。両者の関係を取り違えないよう、何が入って何が入っていないかを
  以下に明記する。
  ...
  | Web 出力器がシステム内のモジュールとして載る形が実証された | **済**。`live-preview/`（React + Vite + KaTeX） |
  | 解決済み文書からの HTML + KaTeX 表示、`ref` の相互リンク | **済**（M5 の一部に相当。ただし LAN 内の read-only プレビューとして） |
  | 目次でのナビゲーション（M5 が要求する「閲覧者が操作できる」の一部） | **未**。`resolve` は `outline` を作るが、画面は連番順の単一ページ（requirements.md F-11 は任意扱いで未実装） |
  | 公開 Web サイトの**生成**（静的成果物の出力、ホスティング前提の構成） | **未**。live-preview はローカル起動のサーバで、サイトを生成しない。M5 は未完了 |
  | ファイル保存 → SSE でブラウザ自動更新 | **済**（M6 の前提となる通知経路。domain-model.md の F10 と同型） |
  | **部分**アップロードによる更新（セグメント単位の受け入れ API、版と楽観ロック） | **未**。live-preview は入力ソースを FS から読むだけで受け入れ経路を持たず、更新時は文書全体を再取得する。M6 は未完了 |
  | 複数人が同時に同じサイトを見ながら更新する運用 | **未**。想定は信頼できる同一 LAN・認証なし・read-only（requirements.md §7） |
