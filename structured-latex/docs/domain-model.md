...
平坦なブロック列だけでは、見出しの親子、ブロックの節所属、補助要素がどの中心要素を支えるかを
文書順から推測するしかない。また `standing` は主張自身の絶対的属性なので、ある補題が下位論証の
中心でありながら上位論証には従属する、という相対的な主従を表せない。定義を主要要素にすることも
できない。

著者入力には `DocumentStructure → Section → ElementGroup` の木を置く。

- `Section.children` が節の入れ子と文書順を同時に表す。
- `ElementGroup.focus` は定理・主張・定義のいずれか一つである。
- 節への所属 `primary` / `supporting` が、その節に対する相対的な身分を表す。
- グループ内の `prerequisiteDefinition` / `supportingClaim` / `subgroup` / `explanation` / `figure`
  が、中心に対する役割を表す。
- 主定理・主な主張・主な定義は「`primary` グループの `focus`」から導出し、別々の真実を持たない。

保存・部分アップロードの単位は既存どおりセグメントのブロック列とする。
`compileDocumentStructure` が木を平坦なブロック列へ正規化すると同時に、節・グループ所属の索引を返す。
したがって更新契約を壊さず、出力器が所属を再推論する必要もない。既存文書の `heading.level` と
`standing` は移行中の互換入力として維持する。

大規模文書の著者入力は章ごとに `defineSection` してから章配列として公開する。生成器はこの構造exportを
直接読み、実行時の `compileDocumentStructure` と型レベルの `BlocksOfSections` で同じ文書順へ平坦化する。
章ごとに文脈型を確定するため、文書全体を単一リテラルとして構造比較する必要がない。
