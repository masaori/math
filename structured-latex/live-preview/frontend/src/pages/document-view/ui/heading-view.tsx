import type { ResolvedHeading } from '@structured-latex/system/domain-model'
...
/**
 * 章見出し1件を描画する。
 * 番号は解決の時点で確定している（採番方針より深い見出しは番号を持たないので null になる）。
 */
export const HeadingView = ({ heading }: { heading: ResolvedHeading }): ReactElement => {
  const className = `scroll-mt-16 first:mt-0 ${levelStyles[heading.level] ?? fallbackStyle}`
  const content = (
    <>
      {heading.number === null ? null : (
        <span className="mr-2 text-slate-500">{heading.number.display}</span>
      )}
      <TitleView title={heading.title} />
    </>
  )
  return heading.level === 1 ? (
    <h2 id={heading.anchor} className={className}>
      {content}
    </h2>
  ) : (
    <h3 id={heading.anchor} className={className}>
      {content}
    </h3>
  )
}
