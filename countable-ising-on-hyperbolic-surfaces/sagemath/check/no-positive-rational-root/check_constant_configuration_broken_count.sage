...
# 式ペア: b_G(sigma_up) = 0 および b_G(sigma_down) = 0
# 帰属: 有限集合、NN
...
for vertex_count, edges in examples:
    ...
    assert NN(len(up_broken_edges)) == NN.zero()
    assert NN(len(down_broken_edges)) == NN.zero()

print("RESULT: PASS — both constant configurations have broken-edge count zero")
