...
| `check_stage_difference.sage` | 2026-08-21 に構造化本文側の作業ディレクトリから、リポジトリ直下を前提とする相対パスで実行した | ERROR | 対象ファイルが見つからず、検算本体は実行されなかった |
| `check_stage_difference.sage` | 各自然数値重複度を `ZZ` へ明示的に移した差が重複度差写像と一致することを照合した後、二つの段別零点因子を `ZZ` 係数の有限台写像として構成し、その差を重複度差の形式的因子と照合する | PASS | 互いに素な零点台と共有零点 `-1` をもつ例の両方で整数係数の差が一致し、共有零点は相殺された |

## 備考

- 各段の零点台外では重複度を零延長し、有限台写像の係数ごとの差を取った。
- 構造化本文のレビューで見つかった自然数値重複度どうしの未定義な減法を、自然数から整数への標準単射を各項へ明示した整数の減法へ修正した。
