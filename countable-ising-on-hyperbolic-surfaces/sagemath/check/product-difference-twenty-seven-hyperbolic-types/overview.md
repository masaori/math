...
- 2026-08-25: レビューで双曲不等式の検算だけが `QQ` 上の除算を使っていることを検出し、本文と台帳に一致する自然数不等式 `2(p+q)<pq` の検算へ修正して PASS。
- 2026-08-25: 修正後の初回再検算は、構造化本文ディレクトリからリポジトリ相対パスを指定したため対象ファイルを検出できず、終了コード `2` で ERROR。リポジトリ直下から同じ四検算を復旧実行して全件 PASS。
