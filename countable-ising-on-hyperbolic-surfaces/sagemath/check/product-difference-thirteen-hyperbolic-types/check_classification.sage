...
assert positive_divisors == (NN(1), NN(13))
...
for p, q in degree_pairs:
    ...
    q_integer = ZZ(q)
    assert 2 * (p + q) < p * q
    assert (p_integer - 2) * (q_integer - 2) == ZZ(13)

print("RESULT: PASS — product difference thirteen gives exactly (3,15) and (15,3)")
