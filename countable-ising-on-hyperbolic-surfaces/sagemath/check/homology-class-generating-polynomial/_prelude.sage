...
def homology_class_map(chosen):
    return quotient_map(edge_subset_coefficient_map(chosen))


polynomial_ring = PolynomialRing(ZZ, names=("u", "v"))
u, v = polynomial_ring.gens()


def homology_class_generating_polynomial(homology_class):
    return sum(
        u ** (len(edges) - len(chosen)) * v ** len(chosen)
