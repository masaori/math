...
assert positive_divisors == (NN(1), NN(7))
...
for p, q in degree_pairs:
    ...
    q_integer = ZZ(q)
    assert 2 * (p + q) < p * q
    assert (p_integer - 2) * (q_integer - 2) == ZZ(7)

print("RESULT: PASS — product difference seven gives exactly (3,9) and (9,3)")
