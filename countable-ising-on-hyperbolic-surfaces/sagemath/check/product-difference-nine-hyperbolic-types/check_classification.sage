...
assert positive_divisors == (NN(1), NN(3), NN(9))
...
for p, q in degree_pairs:
    ...
    q_integer = ZZ(q)
    assert 2 * (p + q) < p * q
    assert (p_integer - 2) * (q_integer - 2) == ZZ(9)

print("RESULT: PASS — product difference nine gives exactly (3,11), (5,5), and (11,3)")
