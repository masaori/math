...
- 範囲: 出典を固定した位数 `168` の有限置換群から生成したセルデータを、既存の向き付け閉曲面・正則型・双曲型述語へ入力する
...
| ファイル | 検証内容 | ステータス | 結果 |
| --- | --- | --- | --- |
| `check.sage` | 出典の射影直線作用との一致、剰余類セル生成、辺の正逆二回出現、頂点リンク単巡回、一次骨格連結、面と頂点の incidence 数、双曲型有理不等式を厳密検算する | PASS | `24` 頂点、`84` 辺、`56` 三角形面からなる向き付け閉曲面セル分割が正則型 `{3,7}` と双曲型判定を満たした |
