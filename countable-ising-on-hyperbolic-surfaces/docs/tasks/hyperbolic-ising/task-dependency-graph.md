...
├── 有限群と剰余類からの格子生成
│   └── 商の塔と被覆写像
├── 独立プロジェクトの有限グラフ Ising 分配多項式を一次骨格へ適用
│   ├── 係数と素因数付値
│   └── Fisher 零点
└── F_2 セル鎖複体
    └── 高温展開のホモロジー類別
...
- 有限グラフ Ising 分配多項式の一般論は独立プロジェクトで完了済みとして参照する
...
- [固定有限商の係数・素因数付値・Fisher 零点](arithmetic/coefficients-valuations-fisher-zeros.md)
