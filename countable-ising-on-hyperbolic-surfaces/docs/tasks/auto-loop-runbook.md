...
この文書は、独立した Codex セッションが毎回まっさらな会話文脈で読み、研究を一つの主張だけ前進させるための手順書である。持続状態は `auto-loop-state.md`、研究のゴールはプロジェクトの `README.md`、依存関係は `hyperbolic-ising/task-dependency-graph.md` を正本とする。
...
- `auto-loop-state.md` の「実行待ち」の最初の一件だけを進める。作業単位ではなく、一ブロック一主張の境界で切る。
