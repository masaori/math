...
[next-research-target.md](next-research-target.md) の有限入力を採用した。積差ごとの分類には戻らない。
元の条件付き成果整理指示とエージェントが加えた許可待ちは、共通の研究停止の元指示と復旧条件で区別する。

## 一 tick のゴール

一 tick は台帳の「現在の研究対象」の最初の未達項目から有限データの一つの判定または一つの主張だけを進める。前提の有限判定が失敗したら証人を残して候補を閉じ、依存する後続項目を非適用とする。全項目が閉じたら終了し、型・入力群・被覆を自動で増やさない。
...
- `auto-loop-state.md` の「現在の研究対象」に未完了項目がなければ、終了済み候補の再レビューを
  台帳やMEMORYへ追記せず、本文検査・コミットも行わず止まる。項目がある場合だけ、最初の
  双曲曲面固有項目を進める。一般有限グラフ理論は進めない。
...
## 監督で具体化した対象の判定（2026-09-06）

[次の有限な研究対象](next-research-target.md) に、READMEの既存目的から導いた入力・採否・完了条件を記録した。
固定商と連結二枚被覆の有限判定は成立したが、二枚被覆は本文の正規商の塔へ適合しないため、
依存する比較を非適用として閉じた。別の型・入力群・被覆は採用していない。新しい対象が採用されるまでは
ファイル変更を行わず終了する。停止実体の変更と研究の進展は別に記録する。
