...
export default defineBlocks([
  ...
  goal("prime_exponent_encoding", "この節の入出力と主張", "入力は正有理数である。出力は有限台の素指数データからなる対数順序群値である。この節で確定するのは写像の定義と有限台による well-defined 性であり、準同型性や単射性は主張しない。", "Lambda"),
  ...
  goal("finite_quotient_hyperbolic_lattice", "この節の入出力と主定理", "入力は有限置換商と役割安定化部分群である。一般の入力について剰余類セルデータを定義する。主定理が有限証明書を与えるのは、八点上の固定した Q_{3,7} 入力から生成される一つのセル分割に限る。一般の有限置換商が閉曲面や双曲正則型を与えること、および曲率 -1 計量の存在・構成は主張しない。", "Q"),
