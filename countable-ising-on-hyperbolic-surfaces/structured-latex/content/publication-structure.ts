...
export default defineBlocks([
  ...
  goal("ising_partition_polynomial", "この節の入出力と主定理", "入力は有限双曲セル分割の一次骨格と二値スピンである。出力は破れ辺数の多重度と Ising 分配多項式である。主定理群は多重度による一般の係数表示と、固定した有限商格子の分配多項式を整係数多項式として与える定理である。偶部分グラフ多項式との恒等式は主張しない。", "ZPolynomial"),
  heading("fixed_lattice_arithmetic", 2, "固定格子の算術的不変量と Fisher 零点"),
  goal("fixed_lattice_arithmetic", "この節の入出力と主定理", "入力は固定した有限商格子の Ising 分配多項式である。出力は係数付値、既約分解、Fisher 零点、分解体、Galois 群、および有理根分離証明書である。主定理群は分配多項式の既約分解、分解体 Galois 群の全対称群同定、および分解体の厳密次数であり、後二者から非可解性と根式非可解性が従う。", "Qbar"),
