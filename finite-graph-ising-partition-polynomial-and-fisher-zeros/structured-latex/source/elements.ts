...
\quad\bigl(\because\ \overline{\mathbb Q}[x]\text{ の有限係数表示}\bigr).`),
      ...
      displayMath(String.raw`
        ...
        \quad
        \bigl(\because\ \text{形式微分の線形性}\bigr).
      `),
      displayMath(String.raw`
        D(fg)
        =
        ...
        D\!\left(a_i b_jx^{i+j}\right)
        \quad
        \bigl(\because\ \text{有限添字集合を }(i,j)=(0,0)\text{ と }i+j>0\text{ に分ける}\bigr).
      `),
      displayMath(String.raw`
        D(fg)
        =
        ...
        D\!\left(a_i b_jx^{i+j}\right)
        \quad
        \bigl(\because\ D(a_0b_0)=0\text{ である形式微分の定数則}\bigr).
      `),
      displayMath(String.raw`
        D(fg)
        =
        ...
        D\!\left(a_i b_jx^{i+j}\right)
        \quad
        \bigl(\because\ \overline{\mathbb Q}[x]\text{ の加法単位元則}\bigr).
      `),
      displayMath(String.raw`
        D(fg)
        =
        ...
        x^{i+j-1}
        \quad
        \bigl(\because\ \text{各 }i+j>0\text{ の項に形式微分の正次数単項式則を適用する}\bigr).
      `),
      displayMath(String.raw`
        D(fg)
        =
...
export type FiniteGraphBlock = (typeof flatFiniteGraphTheory)[number];
export type FiniteGraphLabel = FiniteGraphBlock["labels"][number];
export type FiniteGraphBlockWithLabel<L extends FiniteGraphLabel> = Extract<FiniteGraphBlock, { labels: readonly L[] }>;
...
export function selectFiniteGraphBlocksByLabel<const L extends readonly FiniteGraphLabel[]>(
  labels: L,
): { readonly [K in keyof L]: FiniteGraphBlockWithLabel<L[K]> } {
  ...
  return selected as { readonly [K in keyof L]: FiniteGraphBlockWithLabel<L[K]> };
}

export function selectFiniteGraphBlockByLabel<const L extends FiniteGraphLabel>(label: L): FiniteGraphBlockWithLabel<L> {
...
>(label: L, id: I): BlockWithId<I> {
  const matches = flatFiniteGraphTheory.filter(
