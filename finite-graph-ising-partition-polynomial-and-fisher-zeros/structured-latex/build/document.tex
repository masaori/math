...
\begin{remark}[有限グラフの入力の入出力]
入力: 二つの辺端ラベルと有限な頂点集合・辺集合。出力: ループを持たず多重辺を許す有限グラフ。主定義・主定理・主張: 有限グラフの入力。
...
\section{辺部分集合の偶奇と多項式}

\begin{remark}[辺部分集合の偶奇と多項式の入出力]
入力: 有限グラフ、その辺部分集合、独立な不定元 u,v。出力: 各頂点で偶次数となる辺部分集合とその二変数多項式。主定義・主定理・主張: 偶部分グラフ多項式。
...
\part{有限グラフ上の Ising 模型・分配多項式と Fisher 零点}

\section{スピンと配位}

\begin{remark}[スピンと配位の入出力]
入力: 有限グラフと二つの形式的スピンラベル。出力: スピン配位と一頂点反転。主定義・主定理・主張: 一頂点での配位反転が不動点を持たない対合であること。
...
\section{破れ辺と Ising 分配多項式}

\begin{remark}[破れ辺と Ising 分配多項式の入出力]
入力: 有限グラフ上のスピン配位と形式的不定元 x。出力: 破れ辺数の多重度と整数係数分配多項式。主定義・主定理・主張: 多重度による係数表示と全係数の偶数性。
...
\section{特別値 −1 と奇接続辺数}

\begin{remark}[特別値 −1 と奇接続辺数の入出力]
入力: Ising 分配多項式と頂点に接続する辺数の偶奇。出力: −1 における評価と一次因子による同値条件。主定義・主定理・主張: 零点 −1 による奇接続辺数頂点の特徴付け。
...
\begin{remark}[次数とカットの入出力]
入力: 破れ辺数と頂点二分割。出力: 分配多項式の次数の組合せ論的意味。主定義・主定理・主張: 分配多項式の次数が最大カット辺数に等しいこと。
...
\section{Fisher 零点と係数}

\begin{remark}[Fisher 零点と係数の入出力]
入力: 正の両端係数を持つ Ising 分配多項式。出力: 零点の積・対称式・冪和と係数比。主定義・主定理・主張: Fisher 零点冪和と逆数冪和の Newton 漸化式。
...
\section{全辺を横切る二分割と逆数対称性}

\begin{remark}[全辺を横切る二分割と逆数対称性の入出力]
入力: 有限グラフと、全辺二分割・係数対称性・多項式逆数対称性のいずれかの条件。出力: 係数・多項式・Fisher 零点の逆数対称性。主定義・主定理・主張: 多項式逆数対称性による全辺二分割の特徴付け。
...
\section{評価点から見た Fisher 零点}

\begin{remark}[評価点から見た Fisher 零点の入出力]
入力: 一つまたは二つの代数的評価点と分配多項式の係数（商の分母側と逆数冪和では零点でない評価点）。出力: 零点差の積、商、逆数冪和の有限係数表示。主定義・主定理・主張: 代数的評価点における零点差の逆三乗和の係数表示。
...
\begin{remark}[正の有理評価の順序の入出力]
入力: 正の有理数と Ising 分配多項式。出力: 非零性、零点差積、単調性、順序反映、全配位数との比較、一次多項式 x−1 の非整除。主定義・主定理・主張: 辺を持つ有限グラフでの正の有理評価の厳密単調性。
...
\begin{remark}[形式的高温展開の入出力]
入力: 有限グラフ、独立な不定元 u,v、スピン配位、数学的道具立ての偶部分グラフ多項式。出力: 整数係数二変数多項式としての有限和恒等式。主定義・主定理・主張: 形式的高温展開の有限和恒等式。
