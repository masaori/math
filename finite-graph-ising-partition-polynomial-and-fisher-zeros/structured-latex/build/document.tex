...
\begin{remark}[この節の入出力]
入力: 二つの辺端ラベルと有限な頂点集合・辺集合。出力: ループを持たず多重辺を許す有限グラフ。
...
\begin{remark}[この節の入出力]
入力: 有限グラフ、その辺部分集合、独立な不定元 u,v。出力: 各頂点で偶次数となる辺部分集合とその二変数多項式。
...
\begin{remark}[この節の入出力]
入力: 有限グラフと二つの形式的スピンラベル。出力: スピン配位と一頂点反転。
...
\begin{remark}[この節の入出力]
入力: 有限グラフ上のスピン配位と形式的不定元 x。出力: 破れ辺数の多重度、分配多項式、その係数表示と偶数性。
...
\begin{remark}[この節の入出力]
入力: Ising 分配多項式と頂点に接続する辺数の偶奇。出力: −1 における評価と一次因子による同値条件。
...
\begin{remark}[この節の入出力]
入力: 破れ辺数と頂点二分割。出力: 分配多項式の次数の組合せ論的意味。
...
\section{Fisher 零点の対称式と係数}

\begin{remark}[この節の入出力]
入力: 正の両端係数を持つ Ising 分配多項式。出力: 零点族と逆数族の基本対称式および Newton 漸化式。
...
\section{全辺を横切る二分割の特徴付け}

\begin{remark}[この節の入出力]
入力: 有限グラフと係数対称性または多項式逆数対称性。出力: 全辺を横切る頂点二分割の二つの特徴付け。
...
\section{全辺二分割と Fisher 零点の逆数対称性}

\begin{remark}[この節の入出力]
入力: 全辺を横切る頂点二分割と Fisher 零点。出力: 零点重複度の逆数対称性とその積・奇偶帰結。
...
\section{評価点における Fisher 零点差積}

\begin{remark}[この節の入出力]
入力: 一つまたは二つの代数的評価点と分配多項式。出力: 零点差積の係数比、二評価点での商、有理評価点での特殊化。
...
\section{評価点における Fisher 零点差の逆数冪和}

\begin{remark}[この節の入出力]
入力: Fisher 零点でない代数的評価点と分配多項式の係数。出力: 零点差の逆一・二・三乗和と係数表示。
...
\section{正の有理評価の単調性と順序反映}

\begin{remark}[この節の入出力]
入力: 正の有理数と辺を持つ有限グラフの Ising 分配多項式。出力: 厳密単調性、単射性、狭義・弱順序の反映。
...
\section{正の有理評価と全配位数の比較}

\begin{remark}[この節の入出力]
入力: 正の有理評価と全スピン配位数。出力: 等号、厳密下側、厳密上側の評価点による特徴付け。
...
\begin{remark}[この節の入出力]
入力: 有限グラフ、独立な不定元 u,v、スピン配位、偶部分グラフ多項式。出力: 整数係数二変数多項式としての有限和恒等式。
