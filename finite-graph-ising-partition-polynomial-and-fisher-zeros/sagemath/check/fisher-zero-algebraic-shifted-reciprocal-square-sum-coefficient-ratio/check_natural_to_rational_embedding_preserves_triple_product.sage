...
# 式ペア: eta(Omega) eta(m) eta(m-1) = eta(m(m-1)Omega)
...
for data in examples:
    for exponent in range(2, data["edge_count"] + 1):
        ...
        left = QQ(multiplicity) * QQ(exponent) * QQ(exponent - 1)
        right = QQ(NN(exponent) * NN(exponent - 1) * multiplicity)
        assert left == right, (data["name"], exponent)
print("RESULT: PASS — the natural-to-rational embedding preserves the three-factor product")
