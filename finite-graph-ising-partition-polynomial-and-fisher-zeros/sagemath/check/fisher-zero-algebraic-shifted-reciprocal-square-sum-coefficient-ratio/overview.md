...
| `check_apply_reciprocal_sum_coefficient_ratio.sage` | 逆数和の係数表示定理を明示的な係数有限和へ適用 | PASS | 全例・全対象評価点で一致 |
| `check_first_coefficient_sum_definition.sage` | 明示的な第一係数和を局所定義 `A_G(a)` へ置換 | PASS | 全例・全対象評価点で一致 |
