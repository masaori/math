...
def indexed_sum(roots, power_order, subset_order):
    return sum(
        (
            prod((roots[index] for index in selected), QQbar(1))
            * sum((roots[index]**(power_order - subset_order) for index in selected), QQbar(0))
            for selected in combinations(range(len(roots)), subset_order)
        ),
        QQbar(0),
    )
