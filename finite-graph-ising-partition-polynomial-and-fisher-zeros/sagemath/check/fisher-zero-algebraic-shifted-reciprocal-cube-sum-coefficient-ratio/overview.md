...
| `check_formal_derivative_product_rule_substitute_coefficient_expansion.sage` | 積の有限係数表示を形式微分の引数へ代入する一行 | PASS | 積の形式微分と有限係数展開の形式微分が一致 |
| `check_formal_derivative_product_rule_linearity_step.sage` | 有限係数和へ形式微分の線形性を適用する一行 | PASS | 有限係数和の形式微分と各有限和項を形式微分した和が一致 |
...
| `check_empty_product_convention.sage` | 代数的数係数多項式環と代数的数体における空積規約 | PASS | 各空積が所属する環・体の乗法単位元に一致 |
...
| `check_subtract_reciprocal_cube_from_pair_product.sage` | 逆数積の両辺から逆三乗和を引く操作 | PASS | `R_GS_{G,2}-S_{G,3}=U_{G,2,1}` が一致 |
...
| `check_final_coefficient_ratio_well_defined.sage` | 最終係数比の `QQbar` 内 well-defined 性 | PASS | 全ての非零評価で分母が非零であり、商が `QQbar` に所属 |
