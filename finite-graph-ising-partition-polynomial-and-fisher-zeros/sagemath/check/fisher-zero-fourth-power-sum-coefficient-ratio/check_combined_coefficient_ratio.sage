...
# 式ペア: common-denominator terms combine to the stated coefficient ratio
...
for data in examples:
    ...
    left = ((A1**4 - 3 * B * A1**2 * A2 + 3 * B**2 * A1 * A3) / B**4
            ...
            + B**2 * A1 * A3 / B**4
            - 4 * B**3 * A4 / B**4)
    right = (A1**4 - 4 * B * A1**2 * A2 + 2 * B**2 * A2**2
             + 4 * B**2 * A1 * A3 - 4 * B**3 * A4) / B**4
    assert left == right, data["name"]

print("RESULT: PASS — equal-denominator terms combine to the Fisher-zero fourth-power-sum coefficient ratio")
