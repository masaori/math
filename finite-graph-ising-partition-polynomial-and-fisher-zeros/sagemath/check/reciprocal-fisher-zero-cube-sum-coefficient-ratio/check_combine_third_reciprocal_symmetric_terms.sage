...
# 式ペア: e1hat^3 - 3e1hat e2hat + 9e3hat - 6e3hat = e1hat^3 - 3e1hat e2hat + 3e3hat
...
for data in examples:
    ...
    left = first**3 - 3 * first * second + 9 * third - 6 * third
    right = first**3 - 3 * first * second + 3 * third
    assert left == right, data["name"]

print("RESULT: PASS — the third reciprocal symmetric terms combine to three copies")
