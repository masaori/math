...
def partition_data(name, vertex_count, edges):
    ...
    assert polynomial[0] > 0, name
    assert all(alpha != 0 for alpha in roots), name
    return {"name": name, "degree": degree, "polynomial": polynomial, "roots": roots}


def reciprocal_elementary(roots, order):
    return sum(
        (
            prod((roots[index]**(-1) for index in selected), QQbar(1))
            for selected in combinations(range(len(roots)), order)
        ),
        QQbar(0),
    )


def reciprocal_cube_sum(roots):
    return sum((alpha**(-3) for alpha in roots), QQbar(0))


def reciprocal_repeated_pair_sum(roots):
    return sum(
        (
            roots[first]**(-2) * roots[second]**(-1)
