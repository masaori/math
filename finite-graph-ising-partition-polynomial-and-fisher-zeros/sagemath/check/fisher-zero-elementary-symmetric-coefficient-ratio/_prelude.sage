...
def selected_products(roots, cardinality, sign=ZZ(1)):
    return sum(
        (
            prod(sign * roots[index] for index in selected_indices)
            for selected_indices in combinations(range(len(roots)), cardinality)
        ),
        QQbar(0),
    )
