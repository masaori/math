...
runbook に厳密に従い、既存成果のレビュー後、実行待ちの最初の一件から構造化本文の一つの定義・主張・定理だけを前進させてください。必要な SageMath 検算、全検証、台帳と MEMORY の更新、コミット、origin の remote default branch への push、包含確認、Slack 通知まで行ってください。Slack には公開本文 https://hexcomp-artifacts.web.app/math/finite-graph-ising-partition-polynomial/ を付け、commit URL を人間へ本文を見せる URL として使わないでください。一 tick 一主張を超えて次へ進まないでください。'
