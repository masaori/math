# Fisher 零点と有理評価点との差の積

## 概要

一般有限グラフの重複度込み Fisher 零点について、任意の有理評価点との差の積を、分配多項式の
評価値と最高次係数の比で与える。
...
- `Q` から `Qbar` への標準単射を明記し、係数を `Qbar[x]` へ移した分配多項式の一次因子分解を標準単射で移した有理点で評価する。
...
- 一次因子分解を任意の `q in Q` の `Qbar` への標準単射像で評価する。
- 非零な最高次係数を消去し、零点差積が有理数に属することを得る。

### SageMath 検算

- 一次因子分解の有理点評価を検算する。
- 零点差積と評価値・最高次係数比の一致を検算する。
- 零、負、正の有理評価点における特殊化を検算する。

## 対象ファイル

- `structured-latex/content/main-text.ts`
- `sagemath/check/fisher-zero-rational-shifted-product-coefficient-ratio/`

## 完了条件

- [x] 一つの定理として、Fisher 零点と任意の有理評価点との差の積が係数比で証明されている。
- [x] ステートメントと証明が整合している。
