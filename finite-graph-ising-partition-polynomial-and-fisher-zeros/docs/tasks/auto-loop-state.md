...
| 作業 | 依存 | 状態 | 完了条件 |
| --- | --- | --- | --- |
| 一般有限グラフの Fisher 零点として零が現れないことの定理化 | `theorem_fisher_zero_product_coefficient_ratio` と定数項の正値性 | ready | 一般有限グラフについて零が Fisher 零点でないことを一つの定理として記述し、一行ごとの SageMath 検算を対応させる |
