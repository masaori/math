...
`docs/context/` はリポジトリ全体の**研究哲学・語彙・判断基準の正本**であり、
個別プロジェクトの README や作業手順より**上位**にある（矛盾したら `docs/context/` が勝つ）。

- [docs/context/README.md](docs/context/README.md) — 読む順序
- [00_基本方針_可算コアとℝ脱出.md](docs/context/00_基本方針_可算コアとℝ脱出.md) —
  **理論のコアは可算（ℕ⊂ℚ⊂Λ⊂ℚ̄）で記述し、ℝ/ℂ への移行は「応用のための便宜」として扱う。**
  使ってよい／使ってはいけない操作、実務ルール、落とし穴。
- [01_対数順序群Λ_定義と最小限の道具.md](docs/context/01_対数順序群Λ_定義と最小限の道具.md) —
  Λ の定義・順序・決定可能性の梯子・実現写像 ρ。
- [02_ℝ脱出の台帳と判定手順.md](docs/context/02_ℝ脱出の台帳と判定手順.md) —
  脱出の型 E0–E4、命題ごとに回す判定手順、本文への記録書式、既知の脱出台帳。
- [03_物理数学を可算側で特徴づける課題表.md](docs/context/03_物理数学を可算側で特徴づける課題表.md) —
  Hilbert 空間・作用素代数・経路積分・臨界指数などを可算側で言い直す未完課題（逆方向の宿題）。
- [04_証明の記述スタイルと四層の検証.md](docs/context/04_証明の記述スタイルと四層の検証.md) —
  **証明の記述スタイルの正本**。1ステップ1定理・省略禁止・各行に適用定理を明示・
  インデントによる階層可視化と、四層の検証（記述／SageMath による式変形検証／
  Lean 具体版＝人手証明の正当性保証／Lean 必要十分版＝必要十分な抽象度による再記述）。
...
> 前提として、**どのプロジェクトでも着手前に [docs/context/](docs/context/) を全て読む**
> （このファイル冒頭の「最優先」節）。以下はその上に載るプロジェクト固有のゴール設定である。
...
- 要点: **2値セルオートマトンの規則（舞台グラフ・近傍・時間発展）と、統計力学／場の量子論の
  ...
  種を命題へ昇格させるときは `docs/context/` の脱出台帳に従って帰属（ℕ/ℚ/Λ/ℚ̄/ℝ脱出）を明示する。
