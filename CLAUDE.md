...
- **作業ブランチを指定されている場合も同じ。** セッション側の指示（「ブランチ `X` で開発せよ」「指示がない限り PR を作るな」等）は**作業をどこで行うかの指定**であって、main へ反映しないことの指示ではない。指定ブランチへ push したうえで、そこから main へマージするところまでを完了とする。指定ブランチ以外への push を明示的に禁じられている場合は、PR を作成してマージすることで main へ反映する（この PR 作成は「指示がなければ作らない PR」には当たらない。完了に必要な手続きである）。
